...
then $A=\Op(a)$ is called a \emph{classical pseudodifferential operator of order $m$}.\footnote{Or we can characterise the order using the growth condition: there exists constant $C_{\alpha,\beta}$ such that $\abs{\partial_x^\beta\partial_\xi^\alpha p(x,\xi)}\leqslant C_{\alpha,\beta}(1+\abs{\xi})^{m-\abs{\alpha}}$ for all multi-index $\alpha$ and $\beta$. The order of $A$ is the supremum of such $m$.}
...
\begin{proof}Consider $X\overset{i}{\mono}\R^n\overset{j}{\leftarrowtail}\pt$, where $X\mono\R^n$ is a proper embedding. It induces
\[ \begin{tikzcd}
...
\begin{proof}1 is just Bott periodicity; 2 is by functoriality.
\end{proof}
