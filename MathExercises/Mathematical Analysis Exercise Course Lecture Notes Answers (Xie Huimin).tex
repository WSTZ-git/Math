...
否则对 \(\forall i\in \left\{ 1,2\cdots,m \right\}\),有 \(x_i - x_{i - 1}>\frac{y_0 - x_0}{m}\).从而 \(y_0 - x_0=\sum_{i = 1}^m{\left( x_i - x_{i - 1} \right)}>\sum_{i = 1}^m{\frac{y_0 - x_0}{m}} = y_0 - x_0\)矛盾.再结合$m\geqslant k$可知,\(x_{i_0}-x_{i_0 - 1}\leqslant  \frac{y_0 - x_0}{m}\leqslant  \frac{y_0 - x_0}{k}\).
...
\end{remark}
\begin{proof}
\label{proof:Lagrange中值定理的推广(二阶导数)-证明}
...
f(b)=f\left( \frac{a+b}{2} \right) +f^{\prime}\left( \frac{a+b}{2} \right) \left( \frac{b-a}{2} \right) +\frac{1}{2}f^{\prime\prime}(\eta _2)\left( \frac{b-a}{2} \right) ^2,\eta _2\in \left( \frac{a+b}{2},b \right) 
        ...
        \begin{equation}\label{eq:2..2}
            ...
            % \nonumber
        \end{equation}
        又$\min \left\{ f^{\prime\prime}\left( \eta _1 \right) ,f^{\prime\prime}\left( \eta _2 \right) \right\} \leqslant \frac{1}{2}\left[ f^{\prime\prime}\left( \eta _1 \right) +f^{\prime\prime}\left( \eta _2 \right) \right] \leqslant \max \left\{ f^{\prime\prime}\left( \eta _1 \right) ,f^{\prime\prime}\left( \eta _2 \right) \right\}$,
...
(4)若$2a^2\leqslant 5b$,证明:方程$x^5+ax^4+bx^3+cx^2+\mathrm{d}x+e=0$不可能有5个不同的实根.
...
\begin{proof}
    ...
    从而$\Delta=(24a)^2-4\times 60\times 6b>0$,这与$2a^2\leqslant 5b$矛盾.
    故原方程不可能有5个不同的实根.
...
7. 设\(f(x)\)在\([0,+\infty)\)上可微,且\(0\leqslant f(x)\leqslant\frac{x}{1 + x^2}\).证明:存在\(\xi>0\),使得\(f'(\xi)=\frac{1-\xi^{2}}{(1+\xi^{2})^{2}}\).
    ...
    \begin{proof}
        ...
        \begin{gather}
            0\leqslant f(0)\leqslant 0,
            \nonumber
            \\
            0\leqslant f(+\infty)\leqslant \lim_{x \to +\infty} \frac{x}{1+x^2} 
            \nonumber
        \end{gather}
...
\end{equation}
    ...
    由于
    \begin{equation}
    \sqrt{x(x+1)}-x=\frac{x}{\sqrt{x(x+1)}+x}\leqslant \frac{x}{\sqrt{x^2}+x}=\frac{1}{2}
    \nonumber
    \end{equation}
...
\begin{exercise}
    ...
    \begin{proof}
        ...
        若\(f(a)\)是\(f\)的最大值,
        则$\forall x\in[a,b]$,有$f(x) \geqslant f(a)$.从而
        \begin{equation}
...
\begin{proof}
    ...
    可得绝对误差为
    \begin{equation}
        \mathbb{(}1)|R\left( x \right) |==\frac{\vert f^{(n + 1)}(\xi)\vert}{(n + 1)!}\vert x\vert^{n + 1}
        \nonumber
    \end{equation}

        (1)$|R\left( x \right) |=\left| \frac{e^{\xi}}{\left( n+1 \right) !}x^{n+1} \right|\leqslant \frac{e}{\left( n+1 \right) !};$
        
        (2)$|R\left( x \right) |=\left| \frac{\sin \xi}{4!}x^4 \right|\leqslant \frac{1}{4!\cdot 2^4}=\frac{1}{384};$

        (3)$|R\left( x \right) |=\left| \frac{\tan ^{\left( 4 \right)}x}{4!}x^4 \right|\leqslant \left| \frac{\tan ^{\left( 4 \right)}\left( 0.1 \right)}{4!}0.1^4 \right|\approx 0.004286 \left( \left( \tan x \right) ^{\left( 4 \right)}\text{在}\left( -0.1,0.1 \right) \text{上单调递增} \right) ;$
        
        (4)$|R\left( x \right) |\leqslant \frac{\left| f^{\left( 3 \right)}\left( 1 \right) \right|}{3!}\approx 0.011.$
...
\begin{exercise}
    ...
    \begin{proof}
        ...
        若$C\geqslant 1$,则对$\forall x\in(-\frac{1}{2C},\frac{1}{2C})$,$\forall k\in \mathbb{N}$,
        ...
        设$a=inf\,\,A,b=sup\,\,B$,则根据已证结论可知,$-1\leqslant a<b\leqslant 1$.
...
\begin{exercise}
    ...
    \begin{equation}
        \left| f^{\prime\prime}(\xi ) \right|\geqslant \frac{4}{(b-a)^2}\left| f(b)-f(a) \right|\
        \nonumber
    \end{equation}
    \begin{note}
...
设\(f\)在\([a,+\infty)\)上二阶可微,且\(f(x)\geqslant 0\),\(f^{\prime\prime}(x)\leqslant 0\).证明:在\(x\geqslant  a\)时\(f^{\prime}(x)\geqslant 0\).    
    \begin{proof}
        ...
        \begin{gather}
            ...
            \nonumber
        \end{gather}
        这与$\forall x\in[a,+\infty)$,有$f(x)\geqslant 0$矛盾.
