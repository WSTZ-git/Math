...
\end{align*}
若 $\mathrm{diam}(A) < \infty$, 则称 $A$ 为有界集. 容易验证: $A$ 有界, 当且仅当存在 $M \geqslant 0$ 使得 $d(\mathbb{0}, x) \leqslant M$, $\forall x \in A$.
...
\end{align*}
注意到 $|x_i^{(k)}| \leqslant \left(\sum_{i = 1}^{n}|x_i^{(k)}|^2\right)^{1/2} = d(x_k, \mathbb{0})$, 则 $\{x_1^{(k)}\}$ 是 $\mathbb{R}$ 中的有界数列, 由 $\mathbb{R}$ 上的致密性定理, 存在 $\{x_1^{(k)}\}$ 的子列 $\{x_1^{(k_i)}\}$ 满足 $x_1^{(k_i)} \to x_1$; 同理, 实数列 $\{x_2^{(k_i)}\}$ 进而 $\{x_2^{(k_{i_j})}\}$ 有界, 由 $\mathbb{R}$ 上的致密性定理, 存在 $\{k_{i_j}\}$ 的子列 $\{k_{i_{j_l}}\}$ 使得 $x_2^{(k_{i_{j_l}})} \to x_2$; 以此类推 $\cdots\cdots$; 存在 $\{k_{n - 1}\}$ 的子列 $\{k_n\}$ 使得 $x_n^{(k_n)} \to x_n$. 令 $x = (x_1, x_2, \cdots, x_n)$, 则
\begin{align*}
...
\begin{align*}
d(\mathbb{0}, y_n) \leqslant d(\mathbb{0}, x) + d(x, y_n) \leqslant d(\mathbb{0}, x) + M
\end{align*}
...
\begin{align*}
d(\mathbb{0}, y_n) \leqslant d(\mathbb{0}, x_n) + d(x_n, y_n) \leqslant d(\mathbb{0}, x_n) + M
\end{align*}
