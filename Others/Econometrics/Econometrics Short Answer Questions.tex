...
\item 外生性假定：解释变量与误差项不相关，即 $\text{Cov}(X_i,u_i)=0$；
\item 正态性假定：误差项服从正态分布，即 $u_i \sim N(0,\sigma^2)$；
\item 解释变量之间不存在完全多重共线性，即解释变量之间不存在线性关系。
\end{enumerate}
...
\item 提出假设:
$$
\text{原假设}H_0: \beta_1=\beta_2=\cdots=\beta_k=0, \quad \text{备择假设}H_1: \text{至少有一个 } \beta_i \neq 0,
$$
...
\begin{enumerate}
\item 设原假设 $H_0: \rho = 0$（随机误差项无一阶自相关），备择假设 $H_1: \rho \neq 0$；
...
\item 当DW值在区间\([d_L,d_U]\)与\([4-d_U,4-d_L]\)中时,无法判定自相关性；
...
\section{多重共线性的影响后果}
\begin{enumerate}
\item OLS估计量仍有无偏性、一致性，方差大幅膨胀，估计精度下降；
\item 假设检验失效，难以区分单个解释变量对被解释变量的独立作用;
\item 预测区间变宽，参数估计值不稳定，样本微小变动会造成系数大幅波动。
\end{enumerate}

\section{完全多重共线性对OLS估计量的影响有哪些？}
\begin{enumerate}
\item 矩阵$X'X$不可逆，OLS的参数估计量$\hat{\beta} = (X'X)^{-1}X'Y$不存在；
...
\section{计量经济建模过程中需要对残差进行的检验包含哪些内容？}
\begin{enumerate}
\item 残差的正态性；
\item 残差的平稳性；
\item 残差的同方差；
...
\section{对生产函数的资本和劳动弹性系数之和等于1的假设进行检验的主要步骤？}
该检验为单一线性约束的F检验，主要步骤如下：
\begin{enumerate}
\item 提出假设：原假设 $H_0: \alpha+\beta=1$，备择假设 $H_1: \alpha+\beta\neq1$，其中 $\alpha、\beta$ 分别为资本和劳动的产出弹性系数；
\item 分别用最小二乘法估计施加约束的回归模型与无约束的原回归模型，得到约束残差平方和 $\text{RSS}_\text{R}$ 与无约束残差平方和 $\text{RSS}_\text{U}$；
\item 构造F检验统计量：
$$
F = \frac{(\text{RSS}_\text{R}-\text{RSS}_\text{U})/q}{\text{RSS}_\text{U}/(n-k-1)}
$$
其中约束个数为$q$，$k$ 为无约束模型的解释变量个数，$n$ 为样本容量；当原假设成立时，统计量服从 $F(q,n-k-1)$ 分布；
\item 给定显著性水平$\alpha$，确定对应F分布的临界值$F_{\alpha}(q,n-k-1)$，或计算统计量对应的伴随概率；
\item 得出检验结论：若 $F > F_\alpha(q,n-k-1)$（或伴随概率小于显著性水平），则拒绝原假设，认为弹性系数之和显著不等于1；反之则不拒绝原假设。
\end{enumerate}
