...
\item 分别估计施加约束的回归模型与无约束的原回归模型，得到约束残差平方和 $\text{RSS}_\text{R}$ 与无约束残差平方和 $\text{RSS}_\text{U}$；
\item 构造F检验统计量：
$$
F = \frac{(\text{RSS}_\text{R}-\text{RSS}_\text{U})/1}{\text{RSS}_\text{U}/(n-k-1)}
$$
其中约束个数为1，$k$ 为无约束模型的解释变量个数，$n$ 为样本容量；当原假设成立时，统计量服从 $F(1,\,n-k-1)$ 分布；
\item 给定显著性水平（如 0.05），确定对应F分布的临界值，或计算统计量对应的伴随概率；
\item 得出检验结论：若 $F > F_\alpha(1,n-k-1)$（或伴随概率小于显著性水平），则拒绝原假设，认为弹性系数之和显著不等于1；反之则不拒绝原假设。
\end{enumerate}
