...
\begin{theorem}[无最大基数定理]\label{theorem:无最大基数定理3}
...
\begin{theorem}[Cantor闭集套定理]\label{theorem:实变-Cantor闭集套定理}
...
(ii) 一切\(L_k\)皆非空集，则由\hyperref[theorem:实变-Cantor闭集套定理]{Cantor闭集套定理}可知，存在点\(x_0\in L_k(k = 1,2,\cdots)\)，即\(x_0\in F\)且\(x_0\in H_k^c(k = 1,2,\cdots)\)。这就是说\(F\)中存在点\(x_0\)不属于一切\(H_k\)，与原设矛盾，故第(ii)种情况不存在。
