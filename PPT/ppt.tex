...
对于任意给定的$q\in \sQ$,映射$$-\ldd q\colon\sQ\longrightarrow \sQ,\quad r\ldd q=\bigvee\{p\in \sQ\ver p\with q\leqslant r\}$$是$$-\with q\colon\sQ\longrightarrow \sQ$$的一个右伴,称为$\with$的左推出; 映射$$p\rdd -\colon\sQ\longrightarrow \sQ,\quad p\rdd r=\bigvee\{p\in \sQ\ver p\with q\leqslant r\}$$是$$p\with-\colon\sQ\longrightarrow \sQ$$的一个右伴,称为$\with$的右推出. 
\end{frame}	

\begin{frame}
左右推出满足对任意的$p,q,r\in \sQ$,有$$p\leqslant r\ldd q\Longleftrightarrow p\with q\leqslant r\Longleftrightarrow q\leqslant p\rdd r.$$进一步,若quantale $\sQ$是交换的(即对任意的$p,q\in \sQ$,有$p\with q=q\with p$),则有
...
\item 任给$r\in\sQ$ 和 $x\in X$,有 $\alpha(x)\ldd r\leqslant \alpha(r\otimes x)$.\end{enumerate}
...
\item \label{FF1} $F(k_X)\geqslant k$, 其中$k_X$ 是取值$k$的常值映射$X\lra\sQ$; \item \label{FF2} $F(\lambda)\wedge F(\gamma)\leqslant F(\lambda\wedge\gamma)$; \item \label{FF3} $\gamma\swarrow\lambda  \leqslant F(\gamma)\ldd F(\lambda);$
\item\label{FF4}   $F(r_X)\leqslant r$, 其中$r_X$是取值$r$的常值映射$X\lra\sQ$.   \end{enumerate}
\end{defn}
