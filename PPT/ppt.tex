...
$$ m_{(X,\alpha)}\colon (\sQ^{\sQ^X},\alpha^{\downarrow\downarrow})\lra (\sQ^X,\alpha^\downarrow),\quad  m_{(X,\alpha)}(\Lambda)= \bv_{\gamma\in\sQ^X}\Lambda(\gamma)\with\gamma $$  和 \[ e_{(X,\alpha)}\colon (X,\alpha)\lra (\sQ^X,\alpha^\downarrow),\quad  e_{(X,\alpha)}(x) =k_x \] 都满足Goguen 条件, 由此可证$(\mathscr{U}, m, e)$是$\setQ$上的一个monad, 称为$\setQ$上的协变幂集monad. 
\end{frame}
