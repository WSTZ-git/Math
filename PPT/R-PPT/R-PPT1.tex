...
设独立观测 $(Y_i, X_{i1},\dots,X_{ip})$，模型形式为：
...
\begin{enumerate}
\item 响应变量 $Y_1,\dots,Y_n$ 独立且服从\textbf{指数族分布}

\item 存在严格单调可微函数 $g(\cdot)$，使得
\[ \boldsymbol{g(\mathbb{E}(Y)) = \eta = \boldsymbol{X\beta}}==\beta_0+\beta_1X_1+\dots+\beta_pX_p \]
\end{enumerate}
...
\item 模型公式：$\ln\left(\frac{\pi}{1-\pi}\right)=\beta_0+\beta_1X_1+\dots+\beta_pX_p$
\end{itemize}
\end{block}
\begin{block}{逆变换（预测）}
...
\item 连接函数：对数典则连接
\[ g(\lambda)=\ln(\lambda) \]
\item 模型公式：$\ln(\lambda)=\beta_0+\beta_1X_1+\dots+\beta_pX_p$
\end{itemize}
\end{block}
\begin{block}{逆变换}
