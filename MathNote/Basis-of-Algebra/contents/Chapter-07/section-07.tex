...
\end{matrix} \right) 
\\
&\xlongequal{\text{\hyperref[lemma:反对角矩阵的性质]{反对角矩阵的性质}}}\left( \begin{matrix}
...
显然 \(A\) 的极小多项式为 \(\lambda^n\).又由$A^n=O$知$A$的特征值$\lambda^n=0$,故$A$的特征值全为0,因此特征多项式也是$x^n=0$.从而 \(A\) 的不变因子组是 \(1,\cdots,1,\lambda^n\)。同理 \(B\) 的不变因子组也是 \(1,\cdots,1,\lambda^n\)，因此 \(A\) 和 \(B\) 相似。
...
\((1)\Rightarrow(2)\)：显然$A$的$n-1$阶子阵要么是$O$,要么是$(\lambda-c)^{n-1}$.
...
\begin{proposition}
设 \(A = \mathrm{diag}\{A_1,A_2,\cdots,A_k\}\) 为分块对角矩阵，求证：\(A\) 的初等因子组等于 \(A_i(1\leqslant  i\leqslant  k)\) 的初等因子组的并集。又若交换各块的位置，则所得的矩阵仍和 \(A\) 相似。
\end{proposition}
\begin{proof}

显然 \(\lambda I - A\) 也是一个分块对角矩阵，用 \(\lambda\)-矩阵的初等变换将每一块化为法式，则由\hyperref[theorem:lambda-矩阵和初等因子的基本性质]{$\lambda$-矩阵和初等因子的基本性质(2)}可知，\(A\) 的初等因子组就是所有各块的初等因子组的并集。又交换 \(A\) 的各块并不改变 \(A\) 的初等因子组，因此所得之矩阵仍和 \(A\) 相似。
