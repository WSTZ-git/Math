...
于是
\begin{align*}
\left| \frac{f(z_0 + h)}{f(z_0)} \right| &\leqslant  |1 - e^k| + |e^{k+1} (c_{k+1} d^{k+1} + c_{k+2} d^{k+2} e + \cdots)| \\
&\leqslant  1 - e^k + e^{k+1} (|c_{k+1} d^{k+1}| + |c_{k+2} d^{k+2}| + \cdots) \\
&< 1 - e^k + \frac{1}{2} e^k \\
&= 1 - \frac{1}{2} e^k < 1。
\end{align*}
将这样的 $e$ 代入 $h = ed$，得
...
\\
&\sum_{1\leqslant i<j\leqslant n}{x_ix_j}=x_1x_2+\cdots +x_1x_n+x_2x_3+\cdots +x_{n-1}x_n=\frac{a_2}{a_0},
\\
...
\begin{example}
设多项式 $x^3 + px^2 + qx + r$ 的 3 个根都是实数，求证：$p^2 \geqslant  3q$。
\end{example}
\begin{proof}

设多项式的 3 个根为 $x_1, x_2, x_3$，由已知条件可知：
\begin{align*}
(x_1 - x_2)^2 + (x_2 - x_3)^2 + (x_1 - x_3)^2 \geqslant  0。
\end{align*}
用 Vieta 定理可计算出
\begin{align*}
