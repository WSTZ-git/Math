...
\begin{example}
设\(A \in \mathbb{F}^{n\times n}\)，\(\mathbb{F}\)是数域。证明：若\(\det A = 0\)，则只用初等行变换可以把\(A\)的某一行变成\(0\)。
\end{example}
\begin{proof}
...
\begin{example}\label{可逆阵都能只用第三类初等变换化为对角阵}
...
\end{pmatrix}.
\]
从而原结论对二阶对角阵成立.对于$n$阶对角阵$B=\mathrm{diag}\{a_1,a_2,\cdots,a_n\}$而言,按照上述方法对$B\left( \begin{matrix}
1&		2\\
1&		2\\
...
\]
第三类初等阵已经是这种形状了,即$P_{ij}\left( c \right) =I_n+cE_{ij}$.对第二类初等阵\(P_i(c)\),显然我们有\(P_i(c) = I_n + (c - 1)E_{ii}\).对第一类初等阵\(P_{ij}\),由\hyperref[可逆阵都能只用第三类初等变换化为对角阵]{例题\ref{可逆阵都能只用第三类初等变换化为对角阵}}可知,只用第三类初等变换就可以将\(P_{ij}\)化为\(P_n(-1) = \mathrm{diag}\{1,\cdots,1,-1\}\),因此对第一类初等阵结论也成立.具体地,我们有
\begin{align*}
