...
\begin{definition}
设$D\subset E^n$为$n$维欧氏空间区域，$(x^1,\dots,x^n)$为$D$上的直角坐标系；$U\subset E^n$为另一区域，$(u^1,\dots,u^n)$为$U$上直角坐标系。若存在$C^\infty$微分同胚$f:U\to D,\ g:D\to U$，满足
\begin{align*}
x^\alpha=f^\alpha(u^1,\dots,u^n),\quad u^\alpha=g^\alpha(x^1,\dots,x^n),\quad 1\leqslant\alpha\leqslant n,
\end{align*}
则称$(u^1,\dots,u^n)$为$D$上的\textbf{曲纹坐标系}。
区域$D$在点$p$的切空间自然基为$\left\{\frac{\partial}{\partial u^1},\dots,\frac{\partial}{\partial u^n}\right\}$，余切空间基为$\{\mathrm{d}u^1,\dots,\mathrm{d}u^n\}$。
\end{definition}

\begin{proposition}
$n$个$C^\infty$函数$x^\alpha=f^\alpha(u^1,\dots,u^n)\ (1\leqslant\alpha\leqslant n)$能在点$(x_0^1,\dots,x_0^n)$邻域内引入曲纹坐标系的充要条件为其Jacobi行列式满足
\begin{align}
...
\textbf{充分性}: 由反函数定理，Jacobi行列式非零则局部存在$C^\infty$逆映射$u^\alpha=g^\alpha(x^1,\dots,x^n)$，满足复合恒等式，故可引入曲纹坐标系.

\end{proof}

\begin{definition}
$n$维\textbf{微分流形}是由$n$维欧氏空间的局部区域通过曲纹坐标的$C^\infty$相容变换拼接而成的拓扑空间；其每一点可定义切空间、余切空间，$\{\mathrm{d}u^1,\dots,\mathrm{d}u^n\}$为余切空间的局部基。
\end{definition}

\begin{definition}
设$D\subset E^n$为区域，$(u^1,\dots,u^n)$为$D$上曲纹坐标系。若在每点$p\in D$指定一个$r$次外形式
\begin{align}
\varphi(p)&=\frac{1}{r!}\varphi_{i_1\cdots i_r}(u^1,\dots,u^n)\mathrm{d}u^{i_1}\wedge\cdots\wedge\mathrm{d}u^{i_r}\nonumber\\
&=\sum\limits_{1\leqslant i_1<\cdots<i_r\leqslant n}\varphi_{i_1\cdots i_r}(u^1,\dots,u^n)\mathrm{d}u^{i_1}\wedge\cdots\wedge\mathrm{d}u^{i_r},
\label{eq:r_differential_form}
\end{align}
其中系数$\varphi_{i_1\cdots i_r}$关于指标$i_1,\dots,i_r$反对称且为$C^\infty$函数，则称$\varphi$为$D$上的\textbf{$r$次外微分式}。
\end{definition}
...
\begin{theorem}
设$\varphi$是定义在$n$维区域$D$上的$r$次外微分式，在曲纹坐标系$(u^1,\dots,u^n)$下表示为
\begin{align*}
\varphi = \frac{1}{r!}\varphi_{i_1\cdots i_r}(u^1,\dots,u^n)\mathrm{d}u^{i_1}\land\cdots\land\mathrm{d}u^{i_r},
\end{align*}
在另一曲纹坐标系$(\tilde{u}^1,\dots,\tilde{u}^n)$下表示为
\begin{align*}
\varphi = \frac{1}{r!}\tilde{\varphi}_{i_1\cdots i_r}(\tilde{u}^1,\dots,\tilde{u}^n)\mathrm{d}\tilde{u}^{i_1}\land\cdots\land\mathrm{d}\tilde{u}^{i_r},
\end{align*}
...
设两坐标系间的容许坐标变换为$\tilde{u}^i=\tilde{u}^i(u^1,\dots,u^n)\ (1\leqslant i\leqslant n)$，则
\begin{align}
...
\begin{align}
\tilde{u}^\alpha = \tilde{u}^\alpha(u^1,\dots,u^n),\quad \alpha=1,\dots,m. \label{eq:diff-map}
\end{align}
对$\tilde{D}$上的$r$次外微分式
\begin{align*}
\tilde{\varphi} = \frac{1}{r!}\sum\limits_{1\leqslant\alpha_1,\dots,\alpha_r\leqslant m}\tilde{\varphi}_{\alpha_1\cdots\alpha_r}(\tilde{u}^1,\dots,\tilde{u}^m)\mathrm{d}\tilde{u}^{\alpha_1}\land\cdots\land\mathrm{d}\tilde{u}^{\alpha_r},
\end{align*}
定义$\tilde{\varphi}$通过$\sigma$在$D$上的\textbf{拉回}$\sigma^*\tilde{\varphi}$为将\eqref{eq:diff-map}代入$\tilde{\varphi}$得到的$D$上的$r$次外微分式：
\begin{align}
\sigma^*\tilde{\varphi} &= \frac{1}{r!}\tilde{\varphi}_{\alpha_1\cdots\alpha_r}\big(\tilde{u}^1(u^1,\dots,u^n),\dots,\tilde{u}^m(u^1,\dots,u^n)\big)\frac{\partial\tilde{u}^{\alpha_1}}{\partial u^{i_1}}\cdots\frac{\partial\tilde{u}^{\alpha_r}}{\partial u^{i_r}}\mathrm{d}u^{i_1}\land\cdots\land\mathrm{d}u^{i_r}, \label{eq:pullback-def}
\end{align}
式中采用Einstein和式约定。
\end{definition}
