...
\begin{definition}
我们称方程\eqref{eq:poisson}为\textbf{位势方程}或$\mathbf{Poisson}$\textbf{方程}.
\begin{align}
-\Delta u = f(x), \label{eq:poisson}
\end{align}
其中 $u=u(x)$，$x=(x_1,x_2,\dots,x_n)\in\Omega\subset\mathbb{R}^n$，$f(x)$ 是一个已知函数。当非齐次项 $f\equiv 0$ 时，Poisson 方程 \eqref{eq:poisson} 简化为$\mathbf{Laplace}$\textbf{方程}
\begin{align}
