...
\begin{theorem}[Harnack 定理]\label{theorem:Harnack 定理}
...
\begin{align*}
v(x) = \begin{cases}
u(x_1,x_2,\dots,x_n), & x_n\geqslant 0, \\
-u(x_1,x_2,\dots,-x_n), & x_n<0.
\end{cases}
\end{align*}
...
\begin{enumerate}[(1)]
\item 利用 Poisson 公式 \eqref{eq:poisson-ball} 证明：
\begin{align*}
R^{n-2}\frac{R-|x|}{(R+|x|)^{n-1}}u(0) \leqslant u(x) \leqslant R^{n-2}\frac{R+|x|}{(R-|x|)^{n-1}}u(0);
\end{align*}
\item 证明定理 \eqref{eq:harnack}。
\end{enumerate}
\end{example}

\begin{example}
利用上述不等式证明定理 \eqref{eq:liouville}。
\end{example}

\begin{example}
证明：对于任意 $x=(x_1,x_2,\dots,x_n)\in\mathbb{R}_+^n$，Poisson 核满足
\begin{align*}
\int_{\mathbb{R}^{n-1}} K(x,y)\,\mathrm{d}y = 1,
\end{align*}
其中
\begin{align*}
y=(y_1,y_2,\dots,y_{n-1},0)\in\partial\mathbb{R}_+^n=\mathbb{R}^{n-1},\quad \mathrm{d}y=\mathrm{d}y_1\mathrm{d}y_2\cdots\mathrm{d}y_{n-1},\quad K(x,y)=\frac{2x_n}{n\alpha(n)|y-x|^n}.
\end{align*}
\end{example}
...
\begin{example}
记 $B^+(R)=\{x=(x_1,x_2,\dots,x_n)\in\mathbb{R}^n \mid x_n>0,\ |x|<R\}$ $(n\geqslant 2)$。求边值问题
\begin{align*}
\begin{cases}
-\Delta u = f, & x\in B^+(R), \\
