...
\begin{align*}
\mathcal A\triangleq\left\{u\in C^2(\Omega)\cap C(\overline{\Omega}): u|_{\partial\Omega}=g\right\}.
\end{align*}
对任意 $u\in\mathcal A$，对应的曲面 $z=u(x,y)$ 的面积为
\begin{align*}
A(u)\triangleq\iint_\Omega \sqrt{1+u_x^2+u_y^2}\,dxdy.
\end{align*}
若存在 $u\in\mathcal A$ 使得 $A(u)=\min\limits_{v\in\mathcal A}A(v)$，则称曲面 $z=u(x,y)$ 为（图形式下的）\textbf{极小曲面}.

设$u\in\mathcal A$满足$A(u)=\min\limits_{v\in\mathcal A}A(v)$，则极小值点$u$必满足方程
\begin{align}
\frac{\partial}{\partial x}\left(\frac{u_x}{\sqrt{1+|\nabla u|^2}}\right)
+\frac{\partial}{\partial y}\left(\frac{u_y}{\sqrt{1+|\nabla u|^2}}\right)=0, \label{eq:min-surf-var}
\end{align}
其中 $|\nabla u|^2=u_x^2+u_y^2$. 等价地，展开后得到极小曲面方程
\begin{align}
(1+u_y^2)u_{xx} - 2u_xu_y u_{xy} + (1+u_x^2)u_{yy}=0. \label{eq:min-surf-expand}
\end{align}
上述方程称为\textbf{极小曲面方程}.
\end{proposition}
\begin{remark}
几何上，\eqref{eq:min-surf-var} 或 \eqref{eq:min-surf-expand} 等价于曲面的平均曲率为零.
\end{remark}
\begin{remark}
上述推导与推导 Laplace 方程（能量泛函 $\int|\nabla u|^2$ 的 Euler--Lagrange 方程为 $\Delta u=0$）的思想完全一致，区别仅在于被积函数从 $|\nabla u|^2$ 换成了面积元素 $\sqrt{1+|\nabla u|^2}$，因此得到的方程是非线性的，即为极小曲面方程.
\end{remark}
\begin{proof}
...
\begin{align*}
0=\frac{\mathrm{d}}{\mathrm{d}\varepsilon}\bigg|_{\varepsilon=0}A(u+\varepsilon v)
=\iint_\Omega \frac{u_x v_x+u_y v_y}{\sqrt{1+|\nabla u|^2}}\,\mathrm{d}x\mathrm{d}y.
\end{align*}
利用\hyperref[Basis of Analytics-theorem:Gauss-Green公式(散度定理)]{Gauss-Green公式}，并注意到 $v$ 在边界 $\partial\Omega$ 上为零，可得
\begin{align*}
0=-\iint_\Omega v\left[
\frac{\partial}{\partial x}\left(\frac{u_x}{\sqrt{1+|\nabla u|^2}}\right)
+\frac{\partial}{\partial y}\left(\frac{u_y}{\sqrt{1+|\nabla u|^2}}\right)
\right]\mathrm{d}x\mathrm{d}y.
\end{align*}
由 $v$ 的任意性，被积函数必须恒为零，故得 \eqref{eq:min-surf-var}. 将 \eqref{eq:min-surf-var} 展开即得 \eqref{eq:min-surf-expand}.
\end{proof}
