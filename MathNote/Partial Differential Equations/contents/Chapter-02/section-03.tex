...
在下面的讨论中 $\Omega$ 是 $\mathbb{R}^n$ 上的有界开集。

\begin{theorem}
假设 $c(x)\geqslant 0$，$f(x)<0$。如果 $u\in C^2(\Omega)\cap C(\overline{\Omega})$ 满足方程 \eqref{eq:general-elliptic}
\begin{align}
\mathcal{L}u = -\Delta u + c(x)u = f(x), \quad x\in\Omega,\label{eq:general-elliptic}
\end{align}
...
\end{align*}
则由多元微积分的定理知，$u(x)$ 在 $x_0$ 的梯度向量 $Du(x_0)=0$ 和 Hessian 矩阵 $D^2u(x_0)$ 是非正定的。对 Hessian 矩阵 $D^2u(x_0)$ 求迹得到
\begin{align*}
...
\begin{theorem}
假设 $c(x)\geqslant 0$，$f(x)\leqslant 0$。如果 $u\in C^2(\Omega)\cap C(\overline{\Omega})$ 满足方程
\begin{align*}
...
\end{theorem}
\begin{remark}
在定理 \eqref{eq:general-elliptic} 中，如果 $u(x)$ 在 $\overline{\Omega}$ 上的最大值是负数，上述定理并没有告诉我们任何结论。
\end{remark}
\begin{proof}
...
\end{align*}
由定理 \eqref{eq:general-elliptic} 知，$w$ 的非负最大值只能在 $\partial\Omega$ 上达到，因此
\begin{align*}
...
\begin{theorem}[Hopf 引理]\label{theorem:Hopf引理}
...
\end{align*}
为了使 $w(x)$ 在球壳 $B_R^*$ 的边界上达到非负最大值，由极值原理我们只需要在球壳 $B_R^*$ 上提条件 $\mathcal{L}w\leqslant 0$。因此，我们只需要构造出函数 $v$，使它满足条件
\begin{align*}
...
\end{align*}
所以，由极值原理知，$w(x)$ 在球壳 $B_R^*$ 的边界 $\partial B_R^*$ 达到非负最大值。在球壳的内球面 $\partial B_R^*\cap B_R = \partial B_{\frac{R}{2}}$ 上，
\begin{align*}
...
假设 $\Omega$ 是 $\mathbb{R}^n$ 上的有界连通开集，$c(x)\geqslant 0$ 且有界。如果 $u\in C^2(\Omega)\cap C(\overline{\Omega})$ 在 $\Omega$ 上满足 $\mathcal{L}u\leqslant 0$，且 $u(x)$ 在 $\Omega$ 内达到其在 $\overline{\Omega}$ 上的非负最大值，则 $u$ 在 $\Omega$ 上是常数。
\end{theorem}
...
\end{align*}
而在球 $B(\tilde{x},d)$ 上应用 Hopf 引理，至少存在一个方向 $\nu$ 使得
\begin{align*}
\left.\frac{\partial u}{\partial \nu}\right|_{x=y_0} > 0.
\end{align*}
...
\begin{theorem}
假设 $u\in C^2(\Omega)\cap C(\overline{\Omega})$ 是 Dirichlet 问题
\begin{align}
\begin{cases}
...
的解，则
\begin{align}
\max_{\overline{\Omega}} |u(x)| \leqslant G + CF, \label{eq:max-est-dir}
\end{align}
...
\end{gather*}
由极值原理，在 $\Omega$ 上 $w(x)\leqslant 0$。从而
\begin{align*}
...
\begin{theorem}
假设 $c(x)\geqslant 0$，$\alpha(x)\geqslant \alpha_0>0$。如果 $u\in C^2(\Omega)\cap C^1(\overline{\Omega})$ 是问题 \eqref{eq:third-boundary} 的解，则
\begin{align}
\max_{\overline{\Omega}} |u(x)| \leqslant C(G+F), \label{eq:max-est-third}
\end{align}
...
\end{align*}
由极值原理，我们知道 $w(x)$ 的正的最大值一定在边界 $\partial\Omega$ 上达到。设在点 $x_0\in\partial\Omega$ 处达到最大值。由于 $n$ 是 $\partial\Omega$ 的单位外法向量，于是 $\left.\frac{\partial w}{\partial n}\right|_{x=x_0}\geqslant 0$，从而
\begin{align*}
...
\begin{theorem}
假设 $u_i\in C^2(\Omega)\cap C^1(\overline{\Omega})$ $(i=1,2)$ 满足第三边值问题
\begin{align*}
\begin{cases}
...
\end{theorem}
\begin{proof}
由定理 \eqref{eq:max-est-third} 我们有
\begin{align}
...
\end{cases}
\end{align*}
于是，由定理 \eqref{eq:max-est-third} 有
\begin{align*}
