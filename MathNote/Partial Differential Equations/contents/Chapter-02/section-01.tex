...
\begin{definition}
如果函数 $u:\Omega\to\mathbb{R}$ 具有二阶连续偏导数且满足 Laplace 方程 $\Delta u = 0$，我们称之为\textbf{调和函数}；如果一个多项式满足 Laplace 方程 $\Delta u = 0$，我们称之为\textbf{调和多项式}。
\end{definition}

\begin{proposition}
设$n\in \mathbb{N}$,则下列函数均为调和函数:
\begin{enumerate}[(1)]
...
对于 (4)，由于 $z^k$ 是解析函数，由 (3) 知其实部 $P_k$ 与虚部 $Q_k$ 调和；递推关系 \eqref{rec:pq} 由 $z^{k+1}=z^k\cdot z$ 直接展开得到。
对于 (5)，因为 $e^z=e^x(\cos y+i\sin y)$ 解析，其实部 $e^x\cos y$ 与虚部 $e^x\sin y$ 调和。
\end{proof}
...
作平移和伸缩变换，则
\begin{align*}
\phi(r) = \int_{\partial B(0,1)} u(x+rz)\,\mathrm{d}S(z).
\end{align*}
对 $r$ 求微商，由\hyperref[Basis of Analytics-theorem:Green第一恒等式]{Green第一恒等式}我们得到
\begin{align*}
\phi'(r) &= \int_{\partial B(0,1)} D u(x+rz)\cdot z\,\mathrm{d}S(z)
...
\end{theorem}
\begin{proof}
对于固定的 $x\in\Omega$，任意球 $B(x,r)\subset\Omega$，$\phi(r)$ 如\hyperref[theorem:调和函数平均值公式]{调和函数平均值公式}的证明中定义。由假设我们得到 $\phi(r)$ 是一个常数，因此
\begin{align*}
\phi'(r) = 0.
\end{align*}
由\hyperref[theorem:调和函数平均值公式]{调和函数平均值公式}第一步的证明，我们有
\begin{align*}
\frac{1}{r^{n-1}}\fint_{B(x,r)} \Delta u(y)\,\mathrm{d}y = 0,
\end{align*}
于是
...
假设 $u\in C(\Omega)$ 满足平均值公式，即对于任意 $B(x,r)\subset\Omega$，有
\begin{align*}
u(x) = \fint_{\partial B(x,r)} u(y)\,\mathrm{d}S(y),
\end{align*}
...
\end{remark}
\begin{proof}
设 $\eta:\mathbb{R}^n\to\mathbb{R}$ 是一个光滑化子，即径向对称的非负函数 $\eta\in C_0^\infty(\mathbb{R}^n)$ 满足：
\begin{enumerate}[(1)]
...
利用球坐标变换我们得到
\begin{align*}
\frac{1}{\varepsilon^n}\int_0^\varepsilon \zeta\left(\frac{r}{\varepsilon}\right)\cdot n\alpha(n)r^{n-1}\,\mathrm{d}r = 1.
\end{align*}
...
\end{align*}
由于 $\eta$ 无穷次可微且具有紧支集，容易证明 $u_\varepsilon\in C^\infty(\Omega_\varepsilon)$。对于任意 $x\in\Omega_\varepsilon$，我们利用平均值公式计算得到
\begin{align*}
...
= u(x).
\end{align*}
因而 $u\in C^\infty(\Omega_\varepsilon)$。令 $\varepsilon\to 0$，我们得到 $u\in C^\infty(\Omega)$。再利用\refthe{theorem:调和函数平均值定理逆命题}即可得证。
\end{proof}

\begin{theorem}[Harnack 不等式]
对于 $\Omega$ 上的任何连通紧子集 $V$，存在一个仅与距离函数
\begin{align*}
\operatorname{dist}(V,\partial\Omega) = \min_{x\in V,\, y\in\partial\Omega} |x-y|
\end{align*}
和维数 $n$ 有关的正常数 $C$，使得
\begin{align}
\sup_V u \leqslant C \inf_V u, \label{eq:harnack}
\end{align}
...
(1) 首先考虑 $x,y\in V$，$|x-y|\leqslant r$。于是 $B(y,r)\subset B(x,2r)$，从而
\begin{align*}
u(x) &= \fint_{B(x,2r)} u(z)\,\mathrm{d}z \geqslant \frac{1}{2^n}\fint_{B(y,r)} u(z)\,\mathrm{d}z \geqslant \frac{1}{2^n}u(y).
\end{align*}

(2) 由于 $V$ 是连通的紧集，我们可用一串有限多个半径为 $r$ 的球 $\{B_i\}_{i=1}^N$ 覆盖它，而且满足 $B_i\cap B_{i-1}\neq\varnothing$ $(i=2,\cdots,N)$。于是，对任意 $x,y\in V$，有
\begin{align*}
u(x) \geqslant \frac{1}{2^{nN}} u(y).
\end{align*}
至此我们完成定理的证明。
\end{proof}
...
固定 $x_1\in\Omega_1$，则存在一条路径 $\gamma:[0,1]\to\Omega_1$ 连接 $x_0$ 和 $x_1$ 两点，也就是说，路径 $\gamma(t)$ 关于 $t$ 连续且 $\gamma(0)=x_0$ 和 $\gamma(1)=x_1$。定义
\begin{align*}
l = \sup\{t\in[0,1] \mid u[\gamma(t)] = M\},
\end{align*}
我们将证明 $l=1$，从而得到 $u(x_1)=u[\gamma(1)]=M$。

我们用反证法证明 $l=1$。假设 $l<1$，记 $x_l=\gamma(l)$，由函数 $u[\gamma(t)]$ 的连续性，我们有 $u(x_l)=M$。由于 $x_l$ 是内点，因此存在 $B(x_l,r_1)\subset\Omega_1$，在 $B(x_l,r_1)$ 应用平均值公式并注意到 $u(x_l)=M$，我们得到在 $B(x_l,r_1)$ 上函数 $u(x)=M$。注意到 $\gamma(t)$ 在 $l$ 处的连续性，存在充分小的 $\varepsilon>0$，使得 $\gamma$ 在区间 $[l-\varepsilon,l+\varepsilon]$ 上的象集包含于 $B(x_l,r_1)$。于是 $u[\gamma(l+\varepsilon)]=M$。这与 $l$ 是上确界的定义矛盾。因此假设 $l<1$ 不成立，从而 $l=1$。

这样我们证明了在 $\Omega$ 的包含 $x_0$ 的连通分支 $\Omega_1$ 上，$u(x)$ 恒为常数 $M$。由于函数 $u(x)$ 的连续性，$u(x)$ 在 $\Omega_1$ 的边界 $\partial\Omega_1$ 上也恒为常数 $M$。而 $\partial\Omega_1$ 是 $\Omega$ 的边界 $\partial\Omega$ 的一部分，从而得到定理的结论 (1) 成立。

(2) 当 $\Omega$ 连通时，$\Omega$ 只有一个连通分支，从而 $\Omega_1=\Omega$，定理的结论 (2) 成立。至此我们完成定理的证明。
\end{proof}
...
\end{cases}
\end{align*}
我们需要证明 $w\equiv 0$。利用极值原理我们得到，对于 $x\in\Omega$，$w(x)\leqslant 0$。注意到 $-w$ 满足同样的齐次边值问题，于是对于 $x\in\Omega$，$w(x)\geqslant 0$。因而 $w\equiv 0$。至此我们完成推论证明。
\end{proof}

下面我们利用平均值公式来推导出关于调和函数所有偏导数的估计，然后以此来证明调和函数是解析函数。

\begin{theorem}
假设 $u$ 是 $\Omega$ 上的调和函数，则对于任意球 $B(x,r)\subset\Omega$，任意阶数为 $k$ 的多重指标 $\alpha$，估计
\begin{align}
|D^\alpha u(x)| \leqslant \frac{C_k}{r^{n+k}} \int_{B(x,r)} |u(y)|\,\mathrm{d}y \label{eq:der-est}
\end{align}
成立，其中
\begin{align}
C_0 = \frac{1}{\alpha(n)},\qquad C_k = \frac{(n+k)^{n+k}(n+1)^k}{\alpha(n)(n+1)^{n+1}} \quad (k=1,2,\cdots). \label{eq:Ck}
\end{align}
\end{theorem}

\begin{proof}
我们对 $k$ 利用数学归纳法证明公式 \eqref{eq:der-est} 和 \eqref{eq:Ck}。

(1) 当 $k=0$ 时，公式 \eqref{eq:der-est} 和 \eqref{eq:Ck} 是平均值公式 \eqref{eq:mean-value} 的直接推论；

当 $k=1$ 时，我们注意到 $u_{x_i}$ $(i=1,2,\cdots,n)$ 也是调和函数，于是在球 $B(x,s)$ $(0<s\leqslant r)$ 上利用平均值公式和 Gauss-Green 公式得
\begin{align*}
...
\end{align}
至此我们完成当 $k=1$ 时公式 \eqref{eq:der-est} 和 \eqref{eq:Ck} 的证明。

(2) 现在假设 $k\geqslant 2$ 且对于 $\Omega$ 上的所有球和 $k-1$ 阶的多重指标，公式 \eqref{eq:der-est} 和 \eqref{eq:Ck} 成立。固定 $B(x,r)\subset\Omega$。令 $\alpha$ 为一个 $k$ 阶多重指标，必然存在 $i\in\{1,2,\cdots,n\}$ 和一个 $k-1$ 阶的多重指标 $\beta$，使得 $D^\alpha u=(D^\beta u)_{x_i}$。令 $v=D^\beta u$。由于 $v$ 是调和函数，对于任意 $0<t<1$，在球 $B(x,tr)$ 上应用不等式 \eqref{eq:der-k1} 有
\begin{align*}
|D^\alpha u(x)| \leqslant \frac{n+1}{\alpha(n)(tr)^{n+1}} \int_{B(x,tr)} |D^\beta u(y)|\,\mathrm{d}y.
\end{align*}
注意到 $y\in B(x,tr)$ 蕴涵着 $B(y,(1-t)r)\subset B(x,r)\subset\Omega$。由我们的假设，公式 \eqref{eq:der-est} 和 \eqref{eq:Ck} 蕴涵着：对任意 $0<t<1$，有
\begin{align*}
...
\end{align*}
这就完成当 $|\alpha|=k$ 时公式 \eqref{eq:der-est} 和 \eqref{eq:Ck} 的证明，从而完成定理的证明。
\end{proof}
...
\end{theorem}
\begin{proof}
设 $|u|\leqslant M$。固定 $x\in\mathbb{R}^n$。对任意 $r>0$，在球 $B(x,r)$ 上利用定理 \eqref{eq:der-est} 当 $k=1$ 时的结论，则
\begin{align*}
...
\end{align*}
对任意 $x\in B(x_0,r)$，则 $B(x,r)\subset B(x_0,2r)\subset\Omega$。由定理 \eqref{eq:der-est} 得到
\begin{align*}
|D^\alpha u(x)| \leqslant \frac{A}{r^N}(n+N)^{n+N}(n+1)^N,
\end{align*}
其中 $|\alpha|=N$。利用 Stirling 公式
\begin{align*}
...
我们将要证明，当
\begin{align}
|x-x_0| \leqslant \frac{r}{2n(n+1)e} \label{eq:analytic-domain}
\end{align}
...
\begin{align*}
H(r) = \int_{\partial B_r} u^2\,\mathrm{d}S,\qquad D(r) = r^2\int_{B_r} |\nabla u|^2\,\mathrm{d}y
\end{align*}
...
作伸缩变换，显然有
\begin{align*}
H(r) = \int_{\partial B_1} u^2(rz)\,\mathrm{d}S(z).
\end{align*}
对 $r$ 求微商，我们得到
\begin{align*}
H'(r) &= \int_{\partial B_1} D u^2(rz)\cdot z\,\mathrm{d}S(z) = \int_{\partial B_r} \frac{\partial u^2(y)}{\partial n}\,\mathrm{d}S(y) \\
&= \frac{1}{n\alpha(n)r^{n-1}}\int_{B_r} \Delta u^2\,\mathrm{d}y = \frac{2}{n\alpha(n)r^{n-1}}\int_{B_r} |\nabla u|^2\,\mathrm{d}y \\
...
对 $r$ 求微商，我们得到
\begin{align*}
D'(r) &= 2r\int_{B_1} |\nabla u|^2(rz)\,\mathrm{d}z + r^2\int_{B_1} D(|\nabla u|^2(rz))\cdot z\,\mathrm{d}z \\
&= \frac{2}{r}D(r) + 2r^2\int_{B_1}\sum_{i,j=1}^n u_i(rz)u_{ij}(rz)z_j\,\mathrm{d}z \\
&= \frac{2}{r}D(r) + 2r\int_{B_r}\sum_{i,j=1}^n u_i(y)u_{ji}(y)y_j\,\mathrm{d}y \\
&= \frac{2}{r}D(r) + \frac{2}{\alpha(n)r^{n-1}}\int_{B_r}\sum_{i,j=1}^n [(u_i u_j y_j)_i - u_i u_j \delta_{ij}]\,\mathrm{d}y \\
&= \frac{2}{\alpha(n)r^{n-1}}\int_{\partial B_r}\sum_{i,j=1}^n (u_i u_j y_j)y_i\,\mathrm{d}S(y) \\
&= 2nr\int_{\partial B_r}\sum_{i,j=1}^n \left(u_i\frac{y_i}{r}\right)\left(u_j\frac{y_j}{r}\right)\mathrm{d}S(y) \\
&= 2nr\int_{\partial B_r}\left|\frac{\partial u}{\partial n}\right|^2\,\mathrm{d}S \geqslant 0.
\end{align*}
因此 $D(r)$ 是 $r$ 的单调递增函数。

进一步，我们证明 $f(r)=\frac{D(r)}{H(r)}$ 是 $r$ 的单调递增函数。由上面的结果，
\begin{align*}
D(r) = \frac{nr}{2}H'(r),
\end{align*}
以及
