...
\end{align}
有意义，称为 $f(x)$ 的 Fourier 变换，这里 $\lambda=(\lambda_1,\lambda_2,\cdots,\lambda_n)\in\mathbb{R}^n$，记为 $(f(x))^\wedge=\hat{f}(\lambda)$。
\end{definition}
...
\end{align}
其中 $x=(x_1,x_2,\cdots,x_n)\in\mathbb{R}^n$。$(\hat{f}(\lambda))^\vee$ 表示在 Cauchy 主值意义下的无穷积分，称为 $\hat{f}(\lambda)$ 的 Fourier 逆变换。换句话说，
\begin{align*}
(\hat{f})^\vee=f.
\end{align*}
\end{theorem}
...
\begin{align*}
\left(\frac{\partial f}{\partial x_j}\right)^\wedge=\mathrm{i}\lambda _j\hat{f}(\lambda),\quad j=1,2,\cdots,n.
\end{align*}
\end{proposition}

\begin{proposition}[乘多项式性质]
若 $f(x),x_j f(x)\in L^1(\mathbb{R}^n)$，$j=1,2,\cdots,n$，则
\begin{align*}
...
\end{align*}
这里 $\alpha=(\alpha_1,\alpha_2,\cdots,\alpha_n)$ 是阶数小于或等于 $k$ 的多重指标，$x^\alpha=x_1^{\alpha_1}x_2^{\alpha_2}\cdots x_n^{\alpha_n}$。
\end{remark}
