...
极值原理具有极为明确的物理意义。在叙述极值原理之前，我们先考虑以下实际问题。
...
记区域 $Q_T=(0,l)\times(0,T]$。$Q_T$ 的侧边和底边我们称为\textbf{抛物边界}，通常记为 $\partial_p Q_T$。实际上
\begin{align*}
\partial_p Q_T=\partial Q_T\setminus(0,l)\times\{T\}.
\end{align*}

现在我们考虑长度为 $l$，侧表面绝热的均匀细杆。杆的温度分布 $u(x,t)$ 在 $Q_T$ 上满足热方程
\begin{align*}
\mathcal{L}u=u_t-a^2u_{xx}=f(x,t).
\end{align*}
如果 $f(x,t)\geqslant 0$，则表示杆上有热源；如果 $f(x,t)\leqslant 0$，则表示杆上有热汇。

\begin{theorem}[极值原理]
假设 $u\in C^{2,1}(Q_T)\cap C(\overline{Q}_T)$ 满足方程 $\mathcal{L}u=f\leqslant 0$，则 $u(x,t)$ 在 $\overline{Q}_T$ 上的最大值必在抛物边界 $\partial_p Q_T$ 上达到，即
\begin{align*}
...
\begin{corollary}
假设 $u\in C^{2,1}(Q_T)\cap C(\overline{Q}_T)$ 满足方程 $\mathcal{L}u=f\geqslant 0$，则 $u(x,t)$ 在 $\overline{Q}_T$ 上的最小值必在抛物边界 $\partial_p Q_T$ 达到，即
\begin{align*}
\min_{\overline{Q}_T}u(x,t)=\min_{\partial_p Q_T}u(x,t);
\end{align*}
假设 $u\in C^{2,1}(Q_T)\cap C(\overline{Q}_T)$ 满足方程 $\mathcal{L}u=0$，则 $u(x,t)$ 在 $\overline{Q}_T$ 上的最大值和最小值必在抛物边界 $\partial_p Q_T$ 达到。
\end{corollary}
\begin{proof}
...
\end{corollary}
\begin{proof}
考虑 $w(x,t)=u(x,t)-v(x,t)$。由于 $\mathcal{L}w\leqslant 0$，对 $w(x,t)$ 应用极值原理得到
\begin{align*}
...
\end{align}
利用极值原理我们可以得到下面的最大模估计。

\begin{theorem}
假设 $u\in C^{2,1}(Q_T)\cap C(\overline{Q}_T)$ 是问题 \eqref{eq:heat-dirichlet-max} 的解，则
\begin{align}
...
\end{align*}
由极值原理，在 $\overline{Q}_T$ 上 $w(x,t)\geqslant 0$，从而
\begin{align*}
...
\begin{corollary}
混合问题 \eqref{eq:heat-dirichlet-max} 的解在 $C^{2,1}(Q_T)\cap C(\overline{Q}_T)$ 中是惟一的。
\end{corollary}
\begin{proof}
由于混合问题 \eqref{eq:heat-dirichlet-max} 是线性的，为证明惟一性，只需证明当 $f\equiv 0,\varphi\equiv 0,g_1=g_2\equiv 0$ 时，问题只有零解。由最大模估计 \eqref{eq:heat-max-est1}，这是显然的。
\end{proof}

\begin{corollary}
混合问题 \eqref{eq:heat-dirichlet-max} 在 $C^{2,1}(Q_T)\cap C(\overline{Q}_T)$ 中的解连续地依赖于非齐次项 $f$，初值 $\varphi$，边值 $g_1,g_2$。
\end{corollary}
\begin{remark}
...
\end{align}
其中 $\alpha(t)\geqslant 0,\ \beta(t)\geqslant 0$。当 $\alpha(t)=\beta(t)\equiv 0$ 时，上述问题就是第二边值问题；当 $\alpha(t)>0,\ \beta(t)>0$ 时，上述问题就是第三边值问题。

\begin{lemma}
假设 $u\in C^{2,1}(Q_T)\cap C^{1,0}(\overline{Q}_T)$ 满足
\begin{align*}
\begin{cases}
\mathcal{L}u=u_t-a^2u_{xx}\geqslant 0, & (x,t)\in Q_T,\\
u|_{t=0}\geqslant 0, & x\in[0,l],\\
[-u_x+\alpha(t)u]|_{x=0}\geqslant 0, & t\in[0,T],\\
[u_x+\beta(t)u]|_{x=l}\geqslant 0, & t\in[0,T],
\end{cases}
\end{align*}
...
\begin{theorem}
假设 $u\in C^{2,1}(Q_T)\cap C^{1,0}(\overline{Q}_T)$ 是混合问题 \eqref{eq:heat-third-max} 的解，则
\begin{align}
...
\end{cases}
\end{align*}
我们利用引理得到，在 $\overline{Q}_T$ 上 $v(x,t)\geqslant 0$。于是，对于 $(x,t)\in\overline{Q}_T$，有
\begin{align*}
...
在区域 $Q_T=\mathbb{R}\times(0,T]$ 上考虑初值问题
\begin{align}
\begin{cases}
\mathcal{L}u=u_t-a^2u_{xx}=f(x,t), & (x,t)\in Q_T,\\
...
\end{align}
当 $f\equiv 0$，$\varphi(x)$ 有界连续时，Poisson 公式 \eqref{eq:poisson-1d} 给出上述初值问题的一个有界解。下面的最大模估计将保证初值问题 \eqref{eq:heat-cauchy-max} 的有界解是惟一的。

\begin{theorem}
假设 $u\in C^{2,1}(Q_T)\cap C(\overline{Q}_T)$ 是初值问题 \eqref{eq:heat-cauchy-max} 的有界解，则
\begin{align}
...
\end{cases}
\end{align*}
在 $Q_T^L$ 上利用极值原理可得 $\min_{\overline{Q}_T^L}w(x,t)\geqslant 0$。而对于任意 $(x_0,t_0)\in Q_T$，存在足够大的 $L$，使得 $(x_0,t_0)\in Q_T^L$。由 $w(x_0,t_0)\geqslant 0$ 得到
\begin{align*}
...
\begin{theorem}
假设 $u\in C^{2,1}(Q_T)\cap C(\overline{Q}_T)$ 是初值问题 \eqref{eq:heat-cauchy-max} 的解且满足增长性条件 \eqref{eq:growth}，则
\begin{align*}
...
由定理我们得到初值问题 \eqref{eq:heat-cauchy-max} 的满足增长性条件 \eqref{eq:growth} 的解的惟一性。然而惟一性在一般情形并不成立。
...
\end{align*}
由于篇幅的关系，我们在这里就不给出严格的证明。有兴趣的读者可参阅文献[2]中第五章第五节的内容。
\end{remark}

\subsection{混合问题的能量模估计}

在这一节中我们讨论混合问题的能量模估计。为简单起见，我们考虑第一边值问题
\begin{align}
\begin{cases}
\mathcal{L}u=u_t-a^2u_{xx}=f, & (x,t)\in Q_T,\\
u|_{t=0}=\varphi(x), & x\in[0,l],\\
...
\end{align}
其中 $Q_T=(0,l)\times(0,T]$。其他的边值问题可以类似讨论。我们将证明如下能量模估计。
...
\begin{theorem}
设 $u\in C^{1,0}(\overline{Q}_T)\cap C^{2,1}(Q_T)$ 是问题 \eqref{eq:heat-energy} 的解，则
\begin{align}
...
记
\begin{align*}
G(t)=\int_0^t\int_0^l u^2(x,t)\,\mathrm{d}x\,\mathrm{d}t,\\
...
\begin{theorem}
混合问题 \eqref{eq:heat-energy} 的解 $u\in C^{1,0}(\overline{Q}_T)\cap C^{2,1}(Q_T)$ 是惟一的。
\end{theorem}
