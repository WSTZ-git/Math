...
\begin{definition}
设 $\Omega$ 是 $\mathbb{R}^n$ 中的一个集合，$x=(x_1,x_2,\dots,x_n)$ 表示 $\Omega$ 上的点。假设 $u=u(x):\Omega\to\mathbb{R}$ 是一个函数。对固定正整数 $k$，我们用符号 $D^k u$ 表示 $u$ 的所有 $k$ 阶偏导数
\begin{align*}
...
\begin{align*}
\left( \frac{\partial u}{\partial x_1}, \frac{\partial u}{\partial x_2}, \dots, \frac{\partial u}{\partial x_n} \right)
\end{align*}
...
\begin{definition}
设 $F=(F_1,F_2,\dots,F_n):\Omega\to\mathbb{R}^n$ 是一个向量函数，记 $F$ 的\textbf{散度}为
\begin{align*}
...
\begin{definition}
我们用 $C(\Omega)$ 表示在 $\Omega$ 上的连续函数构成的线性空间。对于它的元素 $u\in C(\Omega)$，其模定义为
\begin{align*}
...
如下形式的方程
\begin{align}
F[D^k u(x), D^{k-1}u(x), \dots, Du(x), u(x), x] = 0,\quad x\in\Omega. \label{eq:pde-general}
\end{align}
...
\item 如果方程 \eqref{eq:pde-general} 可表示成
\begin{align*}
\sum_{|\alpha|=k} a_{\alpha}[D^{k-1}u(x), \dots, Du(x), u(x), x] D^{\alpha} u
= f[D^{k-1}u(x), \dots, Du(x), u(x), x],
\end{align*}
