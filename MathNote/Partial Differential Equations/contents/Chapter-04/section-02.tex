...
\begin{proposition}
设 $u(x,t)=X(x)T(t)$ 是混合问题
\begin{align}
\begin{cases}
u_{tt}-a^2u_{xx}=0, & (x,t)\in(0,l)\times(0,+\infty), \\
u(0,t)=u(l,t)=0, & t\in[0,+\infty)
\end{cases} \label{eq:wave-mixed-hom-bc}
\end{align}
的非零分离变量解，则 $X$ 满足特征值问题
\begin{align}
\begin{cases}
...
其所有特征值为
\begin{align}
\lambda_n=\left(\frac{n\pi}{l}\right)^2,\quad n=1,2,\cdots, \label{eq:eigenvalues}
\end{align}
相应的特征函数为
\begin{align}
X_n(x)=\sin\frac{n\pi}{l}x,\quad n=1,2,\cdots. \label{eq:eigenfunctions}
\end{align}
\end{proposition}
...
\end{align*}
其中 $\lambda$ 为常数。于是 $X''+\lambda X=0$，$T''+a^2\lambda T=0$. 由边值条件得 $X(0)=X(l)=0$. 解 $X''+\lambda X=0$，$X(0)=X(l)=0$，得 $\lambda_n=(n\pi/l)^2$，$X_n=\sin(n\pi x/l)$.
\end{proof}
...
\quad\Longrightarrow\quad v_{tt}-a^2v_{xx}=f-h_{tt}=F.
\end{align*}
初值条件：
\begin{align*}
...
\end{align*}
因此 $v$ 满足非齐次方程、齐次边值的混合问题 \eqref{eq:v-problem}. 由相容性条件 \eqref{eq:comp-bc1}--\eqref{eq:comp-bc3} 可推出 $\varphi_v(0)=\varphi_v(l)=\psi_v(0)=\psi_v(l)=0$ 以及
\begin{align*}
