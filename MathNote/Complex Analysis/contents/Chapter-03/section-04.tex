...
\begin{theorem}[Cauchy积分公式]\label{theorem:复变-Cauchy积分公式}
...
\end{theorem}
\begin{note}
上述定理表明全纯函数在区域中的值由它在边界上的值所完全确定. \eqref{equation:Cauchy积分公式1}式是全纯函数的一种积分表示，通过这种表示，我们可以证明全纯函数有任意阶导数.
\end{note}
\begin{proof}
...
\end{align}
于是，由\eqref{equation:Cauchy积分公式2}式、\eqref{equation:Cauchy积分公式3}式及\hyperref[theorem:长大不等式]{长大不等式}即得
\begin{align*}
...
\end{align*}
根据\hyperref[theorem:复变-Cauchy积分公式]{Cauchy积分公式}可得
\begin{align*}
...
{\color{blue}证法一:}
由\refthe{theorem:复变-Cauchy积分公式},\( f \) 可写为 Cauchy 型积分
\[
f(z) = \frac{1}{2\pi \mathrm{i}} \int_{\gamma} \frac{f(\zeta)}{\zeta - z} \mathrm{d}\zeta.
\]
...
\]
所以由\hyperref[theorem:长大不等式]{长大不等式}得
\[
...
\begin{example}
设 $D$ 为 $\mathbb{C}$ 上边界光滑的区域，$f$ 为 $D$ 上全纯函数，连续延拓到 $\partial D$. 证明：
\begin{align*}
\sup_{z \in \partial D} |f(z) - \overline{z}| \geqslant \frac{2 \operatorname{Area}(D)}{\operatorname{Length}(\partial D)}.
\end{align*}
\end{example}
\begin{proof}
考虑边界积分
\begin{align*}
\int_{\partial D} (f(z) - \overline{z}) \, \mathrm{d}z.
\end{align*}
因为 $f$ 在 $D$ 上全纯，且边界光滑，由\hyperref[theorem:Cauchy-Goursat定理(Cauchy积分定理)]{Cauchy积分定理}得
\begin{align*}
\int_{\partial D} f(z) \, \mathrm{d}z = 0.
\end{align*}
所以
\begin{align}\label{eq::HFWUFOW344123-2-4-24}
\int_{\partial D} (f(z) - \overline{z}) \, \mathrm{d}z
= - \int_{\partial D} \overline{z} \, \mathrm{d}z.
\end{align}
下面计算 $\int_{\partial D} \overline{z} \, \mathrm{d}z$. 设 $z = x + \mathrm{i}y$，则
\begin{align*}
\overline{z} = x - \mathrm{i}y, \qquad \mathrm{d}z = \mathrm{d}x + \mathrm{i}\,\mathrm{d}y,
\end{align*}
于是由\refthe{theorem:连续复变函数积分必存在}知
\begin{align*}
\int_{\partial D}{\overline{z}\mathrm{d}z}=\int_{\partial D}{x\mathrm{d}x+y\mathrm{d}y}+\mathrm{i}\int_{\partial D}{x\mathrm{d}y-y\mathrm{d}x}.
\end{align*}
由\hyperref[theorem:复积分的微分学基本定理]{微分学基本定理}知
\begin{align*}
\int_{\partial D}{x\mathrm{d}x+y\mathrm{d}y}=\left.\frac{x^2+y^2}{2}\right| _{\left( x_0,y_0 \right)}^{\left( x_0,y_0 \right)}=0,
\end{align*}
其中$(x_0,y_0)\in \partial D$.又由\hyperref[Basis of Analytics-theorem:Green公式--数分]{Green公式}可得
\begin{align*}
\int_{\partial D} x\,\mathrm{d}y = \operatorname{Area}(D), \qquad
\int_{\partial D} y\,\mathrm{d}x = - \operatorname{Area}(D),
\end{align*}
因此
\begin{align*}
\int_{\partial D} \overline{z} \, \mathrm{d}z = 2\mathrm{i} \operatorname{Area}(D)\Longrightarrow
\int_{\partial D} (f(z) - \overline{z}) \, \mathrm{d}z
= -2\mathrm{i} \operatorname{Area}(D)\Longrightarrow 2 \operatorname{Area}(D)
= \left| \int_{\partial D} (f(z) - \overline{z}) \, \mathrm{d}z \right|.
\end{align*}
另一方面，令$M = \sup_{z \in \partial D} |f(z) - \overline{z}|$,由\hyperref[theorem:长大不等式]{长大不等式}可得
\begin{align*}
\left| \int_{\partial D} (f(z) - \overline{z}) \, \mathrm{d}z \right|
\leqslant \int_{\partial D} |f(z) - \overline{z}| \, |\mathrm{d}z|
\leqslant M \int_{\partial D} |\mathrm{d}z|
= M \operatorname{Length}(\partial D).
\end{align*}
于是
\begin{align*}
2 \operatorname{Area}(D) \leqslant M \operatorname{Length}(\partial D),
\end{align*}
即
\begin{align*}
M \geqslant \frac{2 \operatorname{Area}(D)}{\operatorname{Length}(\partial D)}.
\end{align*}
\end{proof}
...
设整函数$f(z)$满足条件
\begin{align*}
\iint_{\mathbb{C}} \frac{|f(z)|}{(1+|z|)^{2029}} \mathrm{d}A(z) < \infty.
\end{align*}
这里$\mathrm{d}A(z)$为面积元。证明：$f(z)$是次数不超过$2026$的多项式。
\end{example}
\begin{proof}

因为\(f\) 整函数,所以由\hyperref[theorem:全纯函数的Taylor展开]{全纯函数的Taylor展开},将$f$在原点Taylor展开得
\begin{align*}
f(z)=\sum\limits_{n=0}^\infty a_n z^n,\quad \forall z\in \mathbb{C}.
\end{align*}
其中
\begin{align*}
a_n=\frac{1}{2\pi \mathrm{i}}\int_{|z|=r}{\frac{f(z)}{z^{n+1}}\,\mathrm{d}z}.
\end{align*}

用反证法. 假设存在某个整数 \(n\geqslant 2027\) 使得 \(a_n\neq 0\). 下面证明此时条件积分发散.
令 \(z=re^{\mathrm{i}\theta}\)，则 \(\mathrm{d}A(z)=r\,\mathrm{d}r\mathrm{d}\theta\)，故
\begin{align}\label{eq::OUHOFHOJIOJIOW23424fsf}
I\triangleq \iint_{\mathbb C} \frac{|f(z)|}{(1+|z|)^{2029}}\,\mathrm{d}A(z)
=\int_0^\infty \frac{r}{(1+r)^{2029}}
\left(\int_0^{2\pi} |f(re^{\mathrm{i}\theta})|\,\mathrm{d}\theta\right) \mathrm{d}r.
\end{align}
并且对\(\forall r>0\),有
\begin{align*}
a_n=\frac{1}{2\pi \mathrm{i}}\int_{|z|=r}{\frac{f(z)}{z^{n+1}}\,\mathrm{d}z}=\frac{1}{2\pi}\int_0^{2\pi}{\frac{f(re^{\mathrm{i}\theta})e^{-\mathrm{i}n\theta}}{r^n}\,\mathrm{d}\theta}.
\end{align*}
从而
\begin{align}
|a_n| r^n
=\left|\frac{1}{2\pi}\int_0^{2\pi} f(re^{\mathrm{i}\theta}) e^{-\mathrm{i}n\theta}\,\mathrm{d}\theta\right|
\leqslant \frac{1}{2\pi}\int_0^{2\pi} |f(re^{\mathrm{i}\theta})|\,\mathrm{d}\theta\Longrightarrow 
\int_0^{2\pi} |f(re^{\mathrm{i}\theta})|\,\mathrm{d}\theta \geqslant 2\pi |a_n| r^n. \label{eq:est}
\end{align}
将 \eqref{eq:est}式代入\eqref{eq::OUHOFHOJIOJIOW23424fsf}式，得到
\begin{align*}
I \geqslant 2\pi |a_n| \int_0^\infty \frac{r^{n+1}}{(1+r)^{2029}}\,\mathrm{d}r.
\end{align*}
注意到
\begin{align*}
\frac{r^{n+1}}{(1+r)^{2029}} \sim r^{n-2028},\quad r\to\infty.
\end{align*}
由于 \(n\geqslant 2027\)，因此
\begin{align*}
I=\int_0^\infty \frac{r^{n+1}}{(1+r)^{2029}}\,\mathrm{d}r=+\infty,
\end{align*}
这与题设 \(I<\infty\) 矛盾!

故假设不成立，即对所有 \(n\geqslant 2027\)，都有 \(a_n=0\). 因而
\begin{align*}
