...
\end{theorem}
\begin{note}
公式 \(\eqref{eq:6}\) 在高维空间中也成立，通常称为 $\mathbb{Stokes}$\textbf{公式}，这里只是它的一个特例。
\end{note}
\begin{proof}
...
\end{theorem}
\begin{note}
如果 \( f \in H(D) \)，那么由 Cauchy-Riemann 方程，\( \frac{\partial f}{\partial \bar{\zeta}} = 0 \)，这时公式 \(\eqref{eq:11}\) 右端的第二项就消失了，公式 \(\eqref{eq:11}\) 就是 Cauchy积分公式。所以，公式 \(\eqref{eq:11}\) 是 Cauchy 积分公式在 \( C^1 \) 函数类中的推广，有时也称为\textbf{非齐次}$\mathbb{Cauchy}$\textbf{积分公式}。
