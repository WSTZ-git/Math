...
\end{theorem}
\begin{proof}  
由 \( \gamma \) 围成的区域记为 \( G \)，因为 \( f' \) 连续，即 \( \frac{\partial u}{\partial x}, \frac{\partial v}{\partial x}, \frac{\partial u}{\partial y}, \frac{\partial v}{\partial y} \) 连续，故可用 \hyperref[Basis Analytics-theorem:Green公式--数分]{Green公式}. 又因 \( f \) 在 \( D \) 中全纯，故由\refthe{theorem:复变函数可微的充要条件2}可知 Cauchy-Riemann 方程成立. 于是由\hyperref[Basis Analytics-theorem:Green公式--数分]{Green公式}可得
\[
...
\]
这里$\gamma$都取正方向.由\refthe{theorem:连续复变函数积分必存在}，即得
\[
\int_\gamma f(z)\mathrm{d}z = 0.
\]
\end{proof}
...
\end{align*}
当 \( z \in \gamma_k \) 或 \( P_k \) 时，都有 \( |z - z_{k - 1}| < \eta \leqslant \delta \)，因而 \( |f(z) - f(z_{k - 1})| < \frac{\varepsilon}{2L} \). 对上面两个积分用\hyperref[theorem:长大不等式]{长大不等式}，它们都不超过 \( \frac{\varepsilon}{2L}|\gamma_k| \)，因而
\[
...
\]
利用\eqref{equation--------::2}式、(iii) 和\hyperref[theorem:长大不等式]{长大不等式}，即得
\[
...
\begin{align*}
\int_{C_1}{f\left( z \right) \mathrm{d}z}=\int_{C_2}{f\left( z \right) \mathrm{d}z}.
\end{align*}
此时也将上述积分记为
\begin{align*}
...
\end{corollary}
\begin{proof}
设 $C_1$ 与 $C_2$ 是 $D$ 内连接起点 $z_0$ 与终点 $z_1$ 的任意两条曲线(如\reffig{figure:图3...4}).则正方向曲线 $C_1$ 与负方向曲线 $C_2^-$ 就衔接成 $D$ 内的一条闭曲线 $C$.于是,由\hyperref[theorem:Cauchy-Goursat定理(Cauchy积分定理)]{Cauchy积分定理}与\hyperref[theorem:复积分的基本性质]{复积分的基本性质(3)},有
\begin{align*}
...
\end{align}
\eqref{equation-------..5}式右端的积分分别沿 \( \gamma_1, \cdots, \gamma_n \) 的正方向进行.
\end{theorem}
\begin{proof}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.3]{图3.4.png}
\caption{}
\label{figure:图3.4}
\end{figure}
\end{proof}
...
\]
这里左侧积分沿$\gamma_0$的正方向进行,右侧积分沿$\gamma_1$的正方向进行.
\end{corollary}
\begin{proof}
由\refthe{theorem:定理3.2.5}中$n=1$的情况立得.
\end{proof}

\begin{proposition}\label{proposition:例3.2.7}
设 \( \gamma \) 是一可求长简单闭曲线，\( a \notin \gamma \)，证明
\begin{align*}
\int_{\gamma}{\frac{\mathrm{d}z}{z-a}}=\begin{cases}
0,&a\text{在}\gamma \text{的外部},\\
2\pi \mathrm{i},&a\text{在}\gamma \text{的内部}.\\
\end{cases}
\end{align*}
\end{proposition}
\begin{proof}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.3]{图3.5.png}
\caption{}
\label{figure:图3.5}
\end{figure}
\end{proof}
...
\]
由\refpro{proposition:例3.2.7}即可得
\[
I = 
\begin{cases} 
...
\begin{example}
设$f\in H(D)$,且\(f'(z)\) 在单连通区域 \(D\) 内连续，证明对 \( D \) 中任意的可求长闭曲线 \( \gamma \)，均有
\[
\int_\gamma f(z)\mathrm{d}z = 0.
\]
\end{example}
\begin{proof}
设由 \(\gamma\) 围成的域记为 \(G\)，因为 \(f'\) 连续，故 \(\frac{\partial u}{\partial x}, \frac{\partial v}{\partial x}, \frac{\partial u}{\partial y}, \frac{\partial v}{\partial y}\) 连续.
又因 \(f\) 在 \(D\) 中全纯，故 Cauchy-Riemann 方程成立. 于是由 \hyperref[Basis of Analytics-theorem:Green公式--数分]{Green公式}得
\begin{align*}
\int_{\gamma} u \mathrm{d}x - v \mathrm{d}y &= \iint_{G} \left(-\frac{\partial v}{\partial x} - \frac{\partial u}{\partial y}\right) \mathrm{d}x \mathrm{d}y = 0, \\
\int_{\gamma} v \mathrm{d}x + u \mathrm{d}y &= \iint_{G} \left(\frac{\partial u}{\partial x} - \frac{\partial v}{\partial y}\right) \mathrm{d}x \mathrm{d}y = 0.
\end{align*}
再由\cref{theorem:连续复变函数积分必存在}可知\(\int_{\gamma} f(z) \mathrm{d}z = 0.\)
\end{proof}
