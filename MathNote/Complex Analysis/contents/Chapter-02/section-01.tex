...
\begin{definition}
设 \( f:D \to \mathbb{C} \) 是定义在区域 \( D \) 上的函数，\( z_0 \in D \)。如果极限
\begin{align}
\lim_{z \to z_0} \frac{f(z) - f(z_0)}{z - z_0}. \label{eq:2.1.1}
\end{align}
存在，就说 \( f \) 在 \( z_0 \) 处\textbf{复可微}或\textbf{可微}，这个极限称为 \( f \) 在 \( z_0 \) 处的\textbf{导数}或\textbf{微商}，记作 \( f'(z_0) \)。如果 \( f \) 在 \( D \) 中每点都可微，就称 \( f \) 是区域 \( D \) 中的\textbf{全纯函数}或\textbf{解析函数}。如果 \( f \) 在 \( z_0 \) 的一个邻域中全纯，就称 \( f \) 在 \( z_0 \) 处\textbf{全纯}。
\end{definition}
