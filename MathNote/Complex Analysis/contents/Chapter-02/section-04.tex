...
\begin{proof}
\begin{enumerate}[(i)]
\item \( \mathrm{e}^z \) 在 \( \mathbb{C} \) 上每点实可微是显然的. 经验证它满足 Cauchy-Riemann 方程. 因为 \( u(x, y) = \mathrm{e}^x \cos y \)，\( v(x, y) = \mathrm{e}^x \sin y \)，所以
\[
...
\end{align*}
进而,由\nrefthe{theorem:复变函数求导运算法则}{(2)}知$\left( e^{z_0z} \right) ' =z_0e^{z_0z}.$
...
则
\begin{enumerate}[(1)]
\item \( w = \mathrm{e}^z \) 把平行于实轴的直线\( \mathrm{Im}z = y_0 \)变成一条从原点出发的半射线，它与实轴正方向的夹角是 \( y_0 \)（\reffig{figure:图2.3}）;

\item \( w = \mathrm{e}^z \) 把带状区域 \( \{ z = x + \mathrm{i}y : 0 < y < 2\pi \} \) 变成全平面除掉正实轴的区域 \( \mathbb{C} \setminus \{ z : z \geqslant  0 \} \);

\item \( w = \mathrm{e}^z \) 把直线 \( \mathrm{Im}z = 0 \) 变成正实轴的上岸，直线 \( \mathrm{Im}z = 2\pi \) 变成正实轴的下岸；

\item \( w = \mathrm{e}^z \) 把带状区域 \( \{ z = x + \mathrm{i}y : 0 < y < \pi \} \) 变成上半平面，带状区域 \( \{ z = x + \mathrm{i}y : \pi < y < 2\pi \} \) 变成下半平面.
\end{enumerate}
...
\begin{definition}[支点]
如果当 \( z \) 沿着 \( z_0 \) 的充分小邻域中的任意简单闭曲线绕一圈时，多值函数$f$的值就从一支变到另一支，那么称 \( z_0 \) 为该多值函数$f$的一个\textbf{支点}。
\end{definition}
...
\begin{enumerate}[(1)]
\item $g$是$U$上的单值函数;
\item $g\in H\left( U \right)$;
\item 对$\forall z\in U$,$g\left( z \right)$是$f\left( z \right)$的一个可能值.
\end{enumerate}
那么称$g$为$f$在$U$上的一个\textbf{单值全纯分支}.
\end{definition}

\begin{definition}
我们把从原点出发并伸向无穷远的曲线叫做\textbf{割线}。

用来割破$z$平面,借以分出多值函数$f$的单值全纯分支的割线,称为$f$的\textbf{支割线}.一般地说,支割线可以区分为两岸.如果支割线接近于平行$x$轴的方向,就分成\textbf{上岸}与\textbf{下岸};如果支割线接近平行于$y$轴的方向,就分成\textbf{左岸}与\textbf{右岸}.每一单值分支在支割线的两岸取得不同的值.
\end{definition}
\begin{remark}
对定义在区域$G$上的多值函数$w=f(z)$,对应于支割线的不同作法,分支也就不同.因为这时各分支的定义域$G$随支割线改变而改变,其值域$T_{k}\ (k=0,1,\cdots,n-1)$当然也要随支割线改变而改变.但无论怎样,各分支的总体仍然是$f$,因为改变后的$T_{k}\ (k=0,1,\cdots,n-1)$仍然互不相交而填满(都加上同一端边界)点集$f(G)$,即
\begin{align*}
\bigcup_{k=1}^{n-1}{T_k}=f(G),\quad T_i\cap T_j=\varnothing \left( i\ne j \right) .
\end{align*}
\end{remark}
...
\end{remark}
\begin{proof}
由条件可知\( \mathrm{e}^{u + \mathrm{i}v} = r\mathrm{e}^{\mathrm{i}\theta} ,r=|z|,\theta = \arg z\)，因而 \( \mathrm{e}^u = r \)，\( v = \theta + 2k\pi \)。于是
\begin{align*}
\mathrm{Ln} z = \ln|z| + \mathrm{i}\arg z + 2k\pi\mathrm{i} = \ln|z| + \mathrm{i}\mathrm{Arg} z.
\end{align*}
...
\begin{theorem}[对数函数的基本性质]
设两个复数$z_1,z_2\neq 0,\infty$,则
\begin{gather*}
\mathrm{Ln}\left( z_1z_2 \right) =\mathrm{Ln}z_1+\mathrm{Ln}z_2,
\\
...
\]
因此由\nrefthe{theorem:Cauchy-Riemann方程的等价定义}{(iii)}知道，\( \varphi_{k} \) 是 \( D \) 上的全纯函数，而且由\refthe{theorem:复变函数可微的充要条件2}及\nrefthe{theorem:Cauchy-Riemann方程的等价定义}{(iii)的证明过程}可得
\begin{align*}
...
\]
把 \( D \) 单叶地映为带状区域 \( -\pi < \mathrm{Im} w < \pi \)。其他各分支，例如 \( w = \varphi_k(z)=\ln|z| + \mathrm{i}(\arg z + 2k\pi) \)，就把 \( D \) 单叶地映为带状区域 \( (2k - 1)\pi < \mathrm{Im} w < (2k + 1)\pi \)。一般来说，\( w = \varphi_0(z) \) 把角状区域 \( -\pi \leqslant \alpha < \arg z < \beta \leqslant \pi \) 单叶地映为带状区域 \( \alpha < \mathrm{Im} w < \beta \)。
...
\]
称为$z^\mu$的\textbf{主支}．并且$w$的任意一支都有
\begin{align}\label{equation:幂函数求导}
w_k'(z) =\mu \mathrm{e}^{\left( \mu -1 \right) \mathrm{Ln}z}=\mu \mathrm{e}^{(\mu -1)(\ln \left| z \right|+\mathrm{iarg}z+2k\pi \mathrm{i})},\quad k=0,\pm 1,\cdots .
\end{align}
\end{theorem}
\begin{proof}
...
\begin{theorem}[三角函数的性质]\label{theorem:三角函数的性质}
\begin{enumerate}[(1)]
...
\item\label{theorem:三角函数的性质-vi} $\cos z$和$\sin z$不是有界函数．

\item 对$\forall n\in \mathbb{N}$,$\theta \in \mathbb{R}$,有
\begin{gather*}
...
\item 注意到
\begin{align*}
\sum_{k=1}^n{e^{\mathrm{i}k\theta}}&=\frac{e^{\mathrm{i}\theta}\left( 1-e^{\mathrm{i}n\theta} \right)}{1-e^{\mathrm{i}\theta}}=\frac{e^{\mathrm{i}\theta}\left( 1-e^{-\mathrm{i}\theta} \right) \left( 1-e^{\mathrm{i}n\theta} \right)}{2-e^{\mathrm{i}\theta}-e^{-\mathrm{i}\theta}}=\frac{\left( e^{\mathrm{i}\theta}-1 \right) \left( 1-e^{\mathrm{i}n\theta} \right)}{2-2\cos \theta}
\\
&=\frac{e^{\mathrm{i}\theta}+e^{\mathrm{i}n\theta}-e^{\mathrm{i}\left( n+1 \right) \theta}-1}{4\sin ^2\frac{\theta}{2}}=\frac{\cos \theta -1+\cos n\theta -\cos \left( n+1 \right) \theta +\mathrm{i}\left( \sin \theta +\sin n\theta -\sin \left( n+1 \right) \theta \right)}{4\sin ^2\frac{\theta}{2}}
\\
&=\frac{-2\sin ^2\frac{\theta}{2}+2\sin \frac{\theta}{2}\sin \left( n+\frac{1}{2} \right) \theta +\mathrm{i}\left( 2\sin \frac{\theta}{2}\cos \frac{\theta}{2}-2\cos \left( n+\frac{1}{2} \right) \theta \sin \frac{\theta}{2} \right)}{4\sin ^2\frac{\theta}{2}}
\\
&=\frac{\sin \left( n+\frac{1}{2} \right) \theta -\sin \frac{\theta}{2}+\mathrm{i}\left( \cos \frac{\theta}{2}-\cos \left( n+\frac{1}{2} \right) \theta \right)}{2\sin \frac{\theta}{2}}.
\end{align*}
故
\begin{align*}
\sum\limits_{k=1}^n{\cos k\theta}&=\mathrm{Re}\sum\limits_{k=1}^n{e^{\mathrm{i}k\theta}}=\frac{\sin \left( n+\frac{1}{2} \right) \theta -\sin \frac{\theta}{2}}{2\sin \frac{\theta}{2}},\\
\sum\limits_{k=1}^n{\sin k\theta}&=\mathrm{Im}\sum\limits_{k=1}^n{e^{\mathrm{i}k\theta}}=\frac{\cos \frac{\theta}{2}-\cos \left( n+\frac{1}{2} \right) \theta}{2\sin \frac{\theta}{2}}.
\end{align*}

\item 由\refpro{proposition:n次复根}知
\begin{align*}
\frac{z^n-1}{z-1}=\prod_{k=1}^{n-1}{\left( z-\cos \frac{2k\pi}{n}-\mathrm{i}\sin \frac{2k\pi}{n} \right)},
\end{align*}
令$z\to 1$,利用\hyperref[theorem:L'Hospital(洛必达)法则]{L'Hospital法则}可得
\begin{align*}
n = \prod_{k=1}^{n-1}{\left( 1-\cos \frac{2k\pi}{n}-\mathrm{i}\sin \frac{2k\pi}{n} \right)}.
\end{align*}
两边取模得
\begin{align*}
n&=\left| \prod_{k=1}^{n-1}{\left( 1-\cos \frac{2k\pi}{n}-\mathrm{i}\sin \frac{2k\pi}{n} \right)} \right|=\prod_{k=1}^{n-1}{\left| 1-\cos \frac{2k\pi}{n}-\mathrm{i}\sin \frac{2k\pi}{n} \right|}
\\
&=\prod_{k=1}^{n-1}{\sqrt{2-2\cos \frac{2k\pi}{n}}}=\prod_{k=1}^{n-1}{\sqrt{4\sin ^2\frac{k\pi}{n}}}
\\
&=\prod_{k=1}^{n-1}{\left( 2\sin \frac{k\pi}{n} \right)}=2^{n-1}\prod_{k=1}^{n-1}{\sin \frac{k\pi}{n}}.
\end{align*}
故
\begin{align*}
\sin\frac{\pi}{n}\sin\frac{2\pi}{n}\cdots\sin\frac{(n-1)\pi}{n}=\frac{n}{2^{n-1}}.
...
\end{theorem}
\begin{proof}
由\refthe{theorem:三角函数的性质}易证.

\end{proof}

\begin{theorem}\label{theorem:cot三角函数的一些基本性质}
\begin{enumerate}[(1)]
\item\label{theorem:cot三角函数的一些基本性质-1} 设$0 < \theta < \frac{\pi}{2}, P_m(x) = \sum\limits_{k=0}^{m} (-1)^k\binom{2m+1}{2k+1}x^{m-k}.$证明：
\begin{align*}
\sin(2m + 1)\theta = \sin^{2m+1}\theta P_m(\cot^2\theta).
\end{align*}

\item\label{theorem:cot三角函数的一些基本性质-2} 记$P_m(x) = \sum\limits_{k=0}^{m} (-1)^k\binom{2m+1}{2k+1}x^{m-k}$,证明：
\begin{align*}
\sum\limits_{k=1}^{m} \cot^2 \frac{k\pi}{2m + 1} = \frac{m(2m - 1)}{3};\quad \prod\limits_{k=1}^{m} \cot^2 \frac{k\pi}{2m + 1} = \frac{1}{2m + 1}.
\end{align*}
\end{enumerate}
\end{theorem}
\begin{proof}
...
\item 考虑
\begin{align*}
\cos \left( 2m+1 \right) \theta +\mathrm{i}\sin \left( 2m+1 \right) \theta =e^{\mathrm{i}(2m+1)\theta}=(e^{\mathrm{i}\theta})^{2m+1}=\left( \cos \theta +\mathrm{i}\sin \theta \right) ^{2m+1},
\end{align*}
展开取其虚部
\begin{align*}
\sin(2m + 1)\theta = \mathrm{Im}\left[ \sum\limits_{k=0}^{2m+1} \binom{2m + 1}{k} (\mathrm{i}\sin\theta)^k(\cos\theta)^{2m+1-k} \right]
\end{align*}
仅当$k$为奇数时虚部非零。令$k = 2j + 1$，得
\begin{align*}
\sin(2m + 1)\theta = \sum\limits_{j=0}^{m}(-1)^j \binom{2m + 1}{2j + 1} \sin^{2j+1}\theta \cos^{2m-2j}\theta
\end{align*}
提取$\sin\theta$整理得
\begin{align*}
\sin(2m + 1)\theta = \sin^{2m+1}\theta \cdot \sum\limits_{j=0}^{m}(-1)^j \binom{2m + 1}{2j + 1} \cot^{2(m-j)}\theta = \sin^{2m+1}\theta P_m(\cot^2 \theta).
\end{align*}

\item 当$\theta = \frac{k\pi}{2m+1}$时，$\sin(2m+1)\theta = 0$且$\sin\theta \neq 0$，故由\refthe{theorem:cot三角函数的一些基本性质}{theorem:cot三角函数的一些基本性质-1}知$P_m(\cot^2 \theta) = 0$。则$\cot^2 \frac{k\pi}{2m+1}$为多项式$P_m(x)$的根。展开有
\begin{align*}
P_m(x) = (2m + 1)x^m - \binom{2m + 1}{3}x^{m-1} + \cdots + (-1)^m
\end{align*}
根据Vieta定理，根的和为
\begin{align*}
\sum\limits_{k=1}^{m} \cot^2 \frac{k\pi}{2m + 1} = \frac{\binom{2m+1}{3}}{2m + 1} = \frac{m(2m - 1)}{3}
\end{align*}

多项式$P_m(x)$的常数项为$(-1)^m$，首项系数为$2m + 1$。根据Vieta定理，根的乘积为
\begin{align*}
\prod\limits_{k=1}^{m} \cot^2 \frac{k\pi}{2m + 1} = \frac{(-1)^m \cdot (-1)^m}{2m + 1} = \frac{1}{2m + 1}.
\end{align*}
\end{enumerate}
...
\begin{definition}
设$L$是$\mathbb{C} \setminus \{0\}$内一条简单曲线,$z_0$是$L$的起点,$z_1$是$L$的终点.当$z$沿$L$从$z_0$连续变动到$z_1$时,$\overrightarrow{Oz}$所旋转的角称作$\mathrm{Arg}\, z$在$L$上的改变量,简称\textbf{辐角改变量},记作$\Delta_L \mathrm{Arg}\, z$.显然必存在$k\in \mathrm{Z}$,使得
\begin{align*}
\Delta _L\mathrm{Arg}z=\mathrm{arg}z_2-\mathrm{arg}z_1+2k\pi .
\end{align*}
\end{definition}
...
\begin{enumerate}[(1)]
\item $\Delta _C\mathrm{Arg}(z_1z_2)=\Delta _C\mathrm{Arg}z_1+\Delta _C\mathrm{Arg}z_2.$

\item $\Delta _C\mathrm{Arg}\left( \frac{z_1}{z_2} \right) =\Delta _C\mathrm{Arg}z_1-\Delta _C\mathrm{Arg}z_2.$

\item $\Delta _C\mathrm{Arg}\left( \alpha z \right) =\Delta _C\mathrm{Arg}z\left( \alpha >0 \right) ,\quad \Delta _C\mathrm{Arg}z^n=n\Delta _C\mathrm{Arg}z\left( n\in \mathbb{N} \right) ,\quad \Delta _C\mathrm{Arg}\sqrt[n]{z}=\frac{\Delta _C\mathrm{Arg}z}{n}\left( n\in \mathbb{N} \right) .$
\end{enumerate}
\end{theorem}
\begin{proof}
...
\begin{theorem}
设$f(z)$是一个多值函数的某个单值全纯分支,$C$是起点为$z_1$,终点为$z_2$且不穿过支割线的连续曲线.当$z$从$z_1$沿曲线$C$到终点$z_2$时,$f(z)$的辐角的连续改变量为$\Delta_C \mathrm{Arg} f(z)$,则
\begin{align*}
f(z_2) = |f(z_2)| \mathrm{e}^{\mathrm{i}\Delta_C \mathrm{Arg} f(z)} \cdot \mathrm{e}^{\mathrm{i}\mathrm{arg} f(z_1)},
\end{align*}
其中$\Delta_C \mathrm{Arg} f(z)$与$\mathrm{arg} f(z_1)$的取值无关.
\end{theorem}
\begin{proof}
不妨设
\begin{align*}
\Delta_C \mathrm{Arg} f(z) &= \mathrm{arg} f(z_2) - \mathrm{arg} f(z_1) + 2k_0\pi, \quad k_0 \in \mathbb{Z}.
\end{align*}
显然对$\forall k_1 \in \mathbb{Z}$,再记$k = k_1 - k_0$,则
\begin{align*}
f(z_2) &= |f(z_2)| \mathrm{e}^{\mathrm{i}\left[ \mathrm{arg} f(z_2) + 2k_1\pi \right]} = |f(z_2)| \mathrm{e}^{\mathrm{i}\left[ \mathrm{arg} f(z_2) - \mathrm{arg} f(z_1) + 2k_0\pi + \mathrm{arg} f(z_1) + 2\left( k_1 - k_0 \right)\pi \right]} \\
&= |f(z_2)| \mathrm{e}^{\mathrm{i}\Delta_C \mathrm{Arg} f(z)} \cdot \mathrm{e}^{\mathrm{i}\left( \mathrm{arg} f(z_1) + 2k\pi \right)}=|f(z_2)| \mathrm{e}^{\mathrm{i}\Delta_C \mathrm{Arg} f(z)} \cdot \mathrm{e}^{\mathrm{i} \mathrm{arg} f(z_1)}.
\end{align*}
于是
\begin{align*}
f(z_2) = |f(z_2)| \mathrm{e}^{\mathrm{i}\Delta_C \mathrm{Arg} f(z)} \cdot \mathrm{e}^{\mathrm{i}\left( \mathrm{arg} f(z_1) + 2k\pi \right)}.
\end{align*}
