...
\label{figure:图5.2}
\end{figure}
如\reffig{figure:图5.2}所示,取充分大的 \( R \),使得 \( a_1, \cdots, a_n \) 包含在半圆盘 \( \{ z: |z| < R, \mathrm{Im}z > 0 \} \) 中,记 \( \gamma_R = \{ z: z = R\mathrm{e}^{\mathrm{i}\theta}, 0 \leqslant \theta \leqslant \pi \} \),由\hyperref[theorem:留数定理(残数定理)-定理5.4.9]{留数定理}得
\begin{align}
...
\end{example}
\begin{solution}
令\( f(z) = \frac{1}{b^2 + z^2} \)，它满足\refthe{theorem:定理5.5.6}的条件。因为\( \frac{\mathrm{e}^{\mathrm{i}az}}{b^2 + z^2} \)在上半平面中只有一个1阶极点\( z = b\mathrm{i} \)，且
\[
\mathrm{Res}\left( \frac{\mathrm{e}^{\mathrm{i}az}}{b^2 + z^2}, b\mathrm{i} \right) = \frac{\mathrm{e}^{-ab}}{2b\mathrm{i}},
\]
...
\end{lemma}
\begin{remark}
遇到$f$在实轴上有奇点的情况时，常要使用这个引理,算一小段半圆或圆弧的积分.
\end{remark}
\begin{proof}
...
\end{figure}
\begin{solution}
取函数\( f(z) = \frac{\mathrm{e}^{\mathrm{i}z}}{z} \)，取围道如\reffig{figure:图5.3}所示，它由线段\([-R, -\rho]\)，\([\rho, R]\)和半圆周\(\gamma_{\rho}\)，\(\gamma_R\)组成。在此围道围成的区域中，\( f \)是全纯的，因而由\hyperref[theorem:Cauchy-Goursat定理(Cauchy积分定理)]{Cauchy积分定理}得
\begin{align}
...
\[
z^{p - 1} = \mathrm{e}^{(p - 1)\ln z}
\]
是一个多值函数。在复平面上，取正实轴作割线得一区域，\( z^{p - 1} \)在这个区域中能分出单值的全纯分支。今取定在正实轴上沿取实值的那个全纯分支，即主支：\( z^{p - 1} = \mathrm{e}^{(p - 1)\ln z} \)。让\( f(z) = \frac{\mathrm{e}^{(p - 1)\ln z}}{(1 + z)^m} \)沿如下的闭曲线\( \Gamma \)积分：先沿正实轴的上沿从\( \rho \)到\( R \)（\( 0 < \rho < 1 < R < \infty \)），再按反时针方向，沿以原点为中心、\( R \)为半径的圆周\( \gamma_R \)回到出发处，再沿正实轴的下沿从\( R \)到\( \rho \)，最后按顺时针方向沿以原点为中心、\( \rho \)为半径的圆周\( \gamma_{\rho} \)回到原来的出发处（\reffig{figure:图5.4}）。在正实轴上沿，有
\[
f(z) = \frac{\mathrm{e}^{(p - 1)\ln x}}{(1 + x)^m} = \frac{x^{p - 1}}{(1 + x)^m};
\]
在正实轴下沿，由于
\[
\ln z = \ln|z| + 2\pi \mathrm{i},
\]
所以
\[
\mathrm{e}^{(p - 1)\ln z} = \mathrm{e}^{(p - 1)(\ln x + 2\pi \mathrm{i})}
...
显然，\( f \)在由\( \Gamma \)围成的区域中只有一个\( m \)阶极点\( z = -1 \)。由\hyperref[theorem:留数定理(残数定理)-定理5.4.9]{留数定理}，有
\begin{align}
...
\end{align}
当\( z \in \gamma_R \)时，\( z = R\mathrm{e}^{\mathrm{i}\theta} \)，\( \ln z = \ln R + \mathrm{i}\theta \)，所以
\[
\frac{|z^{p - 1}|}{|1 + z|^m} = \frac{|\mathrm{e}^{(p - 1)\ln z}|}{|1 + z|^m} \leqslant \frac{R^{p - 1}}{(R - 1)^m}.
\]
...
\]
由\refpro{proposition:命题5.4.2},容易算出，当\( m = 1 \)时
\[
...
计算积分
\[
\int_{0}^{\infty} \frac{\ln x}{(1 + x^2)^2} \mathrm{d}x.
\]
\end{example}
\begin{solution}
{\color{blue}解法一:}取函数\( f(z) = \frac{\ln^2 z}{(1 + z^2)^2} \)，取围道如\reffig{figure:图5.4}所示。在正实轴的上沿，有
\[
f(z) = \frac{\ln^2 x}{(1 + x^2)^2};
\]
在正实轴的下沿，由于\( \ln z = \ln x + 2\pi \mathrm{i} \)，所以
\[
\ln^2 z = (\ln x + 2\pi \mathrm{i})^2
= \ln^2 x + 4\pi \mathrm{i} \ln x - 4\pi^2,
\]
因而
\[
f(z) = \frac{\ln^2 x}{(1 + x^2)^2} + 4\pi \mathrm{i} \frac{\ln x}{(1 + x^2)^2} - 4\pi^2 \frac{1}{(1 + x^2)^2}.
\]
\( f \)在\( \Gamma \)所围成的区域中有两个2阶极点\( z = \pm \mathrm{i} \)。对\( f \)用\hyperref[theorem:留数定理(残数定理)-定理5.4.9]{留数定理}，得
\begin{align}
&\int_{\rho}^{R} \frac{\ln^2 x}{(1 + x^2)^2} \mathrm{d}x + \int_{\gamma_R} \frac{\ln^2 z}{(1 + z^2)^2} \mathrm{d}z + \int_{R}^{\rho} \frac{\ln^2 x}{(1 + x^2)^2} \mathrm{d}x
\nonumber\\
&+ 4\pi \mathrm{i} \int_{R}^{\rho} \frac{\ln x}{(1 + x^2)^2} \mathrm{d}x - 4\pi^2 \int_{R}^{\rho} \frac{\mathrm{d}x}{(1 + x^2)^2} + \int_{\gamma_{\rho}} \frac{\ln^2 z}{(1 + z^2)^2} \mathrm{d}z
\nonumber\\
&= 2\pi \mathrm{i} \left[ \mathrm{Res}\left( \frac{\ln^2 z}{(1 + z^2)^2}, \mathrm{i} \right) + \mathrm{Res}\left( \frac{\ln^2 z}{(1 + z^2)^2}, -\mathrm{i} \right) \right]. \label{ex5.5.12_eq1}
\end{align}
\(\eqref{ex5.5.12_eq1}\)式左端的第一个和第三个积分互相抵消了。\( \gamma_R \)和\( \gamma_{\rho} \)上两个积分的估计与\refexa{example:例5.5.11}一样：
\[
\left| \int_{\gamma_R} \frac{\ln^2 z}{(1 + z^2)^2} \mathrm{d}z \right| = \left| \int_{0}^{2\pi} \frac{(\ln R + \mathrm{i}\theta)^2}{(1 + R^2 \mathrm{e}^{2\mathrm{i}\theta})^2} R\mathrm{i}\mathrm{e}^{\mathrm{i}\theta} \mathrm{d}\theta \right|
\leqslant 2\pi R \frac{(\ln R + 2\pi)^2}{(R^2 - 1)^2}
\to 0 \ (R \to \infty),
\]
\[
\left| \int_{\gamma_{\rho}} \frac{\ln^2 z}{(1 + z^2)^2} \mathrm{d}z \right| = \left| \int_{0}^{2\pi} \frac{(\ln \rho + \mathrm{i}\theta)^2}{(1 + \rho^2 \mathrm{e}^{2\mathrm{i}\theta})^2} \rho\mathrm{i}\mathrm{e}^{\mathrm{i}\theta} \mathrm{d}\theta \right|
\leqslant 2\pi \rho \frac{(\ln \rho + 2\pi)^2}{(1 - \rho^2)^2}
\to 0 \ (\rho \to 0).
\]
直接计算留数，得
\[
\mathrm{Res}\left( \frac{\ln^2 z}{(1 + z^2)^2}, \mathrm{i} \right) = \frac{-4\pi + \pi^2 \mathrm{i}}{16},
\quad
\mathrm{Res}\left( \frac{\ln^2 z}{(1 + z^2)^2}, -\mathrm{i} \right) = \frac{12\pi - 9\pi^2 \mathrm{i}}{16}.
\]
在\(\eqref{ex5.5.12_eq1}\)式中令\( \rho \to 0 \)，\( R \to \infty \)，并取两端的虚部，即得
\[
\int_{0}^{\infty} \frac{\ln x}{(1 + x^2)^2} \mathrm{d}x = -\frac{\pi}{4}.
\]

\begin{figure}[H]
\centering
\includegraphics[width=0.4\textwidth]{图5.5.png}
\caption{}
\label{figure:图5.5}
\end{figure}
{\color{blue}解法二:}
在计算过程中我们发现，如果取\( f(z) = \frac{\ln z}{(1 + z^2)^2} \)，则所需计算的积分将被抵消掉，这是取\( f(z) = \frac{\ln^2 z}{(1 + z^2)^2} \)的原因。但若改变围道如\reffig{figure:图5.5}所示，那么取\( f(z) = \frac{\ln z}{(1 + z^2)^2} \)也是可以的。这时，\( f \)在\( \Gamma \)围成的区域中只有一个2阶极点\( z = \mathrm{i} \)。当\( z \in [-R, -\rho] \)时，\( \ln z = \ln|x| + \mathrm{i}\pi \)。对\( f \)在\( \Gamma \)上应用\hyperref[theorem:留数定理(残数定理)-定理5.4.9]{留数定理}，可得
\begin{align}
&\int_{-R}^{-\rho}{\frac{\ln |x|}{(1+x^2)^2}\mathrm{d}x}+\mathrm{i}\pi \int_{-R}^{-\rho}{\frac{\mathrm{d}x}{(1+x^2)^2}}+\int_{\gamma _{\rho}}{\frac{\ln z}{(1+z^2)^2}\mathrm{d}z}\nonumber
\\
&\quad \quad+\int_{\rho}^R{\frac{\ln x}{(1+x^2)^2}\mathrm{d}x}+\int_{\gamma _R}{\frac{\ln z}{(1+z^2)^2}\mathrm{d}z}\nonumber
\\
&=2\pi \mathrm{iRes}\left( \frac{\ln z}{(1+z^2)^2},\mathrm{i} \right) . \label{ex5.5.12_eq2}
\end{align}
与上面的做法一样，可证
\[
\lim_{R \to \infty} \int_{\gamma_R} \frac{\ln z}{(1 + z^2)^2} \mathrm{d}z = 0,
\quad
\lim_{\rho \to 0} \int_{\gamma_{\rho}} \frac{\ln z}{(1 + z^2)^2} \mathrm{d}z = 0,
\]
而
\[
\mathrm{Res}\left( \frac{\ln z}{(1 + z^2)^2}, \mathrm{i} \right) = \frac{\pi}{8} + \frac{\mathrm{i}}{4}.
\]
在\(\eqref{ex5.5.12_eq2}\)式两端令\( \rho \to 0 \)，\( R \to \infty \)，得
\[
2\int_{0}^{\infty} \frac{\ln x}{(1 + x^2)^2} \mathrm{d}x - \mathrm{i}\pi \int_{0}^{\infty} \frac{\mathrm{d}x}{(1 + x^2)^2} = 2\pi \mathrm{i} \left( \frac{\pi}{8} + \frac{\mathrm{i}}{4} \right),
\]
两边取实部，即得
\[
\int_{0}^{\infty} \frac{\ln x}{(1 + x^2)^2} \mathrm{d}x = -\frac{\pi}{4}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.4\textwidth]{图5.6.png}
\caption{}
\label{figure:图5.6}
\end{figure}
...
\arg(z - a) = 0, \arg(b - z) = 0
\]
的一支来讨论。取\( R \)充分大，\( \varepsilon \)充分小，使得由圆周\( \Gamma = \{ z : |z| = R \} \)的内部以及圆周\( \gamma_1 = \{ z : |z - a| = \varepsilon \} \)和圆周\( \gamma_2 = \{ z : |z - b| = \varepsilon \} \)的外部所构成的域\( D \)包含\( f \)的全部奇点\( a_1, \cdots, a_n \)（见\reffig{figure:图5.6}）。在区域\( D \)上对函数\( F \)用\hyperref[theorem:留数定理(残数定理)-定理5.4.9]{留数定理}，得
\begin{align}
...
\begin{gather*}
(z - a)^r = \mathrm{e}^{r \ln(z - a)} = \mathrm{e}^{r \ln|z - a|}
= \mathrm{e}^{r \ln(x - a)} = (x - a)^r,
\\
(b - z)^s = \mathrm{e}^{s \ln(b - z)} = \mathrm{e}^{s \ln|b - z|}
= \mathrm{e}^{s \ln(b - x)} = (b - x)^s.
\end{gather*}
...
\]
\[
(b - z)^s = \mathrm{e}^{s(\ln(b - x) + \mathrm{i} \arg(b - z))}
...
\begin{proposition}\label{proposition:几个特殊积分的计算}
\begin{enumerate}[(1)]
\item\label{proposition:几个特殊积分的计算-Fresnel积分} $\mathbf{Fresnel}$\textbf{(菲涅耳)积分:}\(\int_{0}^{\infty} \cos x^2 \mathrm{d}x=\int_{0}^{\infty} \sin x^2 \mathrm{d}x=\sqrt{\frac{\pi}{8}}\).

\item\label{proposition:几个特殊积分的计算-Poisson积分} $\mathbf{Poisson}$\textbf{(泊松)积分:}\(\int_{0}^{\infty} \mathrm{e}^{-ax^2} \cos bx \mathrm{d}x = \frac{1}{2} \sqrt{\frac{\pi}{a}} \mathrm{e}^{-\frac{b^2}{4a}} \ (a > 0).\)
\end{enumerate}
\end{proposition}
\begin{proof}\begin{enumerate}[(1)]
\item 取函数\( f(z) = \mathrm{e}^{\mathrm{i}z^2} \)，取围道如\reffig{figure:图5.7}所示。因为\( f \)是整函数，由\hyperref[theorem:Cauchy-Goursat定理(Cauchy积分定理)]{Cauchy积分定理}，有
\begin{align}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.4\textwidth]{图5.7.png}
\caption{}
\label{figure:图5.7}
\end{figure}
不难利用计算这两个积分的方法算出
...
\item 取函数\( f(z) = \mathrm{e}^{-az^2} \)，取围道如\reffig{figure:图5.8}所示。因为\( f \)是整函数，由\hyperref[theorem:Cauchy-Goursat定理(Cauchy积分定理)]{Cauchy积分定理}，有
\begin{align}
...
\begin{align*}
\int_{\gamma _2}{\mathrm{e}^{-az^2}\mathrm{d}z}&=-\int_{-R}^R{\mathrm{e}^{-a\left( x^2-\frac{b^2}{4a^2}+\frac{b}{a}x\mathrm{i} \right)}\mathrm{d}x}=-\mathrm{e}^{\frac{b^2}{4a}}\int_{-R}^R{\mathrm{e}^{-ax^2}\mathrm{e}^{-bx\mathrm{i}}\mathrm{d}x}
\\
&=-\mathrm{e}^{\frac{b^2}{4a}}\int_{-R}^R{\mathrm{e}^{-ax^2}\cos bx\mathrm{d}x}+\mathrm{ie}^{\frac{b^2}{4a}}\int_{-R}^R{\mathrm{e}^{-ax^2}\sin bx\mathrm{d}x}
\\
...
\]
由\hyperref[Basis of Analytics-proposition:重要定积分结论-1]{Gauss-Poisson积分}可得
\[
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\textwidth]{图5.8.png}
\caption{}
\label{figure:图5.8}
\end{figure}
\end{enumerate}
