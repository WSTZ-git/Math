...
\begin{theorem}[Laurent定理]\label{theorem:定理5.1.2}
设 \( D = \{ z: r < |z - z_0| < R \} (0\leqslant r<R\leqslant +\infty)\)，如果 \( f \in H(D) \)，那么 \( f \) 在 \( D \) 上可以展开为 Laurent 级数：
\begin{align}
f(z) = \sum_{n = -\infty}^{\infty} a_n (z - z_0)^n, \, z \in D, \label{eq:1111111111211111111}
\end{align}
其中
\begin{align*}
a_n = \frac{1}{2\pi \mathrm{i}} \int_{\gamma_{\rho}} \frac{f(\zeta)}{(\zeta - z_0)^{n + 1}} \mathrm{d}\zeta,\,\,n\in \mathbb{Z},
\end{align*}
而 \( \gamma_{\rho} = \{ \zeta: |\zeta - z_0| = \rho \} \)\(( r < \rho < R) \).并且展开式 \eqref{eq:1111111111211111111} 是唯一的。
\end{theorem}
\begin{proof}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.3]{图5.1.png}
\caption{}
\label{figure:图5.1}
\end{figure}
...
\end{align}
由\hyperref[theorem:定理3.2.5]{多连通区域的 Cauchy 积分定理}，得
\begin{align*}
...
\begin{align*}
\frac{1}{2\pi \mathrm{i}} \int_{\gamma_{\rho}} \frac{f(\zeta)}{(\zeta - z_0)^{m + 1}} \mathrm{d}\zeta = \sum_{n = -\infty}^{\infty} a_n' \frac{1}{2\pi \mathrm{i}} \int_{\gamma_{\rho}} (\zeta - z_0)^{n - m - 1} \mathrm{d}\zeta \xlongequal{\text{\refpro{proposition:例3.1.4}}} a_m',
\end{align*}
