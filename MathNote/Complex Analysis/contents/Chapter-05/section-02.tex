...
\end{align*}
而 \( \gamma_{\rho} = \{ \zeta: |\zeta| = \rho \} \)\( (0 < \rho <\frac{1}{R} )\).
...
\end{align*}
而 \( \gamma_{\rho} = \{ \zeta: |\zeta| = \rho \} \)\(( 0 < \rho < \frac{1}{R}) \).于是\( f \) 在 \( N\setminus \{\infty\}\) 中有下面的Laurent展开:
\[
...
\end{align*}
且$0 < \rho <\frac{1}{R},\gamma _{\rho}=\left\{ \zeta :\left| \zeta \right|=\rho \right\}$.
