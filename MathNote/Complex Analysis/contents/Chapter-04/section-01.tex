...
\end{note}
\begin{proof}
由\refthe{theorem:紧集和闭集无交则距离大于0}，\( \rho = d(K, \partial G) > 0 \)。所以，以 \( K \) 中任意点 \( a \) 为中心、\( \rho \) 为半径的圆盘都包含在 \( G \) 中。对圆盘 \( B(a, \rho) \) 用 \hyperref[theorem:Cauchy不等式(离散版)-复变函数]{Cauchy 不等式}，得
\begin{align*}
...
由条件知,序列$\{f_n(z)\}$在$E$上一致有界,即在$E$上$|f_n(z)|\leqslant M$.设点$z_1,z_2\in \Pi$且$|z_1-z_2|<\frac{\rho}{4}$;用$\Gamma=\{\xi :|\xi-z_1|=\frac{\rho}{2}\}$.显然$\Gamma$的内部包含在$E$中.由\hyperref[theorem:复变-Cauchy积分公式]{Cauchy积分公式}得到
\begin{align*}
