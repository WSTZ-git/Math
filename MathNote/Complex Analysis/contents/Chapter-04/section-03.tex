...
\begin{corollary}\label{corollary:复变函数--推论4.3.2}
\begin{enumerate}[(1)]
\item 若\( f \)在点\( z_0 \)处全纯，则\( f \)在\( z_0 \)的邻域内可展开成Taylor级数：
\begin{align*}
f(z) = \sum_{n=0}^{\infty} \frac{f^{(n)}(z_0)}{n!} (z - z_0)^n, \quad \forall z \in B(z_0, R).
\end{align*}

\item 若\( f \)在\( z_0 \)的邻域内可展开成幂级数：
\begin{align*}
f(z) = \sum_{n=0}^{\infty} a_n (z - z_0)^n, \quad \forall z \in B(z_0, R).
\end{align*}
则\( f \)在点\( z_0 \)处全纯且\( a_n = \frac{f^{(n)}(z_0)}{n!} \)，\( \forall n \in \mathbb{N} \).
\end{enumerate}
\end{corollary}
\begin{proof}
...
\item $\ln(1 + z) = \sum_{n=1}^{\infty} (-1)^{n - 1} \frac{z^n}{n}, \quad |z| < 1.$

\item $e^{\alpha \ln(1 + z)} = \sum_{n=0}^{\infty} \binom{\alpha}{n} z^n, \quad \alpha \in \mathbb{R},|z| < 1.$
\end{enumerate}
\end{proposition}
\begin{proof}
\begin{enumerate}[(1)]
...
\item 函数 \( f(z) = (1 + z)^{\alpha} \)，\( \alpha \) 不是整数，我们考虑它的主支 \( f(z) = e^{\alpha \ln(1 + z)} \) 在 \( z = 0 \) 处的 Taylor 展开式. 这个分支在 \( z = 0 \) 处的值为 1，它的各阶导数在 \( z = 0 \) 处的值为
\[
