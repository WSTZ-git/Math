...
\begin{example}
设$f(z)=\sum\limits_{n=0}^{\infty}a_n z^n$, $|z|<R$, 证明$f(z)$在收敛圆盘的任一点处有幂级数展开.
\end{example}
\begin{proof}
注意到
\begin{align*}
f(z) &= f(z-z_0+z_0) = \sum\limits_{n=0}^{\infty}a_n[(z-z_0)+z_0]^n \\
&= \sum\limits_{n=0}^{\infty}a_n\sum\limits_{k=0}^{n}\binom{n}{k}z_0^{n-k}(z-z_0)^k \\
&= \sum\limits_{k=0}^{\infty}(z-z_0)^k\sum\limits_{n=k}^{\infty}a_n\binom{n}{k}z_0^{n-k}.
\end{align*}
记$b_k=\sum\limits_{n=k}^{\infty}a_n\binom{n}{k}z_0^{n-k},k\in \mathbb{N}$,则
\begin{align*}
f(z)=\sum\limits_{k=0}^{\infty}b_k(z-z_0)^k.
\end{align*}
故结论得证.
