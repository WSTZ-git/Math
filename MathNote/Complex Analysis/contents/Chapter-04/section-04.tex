...
\[
\frac{1}{2\pi \mathrm{i}} \int_{\Gamma} \frac{1}{w} dw = 
\begin{cases} 
1, & \text{如果原点在 } \Gamma \text{ 内部}; \\
0, & \text{如果原点不在 } \Gamma \text{ 内部}.
\end{cases}
...
于是得到
\begin{align}
\frac{1}{2\pi \mathrm{i}} \int_{\Gamma} \frac{dw}{w} = \frac{1}{2\pi} \Delta_{\Gamma} \text{Arg} w. \label{eq:arg_int}
\end{align}
一般来说，当 \( \Gamma \) 是一条不通过原点的任意可求长闭曲线时，\( \frac{1}{2\pi \mathrm{i}} \int_{\Gamma} \frac{dw}{w} \) 等于 \( \Gamma \) 绕原点的圈数，称为 \( \Gamma \) 关于原点的\textbf{环绕指数}，因而 \(\eqref{eq:arg_int}\) 式对于一般的不通过原点的可求长闭曲线都是成立的.
