...
\begin{proposition}\label{proposition:复变例1.7}
设连续映射\(f:E \to \bm{C}\)满足\(f(z) \neq f(w), \forall z,w \in E, z \neq w,\)则称\(f\)是\(E\)上的一一连续映射. 证明: 若\(E\)是紧集, \(f\)是\(E\)上的一一连续映射, 则\(f^{-1}: f(E) \to E\)也是一一连续映射, 即\(f:E \to f(E)\)是一个同胚映射. 将紧集换成闭集, 结论是否成立?
\end{proposition}
