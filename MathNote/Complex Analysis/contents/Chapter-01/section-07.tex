...
\begin{definition}
设$L$是$\mathbb{C} \setminus \{0\}$内一条简单曲线,$z_0$是$L$的起点,$z_1$是$L$的终点.当$z$沿$L$从$z_0$连续变动到$z_1$时,$\overrightarrow{Oz}$所旋转的角称作$\mathrm{Arg}\, z$在$L$上的改变量,简称\textbf{辐角改变量},记作$\Delta_L \mathrm{Arg}\, z$.显然有
\begin{align*}
\Delta _L\mathrm{Arg}z=-\Delta _{L^-}\mathrm{Arg}z.
\end{align*}
并且必存在$k\in \mathrm{Z}$,使得
\begin{align*}
