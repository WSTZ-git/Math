...
\item\label{proposition:复数运算性质-5} $(z+w)^2=|z|^2+|w|^2+2\text{Re}(z\overline{w})=z|^2+|w|^2+2(z\cdot w)=|z|^2+|w|^2+2|z||w|\cos\left< z,w \right> .$

进而$\left| z_1+z_2 \right|^2+\left| z_1-z_2 \right|^2=2\left( \left| z_1 \right|^2+\left| z_2 \right|^2 \right) .$

\item\label{proposition:复数运算性质-6} 若$|z|=\lambda|w|$,$\lambda>0$,则$|z-\lambda^2 w|=\lambda|z-w|.$
\end{enumerate}
\end{proposition}
\begin{note}
\rrefpro{proposition:复数运算性质}{proposition:复数运算性质-5}第二个结论的几何意义是:平行四边形的对角线的平方和等于四条边的平方和.
\end{note}
\begin{proof}
\begin{enumerate}[(1)]
\item 
...
\item 由\rrefpro{proposition:复数运算性质}{proposition:复数运算性质-1}和\rrefpro{proposition:复数运算性质}{proposition:复数运算性质-2}可得$|z + w|^2 = (z + w)(\overline{z + w}) = |z|^2 + 2\mathrm{Re}(z\overline{w}) + |w|^2.$再设$z=x_1+\mathrm{i}y_1,w=x_2+\mathrm{i}y_2,$则
\begin{align*}
\mathrm{Re}z\overline{w}=\mathrm{Re}\overline{z}w=x_1x_2+y_1y_2=z\cdot w=\left| z \right|\left| w \right|\cos \left< z,w \right>.
\end{align*}
...
\end{lemma}
\begin{remark}
这个不等式中的系数$\frac{1}{\pi}$是对任意的$n$都成立的最佳系数($n$表示复数$z_1,z_2,\cdots,z_n$的个数).但是对某个具体的$n$成立的最佳系数还未可知.证明如下 :

设$c>\frac{1}{\pi}$, 取
\begin{align*}
z_k=e^{\frac{2\pi k\mathrm{i}}{n}},\quad \alpha _k=\frac{2\pi k}{n},\quad k=1,2,\cdots ,n.
\end{align*}
则$\sum\limits_{k=1}^n{\left| z_k \right|}=n$. 对$\{ 1,2,\cdots ,n \}$的任意子集$S$, 设$\theta =\mathrm{Arg}\left( \sum\limits_{j\in S}{z_j} \right)$, 则
\begin{align*}
\left| \sum\limits_{j\in S}{z_j} \right|&=\left| e^{-\mathrm{i}\theta}\sum\limits_{j\in S}{z_j} \right|=\left| \sum\limits_{j\in S}{e^{-\mathrm{i}\theta}z_j} \right| \\
&=\mathrm{Re}\left( \sum\limits_{j\in S}{e^{-\mathrm{i}\theta}z_j} \right) =\sum\limits_{j\in S}{\mathrm{Re}\left( e^{-\mathrm{i}\theta}z_j \right)} \\
&=\sum\limits_{j\in S}{\left| z_k \right|\cos \left( \alpha _k-\theta \right)}\leqslant \sum\limits_{k=1}^n{\cos ^+\left( \alpha _k-\theta \right)} \\
&\leqslant \max\limits_{\theta \in \left[ -\pi ,\pi \right]}\sum\limits_{k=1}^n{\cos ^+\left( \alpha _k-\theta \right)}.
\end{align*}
于是
\begin{align*}
\lim\limits_{n\rightarrow \infty}\frac{\left| \sum\limits_{j\in S}{z_j} \right|}{n}\leqslant \lim\limits_{n\rightarrow \infty}\frac{1}{n}\max\limits_{\theta \in \left[ -\pi ,\pi \right]}\sum\limits_{k=1}^n{\cos ^+\left( \alpha _k-\theta \right)}=\frac{1}{2\pi}\int_{-\pi}^{\pi}{\cos ^+\left( \alpha _k-\theta \right) \mathrm{d}\theta}=\frac{1}{\pi}<c.
\end{align*}
故存在$N\in \mathbb{N}$, 使得对$\forall S\subseteq \{ 1,2,\cdots ,n \}$, 都有
\begin{align*}
\frac{\left| \sum\limits_{j\in S}{z_j} \right|}{n}<c\Longleftrightarrow \left| \sum\limits_{j\in S}{z_j} \right|<c\sum\limits_{k=1}^n{\left| z_k \right|}.
\end{align*}
\end{remark}
\begin{proof}
