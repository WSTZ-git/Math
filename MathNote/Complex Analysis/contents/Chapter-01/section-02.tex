...
在平面上取定一个直角坐标系，实数对\((a,b)\)就表示平面上的一个点，所以复数\(z = a + b\mathrm{i}\)可以看成平面上以\(a\)为横坐标、以\(b\)为纵坐标的一个点（\reffig{figure:图1.1}）。这个点的极坐标设为\((r,\theta)\)，那么$a = r\cos\theta, \quad b = r\sin\theta.$
\begin{figure}[H]
\centering
