...
\end{definition}
\begin{remark}
由上面的定义可知，集\(E\)把复平面分成三个互不相交的部分：\(\mathbb{C} = E^\circ \cup (E^c)^\circ \cup \partial E\)，即
...
\begin{definition}
如果\(E\subseteq \mathbb{C}\)的所有点都是它的内点，即\(E = E^\circ\)，就称\(E\)为\textbf{开集}。

如果\(E^c=\mathbb{C}\setminus E\)是开集，就称\(E\)为\textbf{闭集}。
\end{definition}

\begin{example}
设$a\in \mathbb{C}$,则在$\mathbb{C}$中,\(B(a, r)\)是开集，闭圆盘\(\{ z : | z - a | \leqslant r \}\)是闭集，\(B(a, r)\)和它的上半圆周的并集既不是开集也不是闭集。
\end{example}
...
\begin{definition}
点集\(E\subseteq \mathbb{C}\)的\textbf{直径}定义为\(E\)中任意两点间距离的上确界，记为\(\mathrm{diam}E\)，即
\[
\mathrm{diam}E = \sup\{ | z_1 - z_2 | : z_1, z_2 \in E \}.
\]
\end{definition}

\begin{theorem}[Cantor闭集套定理]\label{theorem:复变函数--Cantor闭集套定理}
...
\begin{definition}
设\(E\subseteq \mathbb{C}\)是一个集，\(\mathscr{F} = \{ G \}\)是一个\textbf{开集族}，即\(\mathscr{F}\)中的每一个元素都是开集。

如果\(E\)中每一点至少属于\(\mathscr{F}\)中的一个开集，就说\(\mathscr{F}\)是\(E\)的一个\textbf{开覆盖}。
\end{definition}

\begin{example}
\(E\subseteq \mathbb{C}\)是任一点集，\(\varepsilon\)是一个给定的正数，那么
\[
...
\begin{definition}
我们说点集\(E\subseteq \mathbb{C}\)具有\textbf{有限覆盖性质}，是指从\(E\)的任一个开覆盖中必能选出有限个开集\(G_1, \cdots, G_n\)，使得这有限个开集的并就能覆盖\(E\)，即
\[
...
\begin{example}
空集和有限集都是紧集，但单位圆盘\(B(0, 1) = \{ z \in \mathbb{C} : | z | < 1 \}\)却不是紧集，因为\(G_n = \left\{ z : | z | < 1 - \frac{1}{n} \right\}\)，\(n = 2, 3, \cdots\)，这一串同心圆构成\(B(0, 1)\)的一个开覆盖，但从中找不出有限个集覆盖\(B(0, 1)\)。
\end{example}

\begin{definition}
集\(E\subseteq \mathbb{C}\)称为是\textbf{有界的}，如果存在\(R > 0\)，使得\(E \subset B(0, R)\)。
\end{definition}
...
{\color{blue}证法二:}定义函数$f:\mathbb{C}\to\mathbb{R}$为$f(z)=d(z,F):=\inf\limits_{w\in F}|z-w|$. 因$F$是闭集, 故$z\in F$当且仅当$f(z)=0$. 在不等式
\begin{align*}
|z_1-w|\leqslant |z_1-z_2|+|z_2-w|,\quad |z_2-w|\leqslant |z_2-z_1|+|z_1-w|,
\end{align*}
中关于$w\in F$取下确界得
\begin{align*}
f(z_1)\leqslant f(z_2)+|z_1-z_2|,\quad f(z_2)\leqslant f(z_1)+|z_1-z_2|.
\end{align*}
因此
\begin{align*}
|f(z_1)-f(z_2)|\leqslant|z_1-z_2|,\ z_1,z_2\in\mathbb{C}.
\end{align*}
这说明$f$是连续函数. 由\refthe{theorem:连续的复变函数在紧集上的性质}知, $f$限制在紧集$E$上可取到最小值$f(z_0)=d(E,F)$, $z_0\in E$. 因$E$与$F$不交, 故$z_0\notin F$.又因为$z\in F$当且仅当$f(z)=0$,所以$f(z_0)>0$.
...
\end{theorem}
\begin{remark}
这个定理也可以用证明\hyperref[theorem:复变函数--Cantor闭集套定理]{Cantor闭集套定理}的方法给出另一个证明。
\end{remark}
\begin{proof}
...
若$E$还有界,则$\infty$不是$E$的极限点,否则与$E$有界矛盾!故$E$至少有一个极限点$x_0\in \mathbb{C}$.
