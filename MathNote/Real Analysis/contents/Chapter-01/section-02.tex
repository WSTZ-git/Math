...
\end{align*}
则由\refproposition{proposition:点与集合之间的距离函数在R^n上一致连续}以及连续函数四则运算封闭性知 $f(x)$ 连续. 容易验证 $f(x)$ 满足条件 (i), (ii).
\end{proof}
...
\end{theorem}
\begin{proof}
由于 $F_1, F_2$ 是闭集, 且 $F_1 \cap F_2 = \varnothing$, 则 $\forall x \in F_1$ 都有 $d(x, F_2) > 0$; $\forall y \in F_2$ 都有 $d(y, F_1) > 0$. 由\reftheorem{theorem:距离大于零的两个集合中一定存在两个无交的子集}知结论成立.
\end{proof}
...
\end{align*}
由\refproposition{proposition:点与集合之间的距离函数在R^n上一致连续}以及连续函数四则运算封闭性, 知 $g_1(x)$ 在 $\mathbb{R}^n$ 上连续. 又容易验证
\begin{align*}
