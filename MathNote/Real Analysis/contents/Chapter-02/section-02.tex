...
若只有$E_1\in \mathscr{M}$,则$m^*(E_2\setminus E_1)=m^*(E_2)-m^*(E_1)$.
...
\begin{theorem}[测度的Fatou引理]\label{theorem:测度的Fatou引理}
\begin{enumerate}[(1)]
\item\label{theorem:测度的Fatou引理-1} 若有可测集列$\{E_k\}$，且有$\sum_{k = 1}^{\infty}m(E_k)<+\infty$，则
\begin{align*}
m\left(\varlimsup_{k\to\infty}E_k\right)= 0.
\end{align*}

\item\label{theorem:测度的Fatou引理-2} 设$\{E_k\}$是可测集列，则
\begin{align*}
...
\end{example}
\begin{proof}
由条件知$\bigcup\limits_{k=k_0}^{\infty}E_k\subseteq \left[ a,b \right]$,从而$m\left( \bigcup\limits_{k=k_0}^{\infty}E_k \right) \leqslant b-a<+\infty$.于是由\hyperref[theorem:测度的Fatou引理]{测度的Fatou引理}知
\begin{align*}
m\left( E \right) &=m\left( \overline{\lim\limits_{k\rightarrow \infty}}E_k \right) \geqslant \overline{\lim\limits_{k\rightarrow \infty}}m\left( E_k \right) \geqslant \underline{\lim\limits_{k\rightarrow \infty}}m\left( E_k \right) \geqslant m\left( \underline{\lim\limits_{k\rightarrow \infty}}E_k \right) =m\left( E \right).
\end{align*}
...
&= m\left( \bigcup_{\substack{d_{n_1},\dots,d_{n_k}\in\{1,\dots,9\} \\ d_i\in\{0,\dots,9\},\,i\neq n_1,\dots,n_k}} I_{d_1d_2\cdots d_N} \right) 
= \sum_{\substack{d_{n_1},\dots,d_{n_k}\in\{1,\dots,9\} \\ d_i\in\{0,\dots,9\},\,i\neq n_1,\dots,n_k}} m\left( I_{d_1d_2\cdots d_N} \right) \\
&= \sum_{\substack{d_{n_1},\dots,d_{n_k}\in\{1,\dots,9\} \\ d_i\in\{0,\dots,9\},\,i\neq n_1,\dots,n_k}} \frac{1}{10^N} 
...
\item $E$是闭集,进而是可测集.将$[0,1]$作$10$等分，并舍去除第$3$和$8$个子区间即$[\frac{2}{10},\frac{3}{10})$和$[\frac{7}{10},\frac{8}{10})$之外的区间，则在余下的点的小数表示式中第一位只会是$2$$7$.再将余下的区间又各$10$等分,又舍去除第$3$和$8$个子区间,$\cdots\cdots$如此进行下去之外的区间,$\cdots \cdots$,如此进行下去,就留下了$E$。易知舍去之区间总长为
\begin{align*}
...
此外，由$m(E_{p,q})=\frac{2}{q^{\alpha}}$可知，$m(E_q)\leqslant \frac{2q}{q^\alpha}=\frac{2}{q^{\alpha-1}}$。从而我们有$\sum\limits_{q=1}^{\infty}m(E_q)<+\infty$。因此由\rrefthe{theorem:测度的Fatou引理}{theorem:测度的Fatou引理-1}知$m\left(\varlimsup\limits_{q\to\infty}E_q\right)=0$，也就有$m(E)=0$.
