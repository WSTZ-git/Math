...
\begin{align*}
|T(x)|&\leqslant slant|\xi_1||x_1| + |\xi_2||x_2| + \cdots + |\xi_n||x_n|\\
&\leqslant slant\left(\sum_{i = 1}^{n}|x_i|^2\right)^{1/2}\left(\sum_{i = 1}^{n}|\xi_i|^2\right)^{1/2} = M|x|.
\end{align*}
由此可知
\begin{align*}
|T(y) - T(x)| = |T(y - x)|&\leqslant slant M|y - x|.
\end{align*}
...
记
\begin{align*}
I_0 &= \{x = (\xi_1,\xi_2,\cdots,\xi_n): 0\leqslant slant\xi_i < 1, 1\leqslant slant i\leqslant slant n\},\\
I &= \{x = (\xi_1,\xi_2,\cdots,\xi_n): 0\leqslant slant\xi_i < 2^{-k}, 1\leqslant slant i\leqslant slant n\}.
\end{align*}
...
\[
T(I_0) = \{x = (\xi_1,\xi_2,\cdots,\xi_n): 0\leqslant slant\xi_i < 1\ (i = 2,3,\cdots,n),\ 0\leqslant slant\xi_1 < \beta\ (\beta > 0), \beta < \xi_1\leqslant slant 0\ (\beta < 0)\}.
\]
从而可知 $m(T(I_0)) = |\beta|$, 即 \eqref{eq:thm225_1} 式成立.

在 (iii) 的情形, 此时 $\det T = 1$, 而且有
\[
T(I_0) = \{x = (\xi_1,\xi_2,\cdots,\xi_n): 0\leqslant slant\xi_i < 1\ (i\neq 1),\ 0\leqslant slant\xi_1 - \xi_2 < 1\}.
\]
记
\begin{align*}
...
\begin{example}
圆盘 \(D = \{ (x,y): x^2 + y^2 \leqslant slant r^2\}\) 是 \(\mathbb{R}^2\) 中可测集，且 \(m(D)=\pi r^2\).
\end{example}
\begin{proof}
...
设 \(E \subset (-\pi, \pi]\)，\(0 \leqslant slant a < b \leqslant slant +\infty\)，令
\[
...
(1) 对于一般点集 \(E\)，我们有
\[
m^*(S) \leqslant slant \frac{(b^2 - a^2)m^*(E)}{2}.
\]
...
\begin{align*}
m^*(S_E) &\leqslant slant m^*\left(\bigcup_{n = 1}^{\infty} S_{I_n}\right) \leqslant slant \sum_{n = 1}^{\infty}m^*(S_{I_n})\\
&= (b^2 - a^2)\sum_{n = 1}^{\infty}|I_n|/2 \leqslant slant \frac{b^2 - a^2}{2}(m^*(E) + \varepsilon),
\end{align*}
由 \(\varepsilon\) 的任意性即得所证.

(ii) 设 \(b = +\infty\)，\(m^*(E) = 0\). 此时，对 \(n \geqslant slant 1\)，由 (i) 知
\[
m^*(S_E(a,n)) \leqslant slant \frac{(n^2 - a^2)m^*(E)}{2} = 0.
\]
从而得到
\[
...
&\leqslant slant \frac{b^2 - a^2}{2}\{m^*(I \cap E) + m^*(I \cap E^c)\}\\
...
&\leqslant slant \sum_{n = 1}^{\infty}m^*(T_n \cap S_E) + \sum_{n = 1}^{\infty}m^*(T_n \cap S_{E^c})\\
&\leqslant slant \sum_{n = 1}^{\infty}m(T_n) = m\left(\bigcup_{n = 1}^{\infty} T_n\right) = m(R).
\end{align*}
这说明，对任一矩形 \(R\)，有
\[
