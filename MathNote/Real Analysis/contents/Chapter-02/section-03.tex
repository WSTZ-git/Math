...
\end{align*}
令$G = \bigcup_{k = 1}^{\infty}I_k$，则$G$是包含$E$的开集，且$m(G)<m(E)+\varepsilon$。因为$m(E)<+\infty$，所以移项后再合并得$m(G\setminus E)<\varepsilon$。
...
从而得
\begin{align*}
m(G\setminus E)\leqslant\sum_{k = 1}^{\infty}m(G_k\setminus E_k)\leqslant\sum_{k = 1}^{\infty}\frac{\varepsilon}{2^k}=\varepsilon.
\end{align*}
...
$E\in \mathscr{M}$当且仅当对$\forall \varepsilon>0,$存在开集$G\subset E$、闭集$F\subset E$,使得$m(G\backslash F)<\varepsilon.$
\end{corollary}
\begin{proof}
{\heiti 必要性:}由\refthe{theorem:定理2.13}立得.

{\heiti 充分性:}对$\forall \varepsilon>0$,由\refcor{corollary:Borel集是可测集}可知$G,F$都可测.任取一点集$T$,则$T\cap F$,$T\cap F^c$都可测.从而由外测度和测度的性质可得
\begin{align*}
&m^*\left( T\cap E \right) +m^*\left( T\cup E^c \right) \leqslant m^*\left( T\cap G \right) +m^*\left( T\cap F^c \right) =m^*(T\cap \left( \left( G\backslash F \right) \cup F \right) )+m^*\left( T\cap F^c \right) 
\\
&=m^*\left( \left( T\cap \left( G\backslash F \right) \right) \cup \left( T\cap F \right) \right) +m^*\left( T\cap F^c \right) \leqslant m^*\left( T\cap \left( G\backslash F \right) \right) +m^*\left( T\cap F \right) +m^*\left( T\cap F^c \right) 
\\
&\xlongequal{\text{测度的可数可加性}}m^*\left( T\cap \left( G\backslash F \right) \right) +m^*\left( T\cap \left( F\cup F^c \right) \right) =m^*\left( T\cap \left( G\backslash F \right) \right) +m^*\left( T \right) 
\\
