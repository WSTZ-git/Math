...
\end{align*}
因为$m(E_k)<\infty$（$k = 1,2,\cdots$），所以对任给的$\varepsilon>0$，存在包含$E_k$的开集$G_k$，使得$m(G_k\setminus E_k)<\varepsilon/2^k$。现在作点集$G = \bigcup_{k = 1}^{\infty}G_k$，则$G\supset E$且为开集。由\rrefthe{Set Theory-theorem:集合论-集合的运算及性质}{Set Theory-theorem:集合论-集合的运算及性质-12}我们有
\begin{align*}
