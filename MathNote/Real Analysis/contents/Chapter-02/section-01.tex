...
\end{align*}
其中 \(I_k\) 的边长都小于 \(d(E_1, E_2)/\sqrt{n}(n\geq 2)\). 现在将 \(\{I_k\}\) 分为如下两组：
\begin{align}
...
\end{align}
且其中任一矩体皆不能同时含有 \(E_1\) 与 \(E_2\) 中的点.否则,若对任意的$J_{i_1}, J_{i_2}, \cdots,\bigcup_{k \geq 1} J_{i_k} \supset E_1$或$J_{i_1}, J_{i_2}, \cdots,\bigcup_{k \geq 1} J_{i_k} \supset E_2$,其中$J_{i_k}\in \{I_k\}$,都存在$m\in [1,k]\cap \mathbb{N}$,使得$J_{i_m}$中同时含有$E_1$和$E_2$中的点.设$x_1\in J_{i_m}\cap E_1$,$x_2\in J_{i_m}\cap E_2$,则由$d(E_1,E_2)>0$可知
\begin{align*}
d(x_1,x_2) \geqslant d(E_1,E_2) > 0.
\end{align*}
又因为\(I_k\) 的边长都小于 \(d(E_1, E_2)/\sqrt{n}(n\geq 2)\),所以
\begin{align*}
d(x_1,x_2)\leqslant \frac{\sqrt{2}d\left( E_1,E_2 \right)}{\sqrt{n}}<d(E_1,E_2)<d(x_1,x_2).
\end{align*}
上式显然矛盾!(最大的矩体应为正方体,$\frac{\sqrt{2}d\left( E_1,E_2 \right)}{\sqrt{n}}$为最大的矩体的对角线长)
故\eqref{100.58}式成立.
从而得
\begin{align*}
