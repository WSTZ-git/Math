...
\begin{proposition}\label{proposition:外测度相关的基本性质}
\begin{enumerate}[(1)]
...
\item\label{proposition:外测度相关的基本性质-6} 设$E\subseteq \mathbb{R}^n$，则$m^*(E) \leqslant \bigl(\mathrm{diam}\{E\}\bigr)^n$.
\end{enumerate}
\end{proposition}
\begin{proof}
\begin{enumerate}[(1)]
...
\item 若$\mathrm{diam}(E)=\infty$，则结论显然成立。设$d=\mathrm{diam}(E)<\infty$，令
\begin{align*}
a_i\triangleq \inf\limits\left\{ x_i:x=(x_1,\cdots,x_n)\in E \right\},\quad b_i\triangleq \sup\limits\left\{ x_i:x=(x_1,\cdots,x_n)\in E \right\},\quad i=1,2,\cdots,n.
\end{align*}
从而
\begin{align}\label{eq_31k7s2}
E\subseteq \prod\limits_{i=1}^n\left[ a_i,b_i \right].
\end{align}
对$\forall x=(x_1,\cdots,x_n),y=(y_1,\cdots,y_n)\in E$，有
\begin{align*}
|x_i|-|y_i|\leqslant |x_i-y_i|\leqslant |x-y|\leqslant d,\quad i=1,2,\cdots,n.
\end{align*}
从而
\begin{align*}
b_i-a_i=\sup\limits_{x\in E}|x_i|-\inf\limits_{x\in E}|y_i|=\sup\limits_{x\in E}\left( |x_i|-|y_i| \right) \leqslant d,\quad i=1,2,\cdots,n.
\end{align*}
于是再结合\eqref{eq_31k7s2}式知
\begin{align*}
m^*(E) \leqslant m^*\left( \prod\limits_{i=1}^n\left[ a_i,b_i \right] \right) =\prod\limits_{i=1}^n (b_i-a_i) \leqslant d^n.
\end{align*}
\end{enumerate}
\end{proof}
