...
\end{proposition}
\begin{proof}
显然$\overline{I}=I\cup \partial I$是闭矩体.先证$m^*(I)=|I|$.因为$I$是$I$自身的$L$-覆盖,所以$m^*(I)\leqslant slant |I|$.任取$I$的$L$-覆盖$\{I_k\}$,则
\[
\bigcup_{k = 1}^{\infty} I_{k} \supset I.
\]
从而由\nrefpro{proposition:矩体的性质}{(3)}可知
\begin{align*}
|I| \leqslant slant \sum_{k = 1}^{\infty} |I_k|,
\end{align*}
因此\(|I| \leqslant slant m^*(I)\).故$m^*(I)=|I|.$
...
\begin{align*}
d(x_1,x_2) \geqslant slant d(E_1,E_2) > 0.
\end{align*}
又因为\(I_k\) 的边长都小于 \(d(E_1, E_2)/\sqrt{n}\),所以由\nrefpro{proposition:矩体的性质}{(1)}可知
\begin{align*}
d(x_1,x_2)\leqslant slant \mathrm{diam}\left( J_{i_m} \right) \leqslant slant \sqrt{\left( \sum_{i=1}^n{\frac{d\left( E_1,E_2 \right)}{\sqrt{n}}} \right) ^2}=d(E_1,E_2)<d(x_1,x_2).
\end{align*}
上式显然矛盾!
故\eqref{100.58}式成立.
从而得
