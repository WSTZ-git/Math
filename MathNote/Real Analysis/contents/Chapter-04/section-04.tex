...
对于\hyperref[Basis of Analytics-definition:Riemann积分Darboux上下积分]{Riemann积分Darboux上、下积分}所说的分划序列 \( \{\Delta^{(n)}\} \)，作函数列
\[
\omega_{\Delta^{(n)}}(x) = 
\begin{cases} 
...
\]
其中\( E = \{ x \in [a,b] : x \text{ 是 } \Delta^{(n)} \, (n = 1,2,\cdots) \text{ 的分点} \} \).
显然$E$是可数个有限集的并,因此$E$是可数集且\( m(E) = 0 \).下面证明逐点极限
\[
\lim_{n \to \infty} \omega_{\Delta^{(n)}}(x) = \omega(x), \quad x \in [a,b] \setminus E.
\]
固定 \( x\in [a,b] \setminus E \). 对每个 \( n \)，存在唯一的小区间 \( I_i^{(n)} = [x_{i-1}^{(n)}, x_i^{(n)}] \) 使得 \( x\in (x_{i-1}^{(n)}, x_i^{(n)}) \). 记该区间长度为 \( \ell_n = x_i^{(n)} - x_{i-1}^{(n)} \)，则 \( \ell_n \leqslant |\Delta^{(n)}| \to 0 \). 令 \( d_n = \min\{x-x_{i-1}^{(n)},\; x_i^{(n)}-x\} \)，则 \( d_n>0 \) 且 \( d_n \leqslant \ell_n \)，故 \( d_n\to0 \).

由振幅定义，对任意 \( \delta>0 \)，定义
\[
\omega(x;\delta) \triangleq \sup_{\substack{u,v\in[a,b]\\ |u-x|<\delta,\ |v-x|<\delta}} |f(u)-f(v)|,
\]
则\( \omega(x) = \lim_{\delta\to0^+}\omega(x;\delta) \). 因为 \( (x-d_n, x+d_n) \subset I_i^{(n)} \)，所以
\[
\omega(x; d_n) =\underset{\substack{u,v\in \left[ a,b \right]\\
\left| u-x \right|<d_n,\left| v-x \right|<d_n}}{\mathrm{sup}}\left| f\left( u \right) -f\left( v \right) \right|=\underset{\left| t-x \right|<d_n}{\mathrm{sup}}f\left( x \right) -\underset{\left| t-x \right|<d_n}{\mathrm{inf}}f\left( x \right) \leqslant \underset{x\in I_{i}^{\left( n \right)}}{\mathrm{sup}}f\left( x \right) -\underset{x\in I_{i}^{\left( n \right)}}{\mathrm{inf}}f\left( x \right) = M_i^{(n)} - m_i^{(n)} = \omega_{\Delta^{(n)}}(x).
\]
另一方面，因为 \( I_i^{(n)} \subset (x-\ell_n, x+\ell_n) \)，所以
\[
\omega _{\Delta ^{(n)}}(x)=M_{i}^{(n)}-m_{i}^{(n)}=\underset{x\in I_{i}^{\left( n \right)}}{\mathrm{sup}}f\left( x \right) -\underset{x\in I_{i}^{\left( n \right)}}{\mathrm{inf}}f\left( x \right) \leqslant \underset{\left| t-x \right|<l_n}{\mathrm{sup}}f\left( x \right) -\underset{\left| t-x \right|<l_n}{\mathrm{inf}}f\left( x \right) =\underset{\substack{u,v\in \left[ a,b \right]\\
\left| u-x \right|<l_n,\left| v-x \right|<l_n}}{\mathrm{sup}}\left| f\left( u \right) -f\left( v \right) \right|=\omega (x;\ell _n).
\]
于是
\[
\omega(x; d_n) \leqslant \omega_{\Delta^{(n)}}(x) \leqslant \omega(x; \ell_n).
\]
令 \( n\to\infty \)，由于 \( d_n\to0 \) 且 \( \ell_n\to0 \)，有 \( \omega(x; d_n)\to\omega(x) \) 且 \( \omega(x; \ell_n)\to\omega(x) \). 由夹逼定理即得 \( \omega_{\Delta^{(n)}}(x)\to\omega(x) \). 

现在记 \( A,B \) 各为 \( f(x) \) 在 \( [a,b] \) 上的上、下确界，由于对一切 \( n \)，有 \( \omega_{\Delta^{(n)}}(x) \leqslant  A - B \)，故根据\hyperref[theorem:控制收敛定理]{控制收敛定理}可知，
\[
...
\begin{theorem}[Riemann可积的充要条件]\label{theorem:Riemann可积的充要条件-Riemann可积等价几乎处处连续}
若 \( f(x) \) 是定义在 \( [a,b] \) 上的有界函数，则 \( f(x) \) 在 \( [a,b] \) 上 Riemann 可积的充分必要条件是 \( f(x) \) 在 \( [a,b] \) 上的不连续点集是零测集,也即$f$在$[a,b]$上几乎处处连续.
\end{theorem}
\begin{remark}
...
{\heiti 必要性:}若 \( f(x) \) 在 \( [a,b] \) 上是 Riemann 可积的，则 \( f(x) \) 的 Darboux 上、下积分相等，从而由\reflem{lemma:引理4.23}可知 \( \int_I \omega(x) \, \mathrm{d}x = 0 \). 因为 \( \omega(x) \geqslant  0 \)，所以由\refcor{corollary:非负可测函数积分为0则几乎处处为0}可知\( \omega(x) = 0 \)，\(\text{a. e.}\) \( x \in [a,b] \). 从而
\begin{align*}
...
{\heiti 充分性:}若 \( f(x) \) 在 \( [a,b] \) 上的不连续点集是零测集，则
\begin{align*}
...
\end{remark}
\begin{proof}
首先，根据题设以及\refthe{theorem:Riemann可积的充要条件-Riemann可积等价几乎处处连续}，\( f(x) \) 在 \( [a,b] \) 上是几乎处处连续的. 因此 \( f(x) \) 是 \( [a,b] \) 上的有界可测函数，\( f \in L(I) \).
