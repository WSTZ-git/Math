...
(i) 若 \( f \in \mathcal{F} \)，且 \( a \geqslant  0 \)，则 \( af \in \mathcal{F} \)；

(ii) 若 \( f_1, f_2 \in \mathcal{F} \)，则 \( f_1 + f_2 \in \mathcal{F} \)；

(iii) 若 \( f, g \in \mathcal{F} \)，\( f(x, y) - g(x, y) \geqslant  0 \)，且 \( g \in L(\mathbb{R}^n) \)，则
\(
f - g \in \mathcal{F};
\)

(iv) 若 \( f_k \in \mathcal{F} \) \( (k = 1, 2, \cdots) \)，\( f_k(x, y) \leqslant  f_{k + 1}(x, y) \) \( (k = 1, 2, \cdots) \)，且有 \( \lim_{k \to \infty} f_k(x, y) = f(x, y) \)，\( (x, y) \in \mathbb{R}^p \times \mathbb{R}^q \)，则 \( f \in \mathcal{F} \).
\end{lemma}
\begin{proof}
...
\begin{theorem}[Tonelli定理(非负可测函数的情形)]\label{theorem:Tonelli定理(非负可测函数的情形)}
...
\begin{remark}
\begin{enumerate}[(i)]
\item 在\refthe{theorem:Tonelli定理(非负可测函数的情形)}的证明中，改变 \( x \in \mathbb{R}^p \) 与 \( y \in \mathbb{R}^q \) 的次序，结论同样成立. 因此，实际上我们可得
\[
...
\item 若 \( f(x, y) \) 是 \( E \) 上的非负可测函数，则可用 \( f(x, y) \chi_E(x, y) \) 代替\refthe{theorem:Tonelli定理(非负可测函数的情形)}中的 \( f(x, y) \)，我们有
\[
...
\end{theorem}
\begin{proof}
令 \( f(x, y) = f^+(x, y) - f^-(x, y) \)，则根据非负可测函数的\hyperref[theorem:Tonelli定理(非负可测函数的情形)]{Tonelli 定理}可知，\( f^+(x, y) \) 与 \( f^-(x, y) \) 满足上述条件(A),(B)与(C). 注意到所有的积分值都是有限的，从而可以作减法运算，并立即得出定理的结论.
\end{proof}
\begin{remark}
\begin{enumerate}
...
\item \hyperref[theorem:Tonelli定理(非负可测函数的情形)]{Tonelli 定理}对 Riemann 积分不真. 例如，设 \( E \) 是 \( [0, 1] \times [0, 1] \) 中的稠密集，且任一平行于坐标轴的直线至多交 \( E \) 于一个点，又作函数
\[
f(x, y) = 
\begin{cases} 
1, & (x, y) \in E, \\
...
\end{proof}
\begin{proof}
因为 \( f(x, y) = y \mathrm{e}^{-(1 + x^2)y^2} \) 在 \( [0, +\infty) \times [0, +\infty) \) 上非负可测，所以根据\hyperref[theorem:Tonelli定理(非负可测函数的情形)]{Tonelli 定理}可知
\[
...
\end{theorem}
\begin{proof}
只需在\hyperref[theorem:Tonelli定理(非负可测函数的情形)]{Tonelli定理}中令 \( f = \chi_E \) 便可得证.
\end{proof}
...
\]
所以若能证明 \( E_1 \times E_2 \) 是 \( \mathbb{R}^p \times \mathbb{R}^q \) 中的可测集，则由\hyperref[theorem:Tonelli定理(非负可测函数的情形)]{Tonelli定理}立即推知
\begin{align*}
...
\end{theorem}
\begin{proof}
首先，设 \( f(x) \geqslant  0 \)，\( g(x) \geqslant  0 \). 因为 \( f(x - t) g(t) \) 是 \( \mathbb{R}^n \times \mathbb{R}^n \) 上的可测函数，所以根据\hyperref[theorem:Tonelli定理(非负可测函数的情形)]{非负可测函数的Tonelli定理}可得
\begin{align*}
...
其次，对于一般情形，只需注意
\[
|(f * g)(x)| \leqslant  (|f| * |g|)(x),
\]
从而有
\begin{align*}
\int_{\mathbb{R}^n} |(f * g)(x)| \, \mathrm{d}x \leqslant  \int_{\mathbb{R}^n} (|f| * |g|)(x) \, \mathrm{d}x = \int_{\mathbb{R}^n} |f(x)| \, \mathrm{d}x \int_{\mathbb{R}^n} |g(x)| \, \mathrm{d}x < +\infty.
\end{align*}
\end{proof}

\begin{proposition}[卷积是有界一致连续函数]\label{proposition:卷积是有界一致连续函数}
设 \( f \in L(\mathbb{R}^n) \)，\( g(x) \) 在 \( \mathbb{R}^n \) 上有界可测，则 \( F(x) = (f * g)(x) \) 是 \( \mathbb{R} \) 上的有界且一致连续函数.
\end{proposition}
\begin{proof}
不妨设 \( |g(x)| \leqslant  M \)，\( x \in \mathbb{R}^n \),再结合\hyperref[theorem:积分的平均连续性]{积分的平均连续性}我们有
\begin{align*}
|F(x + h) - F(x)| &= \left| \int_{\mathbb{R}^n} f(x + h - t) g(t) \, \mathrm{d}t - \int_{\mathbb{R}^n} f(x - t) g(t) \, \mathrm{d}t \right| \\
&\leqslant  \int_{\mathbb{R}^n} |f(x - t + h) - f(x - t)| |g(t)| \, \mathrm{d}t \\
&\leqslant  M \int_{\mathbb{R}^n} |f(t + h) - f(t)| \, \mathrm{d}t \to 0 \quad (h \to 0),
\end{align*}
故$F$是 \( \mathbb{R} \) 上的一致连续函数.
由\hyperref[theorem:积分变量的平移变换定理]{积分变量的平移变换定理}知
\begin{align*}
\left| F\left( x \right) \right|&=\left| \int_{\mathbb{R}}{f\left( x-y \right) g\left( y \right) \mathrm{d}y} \right|\leqslant \int_{\mathbb{R}}{\left| f\left( x-y \right) \right|\left| g\left( y \right) \right|\mathrm{d}y}
\\
&\leqslant M\int_{\mathbb{R}}{\left| f\left( x-y \right) \right|\mathrm{d}y}=M\int_{\mathbb{R}}{\left| f\left( y \right) \right|\mathrm{d}y}=M\left\| f \right\| _1.
\end{align*}
因此$F$是 \( \mathbb{R} \) 上的有界函数.
\end{proof}
...
\begin{align*}
\int_E |f(x)|^p \, \mathrm{d}x = p \int_0^{+\infty} \lambda^{p - 1} f_*(\lambda) \, \mathrm{d}\lambda. 
\end{align*}
\end{theorem}
\begin{proof}
作函数
...
\end{cases}
\]
易知 \( F(\lambda, x) \) 作为 \( x \) 的函数是 \( \{ x \in E : |f(x)| > \lambda \} \) 上的特征函数，从而由 \hyperref[theorem:Tonelli定理(非负可测函数的情形)]{Tonelli定理}可得
\begin{align*}
