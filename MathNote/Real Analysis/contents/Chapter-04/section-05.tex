...
\begin{align*}
\int_{\delta}^{X} \int_0^{+\infty} |\sin ax \cdot f(y) \mathrm{e}^{-xy}| \, \mathrm{d}x\mathrm{d}y &\leqslant slant \int_{\delta}^{X} \int_0^{+\infty} |f(y)| \mathrm{e}^{-\delta y} \, \mathrm{d}x\mathrm{d}y \\
&\leqslant slant (X - \delta) \int_0^{+\infty} |f(y)| \, \mathrm{d}y,
\end{align*}
...
注意到（根据积分第二中值定理）
\[
\left| \int_{\delta}^{X} \mathrm{e}^{-xy} \sin ax \, \mathrm{d}x \right| \leqslant slant \frac{2}{a}, \quad 0 < \delta < X < +\infty,
\]
...
\[
A = \{ (\alpha z, \alpha h) : z \in E, 0 \leqslant slant \alpha \leqslant slant 1 \}
\]
...
\end{example}
\begin{proof}
当 \( (x, t) \in A \) 时，\( x = \alpha z \)，\( t = \alpha h \)，也就是 \( \alpha = t/h \)，\( \alpha z = t z / h \). 从而当 \( 0 \leqslant slant t \leqslant slant h \) 时，有
\[
...
\begin{align*}
m^*(A \times B) \leqslant slant m\left( \bigcup_{k, i = 1}^{\infty} (I_k \times J_i) \right) 
\leqslant slant \sum_{k = 1}^{\infty} |I_k| \cdot \sum_{i = 1}^{\infty} |J_i| < \varepsilon \sum_{i = 1}^{\infty} |J_i|.
\end{align*}
...
\]
令 \( E_k = \{ x : k\delta \leqslant slant f(x) < (k + 1)\delta \} \) \( (k = 0, 1, \cdots) \). 显然有
\[
G_E(f) = \bigcup_{k = 0}^{\infty} G_{E_k}(f).
\]
从而得
\begin{align*}
m^*(G_E(f)) \leqslant slant \sum_{k = 0}^{\infty} m^*(G_{E_k}(f)) \leqslant slant \sum_{k = 0}^{\infty} \delta m(E_k) = \delta m(E).
\end{align*}
由 \( \delta \) 的任意性可知
\[
m(G_E(f)) = 0.
...
\[
\underline{G}(f) = \underline{G}_E(f) = \{ (x, y) \in \mathbb{R}^{n + 1} : x \in E, 0 \leqslant slant y \leqslant slant f(x) \},
\]
...
\[
H(y) = \{ x : f(x) \geqslant slant y \},
\]
因此除一零测集中的 \( y \) 值以外，\( \{ x : f(x) \geqslant slant y \} \) 是可测集. 这说明 \( f(x) \) 是 \( E \subset \mathbb{R}^n \) 上的可测函数. 根据(i)即得
\[
m(\underline{G}(f)) = \int_E f(x) \, \mathrm{d}x.
\]
\end{proof}
...
\[
\int_{\mathbb{R}^n} |(f * g)(x)| \, \mathrm{d}x \leqslant slant \left( \int_{\mathbb{R}^n} |f(x)| \, \mathrm{d}x \right) \left( \int_{\mathbb{R}^n} |g(x)| \, \mathrm{d}x \right).
\]
\end{theorem}
\begin{proof}
首先，设 \( f(x) \geqslant  0 \)，\( g(x) \geqslant  0 \). 因为 \( f(x - t) g(t) \) 是 \( \mathbb{R}^n \times \mathbb{R}^n \) 上的可测函数，所以根据\hyperref[theorem:theorem:Tonelli 定理 非负可测函数的情形]{非负可测函数的Tonelli定理}可得
\begin{align*}
...
\end{proposition}
\begin{proof}
不妨设 \( |g(x)| \leqslant  M \)，\( x \in \mathbb{R}^n \). 我们有
\begin{align*}
