...
若 \( f \in L(E) \)，则对 \(\forall \varepsilon > 0 \)，存在 \( \mathbb{R}^n \) 上具有紧支集的连续函数 \( g(x) \)，使得
\[
\int_E |f(x) - g(x)| \mathrm{d}x < \varepsilon.
\]
进而对\( \forall \varepsilon > 0 \)，存在 \( f \) 的分解：
\[
f(x) = g(x) + [f(x) - g(x)] = f_1(x) + f_2(x), \quad x \in E,
\]
其中 \( f_1(x) \) 是 \( \mathbb{R}^n \) 上具有紧支集的连续函数，\( |f_2(x)| \) 满足 \( \int_{E}|f_2(x)|\mathrm{d}x< \varepsilon \).
\end{theorem}
\begin{remark}
上述事实表明:\textbf{可积函数可以被$\mathbb{R}^n$上具有紧支集的可测简单函数逼近.}
\end{remark}
\begin{proof}
...
\begin{theorem}[平均连续性]\label{theorem:积分的平均连续性}
...
所以
\begin{align*}
\bigl| m(E \cap (E + \{ h \})) - m(E) \bigr| \leqslant \int_{\mathbb{R}^n} \bigl| \chi_E(x) \bigr| \bigl| \chi_E(x - h) - \chi_E(x) \bigr| \, \mathrm{d}x \leqslant \int_{\mathbb{R}^n} \bigl| \chi_E(x - h) - \chi_E(x) \bigr| \, \mathrm{d}x.
\end{align*}
根据\hyperref[theorem:积分的平均连续性]{可积函数的平均连续性}可知，上式右端当 \( |h| \to 0 \) 时趋于零，即得所证。
...
\end{align*}
于是对 \( \varepsilon_k = \frac{1}{k} (k = 1, 2, \cdots) \)，就可取到具有紧支集的阶梯函数列 \( \{\varphi_k(x)\} \)，使得
\[
...
\]
且从假设可知存在 \( n_0 \)，当 \( n \geqslant n_0 \) 时，上式右端小于 \(\frac{ \varepsilon}{2} \)，所以
\[
