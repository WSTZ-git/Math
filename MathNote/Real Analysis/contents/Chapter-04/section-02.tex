...
\begin{align*}
\left| \int_E f(x) \mathrm{d}x \right| \leqslant slant \int_E |f(x)| \mathrm{d}x. 
\end{align*}
\end{theorem}
\begin{proof}
...
\begin{align*}
\left| \int_E{f(x)\mathrm{d}x} \right|=\left| \int_E{f^+(x)\mathrm{d}x}-\int_E{f^-(x)\mathrm{d}x} \right|\leqslant slant \int_E{f^+(x)\mathrm{d}x}+\int_E{f^-(x)\mathrm{d}x}=\int_E{|f(x)|\mathrm{d}x}.
\end{align*}
\end{proof}
...
\item 若\(f(x)\)是\(E\)上的可测函数，\(g \in L(E)\)，且\(|f(x)| \leqslant slant g(x)\)，a.e. \(x \in E\)（\(g(x)\)称为\(f(x)\)的\textbf{控制函数}），则\(f \in L(E)\).

\item 若\(f \in L(E)\)，\(e \subset E\)是可测集，则\(f \in L(e)\)．
\end{enumerate}

\item 若 \( f(x) \leqslant slant g(x) \)，a.e. \( x \in E \)，则 \( \int_E f(x)\mathrm{d}x \leqslant slant \int_E g(x)\mathrm{d}x \).

\item \begin{enumerate}[(i)]
\item 设\(f \in L(\mathbf{R}^n)\)，则
\[
\lim_{N \to \infty} \int_{\{x \in \mathbf{R}^n: |x| \geqslant slant N\}} |f(x)| \mathrm{d}x = 0,
\]
或说对任给\(\varepsilon > 0\)，存在\(N\)，使得
\[
\int_{\{x: |x| \geqslant slant N\}} |f(x)| \mathrm{d}x < \varepsilon.
\]

\item 若\(f \in L(E)\)，且有\(E_N = \{x \in E: |x| \geqslant slant N\}\)，则
\[
...
\begin{proof}
\begin{enumerate}[(1)]
\item 不妨设\(|f(x)| \leqslant slant M\) \((x \in E)\)，由于\(|f(x)|\)是\(E\)上的非负可测函数，故有
\[
\int_E |f(x)| \mathrm{d}x \leqslant slant \int_E M \mathrm{d}x = M m(E) < +\infty.
\]
...
\[
\left| \int_E f(x) \mathrm{d}x \right| \leqslant slant \int_E |f(x)| \mathrm{d}x = 0.
\]
...
\[
\int_E |f(x)| \mathrm{d}x \leqslant slant \int_E g(x) \mathrm{d}x < +\infty.
\]
...
\begin{align*}
\int_e{\left| f\left( x \right) \right|\mathrm{d}x}=\int_E{\left| f\left( x \right) \right|\chi _e\left( x \right) \mathrm{d}x}=\int_E{\left| f\left( x \right) \chi _e\left( x \right) \right|\mathrm{d}x}\leqslant slant \int_E{\left| f\left( x \right) \right|\mathrm{d}x}<+\infty .
\end{align*}
故$|f|\in L(e)$,因此由\refthe{theorem:f与|f|的可积性等价}可知$f \in L(e).$
\end{enumerate}

\item 因为\( f(x) \leqslant slant g(x) \)，a.e. \( x \in E \)，所以 \( f^+(x) \leqslant slant g^+(x) \)，\( f^-(x) \geqslant slant g^-(x) \)，a.e. \( x \in E \). 由\hyperref[theorem:非负可测函数积分的性质]{非负可测函数积分的性质(1)}可知
\[
\int_E f^+(x)\mathrm{d}x \leqslant slant \int_E g^+(x)\mathrm{d}x, \quad \int_E f^-(x)\mathrm{d}x \geqslant slant \int_E g^-(x)\mathrm{d}x
\]
从而
\begin{align*}
\int_E f(x)\mathrm{d}x = \int_E f^+(x)\mathrm{d}x - \int_E f^-(x)\mathrm{d}x 
\leqslant slant \int_E g^+(x)\mathrm{d}x - \int_E g^-(x)\mathrm{d}x = \int_E g(x)\mathrm{d}x
\end{align*}
故结论成立.

\item \begin{enumerate}[(i)]
\item 记\(E_N = \{x \in \mathbf{R}^n: |x| \geqslant slant N\}\)，则\(\{|f(x)| \chi_{E_N}(x)\}\)是非负可积函数渐降列，且有
\[
...
\begin{align*}
\int_{\mathbb{R} ^n}{\left| f\left( x \right) \chi _{E_N}\left( x \right) \right|\mathrm{d}x}=\int_{\mathbb{R} ^n}{\left| f\left( x \right) \right|\chi _{E_N}\left( x \right) \mathrm{d}x}\leqslant slant \int_{\mathbb{R} ^n}{\left| f\left( x \right) \right|\chi _E\left( x \right) \mathrm{d}x}=\int_E{\left| f\left( x \right) \right|\mathrm{d}x}<+\infty .
\end{align*}
因此\(f \cdot \chi_{E_N} \in L(\mathbf{R}^n)\).又$E_N\subset E\cap \left\{ x\in \mathbb{R} ^n:\left| x \right|\geqslant slant N \right\}$,故由\hyperref[theorem:非负可测函数积分的性质]{非负可测函数的积分性质(3)}及(i)可得
\begin{align*}
\underset{N\rightarrow \infty}{\lim}\int_{E\cap E_N}{f\left( x \right) \mathrm{d}x}=\underset{N\rightarrow \infty}{\lim}\int_{E_N}{f\left( x \right) \mathrm{d}x}=\underset{N\rightarrow \infty}{\lim}\int_{\left\{ x\in \mathbb{R} ^n:\left| x \right|\geqslant slant N \right\}}{f\left( x \right) \chi _{E_N}\left( x \right) \mathrm{d}x}=0.
\end{align*}
\end{enumerate}
\end{enumerate}
\end{proof}
...
\end{example}
\begin{proof}
为了阐明 $f \in L([0,1])$, 自然想到去寻求可积的控制函数. 题设告诉我们 $|f(x)| \ln(1 + |f(x)|)$ 是 $[0,1]$ 上的可积函数, 难道它能控制 $|f(x)|$ 吗? 显然, 这只是在 $\ln(1 + |f(x)|) \geqslant slant 1$ 或 $|f(x)| \geqslant slant \mathrm{e} - 1$ 时才行. 但注意到 $|f(x)| < \mathrm{e} - 1$ 时, 由于区间 $[0,1]$ 的测度是有限的, 故常数 $\mathrm{e} - 1$ 本身就是控制函数. 也就是说, 可在不同的定义区域寻求不同的控制函数.

为此, 作点集
\[
E_1 = \{ x \in [0,1] : |f(x)| \leqslant slant \mathrm{e} \}, \quad E_2 = [0,1] \setminus E_1,
\]
则我们有
\[
|f(x)| \leqslant slant \mathrm{e}, \quad x \in E_1;
\]
\[
|f(x)| \leqslant slant |f(x)| \ln(1 + |f(x)|), \quad x \in E_2.
\]
...
\[
\lim_{n \to \infty} f_n(x) = f(x) \ (x \in E), \quad f_n(x) \leqslant slant f_{n + 1}(x) \ (n \in \mathbf{N}, x \in E),
\]
则
\[
...
\begin{proposition}
设 $g \in L(E)$，$f_n \in L(E)$（$n \in \mathbf{N}$）。若 $f_n(x) \geqslant slant g(x)$，a. e. $x \in E$，则
\[
\int_E \varliminf_{n \to \infty} f_n(x) \, \mathrm{d}x \leqslant slant \varliminf_{n \to \infty} \int_E f_n(x) \, \mathrm{d}x.
\]
\end{proposition}
\begin{proof}
根据\hyperref[lemma:Fatou引理]{Fatou引理}，我们有
\begin{align*}
&\quad \quad \int_E{\underset{n\rightarrow \infty}{\underline{\lim }}\left[ f_n\left( x \right) -g\left( x \right) \right] \mathrm{d}x}\leqslant slant \underset{n\rightarrow \infty}{\underline{\lim }}\left( \int_E{\left[ f_n\left( x \right) -g\left( x \right) \right] \mathrm{d}x} \right) 
\\
&\Longleftrightarrow \int_E{\underset{n\rightarrow \infty}{\underline{\lim }}f_n\left( x \right) \mathrm{d}x}-\int_E{g_n\left( x \right) \mathrm{d}x}\leqslant slant \underset{n\rightarrow \infty}{\underline{\lim }}\int_E{f_n\left( x \right) \mathrm{d}x}-\int_E{g_n\left( x \right) \mathrm{d}x}
\\
&\Longleftrightarrow \int_E{\underset{n\rightarrow \infty}{\underline{\lim }}f_n\left( x \right) \mathrm{d}x}\leqslant slant \underset{n\rightarrow \infty}{\underline{\lim }}\int_E{f_n\left( x \right) \mathrm{d}x}.
\end{align*}
证毕.
\end{proof}
...
\[
\varphi\left( \int_E f(x)w(x) \, \mathrm{d}x \right) \leqslant slant \int_E \varphi(f(x))w(x) \, \mathrm{d}x.
\]
\end{theorem}
\begin{remark}
...
\end{remark}
\begin{proof}
注意到 $a \leqslant slant f(x) \leqslant slant b$，我们有
\[
a=\int_E{aw(x)\,\mathrm{d}x}\leqslant slant y_0=\int_E{f(x)w(x)\,\mathrm{d}x}\leqslant slant \int_E{bw(x)\,\mathrm{d}x}=b.
\]
故$y_0\in[a,b]$.

(i) 设 $y_0 \in (a,b)$，由 $\varphi(x)$ 之（下）凸性可知有
\[
\varphi(y) \geqslant slant \varphi(y_0) + k(y - y_0), \quad y \in [a,b].
\]
(其中由\refthe{Basis of Analytics-theorem:开区间下凸函数左右导数处处存在}及下凸函数的切线放缩可知$k=\varphi'_+(y_0)$)
以 $f(x)$ 代 $y$ 得
\[
\varphi(f(x)) \geqslant slant \varphi(y_0) + k(f(x) - y_0), \quad \text{a. e. } x \in E.
\]
在上式两端乘以 $w(x)$，并在 $E$ 上作积分，则
\begin{align*}
\int_E \varphi(f(x))w(x) \, \mathrm{d}x &\geqslant slant \int_E \varphi(y_0)w(x) \, \mathrm{d}x + k \int_E (f(x) - y_0)w(x) \, \mathrm{d}x \\
...
\[
\sqrt{1 + A^2} \leqslant slant \int_E \sqrt{1 + f^2(x)} \, \mathrm{d}x \leqslant slant 1 + A.
\]
实际上，考查 $\varphi(x) = (1 + x^2)^{1/2}$，易知 $\varphi(x)$ 是（下）凸函数。根据 Jensen 不等式（$w(x) \equiv 1$），有 $\left( A^2 \leqslant slant \int_E f^2(x) \, \mathrm{d}x \right)$.
\begin{align*}
\sqrt{1 + A^2} &\leqslant slant \left( 1 + \int_E f^2(x) \, \mathrm{d}x \right)^{1/2} = \left( \int_E (1 + f^2(x)) \, \mathrm{d}x \right)^{1/2} \\
&\leqslant slant \int_E \sqrt{1 + f^2(x)} \, \mathrm{d}x \leqslant slant \int_E (1 + f(x)) \, \mathrm{d}x = 1 + A.
\end{align*}
\end{remark}
...
\begin{align*}
\sum_{k=1}^{\infty} \int_{E_k} f^{\pm}(x) \, \mathrm{d}x = \int_E f^{\pm}(x) \, \mathrm{d}x \leqslant slant \int_E |f(x)| \, \mathrm{d}x < +\infty.
\end{align*}
从而可知
\begin{align*}
...
\[
E_n = \left\{ x \in E : |f(x)| \geqslant slant \frac{1}{n} \right\}
\]
...
m(E_{n_0}) > m(E_0)/2.
\]
注意到，当 \( x \in E_{n_0} \) 时，有 \( |f(x)| \geqslant slant 1/n_0 \). 从而
\begin{align*}
\int_E |f(x)|\,\mathrm{d}x &\geqslant slant \int_{E_0} |f(x)|\,\mathrm{d}x \geqslant slant \int_{E_{n_0}} |f(x)|\,\mathrm{d}x \\
&\geqslant slant \int_{E_{n_0}} \frac{1}{n_0}\,\mathrm{d}x = \frac{1}{n_0} \cdot m(E_{n_0}) \\
&> \frac{m(E_0)}{2n_0} > 0
\end{align*}
...
\[
\sum_{n \geqslant slant 1} \int_{[a_n, b_n]} f(x) \, \mathrm{d}x = \int_G f(x) \, \mathrm{d}x = -\int_F f(x) \, \mathrm{d}x > 0\neq 0,
\]
从而存在 $n_0$，使得
\[
...
\[
m(\{ x \in E : k_i \leqslant slant |g(x)| < k_{i+1} \}) = m(E_i) > 0 \quad (i = 1, 2, \cdots).
\]
...
所以 $f \in L^1(E)$，但我们有
\[
\int_E f(x)g(x) \, \mathrm{d}x \geqslant slant \sum_{i=1}^{\infty} \frac{k_i}{i^{1 + (1/2)} m(E_i)} m(E_i) = +\infty,
\]
这说明 $fg \notin L(E)$，矛盾。
\end{proof}
...
\begin{align*}
\left| \int_e f(x) \, \mathrm{d}x \right| \leqslant slant \int_e |f(x)| \, \mathrm{d}x < \varepsilon.
\end{align*}
\end{theorem}
\begin{proof}
不妨假定 $f(x) \geqslant slant 0$,否则用$|f(x)|$代替$f(x)$。根据\hyperref[theorem:简单函数逼近定理]{简单函数逼近定理}可知,存在非负简单可测函数渐升列$\{\varphi_n(x)\}$,使得$\lim_{n\to \infty}\varphi_n(x)=f(x).$
...
\end{align*}
于是对于任给的 $\varepsilon > 0$，存在可测简单函数 $\varphi(x)$，$0 \leqslant slant \varphi(x) \leqslant slant f(x)$（$x \in E$），使得
\[
\int_E (f(x) - \varphi(x)) \, \mathrm{d}x = \int_E f(x) \, \mathrm{d}x - \int_E \varphi(x) \, \mathrm{d}x < \frac{\varepsilon}{2}.
\]
现在设 $\varphi(x) \leqslant slant M$，取 $\delta = \varepsilon / (2M)$，则当 $e \subset E$，且 $m(e) < \delta$ 时，就有
\begin{align*}
\int_e f(x) \, \mathrm{d}x &= \int_e f(x) \, \mathrm{d}x - \int_e \varphi(x) \, \mathrm{d}x + \int_e \varphi(x) \, \mathrm{d}x \\
&\leqslant slant \int_E (f(x) - \varphi(x)) \, \mathrm{d}x + \int_e \varphi(x) \, \mathrm{d}x \\
&< \frac{\varepsilon}{2} + Mm(e) \leqslant slant \frac{\varepsilon}{2} + \frac{\varepsilon}{2} = \varepsilon.
\end{align*}
\end{proof}
...
\begin{align*}
|g(t+\Delta t)-g(t)|=\left| \int_{E\cap [t,t+\Delta t)}{f\left( x \right) \,\mathrm{d}x} \right|\leqslant slant \int_{E\cap [t,t+\Delta t)}{|f(x)|\,\mathrm{d}x}\leqslant slant \int_{[t,t+\Delta t)}{|f(x)|\,\mathrm{d}x}<\varepsilon .
\end{align*}
...
\end{remark}
\begin{proof}
只需考虑 $f(x) \geqslant slant 0$ 的情形。首先看 $f(x)$ 是非负可测简单函数的情形：
\[
...
(ii) 对 $f \in L(I)$，设简单可测函数列 $\{ \varphi_n(x) \}$，使得 $\varphi_n(x) \to f(x)$（$n \to \infty$，$x \in I$），且 $|\varphi_n(x)| \leqslant slant |f(x)|$（$n = 1, 2, \cdots, x \in I$），则令 $\psi_n(x) = \varphi_n(ax)$（$x \in J$，$n = 1, 2, \cdots$），$\psi_n(x) \to g(x)$（$n \to \infty$，$x \in J$），我们有
\begin{align*}
...
若存在 $E$ 上的可积函数 $F(x)$，使得
\[
|f_k(x)| \leqslant slant F(x), \quad \text{a. e. } x \in E \ (k = 1, 2, \cdots),
\]
则
\[
...
\end{theorem}
\begin{proof}
显然，由\refcor{corollary:可测函数列的极限也可测}可知$f(x)$ 是 $E$ 上的可测函数，且由 $|f_k(x)| \leqslant slant F(x)$（a. e. $x \in E$）及\eqref{eq:100.101}式可知 $|f(x)| \leqslant slant F(x)$，a. e. $x \in E$. 因此，由\hyperref[theorem:积分的基本性质]{积分的基本性质(4)(i)}可知$f(x)$ 也是 $E$ 上的可积函数. 作函数列
\[
g_k(x) = |f_k(x) - f(x)| \quad (k = 1, 2, \cdots),
\]
则 $g_k \in L(E)$，且 $0 \leqslant slant g_k(x) \leqslant slant 2F(x)$，a. e. $x \in E$（$k = 1, 2, \cdots$）.注意到$\underset{k\rightarrow \infty}{\lim}g_k\left( x \right) =0$,显然$\lim_{k\to +\infty}g_k(x)$在$E$上也可积.

根据\hyperref[lemma:Fatou引理]{Fatou引理}，我们有
\[
\int_E \lim_{k \to \infty} (2F(x) - g_k(x)) \, \mathrm{d}x \leqslant slant \varliminf_{k \to \infty} \int_E (2F(x) - g_k(x)) \, \mathrm{d}x.
\]
因为 $F(x)$,$\lim_{n\to \infty}g_n(x)$以及每个 $g_k(x)$都是可积的，所以由\hyperref[theorem:积分的线性性质]{积分的线性性质}可得
\[
\int_E 2F(x) \, \mathrm{d}x - \int_E \lim_{k \to \infty} g_k(x) \, \mathrm{d}x \leqslant slant \int_E 2F(x) \, \mathrm{d}x - \varlimsup_{k \to \infty} \int_E g_k(x) \, \mathrm{d}x.
\]
...
\leqslant slant \int_E g_k(x) \, \mathrm{d}x
\end{align*}
令$k\to \infty$,得
\begin{align*}
\underset{k\rightarrow \infty}{\overline{\lim }}\left| \int_E{f_k(x)\,\mathrm{d}x}-\int_E{f(x)\,\mathrm{d}x} \right|\leqslant slant \underset{k\rightarrow \infty}{\overline{\lim }}\int_E{g_k(x)\,\mathrm{d}x}=0.
\end{align*}
...
就有
\begin{align*}
\sigma m(E_k(\sigma)) &= \int_{E_k(\sigma)} \sigma \, \mathrm{d}x \leqslant slant \int_{E_k(\sigma)} |f_k(x) - f(x)| \, \mathrm{d}x \\
&\leqslant slant \int_E |f_k(x) - f(x)| \, \mathrm{d}x \to 0 \quad (k \to \infty),
\end{align*}
...
\[
\lim_{k \to \infty} f_k(x) = f(x), \quad |f_k(x)| \leqslant slant M \quad (k = 1, 2, \cdots),
\]
则\(f \in L(E)\)，且
\[
\lim_{k \to \infty} \int_E f_k(x) \, \mathrm{d}x = \int_E f(x) \, \mathrm{d}x.
...
\[
E_n = \left\{ x \in [c,d] : |f_k'(x) - F(x)| \leqslant slant 1, k \geqslant slant n \right\},
\]
易知每个 \( E_n \) 皆闭集，且 \( [c,d] = \bigcup_{n=1}^{\infty} E_n \)．从而根据 Baire 定理（定理 1.23）可知，存在 \( n_0 \) 以及区间 \( [c',d'] \)，使得 \( E_{n_0} \supset [c',d'] \)．由于
\[
|f_k'(x) - F(x)| \leqslant slant 1 \quad (k \geqslant slant n_0), \, x \in [c',d'],
\]
故知 \( k \geqslant slant n_0 \) 时，\( \{ f_k'(x) \} \) 在 \( [c',d'] \) 上一致有界．这样，由等式
\[
...
\[
|f_k(x)| \leqslant slant F(x) \quad (k = 1,2,\cdots; \text{a. e. } x \in \mathbb{R}^n),
\]
...
\]
从而在 \( |f_{k_i}(x)| \leqslant slant F(x) \) 中令 \( i \to \infty \)，即知 \( f \in L(\mathbb{R}^n) \)，且 \( |f(x)| \leqslant slant F(x) \)，a. e. \( x \in \mathbb{R}^n \)．

其次，把 \( \mathbb{R}^n \) 分解如下：

（i）由 \( F \in L(\mathbb{R}^n) \) 可知，存在 \( N \)，使得
\[
\int_{\{ x : |x| \geqslant slant N \}} F(x) \mathrm{d}x < \frac{\varepsilon}{6}.
\]
自然同时对一切 \( k = 1,2,\cdots \)，也有
\begin{align*}
\int_{\{ x : |x| \geqslant slant N \}} |f_k(x) - f(x)| \mathrm{d}x &\leqslant slant 2 \int_{\{ x : |x| \leqslant slant N \}} F(x) \mathrm{d}x < \frac{\varepsilon}{3}.
\end{align*}
...
\[
\int_{e} |f_k(x) - f(x)| \mathrm{d}x \leqslant slant 2 \int_{e} F(x) \mathrm{d}x < \frac{\varepsilon}{3}.
\]

（iii）再看 \( B = B(0, N) \)，记 \( m(B) = l \)．由 \( f_k(x) \) 依测度收敛于 \( f(x) \) 可知，对 \( \varepsilon/(3l) \) 以及 \( \delta \)，（记 \( E_k = \{ x \in B : |f_k(x) - f(x)| > \varepsilon/(3l) \} \)）必存在 \( K \)，当 \( k \geqslant slant K \) 时，有 \( m(E_k) < \delta \)．

（iv）对 \( k \geqslant slant K \) 作分解
\begin{align*}
\int_{\mathbb{R}^n} |f_k(x) - f(x)| \mathrm{d}x &= \int_{\{ x : |x| \geqslant slant N \}} |f_k(x) - f(x)| \mathrm{d}x + \int_{B} |f_k(x) - f(x)| \mathrm{d}x \\
&< \frac{\varepsilon}{3} + \int_{E_k} |f_k(x) - f(x)| \mathrm{d}x + \int_{B \setminus E_k} |f_k(x) - f(x)| \mathrm{d}x \\
&< \frac{\varepsilon}{3} + \frac{\varepsilon}{3} + \int_{B \setminus E_k} \frac{\varepsilon}{3l} \mathrm{d}x \leqslant slant \frac{2\varepsilon}{3} + \frac{\varepsilon}{3l} \int_{B} \mathrm{d}x = \varepsilon.
\end{align*}
\end{proof}
\begin{remark}
...
记 \( m_k(x) = \min_{x \in E} \{ f_k(x), f(x) \} \)，\( M_k(x) = \max_{x \in E} \{ f_k(x), f(x) \} \)，则对 \( \sigma > 0 \)，由于 \( \{ x \in E : f(x) - m_k(x) > \sigma \} \subset \{ x \in E : |f_k(x) - f(x)| > \sigma \} \)，故知 \( m_k(x) \) 在 \( E \) 上依测度收敛于 \( f(x) \)．再注意到 \( 0 \leqslant slant m_k(x) \leqslant slant f(x) \)（\( x \in E \)），可得
\[
...
\]
易知，在 \(1 < \alpha \leqslant slant 3/2\) 且 \(n\) 充分大时，\(g(x)\) 在 \([0,1]\) 中有极值点，从而在 \(n\) 充分大时，不难得出
\[
0 < \frac{nx\sin x}{1 + (nx)^\alpha} \leqslant slant \frac{1}{\sqrt{x}} \quad (x \in [0,1]).
\]
根据控制收敛定理即得所证．
\end{proof}
...
\]
\[
0 \leqslant slant \chi_{[na, +\infty)}(u) (1 + u^2 / n^2)^{-1} u\text{e}^{-u^2} \leqslant slant u\text{e}^{-u^2} \quad (0 \leqslant slant u < +\infty),
\]
...
\[
|f(x)| \leqslant slant \sum_{k=1}^{\infty} |f_k(x)| = F(x), \quad \text{a. e. } x \in E,
\]
故 \( f \in L(E) \)．

现在令 \( g_m(x) = \sum_{k=1}^{m} f_k(x) \)（\( m = 1,2,\cdots \)），则
\[
|g_m(x)| \leqslant slant \sum_{k=1}^{m} |f_k(x)| \leqslant slant F(x) \quad (m = 1,2,\cdots).
\]
...
\[
\left| \frac{\mathrm{d}}{\mathrm{d}y} f(x,y) \right| \leqslant slant F(x), \quad (x,y) \in E \times (a,b),
\]
则
\[
...
\[
\left| \frac{f(x,y + h_k) - f(x,y)}{h_k} \right| \leqslant slant F(x), \quad x \in E.
\]
...
\lim_{n \to \infty} \int_E f_n(x) \mathrm{d}x = \int_E f(x) \mathrm{d}x,
\]
则 \( \varliminf_{n \to \infty} f_n(x) \leqslant slant f(x) \leqslant slant \varlimsup_{n \to \infty} f_n(x) \)，a. e. \( x \in \mathbb{R} \)．（由此可知，若存在子列 \( \{ f_{n_k}(x) \} \)：\( \lim_{k \to \infty} f_{n_k}(x) = g(x) \)，a. e. \( x \in \mathbb{R} \)，则 \( f(x) = g(x) \)，a. e. \( x \in \mathbb{R} \)．）
\end{example}
\begin{proof}
作 \( g_n(x) = \sup_{k \geqslant slant n} \{ f_k(x) \} \)，且令
\[
p(x) = \lim_{n \to \infty} g_n(x) = \varlimsup_{n \to \infty} f_n(x),
\]
则只需指出在 \( E(m(E) < +\infty) \) 上，有 \( f(x) \leqslant slant p(x) \)，a. e. \( x \in E \) 即可．
...
\[
\int_F f(x) \mathrm{d}x = \lim_{k \to \infty} \int_F f_k(x) \mathrm{d}x \leqslant slant \lim_{k \to \infty} \int_F g_k(x) \mathrm{d}x = \int_F p(x) \mathrm{d}x,
\]
从而可知 \( f(x) \leqslant slant p(x) \)，a. e. \( x \in P_n \)，自然有 \( f(x) \leqslant slant p(x) \)，a. e. \( x \in P \)．注意到 \( p(x) = -\infty \)（\( x \in P \)），故 \( m(P) = 0 \)．

（ii）若 \( p(x) = +\infty \)，则易知 \( f(x) \leqslant slant p(x) \)．

（iii）若 \( -\infty < p(x) < +\infty \)，则令 \( Q_m = \{ x \in \mathbb{R} : -m \leqslant slant p(x) \leqslant slant m \} \)，我们有 \( \mathbb{R} = \bigcup_{m=1}^{\infty} Q_m \)．易知只需考查 \( Q_m \) 上 \( f(x) \) 与 \( p(x) \) 的大小．

作点集 \( S_n = \{ Q_m : g_n(x) - p(x) < 1 \} \)，则 \( Q_m = \bigcup_{n=1}^{\infty} S_n \)．由此又只需指出 \( f(x) \leqslant slant p(x) \)（\( x \in S_n \)）．因为函数 \( p(x), g_n(x), g_{n+1}(x), \cdots \) 均一致有界，所以得到
\[
\lim_{n \to \infty} \int_F g_k(x) \mathrm{d}x = \int_F p(x) \mathrm{d}x \quad (F \subset S_n).
\]
注意到
\begin{align*}
\int_F f(x) \mathrm{d}x &= \lim_{k \to \infty} \int_F f_k(x) \mathrm{d}x \leqslant slant \lim_{k \to \infty} \int_F g_k(x) \mathrm{d}x \\
&= \int_F p(x) \mathrm{d}x \quad (F \in S_n).
\end{align*}
故得 \( f(x) \leqslant slant p(x) \)，a. e. \( x \in S_n \)．当然，此结论在 \( \mathbb{R} \) 上也真．
