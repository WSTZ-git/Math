...
\bigvee_a^b(f) < +\infty,
\end{align*}
则称 $f(x)$ 是 $[a,b]$ 上的\textbf{有界变差函数}(即全体变差形成有界数集), 其全体记为 $\mathrm{BV}([a,b])$.
\end{definition}
\begin{remark}
由定义我们立即可得下述简单事实:
\begin{enumerate}[(1)]
\item 设 $f\in \mathrm{BV}([a,b])$, 则 $f(x)$ 在 $[a,b]$ 上是有界函数;
\item $\mathrm{BV}([a,b])$ 构成一个线性空间.
\end{enumerate}
\end{remark}
\begin{remark}
\textbf{有界变差函数与曲线的可求长.}
...
设$f$是$[a,b]$上的实值函数,
\begin{enumerate}[(1)]
\item\label{proposition:常见的一些有界变差函数-1} 若$f$是单调函数,则$f\in \mathrm{BV}[a,b]$.

\item\label{proposition:常见的一些有界变差函数-2} 若 $f(x)$ 在 $[a,b]$ 上满足利普希兹条件
\begin{align*}
|f(x)-f(x')| \leqslant L|x-x'|, \quad \forall x,x'\in[a,b],
\end{align*}
其中 $L\geqslant 0$ 为常数, 则$f\in \mathrm{BV}[a,b]$.

\item\label{proposition:常见的一些有界变差函数-3} 设 $f(x)$ 在 $[a,b]$ 上处处可导, 且导数一致有界: $\exists M>0$, 使得
\begin{align*}
|f'(x)| \leqslant M, \quad \forall x\in[a,b],
\end{align*}
则$f\in \mathrm{BV}[a,b]$.
\end{enumerate}
\end{proposition}
\begin{proof}
\begin{enumerate}[(1)]
...
\end{align*}
从而 $\bigvee_a^b(f) = f(b)-f(a)$. 因此, $f\in \mathrm{BV}[a,b]$.
...
\end{align*}
从而 $\bigvee_a^b(f) = L(b-a)$. 因此, $f\in \mathrm{BV}[a,b]$.
...
\end{align*}
从而 $\bigvee_a^b(f) = M(b-a)$. 因此, $f\in \mathrm{BV}[a,b]$.
\end{enumerate}
...
\item\label{proposition:有界变差函数的性质-2} $kf\in \mathrm{BV}[a,b]$, 且 $\bigvee_a^b(kf)\leqslant|k|\bigvee_a^b(f)$;
...
\begin{proof}
\begin{enumerate}[(1)]
\item 假设 $f$ 无界, 则存在 $\{x_n\}\subset[a,b]$ 使得 $|f(x_n)|\to+\infty$. 由于 $f\in \mathrm{BV}[a,b]$, 故 $\bigvee_a^b(f)<+\infty$. 对 $\forall n\in\mathbb{N}$, 作分割 $\Delta_n=\{a,x_n,b\}$. 则
\begin{align*}
...
\item  $f,g\in BV[a,b]$, 由 (i) 知, $f,g$ 有界, 故存在 $M_1,M_2>0$, 使得
\begin{align*}
...
$\Longrightarrow$:设$f\in \mathrm{BV}[a,b]$,令
\begin{align*}
g(x) = \frac{1}{2}\left(\bigvee_a^x(f) + f(x)\right), \quad h(x) = \frac{1}{2}\left(\bigvee_a^x(f) - f(x)\right),
\end{align*}
则 $f(x) = g(x) - h(x)$. 下面证 $g$ 单调递增. 设 $x_1 < x_2$, 由有界变差函数的性质 (v)
\begin{align*}
...
(ii) 由 (i) 知, 对 $\varepsilon=1/2^n$, 存在 $[a,b]$ 上的函数列 $\{g_n(x)\}$, 使得
\begin{align*}
...
设 $f\in C(\mathbb{R})\cap L(\mathbb{R})$. 若 $f\in \mathrm{BV}(\mathbb{R})$, 则
\begin{align*}
