...
\item 若对任意的 $I_1,\cdots,I_n\in\mathcal{F}$, 都有 $E\not\subset\bigcup_{i=1}^n I_i$. 在 $\mathcal{F}$ 中任取一个区间记为 $I_1$, 假设已选定互不相交的区间 $I_1,\cdots,I_k$, 由于 $E-\bigcup_{i=1}^k I_i\neq\emptyset$, 注意到 $\bigcup_{i=1}^k I_i$ 是闭集以及 $\mathcal{F}$ 是Vitali覆盖, 故至少存在一个 $I\in\mathcal{F}$, 使得 $I$ 与 $I_1,\cdots,I_k$ 都互不相交. 令
\begin{align*}
\lambda_k=\sup\{|I|:I\in\mathcal{F}, I\cap I_i=\emptyset, i=1,\cdots,k\}.
\end{align*}
注意到 $\mathcal{F}$ 中每个 $I$ 都含于 $G$ 中, 故 $\lambda_k\leqslant m(G)<+\infty$. 在 $\mathcal{F}$ 中选取区间 $I_{k+1}$, 使得
\begin{align}
|I_{k+1}|>\frac{\lambda_k}{2},\quad I_{k+1}\cap I_i=\emptyset,\quad i=1,\cdots,k.\label{eq:::-hr82h39rhswufh8jiojshiojfw8290rui9ejwf90382jfwds1}
\end{align}
...
\end{align}
所有这样的区间 $[x-h,x]$ 构成了 $A$ 的一个Vitali覆盖. 由Vitali覆盖定理, 存在有限个互不相交的区间 $I_i = [x_i-h_i,x_i]$, $i=1,\cdots,n$, 使得 $m^*(A-\bigcup_{i=1}^n I_i) < \varepsilon$. 令 $B = A\cap\bigcup_{i=1}^n I_i^\circ$, 则
\begin{align}
...
\end{align*}
因此, 由\hyperref[lemma:Fatou引理]{Fatou引理}可得
\begin{align}
...
\begin{example}
设 $E$ 为 $(a,b)$ 中的零测集, 对每个 $n\in\mathbb{N}$, 取开集 $G_n\supset E$ 满足 $m(G_n)<1/2^n$. 令
\begin{align*}
