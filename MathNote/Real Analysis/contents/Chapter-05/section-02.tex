...
\end{align*}
(当 $x\overline{\in}[a,b]$ 时, 令 $f(x)=0$), 则有
\begin{align*}
...
\end{align*}
$g(x)$ 是 $[a,b]$ 上的可测函数. 上述引理指出, $F_h(x)$ 是平均收敛于 $f(x)$ 的 ($h\to 0$), 从而我们有
\begin{align*}
\int_a^b |f(x)-g(x)|\mathrm{d}x = \int_a^b \lim_{h\to 0} |f(x)-F_h(x)|\mathrm{d}x \leqslant \liminf_{h\to 0} \int_a^b |f(x)-F_h(x)|\mathrm{d}x = 0.
\end{align*}
...
\begin{align*}
\lim_{h\to 0} \frac{1}{h}\int_0^h |f(x+t)-f(x)|\mathrm{d}t = 0.
\end{align*}
...
现在设 $x$ 为 $[a,b]$ 中不属于 $Z$ 的任一点, 则对任给 $\varepsilon>0$, 必有 $r\in\mathbb{Q}\cap[a,b]$, 使得 $|f(x_0)-r|<\varepsilon/3$, 从而有
\begin{align*}
\big||f(t)-r|-|f(t)-f(x_0)|\big| < \varepsilon/3.
\end{align*}
由此知
\begin{align*}
\frac{1}{h}\int_x^{x+h} |f(t)-r|\mathrm{d}t - \frac{1}{h}\int_x^{x+h} |f(t)-f(x)|\mathrm{d}t < \frac{\varepsilon}{3}.
\end{align*}
因为 $x\overline{\in}Z$, 所以存在 $\delta>0$, 当 $0<|h|<\delta$ 时, 有
\begin{align*}
\left|\frac{1}{h}\int_x^{x+h} |f(t)-r|\mathrm{d}t - |f(x)-r|\right| < \frac{\varepsilon}{3}.
\end{align*}
综上所述, 我们有 (当 $0<|h|<\delta$ 时)
\begin{align*}
\frac{1}{h}\int_x^{x+h} |f(t)-f(x)|\mathrm{d}t < \varepsilon,
\end{align*}
即得所证.
...
\begin{proposition}
设 $f\in L([a,b])$, 且令 $F(x)=\int_a^x f(t)\mathrm{d}t$ ($x\in[a,b]$), 则 $F\in  \mathrm{BV}([a,b])$, 且
\begin{align*}
