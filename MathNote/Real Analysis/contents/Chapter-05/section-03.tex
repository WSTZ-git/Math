...
\[
\sum_{k=1}^n |f(y_i)-f(x_i)| < \varepsilon,
\]
则称 $f(x)$ 是 $[a,b]$ 上的\textbf{绝对连续函数}, 其全体记为 $\mathrm{AC}([a,b])$.
\end{definition}
\begin{remark}
显然, 我们有下述简单事实:
\begin{enumerate}[(1)]
...
\item 在 $[a,b]$ 上绝对连续函数的全体$\mathrm{AC}([a,b])$构成一个线性空间.
\end{enumerate}
\end{remark}
...
\]
则 $f(x)=g(x)$, a.e. $x\in[a,b]$, 其中 $g\in  \mathrm{BV}([a,b])$.
\end{example}

\begin{proof}
作 $[a,b]$ 上的函数列
\[
f_n(x) = n\int_x^{x+1/n} f(t)\mathrm{d}t\quad (n=1,2,\cdots),
\]
我们有
\begin{align*}
\int_a^b |f_n(x+h)-f_n(x)|\mathrm{d}x
&= n\int_a^b \left|\int_0^{1/n} (f(x+h+t)-f(x+t))\mathrm{d}t\right|\mathrm{d}x \\
&\leqslant \int_0^{1/n} \left(\int_a^b |f(x+h+t)-f(x+t)|\mathrm{d}x\right)\mathrm{d}t \\
&= O(|h|),\quad h\to 0 \quad (\text{对 } n \text{ 一致}).
\end{align*}
由于 $f_n'\in L([a,b])$, 而且存在 $M>0$, 使得
\[
\int_a^b |f_n'(x)|\mathrm{d}x \leqslant \liminf_{h\to 0} \int_a^b \left|\frac{f_n(x+h)-f_n(x)}{h}\right|\mathrm{d}x \leqslant M,
\]
故对 $[a,b]$ 的任一分割
\[
...
\end{example}
\begin{proof}
用反证法. 假定 $f'\in R([a,b])$, 则 $f'(x)$ 在 $[a,b]$ 上几乎处处连续, 且 $f\in  \mathrm{AC}([a,b])$. 现在设 $x=x_0$ 是 $f'(x)$ 的连续点, 则 $f'(x_0)=0$. 这是因为如果 $f'(x_0)>0$, 那么由题设知存在 $\{x_n\}: x_n\to x_0$ ($n\to x_0$), $f'(x_n)<0$, 且有 $0\geqslant \lim_{n\to\infty} f'(x_n)=f'(x_0)$, 导致矛盾. 从而可得 $f'(x)=0$, a.e. $x\in[a,b]$, 这又导致 $f(x)=$ 常数 ($x\in[a,b]$), 矛盾. 证毕.

\end{proof}

\begin{proposition}\label{proposition:绝对连续函数零测集的像仍零测}
设 $f\in  \mathrm{AC}([a,b])$, 则
\begin{enumerate}[(1)]
...
\begin{proof}
\begin{enumerate}[(1)]
\item 由绝对连续函数的定义不难进一步推知, 对任给的 $\varepsilon>0$, 存在 $\delta>0$, 以及满足 $\sum_{i=1}^\infty (y_i-x_i) < \delta$, $\bigcup_{i=1}^\infty (x_i,y_i)\supset Z\setminus\{a,b\}$ 的互不相交的开区间列$(x_i, y_i)$ ($i\in\mathbb{N}$),使得
\[
\sum_{i=1}^\infty |f(y_i)-f(x_i)| < \varepsilon.
\]
由于 $f(Z)\subset\bigcup_{i=1}^\infty [f(x_i),f(y_i)]$, 所以
\[
m(f(Z)) \leqslant \sum_{i=1}^\infty |f(y_i)-f(x_i)| < \varepsilon.
\]
显然, 对每个区间 $[x_i, y_i]$, 均可选取其中两点 $c_i, d_i$, 使得 $f([x_i, y_i]) = [f(c_i), f(d_i)]$, 从而得到
\begin{align*}
m(f(Z)) = m(f(Z\setminus\{a,b\})) \leqslant m\left(f\left(\bigcup_{i=1}^{\infty} (x_i, y_i)\right)\right) \leqslant \sum_{i=1}^{\infty} |f(d_i) - f(c_i)| \leqslant \varepsilon.
\end{align*}
...
\begin{example}
设 $f\in C([a,b])\cap  \mathrm{BV}([a,b])$. 若对 $[a,b]$ 中的任一零测集 $Z$, 必有 $m(f(Z))=0$, 则 $f\in  \mathrm{AC}([a,b])$.
\end{example}
\begin{proof}
用反证法. 假定 $f\notin  \mathrm{AC}([a,b])$, 则存在 $\varepsilon_0>0$, 使得对每个 $n\in\mathbb{N}$, 都有 $[a,b]$ 的子区间组
\[
I_{n,k} = [a_{n,k}, b_{n,k}] \quad (k=1,2,\dots,j_n), \quad \sum_{k=1}^{j_n} (b_{n,k} - a_{n,k}) \leqslant \frac{1}{2^n},
\]
\[
...
\]
这与\eqref{eq::923jri2jrijisofgzdx}式矛盾. 证毕.
