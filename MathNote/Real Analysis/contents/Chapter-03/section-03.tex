...
设 \( f(x) \) 为可测集 \( E \) 上几乎处处有限的实值函数, 若对 \( \forall\,\delta > 0 \), 存在闭集 \( F_{\delta} \subset E \), 使得 \( m(E \setminus F_{\delta}) < \delta \), 且 \( f(x) \) 在 \( F_{\delta} \) 上连续, 则 \( f(x) \) 是 \( E \) 上的可测函数.
\end{theorem}
\begin{proof}
对每个 \( n \in \mathbb{N} \), 都存在闭集 \( F_{n} \subset E \) 使得 \( m(E \setminus F_{n}) < \frac{1}{n} \), 且 \( f(x) \) 在 \( F_{n} \) 上连续, 故 \( f(x) \) 在 \( F_{n} \) 上可测.
