...
设 $\{f_n(x)\}$ 为 $E$ 上的可测函数列，对 $\forall\delta > 0$，存在 $E_\delta\subset E$，$m(E_\delta) < \delta$，使得 $\{f_n(x)\}$ 在 $E\backslash E_\delta$ 上一致收敛于 $f(x)$，则称 $\{f_n(x)\}$ 在 $E$ 上\textbf{(接)近一致收敛}于 $f(x)$. 
\end{definition}
...
设 $f(x),f_1(x),f_2(x),\cdots,f_k(x),\cdots$ 是 $E$ 上几乎处处有限的可测函数，且 $m(E)<+\infty$,则$f_k(x)\to f(x)$, a.e. $x\in E$的充要条件是
对任给的 $\delta>0$，存在 $E$ 的可测子集 $E_\delta$：$m(E_\delta)\leqslant\delta$，使得 $\{f_k(x)\}$ 在 $E\backslash E_\delta$ 上一致收敛于 $f(x)$.

这也等价于
对$\forall \delta >0$,存在$E$的可测子集$F_\delta$:$m(E\backslash F_\delta)<\delta$,使得$\{f_k(x)\}$在$F_\delta$上一致收敛于$f(x)$.
也即$\{f_n(x)\}$接近一致收敛于$f(x)$.
\end{theorem}
\begin{remark}
...
\end{remark}
\begin{remark}
等价条件的证明:$\Rightarrow$:对$\forall \delta >0$,只需令$F_\delta=E\backslash E_\delta$,则显然$F_\delta$为$E$的可测子集,且$E_\delta=E\backslash F_\delta$.从而$m(E\backslash F_\delta)=m(E_\delta)<\delta$且$\{f_k(x)\}$也在$E\backslash E_\delta=F_\delta$上一致收敛于$f(x)$.
...
{\heiti 充分性:}分别取 $\delta_k = 1/k$，$k = 1,2,\cdots$，则存在 $F_k\subset E$，$m(F_k)<1/k$，使得$f_n(x)$在每个$E\backslash F_k$上均一致收敛于$f(x)$.
...
\end{align*}
故对 $\forall x_0\in E\backslash F$，存在 $k_0\in\mathbb{N}$，使得 $x_0\in E\backslash F_{k_0}$. 又 $f_k$ 在 $E\backslash F_{k_0}$ 上一致收敛于 $f$，从而$\lim_{k\to \infty}f_k(x)=f(x),\forall x\in E\backslash F_{k_0}$,于是$\lim_{k\to \infty}f_k(x_0)=f(x_0)$.故由$x_0$的任意性可得$f_k(x)$ 在 $E\backslash F$ 上处处收敛于 $f(x)$.因此$f_k(x)\to f(x)$, a.e. $x\in E$.
\end{proof}
...
\begin{theorem}\label{theorem:几乎处处收敛在E的测度有限前提下必依测度收敛}
...
设 $f(x),f_1(x),f_2(x),\cdots,f_k(x)\cdots$ 是 $E$ 上几乎处处有限的可测函数. 若对任给的 $\delta>0$，存在 $E_\delta\subset E$ 且 $m(E_\delta)<\delta$，使得 $\{f_k(x)\}$ 在 $E\backslash E_\delta$ 上一致收敛于 $f(x)$(即$\{f_k(x)\}$接近一致收敛于$f(x)$)，则 $\{f_k(x)\}$ 在 $E$ 上依测度收敛于 $f(x)$.
\end{theorem}
\begin{proof}
