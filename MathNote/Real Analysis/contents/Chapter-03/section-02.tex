...
\end{theorem}
\begin{proof}
因为 $\{f_k(x)\}$ 依测度收敛于 $f(x)$，所以 $\{f_k(x)\}$ 是依测度Cauchy列. 从而由\refthe{theorem:定理3.16}的证明可知，存在子列 $\{f_{k_i}(x)\}$ 以及可测函数 $g(x)$，使得$\{f_{k_i}(x)\}$接近一致收敛于$g(x)$,再由\hyperref[theorem:Egorov定理的逆定理]{Egorov定理的逆定理}知
\begin{align*}
