...
设$0 < m(A) < +\infty$，$f(x)$是$A \subset \mathbb{R}^n$上的非负可测函数，且有$0 < f(x) < +\infty,\,\, \text{a.e.}x \in A$，则对$\forall \delta$：$0 < \delta < m(A)$，存在$B \subset A$以及自然数$k_0$，使得
\begin{align*}
m(A \setminus B) < \delta, \quad \frac{1}{k_0} \leqslant f(x) \leqslant k_0, \quad x \in B.
\end{align*}
\end{proposition}
\begin{proof}
记$A_k = \{x \in A: \frac{1}{k} \leqslant f(x) \leqslant k\}$（$k = 1, 2, \cdots$），$Z_1 = \{x \in A: f(x) = 0\}$，$Z_2 = \{x \in A: f(x) = +\infty\}$，易知$m(Z_1) = m(Z_2) = 0$，且有
\begin{align*}
