...
为了论述的简便和统一，今后我们在谈到可测函数时允许函数取“值”\(\pm\infty\).（称 \(\mathbb{R}\cup\{+\infty\}\cup\{-\infty\}\) 为广义实数集.）现在先将有关 \(\pm\infty\) 的运算规则约定如下（注意，这里的 \(\pm\infty\) 不是指无穷大变量）：

(i) \(-\infty < +\infty\)，若 \(x \in \mathbb{R}\)，则 \(-\infty < x < +\infty\)；

(ii) 若 \(x \in \mathbb{R}\)，则
\begin{align*}
...
设 \(f(x)\) 是可测集 \(E\) 上的函数，\(D\) 是 \(\mathbb{R}\) 中的一个稠密集. 若对任意的 \(r \in D\)，点集 \(\{x: f(x) > r\}\) 都是可测集，则对任意的 \(t \in \mathbb{R}\)，点集 \(\{x: f(x) > t\}\) 也是可测集.进而$f(x)$在$E$上可测.
\end{theorem}
\begin{note}
...
\end{proposition}
\begin{proof}
事实上，对于任意的 \(t \in \mathbb{R}\)，点集 \(\{x \in [a,b]: f(x) > t\}\) 定属于下述三种情况之一：区间、单点集或空集. 从而可知
\[
\{x \in [a,b]: f(x) > t\}
\]
...
(i) \(\{x: f(x) \leqslant t\} = E \setminus \{x: f(x) > t\}\) \((t \in \mathbb{R})\)；

(ii) \(\{x: f(x) \geqslant t\} = \bigcap_{k = 1}^{\infty}\left\{x: f(x) > t - \frac{1}{k}\right\}\) \((t \in \mathbb{R})\)；

(iii) \(\{x: f(x) < t\} = E \setminus \{x: f(x) \geqslant t\}\) \((t \in \mathbb{R})\)；

(iv) \(\{x: f(x) = t\} = \{x: f(x) \geqslant t\} \cap \{x: f(x) \leqslant t\}\) \((t \in \mathbb{R})\)；
...
\item (i) 对$\forall t \in \mathbb{R}$,显然有$\left\{x: \sup_{k \geqslant 1}\{f_k(x)\} > t\right\} \supset \bigcup_{k = 1}^{\infty}\{x: f_k(x) > t\}.$任取$x_0\in \left\{x: \sup_{k \geqslant 1}\{f_k(x)\} > t\right\}$,则$ \sup_{k \geqslant 1}\{f_k(x_0)\} > t$
...
0,&f\left( x \right) \leqslant 0\\
\end{cases},
\\
f^-(x)=\max \{-f(x),0\}=\begin{cases}
-f\left( x \right) ,&f\left( x \right) <0\\
0,&f\left( x \right) \geqslant 0\\
\end{cases},
\]
...
\]
因为对任意的 \(t \in \mathbb{R}\)，显然有$\{(x,y) \in \mathbb{R}^2: f_n(x,y) < t\} 
\supset \bigcup_{k = -\infty}^{\infty}\left\{x \in \mathbb{R}: f\left(x,\frac{k}{n}\right) < t\right\} \times \left(\frac{k - 1}{n}, \frac{k}{n}\right].$任取$(x_0,y_0)\in \{(x,y) \in \mathbb{R}^2: f_n(x,y) < t\},$则$f_n(x_0,y_0)<t$.从而存在$k_0\in \mathbb{Z}$,使得$\frac{k_0-1}{n}<y_0 \leqslant  \frac{k}{n}$,并且$f(x_0,\frac{k_0}{n})=f_n(x_0,y_0)<t$.于是
\begin{align*}
(x_0,y_0)\in\left\{x \in \mathbb{R}: f\left(x,\frac{k_0}{n}\right) < t\right\} \times \left(\frac{k_0 - 1}{n}, \frac{k_0}{n}\right] \subset \bigcup_{k = -\infty}^{\infty}\left\{x \in \mathbb{R}: f\left(x,\frac{k}{n}\right) < t\right\} \times \left(\frac{k - 1}{n}, \frac{k}{n}\right].
\end{align*}
因此$\{(x,y) \in \mathbb{R}^2: f_n(x,y) < t\} 
\subset \bigcup_{k = -\infty}^{\infty}\left\{x \in \mathbb{R}: f\left(x,\frac{k}{n}\right) < t\right\} \times \left(\frac{k - 1}{n}, \frac{k}{n}\right].$
故
\begin{align*}
\{(x,y) \in \mathbb{R}^2: f_n(x,y) < t\} 
&= \bigcup_{k = -\infty}^{\infty}\left\{x \in \mathbb{R}: f\left(x,\frac{k}{n}\right) < t\right\} \times \left(\frac{k - 1}{n}, \frac{k}{n}\right],
\end{align*}
所以由条件及\hyperref[theorem:可测集的性质]{可测集的性质(6)}可知\(f_n(x,y)\) 是 \(\mathbb{R}^2\) 上的可测函数. 而由题设易知
\[
\lim_{n \to \infty}f_n(x,y) = f(x,y), \quad (x,y) \in \mathbb{R}^2,
\]
再由\refcor{corollary:可测函数列的极限也可测}即得所证.
\end{proof}

\begin{proposition}[连续函数必可测]\label{proposition:连续函数必可测}
设 \(E \subset \mathbb{R}^n\) 是可测集. 若 \(f \in C(E)\)，则 \(f(x)\) 是 \(E\) 上的可测函数. 
\end{proposition}
\begin{proof}
...
设$0 < m(A) < +\infty$，$f(x)$是$A \subset \mathbb{R}^n$上的非负可测函数，且有$0 < f(x) < +\infty,\,\, \text{a.e.}x \in A$，则对$\forall \delta$：$0 < \delta < m(A)$，存在可测子集$B \subset A$以及自然数$k_0$，使得
\begin{align*}
m(A \setminus B) < \delta, \quad \frac{1}{k_0} \leqslant f(x) \leqslant k_0, \quad x \in B.
\end{align*}
\end{proposition}
\begin{proof}
记$A_k = \{x \in A: \frac{1}{k} \leqslant f(x) \leqslant k\}$（$k = 1, 2, \cdots$），$Z_1 = \{x \in A: f(x) = 0\}$，$Z_2 = \{x \in A: f(x) = +\infty\}$，易知$A_k$都可测,$m(Z_1) = m(Z_2) = 0$，且有
\begin{align*}
...
\end{proposition}
\begin{proof}
由\refthe{theorem:简单函数是有限个特征函数的线性组合}易证.
\end{proof}
...
(a) 若$f(x_0) \leqslant M$，则对$\forall k > M$，都有$x_0\in \{x\in E:0\leqslant f(x)<k\}=\bigcup_{j=1}^{\infty}E_{k,j}$.从而存在$j_0\in \{1,2,\cdots,k2^k\},$使得$x_0\in E_{k,j_0}$,即
\begin{align*}
...
(b) 若$f(x_0) = +\infty$，则对$\forall k\in \mathbb{N}$,都有$x_0\in E_k$.从而此时就有$\varphi_k(x_0) = k (k = 1, 2, \cdots).$令$k\to \infty$得$$\lim_{k \to \infty} \varphi_k(x_0) =+\infty = f(x_0).$$
...
\end{align*}
若在$E$上有$|f(x)| \leqslant M$，则当$k > M$时,由(1)(a)同理可知
\begin{align*}
...
\\
&\leqslant \underset{x\in E}{\mathrm{sup}}\left| f^+\left( x \right) -\varphi _{k}^{\left( 1 \right)}\left( x \right) \right|+\underset{x\in E}{\mathrm{sup}}\left| f^-\left( x \right) -\varphi _{k}^{\left( 2 \right)}\left( x \right) \right|
\\
...
\end{align*} 
并且若$\varphi_k(x)\leqslant \varphi_{k+1}(x),\forall x\in E$,则当$x\in E\cap B(0,k) \subset E\cap B(0,k + 1)$时,有$\chi _{B\left( 0,k \right)}\left( x \right) \leqslant \chi _{B\left( 0,k+1 \right)}\left( x \right)$,进而
\begin{align*}
...
\end{theorem}
\begin{proof}
由$f$是$\mathbb{R}$上的实值可测函数知,$f(x)<+\infty$.作$E_{k,n} = \{x \in \mathbb{R}: f(x) \in [k2^{-n}, (k + 1)2^{-n}), k \in \mathbb{Z}, n \in \mathbb{N}\}$，且令
\begin{align*}
