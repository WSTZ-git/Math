...
\end{align*}
$f(x)$的系数为$\xi,\xi^p,\cdots,\xi^{p^{n-1}}$的对称多项式，任取系数$g=g(\xi,\xi^p,\cdots,\xi^{p^{n-1}})$，有
\begin{align*}
g^p=g(\xi^p,\cdots,\xi^{p^{n-1}},\xi)=g(\xi,\xi^p,\cdots,\xi^{p^{n-1}})=g.
\end{align*}
...
$K$上左(右)线性空间$V$中，向量组$v_1,v_2,\cdots,v_s$生成的子空间记为$L(v_1,v_2,\cdots,v_s)$。
\end{remark}

\begin{example}
在$K^{1\times n}=\{(\alpha_1,\alpha_2,\cdots,\alpha_n)\mid\alpha_i\in K\}$中定义加法与数乘：
\begin{align*}
(\alpha_1,\alpha_2,\cdots,\alpha_n)+(\beta_1,\beta_2,\cdots,\beta_n)=(\alpha_1+\beta_1,\alpha_2+\beta_2,\cdots,\alpha_n+\beta_n),\\
\alpha(\alpha_1,\alpha_2,\cdots,\alpha_n)=(\alpha\alpha_1,\alpha\alpha_2,\cdots,\alpha\alpha_n),
\end{align*}
...
仅证明有限维情形。
设$v_1,v_2,\cdots,v_m$是$K$对$L$的基，即$m=[K:L]$；$u_1,u_2,\cdots,u_n$是$L$对$M$的基，即$n=[L:M]$。
