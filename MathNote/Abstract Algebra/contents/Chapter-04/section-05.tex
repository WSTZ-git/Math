...
\begin{corollary}
设$F(\alpha_1,\alpha_2,\cdots,\alpha_n)$是域$F$的代数扩张，且$\alpha_1,\alpha_2,\cdots,\alpha_n$是$F$上的可分元素，则该扩张为$F$上的可分扩张。
\end{corollary}
\begin{proof}
...
\end{align*}
则$f(x)$是$F$上可分多项式，其分裂域$K$是$F$上的可分扩张。对任意$\alpha\in F(\alpha_1,\alpha_2,\cdots,\alpha_n)\subseteq K$，$\alpha$是$F$上的可分元素，故$F(\alpha_1,\alpha_2,\cdots,\alpha_n)$是$F$上的可分扩张。
...
\begin{theorem}
设$K=F(\alpha_1,\alpha_2,\cdots,\alpha_n)$是域$F$的代数扩张，且$\alpha_1,\alpha_2,\cdots,\alpha_n$是$F$上的可分元素，则$K$中有本原元素，即$K$是$F$的单代数扩张。
\end{theorem}
\begin{proof}
...
设$n-1$时定理成立，即存在$\beta\in F(\alpha_1,\alpha_2,\cdots,\alpha_{n-1})$，使得$F(\alpha_1,\alpha_2,\cdots,\alpha_{n-1})=F(\beta)$，于是$K=F(\beta,\alpha_n)$。
...
结合$f(x),\mathrm{Irr}(\alpha_n,F)\in F(\theta)[x]$，得$\alpha_n\in F(\theta)$，进而$\beta=\theta-c\alpha_n\in F(\theta)$。于是$F(\theta)\subseteq F(\beta,\alpha_n)\subseteq F(\theta)$，即$F(\theta)=F(\alpha_1,\alpha_2,\cdots,\alpha_n)$，$\theta$是本原元素。
\end{enumerate}
...
\begin{align*}
\mathrm{Irr}(\alpha,K)=x^n+a_{n-1}x^{n-1}+\cdots+a_0,
\end{align*}
其为$K$上可分不可约多项式，故$\alpha$是$F(a_0,a_1,\cdots,a_{n-1})$上的可分元素。

$K$是$F$的可分扩张，故$a_0,a_1,\cdots,a_{n-1}$是$F$上的可分元素。由本原元定理，存在$\theta$使得$F(a_0,a_1,\cdots,a_{n-1})=F(\theta)$。
