...
首先证明 $f$ 是良定义的。根据\reflemma{lemma:稳定化子引理1}，若 $x\,\mathrm{Stab}(s)=y\,\mathrm{Stab}(s)$，则由\reflemma{lemma:关于两个陪集相等的充要条件}可知$x^{-1}y\in\mathrm{Stab}(s)$，故 $xs = ys$。

根据 $\mathrm{Orb}(s)$ 的定义，$f$ 显然是一个满射。

单射则是再次利用\reflemma{lemma:稳定化子引理1}。若 $xs = ys$，则 $x^{-1}y\in\mathrm{Stab}(s)$，故 $x\,\mathrm{Stab}(s)=y\,\mathrm{Stab}(s)$。

假如 $G$ 是有限群，则同时取集合大小，由\reftheorem{theorem:Lagrange定理}就得到了
\begin{align*}
|G| = |\mathrm{Stab}(s)|\cdot|\mathrm{Orb}(s)|
\end{align*}
