...
若$f$还是单射,则称$f$为\textbf{单同态},或$G_1$到$G_2$的\textbf{嵌入映射}.
...
\item\label{theorem:内自同构群及其基本性质-4} \( \text{ad}: g \to \text{ad}g \) 是群 \( G \) 到 \( \text{Int}G \) 上的满同态,其中$\text{ad}g(x)=gxg^{-1},\forall g\in G$.并且$\text{Int}G\cong G/C(G).$

特别地,当$C(G)=\{1\}$时,\( \text{ad}: g \to \text{ad}g \) 是群 \( G \) 到 \( \text{Int}G \) 上的同构.
\end{enumerate}
\end{theorem}
\begin{proof}\begin{enumerate}[(1)]
...
显然$\text{ad}$是满同态.注意到
\begin{align*}
\mathrm{ker}\,\mathrm{ad}=\left\{ g\in G\mid \mathrm{ad}g=\mathrm{id}_G \right\} =\left\{ g\in G\mid \mathrm{ad}g\left( x \right) =gxg^{-1}=x,\forall x\in G \right\} =C\left( G \right) ,
\end{align*}
故由\hyperref[theorem:群的同态基本定理]{群的同态基本定理}知$\mathrm{Int}G\cong G/C\left( G \right)$.
...
\begin{proposition}
\begin{enumerate}[(1)]
\item 有理数加法群 $(\mathbb{Q},+)$ 的自同构群为
\begin{align*}
\mathrm{Aut}(\mathbb{Q},+) = \{f_a \mid a \in \mathbb{Q}, a \neq 0\}.
\end{align*}
并且$\mathrm{Aut}(\mathbb{Q},+) \cong (\mathbb{Q}^*,\cdot)$,其中 $(\mathbb{Q}^*,\cdot)$ 是非零有理数的乘法群.

\item \begin{enumerate}[(i)]
\item 实数域$\mathbb{R}$上的自同构群为$\text{Aut}(\mathbb{R})=\{\text{id}_{\mathbb{R}}\}$.

\item 实数加法群$(\mathbb{R},\times)$上的自同构群满足$\text{Aut}(\mathbb{R},\times)\cong \text{GL}_{\mathbb{Q}}(\mathbb{R})$.

\item 实数乘法群$\mathbb{R}^{\times}$上的自同构群满足$\text{Aut}(\mathbb{R}^{\times})\cong \text{GL}_{\mathbb{Q}}(\mathbb{R})$.
\end{enumerate}
\end{enumerate}
\end{proposition}
\begin{proof}\begin{enumerate}[(1)]
\item 一方面, 由\refpro{proposition:左右平移是群的自同构}知对任一非零有理数 $a$, $L_a$是将 $x$ 变成 $ax$ 是有理加法群 $\mathbb{Q}$ 的一个自同构. 另一方面, 设 $f$ 是有理数加群 $\mathbb{Q}$ 的任一自同构. 令 $f(1)=a$, 则 $a$ 是非零有理数. 因为对任一正整数 $n$ 和 $m$ 有
\begin{gather*}
...
\item \begin{enumerate}[(i)]
\item 设$\sigma:\mathbb{R}\to\mathbb{R}$是域自同构, 则$\sigma(1)=1$, 从而$\sigma(q)=q$对任意$q\in\mathbb{Q}$成立. 对任意$a\geqslant 0$, 存在$b\in\mathbb{R}$使得$a=b^{2}$, 于是$\sigma(a)=\sigma(b)^{2}\geqslant 0$, 故$\sigma$保持序关系. 对任意$x\in\mathbb{R}$, 取有理数列$\{r_{n}\}$满足$r_{n}\to x$, 由$r_{n}\leqslant x$得$r_{n}=\sigma(r_{n})\leqslant\sigma(x)$, 由$r_{n}\geqslant x$得$r_{n}\geqslant\sigma(x)$, 令$n\to\infty$得$\sigma(x)=x$. 因此只有恒等自同构.

\item $\mathbb{R}$视为向量空间,其标量域取$\mathbb{Q}$. 先证明任何加法群同态$\varphi:\mathbb{R}\to\mathbb{R}$是$\mathbb{R}$上的线性映射. 对任意$x\in\mathbb{R}$, 由归纳法得$\varphi(nx)=n\varphi(x)$对$n\in\mathbb{N}$成立, 又$\varphi(0)=0$, 且$\varphi(-x)=-\varphi(x)$, 故对任意$n\in\mathbb{Z}$有$\varphi(nx)=n\varphi(x)$. 对任意有理数$q=\frac{m}{n}$ ($m\in\mathbb{Z}$, $n\in\mathbb{N}^*$), 则
$$
n\varphi(qx)=\varphi(nqx)=\varphi(mx)=m\varphi(x),
$$ 
因此
$$
\varphi(qx)=\frac{m}{n}\varphi(x)=q\varphi(x).
$$
所以$\varphi$是$\mathbb{R}$上的线性映射.

反之, 任何$\mathbb{R}$上的线性映射显然是加法群同态. 于是加法群同态与$\mathbb{R}$上的线性映射一一对应. 可逆的加法自同构对应可逆的$\mathbb{R}$上的线性映射, 即
$\operatorname{GL}_{\mathbb{Q}}(\mathbb{R})$.因此
$$
\operatorname{Aut}(\mathbb{R},+)\cong\operatorname{GL}_{\mathbb{Q}}(\mathbb{R}).
$$

\item 记$\mathbb{R}^{\times}=\mathbb{R}\setminus\{0\}$, $P=\{x\in\mathbb{R}^{\times}\mid x>0\}$. 对任意$x\in P$, 存在$y=\sqrt{x}\in P$使得$x=y^{2}$, 故$P$是$\mathbb{R}^{\times}$中所有平方元素构成的子群, 称为平方子群. 由于平方运算在自同构下保持,故
\begin{align*}
\tau(P)=P,\quad \forall \tau\in\operatorname{Aut}(\mathbb{R}^{\times}).
\end{align*}
考虑符号映射$\operatorname{sgn}:\mathbb{R}^{\times}\to\{\pm1\}$, 则映射
\begin{align*}
f:\mathbb{R}^{\times}\to \{\pm1\}\times P,\,\,x\mapsto(\operatorname{sgn}(x),|x|)
\end{align*}
是同构映射.故
\begin{align}\label{eqMJIOFJIOWEJFIOJIOJIWJ32904890238}
\mathbb{R}^{\times}\cong\{\pm1\}\times P.
\end{align}
定义
\begin{align*}
\Phi : \operatorname{Aut}(\{\pm 1\} \times P) \to \operatorname{Aut}(P), \quad \Phi(\tau)(p) = \pi_2(\tau(1, p)),
\end{align*}
其中$\pi_2$是到第二个分量$P$的投影，即
\begin{align*}
\tau:\{\pm 1\}\times P\to P,\,\,(\pm 1, p) = p.
\end{align*}
显然$\Phi(\tau)\in \text{Aut}(P),\forall \tau \in \operatorname{Aut}(\{\pm 1\} \times P)$,故$\Phi$是良定义的.
再定义映射
\begin{align*}
\Psi : \operatorname{Aut}(P) \to \operatorname{Aut}(\{\pm 1\} \times P), \quad \Psi(\sigma)(\pm 1, p) = (\pm 1, \sigma(p)).
\end{align*}
容易验证$\Psi(\sigma)\in \operatorname{Aut}(\{\pm 1\}),\forall \sigma \in \operatorname{Aut}(P)$,故$\Psi$也是良定义的.并且
$$
\Phi \circ \Psi = \operatorname{id}_{\operatorname{Aut}(P)},\quad \Psi \circ \Phi = \operatorname{id}_{\operatorname{Aut}(\{\pm 1\} \times P)}.
$$
因此$\Phi$是$\operatorname{Aut}(\{\pm 1\} \times P) \to \operatorname{Aut}(P)$的同构映射,即
\begin{align}\label{eqMJIOFJIOWEJFIOJIOJIWJ32904890238-1}
\operatorname{Aut}(\{\pm 1\} \times P) \cong \operatorname{Aut}(P).
\end{align}
定义映射$\ln:P\to\mathbb{R}$, $x\mapsto\ln x$, 它给出乘法群$P$与加法群$(\mathbb{R},+)$的同构. 因此$\operatorname{Aut}(P)\cong\operatorname{Aut}(\mathbb{R},+)$. 由(2)的结论知$\operatorname{Aut}(\mathbb{R},+)\cong\operatorname{GL}_{\mathbb{Q}}(\mathbb{R})$. 从而$$\operatorname{Aut}(P)\cong\operatorname{GL}_{\mathbb{Q}}(\mathbb{R}).$$
于是再结合\eqref{eqMJIOFJIOWEJFIOJIOJIWJ32904890238}\eqref{eqMJIOFJIOWEJFIOJIOJIWJ32904890238-1}式知
\begin{align*}
\operatorname{Aut}(\mathbb{R}^{\times})\cong \operatorname{Aut}(\{\pm 1\} \times P)\cong \operatorname{Aut}(P)\cong\operatorname{GL}_{\mathbb{Q}}(\mathbb{R}).
\end{align*}
\end{enumerate}
\end{enumerate}
\end{proof}
