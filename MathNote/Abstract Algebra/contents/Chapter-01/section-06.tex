...
\begin{theorem}\label{theorem:域上的线性空间}
数域 \( P \) 上的线性空间 \( V \) 就是一个 \( \boldsymbol{P} \) \textbf{模}. 

一般地, 域 \( F \) 上的模都称为 \( F \) 上的\textbf{线性空间}.
\end{theorem}
\begin{proof}
...
\begin{enumerate}[(1)]
\item \( N \) 是 \( M \) 的子群;
\item \( \forall a \in R, x \in N \) 有 \( ax \in N \),即对$\forall x\in N$,有$Rx\subseteq N$
\end{enumerate}
...
\end{theorem}
\begin{proof}
设$x\in M$,$a,b\in R$,于是$ax-bx=(a-b)x\in Rx$,故$Rx$是$M$的子群.又 $aRx\subseteq Rx$.因此$Rx$为$M$的子模,显然$x\neq 0$,则$Rx\neq 0$.
