...
\end{definition}
\begin{note}
不难发现，$0,\cdots,n - 1$ 分别代表了 $n$ 个同余类。并且由\refproposition{proposition:nZ是Z的正规子群}可知$\mathbb{Z}_n$是一个商群.
\end{note}
...
\begin{align*}
\mathbb{Z} _{n}^{\times}=\{k+n\mathbb{Z} :1\leqslant k\leqslant n-1,\mathrm{gcd(}k,n)=1\}=\{\overline{k}:0\leqslant k\leqslant n-1,\mathrm{gcd(}k,n)=1\}.
\end{align*}
因此
\begin{align*}
...
\gcd(k, p) = 1 \iff p \nmid k.
\end{align*}
当然，从$1$到$p - 1$的这些数，都和$p$互素.只有$0$与$p$不互素.因此
\begin{align*}
\mathbb{Z}_p^\times = \{1 + p\mathbb{Z}, 2 + p\mathbb{Z}, \cdots, (p - 1) + p\mathbb{Z}\} .
\end{align*}
故
...
\begin{lemma}\label{lemma:有限群关于元素阶次方引理}
令$(G, \cdot)$是个有限群，则对任意$a \in G$，$a^{|G|} = e$。
\end{lemma}
\begin{proof}
令$\langle a \rangle$是由$a$生成的循环子群。则由拉格朗日定理，
\begin{align*}
|\langle a \rangle| \bigm| |G|
\end{align*}
而我们知道
\begin{align*}
|a| = |\langle a \rangle|
...
\end{theorem}
\begin{proof}
根据$(\mathbb{Z}_p, \cdot)$中乘法的良定义性，我们不失一般性，假设
\begin{align*}
1 \leqslant a \leqslant p - 1
\end{align*}
因此$a \in \mathbb{Z}_p^\times$。根据上面的引理，
\begin{align*}
a^{|\mathbb{Z}_p^\times|} = e
\end{align*}
此即
\begin{align*}
a^{p - 1} \equiv 1 \pmod{p}
