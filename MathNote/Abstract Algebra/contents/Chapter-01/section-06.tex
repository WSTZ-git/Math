...
设 \( M, M' \) 是两个 Abel 群, \( \eta \) 是 \( M \) 到 \( M' \) 的群同态, 则 \( \eta \) 也是 \( \mathbf{Z} \) 模 \( M \) 到 \( \mathbf{Z} \) 模 \( M' \) 的模同态; 
...
\begin{example}
设 \( M \) 为 Abel 群, 于是 \( M \) 为 \( \mathbf{Z} \) 模. 则由\refpro{proposition:抽象代数-例题1.6.10}知 \( \text{End}_\mathbf{Z} M = \text{End}M \).
\end{example}
\begin{proof}
