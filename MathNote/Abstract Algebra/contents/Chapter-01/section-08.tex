...
\end{definition}
\begin{remark}
对$\forall n_1,n_2\in \mathbf{Z}$,有$a^{n_1}a^{-n_2}\in G$.因此$\langle a \rangle$是$G$的子群.故由$a$生成的$G$的子群是良定义的.
\end{remark}
...
\end{proposition}
\begin{proof}
由$p>1$知$G$中至少存在一个非幺元$a\ne e$，则$\langle a\rangle$是$G$的子群。由Lagrange定理知$\langle a\rangle$的阶是$|G|=p$的因数，而$p$为素数，故$\langle a\rangle$的阶为$1$或$p$。由$a,e\in\langle a\rangle$知$\langle a\rangle$的阶必大于$1$，因此$\langle a\rangle$的阶为$p$。又因为$\langle a\rangle\subseteq G$，所以$G=\langle a\rangle$。故$G$为循环群。
...
\begin{enumerate}[(1)]
\item 设 \( m \in \mathbf{Z} \), 则 \( m\mathbf{Z} \triangleq  \{mx|x \in \mathbf{Z}\} \) 是整数加法群 \( \mathbf{Z} \) 的子群. 

\item\label{corollary:抽象代数--定理1.8.1推论-2} 整数加法群$\mathbf{Z}$的任何子群必为$m\mathbf{Z}(m\geqslant 0\text{且}m\in \mathbf{Z})$.
\end{enumerate}
\end{corollary}
\begin{proof}
\begin{enumerate}[(1)]
\item 对$\forall x_1,x_2\in \mathbf{Z},$有
\begin{align*}
mx_1-mx_2=m(x_1-x_2)\in m\mathbf{Z}.
\end{align*}
故$m\mathbf{Z}$是整数加法群 \( \mathbf{Z} \) 的子群. 

\item 事实上, $\mathbf{Z}=\langle 1\rangle$. 设$G_1$为$\mathbf{Z}$的子群. 于是由\refthe{theorem:抽象代数--定理1.8.1}有$m\geqslant 0\text{且}m\in \mathbf{Z}$, 使得$G_1=\langle m\rangle=m\mathbf{Z}$.
\end{enumerate}
...
设 \( m > 0\), 则有
\begin{align*}
m\mathbf{Z} \lhd  \mathbf{Z},\quad \mathbf{Z} = \bigcup_{k=0}^{m-1} (k + m\mathbf{Z}), \quad \mathbf{Z}_m \triangleq  \mathbf{Z}/m\mathbf{Z}  = \{\overline{0}, \overline{1}, \cdots, \overline{m-1}\} ,\quad [\mathbf{Z}: m\mathbf{Z}] = m.
\end{align*}
\end{proposition}
\begin{proof}
由\rrefcor{corollary:抽象代数--定理1.8.1推论}{corollary:抽象代数--定理1.8.1推论-1}知\(m\mathbf{Z} \) 为 \( \mathbf{Z} \) 的子群.

\end{proof}

\begin{theorem}\label{theorem:循环群都同构于整数加群}
设$G=\langle a\rangle$是一个循环群, 若$G$是无限阶的, 则$G$与整数加法群$\mathbf{Z}$同构. 若$G$的阶$m$有限, 则$G$与加法群$\mathbf{Z}_m$同构.
进而两个循环群同构当且仅当它们的阶相同.
\end{theorem}
\begin{proof}
作$\mathbf{Z}$到$G$上的映射$\varphi:\varphi(n)=a^n(n\in\mathbf{Z})$. 于是有
\[
\varphi(n_1+n_2)=a^{n_1+n_2}=a^{n_1}\cdot a^{n_2}=\varphi(n_1)\varphi(n_2),
\]
因而$\varphi$是$\mathbf{Z}$到$G$上的同态映射, 故由\hyperref[theorem:群的同态基本定理]{群的同态基本定理}知$G\cong \mathbf{Z}/\ker\varphi$且$\ker \varphi \lhd \mathbf{Z}$. 由\rrefcor{corollary:抽象代数--定理1.8.1推论}{corollary:抽象代数--定理1.8.1推论-2}知存在$m\geqslant 0\text{且}m\in \mathbf{Z},$使得$\ker\varphi=m\mathbf{Z}$.

若$m>0$, 则由\refpro{proposition:整数加群的商群}知,此时$G\cong \mathbf{Z}/\ker\varphi=\mathbf{Z}/m\mathbf{Z}=\mathbf{Z}_m$且$|G|=|\mathbf{Z}_m|=m$.
...
\end{corollary}
\begin{proof}
设$G$为无限循环群,则由\refthe{theorem:循环群都同构于整数加群}知$G\cong \mathbf{Z}.$
又由\rrefcor{corollary:抽象代数--定理1.8.1推论}{corollary:抽象代数--定理1.8.1推论-2}知$\mathbf{Z}$的非平凡子群为$m\mathbf{Z}(m\neq 0,1)$为无限循环群.故$G$的非平凡子群也为无限循环群.
...
\begin{corollary}
\begin{enumerate}[(1)]
\item 若$a$为$m$阶元素，则$a^k$为$m$阶元素的充要条件是$(m,k)=1$；
\item 若$a,b$的阶分别为$m,n$且$ab=ba$，$(m,n)=1$，则$ab$的阶为$mn$.
\end{enumerate}
\end{corollary}
\begin{proof}
\begin{enumerate}[(1)]
