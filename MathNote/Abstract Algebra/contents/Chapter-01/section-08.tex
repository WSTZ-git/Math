...
对任意整数 \(n\)，由带余除法，存在唯一的 \(r \in \{0,1,\cdots,m-1\}\) 和 \(q \in \mathbb{Z}\) 使得 \(n = r + mq\)。因此 \(n \in r + m\mathbb{Z}\)，所以 \(\mathbb{Z} = \bigcup_{k=0}^{m-1} (k + m\mathbb{Z})\)。若 \(k_1 + m\mathbb{Z} = k_2 + m\mathbb{Z}\)，则 \(k_1 - k_2 \in m\mathbb{Z}\)，即 \(m \mid (k_1 - k_2)\)。由于 \(0 \leq k_1,k_2 \leq m-1\)，故 \(k_1 = k_2\)。因此这些陪集两两不交。
