...
由此知 \( \langle a \rangle = \{1, a, \cdots, a^{d-1}\} \), 即 \( \langle a \rangle \) 是 \( d \) 阶群. 故由\hyperref[theorem:Lagrange定理--抽象代数]{Lagrange定理}知 \( d \) 为 \( [G:1] \) 的因子.
...
\end{theorem}
\begin{proof}
设$G_1,G_2$是$G$的两个不同子群,则由\hyperref[theorem:Lagrange定理--抽象代数]{Lagrange定理}知$[G_1:1],[G_2:1]$都是$m$的因数.
