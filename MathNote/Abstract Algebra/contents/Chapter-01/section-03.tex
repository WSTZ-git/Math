...
\]
所确定的 \( G \) 中的关系 \( R \) 是一个等价关系, 并且 \( a \) 所在的等价类为 \( aH \), 故 \( H \)的左陪集族 \( \{aH:a\in G\} \)(集合无相同元素) 是 \( G \) 的一个分划.即
\begin{align*}
G = \bigsqcup_{a\in G}aH,
\end{align*}
...
\item\label{theorem:子群陪集相关性质-5} 若$H$还是有限群,则$|aH|=|bH|=|Ha|=|Hb|=|aHb|$.
\end{enumerate}
\end{theorem}
\begin{proof}
\begin{enumerate}[(1)]
...
\item\label{proposition:正规子群的基本性质-7} 设$G$是一个群,$N_1,N_2,\cdots N_k\lhd G$,且$N_i\cap N_j=\{1\}(i\neq j)$,则对$\forall i,j\in \{1,2,\cdots,k\},$有
\begin{align*}
n_in_j=n_jn_i,\,\,\forall n_i\in N_i,n_j\in N_j.
\end{align*}
并且
\begin{align*}
N_j\subseteq C_G(N_i) = \{g\in G\mid gn_i = n_ig ,\forall n_i\in N_i\},\,\,\forall i,j\in \{1,2,\cdots,k\}.
\end{align*}
\end{enumerate}
\end{proposition}
\begin{proof}
...
\item {\color{blue}证法一:}对$\forall i,j\in \{1,2,\cdots,k\}$,由$N_i,N_j\lhd G$知
\begin{align*}
gN_j=N_jg,\quad g^{-1}N_i=N_ig^{-1},\ \forall g\in G.
\end{align*}
从而对$\forall n_i\in N_i,n_j\in N_j$,存在$n_{j}^\prime\in N_j,n_{i}^\prime\in N_i$,使得
\begin{align*}
n_in_j=n_{j}^\prime n_i,\quad n_{i}^{-1}n_{j}^{-1}=n_{j}^{-1}n_{i}^\prime,
\end{align*}
\begin{align*}
n_in_jn_{i}^{-1}n_{j}^{-1}=n_{j}^\prime n_in_{i}^{-1}n_{j}^{-1}=n_{j}^\prime n_{j}^{-1}\in N_j,
\end{align*}
\begin{align*}
n_in_jn_{i}^{-1}n_{j}^{-1}=n_in_jn_{j}^{-1}n_{i}^\prime=n_in_{i}^\prime\in N_i.
\end{align*}
故$n_in_jn_{i}^{-1}n_{j}^{-1}\in N_i\cap N_j=\{1\}$.因此
\begin{align*}
n_in_jn_{i}^{-1}n_{j}^{-1}=1\Longleftrightarrow n_in_j=n_jn_i.
\end{align*}
进而
\begin{align*}
n_j\in C_G(N_i),\,\,\forall i,j\in \{1,2,\cdots,k\}.
\end{align*}
再由$n_j$的任意性知
\begin{align*}
N_j\subseteq C_G(N_i),\,\,\forall i,j\in \{1,2,\cdots,k\}.
\end{align*}

{\color{blue}证法二:}对$\forall i,j\in \{1,2,\cdots,k\}$,由\rreflem{lemma:换位子群的基本性质}{lemma:换位子群的基本性质-3}知$[N_i,N_j]\subseteq N_i\cap N_j=\{1\}$,故$[N_i,N_j]=\{1\}$.因此对$\forall n_i\in N_i,n_j\in N_j$,有
\begin{align*}
