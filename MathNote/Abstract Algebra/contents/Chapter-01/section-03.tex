...
\begin{definition}
群 \( G \) 的非空子集 \( H \) 若对 \( G \) 的运算也构成一个群, 则称为 \( G \) 的\textbf{子群},记作$H<G$或$H\leqslant G$.
\end{definition}
\begin{remark}
...
\item 设$A,B$是群$G$的有限子群,则$|A|\cdot |B| = |AB|\cdot |A\cap B|$.
\end{enumerate}
\end{corollary}
\begin{proof}
\begin{enumerate}[(1)]
...
另外,考虑映射
\begin{align*}
\varphi:&K\cap g^{-1}Hg\longrightarrow H\cap gKg^{-1},\\
&x\longmapsto gxg^{-1},\,\forall x\in K\cap g^{-1}Hg.
\end{align*}
设$x\in K\cap g^{-1}Hg$,则存在$k\in K,h\in H$,使得$x=g^{-1}hg=k$.从而
\begin{align*}
\varphi(x)=gxg^{-1}=h=gkg^{-1}\in H\cap gKg^{-1},
\end{align*}
即$\varphi(K\cap g^{-1}Hg)\subseteq H\cap gKg^{-1}$.又显然当$x_1,x_2\in K\cap g^{-1}Hg$且$x_1=x_2$时,有$\varphi(x_1)=\varphi(x_2)$.故$\varphi$是良定义的映射.
设$h_1=gk_1g^{-1}\in H\cap gKg^{-1}$,则令$x_1=g^{-1}h_1g=k_1\in K\cap g^{-1}Hg$,则$\varphi(x_1)=h_1=gk_1g^{-1}$.故$\varphi$是满射.
若$\varphi(x_1)=\varphi(x_2)$,则$gx_1g^{-1}=gx_2g^{-1}$,从而$x_1=x_2$.故$\varphi$是单射.
因此$\varphi$是$K\cap g^{-1}Hg$到$H\cap gKg^{-1}$的双射,故
\begin{align*}
