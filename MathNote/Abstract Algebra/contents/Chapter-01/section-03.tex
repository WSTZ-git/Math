...
\begin{theorem}[Lagrange定理]\label{theorem:Lagrange定理--抽象代数}
...
\end{align*}
也构成一个群, 称为 \( G \) 对 \( H \) 的\textbf{商群}.商群 \( G/H \) 的幺元为 \( 1 \cdot H = H \).为方便计, 常将商群 \( G/H \) 中元素记为 \( \overline{g} = gH \).有时也将商群$G/H$记作$\frac{G}{H}$.
\end{theorem}
\begin{proof}
...
\end{corollary}
\begin{proof}
这是\hyperref[theorem:Lagrange定理--抽象代数]{Lagrange定理}的直接推论. 
...
\item\label{proposition:正规子群的基本性质-4} 若$H$是群$G$的子群,且$N\lhd G,H$,则$H/N$是$G/N$的子群.

\item\label{proposition:正规子群的基本性质-5} 若$G$是一个群,则$G$的任意正规子群的交$\bigcap_{H\lhd G}{H}$也是$G$的子群.

\item\label{proposition:正规子群的基本性质-3} 设$G$是一个群,且$N_1$, $N_2$, $\cdots$, $N_k\lhd G$,则
\begin{align*}
N_1 N_2\cdots N_k\lhd G.
\end{align*}
\end{enumerate}
\end{proposition}
\begin{proof}
...
\item 显然$H/N,G/N$都是商群,且$H/N$中元素也都在$G/N$中,故$H/N$是$G/N$的子群.
