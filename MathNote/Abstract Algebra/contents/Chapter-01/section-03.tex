...
\]
则$A_{\sigma}=\pm A$.若 \( A_\sigma = A \), 则称 \( \sigma \) 为\textbf{偶置换}, 并记 \( \text{sgn}\sigma = 1 \); 若 \( A_\sigma = -A \), 则称 \( \sigma \) 为\textbf{奇置换}, 并记 \( \text{sgn}\sigma = -1 \), \( \text{sgn}\sigma \) 称为 \( \sigma \) 的\textbf{符号}. 故有$\text{sgn}$是$S_n$的自同态且
\[
...
设$H=\{h_1,h_2,\cdots\}$,则
\begin{align*}
aH=\{ah_1,ah_2,\cdots\},\quad Ha=\{h_1a,h_2a,\cdots\}.
\end{align*}
...
进而也有$aH\cap bH=\varnothing\iff aH\ne bH.$
\end{corollary}
\begin{proof}
...
\begin{enumerate}[(1)]
\item 若 \( G \) 为有限群, \( H \lhd  G \), 商群 \( G/H \) 的阶 \( [G/H : H] = [G : H] = \frac{[G:1]}{[H:1]} \).

\item 若 \( G \) 为无限群, \( H \lhd  G \), 商群 \( G/H \) 的阶 \( [G/H : H] = [G : H] \).
\end{enumerate}
\end{corollary}
\begin{proof}
...
\begin{enumerate}[(1)]
\item Abel 群 \( G \) 的任一子群 \( H \) 都是正规子群, 商群 \( G/H \) 也是 Abel 群.
...
如果关系 \( \sim \) 是幺半群 (或半群)$G$中的同余关系, 那么商集合$G/\sim$对导出的运算(见\refthe{theorem:同余关系诱导商集中的乘法-定理1.1.3})也是幺半群 (或半群), 称之为\textbf{商幺半群}(或\textbf{商半群}). 
