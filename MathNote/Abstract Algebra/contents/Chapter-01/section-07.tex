...
\begin{enumerate}[(1)]
\item $H + K$为$R$中包含$K$的子环，$K$是$H+K$的理想,并且
\begin{align*}
H+K=\pi^{-1}(\pi(H)).
\end{align*}

\item $H \cap K$为$H$的理想且$H\cap K=\ker \pi|_{H}.$
...
\begin{enumerate}[(1)]
\item\label{theorem:模同态的基本定理-1} $\ker f = N$是$M$的子模. 若$\pi$是$M$到$M/N$上的自然模同态，则有$M/N$到$M'$的模同构$\overline{f}$，使得
\begin{align}
...
\end{corollary}
\begin{proof}
事实上，只要在\hyperref[theorem:模同态的基本定理]{模同态的基本定理\ref{theorem:模同态的基本定理-3}}中取$M' = M/N$，$f$为$M$到$M' = M/N$的自然同态，再由\rrefpro{proposition:自然模同态的基本性质}{proposition:自然模同态的基本性质-1}知$f(M_1)=M_1/N$,即得本推论.
...
\begin{proof}
设$\pi$为模$M$到商模$M/N$的自然模同态，由于$N$为商群$M/N$中的加法幺元,即商模$M/N$中的零元,于是有$\pi(H + N) =\pi(H)+N= \pi(H)$，因而由\hyperref[theorem:模同态的基本定理-1]{模同态的基本定理\ref{theorem:模同态的基本定理-1}}知
\begin{align*}
