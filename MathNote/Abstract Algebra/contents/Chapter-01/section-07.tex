...
\item 设$N$是$R$模$M$的子模,$\pi$是$M$到商模$M/N$的自然同态,则有$\ker\pi = N$.
\end{enumerate}
\end{example}
...
\item 设$\pi$为$G$到商群$G/\ker f$上的自然同态，则有$G/\ker f$到$H$上的群同构映射$\overline{f}$，使得
\begin{align}
f = \overline{f} \cdot \pi, \label{eq:1.gdgp9343f43dc3237.123r3te4g45y4h5h56}
\end{align}
进而
\begin{align*}
G\backslash \mathrm{ker}\,f\cong H = f(G).
\end{align*}
...
\item 根据$f$是$G$到$H$上的m满映射知$H=f(G)$.并且由\refthe{theorem:抽象代数--定理1.1.2}知$f$在$G$中诱导一个等价关系
\begin{align*}
R: xRy, \quad x, y \in G,
\end{align*}
当且仅当$f(x) = f(y)$，即
...
\end{align*}
又由\refthe{theorem:抽象代数--定理1.1.2}知有$G/\ker f$到$H$的一一对应$\overline{f}$，使得$\overline{f} \cdot \pi = f$，又$\forall x, y \in G$有
\begin{align*}
\overline{f}(\pi(x)\pi(y)) = \overline{f}(\pi(xy)) = f(xy) = f(x)f(y) = \overline{f}(\pi(x)) \cdot \overline{f}(\pi(y)).
\end{align*}
由此知$\overline{f}$是$G/\ker f$到$H$上的群同构.
\end{enumerate}
\end{proof}
...
\item 若$G_1 \lhd G$，$G_1 \supseteq K$，则$$f(G_1)\lhd H.$$
若$H_1 \lhd H$,则$$f^{-1}(H_1)\lhd G.$$

\item 若$G_1 \lhd G$，$G_1 \supseteq K$，则
\begin{align}
G/G_1 \cong H/f(G_1).\label{eq:1.gdgp9343f43dc3237.2}
\end{align}
\end{enumerate}
\end{theorem}
\begin{proof}
...
\item 设$G_1 \in \Sigma$且$G_1 \lhd G$. 由结论(2)的证明知$f(G_1) \lhd H$. 令$\pi'$是$H$到商群$H/f(G_1)$的自然同态，由此可知有$G$到$H/f(G_1)$上的同态映射$\pi' \cdot f$，注意到$H/f(G_1)$的幺元为$f(G_1)$，则知
\begin{align*}
...
\begin{enumerate}[(1)]
\item $\pi$是$\Sigma \to \Gamma$的一一对应；

\item 若$H \lhd G$，$H \supseteq N$，则$$
\pi(H)\lhd G/N.
$$
若$H' \lhd G/N$,则
$$
...
\end{corollary}
\begin{proof}
事实上，由于自然同态必是满同态,故只要在\refthe{theorem:抽象代数-定理1.7.2}中将$H$换成$G/N$，$f$换成$\pi$,即得本推论。对于(3),由\rrefpro{proposition:群的自然映射的基本性质}{proposition:群的自然映射的基本性质-1}知$\pi (H)=H/N$,故我们有如下交换图.
\begin{figure}[H]
\centering
\begin{tikzcd}
G && {G/N} \\
...
\begin{enumerate}[(1)]
\item $HN$是$G$中包含$N$的子群且
\begin{align}
N\lhd HN = \pi^{-1}(\pi(H)). \label{eq:::fi0ejw89f38j843g2fw2f4}
\end{align}
\item $H \cap N \lhd H$且$H \cap N = \ker(\pi|_H)$，$\pi|_H$表示$\pi$在$H$上的限制；
\item
\begin{align}
HN/N \cong H/H \cap N .\label{eq:::fi0ejw89f38j843g2fw2f5}
\end{align}
\end{enumerate}
\end{theorem}
\begin{proof}
...
\item 由(1)的结论及自然同态是满同态知$\pi(HN) = \pi(\pi^{-1}(\pi(H)))=\pi(H)$.又注意到$\ker(\pi|_{HN}) = HN \cap N = N$，故由\hyperref[theorem:群的同态基本定理]{群的同态基本定理}知$HN/N \cong \pi(H)$. 另有$\pi(H) = \pi|_H(H)$，$\ker(\pi|_H) = N \cap H$，故由\hyperref[theorem:群的同态基本定理]{群的同态基本定理}知$\pi(H) \cong H/H \cap N$，即式\eqref{eq:::fi0ejw89f38j843g2fw2f5}成立.
\end{enumerate}
...
\begin{enumerate}[(1)]
\item $\ker f$是$R$的理想；
\item 设$\pi$是$R$到商环$R/\ker f$上的自然同态，则有$R/\ker f$到$R'$上的环同构映射$\overline{f}$，使得
\begin{align}
...
\begin{proof}
\begin{enumerate}[(1)]
\item 设$x,y \in \ker f$，则有$f(x - y) = 0$，故$x - y \in \ker f$.又显然有$\ker f$对乘法满足结合律且加法与乘法间满足左右分配律,因此$\ker f$是$R$的子环.又设$a \in R$，则$f(ax) = f(a)f(x) = 0$，$f(xa) = f(x)f(a) = 0$，即$ax, xa \in \ker f$，故$\ker f$为$R$的理想.

\item $f$为环同态，故也是加法群$R$到加法群$R'$上的同态，$\pi$也是加法群$R$到商群$R/\ker f$上的自然同态，于是由\hyperref[theorem:群的同态基本定理]{群的同态基本定理}知有加法群$R/\ker f$到加法群$R'$上的同构$\overline{f}$，使$f = \overline{f} \cdot \pi$.
...
\end{align*}
因而$\overline{f}$也是环$R/\ker f$到环$R'$上的环同构.
\end{enumerate}
\end{proof}
...
\begin{enumerate}[(1)]
\item $f$是$\Sigma\to \Gamma$的一一对应；

\item 若$H$为$R$的理想且$H\supseteq K$,则$f(H)$为$R'$的理想;

若$H'$为$R'$的理想,则$f^{-1}(H')$为$R$的理想;

\item 若$I$是$R$的理想且$I \supseteq K$，则
\begin{align}
...
\end{corollary}
\begin{proof}
事实上，只要在\refthe{theorem:抽象代数--定理1.7.5}中取$R' = R/A$，$f$为$R$到$R/A$的自然同态，并且由\rrefthe{proposition:环的自然同态的基本性质}{proposition:环的自然同态的基本性质-1}知$\pi(B)=B/A$,因此即得本推论.

\end{proof}

\begin{theorem}
设$H$为环$R$的子环，$K$为$R$的理想，则有$H + K$为$R$的子环，$H \cap K$为$H$的理想且
\begin{align}
(H + K)/K \cong H/(H \cap K). \label{eq:1ger4wg34wgg54.7.g890u34349n8v4b9}
\end{align}
\end{theorem}
\begin{proof}
设$\pi$是环$R$到商环$R/K$上的自然同态. 因$H$是$R$的子环，故$\pi(H)$是$R/K$的子环. 与\refthe{theorem:抽象代数--定理1.7.3}一样有$H + K=\pi^{-1}(\pi(H))$且$K$是$H+K$的理想. 又根据\refthe{theorem:抽象代数--定理1.7.3}知$H + K$是$R$的子环且$\pi(H) \cong (H + K)/K$，又$\pi|_H$是$H$到$\pi(H)$上的同态且$\ker(\pi|_H) = H \cap K$，因而$\pi(H) \cong H/(H \cap K)$，即得式\eqref{eq:1ger4wg34wgg54.7.g890u34349n8v4b9}.
\end{proof}

\begin{theorem}[模同态的基本定理]\label{theorem:模同态的基本定理}
设$M, M'$都是幺环$R$上的模，$f$是$M$到$M'$上的模同态，$M$中包含$N$的子模集合为$\Sigma$,$M'$中子模集合为$\Gamma$,则有下面结论：
\begin{enumerate}[(1)]
\item $\ker f = N$是$M$的子模. 若$\pi$是$M$到$M/N$上的自然模同态，则有$M/N$到$M'$的模同构$\overline{f}$，使得
\begin{align}
\overline{f} \cdot \pi = f \label{eq::::--928fh3489h4h3t4g1.7.10}
\end{align}
\item $f$是$\Sigma\to \Gamma$的一一对应. 

\item 若$M_1$是$M$的子模且$M_1 \supseteq N$，则
\begin{align}
...
\begin{proof}
\begin{enumerate}[(1)]
\item 对$\forall x,y\in \ker f,$由$f$是摸同态知$f(x-y)=f(x)-f(y)=0$.从而$x-y\in \ker f$,于是$\ker f = N$是加法群$M$的子群，设$a \in R$，$x \in N$，则$f(ax) = af(x) = 0$，因而$ax \in N$，故$N$是$M$的子模. 由\hyperref[theorem:群的同态基本定理]{群的同态基本定理}知有加法群$M/N$到加法群$M'$上的同构$\overline{f}$，使$\overline{f} \cdot \pi = f$. 现只需证$\overline{f}$是模同构. 又设$a \in R$，$x \in M$，于是有
\begin{align*}
\overline{f}(a\pi(x)) = \overline{f}(\pi(ax)) = f(ax) = af(x) = a\overline{f}(\pi(x)),
\end{align*}
即$\overline{f}$为模同构. 故定理结论(1)成立.
...
对$\forall H\in \Sigma,H'\in \Gamma$,注意到$H$也是$R$中包含$K$的加法子群,$H'$也是$R'$的加法子群,由\rrefthe{theorem:抽象代数-定理1.7.2}{theorem:抽象代数-定理1.7.2-1}知$f^{-1}f(H)=H,ff^{-1}(H')=H'.$由$H$的任意性知$f^{-1}f=\mathrm{id}_{\Sigma},ff^{-1}=\mathrm{id}_{\Gamma}.$故$f$是$\Sigma\to \Gamma$的一一对应,$f^{-1}$是其逆映射,即结论(2)成立.
...
&= f^{-1}(f(M_1)) =  M_1.
\end{align*}
最后一个等号是因为由结论(2)知$f$是$\Sigma \rightarrow \Gamma$的一一对应.故由结论(1)可知式\eqref{eq::::--928fh3489h4h3t4g1.7.11}成立.
\end{enumerate}
\end{proof}

\begin{corollary}
设$M_1, N$都是$R$模$M$的子模，而且$M_1 \supseteq N$，则$M/M_1$与$(M/N)/(M_1/N)$是模同构.
\end{corollary}
\begin{proof}
事实上，只要在\hyperref[theorem:模同态的基本定理]{模同态的基本定理}中取$M' = M/N$，$f$为$M$到$M' = M/N$的自然同态，即得本推论.
\end{proof}
...
\begin{proof}
设$\pi$为$M$到$M/N$的自然同态，于是有$\pi(H + N) = \pi(H)$，因而
\begin{align*}
H + N/\ker(\pi|_{H + N}) \cong H/\ker(\pi|_H).
\end{align*}
由$\ker(\pi|_{H + N}) = N$，$\ker(\pi|_H) = H \cap N$，即得式\eqref{eq::::--928fh3489h4h3t4g1.7.12}成立.
\end{proof}
