...
\end{lemma}
\begin{proof}
令 \(n \in N\subset G\)，\(m \in M\)，则由$M\lhd G$可知,\(nmn^{-1} \in M\)。因此由\reflemma{lemma:正规子群的等价条件}可知$M\lhd N$.
\end{proof}
...
\end{theorem}
\begin{proof}
首先显然有\(N/M \subset G/M\)。由\reflemma{lemma:正规子群的"传递性"}可知$M\lhd N$.因此 \(N/M\) 是个商群。
