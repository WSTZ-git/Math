...
\end{corollary}
\begin{proof}
由\rrefthe{theorem:抽象代数--定理4.2.2}{theorem:抽象代数--定理4.2.2-1}知$G$在$O_x$上作用可递,故由\refthe{theorem:抽象代数--定理4.2.3}知,$G$在$X$上的作用与$G$在$G/F_x$上的作用等价,即存在$X$到$G/F_x$的双射.因此$|O_x| = [G: F_x]$.再由\hyperref[theorem:Lagrange定理--抽象代数]{Lagrange定理}知
\begin{align*}
...
设$G$是一个群,则有
\begin{enumerate}[(1)]
\item\label{theorem:中心化子的基本性质-1} $C(G)$是$G$的正规子群且$\text{ad}G$与$G/C(G)$同构;

\item\label{theorem:中心化子的基本性质-2} $G$中共轭关系为等价关系,因而$G$的共轭类的集合是$G$的一个分划;

\item\label{theorem:中心化子的基本性质-3} 若$G$是有限群,$g \in G$,则$g$的共轭类$C_g$中所含元素个数$|C_g| = [G: C(g)]$,故是$|G|$的因数;

\item\label{theorem:中心化子的基本性质-4} $h \in C(G)\iff |C_h| = 1 \iff h \in \bigcap\limits_{g \in G} C(g)$.
\end{enumerate}
\end{theorem}
\begin{proof}
\begin{enumerate}[(1)]
...
设$H$是群$G$的子群，则
\begin{enumerate}[(1)]
\item $\forall g \in G, H_1 = gHg^{-1}$也是$G$的子群,称为$H$的\textbf{共轭子群};
