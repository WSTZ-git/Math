...
\begin{proposition}\label{proposition:两个正规子群构成的内直积}
设$G$是一个群,$A\lhd G,B\lhd G$,并且$G=AB,A\cap B=\{1\}$,则$G=A\otimes B.$
\end{proposition}
\begin{proof}
由$B \lhd G$知$G$是$G/B$过$B$的扩张, 扩张核为$B$. 又$G=AB$, $A \cap B=\{1\}$, $A \lhd G$, 故由\refdef{definition:内直积和中心扩张定义}知$G=A \otimes B$.
...
\end{align*}
故$A\lhd G$, 同样$B\lhd G$, 由\refpro{proposition:两个正规子群构成的内直积}知$G=A \otimes B$.
\end{enumerate}
\end{proof}

\begin{definition}
设$N_1$, $N_2$, $\cdots$, $N_k$都是群$G$的正规子群, 并且$G = N_1N_2\cdots N_k$, $G$中任一元素可分解为$N_i$($1\leqslant i\leqslant k$)中元素的积且这种分解是唯一的, 则称$G$是$N_1$, $N_2$, $\cdots$, $N_k$的\textbf{直积}, 记为
\begin{align*}
...
设$A$, $B$是两个群, 则一定存在$B$过$A$的平凡扩张$G=A\times B$($A,B$的外直积), 并且$G$在同构意义下唯一.
\end{theorem}
\begin{proof}
...
$$
都是$G$的正规子群. 又$G = A'B'$, $A'\cap B' = \{(1, 1')\}$, 于是由\refpro{proposition:两个正规子群构成的内直积}知$G = A'\otimes B'$. 
