...
\begin{theorem}\label{theorem:群的外直积的元素的阶}
设$G_1, G_2,\cdots,G_n$是$n$个群，$a_1,a_2,\cdots,a_n$分别是$G_1,G_2,\cdots,G_n$中的有限阶元素，则对于$(a_1,a_2,\cdots,a_n) \in G_1 \times G_2\times \cdots \times G_n$,有
\begin{align*}
\mathrm{ord}(a_1,a_2,\cdots ,a_n)=[\mathrm{ord}\,a_1,\mathrm{ord}\,a_2,\cdots ,\mathrm{ord}a_n].
\end{align*}
\end{theorem}
\begin{proof}
当$n=1$时,结论显然成立.假设结论对$n-1$成立,现在考虑$n$的情况.
...
\begin{theorem}
设$G_1,G_2,\cdots,G_n$分别是$m_1,m_2,\cdots,m_n$阶的循环群，则$G_1 \times G_2\times \cdots\times G_n$是循环群的充要条件是$(m_1,m_2,\cdots,m_n) = 1$.
\end{theorem}
\begin{proof}
设$G_1 = \langle a_1 \rangle,G_2 = \langle a_2 \rangle,\cdots ,G_n=\langle a_n \rangle$,$e_1,e_2,\cdots,e_n$分别为$G_1,G_2,\cdots,G_n$的幺元.
...
因此由\refcor{corollary:抽象代数-推论 1.3.4}知
\begin{align*}
\langle (e_1,\cdots ,\underset{\text{第}i\text{个位置}}{\underbrace{a_{i}^{\frac{m_i}{t}}}},\cdots ,e_n)\rangle ,\,\,i=1,2,\cdots ,n
\end{align*}
都是循环群$G_1\times G_2\times\cdots\times G_n$中的$n$个不同的$t$阶子群.而这与\refcor{corollary:循环群的所有子群的具体形式}矛盾!故$(m_1,m_2,\cdots,m_n)=1$.

{\heiti 充分性:}设$(m_1,m_2,\cdots,m_n)=1$,则由\refthe{theorem:群的外直积的元素的阶}
...
\item\label{proposition:换位子(群)保持群自同构-3} 若$H\lhd G$, 则$G/H$为Abel群的充要条件是$H\supseteq [G,G]$.
\end{enumerate}
\end{proposition}
\begin{proof}
\begin{enumerate}[(1)]
...
\begin{corollary}\label{corollary:正规子群交为1则乘积可交换}
\begin{enumerate}[(1)]
\item\label{corollary:正规子群交为1则乘积可交换-1} 设$G$是一个群,$N_1,N_2,\cdots N_k\lhd G$,且$N_i\cap N_j=\{1\}(i\neq j)$,则
\begin{align*}
n_in_j=n_jn_i,\,\,\forall i,j\in \{1,2,\cdots,k\}.
\end{align*}
并且
\begin{align*}
N_j\subseteq C(N_i),\,\,\forall i,j\in \{1,2,\cdots,k\}.
\end{align*}

\item\label{corollary:正规子群交为1则乘积可交换-2} 设$H,K$是两个群，$N\lhd H,K$，则
\begin{align*}
...
\begin{proof}
\begin{enumerate}[(1)]
\item 对$\forall i,j\in \{1,2,\cdots,k\}$,由\rreflem{lemma:换位子群的基本性质}{lemma:换位子群的基本性质-3}知$[N_i,N_j]\subseteq N_i\cap N_j=\{1\}$,故$[N_i,N_j]=\{1\}$.因此对$\forall n_i\in N_i,n_j\in N_j$,有
\begin{align*}
1=[n_i,n_j]=n_{i}^{-1}n_{j}^{-1}n_in_j\Longleftrightarrow n_in_j=n_jn_i.
\end{align*}
并且由\rreflem{lemma:换位子群的基本性质}{lemma:换位子群的基本性质-1}知
\begin{align*}
N_j\subseteq C(N_i),\,\,\forall i,j\in \{1,2,\cdots,k\}.
\end{align*}
...
(1)如果存在$G$的子群$H$, 使$H\cap N = \{1\}$, $G = HN$, 那么称此扩张为\textbf{非本质扩张}, 并称$G$是$N$与$H$的\textbf{半直积}, 记为$G = H\ltimes N$.
如果$H$还是$G$的正规子群, 则称此扩张为\textbf{平凡扩张}. $G$是$N$与$H$的\textbf{内直积}, 记为$G = H\otimes N$.
...
\begin{align*}
G = N_1N_2\cdots N_k,\,\,\text{其中}N_i\cap \prod_{j\ne i}{N_j}=\left\{ 1 \right\} ,\,\,i=1,2,\cdots ,k.
\end{align*}
...
\begin{enumerate}[(1)]
\item\label{theorem:群的内直积分解的基本性质-1} 设群$N_1,N_2,\cdots,N_k$的内直积为$G$,即$G = N_1\otimes N_2\otimes\cdots\otimes N_k$,则
\begin{align*}
n_in_j=n_jn_i,\,\,\forall i,j\in \{1,2,\cdots,k\}.
\end{align*}

\item\label{theorem:群的内直积分解的基本性质-2} 
设群$N_1,N_2,\cdots,N_k$的内直积为$G$,即$G = N_1\otimes N_2\otimes\cdots\otimes N_k$,则
\begin{align*}
|G|=|N_1||N_2|\cdots |N_k|.
\end{align*}
...
\begin{proof}
\begin{enumerate}[(1)]
\item 由内直积的定义知当$i\neq j$时,有$N_i\cap N_j\subseteq N_i\cap \prod_{j\ne i}{N_j}=\left\{ 1 \right\}$,故利用\rrefcor{corollary:正规子群交为1则乘积可交换}{corollary:正规子群交为1则乘积可交换-1}得
\begin{align*}
n_in_j=n_jn_i,\,\,\forall i,j\in \{1,2,\cdots,k\}.
\end{align*}
...
由\rrefthe{theorem:群的内直积分解的基本性质}{theorem:群的内直积分解的基本性质-1}知
\begin{align}
n_in_j=n_jn_i,\,\,\forall i,j\in \{1,2,\cdots,k\}.\label{eq:::asfj8wj3fj20321.1}
\end{align}
设$g\in G$且
\begin{align*}
