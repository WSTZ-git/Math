...
\begin{align*}
\theta_i=m_\mathrm{i}\theta _1+\phi_i,\ 0\leqslant\phi_i<\theta_1.
\end{align*}
于是$g_ig_1^{-m_i}\in G$的旋转角为$\theta_i-m_\mathrm{i}\theta _i=\phi_i$.若$\phi_i\neq 0$,则$g_ig_1^{-m_i}$的旋转角与$G$中所有元素都不同,即$g_ig_1^{-m_i}\notin G$矛盾!故$\phi_i=0$,即$g_i=g_1^{m_i}$,从而$G$为循环群且$\theta_1=\frac{2\pi}{n}$.
\end{proof}
...
\begin{enumerate}[(1)]
\item 当$n_1=n_2,\ n=2n_3$时,$G$称为\textbf{二面体群},记为$D_{n_3}$.特别地,当$n_1=n_2=1,n_3=2$时,$G$称为\textbf{正二面体群},记为$D_2$或$D_3$.显然有$D_3\cong S_3$.

设$T$是平面上一个正$n_3$边形,以$T$的中心$O$为原点,通过$O$垂直$T$的平面的直线为$Oz$轴.$r$为绕$Oz$旋转$\frac{2\pi}{n}$角.显然$r$使正$n_3$边形不变,$r^k(k=0,1,\cdots,n_3-1)$也使正$n_3$边形不变.$T$有$n_3$条对称轴,绕对称轴旋转$180^\circ$也保持$T$不变.所有保持$T$不变的空间转动构成的群就是$G$,极点集$P$为这些转动轴与$S_r$的交点,在$G$的作用下分为三个轨道.
...
\begin{example}
求正四面体、正六面体、正八面体、正十二面体和正二十面体的旋转群和对称群各有多少个元？
\end{example}
\begin{remark}
注意到正六面体的旋转群和正八面体的旋转群同构；正六面体的对称群和正八面体的对称群同构。正十二面体的旋转群与正二十面体的旋转群同构；正十二面体的对称群与正二十面体的对称群同构。
\end{remark}
\begin{solution}
设 $\Sigma$ 是正多面体的顶点的集合，则相应于五个正多面体，$\Sigma$ 分别含有 $4$, $8$, $6$, $20$, $12$ 个顶点。（注 设 $v$ 为顶点数，$e$ 为棱数，$f$ 为面数，则这五个正多面体的 $(v,e,f)$ 分别为 $(4,6,4)$, $(8,12,6)$, $(6,12,8)$, $(20,30,12)$, $(12,30,20)$。验证 Euler 公式 $v - e + f = 2$。）

相应的旋转群和对称群在 $\Sigma$ 上有自然的作用，且作用是可迁的。因此由轨道公式知 $|G| = |\Sigma||G_1|$，其中 $G_1$ 是顶点 $1$ 的固定子群。
