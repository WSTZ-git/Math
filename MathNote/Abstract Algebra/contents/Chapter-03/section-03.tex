...
设$F_A$是$A$的迷向子群, 于是由\refcor{corollary:有限群轨道的阶与其商掉迷向子群的阶相同}及\hyperref[theorem:Lagrange定理--抽象代数]{Lagrange定理}可得
\begin{align*}
...
\end{remark}
\begin{proof}
设$|P|=p^l$,则由\refcor{corollary:抽象代数-推论 1.3.4}知$\mathrm{ord}\,a \mid |P|$,故存在$k\in \mathbb{N}$,使得$\mathrm{ord}\,a = p^k.$
...
\end{align*}
由\hyperref[theorem:Lagrange定理--抽象代数]{Lagrange定理}知
\begin{align*}
...
F_P=\{g\in G, |gPg^{-1}=P\}.
\end{align*}
显然, $P\lhd F_P$, 故由\hyperref[theorem:Lagrange定理--抽象代数]{Lagrange定理}知$|P|\mid |F_P|$,即$p^l\mid |F_P|$, 因而存在$t$,使得
\begin{align}\label{eq::90j243gy5yhserfgdjirzz}
|F_P|=p^lt.
\end{align}
于是由\hyperref[theorem:Lagrange定理--抽象代数]{Lagrange定理}知
...
\end{align*}
再利用\hyperref[theorem:Lagrange定理--抽象代数]{Lagrange定理}可得
\begin{align*}
...
因为$p,q$都是素数且$p<q$,所以$|P\cup Q|\leqslant p+q<pq=|G|$,因此$G\neq Q\cup P$. 对$\forall a \in G\setminus (P\cup Q)$,则$\langle a\rangle \neq P,Q$,从而由$P,Q$分别是$G$的唯一$p,q$阶子群知$\langle a\rangle$ 的阶既不是 $p$ 也不是 $q$.又由\hyperref[theorem:Lagrange定理--抽象代数]{Lagrange定理}知$|\langle a\rangle|=\mid pq$,于是$|\langle a\rangle|=pq$, 因此$G=\langle a\rangle$为循环群.
