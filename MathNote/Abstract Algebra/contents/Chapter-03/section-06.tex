...
\begin{align*}
[g_1, g_2] = g_1^{-1}g_2^{-1}g_1g_2\text{或}[g_1,g_2]=g_1g_2g_1^{-1}g_2^{-1}
\end{align*}
...
\begin{proposition}\label{proposition:换位子(群)保持群自同构}
设$H, K$是群$G$的两个子群,则
\begin{enumerate}[(1)]
\item\label{proposition:换位子(群)保持群自同构-1} $\alpha([g_1, g_2]) = [\alpha(g_1), \alpha(g_2)],\quad \forall \alpha \in \mathrm{Aut}G,\ g_1, g_2 \in G.$

\item\label{proposition:换位子(群)保持群自同构-2} $\alpha([H, K]) = [\alpha(H), \alpha(K)],\quad \forall \alpha \in \mathrm{Aut}G.$

\item\label{proposition:换位子(群)保持群自同构-3} 若$H\lhd G$, 则$G/H$为Abel群的充要条件是$[G,G]\subseteq H$.
\end{enumerate}
\end{proposition}
\begin{proof}
\begin{enumerate}[(1)]
...
\item 因为$\forall a\in H,b\in K$,有$\alpha(a)\in \alpha(H),\alpha(b)\in \alpha(K)$.注意到
\begin{align*}
\alpha([a,b])=\alpha(aba^{-1}b^{-1})=\alpha(a)\alpha(b)\alpha(a)^{-1}\alpha(b)^{-1}=[\alpha(a),\alpha(b)],
\end{align*}
故$\alpha([H,K])=[\alpha(H),\alpha(K)]$.

\item $G/H$为Abel群, 当且仅当对$\forall a,b\in G$, $(aH)(bH)=(bH)(aH)$, 即$abH=baH,\forall a,b\in G$, 当且仅当$[a,b]=a^{-1}b^{-1}ab\in H,\forall a,b\in G$,即$H\supseteq [G,G]$.
\end{enumerate}

\end{proof}

\begin{proposition}\label{proposition:SnSn=An}
对$\forall n\in \mathbb{N}$，有$[S_n,S_n] = A_n$.
\end{proposition}
\begin{proof}

先证$[S_n,S_n] \subseteq A_n$.
{\color{blue}证法一:}对$\forall \sigma_1,\sigma_2\in S_n$，则由\refthe{theorem:奇置换与偶置换}和\rrefcor{corollary:奇置换与偶置换分别可表示成奇数和偶数个对换之积}{corollary:奇置换与偶置换分别可表示成奇数和偶数个对换之积-5}知
\begin{align*}
\sgn([\sigma_1,\sigma_2]) = \sgn(\sigma_1^{-1}\sigma_2^{-1}\sigma_1\sigma_2) = \sgn(\sigma_1^{-1})\sgn(\sigma_2^{-1})\sgn(\sigma_1)\sgn(\sigma_2) = [\sgn(\sigma_1)]^2[\sgn(\sigma_2)]^2 = 1.
\end{align*}
从而$[\sigma_1,\sigma_2]$是偶置换，因此$[\sigma_1,\sigma_2]\in A_n$，故$[S_n,S_n] \subseteq A_n$.

{\color{blue}证法二:}由\refpro{proposition:Sn的分解和An是Sn的正规子群}知$A_n\lhd S_n$且$|S_n/A_n|=2$，从而由\hyperref[proposition:素数阶群必为循环群]{素数阶群必为循环群}知$S_n/A_n$是循环群，进而也是Abel群，故由\rrefpro{proposition:换位子(群)保持群自同构}{proposition:换位子(群)保持群自同构-3}知$[S_n,S_n] \subseteq A_n$.

再证$[S_n,S_n] \supseteq A_n$.
当$n=1,2$时，结论显然成立，下设$n\geqslant 3$。由\rrefthe{theorem:抽象代数--定理4.4.2}{theorem:抽象代数--定理4.4.2-1}知$A_n$由所有3-轮换生成。
设$(abc)\in A_n$，$a,b,c\in\{1,2,\cdots,n\}$，令$g=(ab)\in S_n$，$h=(ac)\in S_n$，则
\begin{align*}
(abc) = (ab)(ac)(ab)(ac) = g^{-1}h^{-1}gh = [g,h] \in [S_n,S_n].
\end{align*}
故$[S_n,S_n] \supseteq A_n$，综上可知$[S_n,S_n] = A_n$.

\end{proof}

\begin{lemma}\label{lemma:换位子群的基本性质}
设$H, K$是群$G$的子群，则有
\begin{enumerate}[(1)]
\item\label{lemma:换位子群的基本性质-1} $[H, K] = \{1\}\iff H \subseteq C_G(K)$,即$H$中元都与$K$中元可换;

\item\label{lemma:换位子群的基本性质-2} $[H, K] \subseteq K\iff H \subseteq N_G(K)$,
\\
$[H, K] \subseteq H\iff K \subseteq N_G(H)$;

\item\label{lemma:换位子群的基本性质-3} 若$H \lhd G, K \lhd G$，则$[H, K] \lhd G$且$[H, K] \subseteq H \cap K$.

特别地,由$G\lhd G$知$[G,G]\lhd G$.

当$H\cap K=\{1\}$时,$H \subseteq C_G(K)$,即$H$中元都与$K$中元可换.

\item\label{lemma:换位子群的基本性质-4} 当$H_1, K_1$分别为$H, K$的子群时有$[H_1, K_1] \subseteq [H, K]$.

\item\label{lemma:换位子群的基本性质-5} 设$H,K$是两个群，$N\lhd H,K$，则
\begin{align*}
[H,K] \subseteq N \Longleftrightarrow [H/N,K/N] = N \Longleftrightarrow H/N \subseteq C(K/N).
\end{align*}
\end{enumerate}
\end{lemma}
\begin{proof}
...
\item 设$H \lhd G, K \lhd G$，于是对$\forall \alpha=L_gR_{g^{-1}} \in \mathrm{Int}G$,由\rrefpro{proposition:换位子(群)保持群自同构}{proposition:换位子(群)保持群自同构-2}有
\begin{align*}
...
当$H\cap K\{1\}$时,有$[H,K]=1$,再由(1)可知$H\subseteq C_G(K)$,即$H$与$K$可换.

\item 此结论是显然的.

\item 由\rreflem{lemma:换位子群的基本性质}{lemma:换位子群的基本性质-1}知
\begin{align*}
[H/N,K/N] = N \Longleftrightarrow H/N \subseteq C(K/N).
\end{align*}
...
\end{align*}
进而由\rrefpro{Set Theory-proposition:映射的基本性质}{Set Theory-proposition:映射的基本性质-1}知$a,b\in G^{\left( k-1 \right)}.$从而
\begin{align*}
...
\end{align*}
进而由\rrefpro{Set Theory-proposition:映射的基本性质}{Set Theory-proposition:映射的基本性质-1}知$a\in G,b\in \Gamma _{k-1}\left( G \right)$。从而
\begin{align*}
...
\end{align*}
再利用\hyperref[theorem:抽象代数--Lagrange定理]{Lagrange定理}及\rrefthe{theorem:群的内直积分解的基本性质}{theorem:群的内直积分解的基本性质-2}可得
\begin{gather}
...
\end{gather*}
因而由\rrefcor{corollary:群同态第二定理推论}{corollary:群同态第二定理推论-2}知$\Gamma_i(G)/\Gamma_{i+1}(G) \subseteq G/\Gamma_{i+1}(G)$. 设$\pi$为$G$到$G/\Gamma_{i+1}(G)$的自然同态，由\rrefpro{proposition:导出列的基本性质--抽象代数}{proposition:导出列的基本性质--抽象代数-3}及$[G, \Gamma_i(G)] = \Gamma_{i+1}(G)$知
\begin{align*}
...
(2) $\Rightarrow$ (3). 用反序归纳法证明$G_i \subseteq C_{r - i}(G)$，其中$C_0(G) = \{1\}$. 当$i = r$时，$G_r = \{1\} = C_0(G) = C_{r - r}(G)$. 设$i + 1$时已成立，因而$G_{i+1} \subseteq C_{r - (i+1)}(G)$. 又$G_i/G_{i+1} \subseteq C(G/G_{i+1})$，由\rrefpro{proposition:正规子群的基本性质}{proposition:正规子群的基本性质-7}知
\begin{align*}
...
\end{align*}
用数学归纳法证明$\Gamma_i(G) \subseteq C_{k - i + 1}(G),i=1,2,\cdots$. 当$i = 1$时，显然成立.假设结论对$i$成立,现在考虑$i+1$的情形.由于$C_{k - i + 1}(G)/C_{k - i}(G) = C(G/C_{k - i}(G))$,故由\rrefpro{proposition:正规子群的基本性质}{proposition:正规子群的基本性质-7}有$[G, C_{k - i + 1}(G)] \subseteq C_{k - i}(G)$，于是再结合\rreflem{lemma:换位子群的基本性质}{lemma:换位子群的基本性质-4}得
\begin{align*}
