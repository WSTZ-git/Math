...
\begin{proof}
设$A$是$G$的子群，由数学归纳法可证得$\Gamma_k(A) \subseteq \Gamma_k(G)(\forall k \in \mathbf{N})$. 由此知$G$为幂零群则$A$必为幂零群. 又若$f$是$G$到$G_1$上的同态. 用数学归纳法可证明$\Gamma_k(G_1) = f(\Gamma_k(G))$，于是$G$为幂零群可得$G_1$也是幂零群.
