...
(ab^{-1})(cd^{-1})^{-1} = (ad)(bc)^{-1},
\]
故 \( F_1 \setminus \{0\} \) 对乘法为\( F' \setminus \{0\} \) 的子群，因此 \( F_1 \) 是 \( F' \) 的子域. 由\refthe{theorem:抽象代数--定理2.1.1}知,记$\frac{a}{b}\triangleq \left\{ \left( c,d \right) \in R\times R^*\mid ad=bc \right\}$,则$R$的一个分式域为
\begin{align*}
...
\begin{align*}
\frac{a}{b} = \left\{ (c,d) \in R \times R^* \mid ad = bc \right\}.
\end{align*}
则
\begin{align*}
F = R \times R^*/\sim = \left\{ \frac{a}{b} \mid a \in R, b \in R^* \right\}
