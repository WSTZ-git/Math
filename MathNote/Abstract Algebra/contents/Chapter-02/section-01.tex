...
令 \( F = R \times R^*/\sim \) 为商集合，以 \( \frac{a}{b} \) 表示 \( (a, b) \) 所在等价类。于是由\refthe{theorem:同余关系诱导商集中的乘法-定理1.1.3}，在 \( F \) 中有加法与乘法运算如下：
\begin{align*}
...
(ab^{-1})(cd^{-1})^{-1} = (ad)(bc)^{-1},
\]
故 \( F_1 \setminus \{0\} \) 对乘法为\( F' \setminus \{0\} \) 的子群，因此 \( F_1 \) 是 \( F' \) 的子域. 由\refthe{theorem:抽象代数--定理2.1.1}知
\begin{align*}
\frac{a}{b}\triangleq \left\{ \left( c,d \right) \in R\times R\backslash \left\{ 0 \right\} :ad=bc \right\} ,\,F=\left\{ \frac{a}{b}:a\in R,b\in R\backslash \left\{ 0 \right\} \right\} .
\end{align*}
...
\begin{example}
设 \( \mathbf{P} \) 是任一数域，设 \( \mathbf{P}[x] \) 的分式域为 \( \mathbf{P}(x) \)，则
\[
...
\begin{example}
设 \( m \) 是非零整数，则 \( m\mathbf{Z} \) 的分式域为 \( \mathbf{Q} \).
\end{example}
\begin{proof}
