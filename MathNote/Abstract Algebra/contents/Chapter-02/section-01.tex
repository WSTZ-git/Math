...
\begin{proof}
\begin{enumerate}
\item 由\refproposition{proposition:Z_n是一个Abel群}可知 $(\mathbb{Z}_n,+)$是一个Abel群.又由\refproposition{proposition:Z_n是一个交换幺半群}可知$(\mathbb{Z}_n,\cdot)$是一个交换幺群.因此我们只须证明分配律即可.对$\forall \overline{a},\overline{b},\overline{c}\in \mathbb{Z} _n$,都有
\begin{gather*}
...
\item 利用两次(2)的结论和\refproposition{proposition:群中元素取两次逆元还是其自身}，我们就得到
\begin{align*}
(-a)(-b)=-(a(-b))=-(-(ab)) = ab.
\end{align*} 
\end{enumerate}
...
我们来证明充分性($\Leftarrow$)。假设 $0 = 1$，我们只须证明对所有 $a \in R$，都有 $a = 0$。由\refproposition{proposition:环的加法单位元和乘法逆元的性质}可知
\begin{align*}
a = a1 = a0 = 0
\end{align*}
这就证明了这个命题。 
...
\end{definition}
\begin{remark}
由\reflemma{lemma:幺半群中所有可逆元构成了群}可知, $R$ 中所有乘法可逆元素构成了一个群.故上述$(R^\times, \cdot)$的定义是良定义的.
\end{remark}
...
\end{proposition}
\begin{proof}
因为$R\ne \{0\}$,所以由\refproposition{proposition:零环的充要条件}可知$0\ne 1$.于是对$\forall a\in R$,由\refproposition{proposition:环的加法单位元和乘法逆元的性质}可知$a\cdot 0=0\ne 1.$故0一定没有逆元,即0不是单位.
...
\end{proposition}
\begin{proof}
反证,假设$0=1$,则对$\forall a\in R$,由\refproposition{proposition:环的加法单位元和乘法逆元的性质}可知$a=1\cdot a=0\cdot a=0.$从而$R=\{0\}$.于是$R\setminus \{0\}=\varnothing.$而空集一定不是Abel群,故$R\setminus \{0\}=\varnothing$一定不是Abel群,而由\refproposition{proposition:域的充要条件}可知$R\setminus \{0\}=\varnothing$是Abel群,矛盾!
\end{proof}
...
\end{proposition}
\begin{proof}
由\refproposition{proposition:一族幺半群的直积仍是幺半群}和\refproposition{proposition:一族Abel群的直积仍是Abel群}可知,幺半群和Abel群对直积是保持的，从而我们立刻知道 $\prod_{i \in I} R_i$ 对加法构成Abel群，对乘法构成幺半群。因此只须检验乘法对加法的左右分配律。根据对称性，我们只证明左分配律。
