...
\begin{example}
设$\mathbb{H}$为四元数体(见\refthe{theorem:抽象代数--例题1.4.8}),由\refpro{proposition:四元数体的基本结论----1}知
\begin{align*}
\mathbb{H}=\{a+bi+cj+dk\mid a,b,c,d\in\mathbb{R}\},
\end{align*}
因而$\{a+0\cdot i+0\cdot j+0\cdot k\mid a\in\mathbb{R}\}\cong\mathbb{R}$是$\mathbb{H}$的一个子环。由\refpro{proposition:四元数体的基本结论----1}知$i^=j^2=k^2=-1$,故$i,j,k$都是$\mathbb{R}$上的多项式$x^2+1$的根,从而$bi+cj+dk$都是$x^2+1$的根,因此$x^2+1$在$\mathbb{H}$中有无穷多个根.
\end{example}
...
\end{matrix} \right|,
\end{align*}
为$f(x)$与$g(x)$的\textbf{结式}或$\mathbb{Sylvester}$\textbf{行列式}.显然有
\begin{align*}
\begin{cases}
