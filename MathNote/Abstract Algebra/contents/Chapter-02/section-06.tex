...
\end{definition}
\begin{remark}
由\refproposition{proposition:生成的理想的集合表示}即可得到上述$I = (a) = Ra.$
\end{remark}
...
\end{proposition}
\begin{proof}
利用\reflemma{lemma:整数环的每个加法子群都是nZ}即得结论.
\end{proof}
...
{\heiti 充分性:}假设 $R$ 唯二的理想是零和整个环。令 $a \neq 0$，则$\{0\}$就是$R$的极大理想.于是由\refproposition{proposition:素理想的充要条件}可知,$R/\{0\}$就是一个域.又因为$R/\{0\}=R+0=R$,所以$R$ 是一个域。
\end{proof}
...
\end{proposition}
\begin{proof}
由\reflemma{lemma:域只有两个理想:零和自身}可知$R$只有两个理想$0$和$R$.又由\refproposition{proposition:生成的理想的集合表示}可知
\begin{align*}
...
\end{proposition}
\begin{proof}
$\Leftarrow:$由\refproposition{proposition:在交换环中,每一个极大理想都是素理想}立得.
...
\end{proposition}
\begin{proof}
由\refproposition{proposition:整数环中的素理想}可知$p\mathbb{Z}$就是$\mathbb{Z}$的素理想.再由\refproposition{proposition:主理想整环中的非零素理想与极大理想等价}可知$p\mathbb{Z}$也是$\mathbb{Z}$的极大理想.于是由\refproposition{proposition:极大理想的充要条件}可知$\mathbb{Z}/p\mathbb{Z}=\mathbb{Z}_p$是一个域.
\end{proof}
...
由 \(n\) 是合数可知 \(n\) 不是素数, 从而由\refproposition{proposition:整数环中的素理想}可知 \(n\mathbb{Z}\) 不是素理想.
又因为 \(\mathbb{Z}\) 是主理想整环, 所以由\refproposition{proposition:主理想整环中的非零素理想与极大理想等价}可知 \(n\mathbb{Z}\) 不是极大理想.
故由\refproposition{proposition:极大理想的充要条件}可知 \(\mathbb{Z}_n = \mathbb{Z}/n\mathbb{Z}\) 不是域.
