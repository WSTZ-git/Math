...
\[
R[u] = \{a_0 + a_1 u + \cdots + a_n u^n \mid a_i \in R, n \in \mathbb{N}\cup\{0\}\},
\]
也称\( R[u] \)为\textbf{\( R \)上添加\( u \)生成的子环}。
\end{theorem}
\begin{proof}
记 $S = \{a_0 + a_1 u + \cdots + a_n u^n \mid a_i \in R, \,  n \in \mathbb{N}\cup\{0\}\}$。
首先证明 $S \subseteq R[u]$.
...
\begin{definition}\label{definecolor:抽象代数--代数元的定义}
...
\begin{example}
令$\widetilde{R}=\mathbb{C}$，则$\sqrt{-1}$为$\mathbb{Z}$上的代数元，
\begin{align*}
\mathbb{Z}[\sqrt{-1}]=\{m+n\sqrt{-1}\mid m,n\in\mathbb{Z}\}
\end{align*}
称为$\mathbf{Gauss}$\textbf{的整数环}，$\deg(\sqrt{-1},\mathbb{Z})=2$。
同样$\sqrt{-1}$为$\mathbb{Q}$上的代数元，$\deg(\sqrt{-1},\mathbb{Q})=2$。
\end{example}
\begin{proof}
...
\begin{example}
令$\widetilde{R}=\mathbb{Q}$，则$\frac{1}{2}$是$\mathbb{Z}$上代数元且$\mathbb{Z}\subset\mathbb{Z}\left[\frac{1}{2}\right]\subset\mathbb{Q}$，$\deg\left(\frac{1}{2},\mathbb{Z}\right)=1$。
\end{example}
\begin{proof}
...
\begin{definition}
设$R$是交换幺环$\widetilde{R}$的包含幺元$1$的子环，\( u \in \widetilde{R} \)，$R[u]$为$R$添加$u$生成的$\widetilde{R}$的子环，若当$R$中有限个元素$a_0,a_1,\cdots,a_n$不全为0时，
\begin{align*}
a_0+a_1u+\cdots+a_nu^n\neq 0,
\end{align*}
则称$u$为$R$上的\textbf{超越元}或\textbf{不定元}。$R[u]$中的一个元素$f(u)=a_0+a_1u+\cdots+a_nu^n$称为$u$的（系数在$R$中的）一个\textbf{多项式}。若$a_n\neq 0$，则称$n$为$f(u)$的\textbf{次数}，记为$\deg f(u)$。$R[u]$称为$R$上的一个\textbf{一元多项式环}。
\end{definition}

\begin{example}
设$\mathbb{P}$是一个数域，$x$是一个文字，则$\mathbb{P}[x]$是$\mathbb{P}$上的一个一元多项式环，$x$是$\mathbb{P}$上的超越元。
\end{example}
\begin{proof}
...
\end{align}
由于$(a_0,a_1,\cdots),(b_0,b_1,\cdots)\in\widetilde{R}$，故$\exists m\in\mathbb{N}$，使$n>m$时，$a_n=b_n=0$. 于是$a_n+b_n=0$，故$(a_0+b_0,a_1+b_1,\cdots)\in \widetilde{R}$.而当$n>2m$时，$c_n=\sum\limits_{i+j=n}a_ib_j=0$,故$(c_0,c_1,\cdots)\in \widetilde{R}$.由此知上面定义的加法与乘法是良定义的.
...
\begin{theorem}[多项式的通用性质(代入原理)]\label{theorem:抽象代数--定理2.2.2}
设$R,S$都是交换幺环，它们的幺元分别是$1,1'$. 又若$\eta$是$R$到$S$的同态且$\eta(1)=1'$，则对$\forall u\in S$,存在唯一的在$R$上的开拓$\eta_u:R[x]\to S$满足
...
\end{theorem}
\begin{remark}
这个定理告诉我们:“代入”就是一个从多项式环出发的环同态.

设$R$是交换幺环,$R[x]$是一元多项式环,$f(x) =a_0+a_1x+\cdots +a_nx^n\in R[x]$,若对某个元素$u$,存在交换幺环$S$,使得$u\in S$,且存在$R$到$S$的环同态$\eta$,则$u$可以代入$f(x)$得到$f(u)$,即$f(u)$是良定义的.并且存在环同态$\eta_u:R[x] \rightarrow S$满足
\begin{align*}
\eta_u(f(x)) =\eta(a_0) +\eta(a_1)u+\cdots +\eta(a_n)u^n\in S.
\end{align*}
这也说明,如果某个元素$u$可以代入多项式$f(x) \in R[x]$,则$u$所在的满足条件(存在$R$到$S$的环同态)的交换幺环$S$中的所有元素都可以代入$f(x)$.
\end{remark}
\begin{proof}
...
\begin{align*}
\eta' \left( \sum_{i=0}^n{a_ix^i} \right) =\sum_{i=0}^n{\eta' (a_i)u^i}=\sum_{i=0}^n{\eta (a_i)u^i}=\eta _u\left( \sum_{i=0}^n{a_ix^i} \right) ,
\end{align*}
故$\eta'=\eta_u$，即$\eta_u$是满足条件的唯一扩充.

\end{proof}

\begin{corollary}\label{corollary:同一个环生成的一元多项式环必同构}
\begin{enumerate}[(1)]
\item\label{corollary:同一个环生成的一元多项式环必同构-1} 设$R$是交换幺环，$R[x]$与$R[y]$都是$R$上的一元多项式环，则$R[x]$与$R[y]$是同构的.

\item\label{corollary:同一个环生成的一元多项式环必同构-2} 设$R$是交换幺环$\widetilde{R}$的包含幺元$1$的子环，$R[x]$为$R$上的一元多项式环,则对$\forall u\in \widetilde{R}$,存在$R[x]\to R[u]$的满同态$\eta_u$,且满足
\begin{align*}
\eta_u((f(x)))=f(u),\,\,\forall f(x)\in R[x].
\end{align*}
\end{enumerate}
\end{corollary}
\begin{note}
这个推论(1)说明:任何交换幺环上的一元多项式环在同构意义下唯一.
\end{note}
\begin{proof}
\begin{enumerate}[(1)]
\item 事实上，容易验证$R$到$R[y]$的嵌入映射$i(a)=a(\forall a\in R)$是$R$到$R[y]$的环同态，于是由\refthe{theorem:抽象代数--定理2.2.2}知有$R[x]$到$R[y]$的同态$i_y$满足
\begin{align*}
...
\end{align*}
故$i_y$是满同态. 由$y$是$R$上超越元知$\ker\,i_y=\{0\}$,因此由\rrrefpro{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数}{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数-2}{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数-2-2}知$i_y$是单同态. 故$i_y$是同构映射.

\item 考虑$R$到$R[u]$的嵌入映射$\eta$,则不难验证$\eta$是$R$到$R[u]$上的同态. 于是由\refthe{theorem:抽象代数--定理2.2.2}知可将$\eta$扩充为环同态$\eta_u:R[x]\to R[u]$满足
\begin{align}\label{eq:::---28ruj8ejfijiof90jw}
\eta_u|_R=\eta,\quad \eta_u(x)=u.
\end{align}
注意到$\eta_u(R[x])=R[u]$,故$\eta_u$是满同态.再利用\eqref{eq:::---28ruj8ejfijiof90jw}式可得
\begin{align*}
\eta((f(x)))=f(u),\,\,\forall f(x)\in R[x].
\end{align*}
\end{enumerate}
...
\end{corollary}
\begin{proof}
由\rrefcor{corollary:同一个环生成的一元多项式环必同构}{corollary:同一个环生成的一元多项式环必同构-2}知存在$R[x]\to R[u]$的满同态$i_u$,且满足
\begin{align*}
i_u(f(x)) = f(u),\,\,\forall f(x)\in R[x].
\end{align*}
取$I=\ker i_u$,则由\rrefpro{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数}{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数-2}知$I=\ker i_u$为$R[x]$中理想，$R[u]\cong R[x]/I$. 又若$a\in R\cap I$，则$0=i_u(a)=i(a)=a$，故$R\cap I=\{0\}$. 由于$u$为$R$上代数元当且仅当存在$a_n\ne 0$,使得$\sum\limits_{i=0}^n a_iu^i=0$.这也当且仅当
\begin{align*}
...
\begin{theorem}\label{theorem:任意n元多项式环都可由n-1元的生成}
...
\end{align*}
故 \( i_n \) 是满同态. 由 \( y_1, y_2, \cdots, y_n \) 是 \( R \) 上代数无关的知$\ker i_n=\{0\}$,故由\rrrefpro{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数}{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数-2}{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数-2-2}知\( i_n \) 也是单同态, 即 \( i_n \) 为同构映射.
...
\end{align*}
注意到$i_u(R[x_1, x_2, \cdots, x_n])=R[u_1,u_2,\cdots ,u_n]$,故$i_u$是满同态.于是由\rrefpro{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数}{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数-2}知$I=\ker i_u$为$R$[$x_1$, $x_2, \cdots, x_n]$的理想，$R[u_1,u_2,\cdots ,u_n]\cong R[x_1, x_2, \cdots, x_n]/I$. 
