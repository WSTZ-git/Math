...
\end{proposition}
\begin{proof}
根据环同态的定义可直接得到,$f$是环同态等价于$f$既是加法的群同态，又是乘法的幺半群同态..再由\refproposition{proposition:群同态保持逆元和单位元}可知,$f$对加法保持逆元和单位元.
\end{proof}
...
\end{proposition}
\begin{proof}
证明与\refproposition{proposition:一个环同态是单的当且仅当核是平凡的}类似.
\end{proof}
...
\begin{proof}
\begin{enumerate}[(1)]
\item 一方面，根据$H< R$可知, $H$是$R$的一个乘法子幺半群.于是由\reflemma{lemma:子群或子幺半群与自身的乘积还等于其本身}可知,对 $\forall h_1,h_2\in H$，都有$h_1h_2\in H$。故 $HH\subset H$。
...
\begin{proof}
一方面,
由条件可知,$((N_i)_{i\in I},+)$ 是一族 $(R,+)$ 的子群.从而由\refproposition{proposition:子群的任意交仍是子群}可知$(\bigcap_{i\in I}N_i,+)$仍是$(R,+)$的子群.
...
\end{proposition}
\begin{proof}
首先，由\refproposition{proposition:nZ是Z的正规子群}我们知道 $(n\mathbb{Z}, +)$ 是 $(\mathbb{Z}, +)$ 的（加法）正规子群。

其次，注意到 $\mathbb{Z}$ 是一个交换环，故根据\refproposition{proposition:交换环的左理想、右理想、理想等价}可知,我们只须证明 $n\mathbb{Z}$是$\mathbb{Z}$的左理想,也即 $\mathbb{Z} \cdot n\mathbb{Z} \subset n\mathbb{Z}$。
...
设 $a, b \in \mathbb{Z}$，由\refproposition{proposition:nZ是Z的正规子群}可知,$(n\mathbb{Z},+)\lhd(\mathbb{Z},+)$.从而
\begin{align*}
...
\end{align*}
因此由\refproposition{proposition:nZ是Z的理想}可知$\ker(f) = n\mathbb{Z} \lhd R$。
\end{proof}
...
注意到$(R,+)$是Abel群，又因为$I$是$R$的理想,所以$(I,+)$是$(R,+)$的子群.从而由\refproposition{proposition:阿贝尔群的子群与正规子群等价}可知$(I,+)$是$(R,+)$的正规子群.根据\refproposition{proposition:陪集乘法的良定义性}可知,正规子群$I$的陪集的加法是良定义的,即上述加法是良定义的.
...
\end{align*}
其中倒数第二个包含关系是根据理想对乘法的 “吸引” 性质，而最后一个等号是根据\reflemma{lemma:子群或子幺半群与自身的乘积还等于其本身}及$(I,+)<(R,+)$。这样，我们就证明了商环对乘法是良定义的。

接下来，要证明商环是个环，其实只要将 $R$ 上环的结构（利用良定义性）照搬过来即可。

利用 $I$ 对加法构成正规子群，因此利用\refproposition{proposition:商群}可知，$R/I$ 对加法构成群。我们只须证明 $R/I$ 对乘法构成幺半群，且乘法对加法有左右分配律。
...
由于$f$ 对加法是群同构，而对乘法是幺半群同构,因此$f$是双射,且$f$既对加法是群同态,又对乘法是幺半群同态.于是由\refproposition{proposition:环同态的等价条件}可知$f$是环同态.又因为$f$是双射,所以$f$是环同构.
\end{proof}
...
\end{lemma}
\begin{proof}
由$f$是个环同构可知,$f$既对加法是群同态,又对乘法是幺半群同态..从而由\refproposition{proposition:幺半群同构的逆是幺半群同态}和\refproposition{proposition:群同构的逆也是群同构}可知, $f^{-1}$ 既对加法是群同态,又对乘法是幺半群同态.于是由\refproposition{proposition:环同态的等价条件}可知$f$是环同态.又因为$f^{-1}$是双射,所以$f^{-1}$是环同构.
\end{proof}
...
单位元:由\hyperref[definition:商环]{商环的定义}及\refproposition{proposition:环同态的核是理想并且像是子环}可知,$R / \ker(f)$的乘法单位元是$1+\ker f$且$\mathrm{im}(f)<R'$,从而$\mathrm{im}(f)$的乘法单位元就是$R'$的乘法单位元.由$\tilde{f}$的定义及$f$是环同态可得$\tilde{f}(1+\ker f)=f(1)=1'.$
...
从而由\reflemma{lemma:HN<G的条件}可知$(S+I,+)<(R,+)$.因此我们只须证明 $S + I$ 对乘法构成幺半群，即对乘法是封闭的，且包含单位元。第一，$1 = 1 + 0 \in S + I$。第二，只须证明 $(S + I)(S + I) \subset (S + I)$.由\reflemma{lemma:子环和理想与自身的乘积等于其自身}可知$II=I$,由\reflemma{lemma:子群或子幺半群与自身的乘积还等于其本身}可知$SS=S,I+I=I$.根据$I\lhd R$可知$IS,SI= I.$于是再利用$R$的乘法对加法满足左右分配律可得
\begin{align*}
(S + I)(S + I) = SS + SI + IS + II = S + I + I + I = S + I .
\end{align*}

我们再证明 $S \cap I \lhd S$。由\refproposition{proposition:子群的任意交仍是子群}可知，$S \cap I$ 对加法构成子群。我们只须证明 $S \cap I$ 对乘法的 “吸收” 性，即 $(S \cap I)S \subset S \cap I$，及 $S(S \cap I) \subset S \cap I$。根据对称性，我们只证明前面这个包含关系。由$SS=S,IS=I$可得
\begin{align*}
...
\end{theorem}
\begin{proof}
首先,由\reflemma{lemma:理想的"传递性"}可知$I\lhd J$.故$J / I$是一个商环.由 $I, J \lhd R$可知$R/I,R/J$也是商环.
