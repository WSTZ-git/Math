...
\begin{definition}
设 \( R \) 是一个交换幺环，\( R[x_1, x_2, \cdots, x_n] \) 是 \( R \) 上的 \( n \) 元多项式环. 如果\( R[x_1, x_2, \cdots, x_n] \)中的$n$元多项式\( f(x_1, x_2, \cdots, x_n) \) 中非零单项式都是 \( k \) 次的，那么称 \( f(x_1, x_2, \cdots, x_n) \) 为一个\textbf{\( k \) 次齐次多项式}.
\end{definition}
...
\begin{align*}
f(x_1, x_2, \cdots, x_n) &= \sum_{k_1k_2\cdots k_n} a_{k_1k_2\cdots k_n} x_1^{k_1} x_2^{k_2} \cdots x_n^{k_n} \\
&= \sum_{k=0}^m \left( \sum_{\substack{k_1 + k_2 + \cdots + k_n = k}} a_{k_1k_2\cdots k_n} x_1^{k_1} x_2^{k_2} \cdots x_n^{k_n} \right),
\end{align*}
...
\end{align*}
都是对称多项式，称为$\mathbf{Newton}$\textbf{对称幂和}或\textbf{等幂和}.
\end{lemma}
\begin{proof}
...
再取
\[
\left\{ x_1^{l_1} x_2^{l_2} \cdots x_n^{l_n} \bigg| l_i = \sum_{j=i}^n d_j, \sum_{i=1}^n l_i =\sum_{j=1}^n{jd_j}= m \right\}
\]
中的最高项是\( x_1^{k_1} x_2^{k_2} \cdots x_n^{k_n} \),其中\( k_i = \sum_{j=i}^n c_j \).再结合$m$的取法可知\( (c_1, c_2, \cdots, c_n)\in \{(d_1, d_2, \cdots, d_n) \mid a_{d_1 d_2 \cdots d_n} \neq 0\} \). 
