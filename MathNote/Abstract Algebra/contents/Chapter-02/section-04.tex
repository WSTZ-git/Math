...
因本节讨论并未用到 \( R \) 中的加法，因而可以认为 \( R^* \) 是满足消去律的幺半群. 因此，可定义\textbf{唯一析因幺半群} (或 $\mathbb{Gauss}$\textbf{幺半群}). \reflem{lemma:最大公因子条件的基本结论}, \reflem{lemma:抽象代数--引理2.2.3}与\refthe{theorem:UFD的充要条件}对 Gauss 幺半群也成立.
...
令 \( R = \mathbb{Z}[\sqrt{-5}] = \{a + b\sqrt{-5} \mid a, b \in \mathbb{Z}\} \)。设 \( \alpha = a + b\sqrt{-5} \)，称 \( \overline{\alpha} = a - b\sqrt{-5} \) 为 \( \alpha \) 的共轭，称 \( N(\alpha) = \alpha\overline{\alpha} = a^2 + 5b^2 \) 为 \( \alpha \) 的范数，显然 \( N(\alpha) \in \mathbb{Z} \) 且 \( N(\alpha) \geqslant 0 \)，当且仅当 \( \alpha = 0 \) 时等号成立。则$\mathbb{Z}[\sqrt{-5}]$的单位群$U=\{1,-1\}$,且\( 3 \) 是 \( \mathbb{Z}[\sqrt{-5}] \) 的不可约元素，但不是 \( \mathbb{Z}[\sqrt{-5}] \) 的素元素。
\end{example}
\begin{proof}
...
那么称 \( R \) 为\textbf{唯一析因环}(简记为 $\mathbb{UFD}$)或\textbf{唯一分解整环}或$\mathbb{Gauss}$\textbf{环}.称 \( |a| \triangleq r \) 为 \( a \) 的\textbf{长度}. 若 \( u \in U \)，约定 \( |u| \triangleq 0 \).
\end{enumerate}
\end{definition}
\begin{remark}
