...
\item\label{theorem:单位群整除的基本性质-2} $a\sim b$的充要条件是$ac\sim bc.$

\item\label{theorem:单位群整除的基本性质-3} 若\( u \in U \)，则 \( u|a, \forall a \in R^* \). 

\item\label{theorem:单位群整除的基本性质-4} \( u \in U \iff u|1 \).

\item\label{theorem:单位群整除的基本性质-5} $a\sim b\Longleftrightarrow \exists u\in U,\text{使}b=au\Longleftrightarrow \langle a\rangle =\langle b\rangle .$

\item\label{theorem:单位群整除的基本性质-8} 相伴关系是乘法幺半群 \( R^* \) 中的同余关系.

\item\label{theorem:单位群整除的基本性质-7} \( u \in U \iff u \sim 1 \).

\item\label{theorem:单位群整除的基本性质-9} 若 \( a|b, a|c \)，则 \( a|(xb + yc) ,\forall x,y\in R\).
\end{enumerate}
\end{theorem}
\begin{proof}
\begin{enumerate}[(1)]
...
\item {\heiti 必要性:}由$a\sim b$知$b=ad,a=be(d,e\in R)$,于是$bc=adc,ac=bec$,故$ac\mid bc,bc\mid ac$,即$ac\sim bc.$

{\heiti 充分性:}由$ac\sim bc$知$ac=dbc,bc=eac(d,e\in R)$.由\rrefpro{proposition:整环的一些性质}{proposition:整环的一些性质-2}知$R^*$对乘法满足消去律,故$a=db,b=ea$,因此$a\sim b$.

\item 这是因为 \( a = u(u^{-1}a) \).

\item 由\hyperref[theorem:单位群整除的基本性质-3]{性质\ref{theorem:单位群整除的基本性质-3}}知 \( u \in U \) 时，\( u|1 \). 反之，若 \( u|1 \)，即有 \( v \)，使得 \( 1 = vu \)，故 \( v = u^{-1}(u \in U) \).再利用\rrefthe{theorem:主理想和有限生成理想的形状}{theorem:主理想和有限生成理想的形状-2}可得
\begin{align*}
\langle b\rangle =bR=auR\xlongequal{\text{\rrefthe{theorem:主理想和有限生成理想的形状}{theorem:主理想和有限生成理想的形状-2}}}aR=\langle a\rangle .
\end{align*}
...
特别地,$R$的一元多项式环$R[x]$上的不可约元素称为$R[x]$上的\textbf{不可约多项式}.
\end{definition}
\begin{remark}
由定义知若$a$是不可约元素，则$n\mid a\iff n\sim 1\text{或}n\sim a$.
\end{remark}
...
\begin{proposition}\label{proposition:素元素整除性质1}
设 \( R \) 是交换整环，\( R^* = R \setminus \{0\} \),\( U \) 表示 \( R^* \) 中乘法可逆元素的集合, 若$p$是素元素,$u\in U$,$a_i\in R(i=1,2,\cdots,l)$,且$p\mid u\prod_{i=1}^{k}a_i$,则存在$i_0\in \{1,2,\cdots,k\}$,使$p\mid a_{i_0}$.
\end{proposition}
\begin{proof}
由素元素定义可得$p\mid u$或$p\mid \prod_{i=1}^{k}a_i$.若$p\mid u$,则由\rrefthe{theorem:单位群整除的基本性质}{theorem:单位群整除的基本性质-3}知$u\mid p$,故$p\sim u$.再由\rrefthe{theorem:单位群整除的基本性质}{theorem:单位群整除的基本性质-5}知存在$u'\in U$,使$p=uu'\in U$,这与$p$不可约矛盾!故下设$p\mid \prod_{i=1}^{k}a_i$.

由素元素定义可得$p\mid a_k$或$p\mid \prod_{i=1}^{k-1}a_i$.若$p\mid a_k$则结论已经成立.若$p\mid \prod_{i=1}^{k-1}a_i$,则再由素元素定义可得$p\mid a_{k-1}$或$p\mid \prod_{i=1}^{k-2}a_i$.若$p\mid a_{k-1}$则结论已经成立.若$p\mid \prod_{i=1}^{k-2}a_i$,则再由素元素定义可得$p\mid a_{k-2}$或$p\mid \prod_{i=1}^{k-3}a_i$.继续做下去,因为$\prod_{i=1}^{k}a_i$中只有$k$个元素,所以必在有限步终止,故必存在$i_0\in \{1,2,\cdots,k\}$,使$p\mid a_{i_0}$.
\end{proof}

\begin{example}\label{example:抽象代数--例2.4.2}
在整数环 \( \mathbb{Z} \) 中，\( U = \{1, -1\} \)，于是 \( a \sim b \iff a = \pm b \)，因而 \( a \) 为不可约元素当且仅当 \( a \) 为素数或负素数.并且整数环$\mathbb{Z}$的不可约元素都是素元素.
\end{example}
\begin{proof}
...
设 \( \mathbb{P} \) 为数域，则 \( \mathbb{P} \) 上一元多项式环 \( \mathbb{P}[x] \) 为交换整环. 此时 \( U = \mathbb{P}^* = \mathbb{P} \setminus \{0\} \). \( f(x) \sim g(x) \iff \exists c \in \mathbb{P}^* \)，使得 \( f(x) = cg(x) \)，因而 \( f(x) \) 为不可约元素当且仅当 \( f(x) \) 为不可约多项式.并且一元多项式环\( \mathbb{P}[x] \)的不可约元素都是素元素.
\end{example}
\begin{proof}

\end{proof}

\begin{lemma}\label{lemma:交换整环中的素元素必是不可约的}
...
\end{remark}
\begin{proof}
记\( R^* = R \setminus \{0\} \),\( U \) 表示 \( R^* \) 中乘法可逆元素的集合.若 \( a \) 是素元素 \( p \) 的一个因子，即有 \( b \in R^* \)，使 \( p = ab \)，因而 \( p|a \) 或 \( p|b \). 在 \( p|a \) 的情况，说明 \( a \) 不是 \( p \) 的真因子. 若 \( p|b \)，即有 \( c \in R^* \)，使 \( b = pc \)，于是 \( p = pac \)，由\rrefpro{proposition:整环的一些性质}{proposition:整环的一些性质-2}知$R^*$对乘法满足消去律,故 \( ac = 1\)，从而$a \in U$,即 \( a \) 为平凡因子. 这说明 \( p \) 没有非平凡的真因子，故 \( p \) 是不可约元素.
\end{proof}
...
\end{align*}
为$a + b\sqrt{d}$的\textbf{范数}(norm).
\end{definition}

\begin{theorem}\label{theorem:抽象代数--范数的基本性质}
设$d \neq 1,0$且为无平方因子的整数,$D = \mathbb{Z}[\sqrt{d}]$,则对任意的$\alpha,\beta \in D$,有
\begin{enumerate}[(1)]
\item\label{theorem:抽象代数--范数的基本性质-1} $N(\alpha) \in \mathbb{N} \cup \{0\}$,且$N(\alpha) = 0$当且仅当$\alpha = 0$.

\item\label{theorem:抽象代数--范数的基本性质-2} $N(\alpha\beta) = N(\alpha)N(\beta)$,且$\alpha$为单位当且仅当$N(\alpha)=1$.

\item\label{theorem:抽象代数--范数的基本性质-3} 如果$\alpha \mid \beta$(在$D$中),则$N(\alpha) \mid N(\beta)$(在$\mathbb{Z}$中).
\end{enumerate}
\end{theorem}
\begin{proof}
\begin{enumerate}[(1)]
\item 若$\alpha = a + b\sqrt{d}, a,b \in \mathbb{Z}$,则显然$N(\alpha) = |a^2 - db^2| \in \mathbb{N} \cup \{0\}$.若存在不全为零的$a,b \in \mathbb{Z}$,使得$a + b\sqrt{d} = 0$,则$a = -b\sqrt{d}$,所以两边平方得
\begin{align}
a^2 = b^2 d.\label{eq:::::::---:io0fjow::--q8hro2h3r2.5.}
\end{align}
若$b = 0$,则由\eqref{eq:::::::---:io0fjow::--q8hro2h3r2.5.}式得$a = 0$.若$a = 0$,则由\eqref{eq:::::::---:io0fjow::--q8hro2h3r2.5.}式得$b^2 d = 0$,而$d \neq 0$,于是$b = 0$.从而$a,b$都不为0,所以
\begin{align}\label{eq:::::::---:ifjow::--q8hro2h3r2.5.}
d = \frac{a^2}{b^2} > 0.
\end{align}
由于等式\eqref{eq:::::::---:ifjow::--q8hro2h3r2.5.}的右边是一平方数,因此$d$必定也是个平方数.这与$d$无平方因子的假设相矛盾.于是\begin{align}\label{eq:::23i8h892ufwfh}
a + b\sqrt{d} = 0\iff a=b=0.
\end{align}
当$\alpha =a+b\sqrt{d}=0$时,由\eqref{eq:::23i8h892ufwfh}式知$a=b=0,$故此时$N(\alpha)=N(0) = 0$.

当$N(\alpha) = |a^2 - db^2| = 0$时,有$a^2 = b^2d$,故由上述证明同理可得$a = b = 0$,即$\alpha = 0$.

\item 设$\alpha = a + b\sqrt{d}, \beta = a' + b'\sqrt{d}$,则$\alpha\beta = aa' + bb'd + (ab' + ba')\sqrt{d}$.因为
\begin{align*}
(a^2 - db^2)(a'^2 - db'^2) &= (aa')^2 + (bb'd)^2 - [(ab')^2 + (ba')^2]d \\
&= [(aa')^2 + (bb'd)^2 + 2aa'bb'd] - [(ab')^2 + (ba')^2 + 2aa'bb']d \\
&= (aa' + bb'd)^2 - d(ab' + ba')^2,
\end{align*}
所以
\begin{align}\label{eq::230jrsugoj4fijsz}
...
\end{align*}
故此时$\alpha$可逆，即$\alpha$为单位。

当$\alpha$为单位时，由\rrefthe{theorem:抽象代数--范数的基本性质}{theorem:抽象代数--范数的基本性质-1}知$N(\alpha),N(\alpha^{-1})\in\mathbb{N}\cup\{0\}$，若$N(\alpha)\neq1$，则$N(\alpha)>1$，从而由\eqref{eq::230jrsugoj4fijsz}可得
\begin{align*}
1=N(1)=N(\alpha\alpha^{-1})=N(\alpha)N(\alpha^{-1})\Longrightarrow N(\alpha^{-1})=\frac{1}{N(\alpha)}\notin\mathbb{N}\cup\{0\}.
\end{align*}
这与$N(\alpha^{-1})\in\mathbb{N}\cup\{0\}$矛盾！故$N(\alpha)=1$.

\item 如果$\alpha \mid \beta$(在$D$中),则存在$\gamma \in D$,使得$\beta = \alpha\gamma$.因此由\rrefthe{theorem:抽象代数--范数的基本性质}{theorem:抽象代数--范数的基本性质-2}得$N(\beta) = N(\alpha)N(\gamma)$.而$N(\gamma) \in \mathbb{Z}$,所以$N(\alpha) \mid N(\beta)$.
\end{enumerate}
\end{proof}

\begin{example}\label{example:不可约元素不是素元素的例子}
令 \( R = \mathbb{Z}[\sqrt{-5}] = \{a + b\sqrt{-5} \mid a, b \in \mathbb{Z}\} \)。设 \( \alpha = a + b\sqrt{-5} \)，则$\mathbb{Z}[\sqrt{-5}]$的单位群$U=\{1,-1\}$,且\( 3 \) 是 \( \mathbb{Z}[\sqrt{-5}] \) 的不可约元素，但不是 \( \mathbb{Z}[\sqrt{-5}] \) 的素元素。
\end{example}
\begin{proof}
...
再证明 \( 3 \) 是 \( \mathbb{Z}[\sqrt{-5}] \) 的不可约元素，但不是 \( \mathbb{Z}[\sqrt{-5}] \) 的素元素。
...
\begin{definition}
设 \( R \) 是交换整环，\( R^* = R \setminus \{0\} \),\( b, c \in R^* \)。若 \( d \in R^* \) 满足 \( d \mid b \)，\( d \mid c \)，则称 \( d \) 为 \( b, c \) 的\textbf{公因子}。若对 \( b, c \) 的任一公因子 \( d_1 \) 有 \( d_1 \mid d \)，则称 \( d \) 是 \( b, c \) 的\textbf{最大公因子}。也记为$(b,c)$.若 \( (a, b) \sim 1 \)，则称 \( a \) 与 \( b \) 为\textbf{互素}
...
设交换整环 \( R \) 满足最大公因子条件,$a,b,c,a_1,\cdots,a_r,b_1,\cdots,b_r\in R$,则有下列结论：
\begin{enumerate}[(1)]
\item\label{lemma:最大公因子条件的基本结论-1} 设 \( d \) 是 \( a, b \) 的一个最大公因子，则 \( d_1 \) 为$a,b$的最大公因子当且仅当 \( d_1 \sim d \)，即 \( a, b \) 的最大公因子在相伴意义下是唯一的；
...
\item\label{lemma:最大公因子条件的基本结论-4} \( c\left( a_1,a_2,\cdots ,a_r \right) \sim \left( ca_1,ca_2,\cdots ,ca_r \right)\)；

\item\label{lemma:最大公因子条件的基本结论-7} 若$a\in U$,则$(a,b)\sim 1.$ 

\item\label{lemma:最大公因子条件的基本结论-5} 若 \( \left( a,b_i \right) \sim 1,1\leqslant i\leqslant r \),则 \( \left( a,b_1b_2\cdots b_r \right) \sim 1\).

\item\label{lemma:最大公因子条件的基本结论-8} 若$p$是不可约元素,则$p\nmid a\iff (p,a)\sim 1.$

\item\label{lemma:最大公因子条件的基本结论-9} 若$p$是不可约元素,则$(p,(a_1,a_2,\cdots,a_r)) \sim 1$当且仅当存在$s \in \{1,2,\cdots,r\}$，使
\begin{align*}
p \mid a_i,\,1 \leqslant i \leqslant s-1, \quad p \nmid a_s.
\end{align*}
\end{enumerate}
\end{lemma}
\begin{proof}
...
\item 设 \( d = (a, b) \)，\( e = (ca, cb) \)，则 \( cd \mid ca \)，\( cd \mid cb \)。于是 \( cd \mid e \)，因而 \( e = cdu(u\in R^*) \).又由$ca\mid e$知$ca=ex(x\in R^*)$.由此知 \( ca = ex = xucd \)。由\rrefpro{proposition:整环的一些性质}{proposition:整环的一些性质-2}知$R^*$对乘法满足消去律,故\( a = xud \)，即 \( ud \mid a \)，同样有 \( ud \mid b \)，故 \( ud \mid d \)，于是$d=udk(k\in R^*)$,同样由$R^*$对乘法满足消去律可得$uk=1$,因而 \( u \in U \).于是由\rrefthe{theorem:单位群整除的基本性质}{theorem:单位群整除的基本性质-5}知 \( e \) 与 \( cd \) 相伴。再利用数学归纳法易证.

\item 显然$1\mid a,b$. 设$d\mid a,b$，则存在$a_1\in R^*$，使$a=da_1$. 于是由$a\in U$知$1=aa^{-1}=d\left( a_1a^{-1} \right)$，故$d|1$. 因此$\left( a,b \right) \sim 1$.

\item 因为\( (a, b) \sim 1 ,(1,c)\sim 1\)，由\hyperref[lemma:最大公因子条件的基本结论-4]{结论\ref{lemma:最大公因子条件的基本结论-4}} 知 \( (ac, bc) \sim c \), \( (a, ac) \sim a \),故由\hyperref[lemma:最大公因子条件的基本结论-3]{结论\ref{lemma:最大公因子条件的基本结论-3}}及\hyperref[lemma:最大公因子条件的基本结论-6]{结论\ref{lemma:最大公因子条件的基本结论-6}}有 \( 1 \sim (a, c) \sim (a, (ac, bc)) \sim ((a, ac), bc) \sim (a, bc) \).再利用数学归纳法易证.

\item $\Longleftarrow:$ 假设$p \mid a$，则存在$a_1 \in R^*$，使$a = pa_1$。于是由\hyperref[lemma:最大公因子条件的基本结论-4]{结论\ref{lemma:最大公因子条件的基本结论-4}}和\hyperref[lemma:最大公因子条件的基本结论-7]{结论\ref{lemma:最大公因子条件的基本结论-7}}知
...
\item $\Longrightarrow:$假设$p \mid a_i,\,1 \leqslant i \leqslant s$，则$p \mid (a_1,a_2,\cdots,a_r)$。从而$(p,(a_1,a_2,\cdots,a_r)) \sim p$矛盾！故可设$s_1,s_2,\cdots,s_k \in \{1,2,\cdots,r\}$，使
\begin{align*}
p \mid a_i,\,i \notin \{s_1,s_2,\cdots,s_k\}, \quad p \nmid a_s,\,i \in \{s_1,s_2,\cdots,s_k\}.
\end{align*}
取$s = \min\limits_{j=1,2,\cdots,k} s_j$，则
\begin{align*}
p \mid a_i,\,1 \leqslant i \leqslant s-1, \quad p \nmid a_s.
\end{align*}

$\Longleftarrow:$由$p \nmid a_s$知$p \nmid (a_1,a_2,\cdots,a_r)$，否则由$p \mid (a_1,a_2,\cdots,a_r)$知$p \mid a_s$矛盾！于是由\hyperref[lemma:最大公因子条件的基本结论-8]{结论\ref{lemma:最大公因子条件的基本结论-8}}知$$(p,(a_1,a_2,\cdots,a_r)) \sim 1.$$
\end{enumerate}
\end{proof}
...
\end{definition}
\begin{remark}
所谓唯一析因环也就是使因式分解唯一性定理成立的交换整环，因而前面\refexa{example:抽象代数--例2.4.2}与\refexa{example:抽象代数--例2.4.3}中的环$\mathbb{Z}$与$\mathbb{P}[x]$都是UFD，而\refexa{example:不可约元素不是素元素的例子}中的环$\mathbb{Z}[\sqrt{-5}]$就不是。因为$9 = 3^2$与$9 = (2 + \sqrt{-5})(2 - \sqrt{-5})$是9的两种本质上不同的分解，即$\mathbb{Z}[\sqrt{-5}]$不满足唯一析因环定义中的条件(2)。
...
\begin{enumerate}[(1)]
\item\label{theorem:有限析因条件推出标准分解-1} 对$\forall a\in R^*\setminus U$,都存在$r\in \mathbb{N}$,单位$u\in U$以及互不相伴的不可约元素$p_1,p_2,\cdots,p_r$,使
\begin{align*}
a = up_1^{n_1}p_2^{n_2}\cdots p_r^{n_r}, \quad n_i \in \mathbb{N}.
\end{align*}
若 \( c \) 是 \( a \) 的一个非平凡因子，则存在$u_1\in U$以及$n_i'\leqslant n_i$且$n_i'\in \mathbb{N}\cup\{0\}(i=1,2,\cdots,r)$,使
\begin{align*}
c = u_1p_1^{n_1'}p_2^{n_2'}\cdots p_r^{n_r'}.
\end{align*}

\item\label{theorem:有限析因条件推出标准分解-2} 若$a,b\in R^*\setminus U$,则存在$r\in \mathbb{N}$,单位$u,v\in U$以及互不相伴的不可约元素$p_1,p_2,\cdots,p_r$,使
\begin{gather*}
a=up_{1}^{n_1}p_{2}^{n_2}\cdots p_{r}^{n_r},\quad n_i\in \mathbb{N}\cup \{0\};
\\
b=vp_1^{m_1}p_2^{m_2}\cdots p_r^{m_r}, \quad m_i \in \mathbb{N}\cup \{0\}.
\end{gather*}
若还有$d$是$a,b$的公因子,则存在$w\in U$以及$n_i'\leqslant \min \left\{ n_i,m_i \right\} $且$n_i'\in \mathbb{N}\cup\{0\}(i=1,2,\cdots,r)$,使
\begin{align*}
...
a=q_1q_2\cdots q_s.
\end{align*}
将$q_1,q_2,\cdots,q_s$按相伴关系分类,不妨设存在$r\in \mathbb{N}$和
\begin{align*}
...
\item 由(1)知存在$t,s\in \mathbb{N}$,单位$u_1,v_1\in U$,互不相伴的不可约元素$p_1,p_2,\cdots,p_s$和互不相伴的不可约元素$q_1,q_2,\cdots,q_t$,使
\begin{align*}
a=u_1p_{1}^{n_1}p_{2}^{n_2}\cdots p_{s}^{n_s},\quad n_i\in \mathbb{N};
\end{align*}
\begin{align*}
b=v_1q_{1}^{m_1}q_{2}^{m_2}\cdots q_{t}^{m_t},\quad m_i\in \mathbb{N}.
\end{align*}
...
这个因子链是在假设$a_1,a_2,\cdots,a_n,\cdots$都可约且对$\forall n\in \mathbb{N}$有$a_{n+1}$是$a_n$的真因子的条件下得到的.
...
\end{align*}
且\( a^{(n+1)} \mid a^{(n)},a^{(n)}\mid a\),$p_{n+1}$都是$a^{(n)}$的不可约因子,$a^{(n)}=p_{n+1}a^{(n+1)}$.这个因子链是在假设$a^{(n)}\in R^*\setminus U(\forall n\in \mathbb{N})$的条件下得到的.
...
\end{theorem}
\begin{remark}
由这个定理的结论(2)和\reflem{lemma:交换整环中的素元素必是不可约的}知唯一析因环(UFD)中的素元素等价于不可约元素.
\end{remark}
\begin{proof}
...
\end{align}
显然$d$是$a,b$的公因子.又设$d_1$也是\( a, b \) 的公因子,则由\rrefthe{theorem:有限析因条件推出标准分解}{theorem:有限析因条件推出标准分解-2}知存在$u_1\in U$以及$n_i'\leqslant k_i $且$n_i'\in \mathbb{N}(i=1,2,\cdots,r)$,使
\begin{align*}
...
\begin{proposition}\label{proposition:UFD中互素元素整除同一元素则乘积也整除这个元素}
设$R$是唯一析因环,若一组两两互素的素元素$p_1,p_2,\cdots,p_k$都整除$a$,则$\prod_{i=1}^k{p_i}\mid a.$
\end{proposition}
\begin{proof}
由\refthe{theorem:有限析因条件推出标准分解}知存在$u\in U$和互不相伴的不可约元素$q_1,q_2,\cdots,q_r$,使
\begin{align*}
a=uq_1^{n_1}q_2^{n_2}\cdots q_r^{n_r},\quad n_i\in \mathbb{N}.
\end{align*}
对$\forall i\in \{1,2,\cdots,k\}$,由条件知$p_i\mid a$,故由\refpro{proposition:素元素整除性质1}知存在$r_i\in \{1,2,\cdots,r\}$,使$p_i\mid q_{r_i}$.若$q_{r_i}\nmid p_i$,则$p_i$是$q_{r_i}$的真因子.由$q_{r_i}$不可约知$p_i\in U$,这与$p_i$是素元素矛盾!故$p_i\sim q_{r_i}$,由\rrefthe{theorem:单位群整除的基本性质}{theorem:单位群整除的基本性质-5}知存在$u_i\in U$,使
\begin{align*}
q_{r_i}=u_ip_i,\quad i=1,2,\cdots,k.
\end{align*}
因此
\begin{align*}
a&=u\prod_{i=1}^r q_i^{n_i}=u\prod_{i\notin \{r_1,\cdots,r_k\}} q_i^{n_i}\prod_{i=1}^k q_{r_i}^{n_{r_i}} \\
&=u\prod_{i\notin \{r_1,\cdots,r_k\}} q_i^{n_i}\prod_{i=1}^k u_ip_i^{n_i} \\
&=\left( u\prod_{i\notin \{r_1,\cdots,r_k\}} q_i^{n_i}\prod_{i=1}^k u_i \right) \prod_{i=1}^k p_i^{n_i}.
\end{align*}
