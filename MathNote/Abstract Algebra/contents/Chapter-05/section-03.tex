...
取$M$的一组基$e_1,e_2,\dots,e_m$，令
\begin{align*}
M'=\langle e_2,e_3,\dots,e_m\rangle=\left\{\sum_{i=2}^{m}a_ie_i\mid a_i\in D,\,2\leqslant i\leqslant m\right\},
\end{align*}
...
\end{corollary}
\begin{proof}
设$D$模$M$的生成元为$g_1,g_2,\dots,g_m$，$N$为$M$的子模。
秩为$m$的自由$D$模$D^{(m)}$以$e_1,e_2,\dots,e_m$为基，存在模同态$\varphi:D^{(m)}\to M$满足$\varphi(e_i)=g_i,\,1\leqslant i\leqslant m$。

令$K=\varphi^{-1}(N)$，由同态基本定理，$K$为$D^{(m)}$的子模。
由前述定理，$K$为自由$D$模且$r(K)\leqslant m$。
设$f_1,f_2,\dots,f_{r(K)}$为$K$的基，则$\varphi(f_1),\varphi(f_2),\dots,\varphi(f_{r(K)})$为$N$的生成元，故$N$有限生成。
...
\end{theorem}
\begin{proof}
取$M$的一组生成元$x_1,x_2,\dots,x_m$，则存在极大线性无关子集$\{x_1,x_2,\dots,x_r\}$满足：
\begin{enumerate}[(1)]
\item $\sum_{i=1}^{r}a_ix_i=0$当且仅当$a_i=0\,(1\leqslant i\leqslant r)$，即$x_1,x_2,\dots,x_r$线性无关；
\item 对任意$j>r$，存在不全为零的$a_{1j},a_{2j},\dots,a_{rj},a_j\in D$使得
\begin{align}\label{eq:5.3.2}
\sum_{i=1}^{r}a_{ij}x_i+a_jx_j=0,\quad r<j\leqslant m.
\end{align}
\end{enumerate}

令$N=\langle x_1,x_2,\dots,x_r\rangle=Dx_1\oplus Dx_2\oplus\dots\oplus Dx_r$，则$N$为自由$D$模。
由条件，$a_j\neq0\,(r<j\leqslant m)$。
...
设$x_1,x_2,\dots,x_m$为$M$的生成元，$\pi:M\to\overline{M}$为自然同态，则$\{\overline{x}_i=\pi(x_i)\mid 1\leqslant i\leqslant m\}$为$\overline{M}$的生成元，故$\overline{M}$为有限生成无扭$D$模。
由前述定理，$\overline{M}$为自由$D$模。

取$e_1,e_2,\dots,e_r\in M$使得$\overline{e}_1=\pi(e_1),\overline{e}_2=\pi(e_2),\dots,\overline{e}_r=\pi(e_r)$为$\overline{M}$的基，令
\begin{align*}
N=De_1+De_2+\dots+De_r.
\end{align*}
...
\end{align*}
由$\overline{e}_1,\dots,\overline{e}_r$线性无关得$a_1=a_2=\dots=a_r=0$，故$e_1,e_2,\dots,e_r$线性无关，于是
\begin{align*}
N=De_1\oplus De_2\oplus\dots\oplus De_r,\quad N\cap\mathrm{Tor}M=\{0\},
\end{align*}
