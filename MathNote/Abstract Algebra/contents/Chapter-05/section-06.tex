...
主理想整环上有限生成模为有限秩自由模的同态像，即自由模的商模。设$D$为主理想整环，$\mathscr{M}$为$D$上秩$m$的自由模，$\mathscr{N}\subseteq\mathscr{M}$为秩$n$的子模。取$\mathscr{M}$的基$e_1,e_2,\dots,e_m$，$\mathscr{N}$的基$f_1,f_2,\dots,f_n$，则存在$m\times n$矩阵$A\in M_{m\times n}(D)$满足
$$(f_1,f_2,\dots,f_n)=(e_1,e_2,\dots,e_m)A.$$
若更换两组基：
$$(e_1,e_2,\dots,e_m)=(e_1',e_2',\dots,e_m')P,\quad (f_1,f_2,\dots,f_n)=(f_1',f_2',\dots,f_n')Q,$$
其中$P\in GL_m(D)$，$Q\in GL_n(D)$，则
$$(f_1',f_2',\dots,f_n')=(e_1',e_2',\dots,e_m')PAQ^{-1}.$$
...
\end{pmatrix}
\end{align*}
的矩阵，满足$d_i\mid d_{i+1}\ (1\leqslant i\leqslant r-1)$。该矩阵称为$A$的标准形，$d_1,d_2,\dots,d_r$称为$A$的不变因子。
\end{theorem}
...
\begin{corollary}
设$D$为主理想整环，$\mathscr{M}$为秩$m$的自由$D$-模，$\mathscr{N}\subseteq\mathscr{M}$为子模，则存在$\mathscr{M}$的基$e_1,e_2,\dots,e_m$，以及$D$中非零元$d_1,d_2,\dots,d_r$满足：
\begin{enumerate}[(1)]
\item $d_i\mid d_{i+1}\ (1\leqslant i\leqslant r-1)$；
\item $d_1e_1,d_2e_2,\dots,d_re_r$为$\mathscr{N}$的一组基，称$d_1,\dots,d_r$为$\mathscr{N}$的不变因子。
\end{enumerate}
\end{corollary}

\begin{proof}
任取$\mathscr{M}$的基$e_1',\dots,e_m'$，$\mathscr{N}$的生成元$f_1',\dots,f_n'$，则存在$A_1\in M_{m\times n}(D)$使得$f_j'=(e_1',\dots,e_m')\mathrm{col}_jA_1$。

由前述定理，存在$P\in GL_m(D)$，$Q\in GL_n(D)$使得$A=PA_1Q$为标准形。令
$$(e_1,\dots,e_m)=(e_1',\dots,e_m')P^{-1},\quad (f_1,\dots,f_n)=(f_1',\dots,f_n')Q,$$
则$e_1,\dots,e_m$为$\mathscr{M}$的基，$f_1,\dots,f_n$为$\mathscr{N}$的生成元，且
$$(f_1,\dots,f_n)=(e_1,\dots,e_m)A.$$
故$f_i=d_ie_i\ (1\leqslant i\leqslant r)$，$f_i=\boldsymbol{0}\ (i>r)$。由$e_1,\dots,e_m$线性无关，得$d_1e_1,\dots,d_re_r$线性无关，即为$\mathscr{N}$的基。
...
\begin{definition}
设$D$为主理想整环，$A\in M_{m\times n}(D)$，$k\geqslant1$。$A$的所有$k$阶子式的最大公因子称为$A$的$k$级行列式因子，记为$D_k(A)$，规定$D_0(A)=1$。记$A\begin{pmatrix}i_1i_2\cdots i_k\\j_1j_2\cdots j_k\end{pmatrix}$为划去第$i_1,\dots,i_k$行、第$j_1,\dots,j_k$列的$k$阶子式。

若存在$r\in\mathbb{N}$使得$D_r(A)\neq0$且$D_{r+1}(A)=0$，则称$r$为$A$的秩，记为$\mathrm{rank}A=r$。当$k>r$时，$D_k(A)=0$。
\end{definition}

\begin{remark}
由定义可得$D_k(A)\mid D_{k+1}(A)\ (k=0,1,\dots)$。
\end{remark}
...
对任意$k$阶子式，$\det C\begin{pmatrix}i_1\cdots i_k\\j_1\cdots j_k\end{pmatrix}=\det(P_kA_k)$，其中$P_k$为$P$的$i_1,\dots,i_k$行构成的矩阵，$A_k$为$A$的$j_1,\dots,j_k$列构成的矩阵。将行列式按行展开，可得$D_k(A)\mid\det C\begin{pmatrix}i_1\cdots i_k\\j_1\cdots j_k\end{pmatrix}$，故$D_k(A)\mid D_k(C)$。
...
\begin{theorem}
设$D$为主理想整环，$A\in M_{m\times n}(D)$，$d_1,\dots,d_r$为$A$的不变因子，$D_k(A)$为$k$级行列式因子，则
\begin{enumerate}[(1)]
\item $d_k\sim\frac{D_k(A)}{D_{k-1}(A)}\ (1\leqslant k\leqslant r)$；
\item $d_1,\dots,d_r$在相伴意义下唯一；
...
(2) 若$P_1AQ_1$为另一标准形，不变因子为$d_1',\dots,d_r'$，则$D_k(P_1AQ_1)\sim D_k(PAQ)$，故$d_k'\sim d_k$，即不变因子相伴唯一。
...
\begin{theorem}
设$V$为域$F$上$n$维线性空间，线性变换$\mathscr{A}$在基$u_1,\dots,u_n$下的矩阵为$A$。将$V$视为$F[\lambda]$-模，$\eta\colon F[\lambda]^{(n)}\to V$为模同态满足$\eta(e_i)=u_i$，$N=\ker\eta$，则$N$有基
$$f_i=\lambda e_i-(e_1,\dots,e_n)\mathrm{col}_iA,\quad 1\leqslant i\leqslant n.$$
\end{theorem}

\begin{proof}
首先，
\begin{align*}
\eta(f_i)&=\lambda\eta(e_i)-(\eta(e_1),\dots,\eta(e_n))\mathrm{col}_iA\\
&=\mathscr{A}(u_i)-(u_1,\dots,u_n)\mathrm{col}_iA=\boldsymbol{0},
\end{align*}
故$f_i\in N$。

记$A_1e_i=(e_1,\dots,e_n)\mathrm{col}_iA$，则$\lambda e_i=f_i+A_1e_i$。对任意$\sum_{i=1}^ng_i(\lambda)e_i\in F[\lambda]^{(n)}$，存在$h_i(\lambda)\in F[\lambda]$，$b_i\in F$使得
...
$$\sum_{i=1}^nh_i(\lambda)\lambda e_i=\sum_{i=1}^nh_i(\lambda)(e_1,\dots,e_n)\mathrm{col}_iA.$$
由$e_1,\dots,e_n$为$F[\lambda]^{(n)}$的基，得
