...
\end{align*}
其中$u$为$D$中可逆元，$p_1,p_2,\dots,p_r$为互不相伴的元素，则
\begin{align*}
...
\item 若$M\neq\{0\}$且$M=\langle x_1,x_2,\dots,x_r\rangle$，则$a_M=[a_{x_1},a_{x_2},\dots,a_{x_r}]$，即$a_M$是$a_{x_1},a_{x_2},\dots,a_{x_r}$的最小公倍式，且$\mathrm{ann}M$是$D$的非平凡理想。
\end{enumerate}
\end{lemma}
\begin{proof}
\begin{enumerate}[(1)]
...
\item 由(1)，$a_M$是$a_{x_1},a_{x_2},\dots,a_{x_r}$的公倍式。
...
\end{align*}
故$c\in\langle a_M\rangle$，即$a_M\mid c$，因此$a_M=[a_{x_1},a_{x_2},\dots,a_{x_r}]$。
...
\begin{lemma}
设$D$是主理想整环，$M$是$D$上秩为$n$的自由模，$N$是$M$的子模，商模$M/N$是$p$-模，则存在$M$的基$e_1,e_2,\dots,e_n$及非负整数组$k_1,k_2,\dots,k_n$，使得$p^{k_1}e_1,p^{k_2}e_2,\dots,p^{k_n}e_n$为$N$的一组基。
\end{lemma}
\begin{proof}
对$M$的秩$r(M)$用数学归纳法证明。
...
任取$M$的一组基$f_1,f_2,\dots,f_n$，定义
\begin{align*}
I_i=\{a_i\in D\mid \text{若}x=\sum\limits_{j=1}^{n}a_jf_j\in N\},\quad i=1,2,\dots,n,
\end{align*}
...
\end{align*}
则$e_1,f_2,\dots,f_n$是$M$的基，且$p^{k_1}e_1=g\in N$。
...
\end{align*}
故$M_1/N_1$为$p$-模。由归纳假设，存在$M_1$的基$e_2,e_3,\dots,e_n$，使得$p^{k_2}e_2,\dots,p^{k_n}e_n$为$N_1$的基。
因此存在$M$的基$e_1,e_2,\dots,e_n$，使得$p^{k_1}e_1,\dots,p^{k_n}e_n$为$N$的基。
...
\begin{enumerate}[(1)]
\item $M'$可分解为若干循环$p$-模的直和，即$\exists M_1,M_2,\dots,M_m$使得$M_i=Dy_i$，满足
\begin{align*}
M'=\bigoplus\limits_{i=1}^{m}M_i,\quad \mathrm{ann}y_i=\langle p^{k_i}\rangle\ (k_i\geqslant1);
\end{align*}
\item 若分解满足$k_1\leqslant k_2\leqslant\cdots\leqslant k_m$，则数组$k_1,k_2,\dots,k_m$由$M'$唯一确定。
\end{enumerate}
\end{theorem}
\begin{proof}
(1) 设$x_1,x_2,\dots,x_n$为$M'$的生成元，$M$为$D$上秩为$n$的自由$D$-模。
由自由模的同态性质，存在$M$到$M'$的满同态$\eta$，满足$\eta(f_i)=x_i$，其中$f_1,\dots,f_n$为$M$的基。
令$N=\ker\eta$，则$M/N\cong M'$为$p$-模。由引理5.4.3，存在$M$的基$e_1,\dots,e_n$及数组$0\leqslant m_1\leqslant m_2\leqslant\cdots\leqslant m_n$，使得$N$的基为$p^{m_1}e_1,\dots,p^{m_n}e_n$。
于是
\begin{align*}
M/N\cong\bigoplus\limits_{i=1}^{n}De_i/Dp^{m_i}e_i.
\end{align*}
若$m_i=0$，则$De_i/Dp^{m_i}e_i=\{0\}$。将$m_1,\dots,m_n$中非零项记为$k_1,\dots,k_m$，满足$1\leqslant k_1\leqslant k_2\leqslant\cdots\leqslant k_m$，则
\begin{align*}
...
\begin{corollary}
设$N$为秩$n$自由$D$-模$M$的子模，$M/N$为$p$-模。若$e_1,\dots,e_n$与$f_1,\dots,f_n$均为$M$的基，$p^{k_1}e_1,\dots,p^{k_n}e_n$与$p^{l_1}f_1,\dots,p^{l_n}f_n$均为$N$的基，且$k_1\leqslant\cdots\leqslant k_n$，$l_1\leqslant\cdots\leqslant l_n$，则$l_i=k_i,\ \forall i$。
\end{corollary}
\begin{proof}
由
\begin{align*}
...
\end{align*}
其中$\lambda=\max\{\lambda_1,\lambda_2,\dots,\lambda_s\}$。

令$s=\lambda+\lambda_0$，将$x_{01},\dots,x_{0\lambda_0},x^{(1)},\dots,x^{(\lambda)}$记为$z_s,z_{s-1},\dots,z_1$，则
\begin{align*}
