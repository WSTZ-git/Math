...
\label{eq:mo23423efSGE3}
\end{align}
对\eqref{eq:mo23423efSGE3}关于$u_i^\alpha$求导，整理可得
\begin{align}
&\left( \frac{\partial \Gamma _{\gamma \beta}^{(i)\delta}}{\partial u_{i}^{\alpha}}-\frac{\partial \Gamma _{\gamma \alpha}^{(i)\delta}}{\partial u_{i}^{\beta}} \right) \cdot \frac{\partial u_{j}^{\xi}}{\partial u_{i}^{\delta}}+\Gamma _{\gamma \beta}^{(i)\delta}\cdot \frac{\partial ^2u_{j}^{\xi}}{\partial u_{i}^{\delta}\partial u_{i}^{\alpha}}-\Gamma _{\gamma \alpha}^{(i)\delta}\cdot \frac{\partial ^2u_{j}^{\xi}}{\partial u_{i}^{\delta}\partial u_{i}^{\beta}}
\nonumber \\
&=\frac{\partial u_{j}^{\zeta}}{\partial u_{i}^{\gamma}}\cdot \frac{\partial u_{j}^{\eta}}{\partial u_{i}^{\beta}}\frac{\partial u_{j}^{\mu}}{\partial u_{i}^{\alpha}}\left( \frac{\partial \Gamma _{\zeta \eta}^{(j)\xi}}{\partial u_{j}^{\mu}}-\frac{\partial \Gamma _{\zeta \eta}^{(j)\xi}}{\partial u_{j}^{\nu}}+\Gamma _{\zeta \eta}^{(j)\nu}\Gamma _{\nu \mu}^{(j)\xi}-\Gamma _{\zeta \mu}^{(j)\nu}\Gamma _{\nu \eta}^{(j)\xi} \right) .
\label{eq:mo23423efSGE4}
\end{align}
...
代入黎曼曲率张量定义\eqref{eq:mo23423efSGE1}，展开化简得
\begin{align}
...
\label{eq:mo23423efSGE7}
\end{align}
由此可直接推导出对称性\eqref{eq:mo23423efSGE8}，二维情形下其余分量均可由$R_{1212}$表示，故仅一个实质分量.
...
\end{align*}
代入黎曼张量公式\eqref{eq:mo23423efSGE7}，得
\begin{align*}
