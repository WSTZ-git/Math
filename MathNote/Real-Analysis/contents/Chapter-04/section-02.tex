...
\[
\lim_{n \to \infty} f_n(x) = f(x) \ (x \in E), \quad f_n(x) \leqslant f_{n + 1}(x) \ (n \in \mathbb{N}, x \in E),
\]
则
\[
...
&= \sum_{i=1}^{\infty} \frac{1}{i^{1 + (\frac{1}{2})} m(E_i)} m(E_i) < +\infty,
\end{align*}
所以 $f \in L^1(E)$，但我们有
\[
...
若 $f \in L(\mathbb{R}^n)$，则对任意的 $y_0 \in \mathbb{R}^n$，$f(x + y_0) \in L(\mathbb{R}^n)$，有
\begin{align*}
...
\end{align*}
即$\lim\limits_{n\to\infty} f_n(x)=0,\forall x\in \mathbb{R}\setminus E_\varepsilon.$
\end{proof}
...
\leqslant 1.
\end{align*}
故由\hyperref[theorem:积分求导定理]{积分求导定理}可知，对$\forall x\in\mathbb{R}$，都有$g''(x)$存在。再结合\refthe{theorem:f与|f|的可积性等价}知
\begin{align*}
|g''(x)|
