...
\end{align*}
同时有$\overline{B}(x_1,\delta_1)\subset B(x_0,\delta_0)$. 再从$\overline{B}(x_1,\delta_1)$出发以类似的推理使用于$F_2$，则可得$\overline{B}(x_2,\delta_2)\cap F_2=\varnothing$，同时有$\overline{B}(x_2,\delta_2)\subset B(x_1,\delta_1)$，这里可以要求$0<\delta_2<\frac{1}{2}$. 继续这一过程，可得点列$\{x_k\}$与正数列$\{\delta_k\}$，使得对每个自然数$k$，有
\begin{align*}
\overline{B}(x_k,\delta_k)\subset B(x_{k - 1},\delta_{k - 1}), \quad \overline{B}(x_k,\delta_k)\cap F_k=\varnothing,
\end{align*}
其中$0<\delta_k<\frac{1}{k}$. 由于当$l>k$时，有$x_l\in B(x_k,\delta_k)$，故
\begin{align*}
...
\begin{align*}
|f_{n_k}(x)-f(x)|<\frac{1}{2}^{k + 1} \quad (x\in\mathbb{R}).
\end{align*}
