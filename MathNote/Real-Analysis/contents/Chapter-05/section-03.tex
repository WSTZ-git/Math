...
\end{proposition}
\begin{note}
还可以利用\refcor{corollary:可微的可测函数像集外测度小于导数积分}和\refcor{corollary:可微的可测函数像集外测度小于导数积分}的结论立即得到(1)的结论.
\end{note}
\begin{proof}
\begin{enumerate}[(1)]
...
\begin{align}
\sum\limits_{i=1}^{\infty}|K_{ni}|\leqslant \sum\limits_{i=1}^{\infty}|J_{ni}|<\frac{1}{n},\quad K_{ni}\cap K_{nj}=\varnothing\ (\forall i\neq j),\quad m^*\left( f\left( \overline{J_{ni}} \right) \right) =m^*\left( f\left( K_{ni} \right) \right) =f\left( b_{ni} \right) -f\left( a_{ni} \right)\ (\forall i\in \mathbb{N}). \label{eq::::--234823urjreosew2}
\end{align}
...
\end{align}
当$n>\frac{1}{\delta}$时，由\eqref{eq::::--234823urjreosew2}式知$\sum\limits_{i=1}^{\infty}|K_{ni}|<\frac{1}{n}$，故由\eqref{eq::::--234823urjreosew3}式知
\begin{align*}
...
\end{align*}
于是再结合\eqref{eq::::--234823urjreosew1}、\eqref{eq::::--234823urjreosew2}式以及\rrefpro{Set-Theory-proposition:映射的基本性质}{Set-Theory-proposition:映射的基本性质-2}知
\begin{align*}
