...
再任取 $a \in I - \mathfrak{m}$。则 $a + \mathfrak{m} \neq 0 + \mathfrak{m}$.由于$R/\mathfrak{m}$是一个域,故$a+\mathbb{m}$有逆元,即存在 $b \in R$，使得 $(a+\mathfrak{m})(b+\mathfrak{m})=ab + \mathfrak{m} = 1 + \mathfrak{m}$。因此，也存在 $m \in \mathfrak{m}$，使 $1 = ab + m$。
