...
\item\label{proposition:映射与基数之间的关系-2} 若存在从 $A$ 到 $B$ 的满射, 则 $\overline{\overline{A}} \geqslant \overline{\overline{B}}$.
...
\begin{example}\label{example:-1,1与R对等}
$[-1, 1] \sim \mathbb{R}$.
\end{example}
\begin{proof}
由\refexa{example-214234210.2}可知, $\mathbb{R} \sim (-1, 1) \subseteq [-1, 1]$; 又 $[-1, 1] \sim [-1, 1] \subseteq \mathbb{R}$. 由\hyperref[theorem:Cantor-Bernstein定理]{Bernstein定理(1)}可知, $[-1, 1] \sim \mathbb{R}$.
