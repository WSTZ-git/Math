...
设$f:A\to B$,则
\begin{enumerate}
\item 若$B$中每个元素最多只有一个原像,即对$\forall y\in B$,$f^{-1}(y)$所含元素个数为0或1,则称 $f$ 为\textbf{单射}或\textbf{一一映射}.
...
\item $f$为单射的充要条件是
\begin{align*}
f(x_1)=f(x_2)\iff x_1=x_2,\quad \forall x_1,x_2\in A.
\end{align*}
也即
\begin{align*}
f(x_1)=f(x_2)\Longrightarrow x_1=x_2,\quad \forall x_1,x_2\in A.
\end{align*}
亦即
\begin{align*}
...
\end{align*}
此时必有$f=f^{-1}$.即有两个交换图,如\reffig{figure:交换图fiou3nr4-1}所示.
\end{enumerate}
\end{theorem}
\begin{note}
\begin{figure}[H]
...
对于映射$f : C \to D$,$A\subseteq C,B\subseteq D$,$\{A_\alpha\}_{\alpha \in I}\subseteq C,\{B_\alpha\}_{\alpha \in I}\subseteq D$,则下列事实成立:
\begin{enumerate}[(i)]
\item\label{proposition:映射的基本性质-1} 若 $A \subseteq B$, 则 $f(A) \subseteq f(B)$;若$B_1\subseteq B_2\subseteq D$,则$f^{-1}(B_1)\subseteq f^{-1}(B_2).$

\item $f\left(\bigcup_{\alpha \in I} A_{\alpha}\right) = \bigcup_{\alpha \in I} f(A_{\alpha})$;$f^{-1}\left(\bigcup_{\alpha \in I} B_{\alpha}\right) = \bigcup_{\alpha \in I} f(B_{\alpha})$.

\item $f^{-1}\left(\bigcap_{\alpha\in I}B_{\alpha}\right)=\bigcap_{\alpha\in I}f^{-1}(B_{\alpha})$;

$f\left(\bigcap_{\alpha \in I} A_{\alpha}\right) \subseteq \bigcap_{\alpha \in I} f(A_{\alpha})$, 当且仅当 $f$ 为单射时“$=$” 成立.

\item $f\left(f^{-1}(B)\right) \subseteq B$, 当且仅当 $f$ 为满射时“$=$” 成立;
...
(iv)中第二条的直观理解是:$C\setminus A$中的某些元素的像也可能在$f(A)$(即$f$可能不是单射).
\end{note}
\begin{proof}
(i) 显然, (ii) 和 (v) 容易验证.

(iii) 只证明两个集合的情形. 注意到 $A_1 \cap A_2 \subseteq A_1$, $A_1 \cap A_2 \subseteq A_2$, 由 (i) 知
\begin{align*}
...
(iv) 
(1) 设 $y \in f(f^{-1}(B))$, 则存在 $x \in f^{-1}(B)$ 使得 $y = f(x)$. 故 $y = f(x) \in B$. 因此, $f(f^{-1}(B)) \subseteq B$.
...
(2) 设 $x \in A$, 则 $f(x) \in f(A)$, 故 $x \in f^{-1}(f(A))$. 因此, $A \subseteq f^{-1}(f(A))$.
...
\begin{enumerate}[(1)]
\item  $A = B \iff \chi_A(x) = \chi_B(x)$; 

\item $A \subseteq B \iff \chi_A(x) \leqslant \chi_B(x)$;

\item $\chi_{A \cap B}(x) = \chi_A(x) \cdot \chi_B(x)$;

\item $\chi_{A \cup B}(x) = \chi_A(x) + \chi_B(x)\backslash  \chi_A(x) \cdot \chi_B(x)$;

\item \(\chi_{A\setminus B}(x)=\chi_A(x)[1\backslash  \chi_B(x)]\);
