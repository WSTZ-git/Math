...
又若映射 \( f:A_0 \to B \) 与映射 \( g:A \to B \) 满足 \( gi = f \), 即 \( g(x)=f(x) \, (\forall x \in A_0) \), 则称 \( g \) 为 \( f \) 的\textbf{开拓}, \( f \) 为 \( g \) 在 \( A_0 \) 上的\textbf{限制}, 记为 \( f = g|_{A_0} \).即\reffig{figure:嵌入映射交换图}为交换图.
\end{definition}
\begin{figure}[H]
\centering
\includegraphics[scale=0.4]{嵌入映射交换图.png}
\caption{}
