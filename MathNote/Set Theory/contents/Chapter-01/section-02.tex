...
\end{note}
\begin{proof}
如\reffigure{figure:去掉一点的圆周与实轴对等}, 设圆周为 $C$, 从除去的点 $P$ 作过圆心的直线, 取与该直线垂直且与圆周相切 (不过点 $P$ ) 的直线表示实轴 $\mathbb{R}$. 对于 $C - \{P\}$ 上的每一点 $c$, 从点 $P$ 作过点 $c$ 的直线必与实轴相交于某点, 记为 $x$. 建立一一对应: $s : \mathbb{R} \to C - \{P\}$ 为 $s(x) = c$. (点 $P$ 对应 $\infty$) 
\begin{figure}[H]
\centering
...
\end{theorem}
\begin{proof}
由\reftheorem{theorem:对等关系也是等价关系}及\hyperref[definition:集合的基数(势)]{集合的基数(势)的定义}可直接得到.
\end{proof}
...
\begin{proof}
\begin{enumerate}[(1)]
\item 由题设存在单射 $f : A \to B$, $g : B \to A$, 利用\reflemma{lemma:映射定义域与陪域的无交分解}可得到 $A$ 与 $B$ 的分解
\begin{align*}
...
\item 由\refdefinition{definition:集合的基数(势)}和\refdefinition{definition:集合的基数大小关系}可知(2)$\Leftrightarrow$(1).
\end{enumerate}
\end{proof}
...
\end{example}
\begin{proof}
由\refexample{example-214234210.2}可知, $\mathbb{R} \sim (-1, 1) \subset [-1, 1]$; 又 $[-1, 1] \sim [-1, 1] \subset \mathbb{R}$. 由\hyperref[theorem:Bernstein定理]{Bernstein定理(1)}可知, $[-1, 1] \sim \mathbb{R}$.
\end{proof}
