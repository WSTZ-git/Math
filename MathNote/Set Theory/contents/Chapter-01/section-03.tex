...
我们称\((0,1]\)的基数为\textbf{连续基数},记为\(c\)或\(\aleph_1\)或$\aleph$.
\end{definition}
...
\begin{theorem}\label{theorem:无穷多个集列的并的势}
...
由\hyperref[theorem:Cantor-Bernstein定理]{Bernstein定理}可知 $\overline{\overline{(0, 1]}} = \overline{\overline{\mathcal{F}_{\mathbb{N}}}}$. 再由\reflem{lemma:2^A与A上所有特征函数全体对等}可得$\aleph =\overline{\overline{(0,1]}}=\overline{\overline{\mathcal{F} _{\mathbb{N}}}}=\overline{\overline{2^{\mathbb{N}}}}=2^{\aleph _0}.$
