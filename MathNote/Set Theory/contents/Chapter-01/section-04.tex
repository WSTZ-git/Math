...
\begin{remark}
如果$A$是非空集合，$A$中的序列是一个函数$\boldsymbol{N} \to A$。一个序列通常表示成$\{a_0, a_1, \cdots\}$，$\{a_i\}_{i \in \boldsymbol{N}}$或者$\{a_i\}$，其中$a_i \in A$是$i \in \boldsymbol{N}$的象。类似地，函数$\boldsymbol{N}^* \to A$也称作序列，并且表示成$\{a_1, a_2, \cdots\}$，$\{a_i\}_{i \in \boldsymbol{N}^*}$或者$\{a_i\}$，这些符号在课文中不会引起混乱。
\end{remark}
