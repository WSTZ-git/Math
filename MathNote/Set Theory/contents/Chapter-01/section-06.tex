...
\end{definition}
\begin{remark}
等价关系的三个条件是互相独立的,即: 已知任意两个条件不一定能推出第三个条件.

但若$\sim $是集合$A$中的关系,且$\sim$满足对称性和传递性,则$\sim$具有自反性.
\begin{proof}
例如,实数集$\mathbb{R}$中的关系$\sim$
\begin{align*}
...
实数集中关系$\sim$
\begin{align*}
a \sim b \iff |a - b| \leqslant 1
\end{align*}
满足自反性和对称性,但不满足传递性.

在非负整数集$\mathbb{N}_0$中定义关系$\sim$
\begin{align*}
a \sim b \iff a\text{与}b\text{均为正数且有相同的奇偶性}
\end{align*}
则易见$\sim$满足对称性和传递性,但不满足自反性,因为没有$0 \sim 0$.

若$\sim $是集合$A$中的关系,且$\sim$满足对称性和传递性,设$a\sim b$,则由$\sim$满足对称性知$b\sim a$,又由$\sim $满足传递性可得$a\sim a$.
...
设 \( R \) 是集合 \( A \) 的等价关系, 则由所有不同的等价类构成的子集族 \( \{ K_a \} \) 是 \( A \) 的分划.即
\begin{align*}
A=\bigsqcup_{a\in A}K_a.
\end{align*}
其中$a$取遍不同$A$的等价类的代表元.
...
\end{figure}
\begin{proof}
考虑 \( y \in B \) 的原像 \( f^{-1}(y) \) 构成的子集族. 显然, \( A = \bigcup_{y \in B} f^{-1}(y) \). 又若 \( y,z \in B \), \( f^{-1}(y) \cap f^{-1}(z) \neq \varnothing \), 即 \( \exists a \in A \), 使 \( f(a) = y, f(a) = z \), 即 \( y = z \). 故 \( f^{-1}(y) = f^{-1}(z) \), 从而 \( \{ f^{-1}(y) \} \) 是 \( A \) 的一个分划. 于是由\refthe{theorem:等价类就和集合的分划对应-定理1.1.1}知,在 \( A \) 中可定义等价关系 \( R: aRb \), 若 \( \exists f^{-1}(y) \), 使 \( a,b \in f^{-1}(y) \), 即 \( f(a) = f(b) \). 由此知定理的第一部分成立.
...
\begin{definition}[完全代表系]\label{definition:等价类的完全代表系}
设$\sim$是集合$X$上的等价关系,则子集$R\subseteq X$称为等价关系$\sim$的\textbf{完全代表系},当且仅当满足以下条件:
\begin{align*}
|R\cap C|=1,\,\,\forall C\in X/\sim .
\end{align*}
即$R$与每个等价类有且仅有一个公共元素.
\end{definition}

\begin{theorem}\label{theorem:等价关系的完全代表系的存在性}
设$\sim $是集合$X$上的等价关系,则必存在$X$的子集$R$为等价关系$\sim$的完全代表系.
\end{theorem}
\begin{remark}
这个定理表明:任意等价关系的完全代表系的存在性与选择公理等价.

一个等价关系的完全代表系可以不唯一.
\end{remark}
\begin{proof}
由\refthe{theorem:等价类就和集合的分划对应-定理1.1.1}知
\begin{align*}
\bigcup_{K\in X/\sim} K = X.
\end{align*}
由\hyperref[axiom:选择公理]{选择公理}知, 存在一个选择函数
\begin{align*}
f: X/\sim \to \bigcup_{K\in X/\sim} K = X,
\end{align*}
使得对 $\forall K\in X/\sim$, 都有 $f(K) \in K$. 令
\begin{align*}
R = \left\{ f(K) \mid K\in X/\sim \right\},
\end{align*}
则对 $\forall C\in X/\sim$, 有 $f(C) \in C$. 再对 $\forall f(K) \in R\setminus \{f(C)\}$, 有 $K\neq C$, 则由\refthe{theorem:等价类就和集合的分划对应-定理1.1.1}知
\begin{align*}
K\cap C = \varnothing.
\end{align*}
而 $f(K) \in K$, 故 $f(K) \notin C$. 因此 $R\cap C = \left\{ f(C) \right\}$, 即 $|R\cap C| = 1$.

\end{proof}

\begin{theorem}
设$\sim$是集合$X$上的等价关系,$X$的子集$R$为等价关系$\sim$的\textbf{完全代表系}的充分必要条件是满足以下任意一个条件:
\begin{enumerate}[(1)]
\item 对$\forall x\in X$,存在$r\in R$,使得$x\sim r$.并且对$\forall r_1,r_2\in R$且$r_1\neq r_2$,有$r_1$与$r_2$不等价.

\item $X=\bigsqcup_{r\in R}K_r$,也即
\begin{align*}
X=\bigcup_{r\in R}{K_r}\,\text{且}\,K_{r_1}\cap K_{r_2}=\varnothing ,\,\forall r_1,r_2\in R\text{且}r_1\ne r_2.
\end{align*}
...
\begin{example}
\begin{enumerate}
\item 设 \( m \in \mathbb{Z} \)( 所有整数的集合 ), \( m \neq 0 \). 在 \( \mathbb{Z} \) 中定义关系
\[
a \sim b, \quad \text{若 } a \equiv b \pmod{m}.
\]
易证 \( \sim \) 是等价关系且由 \( a \equiv b \pmod{m} \), \( c \equiv d \pmod{m} \) 可得 \( a + c \equiv b + d \pmod{m} \), \( ac \equiv bd \pmod{m} \). 因而 \( \sim \) 对于 \( \mathbb{Z} \) 中的加法与乘法都是同余关系.

\item 设 \( \mathbb{P}[x] \) 是数域 \( \mathbb{P} \) 上一元多项式的集合. 设 \( f(x) \in \mathbb{P}[x] \), \( f(x) \neq 0 \). 在 \( \mathbb{P}[x] \) 中定义关系 \( \sim \): \( g(x) \sim h(x) \), 若 \( f(x) \mid (g(x) - h(x)) \). 与第一问类似可证 \( \sim \) 对 \( \mathbb{P}[x] \) 中的加法与乘法都是同余关系.

\item 以 \( \mathbb{P}^{n \times n} \) 表示数域 \( \mathbb{P} \) 上所有 \( n \) 阶方阵的集合. 方阵的加法与乘法都是 \( \mathbb{P}^{n \times n} \) 中的二元运算. 对 \( \boldsymbol{A} \in \mathbb{P}^{n \times n} \), 用 \( \text{ent}_{ij}\boldsymbol{A} \), \( \text{row}_i\boldsymbol{A} \), \( \text{col}_j\boldsymbol{A} \) 和 \( \det\boldsymbol{A} \) 分别表示 \( \boldsymbol{A} \) 的第 \( i \) 行第 \( j \) 列元素、\( \boldsymbol{A} \) 的第 \( i \) 行、\( \boldsymbol{A} \) 的第 \( j \) 列和 \( \boldsymbol{A} \) 的行列式.

\( \mathbb{P}^{n \times n} \) 中由 \( \det\boldsymbol{A} = \det\boldsymbol{B} \) 确定的关系, 对乘法是同余关系, 但对加法除 \( n = 1 \) 的情形外不是同余关系.
\end{enumerate}
\end{example}
