...
\begin{enumerate}[(1)]
\item $\mathrm{Gal}(K/F)$同构于对称群$S_{\{\alpha_1,\alpha_2,\cdots,\alpha_m\}}$的一个子群；
\item $f(x)\in F[x]$不可约当且仅当$\mathrm{Gal}(K/F)$在$\{\alpha_1,\alpha_2,\cdots,\alpha_m\}$上的作用可递。
\end{enumerate}
\end{theorem}

\begin{proof}

记$X=\{\alpha_1,\alpha_2,\cdots,\alpha_m\}$。
(1) 对任意$\sigma\in\mathrm{Gal}(K/F)$，由$f(\alpha_i)=0$，得$f(\sigma(\alpha_i))=\sigma(f(\alpha_i))=0$，故$\sigma(\alpha_i)\in X$，即$\sigma|_X\in S_X$。
由$K=F(\alpha_1,\alpha_2,\cdots,\alpha_m)$，$\sigma|_X=\tau|_X$当且仅当$\sigma=\tau$，因此$\mathrm{Gal}(K/F)\cong\{\sigma|_X\mid \sigma\in\mathrm{Gal}(K/F)\}\leqslant S_X$。
...
\begin{theorem}
设$F$为域，$x_1,x_2,\cdots,x_n$为不定元，$p_1,p_2,\cdots,p_n$为$x_1,x_2,\cdots,x_n$的初等对称多项式，记$g(x)=\prod\limits_{i=1}^n\limits (x-x_i)\in F(p_1,p_2,\cdots,p_n)[x]$，则
$$\mathrm{Gal}(F(x_1,x_2,\cdots,x_n)/F(p_1,p_2,\cdots,p_n))\cong S_n.$$
\end{theorem}

\begin{proof}

记$G=\mathrm{Gal}(F(x_1,x_2,\cdots,x_n)/F(p_1,p_2,\cdots,p_n))$。
对任意$\sigma\in S_n$，$\sigma$可延拓为多项式环$F[x_1,x_2,\cdots,x_n]$的$F$-自同构：$\sigma(a)=a\ (\forall a\in F)$，$\sigma(x_i)=x_{\sigma(i)}$，进一步延拓为分式域$F(x_1,x_2,\cdots,x_n)$的$F$-自同构，故$S_n\leqslant G$。
由$\sigma(p_i)=p_i\ (1\leqslant i\leqslant n)$，得$F(p_1,p_2,\cdots,p_n)\subseteq \mathrm{Inv}S_n$，因此
\begin{align*}
[F(x_1,x_2,\cdots,x_n):\mathrm{Inv}S_n]\leqslant [F(x_1,x_2,\cdots,x_n):F(p_1,p_2,\cdots,p_n)].
\end{align*}
$F(x_1,x_2,\cdots,x_n)$是$g(x)$的分裂域，$\deg g(x)=n$，故$[F(x_1,x_2,\cdots,x_n):F(p_1,p_2,\cdots,p_n)]\leqslant n!$。
结合$[F(x_1,x_2,\cdots,x_n):\mathrm{Inv}S_n]\geqslant n!$，得
\begin{align*}
[F(x_1,x_2,\cdots,x_n):\mathrm{Inv}S_n]=[F(x_1,x_2,\cdots,x_n):F(p_1,p_2,\cdots,p_n)]=n!,
\end{align*}
于是$\mathrm{Inv}S_n=F(p_1,p_2,\cdots,p_n)$，即$G\cong S_n$.

\end{proof}

\begin{corollary}
$g(x)=\prod\limits_{i=1}^n\limits (x-x_i)$是$F(p_1,p_2,\cdots,p_n)$上的不可约多项式。
\end{corollary}

\begin{proof}

$S_n$在$\{x_1,x_2,\cdots,x_n\}$上的作用可递，故$g(x)$不可约.
...
不妨设$p$阶元$\sigma=(1\ 2\ \cdots\ p)\in G$，对换$(1\ 2)\in G$，则
\begin{align*}
...
\begin{example}
设$k>3$为奇数，$m$为正偶数，$n_1<n_2<\cdots<n_{k-2}$为$k-2$个偶数，令
\begin{align*}
