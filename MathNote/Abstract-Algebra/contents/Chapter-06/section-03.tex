...
求导得$(x^n-1)'=nx^{n-1}$，由$\mathrm{ch}\,F\nmid n$，得$\gcd(x^n-1,nx^{n-1})=1$，故$x^n-1$无重根，$n$次单位根群为$n$阶循环群$\langle\theta\rangle=\{1,\theta,\dots,\theta^{n-1}\}$。
...
\begin{definition}
设$\theta_1,\theta_2,\dots,\theta_{\varphi(n)}$为全体$n$次本原单位根，称首一多项式
\begin{align*}
...
当$k\geqslant3$时，由Abel群基本定理，$U_{2^k}=H_1\times H_2\times\dots\times H_m$，其中$|H_i|=2^{k_i}$。
