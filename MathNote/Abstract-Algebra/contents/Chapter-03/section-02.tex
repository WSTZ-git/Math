...
\end{align*}
则$f_2$也定义了$G$在$G$上的一个作用，称为\textbf{伴随作用}或\textbf{共轭作用}。
\end{enumerate}
...
\end{align*}
则$f$定义了$G$在$G/H$上的\textbf{左平移(左诱导)作用}。特别地，当$H = \{e\}$时，$f$恰是$G$在$G$上的左平移作用。
\end{enumerate}
...
\begin{definition}
设群$G$作用在集合$X$上。若$\forall x, y \in X$, $\exists g \in G$, 使$y = g(x)$，则称$G$在$X$上的作用是\textbf{可递的}或\textbf{可迁的}，$X$称为(对于$G$的)\textbf{齐性空间}。
\end{definition}
...
\begin{definition}
设群$G$作用在集合$X$上，$H\leqslant G$,$x \in X$。称$X$中的子集
\begin{align*}
O_x(H)=Hx = \{g(x)\in X \mid g \in H\}
\end{align*}
为$x$在$H$中的\textbf{轨道}。有时也简记为$O_x$.

$G$中子集
\begin{align*}
S_x(H)=H_x= \{g \in H\mid  g(x) = x\}
\end{align*}
称为$x$在$H$中的\textbf{迷向子群}.也称为$x$在$G$中的\textbf{稳定子}或\textbf{稳定化子}.有时也简记为$S_x$.
\end{definition}
\begin{remark}
容易验证\textbf{$x$的迷向子群是$G$的子群.}
\end{remark}
...
\item 设群$G$作用在集合$X$上。
\begin{enumerate}[(1)]
\item\label{theorem:抽象代数--定理4.2.1-1} 对$\forall g \in G$，定义$X$到$X$的映射
\begin{align}
\sigma _g:X\rightarrow X,\,\,x\rightarrow g\left( x \right) . \label{eq::::--g89eht34y4yehygte4tegg4.2.1}
\end{align}
则定义的$\sigma_g$是$X$的可逆变换，即$\sigma_g \in S_X$。

\item\label{theorem:抽象代数--定理4.2.1-2} 定义的$G$到$S_X$的映射$\sigma$满足
\begin{align*}
\sigma :G\rightarrow S_X,\,\,g\mapsto \sigma _g.
\end{align*}
...
\item 设$G$是一个群,$X$是一个非空集合,若$\sigma$是群$G$到$S_X$的同态，则由
\begin{align}
f:G\times X\rightarrow X,\,\,\left( g,x \right) \mapsto \sigma \left( g \right) \left( x \right) \label{eq::::--g89eht34y4yehygte4tegg4.2.2}
\end{align}
定义了$G$在$X$的作用.
\end{enumerate}
\end{theorem}
...
\item 显然$\sigma_g$都是良定义的.任取$g \in G$，由式\eqref{eq::::--g89eht34y4yehygte4tegg4.2.1}有
\begin{align*}
...
\begin{corollary}[轨道公式]\label{corollary:有限群轨道的阶与其商掉迷向子群的阶相同}
...
设$G$是一个群,在伴随作用下.
对$\forall g\in G$,则$G$上的\hyperref[theorem:内自同构群及其基本性质-2]{内自同构}$\text{ad}g$:
...
设$G$是一个群,$g \in G$, $g$在伴随作用下的轨道称为在$G$中以$g$为代表的\textbf{共轭类},记为$C_g(G)$.在不混淆时,也可简记为$C_g$.若$h \in C_g(G)$,则称$h$与$g$\textbf{共轭}.
...
在伴随作用下,对$\forall g\in G$,则$G$上的\hyperref[theorem:内自同构群及其基本性质-2]{内自同构}$\text{ad}g$:
\begin{align*}
\text{ad}g(x) = gxg^{-1}, \quad \forall x \in G.
\end{align*}
也称$\text{ad}g$为群$G$的\textbf{共轭变换},$x$在共轭变换下的像$gxg^{-1}$称为$x$的\textbf{共轭元}.

再定义$G$到$\text{Int}G$的\hyperref[theorem:内自同构群及其基本性质-4]{同态}$\text{ad}$满足
\begin{align*}
\text{ad}:g \to \text{ad}g,\quad \forall g\in G.
\end{align*}
称$\ker \text{ad}$为$G$的\textbf{中心},记作$C(G)$或$Z(G)$.
\end{definition}
...
\item\label{theorem:共轭类,中心化子,中心的集合形式-5} $C_G(H) = \{g\in G\mid hgh^{-1}=g,\forall h\in H\} = \{g\in G\mid hg = gh,\forall h\in H\}$.

\item\label{theorem:共轭类,中心化子,中心的集合形式-4} $C(G) =\left\{ k\in G\mid kgk^{-1}=g,\forall g\in G \right\}  = \{k \in G \mid kg = gk, \forall g \in G\}$,进而$G$的中心$C(G)\lhd G$.
\end{enumerate}
\end{theorem}
\begin{proof}
\begin{enumerate}[(1)]
...
设$G$是一个群,$k,g\in G$,则有
\begin{enumerate}[(1)]
\item\label{theorem:中心化子的基本性质-0} $C_G(g)=C_G(g^{-1}),\quad C_G(kgk^{-1})=kC_G(g)k^{-1},\quad kC_g(G)k^{-1}=C_g(G)$.

\item 设 $A < G$, 则 $C_G(C_G(C_G(A))) = C_G(A)$.

\item\label{theorem:中心化子的基本性质-1} $C(G)$是$G$的正规子群且$\text{ad}G\cong G/C(G)$.

\item\label{theorem:中心化子的基本性质-2} $G$中共轭关系为等价关系,因而$G$的共轭类的集合是$G$的一个分划.即$X$是所有不同共轭类的并
\begin{align*}
X=\bigsqcup_x O_x,
\end{align*}
...
\end{theorem}
\begin{remark}
群$G$以$g$为代表的共轭类$C_g(G)$一般不构成群,因此虽然其满足$kC_g(G)k^{-1}=C_g(G)$,也不一定是$G$的正规子群.
\end{remark}
\begin{proof}
\begin{enumerate}[(1)]
\item 注意到
\begin{align*}
C_G\left( g \right) =\left\{ k\in G\mid kgk^{-1}=g \right\} =\left\{ k\in G\mid \left( kgk^{-1} \right) ^{-1}=g^{-1} \right\} =\left\{ k\in G\mid kg^{-1}k^{-1}=g^{-1} \right\} =C_G\left( g^{-1} \right) .
\end{align*}
对$\forall y\in G$,都有
\begin{align*}
y\in C_G\left( kgk^{-1} \right) &\Longleftrightarrow y\left( kgk^{-1} \right) =\left( kgk^{-1} \right) y\Longleftrightarrow \left( k^{-1}yk \right) g=g\left( k^{-1}yk \right) 
\\
&\Longleftrightarrow k^{-1}yk\in C_G\left( g \right) \Longleftrightarrow y\in kC_G\left( g \right) k^{-1}.
\end{align*}
故$C_G(kgk^{-1})=kC_G(g)k^{-1}$.

对$\forall x\in C_g(G)$，存在$y\in G$，使得$x=ygy^{-1}$。从而
\begin{align*}
x=ygy^{-1}=k(k^{-1}ygy^{-1}k)k^{-1}=k((k^{-1}y)g(k^{-1}y)^{-1})k^{-1}\in kC_g(G)k^{-1}\Longrightarrow C_g(G)\subseteq kC_g(G)k^{-1},\\
kxk^{-1}=kygy^{-1}k^{-1}=(ky)g(ky)^{-1}\in C_g(G)\Longrightarrow kC_g(G)k^{-1}\subseteq C_g(G).
\end{align*}
故$kC_g(G)k^{-1}=C_g(G)$。

\item 令 $B = C_G(C_G(A))$, 要证 $C_G(B) = C_G(A)$.利用\rrefthe{theorem:共轭类,中心化子,中心的集合形式}{theorem:共轭类,中心化子,中心的集合形式-5}.

设 $x \in C_G(A)$. 对于任一 $y \in B = C_G(C_G(A))$, 则 $y$ 与 $x$ 可换, 即 $x$ 和 $y$ 可换, 即 $x \in C_G(B)$. 故 $C_G(A) \subseteq C_G(B)$.

反之, 设 $y \in C_G(B)$, 即 $y$ 与 $B$ 中任一元可换. 因为 $C_G(A)$ 中任一元与 $A$ 中任一元可换, 故 $A$ 中任一元与 $C_G(A)$ 中的元可换,于是
\begin{align*}
...
\item 考虑$G$在$G$上的共轭作用,再由\rrefthe{theorem:抽象代数--定理4.2.2}{theorem:抽象代数--定理4.2.2-1}即得.
...
\begin{theorem}\label{theorem:正规子群都是共轭类的并}
设$G$是群，$H$是$G$的子群，则
\begin{align*}
H\lhd G\Longleftrightarrow H\text{是}G\text{中若干个共轭类的(无交)并}.
\end{align*}
\end{theorem}
\begin{remark}
这个定理表明:要确定一个群的全部正规子群，可以先确定其所有共轭类，由这个定理可知其正规子群必然是若干个共轭类的并，再去掉不是子群的共轭类的并，就得到了这个群的全部正规子群.
\end{remark}
\begin{proof}

{\heiti 必要性:} 对$\forall h\in H$，由$H\lhd G$知，对$\forall g\in G$，有$ghg^{-1}\in H$。因此$C_h(G)\subseteq H$。于是由\rrefthe{theorem:中心化子的基本性质}{theorem:中心化子的基本性质-2}知
\begin{align*}
\bigsqcup\limits_{h\in H}C_h(G)\subseteq H.
\end{align*}
显然$H\subseteq \bigsqcup\limits_{h\in H}C_h(G)$，故$H=\bigsqcup\limits_{h\in H}C_h(G)$。

{\heiti 充分性:} 由\rrefthe{theorem:中心化子的基本性质}{theorem:中心化子的基本性质-2}可设$H=\bigsqcup\limits_{i\in I}C_i$，其中$I$是指标集，$C_i$都是$G$的互不相同的共轭类。
对$\forall g\in G$，由\rrefthe{theorem:中心化子的基本性质}{theorem:中心化子的基本性质-0}可得
\begin{align*}
gHg^{-1}=g\left( \bigsqcup\limits_{i\in I}C_i \right) g^{-1}=\bigsqcup\limits_{i\in I}\left( gC_ig^{-1} \right)=H.
\end{align*}
故$H\lhd G$。
...
\begin{enumerate}[(a)]
\item 如果$p\mid |C(G)|$,则由\hyperref[theorem:Sylow 第一定理]{Sylow 第一定理}知$C(G)$含有阶为$p$的元素,从而$G$含有阶为$p$的元素.
...
\begin{enumerate}[(1)]
\item 如果$g(x)=x$,则称$x$为$g$的一个\textbf{不动元素}或\textbf{不动点}.$g$的全部不动元素的集合称为$g$的\textbf{不动元素集}或\textbf{不动点集},记作$F_g$.

\item 如果对任意的$g\in G$,都有$g(x)=x$,则称$x$为$G$的一个\textbf{不动元素}或\textbf{不动点}.$G$的全部不动元素的集合称为$G$的\textbf{不动元素集}或\textbf{不动点集},记作$F_G$.
\end{enumerate}
\end{definition}

\begin{proposition}\label{proposition:不动点子群}
设 \(G\) 和 \(H\) 均为群，且 \(G\) 作用在 \(H\) 上。若对 \(\forall g\in G\)，由\rrefthe{theorem:抽象代数--定理4.2.1}{theorem:抽象代数--定理4.2.1-1}定义的可逆变换
\[
\phi_g : H \to H,\quad \phi_g(h)=g\cdot h
\]
是 \(H\) 的一个自同构，则对\(\forall g\in G\)，$g$的不动点集
\[
F_g = \{ h\in H \mid g\cdot h = h \}
\]
是 \(H\) 的子群。更一般地，对任意子集 \(S\subseteq G\)，集合
\[
F_S := \{ h\in H \mid s\cdot h = h,\ \forall s\in S \}
\]
也是 \(H\) 的子群。
\end{proposition}
\begin{proof}
...
首先证明 \(F_g\) 是子群。
单位元：由于 \(\phi_g\) 是群同态，\(\phi_g(e_H)=e_H\)，故 \(e_H\in F_g\)。

封闭性：设 \(h_1,h_2\in F_g\)，则
\[
g\cdot(h_1h_2) = (g\cdot h_1)(g\cdot h_2) = h_1 h_2\Longrightarrow h_1h_2\in F_g.
\]

逆元：设 \(h\in F_g\)，则
\[
g\cdot h^{-1} = (g\cdot h)^{-1} = h^{-1}\Longrightarrow h^{-1}\in F_g.
\]
因此 \(F_g\leqslant H\)。

对于 \(F_S = \bigcap_{s\in S} F_s\)，由上述证明知每个 \(F_s\) 是子群，从而它们的交集也是子群.
\end{proof}
...
{\color{blue}证法一:}令$\Omega = \{ (g, x) \in G \times X \mid gx = x \}$, 我们用两种方法计算$|\Omega|$.

第一种方法: 先固定$g \in G$, 得到
\begin{align*}
|\Omega| = \sum\limits_{g \in G} |F_g|.
\end{align*}

第二种方法: 先固定$x \in X$. 由\rrefthe{theorem:抽象代数--定理4.2.2}{theorem:抽象代数--定理4.2.2-1}可设$X = \bigsqcup\limits_{1 \leqslant i \leqslant n} O_{x_i}$是$X$的按轨道的划分. 注意到若$x \in O_{x_i}$, 则由\rrefthe{theorem:抽象代数--定理4.2.2}{theorem:抽象代数--定理4.2.2-0}知$|O_x| = |O_{x_i}|$. 于是
\begin{align*}
|\Omega |=\sum_{x\in X}{|S_x|}=\sum_{x\in X}{\frac{|G|}{|O_x|}}=\sum_{i=1}^n{\sum_{x\in O_{x_i}}{\frac{|G|}{|O_x|}=\sum_{i=1}^n{\sum_{x\in O_{x_i}}{\frac{|G|}{|O_{x_i}|}}}}}=\sum_{i=1}^n{|O_{x_i}|\frac{|G|}{|O_{x_i}|}}=n|G|.
\end{align*}
故$\sum\limits_{g \in G} |F_g| = n|G|$.
...
=\sum_{i=1}^n{|G|}=n|G|.
\end{align*}
由此得$n = \frac{1}{|G|}\sum_{g\in G}|F_g|.$
...
设$H$是群$G$的子群，则
\begin{enumerate}[(1)]
\item $\forall g \in G, H_1 = gHg^{-1}$也是$G$的子群,并且$H_1\cong H$,称$H_1$为$H$的\textbf{共轭子群};
...
\item $G$在$G$的伴随作用下，$H$的迷向子群恰为$N_G(H)$，故$N_G(H)$是$G$的子群. 因$H$是$G$的子群,且显然有$H \subseteq N_G(H)$，故$H$是$N_G(H)$的子群. 又$\forall g \in N_G(H), gHg^{-1} = H$，因此$H \lhd N_G(H)$.
\end{enumerate}
...
设$H$是群$G$的子群，则
\begin{enumerate}[(1)]
\item\label{theorem:共轭子群的个数和正规化子的性质-1} $G$中与$H$共轭的子群的个数为$[G : N_G(H)]$;

\item\label{theorem:共轭子群的个数和正规化子的性质-2} $H \lhd G\iff N_G(H) = G$.
\end{enumerate}
\end{theorem}
\begin{proof}
\begin{enumerate}[(1)]
...
\begin{proposition}\label{proposition:有限群的真子群的共轭子群之并不能覆盖整个群}
有限群 $G$ 的一个真子群的全部共轭子群之并不能覆盖整个群 $G$.
\end{proposition}
\begin{remark}
\textbf{这个结论对无限群不成立.}例如, 令 $G = GL(n,\mathbb{C})$, $H$ 是 $n$ 阶上三角可逆复矩阵作成的真子群. 由Jordan标准型知任一 $n$ 阶可逆阵 $A$ 相似于其Jordan标准型,而可逆矩阵的Jordan标准型都是$H$ 中的元, 这表明 $G$ 是 $H$ 的所有共轭子群之并.
\end{remark}
\begin{proof}

设 $H$ 是 $G$ 的真子群, 则由\rrefthe{theorem:共轭子群的个数和正规化子的性质}{theorem:共轭子群的个数和正规化子的性质-1}知$H$ 共有 $[G:N_G(H)]$ 个共轭子群. 令 $\Sigma$ 是 $H$ 的所有共轭子群之并, 则(幺元单独计数)
\begin{align*}
|\Sigma| \leqslant (|H| - 1)[G:N_G(H)] + 1 = \frac{|G|}{|N_G(H)|} \cdot |H| - [G:N_G(H)] + 1.
\end{align*}
若 $H$ 是正规子群, 则由\rrefthe{theorem:共轭子群的个数和正规化子的性质}{theorem:共轭子群的个数和正规化子的性质-2}知$N_G(H) = G$, 于是
\begin{align*}
|\Sigma| \leqslant |H| - 1 + 1 = |H| < |G|.
\end{align*}
若 $H$ 不是正规子群, 则由\rrefthe{theorem:共轭子群的个数和正规化子的性质}{theorem:共轭子群的个数和正规化子的性质-2}知$N_G(H) \neq G$, 于是 $[G:N_G(H)] > 1$,
\begin{align*}
|\Sigma| \leqslant \frac{|G|}{|H|} \cdot |H| - [G:N_G(H)] + 1 < |G| - 1 + 1 = |G|.
\end{align*}
...
\begin{example}
令$G=GL(n,\mathbb{C})$, $P$是主对角线上的元均为1的$n\times n$上三角方阵全体形成的$G$的子群.确定$N_G(P),C_G(P)$和$P$的中心$Z(P)$.
\end{example}
\begin{solution}

由矩阵的乘法直接计算可知与任一$n$阶主对角线为1的上三角矩阵可换的矩阵必为上三角矩阵,且形如
\begin{align*}
J_n(\lambda,\mu)=\begin{pmatrix}
\lambda & 0 & \cdots & 0 & \mu \\
& \lambda & \cdots & 0 & 0 \\
& & \ddots & \vdots & \vdots \\
& & & \lambda & 0 \\
...
\end{pmatrix},
\end{align*}
其中$\mu\in\mathbb{C}$.要看出这一点,只要取主对角线为1且$(i,i+1)$处为1,其余全为0的上三角矩阵即可,$i=1,\cdots,n-1$.因此
\begin{align*}
...
下面求
\begin{align*}
N_G(P):&=\{A\in GL(n,\mathbb{C})\mid AP=PA\} \\
&=\{A\in GL(n,\mathbb{C})\mid AJA^{-1}\subseteq P,\ A^{-1}JA\subseteq P,\ \forall J\in P\}.
\end{align*}
首先,所有$n$阶上三角可逆矩阵均在$N_G(P)$中.我们断言:$N_G(P)$恰是所有$n$阶上三角可逆矩阵作成的群.

设$A$是$n$阶可逆矩阵且$A$不是上三角的.设$a_{ij}$是$A$中主对角线以下的非零元且$i$最大,即
\begin{align*}
a_{ij}\ne 0,i>j;\ \text{且若}a_{st}\ne 0,s>t,\text{则}i\geqslant s.
\end{align*}
令$J$是主对角线全为1且$(j,j+1)$处为1,其余全为0的矩阵.则$AJ$是将$A$的第$j$列加到第$j+1$列后得到的矩阵,从而$AJ$不是上三角的.
