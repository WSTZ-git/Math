...
\begin{enumerate}[(1)]
\item\label{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数-1} 设$f$是群$G$到群$G'$的同态，则
\begin{enumerate}[(i)]
\item\label{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数-1-1} $\ker f$是$G$的正规子群,$f(G)$是$G'$的子群.

\item\label{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数-1-2} $f$是单同态的充要条件是$\ker f = \{1\}$.
\end{enumerate}

\item\label{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数-2} 设$f$是环$R$到环$R'$的同态，则
\begin{enumerate}[(i)]
\item\label{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数-2-1} $\ker f$是$R$的理想,$f(R)$是$R'$的子环.

\item\label{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数-2-2} $f$是单同态的充要条件是$\ker f = \{0\}$.
\end{enumerate}

\item\label{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数-3} 设$R$是一个环，$M, M'$都是$R$模，$f$是$M$到$M'$的模同态,则
\begin{enumerate}[(i)]
\item\label{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数-3-1} $\ker f$是$M$的子模,$f(M)$是$M'$的子模.

\item\label{proposition:群同态的核是定义域的子群,像是陪域的子群-抽象代数-3-2} $f$是单同态的充要条件是$\ker f = \{0\}$.
\end{enumerate}
\end{enumerate}
\end{proposition}
\begin{proof}
...
{\heiti 必要性:}设$f$为单同态，则对任意的$x\in\ker f$，有$f(x)=1'=f(1)$。
因为$f$为单同态，所以$x=1$，于是$\ker f=\{1\}$。

{\heiti 充分性:}设$x,y\in G$，如果$f(x)=f(y)$，则$f(xy^{-1})=f(x)(f(y))^{-1}=1'$，从而
$xy^{-1}\in\ker f$。而$\ker f=\{1\}$，所以$xy^{-1}=1$，因此$x=y$，从而$f$为单同态。
\end{enumerate}
...
\begin{enumerate}[(1)]
\item\label{proposition:自然同态的ker等于商掉的正规子群-1} \begin{enumerate}[(i)]
\item 设$H$是群$G$的正规子群. $\pi$是$G$到商群$G/H$的自然同态,则有$\ker\pi = H$.

\item 设$H,H'$都是群$G$的正规子群,且$G/H\cong G/H'$,则$H\cong H'$.
\end{enumerate}

\item\label{proposition:自然同态的ker等于商掉的正规子群-2} \begin{enumerate}[(i)]
\item\label{proposition:自然同态的ker等于商掉的正规子群-2-1} 设$I$是环$R$的理想,$\pi$是$R$到商环$R/I$的自然同态,则有$\ker\pi = I$.

\item\label{proposition:自然同态的ker等于商掉的正规子群-2-2} 设$I,I'$都是环$R$的理想,且$R/I\cong R/I'$,则$I = I'$.
\end{enumerate}
...
\item 设$f$为$R/I$到$R/I'$的同构，$\pi$为$R$到$R/I$的自然同态，$\pi'$为$R$到$R/I'$的自然同态，则
\begin{align*}
f\cdot \pi =\pi'.
\end{align*}
注意$I,I'$分别是$R/I,R/I'$的零元。于是对$\forall x\in \ker\left( f\cdot \pi \right)$，有$f\cdot \pi \left( x \right) =f\left( \pi \left( x \right) \right) =I'$。再结合\rrrefpro{proposition:自然同态的ker等于商掉的正规子群}{proposition:自然同态的ker等于商掉的正规子群-2}{proposition:自然同态的ker等于商掉的正规子群-2-1}可得
\begin{align*}
\pi \left( x \right) \in f^{-1}\left( I' \right) =I\Longrightarrow x\in \ker\pi =I.
\end{align*}
因此$\ker\left( f\cdot \pi \right) \subseteq I$。又因为
\begin{align*}
f\cdot \pi \left( I \right) =f\left( \pi \left( I \right) \right) =f\left( I \right) =I',
\end{align*}
所以$I\subseteq \ker\left( f\cdot \pi \right)$。故$I=\ker\left( f\cdot \pi \right)$。故再结合\rrrefpro{proposition:自然同态的ker等于商掉的正规子群}{proposition:自然同态的ker等于商掉的正规子群-2}{proposition:自然同态的ker等于商掉的正规子群-2-1}可得
\begin{align*}
I=\ker\left( f\cdot \pi \right) =\ker\pi' =I'.
\end{align*}
\end{enumerate}
...
设$f$是群$G$到群$f(G)$上的满同态，$\pi$为$G$到商群$G/\ker f$上的自然同态,则有$G/\ker f$到$f(G)$上的群同构映射$\overline{f}$，使得
\begin{align}
...
\begin{enumerate}[(1)]
\item\label{theorem:抽象代数-定理1.7.2-1} $f$是$\Sigma \to \Gamma$的一一对应,即
\begin{gather*}
H_1\leqslant H\iff K\subseteq f^{-1}(H_1)\leqslant G,
\\
K\subseteq G_1\leqslant G \iff f(G_1)\leqslant H.
\end{gather*}
...
\begin{enumerate}[(1)]
\item\label{corollary:群同态第二定理推论-1} $\pi$是$\Sigma \to \Gamma$的一一对应,即
\begin{gather*}
H_1\leqslant G/N\iff N\subseteq \pi^{-1}(H_1) \leqslant G,
\\
N\subseteq G_1\leqslant G \iff \pi(G_1) = G_1/N \leqslant G/N.
\end{gather*}
...
\begin{enumerate}[(1)]
\item\label{theorem:模同态的基本定理推论-1} $f$是$\Sigma\to \Gamma$的一一对应,即
\begin{gather*}
R_1\leqslant R^{\prime}\Longleftrightarrow K\subseteq f^{-1}\left( R_1 \right) \leqslant R,
\\
K\subseteq R_2\leqslant R\Longleftrightarrow f\left( R_2 \right) \leqslant R^{\prime}.
\end{gather*}
