...
\end{cases}
\end{align*}
则称多项式序列$\{\varphi_n(x)\}_{0}^{\infty}$为在$[a,b]$上带权$\rho(x)$\textbf{正交}，称$\varphi_n(x)$为$[a,b]$上\textbf{带权$\boldsymbol{\rho }\mathbb{(}\boldsymbol{x}\mathbb{)}$的$\boldsymbol{n}$次正交多项式}．
\end{definition}
...
\begin{theorem}
设$\{\varphi_n(x)\}_{0}^{\infty}$是$[a,b]$上带权$\rho(x)$的正交多项式，则对$n \geqslant 0$成立递推关系
...
\begin{theorem}
设$\{\varphi_n(x)\}_{0}^{\infty}$是$[a,b]$上带权$\rho(x)$的正交多项式，则$\varphi_n(x)$（$n \geqslant 1$）在区间$(a,b)$内有$n$个不同的零点．
\end{theorem}
\begin{proof}
...
与$(\varphi_n,q) \neq 0$矛盾，故$l \geqslant n$．而$\varphi_n(x)$只有$n$个零点，故$l = n$，即$n$个零点都是单重的．证毕．
...
当$k \leqslant n-2$时，$xP_k(x)$次数小于等于$n-1$，上式左端积分为$0$，故得$a_0 = 0$．当$k = n$时，$xP_n^2(x)$为奇函数，左端积分仍为$0$，故$a_n = 0$．于是
...
则有
\begin{enumerate}
\item (递推关系)对$ \forall x\in [-1,1]$,有
\begin{align}\label{eq:数值分析-3-2.11}
T_n(x) = \cos(n \arccos x),\quad n=1,2,\cdots.\iff \begin{cases}
...
\item 零点直接代入验证即可;极值点同样代入其一、二阶导数验证即可.
\end{enumerate}
...
\max_{-1 \leqslant x \leqslant 1} |\widetilde{T}_n(x)| = \min_{P \in \widetilde{H}_n} \max_{-1 \leqslant x \leqslant 1} |P(x)| = \frac{1}{2^{n-1}}.
\end{align}
...
$$\max_{-1 \leqslant x \leqslant 1} |f(x) - P_2^*(x)| = \min.$$
