...
距离空间$(\mathscr{X},\rho)$中的收敛列都是基本列.即点列$\{x_n\}\subseteq \mathscr{X}$收敛的必要条件是$\{x_n\}$为基本列.
\end{proposition}
\begin{proof}
设$x_n\to x(n\to \infty)$,则对$\forall n,m\in \mathbb{N}$且$m\geqslant m$,有
\begin{align*}
...
\begin{enumerate}[(1)]
\item 若 $(X, \rho)$ 是完备度量空间，且 $M$ 是 $X$ 的闭子集，则其度量子空间$(M, \rho)$ 是完备度量子空间；
\item 若其度量子空间$(M, \rho)$ 是完备度量子空间，则 $M$ 是 $X$ 的闭子集。
\end{enumerate}
\end{proposition}
\begin{proof}
\begin{enumerate}[(1)]
...
\begin{align*}
\varphi(x)=\varphi(x)-A^{-1}f\big(x,\varphi(x)\big),\,\,\forall x\in\overline{B}(x_0,r)\iff f\big(x,\varphi(x)\big)=0,\,\,\forall x\in\overline{B}(x_0,r).
\end{align*}
