...
\end{proposition}
\begin{proof}
必要性。若\eqref{eq:1.1.4}不成立，必$\exists x_0 \in \mathscr{X}$，$\exists \varepsilon > 0$，使得$\forall n \in \mathbb{N}$，$\exists x_n$使得$\rho(x_n, x_0) < \frac{1}{n}$，但$r(Tx_n, Tx_0) \geqslant \varepsilon$，即得$\lim\limits_{n \to \infty} \rho(x_n, x_0) = 0$，但$\lim\limits_{n \to \infty} r(Tx_n, Tx_0) \neq 0$，矛盾。
