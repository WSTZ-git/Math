...
$M^\perp = \{0\}$ 当且仅当 $\overline{M} = \mathscr{X}$（即 $M$ 在 $\mathscr{X}$ 中稠密）。
\end{enumerate}
\end{proposition}
\begin{proof}
\begin{enumerate}[(1)]
...
对$\forall x\in H$，由题设和\hyperref[theorem:泛函分析--Cauchy-Schwarz不等式]{Cauchy-Schwarz不等式}有
\begin{align*}
...
\end{align*}
由\eqref{ineq:lower}式得
\begin{align*}
\|x_n-x_k\|\leqslant \frac1m\|Ax_n-Ax_k\|\to 0,
\end{align*}
故$\{x_n\}$为Cauchy列. 由$H$的完备性，存在$x\in H$使$x_n\to x$. 又$A$有界，由\refpro{proposition:线性算子有界当且仅当连续}知$A$连续,故$Ax_n\to Ax$，再由极限的唯一性知$Ax=y$，即$y\in\mathscr{R}(A)$. 所以$\mathscr{R}(A)$为闭子空间.
