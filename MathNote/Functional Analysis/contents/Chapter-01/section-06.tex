...
$M^\perp = \{0\}$ 当且仅当 $\overline{M} = H$（即 $M$ 在 $H$ 中稠密）。
\end{enumerate}
\end{proposition}
\begin{proof}
\begin{enumerate}[(1)]
...
\item 若 $\overline{M} = \mathscr{X}$，任取 $z \in M^\perp$，则对$\forall x \in \mathscr{X}$，存在序列 $\{m_n\} \subset M$ 使得 $m_n \to x$。由于 $(m_n, z)=0$，由\hyperref[proposition:内积的连续性]{内积的连续性}得 $(x, z)=0$，故 $z=0$，从而 $M^\perp = \{0\}$.

反过来，若 $\overline{M} \ne \mathscr{X}$，则 $\overline{M}$ 是 $\mathscr{X}$ 的闭真子空间。由正交补空间的定义知$\mathscr{X} = \overline{M} \oplus (\overline{M})^\perp$，且因为 $\overline{M} \ne \mathscr{X}$，有 $(\overline{M})^\perp \ne \{0\}$。取非零 $z \in (\overline{M})^\perp$，,又$M\subset \overline{M}$,故 $z \in M^\perp$，因此 $M^\perp \ne \{0\}$,矛盾!
\end{enumerate}
\end{proof}
...
\end{align*}
其中$\mathscr{L}(H)$表示$H$到$H$的有界线性算子全体。证明：$\exists A^{-1}\in \mathscr{L}(H)$.
\end{example}
\begin{proof}
...
故
\begin{align}
\|Ax\|\geqslant m\|x\|. \label{ineq:lower}
\end{align}
若$Ax=0$,则由上式得
\begin{align*}
0=\|Ax\|\geqslant m\|x\|\Longrightarrow x=0,
\end{align*}
于是$A$为单射.

先证$\mathscr{R}(A)$是闭子空间. 设$Ax_n\to y$（$n\to\infty$），则$\{Ax_n\}$为Cauchy列，即
\begin{align*}
\|Ax_n-Ax_k\|\to 0\quad(n,k\to\infty).
\end{align*}
由\eqref{ineq:lower}得
\begin{align*}
\|x_n-x_k\|\leqslant \frac1m\|Ax_n-Ax_k\|\to 0,
\end{align*}
故$\{x_n\}$为Cauchy列. 由$H$的完备性，存在$x\in H$使$x_n\to x$. 又$A$有界，由\refpro{proposition:线性算子有界当且仅当连续}知$A$连续,故$Ax_n\to Ax$，因此$Ax=y$，即$y\in\mathscr{R}(A)$. 所以$\mathscr{R}(A)$为闭子空间.

再证$\mathscr{R}(A)^\perp=\{0\}$. 任取$z\in\mathscr{R}(A)^\perp$，则对$\forall x\in H$，有$(Ax,z)=0$. 取$x=z$，得$(Az,z)=0$. 由题设知
\begin{align*}
0=|(Az,z)|\geqslant m\|z\|^2\Longrightarrow \|z\|=0\Longrightarrow z=0.
\end{align*}
因此$\mathscr{R}(A)^\perp=\{0\}$.

由\rrefpro{proposition:Hilbert空间稠密线性空间正交补为0}{proposition:Hilbert空间稠密线性空间正交补为0-2}知$\overline{\mathscr{R}(A)}=H$. 又因$\mathscr{R}(A)$为闭子空间，故$\mathscr{R}(A)=H$. 所以$A$是满射，故$A$为双射，从而$A^{-1}$存在.

最后，对$\forall x\in H$,令$y=Ax$，则由\eqref{ineq:lower}式可得
$$
\|A^{-1}y\|=\|x\|\leqslant \frac1m\|y\|,
$$
故$A^{-1}$有界，即$A^{-1}\in\mathscr{L}(H)$.
\end{proof}
...
\end{align*}
由\rrefpro{proposition:正交的基本性质}{proposition:正交的基本性质-3}知$x-\sum_{n=1}^{\infty}{(x,e_n)e_n}\bot \sum_{n=1}^{\infty}{(x,e_n)e_n}$.
...
当$S$是$\mathscr{X}$的正交规范基时,若$S$的元素个数$N < \infty$，则$\mathscr{X}$同构于$\mathbb{K}^N$;
