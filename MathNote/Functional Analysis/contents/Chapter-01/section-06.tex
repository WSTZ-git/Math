...
\begin{proposition}[平行四边形等式$\,$或$\,$极化恒等式]\label{proposition:平行四边形等式--泛函分析}
...
\begin{enumerate}[(1)]
\item\label{proposition:正交的基本性质-1} 若 $x\perp y_i\ (i=1,2)$, 则 $x\perp\lambda_1y_1+\lambda_2y_2\ (\forall\lambda_1,\lambda_2\in\mathbb{K})$.
...
\item\label{proposition:正交的基本性质-4} 若 $x\perp M$, 则 $x\perp\text{span}M$.

\item\label{proposition:正交的基本性质-5} $M^\perp$ 是 $\mathscr{X}$ 的一个闭线性子空间,且$M\cap M^{\bot}=\{\theta\}$.
\end{enumerate}
\end{proposition}
\begin{proof}
