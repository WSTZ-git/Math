...
\begin{definition}
如果 $\|\cdot\|$ 是 $\mathscr{X}$ 上的一个准范数,就称 $(\mathscr{X},\|\cdot\|)$ 为\textbf{赋准范数的线性空间},或称$\bm{F}^*$\textbf{空间}。此时,称$\mathscr{X}$中的点列$\{x_n\}$收敛到$x\in \mathscr{X}$,即
\begin{align*}
x_n\to x\ (n\to\infty)\iff \|x_n-x\|\to0\quad(n\to\infty).
\end{align*}
完备的$F^*$空间称为$\mathbf{Frechet}$\textbf{空间}，简称$\bm{F}$\textbf{空间}。

如果 $\|\cdot\|$ 是 $\mathscr{X}$ 上的一个范数,就称 $(\mathscr{X},\|\cdot\|)$ 为\textbf{赋范线性空间}，或称$\bm{B}^*$\textbf{空间}。此时,称$\mathscr{X}$中的点列$\{x_n\}$收敛到$x\in \mathscr{X}$,即
\begin{align*}
x_n\to x\ (n\to\infty)\iff \|x_n-x\|\to0\quad(n\to\infty).
\end{align*}
...
\begin{proposition}\label{proposition:常见$F$空间}
\begin{enumerate}[(1)]
\item 设$(M,\rho)$是一个紧的度量空间，用$C(M)$表示$M\to\mathbb{R}$的一切连续映射全体.则
\begin{align*}
\|u\|=\max_{x\in M}|u(x)|
\end{align*}
是一个范数，并且$(C(M),\|\cdot\|)$是一个$B$空间。
...
\begin{proposition}\label{proposition:常见$B$空间11111}
...
\item $\Omega=\mathbb{N}$，而测度$\mu$是等分布的：$\mu(\{n\})=1(\forall n\in\mathbb{N})$，这时空间$L^p(\Omega,\mu)$由满足$\sum_{n=1}^\infty|u_n|^p<\infty$的序列$u=\{u_n\}_{n=1}^\infty$组成，对应的空间记作$\boldsymbol{l}^{\boldsymbol{p}}$，其范数是
\begin{align*}
...
\begin{proposition}\label{examples:常见的$B$空间222222}
...
\end{align}
则上述定义的$\|\cdot\|$是一个范数，从而$C^k(\overline{\Omega})$是一个赋范线性空间,并且它还是Banach空间。
\end{proposition}
\begin{proof}
$\|\cdot\|$显然是一个范数.事实上，若$\{u_n\}$是一个基本列，则必存在连续函数$v_\alpha$，使得
$$\partial^\alpha u_n(x) \rightrightarrows v_\alpha(x) \quad (n \to \infty, |\alpha| \leqslant k, \text{对 } x \in \overline{\Omega} \text{ 一致}).$$
先证
\begin{align}
...
\end{align}
则$\|\cdot\|_{m,p}$是范数，但$C^m(\overline{\Omega})$依$\|\cdot\|_{m,p}$不是完备的。
将$C^m(\Omega)$的子集
$$S \triangleq \{u \in C^m(\overline{\Omega}) \mid \|u\|_{m,p} < \infty\}$$
按照范数完备化，得到的完备化空间称为$\mathbf{Sobolev}$\textbf{空间}，记作$H^{m,p}(\Omega)$。特别当$p=2$时，$H^{m,2}(\Omega)$简单地记成$H^m(\Omega)$。
\end{proposition}
...
\begin{proposition}
设在线性空间$\mathscr{X}$上给定了两个范数$\|\cdot\|_1$与$\|\cdot\|_2$，则$\|\cdot\|_2$比$\|\cdot\|_1$强，当且仅当存在常数$C>0$，使得
\begin{align*}
\|x\|_1 \leqslant C\|x\|_2 \quad (\forall x \in \mathscr{X}).
\end{align*}
\end{proposition}
\begin{proof}
充分性是显然的，下证必要性。用反证法。若上述式子不成立，则对$\forall n \in \mathbb{N}, \exists x_n \in \mathscr{X}$，使得$\|x_n\|_1 > n\|x_n\|_2$。令$y_n \triangleq x_n/\|x_n\|_1$。一方面$\|y_n\|_1=1$，另一方面因为
...
\begin{theorem}
设$\mathscr{X}$是一个有限维线性空间，若$\|\cdot\|_1$与$\|\cdot\|_2$都是$\mathscr{X}$上的范数，则必有正常数$C_1$与$C_2$，使得
$$C_1\|x\|_1 \leqslant \|x\|_2 \leqslant C_2\|x\|_1 \quad (\forall x \in \mathscr{X}).$$
...
\end{remark}
\begin{proof}
设$\dim \mathscr{X}=n$，取一组基$e_1,e_2,\cdots,e_n$，则任意$x \in \mathscr{X}$可唯一表示为
\begin{align}
x = \xi_1 e_1 + \xi_2 e_2 + \cdots + \xi_n e_n.
\end{align}
定义$p(\xi) \triangleq \left\| \sum\limits_{j=1}^n \xi_j e_j \right\| (\forall \xi \in \mathbb{K}^n)$，则$p$是$\mathbb{K}^n$上的连续函数，且在单位球面$S_1 = \{\xi \in \mathbb{K}^n \mid |\xi|=1\}$上有最小值$C_1>0$与最大值$C_2$，故
...
其中$Tx=(\xi_1,\cdots,\xi_n)$。因此$\|\cdot\|$与$\mathbb{K}^n$上的范数等价，故$\mathscr{X}$上任意两个范数都等价。
...
\item 设$P: \mathscr{X} \to \mathbb{R}$是线性空间$\mathscr{X}$上的一个函数，若它满足
\begin{enumerate}[(1)]
\item 正定性:$P(x) \geqslant 0 (\forall x \in \mathscr{X})$;
...
\end{align*}
记$M = \text{span}\{e_1,e_2,\cdots,e_n\}, \rho(x,M) = \inf\limits_{y \in M} \|x-y\|$，$x_0 = \sum\limits_{i=1}^n \lambda_i e_i$，则上述式子可改写为
$$\rho(x,x_0) = \rho(x,M).$$
称$x_0$为$x$在$M$上的\textbf{最佳逼近元}。
\end{theorem}
\begin{proof}
设$F(a) = \left\| x - \sum\limits_{i=1}^n a_i e_i \right\| \quad (a \in \mathbb{K}^n)$，则$F$是$\mathbb{K}^n$上的连续函数，且
$$F(a) \geqslant \left\| \sum\limits_{i=1}^n a_i e_i \right\| - \|x\| \quad (\forall a \in \mathbb{K}^n).$$
令$P(a) = \left\| \sum\limits_{i=1}^n a_i e_i \right\|$，则$P$是$\mathbb{K}^n$上的范数，故存在$c_1>0$，使得$P(a) \geqslant c_1 |a|$，因此$F(a) \to \infty$（当$|a| \to \infty$），故$F$有最小值存在。
...
\end{theorem}
\begin{proof}
若存在两个不同的最佳逼近元$y,z \in M$，使得$\|x-y\| = \|x-z\| = d = \rho(x,M)$，则对$\forall \alpha,\beta>0, \alpha+\beta=1$，由严格凸性有
...
\begin{proof}
Minkowski不等式$\|u+v\| \leqslant \|u\| + \|v\|$等号成立的充要条件是：$\exists k_1,k_2 \geqslant 0 \ (k_1+k_2>0)$，使得$k_1 u = k_2 v$（a.e.）。因此当$\|u\|=\|v\|=1, u \neq v$时，
...
\begin{proof}
以$C[0,1]$为例，取$x(t) \equiv 1, y(t)=t$，则$\|x\|=\|y\|=1$，但$\left\| \frac{1}{2}(x+y) \right\| = 1$；以$L^1[0,1]$为例，取$x(t) \equiv 1, y(t)=2t$，则$\|x\|=\|y\|=1$，但$\left\| \frac{1}{2}(x+y) \right\| = 1$，故二者均不严格凸。

\end{proof}


\subsection{有穷为$B^*$空间的刻画}

\begin{theorem}
$B^*$空间$\mathscr{X}$是有限维的当且仅当$\mathscr{X}$的单位球面$S_1 = \{x \in \mathscr{X} \mid \|x\|=1\}$是列紧的。
\end{theorem}
\begin{proof}
{\heiti 必要性：}有限维空间的单位球面是列紧的。

{\heiti 充分性：}若$\mathscr{X}$是无穷维的，则可在$S_1$中选取序列$\{x_n\}$，使得$\|x_n - x_m\| \geqslant 1 \ (n \neq m)$，故$S_1$不列紧，矛盾!因此$\mathscr{X}$必为有限维。
...
\begin{corollary}
$B^*$ 空间 $\mathscr{X}$ 是有穷维的,当且仅当其任意有界集是列紧的.
\end{corollary}
...
\end{align*}
其中 $x' = x_0 + \|y_0-x_0\|x \in \mathscr{X}_0$. 于是对于 $\forall 0<\varepsilon<1$, 只要 $\eta = \frac{d\varepsilon}{1-\varepsilon}$, 便得到 $\|y-x\| > 1-\varepsilon$.
...
\begin{align*}
\|z\|_1 = |x| + |y|; \quad \|z\|_2 = \sqrt{x^2 + y^2}; \\
...
\begin{align*}
\|f\|_1 = \left( \int_{0}^{1} |f(x)|^2 \mathrm{d}x \right)^{\frac{1}{2}}, \\
