...
\begin{align*}
\|u\|=\max_{x\in M}|u(x)|,\quad \forall u\in M.
\end{align*}
...
设$\mathscr{X}$是一个有限维线性空间，$\{e_1,\dots,e_n\}$ 是 $\mathscr{X}$ 的一组基,则可定义线性同构 $T:\mathscr{X}\to\mathbb{K}^n$ 如下：
\[
T x = (\xi_1,\dots,\xi_n),\quad \text{其中 } x = \sum_{j=1}^n \xi_j e_j.
\]
...
\end{remark}
\begin{proof}
设 $\{e_1,\dots,e_n\}$ 是 $\mathscr{X}$ 的一组基,$\|\cdot \|$是$\mathscr{X}$上的一个范数。由线性代数知识,定义线性同构 $T:\mathscr{X}\to\mathbb{K}^n$ 如下：
\[
T x = (\xi_1,\dots,\xi_n),\quad \text{其中 } x = \sum_{j=1}^n \xi_j e_j.
\]
取$\mathbb{K}^n$中的范数为欧几里得范数$|\cdot|$,即
...
\begin{corollary}
\begin{enumerate}[(1)]
\item 有限维$B^*$空间必是$B$空间。

\item $B^*$空间上的任意有限维子空间必是闭子空间.
\end{enumerate}
\end{corollary}
\begin{proof}
设$X$是$n$维$B^*$空间,其范数为$\|\cdot \|$.
\begin{enumerate}[(1)]
...
{\heiti 充分性：}若$\mathscr{X}$是无穷维的,则任取$S_1$上有限个线性无关向量$\{x_1,\cdots,x_n\}$。从而
\begin{align*}
...
由\ref{theorem:有限维B空间的充要条件11}知$S_1$是列紧的，故$\{y_n\}$存在收敛子列$\{y_{n_k}\}$，进而对应的$\{x_n\}$的子列$\{x_{n_k}\}$也是收敛的。
