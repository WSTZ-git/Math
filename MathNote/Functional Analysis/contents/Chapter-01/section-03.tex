...
现在定义最终的下标序列
\[
m_k \triangleq m_k^{(k)} \qquad (k=1,2,\dots).
\]
...
进而由\hyperref[theorem:泛函分析--Hausdorff定理]{Hausdorff定理}知
\begin{enumerate}[(1)]
\item 若$M\subseteq\mathscr{X}$是紧的,则$M$是完全有界的.

\item 若$(\mathscr{X},\rho)$是一个完备度量空间,则$M\subseteq\mathscr{X}$是紧的当且仅当$M$是完全有界的.
\end{enumerate}
\end{theorem}
\begin{proof}
{\heiti 必要性:}设$M$是紧集。
