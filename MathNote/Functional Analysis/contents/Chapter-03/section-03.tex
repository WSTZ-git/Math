...
\begin{definition}
设$\mathscr{X}$为Banach空间，$M\subseteq\mathscr{X}$，若算子$A\in\mathscr{L}(\mathscr{X})$满足$A(M)\subseteq M$，则称$M$为$A$的\textbf{不变子空间}.
\end{definition}
\begin{proposition}
设$\mathscr{X}$为Banach空间，$A\in\mathscr{L}(\mathscr{X})$，则
\begin{enumerate}[(1)]
...
\item 对任意$y\in\mathscr{X}$，记$L_y\triangleq\{P(A)y\mid P为任意多项式\}$，则$L_y$是$A$的不变子空间.
\end{enumerate}
\end{proposition}
\begin{theorem}
若$\dim\mathscr{X}\geqslant2$，则对任意$A\in\mathfrak{C}(\mathscr{X})$，$A$必存在非平凡的闭不变子空间.
\end{theorem}
\begin{proof}
不妨设$\dim\mathscr{X}=\infty$，$A\neq0$且$\sigma_p(A)\setminus\{0\}=\emptyset$，由前述定理得$\sigma(A)=\{0\}$.
...
\begin{proposition}
设 $\mathscr{X}$ 为一线性空间（或 Banach 空间），$T \in \mathscr{L}(\mathscr{X})$ 表示 $\mathscr{X}$ 上的线性算子。记 $T^0 = I$ 为单位算子，$N(T^k)$ 和 $R(T^k)$ 分别为 $T^k$ 的零空间和像空间。显然有如下包含链：
\[
...
\]
\begin{enumerate}[(1)]
\item 若存在非负整数 $k$ 使得 $N(T^k) = N(T^{k+1})$，则对任意 $n \ge k$ 都有 $N(T^n) = N(T^k)$。

\item 若存在非负整数 $k$ 使得 $R(T^k) = R(T^{k+1})$，则对任意 $n \ge k$ 都有 $R(T^n) = R(T^k)$。
\end{enumerate}
\end{proposition}
\begin{proof}
\begin{enumerate}[(1)]
\item 由包含链易知 $N(T^k) \subseteq N(T^n)$ 对任意 $n \ge k$ 成立。下证反向包含。
对 $n$ 进行归纳。当 $n=k$ 时结论平凡。假设对某个 $m \ge k$ 有 $N(T^m) = N(T^k)$，考虑 $x \in N(T^{m+1})$，则 $T^m(Tx) = \theta$，即 $Tx \in N(T^m) = N(T^k)$，从而 $T^{k+1}x = T^k(Tx) = \theta$，故 $x \in N(T^{k+1}) = N(T^k)$。因此 $N(T^{m+1}) \subseteq N(T^k)$，结合包含关系得 $N(T^{m+1}) = N(T^k)$。由归纳法，对所有 $n \ge k$ 结论成立。

\item 由包含链易知 $R(T^n) \subseteq R(T^k)$ 对任意 $n \ge k$ 成立。下证反向包含。
对 $n$ 进行归纳。当 $n=k$ 时结论平凡。假设对某个 $m \ge k$ 有 $R(T^m) = R(T^k)$，取 $x \in R(T^{m+1})$，则存在 $y \in \mathscr{X}$ 使得 $x = T^{m+1}y = T(T^m y)$。由于 $T^m y \in R(T^m) = R(T^k)$，存在 $z \in \mathscr{X}$ 使得 $T^m y = T^k z$，于是 $x = T(T^k z) = T^{k+1}z \in R(T^{k+1}) = R(T^k)$。因此 $R(T^{m+1}) \subseteq R(T^k)$，结合包含关系得 $R(T^{m+1}) = R(T^k)$。由归纳法，对所有 $n \ge k$ 结论成立。
\end{enumerate}
\end{proof}

\begin{definition}[零链长与像链长]
对于算子 $T \in \mathscr{L}(\mathscr{X})$，
\begin{enumerate}[(1)]
...
\]
更一般地，$p(T) \le k$ 当且仅当 $N(T^k) = N(T^{k+1})$，$q(T) \le k$ 当且仅当 $R(T^k) = R(T^{k+1})$。
\end{remark}
