...
\begin{proof}
\begin{enumerate}[(1)]
\item 对任意$g\in L^q[0,1]$，由\hyperref[Real Analysis-theorem:实变函数--Hölder不等式]{Hölder不等式}
\begin{align*}
...
\item 设$\{E_n\}\subseteq[0,1]$满足$E_1\supseteq E_2\supseteq\cdots\supseteq E_n\supseteq\cdots$且$\bigcap\limits_{n=1}^{\infty}E_n=\varnothing $，则
\begin{align*}
...
由\hyperref[theorem:泛函分析--Radon-Nikodym定理]{Radon-Nikodym定理}，存在可测函数$g$，使得对任意可测集$E$，有$\nu(E)=\int_{E}g\mathrm{d}\mu$，因此
\begin{align*}
...
\end{align*}
且由\hyperref[Real Analysis-theorem:实变函数--Hölder不等式]{Hölder不等式}
\begin{align*}
...
\end{align*}
整理得$\left(\int_{E_t}|g|^q\mathrm{d}\mu\right)^{\frac{1}{q}}\leqslant\|F\|$，令$t\to\infty$即得\eqref{eq:gnormbound}式。

\item $p=1$，此时$q=\infty$。对任意$\varepsilon>0$，令$A\triangleq\{x\in[0,1]\mid|g(x)|>\|F\|+\varepsilon\}$，仍记$E_t=\{x\in[0,1]\mid|g(x)|\leqslant t\}$，令$f=\chi_{E_t\cap A}\cdot \mathrm{sign}\,g$，则$\|f\|_{L^1[0,1]}=\mu(E_t\cap A)$，且
\begin{align*}
...
\item 定义映射$T:l^q\to (l^p)^*$如下：
\begin{align*}
T(y)=f_y,\,\text{其中}y=\{y_n\}\in l^q,\text{并且}f_y(x)=\sum\limits_{n=1}^\infty x_n y_n,\,\,x=\{x_n\}\in l^p.
\end{align*}
由\hyperref[Real Analysis-theorem:实变函数--Hölder不等式]{Hölder不等式}知
\begin{align*}
|f_y(x)|\leqslant\|x\|_p\|y\|_q.
\end{align*}
故$f_y\in (l^p)^*$且$\|f_y\|\leqslant\|y\|_q$。
下证反向不等式.
\begin{enumerate}[(i)]
\item 若$p>1$，取$x^{(y)}=\{x_n\}$，其中$x_n=\operatorname{sgn}(y_n)|y_n|^{q-1}$.
则
\begin{align*}
\|x^{(y)}\|_p^p=\sum\limits_{n=1}^\infty |y_n|^{(q-1)p}=\sum\limits_{n=1}^\infty |y_n|^q=\|y\|_q^q,\quad
f_y(x^{(y)})=\sum\limits_{n=1}^\infty |y_n|^q=\|y\|_q^q.
\end{align*}
因此$\|f_y\|\geqslant\frac{|f_y(x^{(y)})|}{\|x^{(y)}\|_p}=\|y\|_q^{\frac{q-q}{p}}=\|y\|_q$。

\item 若$p=1$，则$q=\infty$。对任意$\varepsilon>0$，存在$n_0$使得$|y_{n_0}|>\|y\|_\infty-\varepsilon$。
取$x^{(\varepsilon)}$仅在坐标$n_0$处为$\operatorname{sgn}(y_{n_0})$，其余为$0$，则$\|x^{(\varepsilon)}\|_1=1$，
$f_y(x^{(\varepsilon)})=|y_{n_0}|>\|y\|_\infty-\varepsilon$，从而$\|f_y\|\geqslant\|y\|_\infty-\varepsilon$。
由$\varepsilon$的任意性得$\|f_y\|\geqslant\|y\|_\infty$。
\end{enumerate}
综上可得$\|f_y\|=\|y\|_q$,因而$T$是$l^q$到$(l^p)^*$的等距同构。

再证$T$是满射。任取$f\in (l^p)^*$，则$\|f\|<\infty$.令$y=\{y_n\}$，其中$y_n=f(e_n)$,$e_n$为第$n$个标准单位向量。
\begin{enumerate}[(i)]
\item 当$p>1$时，对每个$N\in\mathbb{N}$，构造
\begin{align*}
x^{(N)}=\{x_n^{(N)}\}\in l^p,\,\,\text{其中}
x_{n}^{(N)}=\begin{cases}
\sgn(y_n)|y_n|^{q-1},&n\leqslant N,\\
0,&n>N.\\
\end{cases}
\end{align*}
则
\begin{align*}
\|x^{(N)}\|_p=\bigl(\sum\limits_{n=1}^N |y_n|^{(q-1)p}\bigr)^{\frac{1}{p}}=\bigl(\sum\limits_{n=1}^N |y_n|^q\bigr)^{\frac{1}{p}},\quad
f(x^{(N)})=\sum\limits_{n=1}^N |y_n|^q.
\end{align*}
由\cref{proposition:有界线性算子的基本不等式}知$|f(x^{(N)})|\leqslant\|f\|\,\|x^{(N)}\|_p$,进而
\begin{align*}
\sum\limits_{n=1}^N |y_n|^q\leqslant\|f\|\bigl(\sum\limits_{n=1}^N |y_n|^q\bigr)^{\frac{1}{p}}\Longrightarrow \|y\|_q=\bigl(\sum\limits_{n=1}^N |y_n|^q\bigr)^{\frac{1}{q}}\leqslant\|f\|<\infty.
\end{align*}
令$N\to\infty$，即得$y\in l^q$.

\item 当$p=1$时，$q=\infty$。由$|y_n|=|f(e_n)|\leqslant\|f\|\,(\forall n\in \mathbb{N})$知$y\in l^\infty$且$\|y\|_\infty\leqslant\|f\|$。
\end{enumerate}
最后，对任意$x=\{x_n\}\in l^p$，显然部分和$\sum_{k=1}^n x_k e_k$按$l^p$范数收敛到$x$。
由$f$的连续性得
\begin{align*}
f(x)=\lim_{n\to\infty}\sum_{k=1}^n x_k f(e_k)=\sum_{n=1}^\infty x_n y_n=f_y(x).
\end{align*}
因此$f=f_y$，从而$T$为满射。
故$T$是$l^q$到$(l^p)^*$上的等距线性同构，即$(l^p)^*=l^q$。
\end{enumerate}
\end{proof}
...
\end{align}
映射 $T : \mathscr{X} \to \mathscr{X}^{**}$，$x\mapsto X$是$B^*$空间$\mathscr{X}$到它的第二共轭空间$\mathscr{X}^{**}$的一个子空间等距同构,称映射$T$为 $\mathscr{X}$ \textbf{到第二共轭空间} $\mathscr{X}^{**}$ 的\textbf{自然映射}或\textbf{自然嵌入}.并且
\begin{align*}
...
由\refcor{corollary:泛函分析--推论2.4.6}，$\exists f\in\mathscr{X}^*$，使得$\|f\|=1$，且$\langle f,x\rangle=\|x\|$，于是
\begin{align}
\|x\|=X(f)\leqslant\|X\|\cdot\|f\|=\|X\|. \label{eq:::HIOUR32s2.5.24}
\end{align}
结合\eqref{eq::--hruhiow3jio2}式与\eqref{eq:::HIOUR32s2.5.24}式，$T$是等距的.
...
\begin{proposition}
设 \(H\) 是一个Hilbert空间，则 \(H\) 是自反的，并且存在从 \(H\) 到它的第二共轭空间 \(H^{**}\) 的一个等距同构,即$H\cong H^{**}.$
\end{proposition}
\begin{proof}

定义自然嵌入映射 \(J: H \to H^{**}\) 如下：对于任意 \(x \in H\)，令 \(J(x) \in H^{**}\) 满足
\begin{align}
...
\begin{theorem}
设$\mathscr{X},\mathscr{Y}$是$B^*$空间，$T\in\mathscr{L}(\mathscr{X},\mathscr{Y})$，则$T^{**}\in\mathscr{L}(\mathscr{X}^{**},\mathscr{Y}^{**})$是$T$在$\mathscr{X}^{**}$上的延拓，且满足$\|T^{**}\|=\|T\|$.
\end{theorem}
\begin{proof}
...
\end{align*}
故 \(\|x_n\| \to \|x\|\)。
再证弱收敛。由\hyperref[theorem:Riesz表示定理(Hilbert空间)]{Riesz表示定理}可知，弱收敛等价于对任意固定的 \(y \in H\)，有 \((x_n, y) \to (x, y)\)。因此，对任意固定的 \(y \in H\)，由Cauchy-Schwarz不等式，有
\begin{align}
...
由\hyperref[theorem:Riesz表示定理(Hilbert空间)]{Riesz表示定理}可知，弱收敛等价于对任意固定的 \(y \in H\)，有 \((x_n, y) \to (x, y)\)。特别地，取 \(y = x\)，则有
\begin{align}
...
\begin{definition}
设$A$是$B^*$空间$\mathscr{X}$的子集,称集$A$是\textbf{弱列紧的}，是指$A$中的任意点列有一个弱收敛子列。又称$A$是*\textbf{弱列紧的}，是指$A$中的任意点列有一个*弱收敛的子列。
\end{definition}
