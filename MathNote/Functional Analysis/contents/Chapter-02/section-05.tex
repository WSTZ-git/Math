...
\end{align*}
构成一个$B$空间，称为$\mathscr{X}$的\textbf{共轭空间}。
\end{theorem}
\begin{proof}
$\mathscr{X}^*$的完备性可由$\mathscr{X}^*=\mathscr{L}(\mathscr{X},\mathbb{K})$直接导出。
\end{proof}

\begin{example}
设$1\leqslant p<\infty$，$q$是$p$的共轭数，即
\begin{align*}
\begin{cases}
...
则
\begin{align}
L^p[0,1]^*=L^q[0,1].\label{eq:lpconj}
\end{align}
\end{example}
\begin{remark}
该结论可推广至一般$\sigma$-有限的完全可加测度空间$(\Omega,\mathscr{B},\mu)$，即
\begin{align}
L^p(\Omega,\mathscr{B},\mu)^*=L^q(\Omega,\mathscr{B},\mu)\quad(1\leqslant p<\infty).\label{eq:lpgenconj}
\end{align}
\end{remark}
\begin{proof}
对任意$g\in L^q[0,1]$，由Hölder不等式
\begin{align*}
\left|\int_{0}^{1}f(x)g(x)\mathrm{d}\mu\right|\leqslant\left(\int_{0}^{1}|f(x)|^p\mathrm{d}\mu\right)^{\frac{1}{p}}\left(\int_{0}^{1}|g(x)|^q\mathrm{d}\mu\right)^{\frac{1}{q}},
\end{align*}
其中$\mu$是$[0,1]$上的Lebesgue测度。定义映射
\begin{align}
F_g(f)\triangleq\int_{0}^{1}f(x)g(x)\mathrm{d}\mu\quad(\forall f\in L^p[0,1]),\label{eq:fgmap}
\end{align}
则$F_g$是$L^p[0,1]$上的连续线性泛函，且
\begin{align}
\|F_g\|_{L^p[0,1]^*}\leqslant\|g\|_{L^q[0,1]}.\label{eq:fnormbound}
\end{align}
即映射$g\mapsto F_g$将$L^q[0,1]$连续嵌入$L^p[0,1]^*$。

下证该映射为等距满射，即对任意$F\in L^p[0,1]^*$，存在$g\in L^q[0,1]$，使得
\begin{align}
F(f)=\int_{0}^{1}f(x)g(x)\mathrm{d}\mu\quad(\forall f\in L^p[0,1]),\label{eq:frep}
\end{align}
且
\begin{align}
\|g\|_{L^q[0,1]}=\|F\|.\label{eq:gnormeqf}
\end{align}

对任意可测集$E\subseteq[0,1]$，令$\nu(E)\triangleq F(\chi_E)$，其中$\chi_E$为$E$的特征函数
\begin{align*}
\chi_E(x)=
\begin{cases}
1,& x\in E,\\
...
可验证$\nu$为完全可加测度：
\begin{enumerate}[(1)]
\item 有限可加性由$F$的可加性直接可得；
\item 设$\{E_n\}\subseteq[0,1]$满足$E_1\supseteq E_2\supseteq\cdots\supseteq E_n\supseteq\cdots$且$\bigcap\limits_{n=1}^{\infty}E_n=\emptyset$，则
\begin{align*}
\nu(E_n)=F(\chi_{E_n})&\leqslant\|F\|\cdot\|\chi_{E_n}\|_{L^p[0,1]}\\
&=\|F\|\left(\int_{0}^{1}|\chi_{E_n}|^p\mathrm{d}\mu\right)^{\frac{1}{p}}\\
&=\|F\|\mu(E_n)^{\frac{1}{p}}\to0\quad(n\to\infty).
\end{align*}
\end{enumerate}
同时$\nu$关于$\mu$绝对连续，即$\mu(E)=0\implies\nu(E)=0$。

由Radon-Nikodym定理，存在可测函数$g$，使得对任意可测集$E$，有$\nu(E)=\int_{E}g\mathrm{d}\mu$，因此
\begin{align*}
F(\chi_E)=\int_{0}^{1}\chi_E(x)g(x)\mathrm{d}\mu.
\end{align*}
进而对所有简单函数$f$，成立
\begin{align*}
F(f)=\int_{0}^{1}f(x)g(x)\mathrm{d}\mu.
\end{align*}

接下来证明
\begin{align}
\|g\|_{L^q[0,1]}\leqslant\|F\|.\label{eq:gnormbound}
\end{align}
简单函数在$L^p[0,1]$中稠密，故对任意$f\in L^p[0,1]$，存在简单函数列$f_n\to f\ (L^p[0,1])$，于是
\begin{align*}
F(f)=\lim\limits_{n\to\infty}F(f_n),
\end{align*}
且由Hölder不等式
\begin{align*}
\left|\int_{0}^{1}[f(x)-f_n(x)]g(x)\mathrm{d}\mu\right|&\leqslant\left(\int_{0}^{1}|f(x)-f_n(x)|^p\mathrm{d}\mu\right)^{\frac{1}{p}}\left(\int_{0}^{1}|g(x)|^q\mathrm{d}\mu\right)^{\frac{1}{q}}\\
&\leqslant\|F\|\cdot\|f-f_n\|_{L^p[0,1]}\to0\quad(n\to\infty),
\end{align*}
因此$\eqref{eq:frep}$成立。

分两种情形证明$\eqref{eq:gnormbound}$：
\begin{enumerate}[(1)]
\item $1<p<\infty$。对任意$t>0$，记$E_t\triangleq\{x\in[0,1]\mid|g(x)|\leqslant t\}$。令$f=\chi_{E_t}|g|^{q-2}g$，则
\begin{align*}
\int_{E_t}|g|^q\mathrm{d}\mu&=\int_{0}^{1}f\cdot g\mathrm{d}\mu=F(f)\\
&\leqslant\|F\|\cdot\|f\|_{L^p[0,1]}=\|F\|\left(\int_{E_t}|g|^q\mathrm{d}\mu\right)^{\frac{1}{p}},
\end{align*}
整理得$\left(\int_{E_t}|g|^q\mathrm{d}\mu\right)^{\frac{1}{q}}\leqslant\|F\|$，令$t\to\infty$即得$\eqref{eq:gnormbound}$。

\item $p=1$，此时$q=\infty$。对任意$\varepsilon>0$，令$A\triangleq\{x\in[0,1]\mid|g(x)|>\|F\|+\varepsilon\}$，仍记$E_t=\{x\in[0,1]\mid|g(x)|\leqslant t\}$，令$f=\chi_{E_t\cap A}\mathrm{sign}\,g$，则$\|f\|_{L^1[0,1]}=\mu(E_t\cap A)$，且
\begin{align*}
\mu(E_t\cap A)(\|F\|+\varepsilon)&\leqslant\int_{A\cap E_t}|g|\mathrm{d}\mu\\
&=\int_{0}^{1}f\cdot g\mathrm{d}\mu\leqslant\|F\|\mu(E_t\cap A).
\end{align*}
令$t\to\infty$得$\mu(A)(\|F\|+\varepsilon)\leqslant\|F\|\mu(A)$，故$\mu(A)=0$，因此$\|g\|_{L^\infty[0,1]}\leqslant\|F\|$。
\end{enumerate}
...
定义空间
\begin{align*}
BV[0,1]\triangleq\left\{g:[0,1]\to\mathbb{C}\,\Bigg|\,
\begin{aligned}
&g(0)=0,\\
...
反之，对任意$f\in C[0,1]^*$，由Hahn-Banach定理，将$f$保范延拓为$\widetilde{f}\in L^\infty[0,1]^*$，满足$\langle\widetilde{f},\chi_{\{0\}}\rangle=0$，$\langle\widetilde{f},\varphi\rangle=\langle f,\varphi\rangle\ (\forall\varphi\in C[0,1])$且$\|\widetilde{f}\|=\|f\|$。
...
称映射 $T : \mathscr{X} \to \mathscr{X}^{**}$，$T(x) = X$ 为 $\mathscr{X}$ \textbf{到第二共轭空间} $\mathscr{X}^{**}$ 的\textbf{自然映射}.
\end{definition}

\begin{theorem}
设 $\mathscr{X}$ 是 $B^*$ 空间，其共轭空间 $\mathscr{X}^*$ 是 $B$ 空间，$\mathscr{X}^{**}$为$\mathscr{X}$的第二共轭空间.对每个 $x \in \mathscr{X}$，定义 $X : \mathscr{X}^* \to \mathbb{K}$如下：
\[
X(f) = \langle f, x \rangle \quad (\forall f \in \mathscr{X}^*),
\]
则$X$ 是 $\mathscr{X}^*$ 上的线性泛函，且满足
\[
|X(f)| \leqslant \|f\| \cdot \|x\| \quad (\forall f \in \mathscr{X}^*),
\]
故 $X$ 连续，即 $X \in \mathscr{X}^{**}$。且
\begin{align}\label{eq::IOUHJIOW22}
X(f)\leqslant \left\| f \right\| \cdot \left\| x \right\| .
\end{align}
\end{theorem}
...
\end{theorem}
\begin{remark}
有时对$x$与$X$不加区别，简记$\mathscr{X}\subseteq\mathscr{X}^{**}$.
\end{remark}
\begin{proof}
...
由Hahn-Banach定理，$\exists f\in\mathscr{X}^*$，使得$\|f\|=1$，且$\langle f,x\rangle=\|x\|$，于是
\begin{align}
\|x\|=X(f)\leqslant\|X\|\cdot\|f\|=\|X\|. \label{eq:::HIOUR32s2.5.24}
\end{align}
结合\eqref{eq::IOUHJIOW22}式与\eqref{eq:::HIOUR32s2.5.24}式，$T$是等距的.

\end{proof}

\begin{definition}
若$\mathscr{X}$到$\mathscr{X}^{**}$的自然映射$T$是满射的，则称$\mathscr{X}$是\textbf{自反的}，记作$\mathscr{X}=\mathscr{X}^{**}$.
\end{definition}
\begin{remark}
当$1<p<\infty$时，空间$L^p(\Omega,\mathscr{B},\mu)$是自反的；当$p=1,\infty$时，空间$L^p(\Omega,\mathscr{B},\mu)$不是自反的.
\end{remark}

\subsection{共轭算子}

\begin{definition}[共轭算子]
设$\mathscr{X},\mathscr{Y}$是B*空间，算子$T\in\mathscr{L}(\mathscr{X},\mathscr{Y})$。算子$T^*:\mathscr{Y}^*\to\mathscr{X}^*$称为是$T$的\textbf{共轭算子}，若满足
\[
...
\begin{remark}
$\forall T\in\mathscr{L}(\mathscr{X},\mathscr{Y})$，$T^*$唯一存在，且$T^*\in\mathscr{L}(\mathscr{Y}^*,\mathscr{X}^*)$.

事实上，对$\forall f\in\mathscr{Y}^*$，令$g(x)=f(Tx)\ (\forall x\in\mathscr{X})$，则$g$线性且有界：
\[
|g(x)|\leqslant\|f\|\cdot\|T\|\cdot\|x\|\quad (\forall x\in\mathscr{X}).
\]
故$g\in\mathscr{X}^*$，对应$f\mapsto g$线性，即为$T^*$. 由定义，
\begin{align}
\|T^*f\|=\|g\|\leqslant\|T\|\cdot\|f\|\quad (\forall f\in\mathscr{Y}^*), \label{eq:::HIOUR32s2.5.25}
\end{align}
因此$T^*\in\mathscr{L}(\mathscr{Y}^*,\mathscr{X}^*)$，且
\begin{align}
\|T^*\|\leqslant\|T\|. \label{eq:::HIOUR32s2.5.26}
\end{align}
唯一性显然.
\end{remark}
...
对$\forall x\in\mathscr{X}$，若$Tx\neq\theta$，由Hahn-Banach定理的推论，$\exists f\in\mathscr{Y}^*$，使得$f(Tx)=\|Tx\|$，且$\|f\|=1$. 于是
\begin{align*}
...
\begin{theorem}
设$\mathscr{X},\mathscr{Y}$是B*空间，$T\in\mathscr{L}(\mathscr{X},\mathscr{Y})$，则$T^{**}\in\mathscr{L}(\mathscr{X}^{**},\mathscr{Y}^{**})$是$T$在$\mathscr{X}^{**}$上的延拓，且满足$\|T^{**}\|=\|T\|$.
\end{theorem}

\begin{proof}
$T^*$的共轭算子$T^{**}=(T^*)^*\in\mathscr{L}(\mathscr{X}^{**},\mathscr{Y}^{**})$. 注意$\mathscr{X}\subseteq\mathscr{X}^{**},\mathscr{Y}\subseteq\mathscr{Y}^{**}$，设自然嵌入映射分别为$U$和$V$，则
\begin{align*}
...
\end{align*}
故$T^{**}Ux=VTx$，即$T^{**}$是$T$在$\mathscr{X}^{**}$上的扩张.
...
而
\begin{align*}
\int_{-\infty}^{\infty}\left|\int_{-\infty}^{\infty}K(x-y)f(y)\mathrm{d}y\right|^p\mathrm{d}x&\leqslant\|K\|_1^{\frac{p}{q}}\int_{-\infty}^{\infty}\int_{-\infty}^{\infty}|K(x-y)|\cdot|f(y)|^p\mathrm{d}y\mathrm{d}x\\
...
\begin{align*}
\int_{-\infty}^{\infty}\left(\int_{-\infty}^{\infty}K(x-y)f(y)\mathrm{d}y\right)g(x)\mathrm{d}x&=\int_{-\infty}^{\infty}f(y)\left(\int_{-\infty}^{\infty}K(x-y)g(x)\mathrm{d}x\right)\mathrm{d}y\\
&=\int_{-\infty}^{\infty}(\wideparen{K}*g)(y)f(y)\mathrm{d}y.
\end{align*}
...
\end{align*}
利用\refcor{corollary:泛函分析--推论2.4.6}，即得$x=y$。

\item 若$x_n\to x\ (n\to\infty)$，则$\forall f\in\mathscr{X}^*$有
\begin{align*}
...
\begin{definition}
设$\mathscr{X}$是$B^*$空间，$\{f_n\}\subseteq\mathscr{X}^*$，$f\in\mathscr{X}^*$。称$\{f_n\}$\textbf{*弱收敛}到$f$，记作$w^*-\lim\limits_{n\to\infty}f_n=f$，是指：对于$\forall x\in\mathscr{X}$，都有$\lim\limits_{n\to\infty}f_n(x)=f(x)$。这时$f$称作泛函序列$\{f_n\}$的\textbf{*弱极限}。
\end{definition}

\begin{remark}
$\mathscr{X}$可以连续地嵌入$\mathscr{X}^{**}$，或者说$\mathscr{X}\subseteq\mathscr{X}^{**}$，因此$\mathscr{X}^*$上的弱收敛蕴含$\mathscr{X}^*$上的*弱收敛，而且当$\mathscr{X}$是一个自反空间时，*弱收敛与弱收敛等价。
\end{remark}

\begin{theorem}
设$\mathscr{X}$是一个$B^*$空间，又设$\{x_n\}\subseteq\mathscr{X},\ x\in\mathscr{X}$，则为了$x_n\rightharpoonup x\ (n\to\infty)$，必须且仅须：
\begin{enumerate}[(1)]
\item $\|x_n\|$有界；
...
应用Banach-Steinhaus定理即得结论。

\end{proof}

\begin{theorem}\label{theorem:泛函分析--定理2.5.21}
设$\mathscr{X}$是一个$B$空间，又设$\{f_n\}\subseteq\mathscr{X}^*,\ f\in\mathscr{X}^*$，则为了$w^*-\lim\limits_{n\to\infty}f_n=f$，必须且仅须
\begin{enumerate}[(1)]
\item $\|f_n\|$有界；
...
\begin{enumerate}[(1)]
\item 若$\|T_n-T\|\to0$，则称$T_n$\textbf{一致收敛}于$T$，记作$T_n\rightrightarrows T$。这时$T$称作$\{T_n\}$的一致极限。
\item 若$\|(T_n-T)x\|\to0\ (\forall x\in\mathscr{X})$，则称$T_n$强收敛于$T$，记作$T_n\to T$。这时$T$称作$\{T_n\}$的\textbf{强极限}。
...
\begin{definition}
称集$A$是\textbf{弱列紧的}，是指$A$中的任意点列有一个弱收敛子列。又称$A$是*\textbf{弱列紧的}，是指$A$中的任意点列有一个*弱收敛的子列。
\end{definition}
...
$$\{\langle f_n,x_m\rangle|n,m\in\mathbb{N}\}$$
...
\begin{theorem}[Banach定理]\label{theorem:泛函分析--Banach定理11}
...
\begin{align*}
\|f-g_{n_k}\|&\leqslant\|f-f_{n_k}\|+\|f_{n_k}-g_{n_k}\|\\
&=\|f-f_{n_k}\|+|1-\|f_{n_k}\||\to0\quad(k\to\infty).
\end{align*}
...
\item 证明$\mathscr{X}_0=\mathscr{X}$。倘若不然，存在$x_0\in\mathscr{X}\setminus\mathscr{X}_0$，不妨设$\|x_0\|=1$，利用定理2.4.7，$\exists f_0\in\mathscr{X}^*$，使得$\|f_0\|=1$，从而$f_0\in S_1^*$，并且$f_0(x)=0\ (\forall x\in\mathscr{X}_0)$。
对此$f_0$我们有
\begin{align*}
...
\end{align}
今证此$x\in\mathscr{X}_0$。倘若不然，由定理2.4.7，$\exists f\in\mathscr{X}^*$，使得
...
这就证明了$\exists x\in\mathscr{X}_0$，使得\eqref{eq:::HIOUR32s2.5.29}式成立。现在要证此$x$还适合\eqref{eq:::HIOUR32s2.5.28}式。事实上，$\forall f_0\in\mathscr{X}_0^*$，由Hahn-Banach定理，存在$f\in\mathscr{X}^*$，使得$f_0=Tf$。从而我们有
...
因为$\mathscr{X}$自反，所以$\mathscr{X}_0$也是自反的。又显然$\mathscr{X}_0$是可分的，这表明$(\mathscr{X}_0^*)^*$是可分的。再由\hyperref[theorem:泛函分析--Banach定理11]{Banach定理}，$\mathscr{X}_0^*$也是可分的。若记$\{g_n\}$为$\mathscr{X}_0^{**}$中的元素，它适合
\begin{align}
...
\begin{example}
求证: $(l^p)^*=l^q \left(1\leqslant p<\infty,\frac{1}{p}+\frac{1}{q}=1\right)$.
\end{example}
...
\begin{align*}
\lim\limits_{n\to\infty}\|x_n\|\geqslant\|x_0\|.
\end{align*}
\end{example}
...
\begin{example}
设$H$是Hilbert空间，求证: 在$H$中$x_n\rightharpoonup x\ (n\to\infty)$的充要条件是
\begin{enumerate}[(1)]
