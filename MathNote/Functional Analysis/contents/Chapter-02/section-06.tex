...
\begin{definition}
设$\mathscr{X}$为维数$\geqslant 1$的复Banach空间，$A:D(A)\subseteq\mathscr{X}\to\mathscr{X}$为闭线性算子。
\begin{enumerate}[(1)]
...
\begin{definition}
设$\mathscr{X}$为维数$\geqslant 1$的复Banach空间，$A:D(A)\subseteq\mathscr{X}\to\mathscr{X}$为闭线性算子。\textbf{算子值函数}$R_{\lambda}(A):\rho(A)\to\mathscr{L}(\mathscr{X})$定义为
\begin{align*}
...
\begin{lemma}
设$T\in\mathscr{L}(\mathscr{X})$，$\|T\|<1$，则$(I-T)^{-1}\in\mathscr{L}(\mathscr{X})$，且
\begin{align}
\|(I-T)^{-1}\|\leqslant\frac{1}{1-\|T\|}.\label{eq:neuman_est}
\end{align}
\end{lemma}
\begin{proof}
由压缩映射原理，映射$Sx\triangleq y+Tx$为压缩映射，存在唯一不动点$x=Sx=y+Tx$，且$\|x\|\leqslant\frac{\|y\|}{1-\|T\|}$。
事实上，
\begin{align*}
x=\lim\limits_{n\to\infty}S^n y=\sum\limits_{k=0}^{\infty}T^k y,
\end{align*}
即
\begin{align*}
(I-T)^{-1}=\sum\limits_{k=0}^{\infty}T^k,
\end{align*}
该级数称为$\mathbf{Neuman}$\textbf{级数}，可直接验证结论成立。
...
\end{align*}
当$|\lambda-\lambda_0|<\|(\lambda_0 I-A)^{-1}\|^{-1}$时，记$B\triangleq\left[I+(\lambda-\lambda_0)(\lambda_0 I-A)^{-1}\right]^{-1}\in\mathscr{L}(\mathscr{X})$，则
\begin{align}
(\lambda I-A)^{-1}=BR_{\lambda_0}(A)\in\mathscr{L}(\mathscr{X}),\label{eq:rho_open}
\end{align}
故$\lambda\in\rho(A)$。
...
\begin{align*}
\|R_{\lambda}(A)\|&\leqslant\|R_{\lambda_0}(A)\|\cdot\left\|\left[I+(\lambda-\lambda_0)R_{\lambda_0}(A)\right]^{-1}\right\|\\
&\leqslant2\|R_{\lambda_0}(A)\|.
\end{align*}
结合第一预解公式\eqref{eq:resolvent_eq}，
\begin{align*}
...
\end{theorem}
\begin{proof}
反证法：若$\rho(A)=\mathbb{C}$，则$R_{\lambda}(A)$在$\mathbb{C}$上解析。
当$|\lambda|>\|A\|$时，Neuman级数展开为
\begin{align}
R_{\lambda}(A)=\sum\limits_{n=0}^{\infty}\frac{1}{\lambda^{n+1}}A^n,\label{eq:resolvent_series}
\end{align}
且
\begin{align}
\|R_{\lambda}(A)\|\leqslant\frac{1}{|\lambda|-\|A\|}.\label{eq:resolvent_norm}
\end{align}
故$\|R_{\lambda}(A)\|$在复平面上有界。

对任意$f\in\mathscr{L}(\mathscr{X})^*$，定义数值解析函数$u_f(\lambda)\triangleq f(R_{\lambda}(A))$。
由Liouville定理，$u_f(\lambda)$为常值函数，故$R_{\lambda}(A)$为常值算子，与第一预解公式矛盾。
...
\end{definition}
\begin{remark}
由定义，$r_{\sigma}(A)\leqslant\|A\|$。
\end{remark}

\begin{theorem}[Gelfand定理]\label{theorem:泛函分析--Gelfand定理}
设$\mathscr{X}$是Banach空间，$A\in\mathscr{L}(\mathscr{X})$，则
\begin{align}
...
\end{remark}
\begin{proof}
首先，由式\eqref{eq:resolvent_series}与Cauchy-Hadamard收敛半径公式，当$|\lambda|>\varlimsup\limits_{n\to\infty}\|A^n\|^{\frac{1}{n}}$时，$R_{\lambda}(A)\in\mathscr{L}(\mathscr{X})$，故
\begin{align}
...
\end{align*}
\begin{enumerate}[(1)]
\item 证明$\sigma_p(A)=\emptyset$。
设$(\lambda I-A)x=0$，则$\lambda x_n=x_{n-1},\ n\geqslant2$，$\lambda x_1=0$。
- 若$\lambda\neq0$，则$x=0$；
- 若$\lambda=0$，则$x=0$。
因此$(\lambda I-A)^{-1}$恒存在，故$\sigma_p(A)=\emptyset$。
...
\begin{proposition}
设$(\mathscr{X},(\cdot,\cdot))$为Hilbert空间，$A\in\mathscr{L}(\mathscr{X})$为对称算子，则$\sigma(A)\subseteq\mathbb{R}$，且$\sigma_r(A)=\emptyset$。
\end{proposition}
\begin{proof}
...
综上$R(\lambda I-A)=\mathscr{X}$，由Banach逆算子定理，$\lambda I-A$存在有界逆，即$\lambda\notin\sigma(A)$，故$\sigma(A)\subseteq\mathbb{R}$。

设$\lambda\in\sigma(A)$且$\lambda\notin\sigma_p(A)$，则$N(\lambda I-A)=\{0\}$，此时
\begin{align*}
R(\lambda I-A)^{\perp}=N(\overline{\lambda}I-A^*)=N(\lambda I-A)=\{0\},
\end{align*}
即$R(\lambda I-A)$在$\mathscr{X}$中稠密，故$\lambda\in\sigma_c(A)$，因此$\sigma_r(A)=\emptyset$。

\end{proof}

\begin{proposition}
设$(\mathscr{X},(\cdot,\cdot))$为Hilbert空间，$U\in\mathscr{L}(\mathscr{X})$为酉算子，则$\sigma(U)\subseteq S^1=\{e^{i\theta}\mid\theta\in[0,2\pi]\}$，且$\sigma_r(U)=\emptyset$。
\end{proposition}
\begin{proof}
由酉算子定义，$\|Ux\|^2=\|x\|^2$，故$U$为单射，且$\|U\|=1$，因此$\sigma(U)\subseteq\{\lambda\in\mathbb{C}\mid|\lambda|\leqslant1\}$。
又$U$满射，由Banach逆算子定理，$U$存在有界逆$U^{-1}$且$\|U^{-1}\|=1$。
...
\end{align*}
即$R(\lambda I-U)$在$\mathscr{X}$中稠密，故$\lambda\in\sigma_c(U)$，因此$\sigma_r(U)=\emptyset$。
