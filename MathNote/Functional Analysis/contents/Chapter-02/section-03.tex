...
\end{proposition}
\begin{proof}
{\heiti 必要性:} 因为$\overline{E}$无内点, 所以$\overline{E}$不能包含任一球$B(x_0,r_0)$. 从而$\exists x_1\in B(x_0,r_0)$, 使得$x_1\notin\overline{E}$. 又由$\overline{E}$闭, 所以$\exists\varepsilon_1>0$, 使得$\overline{B}(x_1,\varepsilon_1)\cap\overline{E}=\varnothing$. 取
\begin{align*}
...
\begin{theorem}[Baire定理]\label{theorem:泛函分析--Baire定理}
...
对球$B(x_1,r_1),\exists B(x_2,r_2)\subseteq B(x_1,r_1)(r_2<1/2)$, 使得$\overline{B}(x_2,r_2)\cap(\overline{E_1}\cup\overline{E_2})=\varnothing$;
...
再证$A_n$没有内点. $\forall f\in A_n,\forall\varepsilon>0$, 由Weierstrass逼近定理, 存在多项式$p$, 使得$\|f-p\|<\frac{\varepsilon}{2}$.

$p$的导数在$[0,1]$上是有界的, 因此根据中值定理, $\exists M>0$, 使得对$\forall s\in[0,1]$及$|h|<1/n$, 成立$|p(s+h)-p(s)|\leqslant M|h|$.

设$g(s)\in C[0,1]$是一个分段线性函数, 满足$\|g\|<\varepsilon/2$. 并且各条线段斜率的绝对值都大于$M+n$, 那么$p+g\in B(f,\varepsilon)$, 而$p+g\notin A_n$.

这样, 我们证明了每个$A_n$是疏集, 从而$\bigcup\limits_{n=1}^{\infty}A_n$是第一纲集. 而$\mathscr{X}$是完备的, 由Baire定理$\mathscr{X}$是第二纲集, 由此根据式, $E$也是第二纲集.
...
\end{theorem}
\begin{remark}
定理中的Banach空间$\mathscr{X},\mathscr{Y}$可以换成更一般的$F$空间, 但证明需稍做修改. 参看关肇直、张恭庆、冯德兴所著《线性泛函分析入门》(上海科学技术出版社, 1979)的第二章§2.
\end{remark}
\begin{proof}
...
\end{align}
事实上, 必要性是显然的. 下证其充分性. 由于$T$的线性, 条件式等价于
\begin{align*}
...
\end{align*}
而$\mathscr{Y}$是完备的, 所以至少有一个$n\in\mathbb{N}$, 使得$TB(\theta,n)$非疏, 即$\overline{TB(\theta,n)}$至少含有一个内点. 因此$\exists U(y_0,r)\subseteq\overline{TB(\theta,n)}$, 注意到$TB(\theta,n)$是一个对称凸集, 便有$U(-y_0,r)\subseteq\overline{TB(\theta,n)}$, 从而
\begin{align*}
U(\theta,r)\subseteq\frac{1}{2}U(y_0,r)+\frac{1}{2}U(-y_0,r)\subseteq\overline{TB(\theta,n)}.
\end{align*}
由$T$的齐次性, 取$\delta=\frac{r}{3n}$, 便有$\overline{TB(\theta,1)}\supseteq U(\theta,3\delta)$.
\begin{figure}[H]
\centering
...
对$y_0\in U(\theta,\delta)$, 按(2), $\exists x_1\in B\left(\theta,\frac{1}{3}\right)$, 使得$\|y_0-Tx_1\|<\frac{\delta}{3}$;

对$y_1=y_0-Tx_1\in U\left(\theta,\frac{\delta}{3}\right)$, 按(2), $\exists x_2\in B\left(\theta,\frac{1}{3^2}\right)$, 使得$\|y_1-Tx_2\|<\frac{\delta}{3^2}$;

……………………

对$y_n=y_{n-1}-Tx_n\in U\left(\theta,\frac{\delta}{3^n}\right)$, 按(2), $\exists x_{n+1}\in B\left(\theta,\frac{1}{3^{n+1}}\right)$, 使得$\|y_n-Tx_{n+1}\|<\frac{\delta}{3^{n+1}}$;

于是$\sum\limits_{n=1}^{\infty}\|x_n\|\leqslant1/2$, 令$x_0\triangleq\sum\limits_{n=1}^{\infty}x_n$, 便有$x_0\in B(\theta,1)$. 而
\begin{align*}
\|y_n\|&=\|y_{n-1}-Tx_n\|=\cdots=\left\|y_0-T(x_1+x_2+\cdots+x_n)\right\|<\frac{\delta}{3^n}\quad(\forall n\in\mathbb{N}),
\end{align*}
即得
\begin{align}
S_n\triangleq\sum\limits_{i=1}^{n}x_i\to x_0,\quad TS_n\to y_0\quad(n\to\infty).
\end{align}
又因为$T$是连续的, 所以
\begin{align}
Tx_0=y_0,
...
\end{remark}
\begin{proof}
依\hyperref[theorem:泛函分析--开映射定理]{开映射定理}证明中的第(3)部分, 已知$U(\theta,1)\subseteq TB\left(\theta,\frac{1}{\delta}\right)$, 即$T^{-1}U(\theta,1)\subseteq B\left(\theta,\frac{1}{\delta}\right)$或$\|T^{-1}y\|<\frac{1}{\delta}\quad(\forall y\in\mathscr{Y},\|y\|<1)$.
...
\begin{definition}
设$T$是$\mathscr{X}\to\mathscr{Y}$的线性算子, $D(T)$是其定义域. 称$T$是\textbf{闭的}, 是指由$x_n\in D(T),x_n\to x$, 以及$Tx_n\to y$就能推出$x\in D(T)$, 而且$y=Tx$.
\end{definition}
...
\end{align*}
于是$\exists x\in B(\theta,1)\cap D(T)$, 使得$\frac{\delta_1 y}{\|y\|}=Tx\implies y=T\left(\frac{\|y\|}{\delta_1}x\right)\implies y\in R(T)$.
...
\end{theorem}
\begin{proof}
任取$x\in\overline{D(T)},\exists x_n\in D(T),\lim\limits_{n\to\infty}x_n=x$, 依假设$T$在$D(T)$上连续, 从而有界, 即$\exists M>0$, 使得$\|Tx\|\leqslant M\|x\|\quad(\forall x\in D(T))$.

于是
\begin{align*}
\|Tx_{n+p}-Tx_n\|\leqslant M\|x_{n+p}-x_n\|.
\end{align*}
由此可见$\{Tx_n\}$是$\mathscr{Y}$中的基本列, 已设$\mathscr{Y}$完备, 所以$\exists y\in\mathscr{Y}$, 使得$Tx_n\to y$. 不难看出$y$仅依赖于$x$, 而与$D(T)$中$x_n$的选择无关. 因此, 可以定义$T_1:x\mapsto y$. 容易验证$T_1$是线性的, 还有$T_1|_{D(T)}=T$, 并且$\|T_1x\|\leqslant M\|x\|(\forall x\in\overline{D(T)})$.
...
\end{align*}
因此$I$是连续的, 它既是单射又是满射. 依\hyperref[theorem:泛函分析--Banach逆算子定理]{Banach逆算子定理}, $I$可逆且$I^{-1}$连续, 即有$M>0$, 使
\begin{align*}
\|I^{-1}x\|_2 \leqslant M\|x\|_1 \quad (\forall x\in\mathscr{X}).
\end{align*}
又因$I^{-1}x$与$x$是同一个元素, 所以$\|\cdot\|_1$与$\|\cdot\|_2$等价.
...
\end{align*}
则$\|x\|_G$实际上是$(x,Tx)$在乘积空间$\mathscr{X}\times\mathscr{Y}$上的范数, 此时将$\|\cdot\|_G$称为\textbf{图模}. 
\end{definition}

\begin{proposition}
设$\mathscr{X},\mathscr{Y}$是$B$空间,则算子$T$是闭的等价于$G(T)$按图模是闭的.
\end{proposition}
...
\end{align*}
可知$\exists x^*\in\mathscr{X}$与$y^*\in\mathscr{Y}$, 使得$x_n\to x^*$, 且$Tx_n\to y^*$. 根据$T$的闭性即得$y^* = Tx^*$, 从而$Tx_n\to Tx^*$. 因此$\|x_n - x^*\|_G\to 0$.
...
\end{align*}
即$\|x_n - x\|_W\to 0$. 再根据\hyperref[corollary:泛函分析--等价范数定理]{等价范数定理}, $\|\cdot\|_W$与$\|\cdot\|$等价, 从而存在常数$M$, 使得
\begin{align*}
...
\end{theorem}
\begin{proof}
{\heiti 必要性:} 根据\hyperref[theorem:泛函分析--共鸣定理]{共鸣定理}, 结论是显然的.

{\heiti 充分性:} 假定$\|A_n\|\leqslant C(\forall n\in\mathbb{N})$, 对$\forall x\in\mathscr{X}$及$\forall\varepsilon>0$, 取$y\in M$, 使得
\begin{align*}
...
&< \frac{\varepsilon}{2} + \|A_ny - Ay\| \quad (\forall n\in\mathbb{N}).
\end{align*}
...
设 $\mathscr{X} $ 和 $\mathscr{Y} $ 为 Banach 空间, $T\in\mathscr{L} (\mathscr{X} ,\mathscr{Y} )$，$\{T_n\}_{n\in\mathbb{N}}\subset\mathscr{L} (\mathscr{X} ,\mathscr{Y} )$。称 $\{T_n\}$ 是 $T$ 的\textbf{近似}（或$T$的近似格式$\{T_n\}$具有\textbf{相容性}），若对每个 $x\in\mathscr{X} $，
\[
...
设 $\mathscr{X} $ 和 $\mathscr{Y} $ 为 Banach 空间,  $T_n\in\mathscr{L} (\mathscr{X} ,\mathscr{Y} )$ 可逆,即存在逆算子 $T_n^{-1}\in\mathscr{L} (\mathscr{Y} ,\mathscr{X} )$.称近似格式 $\{T_n\}$ 具有\textbf{稳定性}，若存在常数 $C>0$ 使得
\[
...
设 $\mathscr{X} $ 和 $\mathscr{Y} $ 为 Banach 空间,  $T\in\mathscr{L} (\mathscr{X} ,\mathscr{Y} )$ 可逆，$\{T_n\}\subset\mathscr{L} (\mathscr{X} ,\mathscr{Y} )$ 可逆。假设$T$的近似格式$\{T_n\}$具有相容性,即对 $\forall x\in\mathscr{X} $，有
