...
对每个 $n\in\mathbb{N}$，令
\[
F_n = \{x\in\mathscr{X} : p(x) \le n\}.
\]
首先证明 $F_n$ 是闭集。设 $\{x_k\}\subset F_n$ 且 $x_k\to x$，则 $p(x_k)\le n$。
由条件 (4) 知
\[
p(x) \le \varliminf_{k\to\infty} p(x_k) \le n,
\]
故 $x\in F_n$，因此 $F_n$ 为闭集。

由条件 (1) 知 $p(x)\ge 0$，且由$p$的陪域是$\mathbb{R}$知$p(x)<\infty$，故每个 $x$ 必属于某个 $F_n$，
...
对 $-x_0$，由$-x_0\in \mathscr{X} = \bigcup_{n=1}^\infty F_n$知,存在 $M$ 使得 $-x_0\in F_M$，即 $p(-x_0)\le M$。
现任取 $y\in B(0,r)$。由 \eqref{eq:ball}式有 $y+x_0\in B(x_0,r)\subseteq F_N$，
故 $p(y+x_0)\le N$。利用条件 (3) 的次可加性，
\[
p(y) \le p(y+x_0) + p(-x_0) \le N + M.
\]
因此 $B(0,r) \subseteq F_{N+M}$，即当 $\|x\|<r$ 时 $p(x)\le N+M$。

对任意 $x\in\mathscr{X}\setminus\{0\}$，令 $t = \frac{r}{2\|x\|}>0$，
则 $\|tx\| = \frac{r}{2} < r$，于是 $p(tx) \le N+M$。
...
\[
p(x) \le M_0 \|x\| \quad (\forall x\in\mathscr{X}),
\]
结论得证。
\end{proof}
...
\forall y\in\mathscr{Y},\ \exists x\in\mathscr{X}\text{ 满足 } Ax=y \text{ 且 } \|x\|\leqslant M\|y\|.
\end{align}
现设 $y_n\to y_0$。由 \eqref{eq:existence}式，存在 $x_0\in\mathscr{X}$ 使得 $Ax_0=y_0$ 且 $\|x_0\|\le M\|y_0\|$。
对每个 $n$，对向量 $y_n-y_0$ 应用 \eqref{eq:existence}式，存在 $u_n\in\mathscr{X}$ 满足
\[
Au_n=y_n-y_0,\qquad \|u_n\|\le M\|y_n-y_0\|.
\]
令$x_n\triangleq x_0+u_n,$则 $Ax_n=Ax_0+Au_n=y_0+(y_n-y_0)=y_n$，并且
\[
\|x_n-x_0\|=\|u_n\|\le M\|y_n-y_0\|\to0,
\]
故 $x_n\to x_0$。
下面证明存在常数 $C>0$ 使得 $\|x_n\|\le C\|y_n\|$ 对一切 $n$ 成立。分两种情形：

情形 1：$y_0\neq0$。
由 $y_n\to y_0$，存在 $N$，当 $n>N$ 时，有
\[
\|y_n-y_0\|\le \|y_0\|,\qquad \|y_n\|\ge \frac12\|y_0\|.
\]
于是当 $n>N$ 时，有
\[
\|x_n\|\le \|x_0\|+\|u_n\|
\le M\|y_0\|+M\|y_n-y_0\|
\le M\|y_0\|+M\|y_0\|=2M\|y_0\|
\le 2M\cdot 2\|y_n\|=4M\|y_n\|.
\]
若$y_n=0(1\leqslant n\leqslant N)$，则令$C_1=0.$否则,令$C_1\triangleq \max_{\substack{1\le n\le N\\ y_n\neq0}}\frac{\|x_n\|}{\|y_n\|}.$
再取
\begin{align*}
C\triangleq \max\{4M,\,C_1\},
\end{align*}
则对所有 $n$ 均有 $\|x_n\|\le C\|y_n\|$。

情形 2：$y_0=0$。
此时由 \eqref{eq:existence} 式,可直接选取$C=M$,则对所有 $n$ 均有 $\|x_n\|\le C\|y_n\|$。

综上所述，总存在常数 $C>0$ 及满足 $Ax_n=y_n$ 的序列 $x_n\to x_0$，使得 $\|x_n\|\le C\|y_n\|$ 对所有 $n$ 成立。
\end{proof}
