...
\begin{theorem}
设 $\mathscr{X}$ 是一个 Hilbert 空间,内积为 $(\cdot,\cdot)$，诱导范数为 $\|\cdot\|$，$\mathscr{X}^*$ 表示 $\mathscr{X}$ 上所有有界线性泛函的全体（即 $\mathscr{L}(\mathscr{X},\mathbb{K})$，其中 $\mathbb{K}$为$\mathscr{X}$的系数域）。则
...
\end{theorem}
\begin{proof}
固定 $y\in\mathscr{X}$，显然 $f_y$ 是线性的，因为内积对第一个变量是线性的。由 Cauchy-Schwarz 不等式，
\begin{align}
...
设$f$是Hilbert空间$\mathscr{X}$上的一个连续线性泛函，内积为 $(\cdot,\cdot)$，诱导范数为 $\|\cdot\|$,则必存在唯一的$y_f\in\mathscr{X}$，使得
\begin{align}
...
\end{remark}
\begin{proof}
不妨设$f$不是0泛函，考察集合$M\triangleq\{x\in\mathscr{X}\mid f(x)=0\}$。由于$f$是连续线性的，则$M$是一个真闭线性子空间，任取$x_0\perp M$，不妨设$\|x_0\|=1$，$\mathscr{X}$中任意元素$x$可以分解如下：
\begin{align}
x=\alpha x_0 + y,
\label{eq:decomp}
\end{align}
...
\begin{proof}
固定$y\in\mathscr{X}$，$x\mapsto a(x,y)$是一个连续线性泛函。由\hyperref[theorem:Riesz表示定理(Hilbert空间)]{Riesz表示定理}，$\exists z=z(y)\in\mathscr{X}$，使得
\begin{align*}
...
\end{align*}
$(\forall x,y_1,y_2\in\mathscr{X},\forall\alpha_1,\alpha_2\in\mathbb{K})$，所以$A$是线性的，并且
\begin{align*}
...
\begin{definition}[Laplace方程$-\Delta u=f$ Dirichlet边值问题的弱解]
...
\end{theorem}
\begin{proof}
存在性。根据\hyperref[theorem:Poincaré(庞加莱)不等式]{Poincaré不等式}，
\begin{align*}
...
\end{remark}
\begin{proof}
利用\hyperref[theorem:Riesz表示定理(Hilbert空间)]{Riesz表示定理}，$\exists u_0\in H_0^1(\Omega)$，使得
\begin{align}
...
\label{eq:varinner}
\end{align}
根据\hyperref[theorem:泛函分析--最佳逼近元的刻画]{最佳逼近元定理}，不等式\eqref{eq:varinner}等价于$u^*$是$u_0$在$C$上的最佳逼近元，而这是存在唯一的.
...
\end{align*}
令$l(u)=\int_\Omega u\mathrm{d}\mu$，显然$l(u)$关于$u$线性，由Cauchy-Schwarz不等式，
\begin{align*}
...
\end{align*}
它还是有界的。根据\hyperref[theorem:Riesz表示定理(Hilbert空间)]{Riesz表示定理}，存在函数$v\in L^2(\Omega,\mu+\nu)$，使得
\begin{align*}
...
\end{align*}
为此，令$F=\{x\in\Omega\mid v(x)\leqslant 0\}$，取$u(x)=\chi_F(x)$代入\eqref{eq:rnkey}式，有
\begin{align*}
...
同样，令$G=\{x\in\Omega\mid v(x)>1\}$，取$u(x)=\chi_G(x)$代入\eqref{eq:rnkey}式，有
\begin{align*}
...
这就证明了$0<v(x)\leqslant 1,x\in\Omega$ a.e.$\mu$。令$g(x)=\frac{1-v(x)}{v(x)}$，则$g(x)\geqslant 0$，且关于$\mu$可测。对$E\in\mathcal{B}$，取$u(x)=\frac{\chi_E(x)}{v(x)+\frac{1}{n}}$代入\eqref{eq:rnkey}式，得
\begin{align*}
...
剩下考虑情形$\mu(\Omega)=\infty$。由$\sigma$有限性，取$\Omega_n\subset\Omega_{n+1},\Omega=\bigcup\limits_{n\geqslant1}\Omega_n,\mu(\Omega_n)<\infty,n\geqslant1$。由先前结论，$E\subset\Omega$，
\begin{align*}
\nu(E\cap\Omega_n)=\int_{E\cap\Omega_n}g_n\mathrm{d}\mu,
\end{align*}
易证：$g_n(x)=g_{n+1}(x),x\in\Omega_n$。令$g(x)=\lim\limits_{n\to\infty}g_n(x)$，由单调收敛性得
\begin{align*}
\nu(E)=\lim\limits_{n\to\infty}\int_{\Omega_n\cap E}\mathrm{d}\nu=\lim\limits_{n\to\infty}\int_{E\cap\Omega_n}g_n\mathrm{d}\mu=\int_E g\mathrm{d}\mu,\quad E\in\mathcal{B}.
\end{align*}
