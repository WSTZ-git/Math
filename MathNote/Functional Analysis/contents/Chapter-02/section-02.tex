...
\begin{theorem}[Riesz表示定理]\label{theorem:Riesz表示定理-泛函}
...
\end{align}
结合\eqref{eq::FJHIOUFHWOAIJ232-1}\eqref{eq::FJHIOUFHWOAIJ232-2}式，得到 $\|f_y\| = \|y\|$。\end{proof}
...
\begin{example}
设 $f_1, f_2, \cdots, f_n$ 是Hilbert空间$H$ 上的一组有界线性泛函,
\begin{align*}
...
\end{example}
\begin{proof}
对$\forall k\in \{1,2,\cdots,n\}$,由\hyperref[theorem:Riesz表示定理(Hilbert空间)]{Riesz表示定理}知,存在唯一的$y_k\in H$,使得
\begin{align*}
f_k(x)=\langle x,y_k \rangle,\quad \forall x\in H. 
\end{align*}
从而由\rrefpro{proposition:正交的基本性质}{proposition:正交的基本性质-4}知
\begin{align*}
N\left( f_k \right) =\left\{ x\in H\mid \langle x,y_k\rangle =0 \right\} =\left\{ y_k \right\} ^{\bot},\quad k=1,2,\cdots,n.
\end{align*}
再由\rrefpro{proposition:正交的基本性质}{proposition:正交的基本性质-4}知
\begin{gather}
N\left( f_k \right) ^{\bot}=\left( \left\{ y_k \right\} ^{\bot} \right) ^{\bot}=\overline{\mathrm{span}\left\{ y_k \right\} }=\mathrm{span}\left\{ y_k \right\},\quad k=1,2,\cdots,n.\label{eqKJIOJFIJIOJFIOJIOioirw}
\\
M=\bigcap_{k=1}^n{N\left( f_k \right)}=\bigcap_{k=1}^n{\left\{ y_k \right\} ^{\bot}}=\left( \bigcup_{k=1}^n{\left\{ y_k \right\}} \right) ^{\bot}
=\left( \mathrm{span}\left\{ \bigcup_{k=1}^n{\left\{ y_k \right\}} \right\} \right) ^{\bot}=\mathrm{span}\left\{ y_1,\cdots ,y_n \right\} ^{\bot}=N\left( f_k \right) ^{\bot}=\left( \left\{ y_k \right\} ^{\bot} \right) ^{\bot}.\nonumber
\end{gather}
于是由\rrefpro{proposition:Hilbert空间稠密线性空间正交补为0}{proposition:Hilbert空间稠密线性空间正交补为0-1}知
\begin{align*}
M^{\bot}=\left( \mathrm{span}\left\{ y_1,\cdots ,y_n \right\} ^{\bot} \right) ^{\bot}=\overline{\mathrm{span}\left\{ y_1,\cdots ,y_n \right\} }=\mathrm{span}\left\{ y_1,\cdots ,y_n \right\}.
\end{align*}
因为$y_0$为 $x_0$ 在 $M$ 上的正交投影,所以$x_0-y_0\in M^{\bot}$.由上式可知,存在$\alpha_1, \alpha_2, \cdots, \alpha_n \in \mathbb{K}$,使得
\begin{align*}
x_0-y_0 = \sum\limits_{k=1}^n \alpha_k y_k\iff y_0=x_0-\sum\limits_{k=1}^n \alpha_k y_k.
\end{align*}
又由\eqref{eqKJIOJFIJIOJFIOJIOioirw}式知$y_k\in N(f_k)^{\bot},k=1,2,\cdots,n.$故结论得证.
\end{proof}

\begin{example}
设 $l$ 是Hilbert空间$H$ 上的实值有界线性泛函, $C$ 是 $H$ 中的一个闭凸子集. 又设
\begin{align*}
...
\begin{proof}
\begin{enumerate}[(1)]
\item 由\hyperref[theorem:Riesz表示定理(Hilbert空间)]{Riesz表示定理}知,存在唯一的$u^*\in H$,使得
\begin{align*}
l(v) = \langle v,u^* \rangle,\quad \forall v\in H.
\end{align*}
从而对$\forall v\in C$,有
\begin{align*}
\frac{1}{2}\left\| u^*-v \right\| ^2-\left\| u^* \right\| ^2=\frac{1}{2}\left< u^*-v,u^*-v \right> -\left< u^*,u^* \right> =\frac{1}{2}\left< v,v \right> -\left< v,u^* \right> =\frac{1}{2}\left\| v \right\| ^2-l\left( v \right) =f\left( v \right) .
\end{align*}

\item 由\hyperref[corollary:泛函分析--最佳逼近元的存在唯一性]{最佳逼近元的存在唯一性}知,存在$u_0\in C$,使得$\|u^*-u_0\|=\inf\limits_{v \in C} \|u^* - v\|.$从而再由(1)可知
\begin{align*}
f(u_0) = \frac{1}{2}\left\| u^*-u_0 \right\| ^2-\left\| u^* \right\| ^2=\underset{v\in C}{\mathrm{inf}}\left\{ \frac{1}{2}\left\| u^*-v \right\| ^2-\left\| u^* \right\| ^2 \right\} =\underset{v\in C}{\mathrm{inf}}f\left( v \right) .
\end{align*}
\end{enumerate}
\end{proof}

\begin{definition}[再生核Hilbert空间]
设 $S$ 为非空集合，$H$ 是由 $S$ 上的复值函数构成的Hilbert空间，内积记为 $\langle \cdot,\cdot\rangle$。若对$\forall x\in S$，泛函
\begin{align*}
E_x:H\to \mathbb C,\qquad E_x(f)=f(x)
\end{align*}
都是连续的（等价于有界），则称 $H$ 为一个\textbf{再生核$\mathbf{Hilbert}$空间}（Reproducing Kernel Hilbert Space）,也简称为$\mathbf{RKHS}$。
\end{definition}

\begin{definition}[再生核]
设 $H$ 是非空集合 $S$ 上的复值函数构成的Hilbert空间,内积记为 $\langle \cdot,\cdot\rangle$.若二元复值函数 $K:S\times S\to\mathbb C$ 满足以下两个条件：
\begin{enumerate}[(1)]
\item 对$\forall x\in S$，有 $K(\cdot,x)\in H$;
\item \textbf{再生性:}对$\forall x\in S,\forall f\in H$，有 $\langle f,K(\cdot,x)\rangle=f(x)$.
\end{enumerate}
其中 $K(\cdot,y)$ 表示固定 $y$ 时作为 $x$ 的函数.则称 $K$是$H$ 的\textbf{再生核}。
\end{definition}

\begin{theorem}[再生核的基本性质]\label{theorem:再生核的基本性质}
设 $H$ 是再生核Hilbert空间,$K$ 为再生核Hilbert空间$H$的再生核,则它满足以下基本性质:
\begin{enumerate}[(1)]
\item \textbf{存在唯一性：}$H$的再生核$K$必存在且唯一.

\item \textbf{共轭对称性：}对$\forall x,y\in S$，有
\begin{align*}
K(x,y)=\overline{K(y,x)}.
\end{align*}
\item \textbf{正定性：}对任意 $n\geqslant 1$，任意点 $x_1,\cdots,x_n\in S$ 和任意复数 $\alpha_1,\cdots,\alpha_n$，有
\begin{align*}
\sum_{i=1}^n\sum_{j=1}^n \alpha_i\overline{\alpha_j}K(x_i,x_j)\geqslant 0.
\end{align*}
特别地，矩阵 $(K(x_i,x_j))$ 是半正定Hermite矩阵。
\end{enumerate}
\end{theorem}
\begin{proof}
\begin{enumerate}[(1)]
\item 先证存在性.
对$\forall y\in S$,由\hyperref[theorem:Riesz表示定理(Hilbert空间)]{Riesz表示定理}知,存在唯一的$k_y\in H$,使得
\begin{align}\label{eq::IOJISJFI343333}
J_y(f) = \langle f,k_y\rangle,\quad \forall f\in H.
\end{align}
取
\begin{align}
K(x,y) \triangleq k_y(x),\quad \forall (x,y)\in S\times S.\label{eqeLLhfuehwu}
\end{align}
则对任意固定的$y\in S$,$K(x,y)$作为$x$的函数就是$k_y\in H$.并且由题设和\eqref{eq::IOJISJFI343333}式知
\begin{align}\label{eq::HFOJWJIJIO324t342444}
f(y) = J_y(f) = \langle f,k_y \rangle = \langle f,K(\cdot ,y) \rangle,\quad \forall f\in H,\forall y\in S.
\end{align}
故\eqref{eqeLLhfuehwu}式定义的二元函数就是$H$的一个再生核.

再证唯一性.若$K_1$也是$H$的再生核,则对$\forall f\in H,\forall y\in S$,由再生核的定义和\eqref{eq::HFOJWJIJIO324t342444}式知
\begin{align*}
...
\begin{theorem}\label{theorem:再生核可由正交规范集生成}
设由非空集合$S$上的复值函数构成的再生核 Hilbert 空间 $\mathcal{H}$的一组正交规范集为$\{e_n\}_{n=0}^{\infty}$,则 $\mathcal{H}$的再生核函数 $K(z,w)$ 可展开为
\begin{align*}
K(z,w)=\sum_{n=0}^{\infty} e_n(z)\overline{e_n(w)},\quad \forall z,w\in S. 
\end{align*}
\end{theorem}
\begin{proof}
对任意固定的$w\in S$，可视$K(\cdot,w)$ 为 $\mathcal{H}$ 中的元素。由\refcor{corollary:正交规范集恒等式}知
\begin{align}
K(\cdot,w)=\sum_{n=0}^{\infty}\langle K(\cdot,w),e_n\rangle e_n(\cdot), \label{rkhs:fourier:step1}
\end{align}
其中级数在 $\mathcal{H}$ 的范数拓扑下收敛。
再由内积的共轭对称性知
\begin{align}
...
因此
\begin{align}
\langle K(\cdot,w),e_n\rangle=\overline{e_n(w)}. \label{rkhs:fourier:coef}
\end{align}
将 \eqref{rkhs:fourier:coef} 代入 \eqref{rkhs:fourier:step1}，得到函数空间中的等式
\begin{align}
K(\cdot,w)=\sum_{n=0}^{\infty}\overline{e_n(w)}e_n(\cdot). \label{rkhs:fourier:func}
\end{align}
故
\begin{align}
K(z,w)=\sum_{n=0}^{\infty} e_n(z)\overline{e_n(w)},\quad \forall z,w\in S. \nonumber
...
\begin{example}
求证: $H^2(D)$ (定义见\rrefpro{proposition:H2D空间--泛函分析}{proposition:H2D空间--泛函分析-3}) 的再生核为
\begin{align*}
K(z,w) = \frac{1}{\pi(1 - z\overline{w})^2} \quad (z,w \in D).
\end{align*}
\end{example}
\begin{proof}
由\rrefpro{proposition:H2D空间--泛函分析}{proposition:H2D空间--泛函分析-3}知，$\{\varphi_n(z)=\sqrt{\frac{n}{\pi}}z^{n-1}\}_{n=1}^{\infty}$ 是 $H^2(D)$ 的一组标准正交基。记
\begin{align}
e_n(z)\triangleq \varphi_{n+1}(z)=\sqrt{\frac{n+1}{\pi}}z^n,\quad n=0,1,2,\cdots. \label{example:en}
\end{align}
则 $\{e_n\}_{n=0}^{\infty}$ 也是标准正交基。由\refthe{theorem:再生核可由正交规范集生成}知$H^2(D)$ 的再生核为
\begin{align}
K(z,w)=\sum_{n=0}^{\infty} e_n(z)\overline{e_n(w)}
=\sum_{n=0}^{\infty} \frac{n+1}{\pi} z^n \overline{w}^{\,n}. \label{example:kernel:series}
\end{align}
因 $z,w\in D$，有 $|z\overline{w}|<1$。利用幂级数恒等式 $\sum\limits_{n=0}^{\infty}(n+1)t^n=\frac{1}{(1-t)^2}$（$|t|<1$），取 $t=z\overline{w}$，得
\begin{align*}
K(z,w)=\frac{1}{\pi}\sum_{n=0}^{\infty}(n+1)(z\overline{w})^n
=\frac{1}{\pi(1-z\overline{w})^2}.
\end{align*}\end{proof}
