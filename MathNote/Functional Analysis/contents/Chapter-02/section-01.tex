...
若线性算子$f$取值于实数（复数）,即值域$R(f)\subseteq \mathbb{R}(\mathbb{C})$,则称$f$为\textbf{实（复）线性泛函}，记作$f(x)$或$\langle f,x\rangle$（即线性函数）。
\end{definition}
\begin{remark}
...
\begin{proposition}\label{proposition:线性算子无界充要条件}
设 \(X,Y\)都是$B^*$空间，\(f\) 为$X$到$Y$的线性算子，则 \(f\) 无界（即 \(f \notin \mathscr{L}(X,Y)\)）当且仅当存在序列 \(\{x_n\} \subset X\)，使得 \(f(x_n) = 1\) 且 \(\|x_n\| \to 0\)（即 \(x_n \to \theta\)）.
\end{proposition}
\begin{proof}
...
\end{align*}
故 \(\|x_n\| \to 0\). 反之，若存在满足条件的 \(\{x_n\}\)，则对任意 \(M > 0\)，取 \(n > M\)，有 \(|f(n x_n)| = n > M \|n x_n\|\)，故 \(f\) 无界.
\end{proof}

\begin{proposition}\label{proposition:线性算子有界当且仅当连续}
设$\mathscr{X},\mathscr{Y}$都是$B^*$空间,则$T\in \mathscr{L}(\mathscr{X},\mathscr{Y})$,即$T$有界当且仅当满足下列条件之一:
\begin{enumerate}[(1)]
\item $T$是连续的线性算子;

\item $T$是线性算子,并将 $\mathscr{X}$ 中的有界集映为 $\mathscr{Y}$ 中的有界集.
\end{enumerate}
\end{proposition}
\begin{proof}
\begin{enumerate}[(1)]
\item 必要性显然。下证充分性。若不然，则对$\forall n\in \mathbb{N}$,$\exists x_n\in\mathscr{X}$，使得
\begin{align*}
\|Tx_n\|>n\|x_n\|.
\end{align*}
令$y_n=\frac{x_n}{n\|x_n\|}$，便有$\|Ty_n\|>1$。但$y_n\to\theta(n\to\infty)$，便与$T$的连续性矛盾！

\item 因为 $T$ 是线性的，且 $\mathscr{X}$ 中的单位球 $B_1 = \{x \in \mathscr{X} : \|x\| \leqslant 1\}$ 是有界集。由假设可知 $T(B_1)$ 是 $\mathscr{Y}$ 中的有界集，因此存在常数 $M > 0$，使得对任意 $\|x\| \leqslant 1$，都有 $\|Tx\| \leqslant M$。根据线性算子范数的定义可知 $\|T\| = \sup\limits_{\|x\| \leqslant 1} \|Tx\| \leqslant M < \infty$，故 $T$ 是有界线性算子，即 $T \in \mathscr{L}(\mathscr{X},\mathscr{Y})$.
\end{enumerate}
\end{proof}

\begin{proposition}\label{proposition:泛函分析--命题1.1231.31.3}
设$\mathscr{X},\mathscr{Y}$都是$B$空间, $T: \mathscr{X} \to \mathscr{Y}$ 是线性的, 令
\begin{align*}
...
\item\label{proposition:泛函分析--命题1.1231.31.3-3} 若 $f$ 是非零线性泛函, 则$f \in \mathscr{X}^*$当前仅当满足下列条件之一:
\begin{enumerate}[(i)]
\item $N(f)$是闭线性子空间.

\item $N(f)$不在$\mathscr{X}$中稠密.
\end{enumerate}
\end{enumerate}
\end{proposition}
\begin{proof}
\begin{enumerate}[(1)]
\item 因为 $T \in \mathscr{L}(\mathscr{X},\mathscr{Y})$，所以 $T$ 连续。由于 $\{\theta\}$ 是 $\mathscr{Y}$ 中的闭集，故其原像 $N(T)$也是 $\mathscr{X}$ 中的闭集。又因为 $T$ 是线性算子，易知 $N(T)$ 对加法和数乘封闭，故 $N(T)$ 是 $\mathscr{X}$ 的闭线性子空间.

\item 不能. 反例：令 $\mathscr{X} = \mathscr{Y} = \ell^2$，取 $\ell^2$ 的标准正交基 $\{e_n\}$，定义线性算子 $T(e_n) = n e_n$,则
\begin{align*}
\frac{\left\| Te_n \right\|}{\left\| e_n \right\|}=\frac{n\left\| e_n \right\|}{\left\| e_n \right\|}=n\rightarrow \infty ,\quad n\rightarrow \infty .
\end{align*}
因此$T$ 在 $\ell^2$ 上无界，故 $T \notin \mathscr{L}(\mathscr{X},\mathscr{Y})$. 但是，若 $Tx = 0$，由 $e_n$ 的线性无关性知必然有 $x = 0$，即 $N(T) = \{0\}$，这是 $\mathscr{X}$ 的闭线性子空间. 故 $N(T)$ 闭不能推出 $T \in \mathscr{L}(\mathscr{X},\mathscr{Y})$.

\item \begin{enumerate}[(i)]
\item 若 $f \in \mathscr{X}^*$，则 $f$ 连续，则由(1)可知$N(T)$是 $\mathscr{X}$ 中的闭线性子空间.

反之，若 $N(f)$ 是闭线性子空间，假设 $f$ 无界，则由\cref{proposition:线性算子无界充要条件},存在 $x_n \in \mathscr{X}$ 使得 $f(x_n) = 1$ 且 $\|x_n\| \to 0$. 令 $z_n = x_n - x_1$，则 $f(z_n) = 0$，即 $z_n \in N(f)$ 且 $z_n \to -x_1$. 因为 $f(-x_1) = -1 \neq 0$，故 $-x_1 \notin N(f)$，从而 $N(f)$ 不闭，矛盾! 

\item 若 $N(f)$ 不在 $\mathscr{X}$ 中稠密，则存在 $x_0 \in \mathscr{X}$，$r > 0$，使得 $B(x_0, r) \cap N(f) = \varnothing$. 
若 $f$ 无界，由\cref{proposition:线性算子无界充要条件}知存在 \(x_n \to \theta\) 且 \(f(x_n) = 1\). 对任意 \(x \in X\)，令\(y_n = x - f(x) x_n\)，则
\begin{align*}
f(y_n) = f(x) - f(x)f(x_n) = 0.
\end{align*}
故 \(y_n \in N(f)\)。由于 \(x_n \to \theta\)，故 \(y_n \to x\)，从而 \(N(f)\) 在 \(X\) 中稠密,这与 $N(f)$ 不在 $\mathscr{X}$ 中稠密矛盾!

反之，若 $f \in \mathscr{X}^*$，则由(i)知$N(f)$ 是闭子空间.假设$N(f)$在$\mathscr{X}$中稠密,则$N(f)=\mathscr{X}$,从而$f \equiv 0$,矛盾!
\end{enumerate}
\end{enumerate}
\end{proof}
...
\begin{proposition}\label{proposition:有限维赋范空间上的线性泛函自动连续}
...
\begin{theorem}\label{theorem:正交投影算子定义}
设$M$是Hilbert空间$\mathscr{X}$的一个闭线性子空间，依\hyperref[corollary:泛函分析--正交分解定理]{正交分解定理}，$\forall x\in\mathscr{X}$，存在唯一的分解
\begin{align*}
x=y+z,
\end{align*}
其中$y\in M,z\in M^\perp$。对应$x\mapsto y$称作由$\mathscr{X}$到$M$的\textbf{正交投影算子}，记作$P_M$。在不强调子空间$M$时，我们省略$M$而简记为$P$。而且$P$还是一个连续线性算子，显然$\ker P_M=M^\bot$.并且如果$M\neq\{\theta\}$，那么$\|P\|=1$。
\end{theorem}
\begin{proof}
...
\begin{example}
设$\mathscr{X},\mathscr{Y}$都是$B$空间, $A \in \mathscr{L}(\mathscr{X},\mathscr{Y})$, 求证:
\begin{enumerate}[(1)]
...
\begin{example}
设$\mathscr{X},\mathscr{Y}$都是$B$空间, $f \in \mathscr{L}(\mathscr{X},\mathbb{R})$, 求证:
\begin{enumerate}[(1)]
\item $\|f\| = \sup_{\|x\| = 1} f(x)$;
...
\begin{example}
设$\mathscr{X}$是$B$空间, $f$ 是 $\mathscr{X}$ 上的非零有界线性泛函, 令
\begin{align*}
...
\begin{example}
设$\mathscr{X}$是$B$空间, $f \in \mathscr{X}^*$, 求证: $\forall \varepsilon > 0, \exists x_0 \in \mathscr{X}$, 使得 $f(x_0) = \|f\|$, 且 $\|x_0\| < 1 + \varepsilon$.
\end{example}

\begin{example}
设$\mathscr{X}$是$B$空间,$f$ 是 $\mathscr{X}$ 上的线性泛函, 记
\begin{align*}
