...
\begin{definition}[Sobolev空间]\label{definition:Sobolev空间}
...
\end{align*}
该赋范空间称为\textbf{Sobolev空间}，也可记作$W_{p}^{m}(\Omega)$或$W^{m,p}(\Omega)$或$H^{m,p}(\Omega)$.特别当$p=2$时,$H^{m,2}(\Omega)$也可简单地记成$H^{m}(\Omega)$.
\end{definition}

\begin{theorem}\label{theorem:Sobolev空间的完备性}
空间$W^{m,p}(\Omega)$是完备的。进而$W^{m,p}(\Omega)$都是Banach空间,$H^{m}(\Omega)$都是Banach空间.
\end{theorem}
\begin{proof}
...
记
\begin{align*}
\Omega_{k}=\left\{x\in\Omega\mid\|x\|<k,\ \mathrm{dist}(x,\partial\Omega)>\frac{1}{k}\right\},\quad k=1,2,\cdots,
\end{align*}
$\Omega_{0}=\Omega_{-1}=\varnothing $，取开集族$\mathscr{O}=\{U_{k}\mid U_{k}=\Omega_{k+1}\cap C\overline{\Omega}_{k-1},\ k=1,2,\cdots\}$，其为$\Omega$的开覆盖。
...
\begin{example}
$\mathbb{R}_{+}^{n}=\{(x_{1},x_{2},\cdots,x_{n})\in\mathbb{R}^{n}\mid x_{n}>0\}$是可扩张的。
\end{example}
...
\begin{cases}
u(x),&x_{n}>0,\\
\sum\limits_{j=1}^{m+1}\lambda_{j}u(x_{1},\cdots,x_{n-1},-jx_{n}),&x_{n}\leqslant 0,
\end{cases}
\end{align*}
其中系数$\lambda_{1},\lambda_{2},\cdots,\lambda_{m+1}$是线性方程组
\begin{align*}
\sum\limits_{j=1}^{m+1}(-j)^{i}\lambda_{j}=1,\quad i=0,1,\cdots,m
\end{align*}
的唯一解。
...
\begin{theorem}[Sobolev嵌入定理]\label{theorem:泛函分析--Sobolev嵌入定理}
...
\begin{align*}
\|u\|_{C(\overline{\Omega})}\leqslant\|Tu\|_{C(\mathbb{R}^{n})}\leqslant c_{n,m}\|Tu\|_{m}'\leqslant c\|u\|_{W^{m,2}(\Omega)},
\end{align*}
故$i:u\mapsto u$是$W^{m,2}(\Omega)\to C(\overline{\Omega})$的连续嵌入。
\end{enumerate}
\end{proof}

\begin{theorem}[Rellich定理]\label{theorem:Rellich定理}
...
\end{align*}
其中$\widehat{U}_{m}(\xi)=\int_{K}U_{m}(x)e^{-2\pi \mathrm{i} x\cdot\xi}\mathrm{d}x=\langle e^{-2\pi \mathrm{i} x\cdot\xi}\alpha_{K},U_{m}\rangle$，$\alpha_{K}(x)=\begin{cases}1,&x\in K,\\0,&x\notin K.\end{cases}$
