...
\begin{proposition}
在线性空间$\mathscr{X}$上，为了两组可数个半范数$\{\|\cdot\|_m\}_{m=1}^\infty$与$\{\|\cdot\|_m'\}_{m=1}^\infty$是等价的当且仅当：
...
\begin{align*}
\int_{|x_1|>M_1'}\frac{\mathrm{d}x_1}{(1+|x_1|^2)^{\frac{1}{2}}}<\frac{\varepsilon}{4M_1'},
\end{align*}
以及
\begin{align*}
...
\begin{proposition}
设$\Omega\subseteq\mathbb{R}^n$为开集,$\varphi_\nu\to\varphi_0(\mathscr{D}(\Omega))$当且仅当存在紧集$K\subseteq\Omega$，使得$\varphi_0,\varphi_\nu\subset\mathscr{D}_K$，而且
\begin{align*}
...
\begin{proposition}
设$\Omega\subseteq\mathbb{R}^n$为开集,$\mathscr{D}'(\Omega)$表示广义函数全体，它是$\mathscr{D}(\Omega)$的共轭空间。对于$\mathscr{D}_K(\Omega),\mathscr{E}(\Omega),\mathscr{S}(\mathbb{R}^n)$，对应的共轭空间分别记作$\mathscr{D}_K'(\Omega),\mathscr{E}'(\Omega),\mathscr{S}'(\mathbb{R}^n)$，特别当$\Omega=\mathbb{R}^n$时，简记为$\mathscr{D}_K',\mathscr{E}',\mathscr{S}'$，显然有
\begin{align*}
...
设$\mathscr{X}$是一个$B_0$空间。$\mathscr{X}$上的线性泛函$f$是连续的当且仅当$\exists m\in\mathbb{N}$及$M_m>0$，使得
\begin{align*}
...
\begin{theorem}\label{theorem:任意B0空集中必存在足够多的连续线性泛函}
设$\mathscr{X}$是$B_0$空间,则$\forall x_0 \in \mathscr{X} \setminus \{\theta\}$，必$\exists f \in \mathscr{X}^*$，使得
\begin{align*}
f(x_0) = \|x_0\|, \quad \text{且} \quad \|f\| = 1.
\end{align*}
\end{theorem}
\begin{remark}
这个定理说明:任意$B_0$空间$\mathscr{X}$具有足够多的连续线性泛函.
\end{remark}
\begin{proof}
...
\end{align*}
对于半范数$\|\cdot\|_m$，应用\hyperref[theorem:泛函分析--(哈恩-巴拿赫)Hahn-Banach定理]{Hahn-Banach定理}，$f_0$可以扩张到全空间$\mathscr{X}$，即$f$满足$f|_{\mathscr{X}_0}=f_0$，且
\begin{align*}
|\langle f,x\rangle|\leqslant M_m\|x\|_m\quad(\forall x\in\mathscr{X}),
\end{align*}
从而$f\in\mathscr{X}^*$。于是对$\forall x_0\neq\theta,\exists f\in\mathscr{X}^*$，使得$\langle f,x_0\rangle=1$。\end{proof}

\begin{proposition}
设$\Omega\subseteq\mathbb{R}^n$为开集,如果有一个$B_0$空间$\mathscr{X}$，满足$\mathscr{D}(\Omega)\hookrightarrow\mathscr{X}$，即$\mathscr{D}(\Omega)\subseteq\mathscr{X}$，并且
\begin{align*}
...
\begin{theorem}
为了$f\in\mathscr{S}'$当且仅当$\exists m\in\mathbb{N}$及$u_\alpha\in L^2(\mathbb{R}^n)(|\alpha|\leqslant m)$，使得
\begin{align*}
...
\end{align}
于是$f$可以连续地扩张到以$\|\cdot\|_m'$为范数的$B$空间$\mathscr{X}_m$上。注意到$\|\cdot\|_m'$满足平行四边形等式，故可引出内积
\begin{align*}
