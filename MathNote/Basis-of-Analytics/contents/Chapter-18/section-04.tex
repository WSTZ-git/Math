...
\begin{theorem}[Cauchy不等式(离散版]\label{theorem:Cauchy不等式(离散版)}
...
对\(\forall n \in \mathbb{N}\),\(x_1,x_2,\cdots,x_n > 0\),由\hyperref[theorem:Cauchy不等式(离散版)]{Cauchy不等式(离散版)}可得
...
\end{example}
\begin{note}
首先我们猜测定义域的端点处可能存在最值,然后我们通过简单的放缩就能得到$y(0)$就是最小值.再利用\hyperref[theorem:Cauchy不等式(离散版)]{Cauchy不等式(离散版)}我们可以得到函数的最大值.构造Cauchy不等式的思路是:利用待定系数法构造相应的Cauchy不等式.具体步骤如下:
...
\begin{align*}
\frac{n}{\frac{1}{a_1}+\frac{1}{a_2}+\cdots +\frac{1}{a_n}}\leqslant \sqrt[n]{a_1a_2\cdots a_n}\leqslant \frac{a_1+a_2+\cdots +a_n}{n}\leqslant \sqrt{\frac{a_{1}^{2}+a_{2}^{2}+\cdots +a_{n}^{2}}{n}}.
\end{align*}
\end{theorem}
...
特别地,对$\forall x\geqslant -1,$,我们有
\begin{gather*}
(1+x)^n\geqslant1+nx,\quad \forall n\geqslant 1;
\\
(1+x)^n\leqslant1+nx,\quad \forall 0\leqslant n\leqslant 1.
\end{gather*}
...
\end{gather*}
由\hyperref[theorem:Jensen不等式]{Jensen不等式}和$f\left( x \right) =\ln x$函数的上凸性可知,上述不等式成立.等号成立的条件可由\hyperref[theorem:Jensen不等式]{Jensen不等式}的等号成立条件直接得到.故原结论也成立.   

\end{proof}

\begin{theorem}[离散的Hölder不等式]\label{theorem:离散的Hölder不等式}
设$\frac{1}{p}+\frac{1}{q}=1,p>1,a_1,a_2,\cdots,a_n\geqslant 0,b_1,b_2,\cdots,b_n\geqslant 0$,则有
\begin{align*}
\sum\limits_{k=1}^n{a_kb_k}\leqslant \sqrt[p]{\sum\limits_{k=1}^n{a_{k}^{p}}}\cdot \sqrt[q]{\sum\limits_{k=1}^n{b_{k}^{q}}}.
\end{align*}
\end{theorem}
\begin{proof}
