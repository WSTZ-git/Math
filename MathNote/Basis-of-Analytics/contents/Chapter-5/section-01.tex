...
给定\(a < x_1 < x_2 < \cdots < x_m < b\)和非负整数\(s_j,j = 0,1,2,\cdots,m\)。设\(f \in C^{\sum\limits_{j = 1}^{m}(s_j + 1)-1}[a,b]\)且\(f \in D^{\sum\limits_{j = 1}^{m}(s_j + 1)}(a,b)\),
则对闭区间\([a,b]\)中的\(m\)个点\(a \leqslant  x_1 < x_2 < \cdots < x_m \leqslant  b\),\(s_j \in \mathbb{N}_0,j = 1,2,\cdots,m\),都有唯一的次数不超过$\sum_{j = 1}^{m}(s_j + 1)-1$的多项式$p(x)\in \mathbb{R}[x]$,满足
\begin{align*}
p^{(i)}(x_j)=f^{(i)}(x_j),i = 0,1,\cdots,s_j,j = 1,2,\cdots,m.
\end{align*}
称满足上述条件的多项式$p(x)$为\textbf{Hermite 插值多项式},
并且对每个$x$ $\in$ $[a,b]$,都存在$\theta$ $\in$ $(\min\{x,x_1\}$, $\max\{x,x_m\})$，使得
\begin{align*}
f(x)=p(x)+\frac{f^{(\sum\limits_{j = 1}^{m}(s_j + 1))}(\theta)}{\left(\sum\limits_{j = 1}^{m}(s_j + 1)\right)!}(x - x_1)^{s_1 + 1}(x - x_2)^{s_2 + 1}\cdots(x - x_m)^{s_m + 1}.
\end{align*}
\end{theorem}
\begin{remark}
如果考试中要使用这个Hermite插值定理,要先用K值法证明(类似这个定理的证明)需要用到的插值等式.
\end{remark}
\begin{remark}
...
$\boldsymbol{p}${\heiti 的存在性}:将每个节点 $x_j$ 重复 $s_j+1$ 次，得到$N$维数组
\[
\left( t_1,t_2,\cdots ,t_N \right) \triangleq \left( \underset{s_1+1\text{个}}{\underbrace{x_1,\cdots ,x_1}},\underset{s_2+1\text{个}}{\underbrace{x_2,\cdots ,x_2}},\cdots ,\underset{s_m+1\text{个}}{\underbrace{x_m,\cdots ,x_m}} \right) ,
\]
其中 $N = \sum_{j=1}^m (s_j+1)$。构造\hyperref[Numerical-Analysis-theorem:牛顿均差插值多项式]{Newton均差插值多项式}
\[
...
\end{align*}
则由\hyperref[Numerical-Analysis-theorem:牛顿均差插值多项式]{牛顿均差插值多项式}知插值余项为
\[
...
\[
f^{(i)}(x_j)-p^{(i)}(x_j)=0,\qquad i=0,1,\cdots,s_j.
\]
故 $p(x)$ 满足 Hermite 插值条件。

$\boldsymbol{p}${\heiti 的唯一性}:若有两个这样的多项式 $q_1,q_2$，则 $q_1-q_2$ 次数 $\leqslant N-1$ 且在每点 $x_j$ 有至少 $s_j+1$ 重零点，故共有 $N$ 个零点（计重数），只能恒为零。因此 $p$ 唯一。

$\boldsymbol{\theta }${\heiti 的存在性}:若 $x$ 等于某个 $x_j$，则两边为零，结论成立。设 $x$ 不是任何节点，令
\[
...
\]
显然 $F(x)=0$。对每个节点 $x_j$，由插值条件及 $\omega$ 在 $x_j$ 处有 $s_j+1$ 重零点，得
\[
F^{(i)}(x_j) = f^{(i)}(x_j)-p^{(i)}(x_j)-K\cdot0 = 0,\quad i=0,\cdots,s_j.
\]
...
现在对 $F,F',\cdots,F^{(N-1)}$反复应用\hyperref[theorem:加强的Rolle中值定理]{Rolle定理}得 $F^{(N)}$ 在区间 $(\min\{x,x_1\},\max\{x,x_m\})$ 内至少有一个零点 $\theta$，即
\[
F^{(N)}(\theta)=0.
\]
由于 $\deg p \leqslant N-1$，故 $p^{(N)}(t)\equiv0$；而 $\omega(t)$ 是首一多项式且次数为 $N$，因此 $\omega^{(N)}(t) \equiv N!$。于是
\[
F^{(N)}(t) = f^{(N)}(t) - K\cdot N!.
\]
代入 $t=\theta$ 得
\[
0 = f^{(N)}(\theta) - K\cdot N! \Longrightarrow K = \frac{f^{(N)}(\theta)}{N!}.
\]
代回 $K$ 的定义，即得
\[
...
\end{remark}
\begin{note}
\textbf{K值法}:先令要证的中值等式中的高阶导数中值点(本题为$f''(\theta)$)为常数,再构造函数由Rolle中值定理推出结论即可.
\end{note}
\begin{proof}
...
\begin{align*}
k(x,y)=\begin{cases}
\frac{\left( x-a \right) \left( y-b \right)}{b-a},&		b\geqslant y\geqslant x\geqslant a,\\
\frac{\left( x-b \right) \left( y-a \right)}{b-a},&		b\geqslant x\geqslant y\geqslant a.\\
\end{cases}
\end{align*}
...
\end{theorem}
\begin{note}
$k(x,y)$就是\hyperref[definition:Green函数的性质]{Green函数},有$|k(x,y)|\leqslant  |k(x,x)|$.
\end{note}
\begin{proof}
...
\begin{cases}
\frac{x-0}{1-0}(y-1),&		1\geqslant y\geqslant x\geqslant 0,\\
\frac{1-x}{1-0}(0-y),&		1\geqslant x\geqslant y\geqslant 0.\\
\end{cases}
\end{align*}
从而
\begin{align*}
...
\begin{example}
设 $f \in C^2[a,b]$, $|f(x)| \leqslant A$, $|f''(x)| \leqslant B$, 且存在 $x_0 \in [a,b]$, 使得 $|f'(x_0)| \leqslant D$, 证明
$|f'(x)| \leqslant 2\sqrt{AB} + D,\; \forall x \in [a,b]$.
\end{example}
\begin{proof}

对$\forall x\in \left[ a,b \right]$, $h\in \mathbb{R}$且$x+h\in \left[ a,b \right]$, 由Taylor定理知, 存在$\theta _{x,h}$, 使得
\begin{align}
f\left( x+h \right) =f\left( x \right) +f'(x) h+\frac{f'' \left( \theta _{x,h} \right)}{2}h^2. \label{eq:::-2903890482933r31}
\end{align}
于是
\begin{align*}
\left| f'(x) \right|=\left| \frac{f\left( x+h \right) -f\left( x \right)}{h}-\frac{f'' \left( \theta _{x,h} \right)}{2}h \right|\leqslant \frac{2A}{\left| h \right|}+\frac{B}{2}\left| h \right|.
\end{align*}
注意到
\begin{align}
\frac{2A}{\left| h \right|}+\frac{B}{2}\left| h \right|\leqslant 2\sqrt{AB}+D \Longleftrightarrow \left| h \right|\in \left[ h_1,h_2 \right], \label{eq:::-2903890482933r32}
\end{align}
其中$h_1=\frac{2\sqrt{AB}+D-\sqrt{D^2+4\sqrt{AB}}}{B}$, $h_2=\frac{2\sqrt{AB}+D+\sqrt{D^2+4\sqrt{AB}}}{B}.$
故当$x$满足
\begin{align*}
\left( \left( x-h_1,x-h_2 \right) \cup \left( x+h_1,x+h_2 \right) \right) \cap \left[ a,b \right] \ne \varnothing,
\end{align*}
时, 任取$h\in \left( \left( x-h_1,x-h_2 \right) \cup \left( x+h_1,x+h_2 \right) \right) \cap \left[ a,b \right]$, 由\eqref{eq:::-2903890482933r31},\eqref{eq:::-2903890482933r32}式知结论成立.

当$x$满足
\begin{align*}
\left( \left( x-h_1,x-h_2 \right) \cup \left( x+h_1,x+h_2 \right) \right) \cap \left[ a,b \right] = \varnothing,
\end{align*}
时, 此即
\begin{align*}
&\left( \left( x-h_1,x-h_2 \right) \cup \left( x+h_1,x+h_2 \right) \right) \cap \left[ a,b \right] = \varnothing \Longleftrightarrow x+h_1>b \text{且} x-h_1<a \\
&\Longleftrightarrow b-h_1<x<a-h_1 \Longleftrightarrow \max\left\{ x-a,b-x \right\} <h_1.
\end{align*}
此时$\left| x-x_0 \right|\leqslant \max\left\{ x-a,b-x \right\} <h_1$, 而不难发现$h_1\leqslant 2\sqrt{\frac{A}{B}}.$于是由Lagrange定理知, 存在$\xi _x$, 使得
\begin{align*}
\left| f'(x) \right|=\left| f'(x_0) +f'' \left( \xi _x \right) \left( x-x_0 \right) \right|\leqslant \left| f'(x_0) \right|+\left| f'' \left( \xi _x \right) \right|\left| x-x_0 \right|\leqslant D+Bh_1\leqslant D+2\sqrt{AB}.
\end{align*}
