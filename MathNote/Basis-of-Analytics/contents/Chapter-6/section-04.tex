...
\begin{theorem}\label{theorem:开区间上的连续函数是一致连续的充要条件111}
\(f\in C(a,b),a,b\in \mathbb{R}\) 一致连续的充要条件是 \(\lim_{x\rightarrow a^{+}}f(x)\)，\(\lim_{x\rightarrow b^{-}}f(x)\) 存在.
\end{theorem}
\begin{remark}
...
若$f\in C[a,b],a,b\in \mathbb{R}$,则$f$在$[a,b]$上一致连续.
\end{theorem}
...
由 \refthe{theorem:开区间上的连续函数是一致连续的充要条件111}，\(f\) 在 \([0, 2T]\) 一致连续，所以对任何 \(\varepsilon > 0\)，存在 \(\delta \in (0, T)\) 使得对 \(|x_1 - x_2| < \delta\)，\(x_1, x_2 \in [0, 2T]\) 都有
\begin{align*}
...
\begin{solution}
\begin{enumerate}[(1)]
\item 不一致连续.由$\underset{x\rightarrow 0^+}{\lim}\ln x=+\infty $及\refthe{theorem:开区间上的连续函数是一致连续的充要条件111}可得.

\item 不一致连续.由$\underset{x\rightarrow 0^+}{\lim}e^x\cos \frac{1}{x}$不存在及\refthe{theorem:开区间上的连续函数是一致连续的充要条件111}可得.

\item 一致连续.由 $\lim_{x \rightarrow 0^+} f(x)$ 存在(连续性),$\lim_{x \rightarrow 0^+} \frac{\sin x}{x} = 1$ 及 \refthe{theorem:开区间上的连续函数是一致连续的充要条件111}可知，$f$ 在 $(0, 1]$ 上一致连续。又因为 $\lim_{x \rightarrow +\infty} \frac{\sin x}{x} = 0$，所以由\hyperref[proposition一致连续的充分不必要条件]{命题\ref{proposition一致连续的充分不必要条件}}可知，$f$ 在 $[1, +\infty)$ 上一致连续。再根据\hyperref[proposition:一致连续函数的拼接]{一致连续函数的拼接}可知，$f$ 在 $(0, +\infty)$ 上一致连续.
...
\item 在$(0,a)$上不一致连续,在$(a,+\infty)$上一致连续.由$\lim_{x \rightarrow 0^+} \frac{x + 2}{x + 1} \sin \frac{1}{x}$不存在,$\lim_{x \rightarrow +\infty} \frac{x + 2}{x + 1} \sin \frac{1}{x} = 0$及\refthe{theorem:开区间上的连续函数是一致连续的充要条件111}可得.
\end{enumerate}
...
当 $\alpha \leqslant 0$ 时，$\lim_{x \rightarrow 0^+} f\left( x \right)$ 不存在，由\refthe{theorem:开区间上的连续函数是一致连续的充要条件111}可知，$f$ 在 $\left( 0,2 \right)$ 上不一致连续。故此时 $f$ 在 $\left( 0,+\infty \right)$ 上不一致连续。
...
\)
故由\refthe{theorem:开区间上的连续函数是一致连续的充要条件111}可知，$f$ 在 $\left( 0,2 \right]$ 上一致连续。于是由\hyperref[proposition:一致连续函数的拼接]{一致连续的拼接}可得，$f$ 在 $\left( 0,+\infty \right)$ 上一致连续.
...
当 $\alpha \leqslant 0$ 时，$\lim_{x \rightarrow 0^+} f\left( x \right)$ 不存在，由\refthe{theorem:开区间上的连续函数是一致连续的充要条件111}可知，$f$ 在 $\left( 0,1 \right)$ 上不一致连续。故此时 $f$ 在 $\left( 0,+\infty \right)$ 上不一致连续。
...
\lim_{x \rightarrow 0^+} f\left( x \right) = 0,
\)
故由\refthe{theorem:开区间上的连续函数是一致连续的充要条件111}可知，$f$ 在 $\left( 0,1 \right]$ 上一致连续。于是由\hyperref[proposition:一致连续函数的拼接]{一致连续的拼接}可得，$f$ 在 $\left( 0,+\infty \right)$ 上一致连续.
