...
\item $\Delta(uv)=u\Delta v+v\Delta u+2\nabla u\cdot\nabla v$.
\end{enumerate}
\end{theorem}
...
\begin{theorem}[Stokes定理]\label{theorem:Stokes公式--多元函数}
设$\Sigma$是$\mathbb{R}^3$中逐片光滑的有向曲面，边界$\partial\Sigma$是分段光滑的闭曲线，方向与$\Sigma$符合右手定则。若向量场$\bm{F}$在包含$\Sigma$的某个开区域内连续可微，则
\begin{align*}
\int_{\Sigma}(\nabla\times\bm{F})\cdot\bm{n}\,\mathrm{d}S=\oint_{\partial\Sigma}\bm{F}\cdot\mathrm{d}\bm{r}. 
\end{align*}
\end{theorem}
