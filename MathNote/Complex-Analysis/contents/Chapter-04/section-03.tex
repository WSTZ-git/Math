...
\item 设$D \subseteq \mathbb{C}$是非空集合，$f: D \to \mathbb{C}$。称$f$\textbf{在$D$上可局部展开成幂级数}，若对$\forall z_0 \in D$，存在$r > 0$使得开圆盘$B(z_0, r) \subseteq D$，且存在$\{a_n\}\subseteq \mathbb{C}$满足
\begin{align*}
f(z) = \sum\limits_{n=0}^{\infty} a_n (z - z_0)^n, \quad \forall z \in B(z_0, r).
\end{align*}
...
设 $D \subseteq \mathbb{C}$ 是非空开集，$f: D \to \mathbb{C}$. 若$f$ 在 $D$上可整体展开为幂级数,则$f$ 在 $D$上可局部展开为幂级数.
\end{proposition}
\begin{proof}
...
f(z) = \sum_{n=0}^{\infty} a_n (z - z_0)^n, \quad \forall z \in B(z_0, r).
\end{align*}
当上述条件成立时，$f$在$B(z_0,r)$上的展开式必为Taylor级数，即
\begin{align*}
...
根据\refthe{theorem:定理4.3.1}，$f$ 在圆盘 $B(z_0, R)$ 内可展开为幂级数：
\begin{align*}
...
再由\refthe{theorem:定理4.3.1}知,此时,$f$在$B(z_0,r)$上的展开式必为Taylor级数，即
\begin{align*}
a_n = \frac{f^{(n)}(z_0)}{n!} = \frac{1}{2\pi \mathrm{i}} \int_{|\zeta - z_0| = \rho} \frac{f(\zeta)}{(\zeta - z_0)^{n+1}} \, \mathrm{d}\zeta \quad (0 < \rho < r),
\end{align*}
且展开式唯一.
