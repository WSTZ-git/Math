...
\begin{definition}
设级数
\begin{align}\label{eq::293hr0ur2}
\sum_{n=0}^{\infty} a_n(z - z_0)^n = a_0 + a_1(z - z_0) + \cdots  + a_n(z - z_0)^n + \cdots . 
\end{align}
如果存在常数 \( R \)，使得当 \( |z-z_0| < R \) 时，级数 \eqref{eq::293hr0ur2}
收敛；当 \( |z-z_0| > R \) 时，级数
\eqref{eq::293hr0ur2}
发散，就称 \( R \) 为级数 \eqref{eq::293hr0ur2} 的\textbf{收敛半径}，\( \{z : |z-z_0| < R\} \) 称为级数\eqref{eq::293hr0ur2}的\textbf{收敛圆}。
\end{definition}

\begin{theorem}[Cauchy-Hadamard收敛半径公式]\label{theorem:Cauchy-Hadamard收敛半径公式}
级数\begin{align*}
\sum_{n=0}^{\infty} a_n(z - z_0)^n = a_0 + a_1(z - z_0) + \cdots  + a_n(z - z_0)^n + \cdots 
\end{align*}
存在收敛半径
...
不妨设$a=0$,否则用$z-a$代替$z$.设 \( K \) 是 \( \{z : |z| < |z_0|\} \) 中的一个紧集，选取 \( r < |z_0| \)，使得 \( K \subseteq B(0, r) \)。于是，当 \( z \in K \) 时，有 \( |z| < r \)。因为 \( \sum_{n=0}^{\infty} a_n z_0^n \) 收敛，所以由\refcor{corollary:复幂级数收敛必要条件}可知 \( |a_n z_0^n| < M \)，这里，\( M \) 是一个常数。于是，当 \( z \in K \) 时，有
\begin{align*}
|a_n z^n| = \left| a_n z_0^n \frac{z^n}{z_0^n} \right| \leqslant M \frac{|z|^n}{|z_0|^n} \leqslant M \left( \frac{r}{|z_0|} \right)^n.
\end{align*}
因为 \( r < |z_0| \)，所以由\hyperref[theorem:Weierstrass一致收敛判别法]{Weierstrass一致收敛判别法}，\( \sum_{n=0}^{\infty} |a_n z^n| \) 在 \( K \) 中一致收敛。从而原级数也在$K$中绝对收敛.
...
\begin{proposition}\label{proposition:4.2.10}
证明
\begin{align*}
...
并且
\begin{align}
\sum_{n=1}^{\infty} \frac{\mathrm{e}^{\mathrm{i}n\theta}}{n} = -\ln(1 - \mathrm{e}^{\mathrm{i}\theta}) = -\ln|1 - \mathrm{e}^{\mathrm{i}\theta}| - \mathrm{i}\arg(1 - \mathrm{e}^{\mathrm{i}\theta}),\quad 0<\theta <2\pi. \label{eq:2-=------2}
\end{align}
\end{proposition}
\begin{proof}

容易知道该级数的收敛半径为1,所以它的和$f(z)$是定义在单位圆盘中的单值全纯函数,因而有
\[
f(z) = \sum_{n=1}^{\infty} \frac{z^n}{n},
\quad
f'(z) = \sum_{n=1}^{\infty} z^{n-1} = \frac{1}{1 - z},\ |z| < 1. 
\]
由\hyperref[theorem:复积分的微分学基本定理]{复积分的微分学基本定理}和\refpro{proposition:1/z的变限积分}得
\begin{align*}
f\left( z \right) =\int_0^z{f' \left( \zeta \right) \mathrm{d}\zeta}=\int_0^z{\frac{1}{1-\zeta}\mathrm{d}\zeta}=-\int_1^{1-z}{\frac{1}{\zeta}\mathrm{d}\zeta}=-\mathrm{Ln}\left( 1-z \right) ,\ |z| < 1. 
\end{align*}
又因为$f$是单位圆盘上的单值全纯函数,所以$f(z)$是$-\mathrm{Ln}\left( 1-z \right)$定义在单位圆盘上的一个单值全纯分支.注意到$f(0)=0$,故
\[
\sum_{n=1}^{\infty} \frac{z^n}{n} = -\ln(1 - z),\ |z| < 1. 
\]
这个级数在收敛圆周上除了点$z = 1$外都收敛,故由\hyperref[theorem:Abel第二定理]{Abel第二定理},当$z = \mathrm{e}^{\mathrm{i}\theta}\neq 1 (0 < \theta < 2\pi)$时,有
\begin{align*}
\sum_{n=1}^{\infty} \frac{\mathrm{e}^{\mathrm{i}n\theta}}{n} = -\ln(1 - \mathrm{e}^{\mathrm{i}\theta}) = -\ln|1 - \mathrm{e}^{\mathrm{i}\theta}| - \mathrm{i}\arg(1 - \mathrm{e}^{\mathrm{i}\theta}). 
\end{align*}
