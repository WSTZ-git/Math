...
\begin{align}
\mathrm{Res}(f,\infty) =\frac{1}{2\pi \mathrm{i}} \int_{\gamma^-} f(z) \mathrm{d}z= -\frac{1}{2\pi \mathrm{i}} \int_{\gamma} f(z) \mathrm{d}z, \label{eq:::897897889---3}
\end{align}
这里,\( \gamma = \{ z: |z| = \rho \}, R < \rho < \infty \).
\end{definition}

\begin{proposition}\label{proposition:积分与留数的关系}
设 \( a \) 是 \( f \) 的一个孤立奇点,对于 \( a \) 点邻域中的任意可求长闭曲线 \( \gamma \),都有
$$
\int_{\gamma} f(z) \mathrm{d}z = 2\pi \mathrm{i} \mathrm{Res}(f,a)\iff \mathrm{Res}\left( f,a \right) =\frac{1}{2\pi \mathrm{i}}\int_{\gamma}{f(z)\mathrm{d}z}.
$$
\end{proposition}
\begin{remark}
...
设$f$是定义在复平面上的一个函数.
\begin{enumerate}[(1)]
\item 若 \( a\in \mathbb{C} \) 是 \( f \) 的 \( m \) 阶极点,则
\[
...
\item 若 \( z = \infty \) 是 \( f \) 的孤立奇点,即 \( f \) 在 \( R < |z| < \infty \) 中全纯,则
\begin{align*}
\mathrm{Res}\left( f\left( z \right) ,\infty \right) =-\mathrm{Res}\left( f\left( \frac{1}{t} \right) \frac{1}{t^2},0 \right) .
\end{align*}
\end{enumerate}
\end{proposition}
\begin{proof}
\begin{enumerate}[(1)]
\item 因为 \( a \) 是 \( f \) 的 \( m \) 阶极点,故由\rrefthe{theorem:m阶极点的充要条件}{theorem:m阶极点的充要条件-1}可知,在 \( a \) 点的邻域中有
\begin{align}
...
\item 令$t=\frac{1}{z}$,则$f\left( \frac{1}{t} \right) =f\left( z \right)$,且$z$平面上无穷远点的去心邻域$N\setminus \left\{ \infty \right\} :0\leqslant r<\left| z \right|<+\infty$被变成$t$平面上原点的去心邻域$K\setminus \left\{ 0 \right\} :0<\left| t \right|<\frac{1}{r}$;
圆周$\Gamma :\left| z \right|=\rho >r$被变成圆周$\gamma :\left| t \right|=\lambda =\frac{1}{\rho}<\frac{1}{r}$.从而
\begin{align*}
\mathrm{Res}\left( f\left( z \right) ,\infty \right) =\frac{1}{2\pi \mathrm{i}}\int_{\Gamma ^-}{f\left( z \right) \mathrm{d}z}=-\frac{1}{2\pi \mathrm{i}}\int_{\Gamma}{f\left( z \right) \mathrm{d}z}
=-\frac{1}{2\pi \mathrm{i}}\int_{\gamma}{f\left( \frac{1}{t} \right) \frac{1}{t^2}\mathrm{d}t}=-\mathrm{Res}\left( f\left( \frac{1}{t} \right) \frac{1}{t^2},0 \right).
\end{align*}
\end{enumerate}
...
由\refpro{proposition:命题5.4.2}即得
\[
...
在所设的条件下,\( a \) 是 \( f \) 的1阶极点,故由\refpro{proposition:命题5.4.2}即得
\begin{align*}
...
\begin{align}
\int_{\gamma}{f\left( z \right) \mathrm{d}z}=2\pi \mathrm{i}\sum_{k=1}^n{\mathrm{Res}\left( f,z_k \right)}\Longleftrightarrow \mathrm{Res}\left( f,\infty \right) +\sum_{k=1}^n{\mathrm{Res}\left( f,z_k \right)}=0.\label{eq::---2837128-5}
\end{align}
\end{theorem}
\begin{remark}
...
\]
再由\refpro{proposition:积分与留数的关系},即得所要证的公式\(\eqref{eq::---2837128-5}\)左边.由\(\eqref{eq:::897897889---3}\)式得
\begin{align*}
- \int_{|z| = R} f(z) \mathrm{d}z = 2\pi \mathrm{i} \mathrm{Res}(f, \infty).
\end{align*}
故公式\(\eqref{eq::---2837128-5}\)成立.
...
\]
由\refpro{proposition:命题5.4.2}和\refpro{proposition:命题5.4.2},得
\begin{align*}
