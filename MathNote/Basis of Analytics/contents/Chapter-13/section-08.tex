...
\end{align*}
其中$\mathbf{1}_k^T = (1,1,\cdots,1)_{1\times k}$. 由$H$正定可知$|H_k| > 0$，又因为$\lambda > 0$，所以再由\reflem{lemma:Hilbert矩阵逆矩阵元素和}可得
\begin{align*}
|\lambda H_k - J_k| > 0 \Longleftrightarrow \lambda > \mathbf{1}_k^T H_k^{-1}\mathbf{1}_k \xlongequal{\text{\reflem{lemma:Hilbert矩阵逆矩阵元素和}}} n^2.
\end{align*}
