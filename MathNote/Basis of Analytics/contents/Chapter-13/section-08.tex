...
\[
f(x)=f(0)+xf' (\eta )\geqslant xf' (\eta )\geqslant x\cdot \min |f' |\Longrightarrow \int_0^1{|f(x)|\mathrm{d}x} \geqslant \frac{1}{2}\min |f' |.
\]
...
\end{align*}
因此$\underset{a\in \mathbb{R} ^n\backslash \left\{ 0 \right\}}{\mathrm{sup}}\frac{a^TJa}{a^THa} = \inf\{\lambda > 0 \mid \lambda H - J\text{半正定}\}$. 设$H_k,J_k$分别为$H,J$的$k$阶主子阵，再设$\lambda> 0$.根据\hyperref[Basis of Algebra-corollary:打洞原理推论]{打洞原理}及\refpro{Basis of Algebra-proposition:一些能写成两个向量乘积的矩阵}可得
\begin{align*}
...
\end{align*}
但是左边暂时不知道是否有$\left(\int_{\mathbb{R}}|f(x)|^2\mathrm{d}x\right)^2 < \infty$，因此不能直接换序. 但是\hyperref[Real Analysis-theorem:Fatou引理]{Fatou引理}告诉我们
\begin{align*}
\left( \int_{\mathbb{R}}|f(x)|^2\mathrm{d}x \right)^2 &= \left( \int_{\mathbb{R}}\varliminf_{n\to\infty}|h_n(x)f(x)|^2\mathrm{d}x \right)^2 \leqslant \varliminf_{n\to\infty}\left( \int_{\mathbb{R}}|h_n(x)f(x)|^2\mathrm{d}x \right)^2 \\
...
\begin{align*}
\varphi(1) &= \int_{0}^{1} f(t)\mathrm{d}t - \int_{0}^{1} G(t)\mathrm{d}t = \int_{0}^{1} f(t)\mathrm{d}t - \left[ \left.tG(t)\right|_{0}^{1} - \int_{0}^{1} tg(t)\mathrm{d}t \right] = \int_{0}^{1} f(t)\mathrm{d}t - G(1) + \int_{0}^{1} tg(t)\mathrm{d}t \\
...
因为
\[
\int_{0}^{1} g(x)\varphi(x)\mathrm{d}x = \left.G(x)\varphi(x)\right|_{0}^{1} - \int_{0}^{1} G(x)\varphi'(x)\mathrm{d}x = -\int_{0}^{1} G(x)\varphi'(x)\mathrm{d}x
\]
所以
\[
...
\begin{example}
设$f(x)$在$[a,b]$上非负且连续可导,证明:
\begin{align*}
\left| \int_a^b f^3(x) \mathrm{d}x - f^2(a) \int_a^b f(x) \mathrm{d}x \right| \leqslant 2 \max\limits_{x\in[a,b]} |f'(x)| \left( \int_a^b f(x) \mathrm{d}x \right)^2.
\end{align*}
\end{example}
\begin{proof}
不妨设$a=0,b=1,\int_a^b{f\left( x \right)}\mathrm{d}x=1,$否则,分别用$f\left( a\left( 1-x \right) +bx \right)$和$\frac{f\left( x \right)}{\int_a^b{f\left( x \right)}}$代替$f\left( x \right).$并记$M=\max\limits_{x\in \left[ 0,1 \right]}\left| f^\prime \left( x \right) \right|.$

{\color{blue}法一:}注意到
\begin{align*}
\int_0^1{f^3\left( x \right) \mathrm{d}x}-f^2\left( 0 \right) \int_0^1{f\left( x \right) \mathrm{d}x}=\int_0^1{f\left( x \right) \left[ f^2\left( x \right) -f^2\left( 0 \right) \right] \mathrm{d}x},
\end{align*}
又由牛顿莱布尼兹公式知
\begin{align*}
f^2\left( x \right) -f^2\left( 0 \right) =\int_0^x{2f\left( t \right) f^\prime \left( t \right) \mathrm{d}t},
\end{align*}
故
\begin{align*}
\left| \int_0^1{f^3\left( x \right) \mathrm{d}x}-f^2\left( 0 \right) \int_0^1{f\left( x \right) \mathrm{d}x} \right|&=2\left| \int_0^1{f\left( x \right) \left( \int_0^x{f\left( t \right) f' \left( t \right) \mathrm{d}t} \right) \mathrm{d}x} \right|=2\left| \int_0^1{\mathrm{d}t\int_t^1{f\left( x \right) f\left( t \right) f' \left( t \right)}\mathrm{d}x} \right|
\\
&=2\left| \int_0^1{f\left( t \right) f' \left( t \right) \left( \int_t^1{f\left( x \right)}\mathrm{d}x \right) \mathrm{d}t} \right|\leqslant 2\left| \int_0^1{f\left( t \right) f' \left( t \right) \left( \int_0^1{f\left( x \right)}\mathrm{d}x \right) \mathrm{d}t} \right|
\\
&=2\int_0^1{f\left( x \right)}\mathrm{d}x\cdot \left| \int_0^1{f\left( t \right) f' \left( t \right) \mathrm{d}t} \right|\leqslant 2M\left( \int_0^1{f\left( x \right)}\mathrm{d}x \right) ^2=2M.
\end{align*}

{\color{blue}法二:}只需证
\begin{align*}
\left| \int_0^1{f^3\left( x \right) \mathrm{d}x}-f^2\left( 0 \right) \right|\leqslant 2M.
\end{align*}
注意到
\begin{align*}
-2M\leqslant 2\int_0^1{f\left( x \right) f^\prime \left( x \right) \mathrm{d}x}\leqslant 2M\Longleftrightarrow -2M\leqslant f^2\left( x \right) -f^2\left( 0 \right) \leqslant 2M,
\end{align*}
上式两边同乘$f\left( x \right)$得
\begin{align*}
-2Mf\left( x \right) \leqslant f^3\left( x \right) -f^2\left( 0 \right) f\left( x \right) \leqslant 2Mf\left( x \right).
\end{align*}
再同时对$x$在$\left[ 0,1 \right]$上积分得
\begin{align*}
-2M\leqslant \int_0^1{f^3\left( x \right) \mathrm{d}x}-f^2\left( 0 \right) \leqslant 2M.
\end{align*}
...
设 \( n \) 是正整数，且 \( m > 2 \)。求证：
\begin{align*}
\int_{0}^{\frac{\pi}{2}} t \left| \frac{\sin nt}{\sin t} \right|^m \, \mathrm{d}t \leqslant \left( \frac{m \cdot n^{m - 2}}{8(m - 2)} - \frac{1}{4(m - 2)} \right) \pi^2.
\end{align*}
\end{example}
\begin{remark}
...
\begin{align*}
\int_{0}^{\frac{\pi}{2}} t \left| \frac{\sin nt}{\sin t} \right|^m \, \mathrm{d}t &= \int_{0}^{a} t \left( \frac{\sin nt}{\sin t} \right)^m \, \mathrm{d}t + \int_{a}^{\frac{\pi}{2}} t \left( \frac{\sin nt}{\sin t} \right)^m \, \mathrm{d}t \\
&\leqslant \int_{0}^{a} tn^m \, \mathrm{d}t + \int_{a}^{\frac{\pi}{2}} t \left( \frac{1}{2t/\pi} \right)^m \, \mathrm{d}t \\
&= \frac{1}{2} n^m a^2 + \frac{1}{m - 2} \left( \frac{\pi}{2} \right)^m \left( \frac{1}{a^{m - 2}} - \frac{1}{(\frac{\pi}{2})^{m - 2}} \right).
\end{align*}
...
\begin{align*}
\frac{1}{\pi} \int_{0}^{\frac{\pi}{2}} \frac{|\sin(2n + 1)t|}{\sin t} \, \mathrm{d}t < \frac{2n^2 + 1}{2n^2 + n} + \frac{1}{2} \ln n.
\end{align*}
\end{example}
\begin{proof}
注意到
\[
\int_{0}^{\frac{\pi}{2}} \frac{|\sin(2n + 1)t|}{\sin t} \, \mathrm{d}t = \int_{0}^{\frac{\pi}{2}n} \frac{|\sin(2n + 1)t|}{\sin t} \, \mathrm{d}t + \int_{\frac{\pi}{2}n}^{\frac{\pi}{2}} \frac{|\sin(2n + 1)t|}{\sin t} \, \mathrm{d}t.
\]
因为当 \( x \in \left( 0, \frac{\pi}{2} \right) \) 时，\( \sin x > \frac{2x}{\pi} \)，所以
\[
\int_{\frac{\pi}{2}n}^{\frac{\pi}{2}} \frac{|\sin(2n + 1)t|}{\sin t} \, \mathrm{d}t < \int_{\frac{\pi}{2}n}^{\frac{\pi}{2}} \frac{1}{2t/\pi} \, \mathrm{d}t = \frac{\pi}{2} \ln n.
\]
另一方面，
\[
\int_{0}^{\frac{\pi}{2}n} \frac{|\sin(2n + 1)t|}{\sin t} \, \mathrm{d}t = \int_{0}^{\pi/(2n + 1)} \frac{\sin(2n + 1)t}{\sin t} \, \mathrm{d}t - \int_{\pi/(2n + 1)}^{\frac{\pi}{2}n} \frac{\sin(2n + 1)t}{\sin t} \, \mathrm{d}t.
\]
...
\end{align*}
\begin{align*}
-\int_{\pi/(2n+1)}^{\frac{\pi}{2}n}{\frac{\sin\mathrm{(}2n+1)t}{\sin t}\,\mathrm{d}t}=-\int_{\pi/(2n+1)}^{\frac{\pi}{2}n}{\left( \frac{\sin 2nt\cos t}{\sin t}+\cos 2nt \right) \,\mathrm{d}t<}-\int_{\pi/(2n+1)}^{\frac{\pi}{2}n}{\cos 2nt\,\mathrm{d}t=\frac{1}{2n}\sin \frac{\pi}{2n+1}.}
\end{align*}
因此
\[
\int_{0}^{\frac{\pi}{2}n} \frac{|\sin(2n + 1)t|}{\sin t} \, \mathrm{d}t < \left( 2n + \frac{1}{n} \right) \sin \frac{\pi}{2n + 1} < \left( 2n + \frac{1}{n} \right) \frac{\pi}{2n + 1} = \frac{2n^2 + 1}{2n^2 + n} \pi.
\]
于是
\[
\int_{0}^{\frac{\pi}{2}} \frac{|\sin(2n + 1)t|}{\sin t} \, \mathrm{d}t < \frac{2n^2 + 1}{2n^2 + n} \pi + \frac{\pi}{2} \ln n.
\]
两边同时除以 \( \pi \) 得：
\[
\frac{1}{\pi} \int_{0}^{\frac{\pi}{2}} \frac{|\sin(2n + 1)t|}{\sin t} \, \mathrm{d}t < \frac{2n^2 + 1}{2n^2 + n} + \frac{1}{2} \ln n.
...
\int_{a+u}^b{|f(v)|\,\mathrm{d}v,}&		u\geqslant 0,\\
...
\begin{example}
设$f\in C^1[0,1],f(0)=0,\int_0^1 f(x)\mathrm{d}x=0.$
\begin{align*}
\int_0^1 f^2(x)\mathrm{d}x\leqslant\frac{1}{6}\int_0^1 f'^2(t)\mathrm{d}t.
\end{align*}
\end{example}
\begin{remark}
这个不等式的最佳系数见\hyperref[theorem:Fourier型积分不等式]{Fourier型积分不等式}.
\end{remark}
\begin{proof}
由分部积分可知
\begin{align*}
\int_0^1 (1-t)f'(t)\mathrm{d}t=\int_0^1 f(t)\mathrm{d}t=0.
\end{align*}
待定$c\in\mathbb{R}$,由牛顿莱布尼兹公式和Cauchy公式知
\begin{align*}
f^2(x)&=\left( \int_0^x f'(t)\mathrm{d}t \right) ^2=\left[ \int_0^1 \left( \chi_{[0,x]}(t)-c(1-t) \right) f'(t)\mathrm{d}t \right] ^2\\
&\leqslant \int_0^1 \left( \chi_{[0,x]}(t)-c(1-t) \right) ^2\mathrm{d}t\int_0^1 f'^2(t)\mathrm{d}t\\
&=\left[ \int_0^x \left( 1-c(1-t) \right) ^2\mathrm{d}t+\int_x^1 c^2(1-t)^2\mathrm{d}t \right] \int_0^1 f'^2(t)\mathrm{d}t\\
&=\left( cx^2+(1-2c)x+\frac{c^2}{3} \right) \int_0^1 f'^2(t)\mathrm{d}t.
\end{align*}
上式两边对$x$在$[0,1]$上积分得
\begin{align*}
\int_0^1 f^2(x)\mathrm{d}x&\leqslant \int_0^1 \left[ \left( cx^2+(1-2c)x+\frac{c^2}{3} \right) \int_0^1 f'^2(t)\mathrm{d}t \right] \mathrm{d}x=\left( \frac{1}{2}-\frac{2}{3}c+\frac{c^2}{3} \right) \int_0^1 f'^2(t)\mathrm{d}t.
\end{align*}
注意到
\begin{align*}
\min\limits_{c\in\mathbb{R}}\left\{ \frac{1}{2}-\frac{2}{3}c+\frac{c^2}{3} \right\} =\frac{1}{6},
\end{align*}
故取$c=1$得
\begin{align*}
\int_0^1 f^2(x)\mathrm{d}x\leqslant\frac{1}{6}\int_0^1 f'^2(t)\mathrm{d}t.
