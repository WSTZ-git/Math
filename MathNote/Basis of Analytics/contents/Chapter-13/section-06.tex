...
\begin{align*}
|f(x)|\leqslant \sqrt{\frac{\pi}{3}\int_0^{\pi}{|f\prime (t)|^2dt}},\forall x\in [0,\pi ].
\end{align*} 
\end{example}
\begin{remark}
原不等式等价于
\begin{align*}
f^2(x) \leqslant \frac{\pi}{3}\int_0^{\pi} |f'(t)|^2dt, \forall x\in [0, \pi].
\end{align*}
显然要利用Cauchy不等式，先待定\(g(x)\)，由Cauchy不等式可得
\begin{align}
\int_0^{\pi} |f'(t)|^2dt\int_0^{\pi} g^2(t) dt\geqslant \left( \int_0^{\pi} f'(t)g(t) dt \right) ^2, \forall x\in [0, \pi]. \label{eq:101.2}
\end{align}
此时，我们希望对\(\forall x\in [0, \pi]\)，固定\(x\)，都有\(\int_0^{\pi} f'(t)g(t) dt = kf(x)\)，其中\(k\)为某一常数。因此\(g(t)\)必和\(x\)有关，于是令
\[g(t) = 
\begin{cases}
t - \pi, & t\in [x, \pi]\\
t, & t\in [0, x]
...
希望 \(\int_0^{\pi} f(t)g'(t) dt = f(x)\)，考虑广义导数，使得 \(g'(x) = \delta(x)\)。实际上，这里的 \(g\) 就是 \(H\) 函数(详细参考rudin的泛函分析).
\end{remark}
\begin{proof}
令
\[g(t) = 
...
\begin{align*}
\left( \int_0^{\pi} f'(t)g(t) dt \right) ^2&=\left( \int_x^{\pi} (t - \pi)f'(t) dt+\int_0^x tf'(t) dt \right) ^2\\
&\xlongequal{\text{分部积分}}\left( -(x - \pi)f'(x) - \int_x^{\pi} f(t) dt+xf(x) - \int_0^x f(t) dt \right) ^2\\
&=\pi ^2|f(x)|^2
\end{align*}
\begin{align*}
\int_0^{\pi} g^2(t) dt=\int_x^{\pi} (t - \pi)^2dt+\int_0^x t^2dt=\frac{\pi}{3}(3x^2 - 3\pi x + \pi ^2)
\end{align*}
故由Cauchy不等式可得
\begin{align*}
\frac{\pi}{3}(3x^2 - 3\pi x + \pi ^2)\int_0^{\pi} |f'(t)|^2dt=\int_0^{\pi} |f'(t)|^2dt\int_0^{\pi} g^2(t) dt
\geqslant \left( \int_0^{\pi} f'(t)g(t) dt \right) ^2=\pi ^2|f(x)|^2, \forall x\in [0, \pi]
\end{align*}
即
\begin{align*}
|f(x)|^2\leqslant \frac{1}{3\pi}(3x^2 - 3\pi x + x^2)\int_0^{\pi} |f'(t)|^2dt
\leqslant \frac{\pi}{3}\int_0^{\pi} |f'(t)|^2dt, \forall x\in [0, \pi]
\end{align*}
\end{proof}
...
\begin{align*}
\iint_{[0, +\infty)} \frac{f(x)g(y)}{(\sqrt{x} + \sqrt{y})^2} dxdy \leqslant 2 \sqrt{\int_0^\infty f^2(x) dx \int_0^\infty g^2(x) dx}.
\end{align*}
