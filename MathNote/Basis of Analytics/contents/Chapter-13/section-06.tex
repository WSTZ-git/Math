...
\begin{example}
若 $f\in C^1[a,b],f(a)=0$。证明：
\begin{align*}
\int_{a}^{b} f^{2}(x)\mathrm{d}x \leqslant \frac{(b-a)^{2}}{2}\int_{a}^{b} [f'(x)]^{2}\mathrm{d}x - \frac{1}{2}\int_{a}^{b} [f'(x)]^{2}(x-a)^{2}\mathrm{d}x.
\end{align*}
\end{example}
\begin{proof}
不妨设$a=0,b=1$，否则用$f(a(1-x)+bx)$代替$f(x)$即可。此时只需证明
\begin{align*}
\int_{0}^{1} f^2(x)\mathrm{d}x \leqslant \frac{1}{2}\int_{0}^{1} \left[f'(x)\right]^2\mathrm{d}x - \frac{1}{2}\int_{0}^{1} x^2\left[f'(x)\right]^2\mathrm{d}x = \frac{1}{2}\int_{0}^{1} (1-x^2)\left[f'(x)\right]^2\mathrm{d}x.
\end{align*}
由\hyperref[theorem:Newton-Leibniz公式]{Newton-Leibniz公式}和\hyperref[theorem:Cauchy积分不等式]{Cauchy积分不等式}知
\begin{align*}
f^2(x) = \left( \int_{0}^{x} f'(t)\mathrm{d}t \right)^2 \leqslant x\int_{0}^{x} \left[f'(t)\right]^2\mathrm{d}t,\quad \forall x\in [0,1].
\end{align*}
对上式两边积分得
\begin{align*}
\int_{0}^{1} f^2(x)\mathrm{d}x \leqslant \int_{0}^{1} \left( x\int_{0}^{x} \left[f'(t)\right]^2\mathrm{d}t \right)\mathrm{d}x = \int_{0}^{1} \left( \int_{t}^{1} x\left[f'(t)\right]^2\mathrm{d}x \right)\mathrm{d}t = \int_{0}^{1} \frac{1-t^2}{2}\left[f'(t)\right]^2\mathrm{d}t.
\end{align*}
\end{proof}
...
于是
\begin{align*}
\int_{a}^{b}|f(x)f'(x)|\mathrm{d}x \leqslant \int_{a}^{b}F(x)F'(x)\mathrm{d}x = \left.\frac{1}{2}F^2(x)\right|_{a}^{b} = \frac{1}{2}F^2(b) = \frac{1}{2}\left( \int_{a}^{b}|f'(y)|\mathrm{d}x \right)^2 \\
...
于是
\begin{align*}
\int_{a}^{b}|f(x)|^{p}|f'(x)|^{q}\mathrm{d}x &\leqslant \int_{a}^{b}F^{\frac{p}{q}}(x)(x - a)^{\frac{p(q - 1)}{q}}|f'(x)|^{q}\mathrm{d}x = \int_{a}^{b}F^{\frac{p}{q}}(x)(x - a)^{\frac{p(q - 1)}{q}}\mathrm{d}F(x)\\
&\leqslant (b - a)^{\frac{p(q - 1)}{q}}\int_{a}^{b}F^{\frac{p}{q}}(x)\mathrm{d}F(x) = \frac{q}{q + p}(b - a)^{\frac{p(q - 1)}{q}}F^{\frac{p + q}{q}}(b)\\
&=\frac{q}{q + p}(b - a)^{\frac{p(q - 1)}{q}}\left(\int_{a}^{b}|f'(y)|^{q}\mathrm{d}y\right)^{\frac{p + q}{q}}\\
&\stackrel{\text{\hyperref[theorem:Hölder(赫尔德)不等式(积分形式)]{Hölder不等式}}}{\leqslant} \frac{q}{q + p}(b - a)^{\frac{p(q - 1)}{q}}\left(\int_{a}^{b}|f'(y)|^{q(\frac{p + q}{q})}\mathrm{d}y\right)^{\frac{q}{q + p}}\left(\int_{a}^{b}1^{(\frac{p + q}{q - 1})}\mathrm{d}y\right)^{\frac{q - 1}{q + p}}\\
...
\begin{align*}
\left|f(x)\right| = \left|f(a) + f'(\theta_1)(x - a) \right|\leqslant M(x - a), \forall x \in \left[a, \frac{a + b}{2}\right].
\end{align*}
\begin{align*}
\left|f(x)\right| = \left|f(b) + f'(\theta_2)(x - b)\right| \leqslant M(b - x), \forall x \in \left[\frac{a + b}{2}, b\right].
\end{align*}
从而
\begin{align*}
...
即
\begin{align*}
|f(x)|^2\leqslant \frac{1}{3\pi}(3x^2 - 3\pi x + \pi^2)\int_0^{\pi} |f'(t)|^2\mathrm{d}t
...
\Longleftrightarrow \int_0^1 \left| f''(x) \right|^2 \, \mathrm{d}x \geqslant \frac{g^2(1)}{\int_0^1 g^2(x) \, \mathrm{d}x}. \label{eq:100.111111qfsgdsgdfsgd7}
\end{gather}
因此只要 \(g(x)\) 还满足 \(\frac{g^2(1)}{\int_0^1 g^2(x) \, \mathrm{d}x} \geqslant 4\) 即可。

因为 \(g''(x) \equiv 0\)，所以我们可以设 \(g(x)\) 为一次函数，即 \(g(x) = ax + b\)，\(a \ne 0\)。又因为 \(\frac{g^2(1)}{\int_0^1 g^2(x) \, \mathrm{d}x}\) 越大，不等式 \eqref{eq:100.111111qfsgdsgdfsgd7} 越强，所以现在我们想要找到一个一次函数 \(g(x)\) 使得 \(\frac{g^2(1)}{\int_0^1 g^2(x) \, \mathrm{d}x}\) 达到最大值。
...
0 \\
1
\end{pmatrix}. \label{eq:qfefds34fzsdf100.8}
\end{align}
注意到上述方程的系数行列式为
\begin{align*}
\begin{vmatrix}
...
= f(0) + f(2) - 2f(1) \ne 0.
\end{align*}
故由 Cramer 法则可知，存在唯一的解 \(a = a_0\)，\(b = b_0\)，\(c = c_0\) 满足方程组 \eqref{eq:qfefds34fzsdf100.8}。即 \(g(x) = c_0 f(x) - a_0 x - b_0\) 满足 \(g(0) = g(2) = 0\)，\(g(1) = 1\)。
...
\end{align}
即对 \(t = \int_0^1 f(x) \, \mathrm{d}x \in \mathbb{R}\)，找到一个 \(c\)，记 \(K = \frac{3}{3c^2 + 3c + 1} \in \mathbb{R}\)，使得
\begin{align*}
