...
& \xlongequal{\text{对称性}} \iint_{[a,b]^2} p(y) p(x) g(x) [f(y) - f(x)] \mathrm{d}y \mathrm{d}x \\
...
\\
&\stackrel{\hyperref[theorem:Cauchy积分不等式]{\text{Cauchy不等式}}}{\leqslant } \int_{0}^{1}|f(x) - A|^2\mathrm{d}x \cdot \int_{0}^{1}|g(x) - B|^2\mathrm{d}x
\\
