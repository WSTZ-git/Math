...
{\color{blue}证法一:}对任意固定的$t\in(0,1)$，切线$L(x)=f(t)+f'(t)(x-t)$满足$f(x)\geqslant L(x)$对所有$x\in[0,1]$成立。分别在$[0,t]$和$[t,1]$上积分得
\begin{align*}
\int_{0}^{t}f(x)\mathrm{d}x&\geqslant\int_{0}^{t}\left[f(t)+f'(t)(x-t)\right]\mathrm{d}x=tf(t)-\frac{t^{2}}{2}f'(t),\\
\int_{t}^{1}f(x)\mathrm{d}x&\geqslant\int_{t}^{1}\left[f(t)+f'(t)(x-t)\right]\mathrm{d}x=(1-t)f(t)+\frac{(1-t)^{2}}{2}f'(t).
\end{align*}
整理得
\begin{align*}
\frac{2}{t^{2}}\left(tf(t)-\int_{0}^{t}f\right)\leqslant f'(t)\leqslant\frac{2}{(1-t)^{2}}\left(\int_{t}^{1}f-(1-t)f(t)\right).
\end{align*}
结合两个不等式消去$f'(t)$即得
\begin{align*}
...
\begin{proposition}\label{proposition:凸性不等式111111}
设$f$是$[0,1]$上的下凸函数，满足$f(0)=0$且$f(x)\geqslant 0$，证明
\begin{align}\label{eq:::2890fjw3j890ejei490dke90iyw3490-gudjgh89jrhgdkf90gk0etge43756i78opaioopp21.2}
\int_0^1{xf\left( x \right) \mathrm{d}x}\geqslant \frac{2}{3}\int_0^1{f\left( x \right) \mathrm{d}x}.
\end{align}
\end{proposition}
\begin{note}
证明思路:待定二次函数$g(x)=ax^2+bx+c$，则期望
\begin{align}\label{eq:::2890fjw3j890ejei490dke90iyw3490-gudjgh89jrhgdkf90gk0etge43756i78opaioopp21.3}
a\geqslant 0;\quad g(1)=0;\quad g(x)\text{在}[0,t]\text{上递增，在}[t,1]\text{上递减};\quad \int_0^1{g\left( x \right) \mathrm{d}x}=0.
\end{align}
于是
\begin{align*}
\int_0^1{g\left( x \right) f'\left( x \right) \mathrm{d}x}\xlongequal{\text{分部积分}}f\left( 1 \right) g\left( 1 \right) -\int_0^1{g'\left( x \right) f\left( x \right) \mathrm{d}x}=-\int_0^1{g'\left( x \right) f\left( x \right) \mathrm{d}x}.
\end{align*}
又由$f$在$[0,1]$上下凸知，$f'$在$[0,1]$上递增。故$g(x)f'(x)$在$[0,1]$上递增，在$[t,1]$上递减。因此
\begin{align*}
\int_0^1{g\left( x \right) f'\left( x \right) \mathrm{d}x}&=\int_0^t{g\left( x \right) f'\left( x \right) \mathrm{d}x}+\int_t^1{g\left( x \right) f'\left( x \right) \mathrm{d}x} \\
&\leqslant f'\left( t \right) \int_0^t{g\left( x \right) \mathrm{d}x}+f'\left( t \right) \int_t^1{g\left( x \right) \mathrm{d}x} \\
&=f'\left( t \right) \int_0^1{g\left( x \right) \mathrm{d}x}=0.
\end{align*}
进而
\begin{align*}
&\quad \quad \,\, \int_0^1{g\left( x \right) f' \left( x \right) \mathrm{d}x}=-\int_0^1{g' \left( x \right) f\left( x \right) \mathrm{d}x}\leqslant 0
\\
&\Longleftrightarrow \int_0^1{\left( 2ax+b \right) f\left( x \right) \mathrm{d}x}=\int_0^1{g' \left( x \right) f\left( x \right) \mathrm{d}x}\geqslant 0
\\
&\Longleftrightarrow -\frac{b}{2a}\int_0^1{f\left( x \right) \mathrm{d}x}\leqslant \int_0^1{xf\left( x \right) \mathrm{d}x}.
\end{align*}
由\eqref{eq:::2890fjw3j890ejei490dke90iyw3490-gudjgh89jrhgdkf90gk0etge43756i78opaioopp21.2}式知，我们期望$-\frac{b}{2a}=\frac{2}{3}$。又因为我们期望\eqref{eq:::2890fjw3j890ejei490dke90iyw3490-gudjgh89jrhgdkf90gk0etge43756i78opaioopp21.3}式成立，所以
\begin{align*}
g\left( x \right) =a\left( x-1 \right) \left( x-t \right) =ax^2-a\left( 1+t \right) x+at\Longrightarrow \frac{2}{3}=-\frac{b}{2a}=\frac{1+t}{2}\Longrightarrow t=\frac{1}{3}.
\end{align*}
此时
\begin{align*}
\int_0^1{g\left( x \right) \mathrm{d}x}=a\int_0^1{\left( x^2-\frac{4}{3}x+\frac{1}{3} \right) \mathrm{d}x}=0.
\end{align*}
现在只要任取$a\geqslant 0$，$g(x)=a\left( x^2-\frac{4}{3}x+\frac{1}{3} \right)$就能满足上述所有期望，进而就能完成证明。
\end{note}
\begin{proof}

令$g(x)=3x^2-4x+1$，则$g(x)$在$[0,\frac{1}{3}]$上递增，在$[\frac{1}{3},1]$上递减。由$f$在$[0,1]$上下凸知，$f'$在$[0,1]$上递增。故$g(x)f'(x)$在$[0,1]$上递增，在$[t,1]$上递减。因此
\begin{align*}
\int_0^1{g\left( x \right) f'\left( x \right) \mathrm{d}x}&=\int_0^{\frac{1}{3}}{g\left( x \right) f'\left( x \right) \mathrm{d}x}+\int_{\frac{1}{3}}^1{g\left( x \right) f'\left( x \right) \mathrm{d}x} \\
&\leqslant f'\left( \frac{1}{3} \right) \int_0^{\frac{1}{3}}{g\left( x \right) \mathrm{d}x}+f'\left( \frac{1}{3} \right) \int_{\frac{1}{3}}^1{g\left( x \right) \mathrm{d}x} \\
&=f'\left( \frac{1}{3} \right) \int_0^1{g\left( x \right) \mathrm{d}x}=0.
\end{align*}
再利用分部积分得
\begin{align*}
0\geqslant \int_0^1{g\left( x \right) f'\left( x \right) \mathrm{d}x}=-\int_0^1{g'\left( x \right) f\left( x \right) \mathrm{d}x}\Longleftrightarrow \int_0^1{\left( 6x-4 \right) f\left( x \right) \mathrm{d}x}\geqslant 0\Longleftrightarrow \int_0^1{xf\left( x \right) \mathrm{d}x}\geqslant \frac{2}{3}\int_0^1{f\left( x \right) \mathrm{d}x}.
\end{align*}
...
设$f$是$[0,1]$上的下凸函数，满足$f(0)=0$且$f(x)\geqslant 0$，证明
\begin{align*}
\left( \int_0^1{f\left( x \right) \mathrm{d}x} \right) ^2\leqslant \frac{3}{4}\int_0^1{f^2\left( x \right) \mathrm{d}x}.
\end{align*}
\end{example}
\begin{proof}

由\refpro{proposition:凸性不等式111111}知
\begin{align*}
\int_0^1{xf\left( x \right) \mathrm{d}x}\geqslant \frac{2}{3}\int_0^1{f\left( x \right) \mathrm{d}x}.
\end{align*}
再利用Cauchy不等式可得
\begin{align*}
\left( \int_0^1{f\left( x \right) \mathrm{d}x} \right) ^2\leqslant \left( \frac{3}{2}\int_0^1{xf\left( x \right) \mathrm{d}x} \right) ^2\leqslant \frac{9}{4}\int_0^1{x^2\mathrm{d}x}\int_0^1{f^2\left( x \right) \mathrm{d}x}=\frac{3}{4}\int_0^1{f^2\left( x \right) \mathrm{d}x}.
\end{align*}
