...
设$\alpha>0,f\in C^1(\mathbb{R})$，且存在$a\in \mathbb{R}$,使得$f(a)=\underset{x\in \mathbb{R}}{\min}f\left( x \right)$.若
\begin{align*}
\left| f' \left( x \right) -f' \left( y \right) \right|\leqslant M\left| x-y \right|^{\alpha},\quad \forall x,y\in \mathbb{R} ,
\end{align*}
其中$M$是常数.则
\begin{align}\label{eq::20jrewjf90j23raesfdzcgdfg}
\left| f' \left( x \right) \right|^{\alpha +1}\leqslant M\left( \frac{\alpha +1}{\alpha} \right) ^{\alpha}\left| f\left( x \right) -f\left( y \right) \right|^{\alpha},\quad \forall x,y\in \mathbb{R} ,
\end{align}
\end{proposition}
\begin{note}
可以先待定$h$,最后再确定$h$的取值.
\end{note}
\begin{proof}
不妨设$f(a)=0$，否则用$f(x) - f(a)$代替$f(x)$. 当$M = 0$，则不等式\eqref{eq::20jrewjf90j23raesfdzcgdfg}显然成立. 当$M\neq0$可以不妨设$M = 1$,否则用$\frac{f}{M}$代替$f$.
现在对非负函数$f$,任取$\forall x \in \mathbb{R}$，固定$x$.

（i）若$f'(x) = 0$，则结论显然成立.

（ii）若$f'(x) < 0$，则令$h = \left( -f'(x) \right)^{\frac{1}{\alpha}} > 0$. 由Newton-Leibniz公式可得
\begin{align*}
0 &\leqslant f(x + h) = f(x) + \int_x^{x + h} f'(t) \mathrm{d}t = f(x) + \int_x^{x + h} \left[ f'(t) - f'(x) \right] \mathrm{d}t + f'(x) h \\
...
于是
$$
-\left[ f'(x) - \frac{1}{\alpha + 1} f'(x) \right] \left( -f'(x) \right)^{\frac{1}{\alpha}} \leqslant f(x)\Longleftrightarrow \left( -f'(x) \right)^{\frac{\alpha+1}{\alpha}} \leqslant \frac{\alpha + 1}{\alpha} f(x).
$$
从而
$$
\left| f'(x) \right|^{\frac{\alpha + 1}{\alpha}} \leqslant \frac{\alpha + 1}{\alpha} |f(x)|.
$$

（iii）若$f'(x) > 0$，则令$h = \left( f'(x) \right)^{\frac{1}{\alpha}} > 0$. 由Newton-Leibniz公式可得
\begin{align*}
0 &\leqslant f(x - h) = -\int_{x - h}^x f'(t) \mathrm{d}t + f(x) = \int_{x - h}^x \left[ f'(x) - f'(t) \right] \mathrm{d}t + f(x) - f'(x) h \\
