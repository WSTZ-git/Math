...
\begin{theorem}[可微性]\label{theorem:含参反常积分的可微性}
设 \( f(x,y) \) 与 \( f_x(x,y) \) 在区域 \( I \times [c, +\infty) \) 上连续。若 \( \Phi(x) = \int_{c}^{+\infty} f(x,y) \, \mathrm{d}y \) 在 \( I \) 上收敛，\( \int_{c}^{+\infty} f_x(x,y) \, \mathrm{d}y \) 在 \( I \) 上(内闭)一致收敛，则 \( \Phi(x) \) 在 \( I \) 上可微，且
...
设 \( f(x,y) \) 在 \( [a,b] \times [c, +\infty) \) 上连续，若 \( \Phi(x) = \int_{c}^{+\infty} f(x,y) \, \mathrm{d}y \) 在 \( [a,b] \) 上(内闭)一致收敛，则 \( \Phi(x) \) 在 \( [a,b] \) 上可积，且
