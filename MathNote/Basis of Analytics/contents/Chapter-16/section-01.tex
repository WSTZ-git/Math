...
\begin{theorem}[Leibniz(莱布尼兹)判别法]\label{theorem:Leibniz(莱布尼兹)判别法}
...
从而对$\forall n\geqslant N$，有
\begin{align*}
a_n&=a_N\prod_{k=N}^{n-1}{\frac{a_{k+1}}{a_k}}=a_N\exp \left\{ \sum_{k=N}^{n-1}{\ln \frac{a_{k+1}}{a_k}} \right\} >a_N\exp \left\{ \sum_{k=N}^{n-1}{\frac{\mu}{k}} \right\} \geqslant a_N\exp \left\{ \sum_{k=N}^{n-1}{\int_{k-1}^k{\frac{\mu}{x}\mathrm{d}x}} \right\} 
\\
&=a_N\exp \left\{ \int_{N-1}^{n-1}{\frac{\mu}{x}\mathrm{d}x} \right\} =a_Ne^{\mu \ln \frac{n-1}{N-1}}=\frac{a_N}{(N-1)^{\mu}}(n-1)^{\mu}.
\end{align*}
...
延续\refdef{definition:Cauchy积},我们有
\begin{enumerate}
\item 若$\sum\limits_{n=0}^{\infty} a_n,\sum\limits_{n=0}^{\infty} b_n$有一个绝对收敛,则$\sum\limits_{n=0}^{\infty} c_n$收敛.

\item 若$\sum\limits_{n=0}^{\infty} a_n,\sum\limits_{n=0}^{\infty} b_n$都绝对收敛,则$\sum\limits_{n=0}^{\infty} c_n$绝对收敛.
\end{enumerate}
\end{theorem}
\begin{proof}
\begin{enumerate}
