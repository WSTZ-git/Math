...
$\mathbf{Step}\,\,\mathbb{2}$ 当 \(\eqref{eq::18.9--1}\) 成立，再利用\eqref{eq:::18.11---11}式,存在 \( c > 1, N \in \mathbb{N} \) 使得
\[
...
$\mathbf{Step}\,\,\mathbb{3}$ 若存在 \( c \leqslant 1, N \in \mathbb{N} \) 使得
\[
\frac{\ln \frac{F'(n)}{a_n}}{\ln F(n)} \leqslant c, F(n) \geqslant 1, \forall n \geqslant N.
\]
根据$\mathbf{Step}\,\,\mathbb{2}$，同样的我们有 \(\frac{F'(n)}{F(n)} \leqslant \frac{F'(n)}{F^c(n)} \leqslant a_n, \forall n \geqslant N\) 以及由\hyperref[theorem:积分判别法]{积分判别法}有
\[
