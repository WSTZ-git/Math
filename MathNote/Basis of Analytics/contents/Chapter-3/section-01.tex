...
根据闭区间套定理,可知存在唯一\(\xi=\lim_{n\rightarrow\infty}a_n=\lim_{n\rightarrow\infty}b_n\),且\(\xi\in [a_n,b_n]\),\(\forall n\in\mathbb{N}_+\).又由\(f(x)\)在\([0,1]\)上单调递增及\(f(a_n)>a_n\),\(f(b_n)<b_n\),\(\forall n\in\mathbb{N}_+\),可知\(a_n<f(a_n)\leqslant slant f(\xi)\leqslant slant f(b_n)<b_n\).令\(n\rightarrow\infty\)可得\(\xi\leqslant slant f(\xi)\leqslant slant\xi\),即\(f(\xi)=\xi\).这与假设矛盾.
\end{proof}
...
\item {\color{blue}(聚点定理):}(反证法)假设$f(x)$在$[a,b]$上无界,则对$\forall n>0$,都存在$x_n\in[a,b]$,使得$\left| f\left( x_n \right) \right|>n$.从而得到一个有界数列$\{x_n\}$.由聚点定理,可知其存在收敛子列$\{x_{n_k}\}$,设$\underset{k\rightarrow \infty}{\lim}x_{n_k}=x_0$.由条件可知,存在\(\delta _{x_0}>0\)以及\(M_{x_0}\),使得\(\vert f(y)\vert\leqslant slant M_{x_0}\),\(\forall y\in(x_0 - \delta _{x_0},x_0+\delta _{x_0})\).
又由\(\lim_{k\rightarrow\infty}x_{n_k}=x_0\)可知,存在\(K > M_{x_0}\),使得\(\vert x_{n_K}-x_0\vert<\delta _{x_0}\),即\(x_{n_K}\in(x_0 - \delta _{x_0},x_0+\delta _{x_0})\).于是\(\vert f(x_{n_K})\vert\leqslant slant M_{x_0}\).
而\(\vert f(x_{n_K})\vert>n_K\geqslant slant K > M_{x_0}\)矛盾.

\item {\color{blue}(闭区间套定理):}(反证法)假设存在$a,b\in \mathbb{R}$,使得$f(a)\geqslant  f(b)$.记闭区间\([a,b]=[a_1,b_1]\),若\(f\left(\frac{a_1 + b_1}{2}\right) \leqslant slant f\left(a_1\right)\),则记闭区间\(\left[a_1,\frac{a_1 + b_1}{2}\right]=[a_2,b_2]\);若\(f\left(\frac{a_1 + b_1}{2}\right) \geqslant slant f\left(b_1\right)\),则记闭区间\(\left[\frac{a_1 + b_1}{2},b_1\right]=[a_2,b_2]\).以此类推,可以得到一列闭区间\(\{[a_n,b_n]\}\),满足\([a_n,b_n]\subset [a_{n + 1},b_{n + 1}]\),\(f(a_n)\geqslant slant f(b_n)\),\(\forall n\in \mathbb{N}_+\).
...
又由\(\xi = \lim_{n\rightarrow \infty} a_n = \lim_{n\rightarrow \infty} b_n\)可知,存在\(N > 0\),使得\(\left|a_N - \xi\right| < \delta\),\(\left|b_N - \xi\right| < \delta\),即\(a_N,b_N\in \left(\xi - \delta,\xi + \delta\right)\),且\(a_N < b_N\).于是\(f\left(a_N\right) \leqslant slant f\left(b_N\right)\).而\(f\left(a_N\right) \geqslant slant f\left(b_N\right)\),这就产生了矛盾.
\end{enumerate}
\end{proof}
...
\[
A = \{f(x)|\exists\delta > 0,\forall t\in(x - \delta,x + \delta),f(t)\geqslant slant f(x)\}.
\]
故对\(\forall y\in A\),都存在\(x\in I\),使得\(y = f(x)\),并且\(\exists\delta > 0,\forall t\in(x - \delta,x + \delta),f(t)\geqslant slant f(x)\).由有理数的稠密性可知,存在\(r\in(x - \delta,x)\cap\mathbb{Q}\),\(s\in(x,x + \delta)\cap\mathbb{Q}\).从而\((r,s)\subset(x - \delta,x + \delta)\),于是对\(\forall t\in(r,s)\),同样有\(f(t)\geqslant slant f(x)\).

再设全体有理开区间构成的集合为\(B\),现在定义一个映射
\[
\varphi:A\longrightarrow B;\quad y\longmapsto(r,s).
\]
任取\(y_1,y_2\in A\)且\(y_1\neq y_2\),则存在\(x_1,x_2\in I\),使得\(f(x_1)=y_1\),\(f(x_2)=y_2\).假设\(\varphi(y_1)=\varphi(y_2)=(r_0,s_0)\),则对\(\forall t\in(r_0,s_0)\),都有\(f(t)\geqslant slant y_1,y_2\).于是\(y_1 = f(x_1)\geqslant slant y_2\),\(y_2 = f(x_2)\geqslant slant y_1\),从而\(y_1 = y_2\),这产生了矛盾.故\(\varphi(y_1)\neq\varphi(y_2)\),因此\(\varphi\)是单射.
...
{\color{blue}证法二(闭区间套定理):}假设$f(x)$不是常值函数,则存在$a_1,b_1 \in I$,使得$f(a_1)\ne f(b_1)$.不妨设$f(a_1)<f(b_1)$.因为\(f\)在\(I\)上连续,所以由介值定理可知,存在\(c_1\in [a_1,b_1]\),使得\(f(a_1) < f(c_1)=\frac{f(a_1) + f(b_1)}{2}<f(b_1)\).若\(b_1 - c_1\leqslant slant\frac{b_1 - a_1}{2}\),则令\([a_2,b_2]=[c_1,b_1]\);若\(c_1 - a_1\leqslant slant\frac{b_1 - a_1}{2}\),则令\([a_2,b_2]=[a_1,c_1]\).无论哪种情况,都有\(f(a_2) < f(b_2)\).
...
\]
使得\([x_n - \delta_n,x_n + \delta_n]\cap A_n=\varnothing,\delta_n < \frac{1}{n}(n\geqslant slant1)\).
根据闭区间套原理, 存在\(\xi\in[x_n - \delta_n,x_n + \delta_n],\forall n\geqslant slant1\). 因此\(\xi\notin\bigcup_{n\geqslant slant1}A_n = A\),
...
\begin{example}
设数列\(a_n\)单调递增趋于正无穷,并且\(\lim_{n \to \infty} \frac{a_{n + 1}}{a_n} \leqslant slant 1\),函数\(f(x)\)定义在\((0,+\infty)\)中且对任意\(x\geqslant 1\)都有\(\lim_{n \to \infty} f(a_nx) = 0\).
\begin{enumerate}
