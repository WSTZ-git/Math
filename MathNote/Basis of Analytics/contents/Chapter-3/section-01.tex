...
\begin{proposition}\label{proposition:确界不在集合中必存在严格单调数列}
设$E$是非空集合且$m=\inf\limits_{}E$，$M=\sup\limits_{}E\notin E$，则存在$\left\{ x_n \right\} \subseteq E$严格递减趋于$m$，$\left\{ y_n \right\} \subseteq E$严格递增趋于$M$。
\end{proposition}
\begin{note}
...
\begin{proposition}
设$f$是$\mathbb{R}$上非常数的周期函数，且$f$在某一点$x_0$处存在单侧极限（有限或无穷），则$f$必有最小正周期。
\end{proposition}
\begin{note}
若条件改为$f$是$\mathbb{R}$上非常数的连续周期函数,与下述证明一样定义$\mu,T_n$,那么可以直接利用连续性得到
\begin{align*}
f\left( x+\mu \right) =f\left( x+\underset{n\rightarrow \infty}{\lim}T_n \right) =\underset{n\rightarrow \infty}{\lim}f\left( x+T_n \right) =f\left( x \right) .
\end{align*}
进而$\mu$就是$f$的周期,若$\mu =0$,则$f$为常函数,矛盾!即证.
\end{note}
\begin{proof}
不妨设$f\left( x \right)$在$x_0$处存在左侧极限（有限或无穷）。记
\begin{align*}
E=\left\{ T>0\mid f\left( x \right) =f\left( x+T \right) ,\forall x\in \mathbb{R} \right\},
\end{align*}
显然$E$有下界$0$，由确界原理知$E$存在下确界，记$\inf\limits_{}E=\mu$。下面先证$\mu >0$。

\begin{enumerate}[(i)]
\item 当$\lim\limits_{x\rightarrow x_{0}^{-}}f\left( x \right) =+\infty$或$-\infty$时，由$f$在$\mathbb{R}$上非常数知，存在$x\in \mathbb{R}$，使得$\left| f\left( x \right) \right|\ne 0$。此时存在$\delta >0$，使得
\begin{align*}
\left| f\left( y \right) \right|>\left| f\left( x \right) \right|,\quad \forall x\in \left( x_0-\delta ,x_0 \right) .
\end{align*}
故此时必有$\mu \geqslant \delta >0$。

\item 当$\lim\limits_{x\rightarrow x_{0}^{-}}f\left( x \right) =a<+\infty$时，若$\mu =0\notin E$，则由\refpro{proposition:确界不在集合中必存在严格单调数列}知，存在$\left\{ T_n \right\} \subseteq E$严格递减趋于$0$。对$\forall n\in \mathbb{N}$，$y\in \mathbb{R}$，取
\begin{align*}
m_n=\lfloor \frac{y-x_0}{T_0} \rfloor +1,\quad x_n=y-m_nT_n,
\end{align*}
从而
\begin{align*}
\lim\limits_{n\rightarrow \infty}x_n=\lim\limits_{n\rightarrow \infty}\left( y-m_nT_n \right) =x_0,\,\,\text{且}x_n=y-m_nT_n\leqslant x_0.
\end{align*}
于是由Henie归结原则和$f$的周期性知
\begin{align*}
f\left( y \right) =\lim\limits_{n\rightarrow \infty}f\left( y-m_nT_n \right) =\lim\limits_{n\rightarrow \infty}f\left( x_n \right) =\lim\limits_{x\rightarrow x_{0}^{-}}f\left( x \right) =a.
\end{align*}
由$y$的任意性知$f\left( y \right) =a\left( \forall y\in \mathbb{R} \right)$，这与$f$在$\mathbb{R}$上非常数矛盾！故$\mu >0$。
\end{enumerate}

再证明$\mu \in E$。若$\mu \notin E$，则由\refpro{proposition:确界不在集合中必存在严格单调数列}知，存在$\left\{ T_n \right\} \subseteq E$严格递减趋于$\mu$。于是由Cauchy收敛准则知，存在$n,m\in \mathbb{N}$且$m>n$，使得
\begin{align*}
0<T_n-T_m<\mu .
\end{align*}
显然$T_n-T_m\in E$，这与$\mu =\inf\limits_{}E$矛盾！故$\mu \in E$。

\end{proof}

\begin{example}
设$f(x)$定义在$\mathbb{R}$中,且对任意$x$,都存在与$x$有关的$r>0$使得$f(x)$在区间$(x-r,x+r)$中恒等于某个多项式,证明:$f(x)$是一个多项式.
\end{example}
\begin{proof}
...
\end{align*}
显然$x_0+r_{x_0}\in A$,进而$A\ne \varnothing$.由确界原理知,$A$存在上确界.设$\sup A=M<+\infty$,则由条件知,存在$r_M\in (0,M-x_0+r_{x_0}),P_M\in \mathbb{R}[x]$,使得
\begin{align*}
...
从而
\begin{align*}
P_{x_0}(t)=P_M(t)\Longleftrightarrow P_{x_0}(t)-P_M(t)=0,\quad \forall t\in (M-r_M,M-\frac{r_M}{2}).
\end{align*}
...
\end{align*}
显然$x_0-r_{x_0}\in B$,进而$B\ne \varnothing$.由确界原理知,$A$存在下确界.同理可证$\inf B=-\infty$.综上可知
\begin{align*}
...
\end{remark}
\begin{proof}
(反证法)假设对$\forall x\in[0,1]$,都有$f(x)\ne x$.将闭区间\([0,1]\)记作\([a_1,b_1]\),且由条件可知\(f(a_1)>a_1\),\(f(b_1)<b_1\).令\(c_1=\frac{a_1 + b_1}{2}\),若\(f(c_1)>c_1\),则取\([a_2,b_2]=[c_1,b_1]\);若\(f(c_1)<c_1\),则取\([a_2,b_2]=[a_1,c_1]\).从而得到闭区间\([a_2,b_2]\subset [a_1,b_1]\),并且\(f(a_2)>a_2\),\(f(b_2)<b_2\).以此类推,可得到一列闭区间\(\{[a_n,b_n]\}\),并且\([a_n,b_n]\subset [a_{n + 1},b_{n + 1}]\),\(f(a_n)>a_n\),\(f(b_n)<b_n\),\(\forall n\in\mathbb{N}\).

根据闭区间套定理,可知存在唯一\(\xi=\lim_{n\rightarrow\infty}a_n=\lim_{n\rightarrow\infty}b_n\),且\(\xi\in [a_n,b_n]\),\(\forall n\in\mathbb{N}\).又由\(f(x)\)在\([0,1]\)上单调递增及\(f(a_n)>a_n\),\(f(b_n)<b_n\),\(\forall n\in\mathbb{N}\),可知\(a_n<f(a_n)\leqslant f(\xi)\leqslant f(b_n)<b_n\).令\(n\rightarrow\infty\)可得\(\xi\leqslant f(\xi)\leqslant\xi\),即\(f(\xi)=\xi\).这与假设矛盾.
...
\end{lemma}
\begin{note}
本题谢惠民上的证明是利用有限覆盖定理,而CMC红宝书上通过直接构造出$\delta$进行证明.这里我们采用的是聚点定理进行证明.
\end{note}
\begin{proof}
(反证法)假设对$\forall n\in\mathbb{N}$,取$\delta=\frac{1}{n}>0$,都存在相应的$x_n,y_n\in [a,b]$且$\left| x_n-y_n \right|<\delta$,使得对$\forall I\in\{\mathcal{O} _{\alpha}\}$,要么$x_n \notin I$,要么$y_n \notin I$.由聚点定理可知,有界数列$\{x_n\},\{y_n\}$一定存在收敛子列.设$\{x_{n_k}\},\{y_{m_k}\}$为相应的收敛子列,则由$\left| x_n-y_n \right|<\delta=\frac{1}{n},\forall n\in \mathbb{N}$可知${x_{n_k}},{y_{m_k}}$收敛于同一个极限点.故设$\underset{k\rightarrow \infty}{\lim}x_{n_k}=\underset{k\rightarrow \infty}{\lim}y_{m_k}=x_0\in[a,b]$.
...
\end{proof}
\begin{remark}
注意对于两个收敛子列$\{x_{n_k}\},\{y_{m_k}\}$,此时$n_k = m_k$并不一定对$\forall k\in \mathbb{N}$都成立,即这两个收敛子列的指标集$\left\{ n_k \right\} _{k=1}^{\infty},\left\{ m_k \right\} _{k=1}^{\infty}$不相同也不一定有交集,故无法利用聚点定理反复取子列的方法取到两个指标相同且同时收敛的子列$\left\{ x_{n_k} \right\} _{k=1}^{\infty},\left\{ y_{n_k} \right\} _{k=1}^{\infty}$(取$\{x_n\}$为一个奇子列收敛,偶子列发散的数列;取$\{y_n\}$为一个奇子列发散,偶子列收敛的数列就能得到反例.).
\end{remark}
...
{\color{blue}证法三(闭区间套定理):}(反证法)假设存在$a,b\in \mathbb{R}$,使得$f(a)\ne f(b)$.不妨设\(f(a) < f(b)\),则记闭区间\([a,b]=[a_1,b_1]\).若\(f(\frac{a_1 + b_1}{2}) > f(a_1)\),则记闭区间\([a_1,\frac{a_1 + b_1}{2}]=[a_2,b_2]\);若\(f(\frac{a_1 + b_1}{2}) < f(b_1)\),则记闭区间\([\frac{a_1 + b_1}{2},b_1]=[a_2,b_2]\).以此类推,可以得到一列闭区间\(\{[a_n,b_n]\}\),满足\([a_n,b_n]\subset [a_{n + 1},b_{n + 1}]\),\(f(a_n) < f(b_n)\),\(\forall n\in\mathbb{N}\).由闭区间套定理,可知存在唯一\(\xi=\lim_{n\rightarrow\infty}a_n=\lim_{n\rightarrow\infty}b_n\),且\(\xi\in [a_n,b_n]\).又由条件可知,存在\(r > 0\),使得\(f(t)=f(\xi)\),\(\forall t\in (\xi - r,\xi + r)\).从而存在充分大的\(N\in\mathbb{N}\),使得\(\vert a_N - \xi\vert < r\),\(\vert b_N - \xi\vert < r\),即\(a_N,b_N\in (\xi - r,\xi + r)\).于是\(f(a_N)=f(b_N)\),这与\(f(a_N) < f(b_N)\)矛盾.
...
\]
任取\(y_1,y_2\in A\)且\(y_1\neq y_2\),则存在\(x_1,x_2\in I\),使得\(f(x_1)=y_1\),\(f(x_2)=y_2\).假设\(\varphi(y_1)=\varphi(y_2)=(r_0,s_0)\),则对\(\forall t\in(r_0,s_0)\),都有\(f(t)\geqslant y_1,y_2\).于是\(y_1 = f(x_1)\geqslant y_2\),\(y_2 = f(x_2)\geqslant y_1\),从而\(y_1 = y_2\),因此\(\varphi\)是单射.故$\overline{A}\leqslant \overline{B}.$
...
\]
由闭区间套定理可知,存在唯一\(x_0\in\bigcap_{n = 1}^{\infty}[a_n,b_n]\),使得\(x_0=\lim_{n\rightarrow\infty}a_n=\lim_{n\rightarrow\infty}b_n\).再由\(f\)的连续性以及\(Heine\)归结原则可知,\(f(a_n)\)严格递增收敛于\(f(x_0)\),\(f(b_n)\)严格递减收敛于\(f(x_0)\).故\(f(a_n) < f(x_0) < f(b_n)\),\(\forall n\in\mathbb{N}\).
...
\begin{proof}
\begin{enumerate}
\item 对$\forall \varepsilon >0,n\in \mathbb{N}$，令
\begin{align*}
E_n=\left\{ x\geqslant 1:\,\forall k\geqslant n,\left| f\left( a_kx \right) \right|\leqslant \varepsilon \right\},
\end{align*}
由$f$的连续性知$E_n$是一列闭集，根据条件有$\bigcup\limits_{n=1}^{\infty}{E_n}=\left[ 1,+\infty \right).$于是由\hyperref[theorem:Baire纲定理]{Baire纲定理}知，存在$N\in \mathbb{N}$及$\left( u,v \right) \subseteq \left[ 1,+\infty \right)$，使得$\left( u,v \right) \subseteq E_N.$即对$\forall k\geqslant N,\forall x\in \left( u,v \right)$有
\begin{align*}
\left| f\left( a_kx \right) \right|\leqslant \varepsilon.
\end{align*}
从而对$\forall k\geqslant N,\forall x\in \left( a_ku,a_kv \right)$有
\begin{align}
\left| f\left( x \right) \right|\leqslant \varepsilon. \label{eq::---23ruw90u--2tuu-2u-u2-utjwefjsjilevere1}
\end{align}
由$\lim\limits_{k\rightarrow \infty}\frac{a_{k+1}}{a_k}=1<\frac{v}{u}$知，存在$N_1\in \mathbb{N}$，使得$\forall k\geqslant N_1$有
\begin{align}
\frac{a_{k+1}}{a_k}<\frac{v}{u}\Longleftrightarrow a_{k+1}u<a_kv. \label{eq::---23ruw90u--2tuu-2u-u2-utjwefjsjilevere2}
\end{align}
取$N' =\max \left\{ N,N_1 \right\}$，则由\eqref{eq::---23ruw90u--2tuu-2u-u2-utjwefjsjilevere2}式知，对$\forall k\geqslant N'$有
\begin{align*}
\bigcup\limits_{k=N'}^{\infty}{\left( a_ku,a_kv \right)}=\left( a_{N'}u,+\infty \right).
\end{align*}
进而对$\forall x\in \left( a_{N'}u,+\infty \right) =\bigcup\limits_{k=N'}^{\infty}{\left( a_ku,a_kv \right)}$，都存在$n\geqslant N'$，使得$x\in \left( a_nu,a_nv \right).$再结合\eqref{eq::---23ruw90u--2tuu-2u-u2-utjwefjsjilevere1}式知
\begin{align*}
\left| f\left( x \right) \right|\leqslant \varepsilon.
\end{align*}
此即$\lim\limits_{x\rightarrow +\infty}f\left( x \right) =0.$
...
\begin{example}
设 $f \in C([0, +\infty))$ 满足对任何 $h \geqslant 0$ 都有
\begin{align*}
\lim\limits_{x \to +\infty} [f(x+h) - f(x)] = 0.
\end{align*}
证明：对任何 $[a, b] \subset [0, +\infty)$ 和 $\varepsilon > 0$，都有 $X > 0$ 使得
\begin{align*}
|f(x+h) - f(x)| < \varepsilon, \quad \forall h \in [a, b], \; x \geqslant X.
\end{align*}
\end{example}
\begin{proof}
...
\begin{align*}
E_n \triangleq \left\{ h \in [0, 1] : |f(x + h) - f(x)| \leqslant \frac{\varepsilon}{6}, \; \forall x \geqslant n \right\}.
\end{align*}
由$f$的连续性知$E_n$ 是闭集.再由题设我们有
\begin{align*}
[0, 1] = \bigcup\limits_{n=1}^{\infty} E_n.
\end{align*}
从而由\hyperref[theorem:Baire纲定理]{Baire纲定理}知,存在$[c, d] \subset [0, 1]$,$N \in \mathbb{N}$,使得
\begin{align*}
[c, d] \subset E_N.
\end{align*}
现在对任何 $t \in [0, d - c]$，$x \geqslant N + c$，利用 $c, c + t \in E_N,x-c\geqslant N$,由$E_N$定义有
\begin{align*}
&|f(x + t) - f(x)| = |f(x - c + c + t) - f(x - c + c)| \\
&\leqslant |f(x - c + c + t) - f(x - c)| + |f(x - c + c) - f(x - c)| \\
&\leqslant \frac{\varepsilon}{6} + \frac{\varepsilon}{6} = \frac{\varepsilon}{3} < \frac{\varepsilon}{2}.
\end{align*}
因此我们实际上证明了对$\forall \varepsilon > 0$，都存在 $\delta > 0$，$X_0 > 0$ 使得
\begin{align}
|f(x + t) - f(x)| < \frac{\varepsilon}{2}, \quad \forall t \in [0, \delta], \; x \geqslant X_0. \label{step1}
\end{align}
现在对任何 $[a, b]\subset [0,+\infty)$,
对 $\forall h \in [a, b]$，由题设知,存在$X_h > 0$,使得
\begin{align}
|f(x + h) - f(x)| < \frac{\varepsilon}{2}, \quad \forall x \geqslant X_h. \label{step2}
\end{align}
注意到
\begin{align*}
[a, b] \subset \bigcup\limits_{h \in [a, b]} (h - \delta, h + \delta).
\end{align*}
由有限覆盖定理知,存在$h_1, h_2, \cdots, h_m \in [a, b]$ 使得
\begin{align*}
[a, b] \subset \bigcup\limits_{i=1}^m (h_i - \delta, h_i + \delta).
\end{align*}
于是对$\forall h \in [a, b]$，都存在 $1 \leqslant i \leqslant m$ 使得 $|h - h_i| < \delta$。取$X \triangleq \max\{X_0, X_{h_1}, \cdots, X_{h_m}\} > 0$,则对$\forall x \geqslant X$,利用\eqref{step1}\eqref{step2}式就有
\begin{align*}
|f(x + h) - f(x)| \leqslant |f(x + h) - f(x + h_i)| + |f(x + h_i) - f(x)|< \frac{\varepsilon}{2} + \frac{\varepsilon}{2} = \varepsilon,
\end{align*}
\end{proof}
...
\begin{proposition}\label{proposition:递增函数Schwarz导下极限为0必为常函数}
$f$在区间$I$上递增,且有
\begin{align*}
\varliminf\limits_{h\rightarrow 0^+}\frac{f\left( x+h \right) -f\left( x-h \right)}{h}=0,\quad \forall x\in I.
\end{align*}
证明:$f$在$I$上恒为常数.
\end{proposition}
\begin{proof}
反证,假设存在$a,b\in I$且$a<b$,使得$f\left( a \right) <f\left( b \right)$.取$\varepsilon =\frac{f\left( b \right) -f\left( a \right)}{2\left( b-a \right)}$,由条件知,对$\forall x\in \left( a,b \right)$,存在$h_x>0$,当$0<h<h_x$时,有
\begin{align*}
\frac{f\left( x+h_x \right) -f\left( x-h_x \right)}{h_x}<\varepsilon \Longleftrightarrow f\left( x+h_x \right) -f\left( x-h_x  \right) <\varepsilon h_x.
\end{align*}
注意到$\left( a,b \right) \subseteq \bigcup\limits_{x\in \left( a,b \right)}{\left( x-h_x ,x+h_x \right)}$,由有限覆盖定理知,存在$n\in \mathbb{N}$,使得
\begin{align*}
\left( a,b \right) \subseteq \bigcup\limits_{i=1}^n{\left( x_i-h_{x_i} ,x_i+h_{x_i} \right)}.
\end{align*}
不妨设$a=x_1<x_2<\cdots <x_n<b<x_n+h_{x_n}=x_{n+1}$,且$\left( a,b \right)$中任意点不会同时属于三个区间$\left( x_k-h_{x_k},x_k+h_{x_k} \right)$.容易验证$\left( x_i,x_{i+1} \right) \subseteq \left( x_i-h_{x_i},x_i+h_{x_i} \right)$.于是由$f$的递增性知
\begin{align*}
f\left( b \right) -f\left( a \right) \leqslant \sum\limits_{k=1}^{n+1}{\left[ f\left( x_{k+1} \right) -f\left( x_k \right) \right]}\leqslant \sum\limits_{k=1}^n{\left[ f\left( x_k+h_{x_k} \right) -f\left( x_k-h_{x_k} \right) \right]}<\varepsilon \sum\limits_{k=1}^n{h_{x_k}}\\
\leqslant \varepsilon \left[ \sum\limits_{k=1}^{n-1}{\left( x_{k+2}-x_k \right)}+\left( x_{n+1}-x_n \right) \right] <2\varepsilon \left( b-a \right) =\frac{f\left( b \right) -f\left( a \right)}{2}.
\end{align*}
显然矛盾!故$f$在$I$上恒为常数.
...
\begin{align*}
f(a+x)-f(a-x)\geqslant cx\quad (0\leqslant x\leqslant 1).
\end{align*}
\end{example}
\begin{note}
也可以用闭区间套定理证明(对递增的区间进行二分),可见周民强P12(例1.2.13).
\end{note}
\begin{remark}
注意到
\begin{align*}
2\chi_{J(t)} = \sum_{i=1}^n \chi_{(a_i - x_{a_i}, a_i + x_{a_i})}(t), \ \forall t \in J,
\end{align*}
于是
\begin{align*}
2|J| = \int_J 2\chi_{J(t)}\mathrm{d}t = \sum_{i=1}^n \int_J \chi_{(a_i - x_{a_i}, a_i + x_{a_i})}(t)\mathrm{d}t = \sum_{i=1}^n |(a_i - x_{a_i}, a_i + x_{a_i})|.
\end{align*}
\end{remark}
\begin{proof}

{\color{blue}证法一:}当$x=0$时,结论显然成立.只需证明$x\in \left( 0,1 \right]$的情形.反证,假设对$\forall a>0,n\in \mathbb{N}$,都存在$x_n\in \left( 0,1 \right]$,使得
\begin{align*}
f\left( a+x_n \right) -f\left( a-x_n \right) <\frac{x_n}{n}\Longrightarrow \frac{f\left( a+x_n \right) -f\left( a-x_n \right)}{x_n}<\frac{2}{n}.
\end{align*}
令$n\rightarrow \infty$得
\begin{align*}
\varliminf\limits_{n\rightarrow \infty}\frac{f\left( a+x_n \right) -f\left( a-x_n \right)}{x_n}=0,\quad \forall a>0.
\end{align*}
由\refpro{proposition:递增函数Schwarz导下极限为0必为常函数}知$f$恒为常数,这与题设矛盾!

{\color{blue}证法二:}
设 $r_0 < s_0$ 使得 $f(r_0) < f(s_0)$. 定义
\begin{align*}
c \triangleq \frac{f(s_0) - f(r_0)}{2(s_0 - r_0 + 2)} > 0.
\end{align*}
若对任何 $a \in \mathbb{R}$ 都有 $x_a \in (0,1]$ 使得
\begin{align*}
f(a + x_a) - f(a - x_a) < c x_a.
\end{align*}
注意到
\begin{align*}
...
\begin{align*}
|J| = |(a_1 - x_{a_1}, a_n + x_{a_n})| \leqslant s_0 + 1 - (r_0 - 1) = s_0 - r_0 + 2,
\end{align*}
从而注意到 $J$ 中任意一个点至多只被上述区间覆盖两次知(见注)
\begin{align*}
\sum_{i=1}^n 2x_{a_i} = \sum_{i=1}^n |(a_i - x_{a_i}, a_i + x_{a_i})| \leqslant 2(s_0 - r_0 + 2).
\end{align*}
现在就有
\begin{align*}
f(s_0) - f(r_0) &\leqslant f(a_n + x_{a_n}) - f(a_1 - x_{a_1}) \\
&\leqslant \sum_{k=1}^n \left[f(a_k + x_{a_k}) - f(a_n - x_{a_k})\right] < c \sum_{k=1}^n x_{a_k} \\
&\leqslant f(s_0) - f(r_0),
\end{align*}
这就是一个矛盾! 于是我们证明了存在 $a \in \mathbb{R}, c > 0$ 使得
\begin{align*}
f(a + x) - f(a - x) \geqslant c x, \ \forall x \in [0, 1].
\end{align*}
