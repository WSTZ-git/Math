...
$f$在区间$I$上递增,且有
\begin{align*}
\varliminf\limits_{h\rightarrow 0^+}\frac{f\left( x+h \right) -f\left( x \right)}{h}=0,\quad \forall x\in I.
\end{align*}
证明:$f$在$I$上恒为常数.
\end{proposition}
\begin{proof}
...
\begin{align*}
\frac{f\left( x+h_x \right) -f\left( x \right)}{h_x}<\varepsilon \Longleftrightarrow f\left( x+h_x \right) -f\left( x \right) <\varepsilon h_x.
\end{align*}
注意到$\left( a,b \right) \subseteq \bigcup\limits_{x\in \left( a,b \right)}{\left( x,x+h_x \right)}$,由有限覆盖定理知,存在$n\in \mathbb{N}$,使得
\begin{align*}
\left( a,b \right) \subseteq \bigcup\limits_{i=1}^n{\left( x_i,x_i+h_{x_i} \right)}.
\end{align*}
不妨设$a=x_1<x_2<\cdots <x_n<b<x_n+h_{x_n}=x_{n+1}$,且$\left( a,b \right)$中任意点不会同时属于三个区间$\left( x_k,x_k+h_{x_k} \right)$.容易验证$\left( x_i,x_{i+1} \right) \subseteq \left( x_i,x_i+h_{x_i} \right)$.于是由$f$的递增性知
\begin{align*}
f\left( b \right) -f\left( a \right) \leqslant \sum\limits_{k=1}^{n+1}{\left[ f\left( x_{k+1} \right) -f\left( x_k \right) \right]}\leqslant \sum\limits_{k=1}^n{\left[ f\left( x_k+h_{x_k} \right) -f\left( x_k \right) \right]}<\varepsilon \sum\limits_{k=1}^n{h_{x_k}}\\
...
\end{note}
\begin{proof}
当$x=0$时,结论显然成立.只需证明$x\in \left( 0,1 \right]$的情形.反证,假设对$\forall a>0,n\in \mathbb{N}$,都存在$x_n\in \left( 0,1 \right]$,使得
\begin{align*}
f\left( a+x_n \right) -f\left( a-x_n \right) <\frac{x_n}{n}\Longrightarrow \frac{f\left( a+x_n \right) -f\left( a-x_n \right)}{2x_n}<\frac{2}{n}.
\end{align*}
令$n\rightarrow \infty$得
\begin{align*}
\varliminf\limits_{n\rightarrow \infty}\frac{f\left( a+x_n \right) -f\left( a-x_n \right)}{2x_n}=0,\quad \forall a>0.
\end{align*}
