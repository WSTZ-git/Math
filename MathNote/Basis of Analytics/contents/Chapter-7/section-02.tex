...
\sum\limits_{i=1}^n{a_{i}^{2}\cdot}\sum\limits_{i=1}^n{b_{i}^{2}}\geqslant slant \left( \sum\limits_{i=1}^n{a_ib_i} \right) ^2.
\end{align}
...
(ii) 当\(b_i\)不全为零时,注意到\(\left(\sum\limits_{i = 1}^{n}(a_i + tb_i)\right)^2 \geqslant slant 0\),\(\forall t \in \mathbb{R}\).等价于

\[
t^2\sum\limits_{i = 1}^{n}b_{i}^{2} + 2t\sum\limits_{i = 1}^{n}a_ib_i + \sum\limits_{i = 1}^{n}a_{i}^{2} \geqslant slant 0,  \forall t \in \mathbb{R}.
\]

根据一元二次方程根的存在性定理,可知\(\Delta = \left(\sum\limits_{i = 1}^{n}a_ib_i\right)^2 - \sum\limits_{i = 1}^{n}a_{i}^{2} \cdot \sum\limits_{i = 1}^{n}b_{i}^{2} \leqslant slant 0\).

从而\(\sum\limits_{i = 1}^{n}a_{i}^{2} \cdot \sum\limits_{i = 1}^{n}b_{i}^{2} \geqslant slant \left(\sum\limits_{i = 1}^{n}a_ib_i\right)^2\).
...
\begin{example}
证明:\begin{align*}
\sum\limits_{i=1}^n{\frac{1}{x_i}}\geqslant slant \frac{n^2}{\sum\limits_{i=1}^n{x_i}},\forall n\in \mathbb{N} ,x_1,x_2,\cdots ,x_n>0.
\end{align*}
\end{example}
\begin{proof}
...
\[
\sum\limits_{i = 1}^{n}\frac{1}{x_i} \cdot \sum\limits_{i = 1}^{n}x_i = \sum\limits_{i = 1}^{n}\left(\frac{1}{\sqrt{x_i}}\right)^2 \cdot \sum\limits_{i = 1}^{n}\left(\sqrt{x_i}\right)^2 \geqslant slant \left(\sum\limits_{i = 1}^{n}\sqrt{x_i} \cdot \frac{1}{\sqrt{x_i}}\right)^2 = n^2.
\]

故\(\sum\limits_{i = 1}^{n}\frac{1}{x_i} \geqslant slant \frac{n^2}{\sum\limits_{i = 1}^{n}x_i}\),\(\forall n \in \mathbb{N}\),\(x_1,x_2,\cdots,x_n > 0\). 
\end{proof}
...
\[
\left(\frac{1}{\sqrt{A}}\sqrt{Ax + 27A} + \frac{1}{\sqrt{B}}\sqrt{13B - Bx} + \frac{1}{\sqrt{C}}\sqrt{Cx}\right)^2 \leqslant slant \left(\frac{1}{A} + \frac{1}{B} + \frac{1}{C}\right)[(A + C - B)x + 27A + 13B]
\]
...
\[
\left(\sqrt{x + 27} + \frac{1}{\sqrt{3}}\sqrt{39 - 3x} + \frac{1}{\sqrt{2}}\sqrt{2x}\right)^2 \leqslant slant \left(1 + \frac{1}{3} + \frac{1}{2}\right)(x + 27 + 39 - 3x + 2x)
\]

并且当且仅当\(\sqrt{x + 27} = \sqrt{3} \cdot \sqrt{39 - 3x} = \sqrt{2} \cdot \sqrt{2x}\),即\(x = 9\)时,等号成立. 
\end{note}
\begin{solution}
由题可知,函数\(y\)的定义域就是:\(0 \leqslant slant x \leqslant slant 13\).而
\begin{align*}
&y(x) = \sqrt{x + 27} + \sqrt{[\sqrt{13 - x} + \sqrt{x}]^2}
\\
&= \sqrt{x + 27} + \sqrt{13 + 2\sqrt{x(13 - x)}}
\\
&\geqslant slant \sqrt{27} + \sqrt{13} = 3\sqrt{3} + \sqrt{13} = y(0)
\end{align*}
...
&\leqslant slant (1 + \frac{1}{3} + \frac{1}{2})(x + 27 + 39 - 3x + 2x)\\
&= 121 = y^2(9)
\end{align*}
即\(y(x) \leqslant slant y(9) = 11\).并且当且仅当\(\sqrt{x + 27} = \sqrt{3} \cdot \sqrt{39 - 3x} = \sqrt{2} \cdot \sqrt{2x}\),即\(x = 9\)时,等号成立.故\(y\)的最大值为\(11\). 
\end{solution}
...
\begin{align*}
\frac{n}{\frac{1}{a_1}+\frac{1}{a_2}+\cdots +\frac{1}{a_n}}\leqslant slant \sqrt[n]{a_1a_2\cdots a_n}\le \frac{a_1+a_2+\cdots +a_n}{n}\leqslant slant \sqrt{\frac{a_{1}^{2}+a_{2}^{2}+\cdots +a_{n}^{2}}{n}}.
\end{align*}
\end{theorem}
...
\\
&\leqslant slant 2\cdot \left[ \frac{2x+1-x+1-x}{3} \right] ^3
\\
...
\begin{align*}
\left( 1+x_1 \right) \left( 1+x_2 \right) \cdots \left( 1+x_n \right) \geqslant slant 1+x_1+x_2+\cdots +x_n.
\end{align*}
\end{theorem}
\begin{proof}
...
\begin{align*}
&\left( 1+x_1 \right) \left( 1+x_2 \right) \cdots \left( 1+x_{k+1} \right) \geqslant slant \left( 1+x_1+x_2+\cdots +x_k \right) \left( 1+x_{k+1} \right) 
\\
&=1+x_1+x_2+\cdots +x_k+x_{k+1}+x_1x_{k+1}+x_2x_{k+1}+\cdots +x_kx_{k+1}
\\
&\geqslant slant 1+x_1+x_2+\cdots +x_k+x_{k+1}
\end{align*}
故由数学归纳法可知,结论成立.
\end{proof}
...
\begin{align*}
ab\leqslant slant \frac{a^p}{p}+\frac{b^q}{q}.
\end{align*}
当且仅当$a^p=b^q$时等号成立.
\end{theorem}
\begin{note}
...
ab\leqslant slant \frac{a^p}{p}+\frac{b^q}{q}
\\
\Leftrightarrow \ln a+\ln b\leqslant slant \ln \left( \frac{a^p}{p}+\frac{b^q}{q} \right) 
\\
\Leftrightarrow \frac{1}{p}\ln a^p+\frac{1}{q}\ln b^q\leqslant slant \ln \left( \frac{a^p}{p}+\frac{b^q}{q} \right) 
\end{gather*}
...
\end{align*}
从而只需证明$\sum\limits_{k=1}^n{a_{k}^{\prime}b_{k}^{\prime}}\leqslant slant 1$.由\hyperref[theorem:Young不等式]{Young不等式}可得
\begin{align*}
\sum\limits_{k=1}^n{a_{k}^{\prime}b_{k}^{\prime}}\leqslant slant &\sum\limits_{k=1}^n{\left[ \frac{\left( a_{k}^{\prime} \right) ^p}{p}+\frac{\left( b_{k}^{\prime} \right) ^q}{q} \right]}=\sum\limits_{k=1}^n{\left( \frac{a_{k}^{p}}{p\sum\limits_{k=1}^n{a_{k}^{p}}}+\frac{b_{k}^{p}}{q\sum\limits_{k=1}^n{b_{k}^{q}}} \right)}
\\
