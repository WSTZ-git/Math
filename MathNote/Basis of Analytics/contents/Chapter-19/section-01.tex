...
\end{align*}
故$\lim_{x \to 0^+} xf\left( x \right) = -\frac{\mu A}{\mu - 1}.$\end{proof}
...
综上，$f$在任意长度为$2\pi$的区间$[c, c+2\pi]$上都至少改变符号$2n+2$次。又因为$[c, c+2\pi] \subseteq I$，因此$f$在区间$I$上至少改变符号$2n+2$次。\end{proof}
...
\end{align*}
这就是矛盾！从而\(F\)在\([0,1]\)没有正的最大值.类似的可以讨论最小值,得到\(F\)在\([0,1]\)没有负的最小值.因此\(0 \leqslant F(x) \leqslant 0\),\(\forall x \in [0,1]\).即\(F(x) \equiv 0\),\(\forall x \in [0,1]\).故\(f\)在\([0,1]\)上恒为\(0\). \end{proof}
...
对 \(T > 0\)，记
\[
M_f(T) = \frac{1}{T} \int_0^T f(t) \mathrm{d}t,
\]
并记
\[
\|f\|_{\mathrm{osc}([0,T])} = \max_{t_1,t_2 \in [0,T]} |f(t_1) - f(t_2)|.
\]
证明：对于任意 \(T > 0\) 及任意 \([0,T]\) 上连续函数 \(f,g\)，以下不等式成立：
\[
|M_{fg}(T) - M_f(T)M_g(T)| \leqslant \|f\|_{\mathrm{osc}([0,T])} \|g\|_{\mathrm{osc}([0,T])}.
\]
\end{example}
\begin{proof}
注意到
\begin{align*}
&M_{fg}\left( T \right) -M_f(T)M_g(T)=\frac{1}{T}\int_0^T{f\left( t \right) g\left( t \right) \,\mathrm{d}t}-M_f(T)M_g(T)\\
&=\frac{1}{T}\int_0^T{f\left( t \right) g\left( t \right) \,\mathrm{d}t}-M_f(T)M_g(T)-M_g(T)M_f(T)+M_f(T)M_g(T)
\\
&=\frac{1}{T}\int_0^T{f\left( t \right) g\left( t \right) \,\mathrm{d}t}-\frac{M_f(T)}{T}\int_0^T{g\left( t \right) \,\mathrm{d}t}-\frac{M_g(T)}{T}\int_0^T{f\left( t \right) \,\mathrm{d}t}+\frac{1}{T}\int_0^T{M_f(T)M_g(T)\,\mathrm{d}t}
\\
&=\frac{1}{T}\int_0^T{\bigl[ f\left( t \right) g\left( t \right) -M_f(T)g\left( t \right) -M_g(T)f\left( t \right) +M_f(T)M_g(T) \bigr] \mathrm{d}t}
\\
&=\frac{1}{T}\int_0^T{\left( f\left( t \right) -M_f(T) \right) \left( g\left( t \right) -M_g(T) \right) \,\mathrm{d}t}.
\end{align*}
因此
\begin{align}\label{eq::jkfkpieijwefijwoefijo}
|M_{fg}(T) - M_f(T)M_g(T)| 
\leqslant \frac{1}{T} \int_0^T |f(t) - M_f(T)|\,|g(t) - M_g(T)|\,\mathrm{d}t.
\end{align}
由连续函数最大、最小值定理可知
\begin{align*}
\min_{[0,T]} f \leqslant M_f(T) \leqslant \max_{[0,T]} f,\quad \min_{[0,T]} g \leqslant M_g(T) \leqslant \max_{[0,T]} g.
\end{align*}
于是对任意 $t \in [0,T]$，都有
\begin{gather*}
|f(t) - M_f(T)| \leqslant \max_{[0,T]} f - \min_{[0,T]} f = \|f\|_{\mathrm{osc}([0,T])},\\
|g(t) - M_f(T)| \leqslant \max_{[0,T]} g - \min_{[0,T]} g = \|g\|_{\mathrm{osc}([0,T])}.
\end{gather*}
代入\eqref{eq::jkfkpieijwefijwoefijo}式得
\begin{align*}
\frac{1}{T} \int_0^T |f(t) - M_f(T)|\,|g(t) - M_g(T)|\,\mathrm{d}t 
\leqslant \frac{1}{T} \int_0^T \|f\|_{\mathrm{osc}} \|g\|_{\mathrm{osc}} \,\mathrm{d}t 
= \|f\|_{\mathrm{osc}} \|g\|_{\mathrm{osc}}.
\end{align*}\end{proof}

\begin{example}
对 \(T > 0\)，记
\begin{align*}
M_T(f) = \frac{1}{T} \int_0^T f(t) \mathrm{d}t.
\end{align*}
证明：存在常数 \(C > 0\)，使得对任意的 \(T > 0\) 以及任意实数 \(\alpha,\beta\)，下列不等式成立：
\begin{align*}
|M_T(\mathrm{e}^{\mathrm{i}(\alpha+\beta)t}) - M_T(\mathrm{e}^{\mathrm{i}\alpha t}) [1 + T M_T(\mathrm{i}\beta \mathrm{e}^{\mathrm{i}\beta t})]| \leqslant CT|\beta|.
\end{align*}
\end{example}
\begin{proof}
注意到对任意实数 $\beta$，有
\begin{align*}
T\,M_T(\mathrm{i}\beta \mathrm{e}^{\mathrm{i}\beta t}) = \mathrm{i}\beta \int_0^T \mathrm{e}^{\mathrm{i}\beta t}\,\mathrm{d}t = \mathrm{e}^{\mathrm{i}\beta T} - 1,
\end{align*}
因此
\begin{align*}
1 + T\,M_T(\mathrm{i}\beta \mathrm{e}^{\mathrm{i}\beta t}) = \mathrm{e}^{\mathrm{i}\beta T}.
\end{align*}
于是要估计的量化为
\begin{align*}
E \triangleq \bigl| M_T(\mathrm{e}^{\mathrm{i}(\alpha+\beta)t}) - M_T(\mathrm{e}^{\mathrm{i}\alpha t})\, \mathrm{e}^{\mathrm{i}\beta T} \bigr|,
\end{align*}
即
\begin{align*}
E = \frac{1}{T}\biggl| \int_0^T \mathrm{e}^{\mathrm{i}(\alpha+\beta)t}\,\mathrm{d}t - \int_0^T \mathrm{e}^{\mathrm{i}\alpha t} \mathrm{e}^{\mathrm{i}\beta T}\,\mathrm{d}t \biggr| = \frac{1}{T}\biggl| \int_0^T \mathrm{e}^{\mathrm{i}\alpha t}\bigl(\mathrm{e}^{\mathrm{i}\beta t} - \mathrm{e}^{\mathrm{i}\beta T}\bigr)\,\mathrm{d}t \biggr|.
\end{align*}
再注意到 $|\mathrm{e}^{\mathrm{i}\alpha t}|=1$，有
\begin{align}\label{eq:111111-qaqa}
E \leqslant \frac{1}{T}\int_0^T \bigl| \mathrm{e}^{\mathrm{i}\beta t} - \mathrm{e}^{\mathrm{i}\beta T} \bigr|\,\mathrm{d}t = \frac{1}{T}\int_0^T \bigl| 1 - \mathrm{e}^{\mathrm{i}\beta (T-t)} \bigr|\,\mathrm{d}t = \frac{1}{T}\int_0^T \bigl| 1 - \mathrm{e}^{\mathrm{i}\beta u} \bigr|\,\mathrm{d}u.
\end{align}
又注意到
\begin{align*}
\left| 1-\mathrm{e}^{\mathrm{i}\beta u} \right|^2&=\left| 1-\left( \cos \beta u+\mathrm{i}\sin \beta u \right) \right|^2=\left( 1-\cos \beta u \right) ^2+\sin ^2\beta u
\\
&=1-2\cos \beta u+\cos ^2\beta u+\sin ^2\beta u=2-2\cos \beta u
\\
&=4\cdot \frac{1-\cos \beta u}{2}=4\sin ^2\frac{\beta u}{2},
\end{align*}
故
\begin{align*}
\left| 1-\mathrm{e}^{\mathrm{i}\beta u} \right|\leqslant 2\left| \sin \frac{\beta u}{2} \right|\leqslant \left| \beta \right|u.
\end{align*}
代入\eqref{eq:111111-qaqa}式得
\begin{align*}
E \leqslant \frac{1}{T}\int_0^T |\beta| u\,\mathrm{d}u = \frac{|\beta|}{T}\cdot\frac{T^2}{2} = \frac{1}{2}\,T|\beta|.
\end{align*}
因此取 $C = \frac{1}{2}$，对任意 $T>0$ 及任意实数 $\alpha,\beta$ 均有
\begin{align*}
\bigl| M_T(\mathrm{e}^{\mathrm{i}(\alpha+\beta)t}) - M_T(\mathrm{e}^{\mathrm{i}\alpha t})\bigl[1 + T M_T(\mathrm{i}\beta \mathrm{e}^{\mathrm{i}\beta t})\bigr] \bigr|
\leqslant \frac{1}{2}\,T|\beta| \leqslant C\,T|\beta|.
\end{align*}\end{proof}
