...
\begin{align*}
\left| \int_{0}^{2} f(x) \, dx \right| &\leq \left| \int_{0}^{1} f(x) \, dx \right| + \left| \int_{1}^{2} f(x) \, dx \right| 
\leq \int_{0}^{1} |f(x)| \, dx + \int_{1}^{2} |f(x)| \, dx \\
&\leq \int_{0}^{1} (Mx) \, dx + \int_{1}^{2} (M(2 - x)) \, dx 
= M.
\end{align*}
\end{proof}
...
\begin{align*}
\left| \int_{0}^{1} f(x) \, dx \right| \leq \frac{M}{12}.
\end{align*}
\end{example}
\begin{note}
...
积分并取绝对值就有
\begin{align*}
\left| \int_{0}^{1} f(x) \, dx \right| = \left| \int_{0}^{1} \frac{f''(\theta(x))}{2} x (x - 1) \, dx \right| \leq \int_{0}^{1} \left| \frac{f''(\theta(x))}{2} \right| \left| x (x - 1) \right| \, dx 
\leq \frac{M}{2} \int_{0}^{1} x (1 - x) \, dx = \frac{M}{12}.
\end{align*}
\end{proof}
...
\begin{align*}
\int_{a}^{b} f(x) \, dx = (b - a) \frac{f(a) + f(b)}{2} - \frac{(b - a)^3}{12} f''(\xi).
\end{align*}
\end{example}
\begin{note}
...
本题也可以直接用K值法证明.只需令$g(y)=\int_a^y{f(x)\,dx}=(y-a)\frac{f(a)+f(y)}{2}-\frac{(y-a)^3}{12}K$即可.
\end{remark}
\begin{proof}
...
\begin{align*}
\int_{0}^{1} \left| \frac{f''(x)}{f(x)} \right| dx \geq 4.
\end{align*}
\end{example}
\begin{proof}
