...
\begin{remark}
\begin{enumerate}
\item $\mathbb{Step}\mathbb{1}$考虑微分方程\(y'=\frac{2x - y}{x}\), 解得\(y=\frac{c}{x}+x\).

$\mathbb{Step}\mathbb{2}$分离常数\(c\)得\(c = x(y - x)\), 常数变易得构造函数\(c(x)=x(f(x)-x)\).

\item $\mathbb{Step}\mathbb{1}$考虑微分方程\(y'=\frac{xy}{1 - x}\), 解得\(y=\frac{ce^{-x}}{x - 1}\).

$\mathbb{Step}\mathbb{2}$分离常数\(c\)得\(c = e^{x}(x - 1)y\), 常数变易得构造函数\(c(x)=e^{x}(x - 1)f(x)\).

\item $\mathbb{Step}\mathbb{1}$考虑微分方程\(y'=\lambda\left[y-\frac{x}{2}\right]+\frac{1}{2}\), 解得\(y = ce^{\lambda x}+\frac{x}{2}\).

$\mathbb{Step}\mathbb{2}$分离常数\(c\)得\(c=\frac{y-\frac{x}{2}}{e^{\lambda x}}\), 常数变易得构造函数\(c(x)=\frac{f(x)-\frac{x}{2}}{e^{\lambda x}}\).
\end{enumerate}
\end{remark}
\begin{proof}
\begin{enumerate}
...
需要
\begin{align*}
2=f' (\lambda )(1+f' (\mu ))=\frac{f(c)}{c}\left[ 1+\frac{f(c)-1}{c-1} \right] \Longleftrightarrow \left( f\left( c \right) +c \right) \left( f\left( c \right) +2c-2 \right) =0.
\end{align*}
...
若考虑\(f(c)=2 - 2c\),则此时令\(g(x)=f(x)+2x - 2\),则现在我们只需找到一个\(c\in(0,1)\),使得\(g(c)=0\)即可.由条件可知\(g(0)=-2,g(1)=1\),从而由连续函数介值定理可得,存在\(c\in(0,1)\),使得\(g(c)=0\).故取$k=\frac{1 - 2c}{c - 1}$能找到满足我们的需求的$c$,进而就确定了满足题目要求的\(\lambda\)和\(\mu\). 
\end{remark}
\begin{proof}
