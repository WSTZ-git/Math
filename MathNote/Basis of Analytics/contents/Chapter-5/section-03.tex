...
f(0) = 0, \quad f(1) = 1.
\end{align*}
对于任意正数 $\lambda, m$，存在两个不同的点 $\xi, \eta \in (0,1)$，使得
\begin{align*}
...
\begin{align}
g(c) = (\lambda + m) f(c) - \lambda = 0 \Longrightarrow f(c) = \frac{\lambda}{\lambda + m}. \label{eq:fm}
\end{align}
再由Lagrange中值定理知 $\exists \xi \in (0,c)$，$\eta \in (c,1)$，使得
\begin{align*}
f'(\xi) = \frac{f(c) - f(0)}{c - 0} = \frac{f(c)}{c}, \quad f'(\eta) = \frac{f(c) - f(1)}{c - 1} = \frac{f(c) - 1}{c - 1}.
\end{align*}
从而再结合\eqref{eq:fm}式可知
\begin{align*}
\frac{\lambda}{f'(\xi)} + \frac{m}{f'(\eta)} = \frac{\lambda c}{f(c)} + \frac{m(c-1)}{f(c) - 1} = c(\lambda + m) + (1-c)(\lambda + m) = \lambda + m.
\end{align*}
\end{proof}
