...
\begin{align*}
\lim_{b\rightarrow a^+} \frac{\int_a^b{f(x)\mathrm{d}x}-(b-a)f(\frac{a+b}{2})}{\frac{1}{6}(b-a)^3}=&\lim_{b\rightarrow a^+} \frac{f(b)-f(\frac{a+b}{2})-\frac{b-a}{2}f\prime (\frac{a+b}{2})}{\frac{1}{2}(b-a)^2}=\lim_{b\rightarrow a^+} \frac{f\prime (b)-f\prime (\frac{a+b}{2})-\frac{b-a}{4}f'' (\frac{a+b}{2})}{b-a}\\
=&\lim_{b\rightarrow a^+} \left( f'' (b)-\frac{3}{4}f'' \left( \frac{a+b}{2} \right) -\frac{b-a}{8}f''' \left( \frac{a+b}{2} \right) \right) =\frac{1}{4}f'' (a)\geqslant 0.
\end{align*}
因此
...
\begin{enumerate}[(1)]
\item 设\(f \in D(a,b)\)且在\([a,b]\)上\(f\)有介值性，则若\(f(a)=f(b)\)，必然存在\(\theta \in (a,b)\)，使得\(f'(\theta)=0\)。

\item 设\(f \in C[a,+\infty)\cap D^{1}(a,+\infty)\)满足\(f(a)=\lim_{x \to +\infty}f(x)\)，则存在\(\theta \in (a,+\infty)\)使得\(f'(\theta)=0\)。
\end{enumerate}
\end{theorem}
\begin{note}
...
\leqslant \left( \int_{0}^{\pi} |\sin x - f(x)|^2 \, \mathrm{d}x \right)^{\frac{1}{2}} \left( \int_{0}^{\pi} |f(x) - \cos x|^2 \, \mathrm{d}x \right)^{\frac{1}{2}}.
\]
...
\[
\int_{0}^{\pi} (\sin x - f(x))(f(x) - \cos x) \, \mathrm{d}x
= \left( \int_{0}^{\pi} |\sin x - f(x)|^2 \, \mathrm{d}x \right)^{\frac{1}{2}} \left( \int_{0}^{\pi} |f(x) - \cos x|^2 \, \mathrm{d}x \right)^{\frac{1}{2}} = \frac{\pi}{4}.
\]
...
注意到
\begin{align*}
1=\left| \int_0^1{(x}-\frac{1}{2})f(x)\,\mathrm{d}x \right|\leqslant \int_0^1{|x}-\frac{1}{2}|\cdot |f(x)|\,\mathrm{d}x\leqslant \max_{x\in [0,1]} |f(x)|\cdot \int_0^1{|x}-\frac{1}{2}|\,\mathrm{d}x=\frac{1}{4}\max_{x\in [0,1]} |f(x)|.
\end{align*}
...
\begin{example}
已知 $f$ 在 $[-1,1]$ 上二阶可导，且 $f(0) = f'(0) = 0$，$f(-1) = -1$，$f(1) = 1$。证明：
...
(i)反证，假设$| f''(x) | \leqslant 2, \forall x \in (-1,1)$。令$g(x) \triangleq f(x) - x^2, x \in [0,1]$。则
\begin{align*}
g'(x) = f'(x) - 2x, \quad g''(x) = f''(x) - 2 \leqslant 0, \, \forall x \in [0,1]
\end{align*}
从而$g'(x)$在$[0,1]$上单调递减。又$g'(0) = 0$，故
\begin{align*}
g'(x) \leqslant g'(0) = 0, \, \forall x \in [0,1]
\end{align*}
因此$g(x)$在$[0,1]$上单调递减。又$g(1) = g(0) = 0$，故
\begin{align*}
0 = g(1) \leqslant g(x) \leqslant g(0) = 0 \Longrightarrow g(x) = 0, \, \forall x \in [0,1]
\end{align*}
即
\begin{align*}
...
\end{cases}
\end{align*}
但此时$f''_+(0) = 2 \ne -2 = f''_-(0)$，故$f$此时在$x=0$处二阶不可导，矛盾！故结论成立。

(ii)令
\begin{align*}
g(x) \triangleq f(x) - x^2, \quad h(x) \triangleq f(x) + x^2
\end{align*}
则$g(0) = g(1) = h(0) = h(-1) = 0, g'(0) = h'(0) = 0$。由Rolle中值定理知，存在$x_1 \in (0,1), x_2 \in (-1,0)$，使得
\begin{align*}
g'(x_1) = g'(0) = 0, \quad h'(x_2) = h'(0) = 0
\end{align*}
再由Rolle中值定理知，存在$\xi_1 \in (0, x_1), \xi_2 \in (x_2, 0)$，使得
\begin{align*}
g''(\xi_1) = h''(\xi_2) = 0
\end{align*}
即
...
\begin{align*}
f''(\xi_2) < f''(\xi) < f''(\xi_1) \Longrightarrow | f''(\xi) | < 2
\end{align*}
