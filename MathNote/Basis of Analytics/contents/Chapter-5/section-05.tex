...
\(f\)是\([a,b]\)上的下凸函数，则
\begin{align*}
\frac{f(a)+f(b)}{2} \geqslant slant \frac{1}{b - a}\int_{a}^{b}f(x)\mathrm{d}x \geqslant slant f\left(\frac{a + b}{2}\right).
\end{align*} 
\end{theorem}
\begin{note}
...
\begin{align*}
\frac{1}{b - a}\int_{a}^{b}f(x)\mathrm{d}x = \int_{0}^{1}f(a(1 - t)+bt)\mathrm{d}t \leqslant slant \int_{0}^{1}[(1 - t)f(a)+tf(b)]\mathrm{d}t = \frac{f(a)+f(b)}{2}.
\end{align*}
另一方面,我们有
\begin{align*}
\frac{1}{b - a}\int_{a}^{b}f(x)\mathrm{d}x &= \frac{1}{b - a}\int_{a}^{b}f(a + b - x)\mathrm{d}x
= \frac{1}{2(b - a)}\int_{a}^{b}[f(a + b - x)+f(x)]\mathrm{d}x\\
&\geqslant slant \frac{1}{b - a}\int_{a}^{b}f\left(\frac{a + b}{2}\right)\mathrm{d}x
...
\begin{align*}
\frac{\int_{a}^{b}f(x)\mathrm{d}x}{b - a} \geqslant slant f\left(\frac{a + b}{2}\right),\quad \forall b\neq a.
\end{align*}
证明 :\(f\)是下凸函数. 
\end{proposition}
\begin{remark}
...
=&\frac{1}{4}f''(a)\geqslant slant 0.
\end{align*}
因此
\[f''(x)\geqslant slant 0,\forall x\in \mathbb{R},\]
所以\(f\)是下凸函数.
\end{proof}
...
\[f(x)\geqslant slant f(a + 1)>f(a),\forall x\geqslant slant a + 1,\]
于是
\[f(a)=\lim_{x \to +\infty}f(x)\geqslant slant f(a + 1)>f(a),\]
...
\begin{align*}
g(x) \leqslant slant g(c) <0,\forall x\in (c-\delta ,c+\delta).
\end{align*}
又因为$g(a) =0$,所以
\begin{align}
g(x) \leqslant slant g(c) <g(a),\forall x\in (c-\delta ,c+\delta).\label{example000.6-1.1}
\end{align}
...
\end{align*}
故$\max_{x\in [0,1]} |f(x)|\geqslant slant 4$.又因为$\int_0^1{f(x) \mathrm{d}x}=0$,所以由积分中值定理可知,存在$\theta \in (0,1)$,使得$f(\theta) =\left| f(\theta) \right|=0$.从而由介值定理可知,存在$\xi \in [0,1]$,使得$\left| f(\xi) \right|=4$.
\end{proof}
