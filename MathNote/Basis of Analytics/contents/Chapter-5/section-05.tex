...
\begin{align*}
\int_a^b f(x) \mathrm{d}x \xlongequal{x = g(t)} \int_0^1 f(g(t))g'(t) dt.
\end{align*}
即$g(0) = a$，$g(1) = b$。因此$g(t) = \frac{b - a}{1 - 0}t + a = a + (b - a)t$。从而
\begin{align*}
\int_a^b f(x) \mathrm{d}x \xlongequal{x = a + (b - a)t} (b - a)\int_0^1 f(a + (b - a)t) dt = (b - a)\int_0^1 f((1 - t)a + bt) dt.
\end{align*} 
\end{remark}
\begin{proof}
...
\begin{proof}
\begin{enumerate}[(1)]
\item 令$F(x) = \int_0^x e^{f(t)} dt - \int_x^1 e^{-f(t)} dt$，则$F(0) = -\int_0^1 e^{-f(t)} dt < 0$，$F(1) = \int_0^1 e^{f(t)} dt > 0$。又$F^\prime(x) = e^{f(x)} + e^{-f(x)} > 0$，
故由零点存在定理可知，存在唯一的$\xi \in (0,1)$ ，使得$F(\xi) = 0$，即$\int_0^{\xi} e^{f(t)} dt = \int_{\xi}^1 e^{-f(t)} dt$.

\item 令$F_n(x)=\int_{\frac{1}{n}}^x{e^{f(t)}dt}-\int_x^1{e^{-f(t)}dt}$,则$F_n\left(\frac{1}{n}\right)=-\int_{\frac{1}{n}}^1{e^{-f(t)}dt}<0$,$F_n(1)=\int_x^1{e^{-f(t)}dt}>0$.又$F_{n}^{\prime}(x)=e^{f(x)}+e^{-f(x)}>0$,故由零点存在定理可知,存在唯一的$\xi_n\in\left(\frac{1}{n},1\right)$,使得$F(\xi_n)=0$,即
\begin{align}
\int_{\frac{1}{n}}^{\xi_n}{e^{f(t)}dt}=\int_{\xi_n}^1{e^{-f(t)}dt}.\label{example234-1.1}
\end{align}
