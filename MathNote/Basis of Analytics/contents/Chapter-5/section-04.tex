...
因此由L'Hospital法则可得
\begin{align*}
f''(0)\lim_{x\to 0^+}\frac{\xi^2(x)}{x^2} &= \lim_{x\to 0^+}\frac{f''(\theta(x))\xi^2(x)}{x^2} = \lim_{x\to 0^+}\frac{2\left(\int_0^x f(t)dt - xf(0)\right)}{x^3}\\
...
\begin{example}
设$f$在$x = a$的邻域$n + p$阶可导且$p\geq 1$，于是有
\begin{align}
