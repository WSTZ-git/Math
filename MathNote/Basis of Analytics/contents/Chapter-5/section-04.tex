...
\begin{align*}
\lim_{x\to a^+}\left(\frac{c - a}{x - a}\right)^p &= \lim_{x\to a^+}\left[p!\cdot\frac{f^{(n)}(c) - f^{(n)}(a)}{(x - a)^pf^{(n + p)}(\theta)}\right] = \lim_{x\to a^+}\left[p!\cdot\frac{\frac{n![f(x) - \sum\limits_{j = 0}^{n - 1}\frac{f^{(j)}(a)}{j!}(x - a)^j]}{(x - a)^n} - f^{(n)}(a)}{(x - a)^pf^{(n + p)}(\theta)}\right]\\
