...
证明:
\begin{align*}
\mu(t)\leqslant\alpha(t)+\int_{a}^{t}\beta(s)\alpha(s)e^{\int_{s}^{t}\beta(u)\mathrm{d}u}ds,\forall t\in[a,b].
\end{align*}
若还有 $\alpha$ 递增，我们有
\begin{align*}
...
这就得到了
\begin{align*}
F(t)e^{-\int_{a}^{t}\beta(s)ds}\leqslant\int_{a}^{t}\beta(s)\alpha(s)e^{\int_{s}^{a}\beta(u)\mathrm{d}u}ds.
\end{align*}
再用一次不等式\eqref{equation---12.6}，即得
\begin{align*}
\mu(t)\leqslant\alpha(t)+F(t)\leqslant\alpha(t)+\int_{a}^{t}\beta(s)\alpha(s)e^{\int_{s}^{t}\beta(u)\mathrm{d}u}ds,\forall t\in[a,b].
\end{align*}
特别的，当 $\alpha$ 递增，对$\forall t\in [a,b]$,固定$t$,记$G(s)=\int_s^t{\beta (u)\mathrm{d}u}$，我们有不等式
\begin{align*}
\mu (t)&\leqslant \alpha (t)+\alpha (t)\int_a^t{\beta (s)e^{\int_s^t{\beta (u)\mathrm{d}u}}ds}=\alpha (t)+\alpha (t)\int_a^t{-G' \left( s \right) e^{G\left( s \right)}ds}
\\
