...
\begin{enumerate}[(1)]
\item 设$f\in C[0,+\infty)$满足$\int_{0}^{\infty}f(x)\mathrm{d}x$收敛，则存在趋于$+\infty$的$\{x_n\}_{n = 1}^{\infty}\subset(0,+\infty)$，使得$\lim_{n\to\infty}f(x_n)=0$.

\item 设$f\in R[0,+\infty)$,且$\int_{0}^{\infty}|f(x)|\mathrm{d}x < \infty$，则存在严格递增的$\lim_{n\to\infty}x_n = +\infty$，使得
$\lim_{n\to\infty}x_n\ln x_nf(x_n)=0$.
\end{enumerate}
\end{proposition}
\begin{note}
连续性是否可以去掉构成一个有趣的话题. 第一问结论可以直接用，第二问主要告诉我们积分绝对收敛性，我们总能找到很好的子列极限.并且(2)中结论的$x_n \ln x_n$可以换成任意数列$\{a_n\}$,只要满足$\int_{a}^{\infty}a_n\mathrm{d}x=+\infty$即可
\end{note}
\begin{proof}
\begin{enumerate}[(1)]
\item 运用积分中值定理，我们知道
...
\item 若$|f(x)| > \frac{1}{x\ln x}, \forall x > e$，则由$\int_e^{\infty}{\frac{1}{x\ln x}\mathrm{d}x}=\underset{x\rightarrow +\infty}{\lim}\ln\ln x=+\infty $可得$\int_e^{\infty}{\left| f\left( x \right) \right|\mathrm{d}x}=+\infty $矛盾! 故存在$x_1 > e$使得$|f(x_1)| \leqslant \frac{1}{x_1\ln x_1}$.
...
\item 设$\int_a^{+\infty}{f\left( x \right) \mathrm{d}x}<+\infty$, 存在$p\in \left( 0,1 \right)$, 使得$x^pf\left( x \right)$递减且$\lim\limits_{x\rightarrow +\infty}{x^pf\left( x \right)} =0$, 则$\lim\limits_{x\rightarrow +\infty}{xf\left( x \right)} =0$.
...
\item 由条件可得, 对$\forall A>0$, 有
\begin{align*}
0\leqslant Af\left( A \right) =A\cdot A^pf\left( A \right) \cdot A^{-p}\leqslant 2\int_{\frac{A}{2}}^A{x^pf\left( x \right) \cdot x^{-p}\mathrm{d}x}=2\int_{\frac{A}{2}}^A{f\left( x \right) \mathrm{d}x}.
\end{align*}
令$A\rightarrow +\infty$, 由$\int_a^{+\infty}{f\left( x \right) \mathrm{d}x}$收敛的Cauchy收敛准则知$\lim\limits_{A\rightarrow +\infty}{Af\left( A \right)} =0$.
...
\begin{proposition}\label{proposition:一致连续无穷积分收敛则极限为0}
若$f$在$[0, +\infty)$上一致连续, 且$\int_0^{+\infty} f(x) \, \mathrm{d}x < \infty$, 则$\lim_{x \to +\infty} f(x) = 0$.
\end{proposition}
\begin{note}
本题也有导数版本{见\refpro{proposition:经典导数一致连续问题}}.
\end{note}
\begin{proof}
\textbf{反证}，假设$\lim_{x\rightarrow +\infty} f\left( x \right) \ne 0$,则可不妨设存在$c>0,$,$x_n\rightarrow +\infty$，使得
\begin{align*}
f\left( x_n \right) \geqslant c>0.
\end{align*}
由$f$在$\left[ 0,+\infty \right)$上一致连续可知,存在$\delta >0$，使$\forall n\in \mathbb{N}$，都有
\begin{align*}
\left| f\left( x \right) -f\left( x_n \right) \right|<\frac{c}{2}\Longrightarrow f\left( x \right) >f\left( x_n \right) -\frac{c}{2}\geqslant \frac{c}{2},\quad \forall x\in \left( x_n,x_n+\delta \right) >0.
\end{align*}
于是
\begin{align*}
\int_{x_n}^{x_n+\delta}{f\left( x \right) \mathrm{d}x}\geqslant \int_{x_n}^{x_n+\delta}{\frac{c}{2}\mathrm{d}x}=\frac{c\delta}{2},\quad \forall n\in \mathbb{N} .
\end{align*}
...
\begin{proof}
\begin{enumerate}[(1)]
\item 由$f>0$和\refpro{proposition:反常积分收敛的基本结论2}可知
\begin{align*}
\int_a^{\infty}{f\left( x \right) \mathrm{d}x}=\underset{m\rightarrow \infty}{\lim}\int_a^{a+mx_0}{f\left( x \right) \mathrm{d}x}=\sum_{n=1}^{\infty}{\int_{a+\left( n-1 \right) x_0}^{a+nx_0}{f\left( x \right) \mathrm{d}x}}\triangleq \sum_{n=1}^{\infty}{a_n.}
\end{align*}
...
\item 根据题设,令$x=e^t$,任取$c>0$,再令$g(x)=f(x)e^x$,则
\begin{align*}
\int_c^{\infty}{f\left( x \right) \mathrm{d}x}=\int_{\ln c}^{\infty}{f\left( e^t \right) e^t\mathrm{d}t}=\int_{\ln c}^{\infty}{g\left( t \right) \mathrm{d}t},
\end{align*}
\begin{align*}
\underset{t\rightarrow +\infty}{\lim}\frac{g\left( t+\ln k \right)}{g\left( t \right)}=\underset{t\rightarrow +\infty}{\lim}\frac{f\left( e^{t+\ln k} \right) e^{t+\ln k}}{f\left( e^t \right) e^t}=k\underset{t\rightarrow +\infty}{\lim}\frac{f\left( ke^t \right)}{f\left( e^t \right) e^t}=k\ell .
\end{align*}
于是由(1)可知结论成立.
...
\begin{align*}
f\left( x \right) \leqslant \frac{1}{x^{-p-\varepsilon}},\forall x>X_1.
\end{align*}
注意到此时$-p-\varepsilon>1$,故$\int_{X_1}^{\infty}{\frac{1}{x^{-p-\varepsilon}}\mathrm{d}x}<+\infty .$由比较判别法知$\int_{X_1}^{\infty}{f\left( x \right) \mathrm{d}x}<+\infty .$因此$\int_{a}^{\infty}{f\left( x \right) \mathrm{d}x}<+\infty .$

当$p\geqslant -1$也即$-p \leqslant 1$时,取$\varepsilon =\frac{p+1}{2}$,则由上式知,存在$X_2>e$,使得
\begin{align*}
f\left( x \right) \geqslant \frac{1}{x^{-p+\varepsilon}},\forall x>X_2.
\end{align*}
注意到此时$-p+\varepsilon \leqslant 1$,故$\int_{X_1}^{\infty}{\frac{1}{x^{-p+\varepsilon}}\mathrm{d}x}=+\infty .$由比较判别法知$\int_{X_1}^{\infty}{f\left( x \right) \mathrm{d}x}=+\infty .$因此$\int_{a}^{\infty}{f\left( x \right) \mathrm{d}x}=+\infty .$
...
于是
\begin{align*}
\int_0^{\infty} \left| \frac{1}{f(x) + f'(x)} - \frac{1}{f(x)} \right| \, \mathrm{d}x &= \int_0^{\infty} \frac{f'(x)}{f(x) \left[ f(x) + f'(x) \right]} \, \mathrm{d}x \leqslant \int_0^{\infty} \frac{f'(x)}{f^2(x)} \, \mathrm{d}x \\
&= \int_0^{\infty} \frac{1}{f^2(x)} \, \mathrm{d}f(x) = \frac{1}{f(0)} - \frac{1}{f(+\infty)} < +\infty.
\end{align*}
注意到
\begin{align*}
...
{\color{blue} 证法二:}由\hyperref[Real Analysis-theorem:Fatou引理]{Fatou引理}可得
\begin{align*}
...
\end{align*}
从而$\lim_{x \to +\infty} f^3(x)$存在. 由$\int_0^{+\infty} \left| f(x) \right|^2 \, \mathrm{d}x < \infty$及\nrefpro{proposition:积分收敛必有子列趋于0}{(1)}可知, 存在$\{x_n\}$, 满足$x_n \to +\infty$, 使得$f^2(x_n) \to 0$.
故$\lim_{x\rightarrow +\infty} f^3(x)=\lim_{n\rightarrow \infty} \left[ f^2(x_n) \right] ^{\frac{3}{2}}=0$, 因此$\lim_{x \to +\infty} f(x) = 0$.
