...
\item $\mathbf{Step}\mathbb{1}$ 我们证明\(k = 0\)时命题成立. 对任何\(\varepsilon>0\)，存在\(\delta>0\)，使得只要\(\vert x - y\vert\leqslant\delta\)，就有
\[
...
$\mathbf{Step}\mathbb{3}$ \((*)\) 我们证明一个中值定理的结果.
...
$\mathbf{Step}\mathbb{5}$ \((*)\) $\mathbf{Step}\mathbb{1}$告诉我们关于\(x\in[0,1]\)，一致的有
\[
...
\]
因此当正整数\(n>\max\left\{\frac{2j}{\delta},j,N\right\}\)，利用$\mathbf{Step}\mathbb{4}$，我们有
\[
...
\end{note}
\begin{proof}
$\mathbf{Step}\mathbb{1}$: 当 $f\in C_c(\mathbb{R})$ 时, 则存在 $X>0$, 使得
\begin{align*}
...
\end{align*}
$\mathbf{Step}\mathbb{2}$: 对一般的 $f$, 满足 $\int_{-\infty}^{\infty} |f(x)|^p \mathrm{d}x < \infty$. 对 $\forall \varepsilon > 0$, 由\hyperref[theorem:可积被连续函数逼近]{定理\ref{theorem:可积被连续函数逼近}(3)}可知, 存在 $g\in C_c(\mathbb{R})$, 使得
\begin{align*}
\int_{-\infty}^{+\infty} |f(x) - g(x)|^p \mathrm{d}x < \varepsilon.
\end{align*}
从而
...
\end{align}
由 $g\in C_c(\mathbb{R})$, 结合 $\mathbf{Step}\mathbb{1}$ 可知
\begin{align}
