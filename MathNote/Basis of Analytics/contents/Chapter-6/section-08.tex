...
\begin{proof}
\begin{enumerate}[(1)]
\item 对\(n\geqslant slant1\)，直接计算得
\begin{align*}
...
\item 如果\(f\)递增，那么就有$\varphi\geqslant slant 0$,则由\eqref{equation--Bernstein多项式性质13.115}知\(B_n^\prime(f,x)=B_{n - 1}(\varphi,x)\geqslant slant0\)，故\(B_n(f,x)\)递增. 类似可得递减.

\item 如果\(f\)下凸，对\(n = 1\)的情况是否符合条件都可以单独验证，我们略去过程，下设\(n\geqslant slant2\). 注意继续由\eqref{equation--Bernstein多项式性质13.115}知
...
\item $\mathbf{Step}\mathbf{1}$ 我们证明\(k = 0\)时命题成立. 对任何\(\varepsilon>0\)，存在\(\delta>0\)，使得只要\(\vert x - y\vert\leqslant slant\delta\)，就有
\[
\vert f(x)-f(y)\vert\leqslant slant\varepsilon.
\]
注意到
\begin{align*}
\vert B_n(f,x)-f(x)\vert&=\left|\sum_{k = 0}^n\left[f\left(\frac{k}{n}\right)-f(x)\right]C_n^k x^k(1 - x)^{n - k}\right|\\
&\leqslant slant\sum_{\vert\frac{k}{n}-x\vert\leqslant slant\delta}\left|\left[f\left(\frac{k}{n}\right)-f(x)\right]C_n^k x^k(1 - x)^{n - k}\right|+\sum_{\vert\frac{k}{n}-x\vert>\delta}\left|\left[f\left(\frac{k}{n}\right)-f(x)\right]C_n^k x^k(1 - x)^{n - k}\right|\\
&\leqslant slant\varepsilon\sum_{\vert\frac{k}{n}-x\vert\leqslant slant\delta}\left|C_n^k x^k(1 - x)^{n - k}\right|+2\sup\vert f\vert\sum_{\vert\frac{k}{n}-x\vert>\delta}\left|C_n^k x^k(1 - x)^{n - k}\right|\\
&\overset{\text{类似}Chebeshev\text{不等式的证明}}{\leqslant slant}\varepsilon\sum_{k = 0}^n C_n^k x^k(1 - x)^{n - k}+\frac{2\sup\vert f\vert}{\delta^2}\sum_{\vert\frac{k}{n}-x\vert>\delta}\left(\frac{k}{n}-x\right)^2C_n^k x^k(1 - x)^{n - k}\\
&\leqslant slant\varepsilon+\frac{2\sup\vert f\vert}{\delta^2}\sum_{k = 0}^n\left(\frac{k}{n}-x\right)^2C_n^k x^k(1 - x)^{n - k}\\
&\overset{\reflem{lemma:一些常见函数的Bernstein多项式}}{=}\varepsilon+\frac{2\sup\vert f\vert}{n\delta^2}x(1 - x),
\end{align*}
于是从上式立得
\[
\sup_{x\in[0,1]}\vert B_n(f,x)-f(x)\vert\leqslant slant \varepsilon+\frac{\sup\vert f\vert}{2n\delta^2}.
\]
令$n\to \infty$,得
\begin{align*}
\underset{n\rightarrow \infty}{\overline{\lim }}\mathop {\mathrm{sup}} \limits_{x\in [0,1]}\left| B_n(f,x)-f(x) \right|\leqslant slant \varepsilon .
\end{align*}
...
\[
B_n^{(j)}(f,x)=B_{n - j}(T_{n - j + 1}\cdots T_{n - 1}T_nf,x),1\leqslant slant j\leqslant slant k,n>k.
\]
于是运用$\mathbf{Step}\mathbf{3}$，我们有
\begin{align*}
&\vert B_n^{(j)}(f,x)-f^{(j)}(x)\vert
\leqslant slant\vert B_{n - j}(f^{(j)},x)-f^{(j)}(x)\vert+\vert B_{n - j}(f^{(j)},x)-B_{n - j}(T_{n - j + 1}\cdots T_{n - 1}T_nf,x)\vert\\
&\leqslant slant\vert B_{n - j}(f^{(j)},x)-f^{(j)}(x)\vert
...
&\leqslant slant\vert B_{n - j}(f^{(j)},x)-f^{(j)}(x)\vert+\sum_{i = 0}^{n - j}\vert f^{(j)}\left(\frac{i}{n - j}\right)-f^{(j)}(\theta)\vert C_{n - j}^ix^i(1 - x)^{n - j - i}\\
...
以及
\[
\left|\frac{i}{n}-\frac{i}{n - j}\right|=\frac{ji}{n(n - j)}\leqslant slant\frac{j}{n},\left|\frac{i + j}{n}-\frac{i}{n - j}\right|\leqslant slant\frac{2j}{n},\forall n>j.
\]
我们同时假设
\[
...
\begin{align*}
\left| f^{(s)}(x) - p^{(s)}(x) \right| \leqslant slant \varepsilon, \forall x \in [a,b], s = 0,1,2,\cdots,k.
\end{align*}
\end{theorem}
\begin{remark}
...
\begin{align}
|q(x) - f^{(k)}(x)| \leqslant slant \varepsilon, \forall x \in [a,b]. \label{theorem-equation-多项式逼近有限光滑函数1.1}
\end{align}
设
\begin{align*}
...
\begin{align*}
|f^{(s)}(x) - p^{(s)}(x)| = \left| \frac{1}{(k - s - 1)!} \int_a^x (x - t)^{k - s - 1} [f^{(k)}(t) - q(t)] \mathrm{d}t \right| \leqslant slant \frac{\varepsilon (b - a)^{k - s}}{(k - s)!}, \forall x \in [a,b].
\end{align*}
故结论得证.
\end{proof}
...
\begin{align*}
|p_m(x) - p_n(x)| \leqslant slant 1, \forall m > n \geqslant slant N, x\in \mathbb{R}.
\end{align*}
由于有界的多项式函数一定是常值函数, 因此 $p_m(x) - p_n(x) = C, \forall m > n \geqslant slant N, x\in \mathbb{R}$. 其中 $C$ 是一个常数. 故
\begin{align}
p_n(x) = p_N(x) + c_n, \forall n \geqslant slant N, x\in \mathbb{R}. \label{equation----:::-1.1}
\end{align}
...
\item 设 $p \geqslant slant 1$ 且反常积分 $\int_{-\infty}^{\infty} |f(x)|^{p}\mathrm{d}x < \infty$, 则对任何 $\varepsilon > 0$, 存在 $g \in C_c(\mathbb{R})$, 使得
\begin{align*}
...
\[
\sum_{i = 1}^{n} w_i(f)(x_i - x_{i - 1}) \leqslant slant \varepsilon, w_i(f) \text{ 表示 } f \text{ 在 } [x_{i - 1},x_i], i = 1,2,\cdots,n \text{ 的振幅}.
\]
连接线段 $(x_{i - 1},f(x_{i - 1})),(x_i,f(x_i)), i = 1,2,\cdots,n$ 得到连续函数 $g$.(不难发现$\underset{x\in \left[ a,b \right]}{\mathrm{sup}}\left| g \right|\leqslant slant \underset{x\in \left[ a,b \right]}{\mathrm{sup}}\left| f \right|$) 则
\begin{align*}
\int_{a}^{b} |f(x) - g(x)|\mathrm{d}x &= \sum_{i = 1}^{n} \int_{x_{i - 1}}^{x_i} |f(x) - g(x)|\mathrm{d}x \\
&\leqslant slant \sum_{i = 1}^{n} \int_{x_{i - 1}}^{x_i} |f(x) - f(x_{i - 1})|\mathrm{d}x + \sum_{i = 1}^{n} \int_{x_{i - 1}}^{x_i} |g(x) - f(x_{i - 1})|\mathrm{d}x \\
&\leqslant slant \sum_{i = 1}^{n} w_i(f)(x_i - x_{i - 1}) + \sum_{i = 1}^{n} \int_{x_{i - 1}}^{x_i} \left| \frac{f(x_i) - f(x_{i - 1})}{x_i - x_{i - 1}} (x - x_{i - 1}) \right|\mathrm{d}x \\
&\leqslant slant \sum_{i = 1}^{n} w_i(f)(x_i - x_{i - 1}) + \sum_{i = 1}^{n} w_i(f) \int_{x_{i - 1}}^{x_i} \frac{x - x_{i - 1}}{x_i - x_{i - 1}}\mathrm{d}x \\
&= \sum_{i = 1}^{n} w_i(f)(x_i - x_{i - 1}) + \frac{1}{2} \sum_{i = 1}^{n} w_i(f)(x_i - x_{i - 1}) \leqslant slant \frac{3}{2}\varepsilon,
\end{align*}
这就完成了证明.
...
(a): $0 \leqslant slant h(x) \leqslant slant 1, \forall x \in \mathbb{R}$;

(b): $h(x) = 0, \forall x \in (-\infty,a + \delta) \cup (b - \delta,+\infty)$;

(c): $h(x) = 1, \forall x \in [a + 2\delta,b - 2\delta]$.

于是取 $g_1(x) = h(x)g(x) \in C_c(a,b)$,由第1问可知$\underset{x\in \left[ a,b \right]}{\mathrm{sup}}\left| g \right|\leqslant slant \underset{x\in \left[ a,b \right]}{\mathrm{sup}}\left| f \right|$,从而$\underset{x\in \left[ a,b \right]}{\mathrm{sup}}\left| g_1 \right|\leqslant slant \underset{x\in \left[ a,b \right]}{\mathrm{sup}}\left| f \right|$.
从而
\begin{align*}
\int_{a}^{b} |f(x) - g_1(x)|\mathrm{d}x &= \int_{a}^{b} |f(x) - g(x)h(x)|\mathrm{d}x \\
&\leqslant slant \int_{a}^{a + \delta} |f(x)|\mathrm{d}x + \int_{b - \delta}^{b} |f(x)|\mathrm{d}x + \int_{a + \delta}^{b - \delta} |f(x) - h(x)g(x)|\mathrm{d}x \\
&\leqslant slant \int_{a}^{a + \delta} |f(x)|\mathrm{d}x + \int_{b - \delta}^{b} |f(x)|\mathrm{d}x + \int_{a + \delta}^{a + 2\delta} |f(x) - g(x)|\mathrm{d}x + \int_{a + \delta}^{b - \delta} |g(x) - h(x)g(x)|\mathrm{d}x \\
&\leqslant slant \int_{a}^{a + \delta} |f(x)|\mathrm{d}x + \int_{b - \delta}^{b} |f(x)|\mathrm{d}x + \int_{a}^{b} |f(x) - g(x)|\mathrm{d}x + \int_{a + \delta}^{b - \delta} |g(x) - h(x)g(x)|\mathrm{d}x \\
&\leqslant slant \frac{3\varepsilon}{4} + \int_{a + \delta}^{a + 2\delta} |g(x) - h(x)g(x)|\mathrm{d}x + \int_{b - 2\delta}^{b - \delta} |g(x) - h(x)g(x)|\mathrm{d}x \\
&\leqslant slant \frac{3\varepsilon}{4} + 2 \int_{a + \delta}^{a + 2\delta} |g(x)|\mathrm{d}x + 2 \int_{b - 2\delta}^{b - \delta} |g(x)|\mathrm{d}x \\
&\leqslant slant \varepsilon.
\end{align*}
这就完成了证明.

\item 证明的想法和第2问类似. 由条件可知,对任何 $\varepsilon > 0$, 存在$X > 0$, 使得
\[
\int_{|x| \geqslant slant X} |f(x)|^{p}\mathrm{d}x=
...
从前两问的构造可以看到
\[
\sup_{[-X,X]} |g| \leqslant slant \sup_{[-X,X]} |f|,
\]
于是
\begin{align*}
\int_{-\infty}^{\infty} |f(x) - g(x)|^{p}\mathrm{d}x &= \int_{|x| \geqslant slant X} |f(x)|^{p}\mathrm{d}x + \int_{-X}^{X} |f(x) - g(x)|^{p}\mathrm{d}x \\
&\leqslant slant \varepsilon + \sup_{[-X,X]} |f - g|^{p - 1} \int_{-X}^{X} |f(x) - g(x)|\mathrm{d}x \\
&\leqslant slant \varepsilon + \sup_{[-X,X]} (2|f|)^{p - 1} \int_{-X}^{X} |f(x) - g(x)|\mathrm{d}x \\
...
\begin{align*}
|A + B + C|^p \leqslant slant C_p(|A|^p + |B|^p + |C|^p).
\end{align*}
\end{lemma}
\begin{note}
利用齐次化方法证明齐次不等式的应用.
...
\begin{align}
|A + B + C|^p \leqslant slant C_p, \forall (A,B,C) \in S. \label{theorem-equation-积分的绝对连续性-0.1}
\end{align}
...
\begin{align*}
|LA + LB + LC|^p \leqslant slant C_p.
\end{align*}
于是
\begin{align*}
|A + B + C|^p \leqslant slant \frac{C_p}{L^p} = C_p(|A|^p + |B|^p + |C|^p).
\end{align*}
故结论得证. 
\end{proof}

\begin{theorem}[积分的绝对连续性]\label{theorem:积分的绝对连续性}
设 $p \geqslant slant 1$ 且反常积分 $\int_{-\infty}^{\infty} |f(x)|^{p}\mathrm{d}x < \infty$, 证明
\begin{align}
...
\begin{align*}
f(x) = 0, \forall |x| \geqslant slant X.
\end{align*}
从而当 $h\in (-1,1)$ 时, 就有
\begin{align*}
f(x) = 0, \forall |x| \geqslant slant X + 1.
\end{align*}
又因为 $f\in C[-X - 1,X + 1]$, 所以由 Cantor 定理可知 $f$ 在 $[-X - 1,X + 1]$ 上一致连续. 于是
\begin{align*}
\lim_{h\rightarrow 0} \int_{-\infty}^{+\infty} |f(x + h) - f(x)|^p \mathrm{d}x &= \lim_{h\rightarrow 0} \int_{|x| \leqslant slant X + 1} |f(x + h) - f(x)|^p \mathrm{d}x \\
&= \int_{|x| \leqslant slant X + 1} \lim_{h\rightarrow 0} |f(x + h) - f(x)|^p \mathrm{d}x = 0.
\end{align*}
...
从而
\begin{align*}
\int_{-\infty}^{+\infty} |f(x + h) - f(x)|^p \mathrm{d}x &\leqslant slant \int_{-\infty}^{+\infty} |f(x + h) - g(x + h) + g(x + h) - g(x) + g(x) - f(x)|^p \mathrm{d}x
\end{align*}
...
故
\begin{align}
\int_{-\infty}^{+\infty} |f(x + h) - f(x)|^p \mathrm{d}x &\leqslant slant C_p\left( \int_{-\infty}^{+\infty} |f(x + h) - g(x + h)|^p \mathrm{d}x + \int_{-\infty}^{+\infty} |g(x + h) - g(x)|^p \mathrm{d}x + \int_{-\infty}^{+\infty} |f(x) - g(x)|^p \mathrm{d}x \right) \nonumber \\
&\xlongequal[y=x+h]{\text{换元}} 2C_p \int_{-\infty}^{+\infty} |f(x) - g(x)|^p \mathrm{d}x + C_p \int_{-\infty}^{+\infty} |g(x + h) - g(x)|^p \mathrm{d}x \nonumber \\
&\leqslant slant 2\varepsilon C_p + C_p \int_{-\infty}^{+\infty} |g(x + h) - g(x)|^p \mathrm{d}x. \label{theorem-equation-积分的绝对连续性-2.0}
\end{align}
...
\begin{align*}
\underset{h\rightarrow 0}{\overline{\lim }} \int_{-\infty}^{+\infty} |f(x + h) - f(x)|^p \mathrm{d}x \leqslant slant 2\varepsilon C_p.
\end{align*}
