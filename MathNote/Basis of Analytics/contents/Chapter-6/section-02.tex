...
依次下去可得
\begin{align}
f(nx)=nf(x),\forall n\in \mathbb{N}. \label{proposition6.10-1.1}
\end{align}
...
\item 在区间单调自然在子区间上有界，再利用(2)即得\(f(x)=f(1)x,\forall x\in\mathbb{R}\).

\item 见\refpro{Real Analysis-proposition:存在某个正测集的Cauchy方程函数必是线性函数}.

\item 由\hyperref[Real Analysis-theorem:Lusin(卢津)定理]{Lusin定理}，存在有正测度的闭集\(K\),使得\(f(x)\)在$K$上连续.再利用(1)的结论即得\(f(x)=f(1)x,\forall x\in\mathbb{R}\).
...
\begin{proposition}[\(\mathbb{R}\)上的既凸又凹的连续函数是直线]\label{proposition:R上的既凸又凹的连续函数是直线}
...
\begin{example}
定义在$(-\infty,+\infty)$的函数$f(x)$有非负的三阶导数，满足方程
\begin{align*}
f'(x) = f(x+1) - f(x) - 1.
\end{align*}
求所有满足条件的$f(x)$。
...
由题设知$f'''(x) \geqslant 0\left( \forall x\in \mathbb{R} \right)$, 故$f''$在$\mathbb{R}$上递增.

下证$f''$在$\mathbb{R}$上恒为常数. 假设存在$x_0,x_1\in \mathbb{R}$且$x_1>x_0$, 满足
\begin{align*}
f''(x_0) < f''(x_1).
\end{align*}
由条件和Lagrange中值定理知, $\exists \xi_0\in (x_0,x_0+1)$, 使得
\begin{align}
f''(x_0) = f'(x_0+1) - f'(x_0) = f''(\xi_0).
\label{eq:f2nd_xi0_31a}
\end{align}
令
\begin{align*}
E \triangleq \left\{ x\in \mathbb{R} \mid f''(x) = f''(x_0) \right\},
\end{align*}
则由\eqref{eq:f2nd_xi0_31a}式知$\xi_0\in E$, 故$E\ne \varnothing$. 再记$M \triangleq \sup E$.

若$M<+\infty$, 则由$f\in D^3(\mathbb{R})$知$f''\in C(\mathbb{R})$, 故$f''(M) = f''(x_0)$. 从而由条件和Lagrange中值定理知, $\exists \xi_M\in (M,M+1)$, 使得
\begin{align*}
f''(x_0) = f''(M) = f'(M+1) - f'(M) = f''(\xi_M) \Longrightarrow \xi_M\in E,
\end{align*}
...
代入题设等式得
\begin{align*}
f'(x) = f(x+1) - f(x) - 1,\forall x\in \mathbb{R} \Longleftrightarrow 2ax+b=2ax+a+b-1,\forall x\in \mathbb{R} \Longleftrightarrow a=1.
\end{align*}
故所有满足条件的$f(x)$只有
$x^2+bx+c$, 其中$b,c$为任意实数.
\end{solution}
