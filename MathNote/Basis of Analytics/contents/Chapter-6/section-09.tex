...
\begin{example}
已知$f(x)\in C^1(\mathbb{R})$且有界。证明：若$|f'(x)-f(x)|\leqslant 1$，则$|f(x)|\leqslant 1$。
\end{example}
\begin{proof}
令$g(x)=\frac{f(x)}{e^x}$，则由条件知
\begin{align*}
|g'(x)|=\frac{|f'(x)-f(x)|}{e^x}\leqslant e^{-x},\quad \forall x\in\mathbb{R}.
\end{align*}
...
从而
\begin{align*}
|g(x)|=\frac{|f(x)|}{e^x}\leqslant M e^{-x},\quad \forall x\in\mathbb{R}.
\end{align*}
令$x\to+\infty$得$\lim\limits_{x\to+\infty}|g(x)|=0$。对$\forall x,y\in\mathbb{R}$且$x<y$，有
\begin{align*}
|g(x)-g(y)|=\biggl|\int_{x}^{y}g'(t)\,\mathrm{d}t\biggr|\leqslant \int_{x}^{y}e^{-t}\,\mathrm{d}t=e^{-x}-e^{-y}.
\end{align*}
于是
\begin{align*}
|g(x)|\leqslant |g(x)-g(y)|+|g(y)|\leqslant e^{-x}-e^{-y}+|g(y)|.
\end{align*}
令$y\to+\infty$得$|g(x)|\leqslant e^{-x},\forall x\in\mathbb{R}$。因此
\begin{align*}
|f(x)|=|g(x)|\cdot e^x\leqslant e^{-x}\cdot e^x=1,\quad \forall x\in\mathbb{R}.
\end{align*}
\end{proof}
...
\end{align*}
于是 $g$ 递减，从而 $0\leqslant g(x)\leqslant g(0)=0$, 故 $f(x)\equiv0$. \end{proof}
...
{\color{blue}证法一:}先证明 $f(x)=0$ 对一切 $x\in[0,1)$ 成立。固定任意 $x_0\in[0,1)$，令 $M=3$，并定义
\begin{align*}
g(x)=e^{-Mx}\bigl(|f(x)|^2+|f'(x)|^2\bigr),\qquad x\in[0,x_0].
\end{align*}
利用不等式 $1+t^2\geqslant\sqrt{t}$（$t\geqslant0$），取 $t=\frac{|f(x)|}{|f'(x)|}$（若 $f'(x)=0$ 则结论显然，可单独处理），得到
\begin{align}
|f(x)|^2+|f'(x)|^2\geqslant |f'(x)|\sqrt{|f(x)f'(x)|}. \label{eq:ineq_aux}
\end{align}
于是对 $x\in[0,x_0]$，
\begin{align*}
g'(x)&=e^{-Mx}\left[2ff'+2f'f''-Mf^2-M(f')^2\right] \\
&\leqslant e^{-Mx}\left[2|ff'|+2|f'|\sqrt{|ff'|}-Mf^2-M(f')^2\right] \\
&\stackrel{\eqref{eq:ineq_aux}}{\leqslant} e^{-Mx}\left[2|ff'|+2|f'|^2+2|f|^2-Mf^2-M(f')^2\right] \\
&\leqslant e^{-Mx}\left[|f|^2+|f'|^2+2|f'|^2+2|f|^2-Mf^2-M(f')^2\right]=0,
\end{align*}
最后一步用了均值不等式 $2|ff'|\leqslant |f|^2+|f'|^2$ 以及 $M=3$。因此 $g$ 在 $[0,x_0]$ 上单调递减，从而
\begin{align*}
0\leqslant g(x)\leqslant g(0)=0,
\end{align*}
故 $g(x)\equiv0$，特别 $f(x_0)=0$。由 $x_0$ 的任意性，$f(x)=0$ 对一切 $x\in[0,1)$ 成立。

再证 $x\in(-1,0]$ 的情形。令 $h(t)=f(-t)$，则 $h$ 在 $(0,1)$ 内二阶可导，且
\begin{align*}
h(0)=f(0)=0,\qquad h'(0)=-f'(0)=0,
\end{align*}
并且
\begin{align*}
|h''(t)|^2=|f''(-t)|^2\leqslant |f(-t)f'(-t)|=|h(t)h'(t)|.
\end{align*}
由已证的非负区间结论，$h(t)=0$ 对一切 $t\in[0,1)$ 成立，即 $f(x)=0$ 对一切 $x\in(-1,0]$ 成立。
综上，$f(x)\equiv0$ 在 $(-1,1)$ 内成立。

{\color{blue}证法二:}任取 $x_0 \in (-1,1)$. 当 $x_0 = 0$ 时结论显然成立；以下设 $x_0 \neq 0$. 令
\begin{align*}
...
\begin{align}
M = \sup_{x \in I} |f''(x)| < \infty. \label{eq:Mdef}
\end{align}
下面估计 $f'$. 对任意 $x \in I$，由拉格朗日中值定理，存在介于 $0$ 与 $x$ 之间的点 $\xi$，使得
\begin{align*}
f'(x) - f'(0) = f''(\xi)x.
\end{align*}
利用 $f'(0) = 0$ 以及 $|x| \leqslant L$，得到
\begin{align*}
|f'(x)| = |f''(\xi)| |x| \leqslant ML.
\end{align*}
因此
\begin{align*}
\sup_{x \in I} |f'(x)| \leqslant ML.
\end{align*}
再估计 $f$. 对任意 $x \in I$，再次由拉格朗日中值定理，存在介于 $0$ 与 $x$ 之间的点 $\eta$，使得
\begin{align*}
f(x) - f(0) = f'(\eta)x.
\end{align*}
利用 $f(0) = 0$ 和上面对 $f'$ 的估计，得到
\begin{align*}
|f(x)| = |f'(\eta)| |x| \leqslant ML \cdot L = ML^2.
\end{align*}
所以
\begin{align*}
\sup_{x \in I} |f(x)| \leqslant ML^2.
\end{align*}
将以上两个估计代入原不等式. 对每个 $x \in I$，都有
\begin{align*}
|f''(x)|^2 \leqslant |f(x)f'(x)| \leqslant (ML^2)(ML) = M^2L^3.
\end{align*}
对 $x \in I$ 取上确界，得到
\begin{align*}
M^2 \leqslant M^2L^3\Longrightarrow (1 - L^3)M^2 \leqslant 0.
\end{align*}
但 $0 < L < 1$，故 $1 - L^3 > 0$；同时 $M^2 \geqslant 0$. 因此只能有
\begin{align*}
M = 0.
\end{align*}
...
&\overset{\text{均值不等式}}{\leqslant} e^{-Mx}\left[f^2+(f')^2+2(f')^2+f^2+(f')^2-Mf^2-M(f')^2\right]\\
...
故 $f(x)=0,\forall x\geqslant0$. \end{proof}
...
进而 $f$ 就是常值函数, 又 $f(0)=0$, 故 $f(x)=0,\forall x\geqslant0$. \end{proof}
...
\end{note}
\begin{remark}
也可以类似\hyperref[theorem:Schwarz导数的Lagrange中值定理]{Schwarz导数的Lagrange中值定理}的证明来构造集合$A$.
\end{remark}
\begin{proof}
反证，假设
\begin{align*}
