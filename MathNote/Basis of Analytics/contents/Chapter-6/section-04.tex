...
\begin{example}
设$\{f_n(x)\}$是定义在$[a,b]$上的无穷次可微的函数序列且逐点收敛，并在$[a,b]$上满足$\left|f_n'(x)\right| \leq M$.
\begin{enumerate}[(1)]
...
\end{align*}
满足$x_i-x_{i-1}<\varepsilon$，$i=1,2,\cdots,n.$于是当$\forall n>m>N$时，对$\forall x\in [a,b]$，都存在$i\in \{ 1,2,\cdots,n \}$，使得$x\in [x_{i-1},x_i]$.再利用\eqref{eq:21.1}和Lagrange中值定理可得
\begin{align*}
