...
\end{note}
\begin{proof}
因为\(\lim_{x\rightarrow +\infty}f'(x)=c > 0\)，所以存在\(X > a\)，使得\(f'(x)>\frac{c}{2},\forall x\geqslant slant X\)。于是由Lagrange中值定理得到,对$\forall x\geqslant slant X$,存在$\theta \in (X,x)$,使得
\[f(x)=f(X)+f'(\theta)(x - X)\geqslant slant f(X)+\frac{c}{2}(x - X),\forall x\geqslant slant X.\]
...
\item \hypertarget{m的存在性}{存在$m>0$,使得$f^{(k)}(x)\geqslant slant m>0,\forall x\geqslant slant X$的原因:}假设对\(\forall m>0\)，有\(m > f^{(k)}(x) > 0\)，\(\forall x\geqslant slant X\)。再令\(m\rightarrow 0^+\)，则由夹逼准则可得\(f^{(k)}(x) = 0\)，\(\forall x\geqslant slant X\)。这与假设矛盾!
(也可以用下极限证明)
\end{enumerate}
\end{note}
\begin{proof}
...
\begin{align*}
f^{(k)}(x)>0,\forall x\geqslant slant X.
\end{align*}
从而\hyperlink{m的存在性}{一定存在$m>0$(见笔记(2))},使得
\begin{align}\label{proposition5.4-12.5}
f^{(k)}(x)\geqslant slant m>0,\forall x\geqslant slant X.
\end{align}
则由Taylor中值定理,我们知道对每个\(x > X\)，运用\eqref{proposition5.4-12.5}，都有
\[f(x)=\sum_{j = 0}^{k - 1}\frac{f^{(j)}(X)}{j!}(x - X)^j+\frac{f^{(k)}(\theta)}{k!}(x - X)^k\geqslant slant\sum_{j = 0}^{k - 1}\frac{f^{(j)}(X)}{j!}(x - X)^j+\frac{m}{k!}(x - X)^k,\]
...
\[
f(s)\leqslant slant f(c - h)<f(c)<f(c + h)\leqslant slant f(t),
\]
...
设\(p_1,p_2,\cdots,p_n\geqslant slant0\)且\(\sum_{j = 1}^{n}p_j = 1\)。则对有介值性函数\(f:[a,b]\to\mathbb{R}\)和\(a\leqslant slant x_1\leqslant slant x_2\leqslant slant\cdots\leqslant slant x_n\leqslant slant b\)，必然存在\(\theta\in[x_1,x_n]\)，使得
\begin{align*}
...
设
\[
M=\max_{1\leqslant slant i\leqslant slant n}f(x_i),m=\min_{1\leqslant slant i\leqslant slant n}f(x_i).
\]
于是
\[
m = m\sum_{j = 1}^{n}p_j\leqslant slant\sum_{j = 1}^{n}p_jf(x_j)\leqslant slant M\sum_{j = 1}^{n}p_j = M.
\]
...
\begin{align*}
\lim_{x\rightarrow c^-}f(x) \leqslant slant f(c) \leqslant slant \lim_{x\rightarrow c^+}f(x).
\end{align*}
又因为$f(x)$在$x=c$处间断，所以上式至少有一个严格不等号成立，故不妨设
\begin{align*}
\lim_{x\rightarrow c^-}f(x) \leqslant slant f(c) < \lim_{x\rightarrow c^+}f(x).
\end{align*}
对$\forall x>c$，固定$x$，由$f$在$I$上递增可知
\begin{align*}
f(x) > f(y), \quad \forall y \in (c, x).
\end{align*}
令$y\rightarrow c^+$，得$f(x) \geqslant slant \lim_{x\rightarrow c^+}f(x)$。对$\forall x<c$，由$f$在$I$上递增可知$f(x) \leqslant slant f(c)$。
...
\end{example}
\begin{note}
\hypertarget{取这个函数的原因}{\textbf{取$\boldsymbol{f}\mathbf{(}\boldsymbol{x}\mathbf{)}=\sqrt[\mathbf{4}]{\boldsymbol{k}}\boldsymbol{x}^{\mathbf{3}}$的原因:}}当\(k\geqslant slant0\)时,我们可待定\(f(x)=cx^{3}\)，需要\(c^{4}x^{9}=kx^{9}\),从而可取\(c = \sqrt[4]{k}\).
\end{note}
\begin{proof}
...
当\(k\geqslant slant0\)时,\hyperlink{取这个函数的原因}{取$f(x)=\sqrt[4]{k}x^3$},此时$f(x)$满足条件.
\end{proof}
...
\end{note}
\begin{proof}
考虑\(g(x)=f(x)-x\in C[a,b]\)，注意到\(g(a)\geqslant slant0,g(b)\leqslant slant0\)，由连续函数的零点定理知道\(f\)在\([a,b]\)上有不动点。
\end{proof}
...
若在\([a,b]\)上\(f\)不严格递增，则存在\(x_2>x_1\)，使得\(f(x_2)\leqslant slant f(x_1)\)。

若\(x_1>a\)，在\([a,x_2]\)上我们注意到\(f(x_1)\geqslant slant\max\{f(a),f(x_2)\}\)，又由$f$的连续性可知,$f$一定能在$[a,x_2]$上取到最大值.于是\(f\)只能在\((a,x_2)\)达到最大值,从而$f$在\((a,x_2)\)存在极大值点,这和\(f\)在\((a,b)\)没有极值点矛盾！

若\(x_1 = a,x_2 < b\)，则注意到\(f(x_2)\leqslant slant\min\{f(a),f(b)\}\)，同样的\(f\)在\((a,b)\)取得极小值而矛盾。
...
当\(f(c)\geqslant slant f(d)\)时,\hyperlink{令
sigma=max{x in[c,xi]:f(x)=f(d)}的原因
}{令
\[
\sigma=\max\{x\in[c,\xi]:f(x)=f(d)\}.
\]}
注意到\(\sigma<\xi\)。显然\(f'(\sigma)\leqslant slant0\)，因为如果\(f'(\sigma)>0\)会导致在\(\sigma\)右邻域内有大于\(f(d)\)的点，由介值定理可以找到\(\xi>\sigma'>\sigma\)，使得\(f(\sigma')=f(d)\)而和\(\sigma\)是最大值矛盾！而函数值相同的点导数值也相同，因此\(f'(\sigma)=f'(d)>0\)，这与\(f'(\sigma)\leqslant slant0\)矛盾！

当\(f(c)\leqslant slant f(d)\)时类似可得矛盾！我们完成了证明。
\end{proof}

\begin{proposition}[一个经典初等不等式]\label{proposition:一个经典初等不等式}
设\(a,b\geqslant slant0\)，证明：
\begin{align}\label{equation-12.777}
\begin{cases}
a^p + b^p\leqslant slant(a + b)^p\leqslant slant2^{p - 1}(a^p + b^p),& p\geqslant slant1,p\leqslant slant0\\
a^p + b^p\geqslant slant(a + b)^p\geqslant slant2^{p - 1}(a^p + b^p),& 0 < p < 1
\end{cases}
\end{align}
\end{proposition}
\begin{note}
...
\[
\begin{cases}
t^p + 1\leqslant slant(t + 1)^p\leqslant slant2^{p - 1}(t^p + 1),& p\geqslant slant1,p\leqslant slant0\\
t^p + 1\geqslant slant(t + 1)^p\geqslant slant2^{p - 1}(t^p + 1),& 0 < p < 1
\end{cases}.
\]
\end{note}
\begin{proof}
...
\begin{cases}
\geqslant slant0,& p\geqslant slant1,p\leqslant slant0\\
<0,& 0 < p < 1
\end{cases}
\]
于是
\[
\begin{cases}
2^{p - 1}=f(1)\geqslant slant f(t)\geqslant slant f(0)=1,& p\geqslant slant1,p\leqslant slant0\\
2^{p - 1}=f(1)\leqslant slant f(t)\leqslant slant f(0)=1,& 0 < p < 1
\end{cases}
\]
这就完成了证明.
\end{proof}
