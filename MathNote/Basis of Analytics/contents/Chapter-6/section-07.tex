...
\end{align}
如果$f$在$(a,b)$处处Schwarz可导, 则称$f$在$(a,b)$上Schwarz可导.
\end{definition}
\begin{note}
显然$f$在$x_0$可导则必然在$x_0$处Schwarz可导且$f'(x_0) = f^S(x_0)$, 但反之不一定成立.
\end{note}

\begin{theorem}[Schwarz导数的Lagrange中值定理]\label{theorem:Schwarz导数的Lagrange中值定理}
设$f$在$[a,b]$连续且在$(a,b)$上Schwarz可导, 证明存在$x_1,x_2 \in (a,b)$, 使得
\begin{align}
...
\end{theorem}
\begin{remark}
本定理是此类Schwarz导数问题的核心定理. 其余结果都是本定理的平凡推论.
\end{remark}
\begin{remark}
这题反证后,用有限覆盖定理极难处理同过拼接区间实现函数值的传递.

注意条件有连续性的性态分析问题,一般使用确界原理进行证明是简便直观的.然后只需要取出一段单调区间,再将单调区间中点取为$c$即可完成证明,或者取单调区间的某一点作为$c$,再取其邻域中点作为对称点也能完成证明(如下述证明).即要保证取的确界$c$满足函数在点$c$附近穿过$f(c)$且单调.例如,我们还可以在下述证明中取$U=\{x\in [a,b]:\forall t\in [a,x],f(t)\leqslant k\}$,取$c=\mathrm{sup}\,U$.

但是注意不能让$c$取成极值点,若极值点$c$发生左偏或右偏时,此时$c$就不满足结论了(无法实现关于$c$的对称点的函数值放缩).例如,若令下述证明中的$U=\{x\in [a,c]:f(x)\geqslant k\}$,此时$c=\mathrm{inf}\, U$就可能取到$f$的极值大点,比如当$c$这个极大值点右偏时就不满足结论.

类似习题和解析见裴礼文(第3版)P196页(3.2.35).
\end{remark}
\begin{note}
类似这种用确界原理的证明,可行的构造集合通常有多种,只需先构造一个类似的集合,然后画图结合端点值、极值点的性态调整一下即.然后取上确界或下确界就能完成证明.

设$f\in C[a,b],f(a)<\mu <f(b)$,则常用的可行集合有
\begin{gather*}
U=\left\{ x\in \left[ a,b \right] :f\left( t \right) >\mu \right\} ,\text{此时取}c=\mathrm{inf}\,U.
\\
V=\left\{ x\in \left[ a,b \right] :f\left( t \right) <\mu \right\} ,\text{此时取}c=\mathrm{sup}\,U.
\\
E=\left\{ x\in \left[ a,b \right] :\forall t\in \left[ a,x \right] ,f\left( t \right) \leqslant \mu \right\} ,\text{此时取}c=\mathrm{sup}\,E.
\\
F=\left\{ x\in \left[ a,b \right] :\forall t\in \left[ x,b \right] ,f\left( t \right) \geqslant \mu \right\} ,\text{此时取}c=\mathrm{inf}\,E.
\end{gather*}
上述构造的上确界、下确界$c$都满足:
...
从而当$n$充分大时,有
\begin{gather*}
t_n\in U\Longrightarrow f\left( t_n \right) >k,
\\
x_1-t_n+x_1\in \left[ a,c \right] \backslash U\Longrightarrow f\left( x_1-t_n+x_1 \right) \leqslant k.
\end{gather*}
于是
\begin{align*}
f^S(x_1)&=\lim_{h\rightarrow 0} \frac{f\left( x_1+h \right) -f\left( x_1-h \right)}{2h}=\lim_{n\rightarrow \infty} \frac{f\left( x_1+t_n-x_1 \right) -f\left( x_1-\left( t_n-x_1 \right) \right)}{2\left( t_n-x_1 \right)}
\\
&=\lim_{n\rightarrow \infty} \frac{f\left( t_n \right) -f\left( x_1-t_n+x_1 \right)}{2\left( t_n-x_1 \right)}>\lim_{n\rightarrow \infty} \frac{k-k}{2\left( t_n-x_1 \right)}=0.
\end{align*}
...
\begin{proposition}
设$f$在$[a,b]$连续且在$(a,b)$上Schwarz可导, 若$f^S(x) \geqslant  0, \forall x \in (a,b)$, 则$f$在$[a,b]$递增.
\end{proposition}
\begin{proof}
...
\begin{example}
设$f\in C\left( \mathbb{R} \right)$满足
\begin{align}
\lim_{h\rightarrow +\infty}\limits\left[ f\left( x+h \right) -2f\left( x \right) +f\left( x-h \right) \right] =0,\quad \forall x\in \mathbb{R}.\label{eq:::---289jrhejfwewfef.1}
\end{align}
证明$f$是线性函数.
\end{example}
\begin{proof}
显然存在奇函数$f_1\in C\left( \mathbb{R} \right)$和偶函数$f_2\in C\left( \mathbb{R} \right)$，使得
$$f\left( x \right) =f_1\left( x \right) +f_2\left( x \right).$$
由\eqref{eq:::---289jrhejfwewfef.1}式知
$$\lim_{h\rightarrow +\infty}\limits\left[ f_i\left( x+h \right) -2f_i\left( x \right) +f_i\left( x-h \right) \right] =0,\quad i=1,2,\quad \forall x\in \mathbb{R}.$$
因此只需分别证明$f$为偶函数和奇函数的情形即可.
\begin{enumerate}[(i)]
\item 当$f$为偶函数时，由\eqref{eq:::---289jrhejfwewfef.1}式可得
\begin{align*}
\lim_{h\rightarrow +\infty}\limits\left[ f\left( h \right) -2f\left( 0 \right) +f\left( -h \right) \right] =\lim_{h\rightarrow +\infty}\limits\left[ 2f\left( h \right) -2f\left( 0 \right) \right] =0\Longrightarrow \lim_{x\rightarrow +\infty}\limits f\left( x \right) =f\left( 0 \right).
\end{align*}
继续利用\eqref{eq:::---289jrhejfwewfef.1}式可得，对$\forall x\in \mathbb{R}$，有
\begin{align*}
\lim_{h\rightarrow +\infty}\limits\left[ f\left( x+h \right) -2f\left( x \right) +f\left( x-h \right) \right] =\lim_{h\rightarrow +\infty}\limits\left[ f\left( x+h \right) -2f\left( x \right) +f\left( h-x \right) \right] =2f\left( 0 \right) -2f\left( x \right).
\end{align*}
利用极限的唯一性和\eqref{eq:::---289jrhejfwewfef.1}式知$f\left( x \right) =f\left( 0 \right),\forall x\in \mathbb{R}.$

\item 当$f$为奇函数时，由\eqref{eq:::---289jrhejfwewfef.1}式知
\begin{align*}
\lim_{n\rightarrow +\infty}\limits\left[ f\left( x+nx \right) -2f\left( x \right) +f\left( x-nx \right) \right] =\lim_{n\rightarrow +\infty}\limits\left[ f\left( \left( n+1 \right) x \right) -f\left( \left( n-1 \right) x \right) -2f\left( x \right) \right] =0,\quad \forall x>0.
\end{align*}
即
\begin{align*}
\lim_{n\rightarrow +\infty}\limits\left[ f\left( \left( n+1 \right) x \right) -f\left( \left( n-1 \right) x \right) \right] =2f\left( x \right),\quad \forall x>0.
\end{align*}
由\refpro{proposition:Stolz公式的推论111}知
\begin{align*}
\lim_{n\rightarrow +\infty}\limits\frac{f\left( nx \right)}{n}=f\left( x \right),\quad \forall x>0.
\end{align*}
再由$f$是奇函数知
\begin{align*}
\lim_{n\rightarrow +\infty}\limits\frac{f\left( nx \right)}{n}=f\left( x \right),\quad \forall x\in \mathbb{R}.
\end{align*}
于是对$\forall m\in \mathbb{N}$，有
\begin{align*}
f\left( mx \right) =\lim_{n\rightarrow +\infty}\limits\frac{f\left( nmx \right)}{n}=m\lim_{nm\rightarrow +\infty}\limits\frac{f\left( nmx \right)}{nm}=mf\left( x \right),\quad \forall x\in \mathbb{R}.
\end{align*}
再由$f$是奇函数知
\begin{align*}
f\left( mx \right) =mf\left( x \right),\quad \forall m\in \mathbb{Z},\,x\in \mathbb{R}.
\end{align*}
对$\forall \frac{p}{q}\in \mathbb{Q}$，有$p\in \mathbb{Z},q\in \mathbb{N}.$从而
\begin{align*}
qf\left( \frac{p}{q}x \right) =f\left( px \right) =pf\left( x \right) \Longrightarrow f\left( \frac{p}{q}x \right) =\frac{p}{q}f\left( x \right),\quad \forall \,x\in \mathbb{R}.
\end{align*}
对$\forall r\in \mathbb{R} \setminus \mathbb{Q}$，必存在有理数列$\left\{ r_n \right\}$趋于$r.$再由$f\in C\left( \mathbb{R} \right)$可得
\begin{align*}
f\left( rx \right) =\lim_{n\rightarrow +\infty}\limits f\left( r_nx \right) =\lim_{n\rightarrow +\infty}\limits r_nf\left( x \right) =rf\left( x \right),\quad \forall \,x\in \mathbb{R}.
\end{align*}
综上可知
\begin{align*}
f\left( rx \right) =rf\left( x \right),\quad \forall r,x\in \mathbb{R}.
\end{align*}
故
\begin{align*}
f\left( x \right) =xf\left( 1 \right),\quad \forall x\in \mathbb{R}.
\end{align*}
...
\end{proposition}
\begin{proof}
事实上当$M$或者$K$为0时命题显然成立($K=0$意味着$x_n$是有界的等差数列,必然常数列),因此不妨假设$M,K>0$.

$\mathbf{Step}\,\,\mathbf{1}$注意到对$\forall n \geqslant 2$,我们有
\begin{align*}
...
(i)若$x_0>1$,则
\begin{align*}
x_0=2\sqrt{\frac{M}{K}}+\frac{1}{2}>1\Longrightarrow \sqrt{\frac{M}{K}}>1\Longrightarrow \left| x_1-x_2 \right|=\sqrt{1+8\sqrt{\frac{M}{K}}}>3.
\end{align*}
从而$x_1=x_0-\frac{1}{2}\left| x_1-x_2 \right|\leqslant x_0-3$.于是$\lfloor x_0 \rfloor \in \left[ x_1,x_0 \right]$,进而此时$f\left( \lfloor x_0 \rfloor \right) \leqslant 0$.

(ii)若$x_0\leqslant 1$,则
\begin{align*}
x_2=x_0+\frac{1}{2}\left| x_1-x_2 \right|>\frac{1}{2}+\frac{1}{2}=1\geqslant x_0\Longrightarrow 1\in \left[ x_0,x_2 \right].
\end{align*}
故此时$f\left( 1 \right) \leqslant 0$.

综上可知,必存在$n \in \mathbb{N}$,使得$f(n) \leqslant 0$.由\eqref{eq::928453j3f34tg453234}式知
\begin{align*}
...
$\mathbf{Step}\,\,\mathbf{2}$假设对$m\in\mathbb{N}$,有
$$|x_{m+1}-x_m|\leqslant2\sqrt{MK}.$$
令$y_n = x_{n+m}, n\in \mathbb{N},$则
\begin{gather*}
\mathop {\mathrm{sup}} \limits_{n\geqslant 0}\left| y_n \right|=\mathop {\mathrm{sup}} \limits_{n\geqslant 0}\left| x_{n+m} \right|\leqslant M,
\\
\mathop {\mathrm{sup}} \limits_{n\geqslant 0}\left| y_{n+2}-2y_{n+1}+y_n \right|=\mathop {\mathrm{sup}} \limits_{n\geqslant 0}\left| x_{n+m+2}-2x_{n+m+1}+x_{n+m} \right|\leqslant K.
\end{gather*}
根据$\mathbf{Step}\,\,\mathbf{1}$的结论，对数列$\{y_n\}$有
$$
|y_1-y_0|\leqslant2\sqrt{MK}\iff |x_{m+2}-x_{m+1}|\leqslant2\sqrt{MK}.
$$
由数学归纳法，对所有$n\in\mathbb{N}$，都有
$$|x_{n+1}-x_n|\leqslant2\sqrt{MK}.$$
...
{\color{blue}证法一:}由条件知,对任何$\varepsilon > 0$,存在$\delta > 0$,使得
\begin{align*}
...
{\color{blue}证法二:}首先注意到如下差分恒等式:
\begin{align}
2\left(f(x+h)-f(x)\right) = f(x+2h) - f(x) - \left(f(x+2h) - 2f(x+h) + f(x)\right).
\label{eq:diff_identity_2h}
\end{align}
记
\begin{align*}
A(h) \triangleq \sup_{x \in \mathbb{R}} \left| f(x+h) - f(x) \right|, \quad
B(h) \triangleq \sup_{x \in \mathbb{R}} \left| f(x+h) - 2f(x) + f(x-h) \right|, \quad
M \triangleq \sup_{x \in \mathbb{R}} \left| f(x) \right|.
\end{align*}
由$f$在$\mathbb{R}$上有界可知$M < +\infty$, 且显然有$A(h) \leqslant 2M$.
对\eqref{eq:diff_identity_2h}两边同时在$\mathbb{R}$上取上确界, 由三角不等式可得
\begin{align}
A(h) \leqslant \frac{1}{2}A(2h) + \frac{1}{2}B(h).
\label{eq:iter_ineq_base}
\end{align}
将\eqref{eq:iter_ineq_base}迭代$n$次, 依次有
\begin{align*}
&A(2h)\leqslant \frac{1}{2}A(4h)+\frac{1}{2}B(2h),
\\
&A(4h)\leqslant \frac{1}{2}A(8h)+\frac{1}{2}B(4h),
\\
&\cdots \cdots \cdots \cdots 
\\
&A(2^{n-1}h)\leqslant \frac{1}{2}A(2^nh)+\frac{1}{2}B(2^{n-1}h).
\end{align*}
将上述式子从前往后逐一代入\eqref{eq:iter_ineq_base}式可得
\begin{align}
A(h) \leqslant \frac{1}{2^n} A(2^n h) + \sum\limits_{k=0}^{n-1} \frac{1}{2^{k+1}} B(2^k h).
\label{eq:iter_ineq_n}
\end{align}

下面证明$\lim\limits_{h \to 0} A(h) = 0$.
由题设条件$\lim\limits_{h \to 0} B(h) = 0$, 故对任意$\varepsilon > 0$, 存在$\delta > 0$, 使得当$0 < |h| < \delta$时, 成立
\begin{align*}
B(h) < \frac{\varepsilon}{2}.
\end{align*}
任取满足$0 < |h| < \frac{\delta}{2}$的$h$, 取正整数$n$使得
\begin{align*}
\frac{\delta}{2} \leqslant 2^n |h| < \delta.
\end{align*}
此时对任意$0 \leqslant k \leqslant n-1$, 有$2^k |h| \leqslant 2^{n-1} |h| < \frac{\delta}{2} < \delta$, 因此
\begin{align*}
B(2^k h) < \frac{\varepsilon}{2}, \quad \forall 0 \leqslant k \leqslant n-1.
\end{align*}
一方面, 由$A(2^n h) \leqslant 2M$以及$2^n |h| \geqslant \frac{\delta}{2}$, 可得
\begin{align}
\frac{1}{2^n} A(2^n h) \leqslant \frac{2M}{2^n} = \frac{2M \cdot |h|}{2^n |h|} \leqslant \frac{2M \cdot |h|}{\delta / 2} = \frac{4M}{\delta} |h|.
\label{eq:first_term_est}
\end{align}
另一方面, 结合$B(2^k h) < \frac{\varepsilon}{2}$以及等比级数求和公式, 有
\begin{align}
\sum\limits_{k=0}^{n-1} \frac{1}{2^{k+1}} B(2^k h)
< \sum\limits_{k=0}^{n-1} \frac{1}{2^{k+1}} \cdot \frac{\varepsilon}{2}
< \sum\limits_{k=0}^{\infty} \frac{1}{2^{k+1}} \cdot \frac{\varepsilon}{2}
= \frac{\varepsilon}{2}.
\label{eq:second_term_est}
\end{align}
将\eqref{eq:first_term_est}与\eqref{eq:second_term_est}代入\eqref{eq:iter_ineq_n}, 得到
\begin{align}
A(h) \leqslant \frac{4M}{\delta} |h| + \frac{\varepsilon}{2}.
\label{eq:A_h_final_est}
\end{align}
取$\delta' = \min\left\{ \frac{\delta \varepsilon}{8M}, \frac{\delta}{2} \right\}$, 则当$0 < |h| < \delta'$时, 有$\frac{4M}{\delta} |h| < \frac{\varepsilon}{2}$, 代入\eqref{eq:A_h_final_est}得
\begin{align*}
A(h) < \frac{\varepsilon}{2} + \frac{\varepsilon}{2} = \varepsilon.
\end{align*}
即
\begin{align*}
\lim\limits_{h \to 0} \sup_{x \in \mathbb{R}} \left| f(x+h) - f(x) \right| = 0,
\end{align*}
...
现在
\begin{align*}
0&=\lim_{h\rightarrow 0} \frac{1}{h^3}\left\{ \int_0^h{[f_{\varepsilon}(x_1)+f_{\varepsilon}(x_1)-2f_{\varepsilon}(x_1)]\,\mathrm{d}u} \right\} \geqslant \lim_{h\rightarrow 0} \frac{1}{h^3}\left\{ \int_0^h{[f_{\varepsilon}(x_1+u)+f_{\varepsilon}(x_1-u)-2f_{\varepsilon}(x_1)]\,\mathrm{d}u} \right\} 
\\
&=\lim_{h\rightarrow 0} \frac{\int_0^h{\varepsilon \left[ \left( x_1-a+u \right) \left( x_1-b+u \right) +\left( x_1-a-u \right) \left( x_1-b-u \right) -2\left( x_1-a \right) \left( x_1-b \right) \right] \mathrm{d}u}}{h^3}
\\
