...
\begin{proposition}\label{proposition:第一型曲线积分在等距变换下的结论}
设可求长的曲线$L\subseteq \mathbb{R} ^n$,函数$f:\mathbb{R} ^n\rightarrow \mathbb{R}$,$T$是定义在$\mathbb{R} ^n$上的等距变换,并且存在$\lambda \in \mathbb{R}$,使得
\begin{align*}
f( \mathbf{x} ) =\lambda f( T( \mathbf{x} ) ),\quad \forall \mathbf{x}\in L.
\end{align*}
则
\begin{align*}
\int_L{f( \mathbf{x} ) \mathrm{d}s}=\lambda \int_{T( L )}{f( T( \mathbf{x} ) ) \mathrm{d}s}.
\end{align*}
\end{proposition}
\begin{remark}
轮换变换、反射(对称)变换、平移变换都是等距变换,都满足上述结论.平时计算第一型曲面积分最常用的也是这三种等距变换.
\end{remark}
\begin{proof}
设曲线$L=\gamma ( t )$, $t\in [ a,b ]$,则
\begin{align*}
\int_L{f( \mathbf{x} ) \mathrm{d}s_{\mathbf{x}}}=\int_a^b{f( \gamma ( t ) ) \left\| \gamma '( t ) \right\|}\mathrm{d}t.
\end{align*}
由$T$是等距变换知,可设$T( \mathbf{x} ) =Q\mathbf{x}+\mathbf{b}$,其中$Q$是正交矩阵,$\mathbf{b}$是常向量.从而
\begin{align*}
\left\| ( T( \gamma ( t ) ) )' \right\| = \left\| Q\gamma '( t ) \right\| = \left\| \gamma '( t ) \right\|,\quad \forall t\in [ a,b ].
\end{align*}
又因为等距变换$T$保持向量长度不变,所以曲线$L$和$T( L )$的弧长微元相同,即
\begin{align}
\mathrm{d}s_{\mathbf{x}}=\Delta s_{\mathbf{z}}=\Delta s_{T( \mathbf{x} )}=\mathrm{d}s_{T( \mathbf{x} )},\quad \forall \mathbf{x}\in L.\label{eq::::---2r89fhew9fnw23rfdszvfcbn456m:21.0}
\end{align}
因此
\begin{align}
\int_{T( L )}{f( T( \mathbf{x} ) ) \mathrm{d}s_{T( \mathbf{x} )}}=\int_a^b{f( T( \gamma ( t ) ) ) \left\| ( T( \gamma ( t ) ) )' \right\| \mathrm{d}t}=\int_a^b{f( T( \gamma ( t ) ) ) \left\| \gamma '( t ) \right\| \mathrm{d}t}.\label{eq::::---2r89fhew9fnw23rfdszvfcbn456m:21.1}
\end{align}
由条件知$f( \mathbf{x} ) =\lambda f( T( \mathbf{x} ) )(\forall \mathbf{x}\in L)$,从而
\begin{align}
\int_L{f( \mathbf{x} ) \mathrm{d}s_{\mathbf{x}}}=\int_a^b{f( \gamma ( t ) ) \left\| ( \gamma ( t ) )' \right\| \mathrm{d}t}=\lambda \int_a^b{f( T( \gamma ( t ) ) ) \left\| \gamma '( t ) \right\| \mathrm{d}t}.\label{eq::::---2r89fhew9fnw23rfdszvfcbn456m:21.2}
\end{align}
记$\mathrm{d}s=\mathrm{d}s_{\mathbf{x}}$,结合\eqref{eq::::---2r89fhew9fnw23rfdszvfcbn456m:21.0}\eqref{eq::::---2r89fhew9fnw23rfdszvfcbn456m:21.1}\eqref{eq::::---2r89fhew9fnw23rfdszvfcbn456m:21.2}式可知
\begin{align*}
\int_L{f( \mathbf{x} ) \mathrm{d}s}=\lambda \int_{T( L )}{f( T( \mathbf{x} ) ) \mathrm{d}s_{T( \mathbf{x} )}}=\lambda \int_{T( L )}{f( T( \mathbf{x} ) ) \mathrm{d}s}.
\end{align*}
