...
\begin{theorem}[第一类曲线积分计算公式]\label{theorem:第一类曲线积分计算公式}
设有光滑曲线
\begin{align*}
...
\begin{example}
计算第一型曲线积分 \(\int_L (x+y)\mathrm{d}s\)，其中 \(L\) 为曲线
\begin{align*}
...
因此，曲线积分的弧长微元为
\begin{align*}
\mathrm{d}s = \sqrt{[x'(t)]^2 + [y'(t)]^2} \mathrm{d}t = \sqrt{(at\cos t)^2 + (at\sin t)^2} \mathrm{d}t = \sqrt{a^2 t^2 (\cos^2 t + \sin^2 t)} \mathrm{d}t = at \mathrm{d}t \quad (a>0, t \geqslant 0).
\end{align*}
由\hyperref[theorem:第一类曲线积分计算公式]{第一类曲线积分计算公式}可得
\begin{align*}
\int_L (x+y)\mathrm{d}s &= \int_0^{2\pi} [x(t) + y(t)] \cdot at \mathrm{d}t 
= a^2 \int_0^{2\pi} \left[ (\cos t + t\sin t) + (\sin t - t\cos t) \right] \cdot t \mathrm{d}t \\
&= a^2 \int_0^{2\pi} \left( t\cos t + t^2\sin t + t\sin t - t^2\cos t \right) \mathrm{d}t.
\end{align*}
我们分别计算上述四项积分。
\begin{enumerate}[(1)]
\item 对于 \(\int_0^{2\pi} t\cos t \mathrm{d}t\)，使用分部积分法，令 \(u=t\)，\( \mathrm{d}v=\cos t\mathrm{d}t \)：
\begin{align*}
\int_0^{2\pi} t\cos t \mathrm{d}t = [t\sin t]_0^{2\pi} - \int_0^{2\pi} \sin t \mathrm{d}t = 0 - 0 = 0.
\end{align*}
\item 对于 \(\int_0^{2\pi} t\sin t \mathrm{d}t\)，使用分部积分法，令 \(u=t\)，\( \mathrm{d}v=\sin t\mathrm{d}t \)：
\begin{align*}
\int_0^{2\pi} t\sin t \mathrm{d}t = [-t\cos t]_0^{2\pi} + \int_0^{2\pi} \cos t \mathrm{d}t = -2\pi.
\end{align*}
\item 对于 \(\int_0^{2\pi} t^2\sin t \mathrm{d}t\)，使用分部积分法，令 \(u=t^2\)，\( \mathrm{d}v=\sin t\mathrm{d}t \)：
\begin{align*}
\int_0^{2\pi} t^2\sin t \mathrm{d}t = [-t^2\cos t]_0^{2\pi} + 2\int_0^{2\pi} t\cos t \mathrm{d}t = -4\pi^2.
\end{align*}
\item 对于 \(\int_0^{2\pi} -t^2\cos t \mathrm{d}t\)，使用分部积分法，令 \(u=t^2\)，\( \mathrm{d}v=\cos t\mathrm{d}t \)：
\begin{align*}
\int_0^{2\pi} t^2\cos t \mathrm{d}t = [t^2\sin t]_0^{2\pi} - \int_0^{2\pi} 2t\sin t \mathrm{d}t = 0-2(-2\pi) = 4\pi.
\end{align*}
\end{enumerate}
将上述四个积分的结果相加，并乘以 \(a^2\)，可得原积分为
\begin{align*}
\int_L (x+y)\mathrm{d}s = a^2 \left( 0 - 2\pi - 4\pi^2 - 4\pi \right) = -2a^2\pi(3+2\pi).
\end{align*}
\end{solution}
