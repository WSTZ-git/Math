...
由条件可得
\[
u_{n + 1}=u_{n}^{2}+(1 - 2a)u_n + a^2=(u_n - a)^2 + u_n\geqslant slant u_n.
\]
故 \(u_n\) 单调递增。 
...
\[
a - 1 < a - \frac{1}{4}=f\left(\frac{2a - 1}{2}\right)\leqslant slant f(x)\leqslant slant \max\{f(a - 1),f(a)\}=a,\forall x\in [a - 1,a].
\]
由于 \(u_1 = b\in [a - 1,a]\)，假设 \(u_n\in [a - 1,a]\)，则
\[
a - 1\leqslant slant u_{n + 1}=f(u_n)\leqslant slant a.
\]
...
\begin{note}
\begin{enumerate}
\item 不妨设$x_1\geqslant  0$的原因:我们只去掉原数列$\{x_n\}$的第一项,得到一个新数列,并且此时新数列是从原数列$\{x_n\}$的第二项$x_2$开始的.对于原数列$\{x_n\}$而言,有$x_{n+1}=\sqrt{6+x_n}\geqslant  0,\forall n \in \mathbb{N}_+$.故新数列的每一项都大于等于0.将新数列重新记为$\{x_n\}$,则$x_1\geqslant  0$.若此时能够证得新数列收敛到$x_0$,则由于数列去掉有限项不会影响数列的敛散性以及极限值,可知原数列也收敛到$x_0$.故不妨设$x_1\geqslant  0$是合理地.

(简单地说,就是原数列用$x_2$代替$x_1$,用$x_{n+1}$代替$x_n$,$\forall n \in \mathbb{N}_+$,而由$x_1>-6$,可知$x_2=\sqrt{6+x_1}\geqslant  0$.)
...
假设当\(n = k\)时,有\(x_{k + 1}\geqslant slant x_k\).则当\(n = k + 1\)时,就有
\[
x_{k + 2}=f(x_{k + 1})\geqslant slant f(x_k)=x_{k + 1}.
\]
故由数学归纳法,可知\(\{x_n\}\)单调递增.
于是由单调有界定理,可得数列\(\{x_n\}\)收敛.

当\(x_1\geqslant slant 3\)时,由\eqref{eqq123}式可知,此时\(x_2\leqslant slant x_1\).假设当\(n = k\)时,有\(x_k\geqslant slant 3\).则当\(n = k + 1\)时,就有
\[
x_{k + 1}=f(x_k)=\sqrt{6 + x_k}\geqslant slant\sqrt{6 + 3}=3.
\]
故由数学归纳法,可知\(x_n\geqslant slant 3\),\(\forall n\in\mathbb{N}_+\).

假设当\(n = k\)时,有\(x_{k + 1}\leqslant slant x_k\).则当\(n = k + 1\)时,就有
\[
x_{k + 2}=f(x_{k + 1})\leqslant slant f(x_k)=x_{k + 1}.
\]
故由数学归纳法,可知\(\{x_n\}\)单调递减.
于是由单调有界定理,可得数列\(\{x_n\}\)收敛.

综上,无论\(x_1 > 3\)还是\(x_1\leqslant slant 3\),都有数列\(\{x_n\}\)收敛.设\(\lim_{n\rightarrow\infty}x_n = a\).则对\(x_{n + 1}=\sqrt{6 + x_n}\)两边同时令\(n\rightarrow\infty\)可得\(a=\sqrt{6 + a}\),解得\(\lim_{n\rightarrow\infty}x_n = a = 3\).
...
\geqslant slant \left((m + 1)\sqrt[m + 1]{\frac{3}{4}x^2\cdot\frac{1}{m^m}}\right)^2
=\left(\frac{3}{4}\right)^{\frac{2}{m + 1}}\cdot\frac{m + 1}{m^{\frac{2m}{m + 1}}}x^{\frac{4}{m + 1}}.
\end{align*}
从而我们希望 \(x^{\frac{4}{m + 1}} = x\)，即 \(m = 3\)。这样就能使得
\[
\frac{x}{(1+\frac{3}{4}x^2)^2}\leqslant slant \left(\frac{3}{4}\right)^{\frac{2}{m + 1}}\cdot\frac{m + 1}{m^{\frac{2m}{m + 1}}}x^{\frac{4}{m + 1}}=\left(\frac{3}{4}\right)^{\frac{2}{3 + 1}}\cdot\frac{3 + 1}{3^{\frac{2\cdot 3}{3 + 1}}}<1.
\]
故取 \(m = 3\)。
\end{remark}
\begin{proof}
...
由均值不等式可得
\[
x^2 + xy + y^2=(x + y)^2 - xy\geqslant slant (x + y)^2 - \left(\frac{x + y}{2}\right)^2=\frac{3}{4}(x + y)^2.
\]
...
\begin{remark}
\begin{enumerate}
\item \label{example3.23-1}记\(b\triangleq\max\{c_1,c_2,4\}\)的原因:为了证明数列${c_n}$有界,我们需要先定界(即确定$c_n$的上下界的具体数值),然后再利用数学归纳法证得数列${c_n}$有界.显然${c_n}$有一个下界0,但上界无法直接观察出来.为了确定出数列${c_n}$的一个上界,我们可以先假设${c_n}$有一个上界$b$(此时$b$是待定常数).则$c_{n+1}=\sqrt{c_n}+\sqrt{c_{n - 1}}\leqslant slant\sqrt{b}+\sqrt{b}=2\sqrt{b}\leqslant slant b$,由此解得$b\geqslant  4$.又由数学归纳法的原理,可知需要保证$b$同时也是$c_1,c_2$的上界.故只要取$b\geqslant  4,c_1,c_2$就一定能归纳出$b$是${c_n}$的一个上界.而我们取$b\triangleq\max\{c_1,c_2,4\}$满足这个条件.
...
\end{align*}
由此解得$M\geqslant  (2021)^{\frac{1}{2021}}$.又由数学归纳法的原理,可知需要保证$M$同时也是$a_1,a_2,\cdots,a_{2020}$的上界.故只要取$M\geqslant  \left(2021\right)^{\frac{1}{2021}},a_1,a_2,\cdots,a_{2020}$就一定能归纳出$M$是${a_n}$的一个上界.而我们取$M=\max \left\{ \left( 2021 \right) ^{\frac{1}{2021}},a_1,a_2,\cdots ,a_{2020} \right\}$满足这个条件.
\end{enumerate}
\end{remark}
\begin{solution}
\begin{enumerate}
...
\[
0 < c_{n + 1}=\sqrt{c_n}+\sqrt{c_{n - 1}}\leqslant slant\sqrt{b}+\sqrt{b}=2\sqrt{b}\leqslant slant b.
\]
...
\[
L=\varlimsup_{n\rightarrow\infty}c_{n + 1}=\varlimsup_{n\rightarrow\infty}(\sqrt{c_n}+\sqrt{c_{n - 1}})\leqslant slant\varlimsup_{n\rightarrow\infty}\sqrt{c_n}+\varlimsup_{n\rightarrow\infty}\sqrt{c_{n - 1}}=2\sqrt{L}\Rightarrow L\leqslant slant 4,
\]
\[
l=\varliminf_{n\rightarrow\infty}c_{n + 1}=\varliminf_{n\rightarrow\infty}(\sqrt{c_n}+\sqrt{c_{n - 1}})\geqslant slant\varliminf_{n\rightarrow\infty}\sqrt{c_n}+\varliminf_{n\rightarrow\infty}\sqrt{c_{n - 1}}=2\sqrt{l}\Rightarrow l\geqslant slant 4.
\]
又\(l=\varliminf_{n\rightarrow\infty}c_n\leqslant slant\varlimsup_{n\rightarrow\infty}c_n = L\),故\(L = l = 4\).即\(\lim_{n\rightarrow\infty}c_n = 4\).
...
\end{align*}
由数学归纳法,可知$0<a_n\leqslant  M,\forall n\in\mathbb{N}_+$.即数列${a_n}$有界.因此可设$A=\underset{n\rightarrow \infty}{{\varlimsup }}a_n<\infty ,a=\underset{n\rightarrow \infty}{{\varliminf }}a_n<\infty$.由条件可得
\begin{align*}
...
\\
&\leqslant slant \sqrt[2022]{\underset{n\rightarrow \infty}{{\varlimsup }}a_n+\underset{n\rightarrow \infty}{{\varlimsup }}a_{n+1}+\cdots +\underset{n\rightarrow \infty}{{\varlimsup }}a_{n+2020}}=\sqrt[2022]{A+A+\cdots +A}\Rightarrow A\leqslant slant \left( 2021 \right) ^{\frac{1}{2021}},   
\end{align*}
\begin{align*}
...
\\
&\geqslant slant \sqrt[2022]{\underset{n\rightarrow \infty}{{\varliminf }}a_n+\underset{n\rightarrow \infty}{{\varliminf }}a_{n+1}+\cdots +\underset{n\rightarrow \infty}{{\varliminf }}a_{n+2020}}=\sqrt[2022]{a+a+\cdots +a}\Rightarrow a\geqslant slant \left( 2021 \right) ^{\frac{1}{2021}}.
\end{align*}
又$a=\underset{n\rightarrow \infty}{{\varliminf }}a_n\leqslant slant \underset{n\rightarrow \infty}{{\varlimsup }}a_n=A$,故$A=a=\left( 2021 \right) ^{\frac{1}{2021}}$.即$\underset{n\rightarrow \infty}{\lim}a_n=\left( 2021 \right) ^{\frac{1}{2021}}$.
\end{enumerate}
\end{solution}
...
\begin{proof}
\begin{enumerate}
\item \hyperref[example3.24(1)]{取\(a=\min \left\{ a_1,a_2,\frac{2}{a_1},\frac{2}{a_2} \right\}>0\)},则有$0<a\leqslant  a_1,a_2\leqslant  \frac{2}{a}$成立.假设$0<a\leqslant  a_n\leqslant  \frac{2}{a}$,则由条件可得
\begin{align*}
a_{n+2}=\frac{1}{a_{n+1}}+\frac{1}{a_n}\leqslant slant \frac{1}{a}+\frac{1}{a}=\frac{2}{a}.
\end{align*}
...
\begin{align*}
&A=\underset{n\rightarrow \infty}{{\varlimsup }}a_{n+2}=\underset{n\rightarrow \infty}{{\varlimsup }}\left( \frac{1}{a_{n+1}}+\frac{1}{a_n} \right) \leqslant slant \underset{n\rightarrow \infty}{{\varlimsup }}\frac{1}{a_{n+1}}+\underset{n\rightarrow \infty}{{\varlimsup }}\frac{1}{a_n}
\\
&=\frac{1}{\underset{n\rightarrow \infty}{{\varliminf }}a_{n+1}}+\frac{1}{\underset{n\rightarrow \infty}{{\varliminf }}a_n}=\frac{1}{B}+\frac{1}{B}=\frac{2}{B}\Rightarrow AB\leqslant slant 2.
\end{align*}
\begin{align*}
&B=\underset{n\rightarrow \infty}{{\varliminf }}a_{n+2}=\underset{n\rightarrow \infty}{{\varliminf }}\left( \frac{1}{a_{n+1}}+\frac{1}{a_n} \right) \geqslant slant \underset{n\rightarrow \infty}{{\varliminf }}\frac{1}{a_{n+1}}+\underset{n\rightarrow \infty}{{\varliminf }}\frac{1}{a_n}
\\
&=\frac{1}{\underset{n\rightarrow \infty}{{\varlimsup }}a_{n+1}}+\frac{1}{\underset{n\rightarrow \infty}{{\varlimsup }}a_n}=\frac{1}{A}+\frac{1}{A}=\frac{2}{A}\Rightarrow AB\geqslant slant 2.
\end{align*}
...
\\
&=\frac{1}{l_1}+\frac{1}{l_2}\leqslant slant \frac{1}{B}+\frac{1}{B}=\frac{2}{B}=A\Rightarrow l_1=l_2=B.
\end{align*}
...
\\
&=\frac{1}{l_2}+\frac{1}{l_3}\geqslant slant \frac{1}{A}+\frac{1}{A}=\frac{2}{A}=B\Rightarrow l_2=l_3=A.
\end{align*}
...
\item 根据递推条件显然，\(x_n\geqslant slant 3,\forall n\geqslant slant 3\)。从而 \(x_5 = 3+\frac{1}{x_{4}^{2}}+\frac{1}{x_{3}^{2}}\leqslant slant 3+\frac{1}{9}+\frac{1}{9}<4\)。假设 \(x_n\leqslant slant 4,\forall n\geqslant slant 5\)，则
\[
x_{n + 1}=3+\frac{1}{x_{n}^{2}}+\frac{1}{x_{n - 1}^{2}}\leqslant slant 3+\frac{1}{9}+\frac{1}{9}<4.
\]
由数学归纳法可知 \(x_n\in [3,4],\forall n\geqslant slant 5\)。于是
\begin{align*}
\left| x_{n+2}-x_{n+1} \right|&=\left| \frac{1}{x_{n+1}^{2}}-\frac{1}{x_{n-1}^{2}} \right|\leqslant slant \left| \frac{1}{x_{n+1}^{2}}-\frac{1}{x_{n}^{2}} \right|+\left| \frac{1}{x_{n}^{2}}-\frac{1}{x_{n-1}^{2}} \right|=\frac{\left| x_{n}^{2}-x_{n+1}^{2} \right|}{x_{n+1}^{2}x_{n}^{2}}+\frac{\left| x_{n-1}^{2}-x_{n}^{2} \right|}{x_{n}^{2}x_{n-1}^{2}}
\\
...
\\
&\leqslant slant \frac{2}{27}\left| x_{n+1}-x_n \right|+\frac{2}{27}\left| x_n-x_{n-1} \right|,\forall n\geqslant slant 6.      
\end{align*}
\hyperlink{取q,lambda的原因}{记 \(q = \frac{1}{2}\in(0,1)\)，\(\lambda=\frac{1}{3}\)},\(u_n = |x_n - x_{n - 1}|\)，则由上式可得
\begin{align*}
&u_{n + 2}\leqslant slant \frac{2}{27}u_{n + 1}+\frac{2}{27}u_n
\leqslant slant (q - \lambda)u_{n + 1}+q\lambda u_n,\forall n\geqslant slant 6.\\
&\Leftrightarrow u_{n + 2}+\lambda u_{n + 1}\leqslant slant q(u_{n + 1}+\lambda u_n),\forall n\geqslant slant 6.
\end{align*}
从而对 \(\forall n\geqslant slant 10\)（\(n\) 大于 \(7\) 就行），我们有
\[
u_n\leqslant slant u_n+\lambda u_{n - 1}\leqslant slant q(u_{n - 1}+\lambda u_{n - 2})\leqslant slant\cdots\leqslant slant q^{n - 7}(u_7+\lambda u_6).
\]
于是对 \(\forall n\geqslant slant 10\)，我们有
\[
x_n\leqslant slant \sum_{k = 10}^n|x_{k + 1}-x_k|+x_6=\sum_{k = 10}^n u_k+x_6\leqslant slant (u_7+\lambda u_6)\sum_{k = 10}^n q^{k - 7}+x_6.
\]
...
取$a=\min \left\{ a_1,a_2,\frac{2}{a_1},\frac{2}{a_2} \right\}$的原因:为了证明数列${a_n}$有界,我们需要先定界,然后再利用数学归纳法证得数列${a_n}$有界.显然${a_n}$有一个下界0,但上界无法直接观察出来.为了确定出数列${a_n}$的上下界,我们可以先假设$b$为数列${a_n}$的一个上界(此时$b$是待定常数),但是我们根据$a_n>0$和$a_{n+2}=\frac{1}{a_{n+1}}+\frac{1}{a_n}$只能得到$a_{n+2}=\frac{1}{a_{n+1}}+\frac{1}{a_n}<+\infty$,无法归纳法出$a_n\leqslant  b$,故我们无法归纳出$0<a_n<b,\forall n\in\mathbb{N}_+$.因此仅待定一个上界并不够,下界并不能简单的取为0,我们还需要找到一个更接近下确界的大于零的下界,不妨先假设这个下界为$a>0$(此时$a$也是待定常数).利用这个下界和递推式$a_{n+2}=\frac{1}{a_{n+1}}+\frac{1}{a_n}$归纳出$0<a\leqslant  a_n\leqslant  b,\forall n\in\mathbb{N}_+$(此时$a,b$都是待定常数).于是由已知条件可得
\begin{gather*}
a_{n+2}=\frac{1}{a_{n+1}}+\frac{1}{a_n}\leqslant slant \frac{1}{a}+\frac{1}{a}=\frac{2}{a}\leqslant slant b\Rightarrow ab\geqslant slant 2,
\\
a_{n+2}=\frac{1}{a_{n+1}}+\frac{1}{a_n}\geqslant slant \frac{1}{b}+\frac{1}{b}=\frac{2}{b}\geqslant slant a\Rightarrow ab\leqslant slant 2.
\end{gather*}
从而$ab=2$,即$b=\frac{2}{a}$.进而$0<a\leqslant  a_n\leqslant \frac{2}{a}$.又由数学归纳法的原理,可知我们需要同时保证$0<a\leqslant  a_1,a_2\leqslant  \frac{2}{a}$.因此找到一个合适的$a$,使得$0<a\leqslant  a_1,a_2\leqslant  \frac{2}{a}$成立就一定能归纳出$0<a\leqslant  a_n\leqslant  \frac{2}{a},\forall n\in\mathbb{N}_+$,即数列$\{a_n\}$有界.而当我们取$a=\min \left\{ a_1,a_2,\frac{2}{a_1},\frac{2}{a_2} \right\}$时,有$a_1,a_2\leqslant slant a,\,\,\frac{2}{a}\ge \frac{2}{\frac{2}{a_1}}=a_1,\,\,\frac{2}{a}\ge \frac{2}{\frac{2}{a_2}}=a_2.$恰好满足这个条件.
...
\end{align*}
于是当$x_1\geqslant  \frac{\sqrt{5}-1}{2}$时,有$x_3-x_1=\frac{1+x_1}{2+x_1}-x_1\leqslant slant 0$,即$x_3\leqslant slant x_1$.从而由\hyperref[proposition:递增函数递推数列]{递增递推结论}可知,$\{x_{2n-1}\}$单调递减且$x_{2n-1}>\frac{\sqrt{5}-1}{2},\forall n \in \mathbb{N}_+$.此时\(x_2<\frac{\sqrt{5}-1}{2}\)(由\(x = \frac{1}{1 + x}\)以及$x_n>0$可以解得不动点\(x_0=\frac{\sqrt{5}-1}{2}\),又因为原数列是递减递推,所以\(x_n\)与\(x_0\)大小关系交错.而\(x_1\geqslant slant\frac{\sqrt{5}-1}{2}\),故\(x_2<\frac{\sqrt{5}-1}{2}\)).
...
\begin{align*}
\left| x_{n+1}-x \right|=\left| \frac{1}{1+x_n}-x \right|=\left| \frac{1}{1+x_n}-\frac{1}{1+x} \right|=\frac{\left| x_n-x \right|}{\left( 1+x_n \right) \left( 1+x \right)}\leqslant slant \frac{1}{1+x}\left| x_n-x \right|.
\end{align*}
从而
\begin{align*}
\left| x_{n+1}-x \right|\leqslant slant \frac{1}{1+x}\left| x_n-x \right|\leqslant slant \frac{1}{\left( 1+x \right) ^2}\left| x_{n-1}-x \right|\leqslant slant \cdots \leqslant slant \frac{1}{\left( 1+x \right) ^n}\left| x_1-x \right|.
\end{align*}
...
\leqslant slant\frac{1}{6}\vert x_n - 2\vert.
\end{align*}
从而对$\forall n \in \mathbb{N}_+$,都有
\begin{align*}
\vert x_{2n}-2\vert&\leqslant slant\frac{1}{6}\vert x_{2n - 2}-2\vert\leqslant slant\frac{1}{6^2}\vert x_{2n - 4}-2\vert\leqslant slant\cdots\leqslant slant\frac{1}{6^{n - 1}}\vert x_2 - 2\vert;\\
\vert x_{2n + 1}-2\vert&\leqslant slant\frac{1}{6}\vert x_{2n - 1}-2\vert\leqslant slant\frac{1}{6^2}\vert x_{2n - 3}-2\vert\leqslant slant\cdots\leqslant slant\frac{1}{6^n}\vert x_1 - 2\vert.
\end{align*}
...
\end{example}
\begin{solution}
令\(g(x)=x - \cos x\),则\(g^\prime(x)=1 + \sin x\geqslant slant0\),且\(g^\prime(x)\)不恒等于\(0\).
...
\[
\vert x_{n + 1}-x_0\vert=\vert f(x_n)-f(x_0)\vert=\vert f^\prime(\theta_n)\vert\vert x_n - x_0\vert\leqslant slant C\vert x_n - x_0\vert.
\]
进而对\(\forall n\in\mathbb{N}_+\),都有
\[
\vert x_{n + 1}-x_0\vert\leqslant slant C\vert x_n - x_0\vert\leqslant slant C^2\vert x_{n - 1}-x_0\vert\leqslant slant\cdots\leqslant slant C^n\vert x_1 - x_0\vert.
\]
...
\end{note}
\begin{proof}
令\(g(x)=x - f(x)\),则\(g(a)=a - f(a)\leqslant slant0\),\(g(b)=b - f(b)\geqslant slant0\).由零点存在定理可知,存在\(x_0\in[a,b]\),使得\(x_0 = f(x_0)\).
令\(h(x)=\begin{cases}
...
从而对\(\forall n\in\mathbb{N}_+\),都有
\[
\vert f^\prime(\xi_n)\vert=\left\vert\frac{f(x_n) - f(x_0)}{x_n - x_0}\right\vert=\vert h(x_n)\vert\leqslant slant L
\]
进而对\(\forall n\in\mathbb{N}_+\),都有
\[
\vert x_{n + 1}-x_0\vert=\vert f^\prime(\xi_n)\vert\vert x_n - x_0\vert\leqslant slant L\vert x_n - x_0\vert\leqslant slant L^2\vert x_{n - 1}-x_0\vert\leqslant slant\cdots\leqslant slant L^n\vert x_1 - x_0\vert
\]
...
\[
\left\vert\frac{f(x) - f(a)}{x - a}\right\vert\geqslant slant r > 1, \quad\forall x\in [a - \delta, a + \delta].
\]
即
\[
\vert f(x) - f(a)\vert\geqslant slant r\vert x - a\vert, \quad\forall x\in [a - \delta, a + \delta].
\]
...
由于\(\lim_{n\rightarrow\infty}\vert x_n - a\vert = 0\),因此存在\(N\in\mathbb{N}\),使得对\(\forall n\geqslant slant N\),有
\[
\vert x_{n + 1} - a\vert = \vert f(x_n) - f(a)\vert\geqslant slant r\vert x_n - a\vert.
\]
故对\(\forall n\geqslant slant N\),有
\[
\vert x_{n + 1} - a\vert\geqslant slant r\vert x_n - a\vert\geqslant slant r^2\vert x_{n - 1} - a\vert\geqslant slant\cdots\geqslant slant r^n\vert x_1 - x_0\vert.
\]
...
令\(u_n(x) = (y_n(x))^n=\overbrace{[x[\cdots [x]\cdots ]]}^{n\text{次复合}}\geqslant slant 0\)，由于单调递增函数的复合仍是单调递增函数，且\([x]\)在\([0, +\infty)\)上单调递增，故\(u_n(x)\)在\([0, +\infty)\)上单调递增。
从而由\(u_n(x)\)的单调性可得
\[
u_n(x) \geqslant slant u_n(2)=\overbrace{[2[\cdots [2]\cdots ]]}^{n\text{次复合}} = 2^n\rightarrow \infty, n\rightarrow \infty.
\]
再结合\([x]\)的基本不等式:$x-1<\left[ x \right] \leqslant slant x$可知
\begin{align*}
...
\end{example}
\begin{remark}
\hypertarget{example4.58不妨设的原因}{不妨设 \(m_k > n_k\) 的原因:}由假设 \(a_n\) 不收敛可知，存在 \(\delta > 0\)，对 \(\forall N > 0\)，都存在 \(m \in \mathbb{N}\)，使得 \(\vert a_m - A \vert \geqslant slant \delta\)。从而
\begin{align*}
&\text{取 }N = n_1 > 0\text{，则存在 }m_1 \in \mathbb{N}\text{，使得 }\vert a_{m_1} - A \vert \geqslant slant \delta.\\
&\text{取 }N = n_2 > 0\text{，则存在 }m_2 \in \mathbb{N}\text{，使得 }\vert a_{m_2} - A \vert \geqslant slant \delta.\\
&\cdots\cdots\\
&\text{取 }N = n_k > 0\text{，则存在 }m_k \in \mathbb{N}\text{，使得 }\vert a_{m_k} - A \vert \geqslant slant \delta.\\
&\cdots\cdots
\end{align*}
这样就得到了一个子列 \(\{ a_{m_k} \}\) 满足对 \(\forall n \in \mathbb{N}_+\)，都有 \(m_k > n_k\) 且 \(\vert a_{m_k} - A \vert \geqslant slant \delta\)。
\end{remark}
\begin{proof}
由 \(a_{n + 1} < a_n + b_n\) 可得
\begin{align}
a_n = a_1 + \sum_{i = 1}^{n - 1} (a_{i + 1} - a_i) < a_1 + \sum_{i = 1}^{n - 1} b_i, \forall n \geqslant slant 2. \label{example4.58-1.1}
\end{align}
...
\begin{align*}
\vert a_{m_k} - A \vert \geqslant slant \delta, \forall n \in \mathbb{N}_+. 
\end{align*}
\hyperlink{example4.58不妨设的原因}{不妨设 \(m_k > n_k, \forall n \in \mathbb{N}_+\)}。此时分两种情况讨论。

(i) 如果有无穷多个 \(k\)，使得 \(a_{m_k} \geqslant slant A + \delta\) 成立。再结合条件可得，对这些 \(k\)，都有
\begin{align}
a_{m_k} - a_{n_k} &= \sum_{i = n_k}^{m_k - 1} (a_{i + 1} - a_i) < \sum_{i = n_k}^{m_k - 1} b_i, \label{example4.58-1.3}\\
a_{m_k} - a_{n_k} &= (a_{m_k} - A) + (A - a_{n_k}) \geqslant slant \delta + (A - a_{n_k}). \label{example4.58-1.4}
\end{align}
...
\[
0 < \delta \leqslant slant \lim_{k \to \infty} (a_{m_k} - a_{n_k}) \leqslant slant \lim_{k \to \infty} \sum_{i = n_k}^{m_k - 1} b_i = 0.
\]
上述不等式矛盾。

(ii) 如果有无穷多个 \(k\)，使得 \(a_{m_k} \leqslant slant A - \delta\) 成立。取 \(\{ a_{n_k} \}\) 的一个子列 \(\{ a_{t_k} \}\)，使得 \(t_k > m_k, \forall n \in \mathbb{N}_+\)，则 \(\lim_{k \to \infty} a_{t_k} = \lim_{k \to \infty} a_{n_k} = A\)。
再结合条件可得，对这些 \(k\)，都有
\begin{align}
a_{t_k} - a_{m_k} &= \sum_{i = m_k}^{t_k - 1} (a_{i + 1} - a_i) < \sum_{i = m_k}^{t_k - 1} b_i, \label{example4.58-1.5}\\
a_{t_k} - a_{m_k} &= (a_{t_k} - A) + (A - a_{m_k}) \geqslant slant (a_{t_k} - A) + \delta. \label{example4.58-1.6}
\end{align}
...
\[
0 < \delta \leqslant slant \lim_{k \to \infty} (a_{t_k} - a_{m_k}) \leqslant slant \lim_{k \to \infty} \sum_{i = m_k}^{t_k - 1} b_i = 0.
\]
上述不等式矛盾。结论得证。
\end{proof}
...
\[
a_n\leqslant slant c_0\lambda ^n,n\rightarrow \infty \Leftrightarrow \frac{a_{n+1}}{a_n}\le \lambda ,n\rightarrow \infty \Leftrightarrow \frac{f(a_n)}{a_n}\le \lambda ,n\rightarrow \infty 
\]
...
当数列\(x_n\)不收敛时，因为数列\(x_n\)有界，所以可设\(\limsup_{n\rightarrow \infty}x_n = L<\infty\),\(\liminf_{n\rightarrow \infty}x_n = l<L\)。假设\(\exists A\in (l,L)\)，使得\(A\)不是\(x_n\)的极限点。则\(\exists \delta \in \left( 0,\min \{ L - A,A - l \} \right)\)，使得区间\((A - \delta,A + \delta)\subseteq (l,L)\)中只包含了数列\(x_n\)中有限项。因此存在\(N_1\in \mathbb{N}\)，使得当\(n > N_1\)时，有\(\vert x_n - A\vert\geqslant slant \delta\)。即
\begin{align}
\text{当}n>N_1\text{时},\text{要么}x_n\geqslant slant A+\delta ,\text{要么}x_n\leqslant slant A-\delta .\label{lemma4.1-1.1}
\end{align}
...
\[
x_{m_k}<l + \varepsilon \leqslant slant A - \delta <A + \delta \leqslant slant L - \varepsilon <x_{n_k}.
\]
任取\(K\in \mathbb{N}_+\cap (N,+\infty)\)，则\(x_{m_K}<l + \varepsilon \leqslant slant A - \delta <A + \delta \leqslant slant L - \varepsilon <x_{n_K}\)。不妨设\(n_K>m_K\)，则\(n_K>m_K\geqslant slant K>N\)。现在考虑\(x_{m_K},x_{m_K + 1},\cdots,x_{n_K - 1},x_{n_K}\)这些项。将其中最后一个小于等于\(A - \delta\)的项记为\(x_s\)，显然\(n_K - 1\geqslant slant s\geqslant slant m_K\geqslant slant K>N\)，进而\(s + 1\in [m_K + 1,n_K]\)，于是\(x_{s + 1}>A - \delta\)。又因为\(s + 1\geqslant slant m_K + 1>K>N\)，所以结合\eqref{lemma4.1-1.1}可知，\(x_{s + 1}\geqslant slant A + \delta\)。因此\(\vert x_{s + 1} - x_s\vert\geqslant slant 2\delta\)。这与\eqref{lemma4.1-1.2}式矛盾! 因此\(x_n\)的全体聚点构成一个闭区间$[l,L]$.
\end{proof}
