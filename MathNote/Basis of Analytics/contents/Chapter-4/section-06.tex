...
\end{align*}
于是当$x_1\geqslant  \frac{\sqrt{5}-1}{2}$时,有$x_3-x_1=\frac{1+x_1}{2+x_1}-x_1\leqslant 0$,即$x_3\leqslant x_1$.从而由\hyperref[proposition:递增函数递推数列]{递增递推结论}可知,$\{x_{2n-1}\}$单调递减且$x_{2n-1}>\frac{\sqrt{5}-1}{2},\forall n \in \mathbb{N}_+$.此时\(x_2<\frac{\sqrt{5}-1}{2}\)(由\(x = \frac{1}{1 + x}\)以及$x_n>0$可以解得不动点\(x_0=\frac{\sqrt{5}-1}{2}\),又因为原数列是递减递推,所以\(x_n\)与\(x_0\)大小关系交错.而\(x_1\geqslant\frac{\sqrt{5}-1}{2}\),故\(x_2<\frac{\sqrt{5}-1}{2}\)).
...
\end{align*}
因此$x_1,\cdots ,x_{2025}$都是$\left\{ x_n \right\}$的聚点.于是取$\varepsilon =\underset{q=1,\cdots ,2025}{\min}\left\{ \frac{\left| x_0-x_q \right|}{2} \right\},$对$\forall q\in \left\{ 1,2,\cdots ,2025 \right\},$都存在$N_q\in \mathbb{N},$使得
\begin{align}
\left| x_{2025p+q}-x_q \right|<\varepsilon ,\quad \forall p>N_q.\label{eq:155.1}
\end{align}
...
\begin{align}
\left| x_{n_k}-x_0 \right|<\varepsilon ,\quad \forall k>K.\label{eq:155.2}
\end{align}
...
\end{align*}
于是由\eqref{eq:155.1}式知
\begin{align*}
\left| x_{n_k}-x_{q_k} \right|=\left| x_{2025p_k+q_k}-x_{q_k} \right|<\varepsilon .
\end{align*}
再结合\eqref{eq:155.2}式可得
\begin{align*}
\left| x_0-x_{q_k} \right|\leqslant \left| x_{n_k}-x_0 \right|+\left| x_{n_k}-x_{q_k} \right|<2\varepsilon <\left| x_0-x_{q_k} \right|
\end{align*}
显然矛盾！故结论得证.
\end{enumerate}
\end{proof}
