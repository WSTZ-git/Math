...
\item 分部积分的优先级(即优先考虑凑入微分的函数): 三角函数($\sin x \cos x$等)$\gg $ 指数函数($e^x$等) $\gg $ 幂函数($x^n$等) $\gg $ 对数函数($\ln x$等) $\gg $ 反三角函数($\arcsin x,\arccos x$等). 
\end{enumerate}

\begin{theorem}[Newton-Leibniz公式]\label{theorem:Newton-Leibniz公式}
\begin{enumerate}
\item 若$f\in \mathbb{R}[a,b]$,且有原函数$F$,则有
\begin{align*}
...
\begin{example}
设 $\varphi \in C(\mathbb{R})$ 满足周期 $1$ 且 $\int_0^1 \varphi(x)\,\mathrm{d}x = 0$. 则对 $f \in C^1[0, 1]$，证明 $\sum\limits_{n=1}^\infty \left( \int_0^1 f(x) \varphi(nx)\,\mathrm{d}x \right)^2$ 收敛.
\end{example}
\begin{note}
...
定义
\begin{align*}
g(x) \triangleq \int_0^x \varphi(y)\,\mathrm{d}y,
\end{align*}
则由 $\varphi\in C(\mathbb{R}),\int_0^1 \varphi(y)\,\mathrm{d}y=0$，知 $g(0)=g(1)=0$，且 $g\in C(\mathbb{R})$ 以 $1$ 为周期. 记
\begin{align*}
M \triangleq \sup_{x\in\mathbb{R},\, y\in[0,1]} |g(x) f'(y)|.
\end{align*}
对 $n\in\mathbb{N}$，分部积分得
\begin{align*}
\int_0^1 f(x)\varphi(nx)\,\mathrm{d}x
= \frac{1}{n}\left[f(x)g(nx)\right]_0^1 - \frac{1}{n}\int_0^1 f'(x)g(nx)\,\mathrm{d}x= -\frac{1}{n}\int_0^1 f'(x)g(nx)\,\mathrm{d}x.
\end{align*}
因此
\begin{align*}
\left|\int_0^1 f(x)\varphi(nx)\,\mathrm{d}x\right|
\leqslant \frac{1}{n}\int_0^1 |f'(x)|\,|g(nx)|\,\mathrm{d}x 
\leqslant \frac{M}{n}.
\end{align*}
于是
\begin{align*}
\left(\int_0^1 f(x)\varphi(nx)\,\mathrm{d}x\right)^2 \leqslant \frac{M^2}{n^2}\Longrightarrow \sum_{n=1}^\infty \left(\int_0^1 f(x)\varphi(nx)\,\mathrm{d}x\right)^2
\leqslant \sum_{n=1}^\infty \frac{M^2}{n^2} < \infty.
\end{align*}
\end{proof}
