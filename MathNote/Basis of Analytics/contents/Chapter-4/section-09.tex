...
\begin{note}
\begin{enumerate}[(1)]
\item\label{example-note:不妨设的原因(1)} 不妨设 \(E = \mathbb{R}\) 的原因:若 \eqref{equation:4.1-1.1}式在 \(E = \mathbb{R}\) 时已得证明,则当 \(E\subseteq \mathbb{R}\) 时,令 \(\widetilde{f}(y) = f(y)\cdot \mathcal{X}_E\),\(y\in \mathbb{R}\),则由 \(f(y)\) 在 \(E\) 上绝对可积,可得 \(\widetilde{f}(y)\) 在 \(\mathbb{R}\) 上也绝对可积.从而由假设可知
\[
...
\iff &\lim_{x\rightarrow +\infty} \int_E{f(y)\left[ g(xy) - \int_0^1{g(t)\mathrm{d}t} \right] \mathrm{d}y}=\int_E{f(y)\mathrm{d}y\int_0^1{\left[ g(y) - \int_0^1{g(t)\mathrm{d}t} \right] \mathrm{d}y}}\\
\iff &\lim_{x\rightarrow +\infty} \left(\int_E{f(y)g(xy)\mathrm{d}y}-\int_E{f(y)\int_0^1{g(t)\mathrm{d}t}\mathrm{d}y}\right)=\int_E{f(y)\mathrm{d}y\int_0^1{g(y)\mathrm{d}y}}-\int_E{f(y)\mathrm{d}y}\int_0^1{g(t)\mathrm{d}t}=0\\
\iff &\lim_{x\rightarrow +\infty} \int_E{f(y)g(xy)\mathrm{d}y}=\int_E{f(y)\int_0^1{g(t)\mathrm{d}t}\mathrm{d}y}
\end{align*}
再结合\hyperref[example-note:不妨设的原因(3)]{\ref{example-note:不妨设的原因(2)}}的不妨设可知,此时原结论成立.故可以不妨设 \(\int_0^1{g(y)\mathrm{d}y} = 0\).
\end{enumerate}
\end{note}
\begin{proof}
...
\end{align}
由\(f\in R[0, 2\pi]\)可知,\(f\)在\([0, 2\pi]\)上有界也内闭有界.从而利用\eqref{example4.61-1.1}式可知,对\(\forall n\in \mathbb{N}\),一方面,我们有
\begin{align}
...
证明:
\begin{align*}
\lim_{n\to\infty} \int_{-\infty}^{+\infty} \frac{\sin nx}{\sqrt{x^2+x+1}} \, \mathrm{d}x = 0.
\end{align*}
\end{example}
\begin{proof}
...
\begin{align}
\int_{-\infty}^{+\infty} \frac{\sin nx}{\sqrt{x^2+x+1}} \, \mathrm{d}x 
&= \lim_{x\to -\infty} \frac{\cos nx}{n\sqrt{x^2+x+1}} - \lim_{x\to +\infty} \frac{\cos nx}{n\sqrt{x^2+x+1}} \nonumber \\
&\quad + \frac{1}{n} \int_{-\infty}^{+\infty} \frac{2x+1}{2(x^2+x+1)^{\frac{3}{2}}} \cdot \cos nx \, \mathrm{d}x \nonumber \\
&= \frac{1}{n} \int_{-\infty}^{+\infty} \frac{2x+1}{2(x^2+x+1)^{\frac{3}{2}}} \cdot \cos nx \, \mathrm{d}x. \label{eq:part}
\end{align}
注意到
\[
\int_{-1}^1 \frac{2x+1}{2(x^2+x+1)^{\frac{3}{2}}} \, \mathrm{d}x \leqslant \int_{-1}^1 \frac{3}{2^{\frac{3}{2}}} \, \mathrm{d}x = \frac{3}{\sqrt{2}} < +\infty.
\]
又因为
\[
\frac{2x+1}{2(x^2+x+1)^{\frac{3}{2}}} \sim \frac{1}{x^2}, \quad x\to \pm\infty,
\]
所以
\[
\int_1^{+\infty} \frac{2x+1}{2(x^2+x+1)^{\frac{3}{2}}} \, \mathrm{d}x, \qquad \int_{-\infty}^{-1} \frac{2x+1}{2(x^2+x+1)^{\frac{3}{2}}} \, \mathrm{d}x < +\infty.
\]
故 $\frac{2x+1}{2(x^2+x+1)^{\frac{3}{2}}}$ 在 $(-\infty,+\infty)$ 上绝对可积. 因此由 \hyperref[theorem:Riemann引理]{Riemann引理}知
\[
\lim_{n\to\infty} \int_{-\infty}^{+\infty} \frac{2x+1}{2(x^2+x+1)^{\frac{3}{2}}} \cdot \cos nx \, \mathrm{d}x = 0.
\]
再结合\eqref{eq:part}式知
\[
\lim_{n\to\infty} \int_{-\infty}^{+\infty} \frac{\sin nx}{\sqrt{x^2+x+1}} \, \mathrm{d}x = 0.
\]
\end{proof}
...
\begin{example}
设\(f\in C^{2}[0,\frac{\pi}{2}],f(0)=0\),计算
\[
...
由于\(f\in C^2\left[0, \frac{\pi}{2}\right]\),故利用L'Hospital法则可知
\[
...
\begin{align}
\lim_{n\rightarrow \infty} \int_0^{\frac{\pi}{2}}{\frac{f(x) - f^\prime(0)x}{\sin^2x}\sin^2(nx)\mathrm{d}x}=\frac{1}{\pi}\int_0^{\frac{\pi}{2}}{\frac{f(x) - f^\prime(0)x}{\sin^2x}\mathrm{d}x}\int_0^{\pi}{\sin^2x\mathrm{d}x}
= \frac{1}{2}\int_0^{\frac{\pi}{2}}{\frac{f(x) - f^\prime(0)x}{\sin^2x}\mathrm{d}x}<\infty. \label{example4.62-1.2}
\end{align}
...
\begin{example}
计算$\displaystyle\lim_{x\rightarrow 0}\frac{1}{x}\int_{x}^{2x}\sin\frac{1}{t}\mathrm{d}t$.
\end{example}
\begin{solution}

\begin{align*}
\lim_{x\rightarrow 0}\limits\frac{1}{x}\int_{x}^{2x}\sin\frac{1}{t}\mathrm{d}t &= \lim_{x\rightarrow 0}\limits\frac{1}{x}\int_{\frac{1}{x}}^{\frac{1}{2x}}\frac{\sin y}{y^2}\mathrm{d}y = \lim_{x\rightarrow \infty}\limits x\int_{x}^{\frac{x}{2}}\frac{\sin y}{y^2}\mathrm{d}y \\
&= \lim_{x\rightarrow \infty}\limits\int_{\frac{1}{2}}^{1}\frac{\sin(xy)}{y^2}\mathrm{d}y \xlongequal{\text{\hyperref[theorem:Riemann引理]{Riemann引理}}} 0.
\end{align*}
