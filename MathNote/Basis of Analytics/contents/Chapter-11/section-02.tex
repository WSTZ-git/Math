...
\end{align*}
于是由\hyperref[theorem:单侧导数极限定理]{导数极限定理}可知$h^{(k)}(0)=\frac{f^{(k+1)}(0)}{k+1},\forall k\in \mathbb{N}$,故$h$在$x=0$处无穷次可微.又$h\in C^{\infty}(\mathbb{R}\backslash \{0\})$,故$h\in C^{\infty}(\mathbb{R})$.于是
\begin{gather}
