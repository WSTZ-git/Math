...
\item 我们断言
\begin{align}
f^{(n)}(x) = a^n \sin\left(ax + \frac{n}{2}\pi\right), \quad \forall n \in \mathbb{N}. \label{100.2412424234SG116}
\end{align}
当 \( n = 0 \) 时，上式显然成立。假设当 \( n = k \) 时上式成立，则
\[
f^{(k+1)}(x) = a^{k+1} \cos\left(ax + \frac{k}{2}\pi\right) = a^{k+1} \sin\left(ax + \frac{k+1}{2}\pi\right).
\]
故由数学归纳法可知 \eqref{100.2412424234SG116} 式成立。
...
\[
(\ln x)^{(n)} = (xy)^{(n)} \Longleftrightarrow \frac{(-1)^{n - 1}(n - 1)!}{x^n} = \sum_{k=0}^{n} \mathrm{C}_{n}^{k} x^{(k)} y^{(n - k)} = xy^{(n)} + n y^{(n - 1)}.
\]
...
\begin{align*}
f^{\left( n \right)}\left( x \right) &=\left( \frac{1}{1+x^2} \right) ^{\left( n-1 \right)}=\frac{\left( -1 \right) ^{n-1}\left( n-1 \right) !}{2\mathrm{i}}\left[ \frac{1}{\left( x-\mathrm{i} \right) ^n}-\frac{1}{\left( x+\mathrm{i} \right) ^{n}} \right] =\frac{\left( -1 \right) ^{n-1}\left( n-1 \right) !}{2\mathrm{i}\left( x^2+1 \right) ^{n}}\left[ \left( x+\mathrm{i} \right) ^{n}-\left( x-\mathrm{i} \right) ^n \right] 
\\
&=\frac{\left( -1 \right) ^{n-1}\left( n-1 \right) !}{2\mathrm{i}\left( x^2+1 \right) ^{n}}\left[ \left( \sqrt{1+x^2}e^{\mathrm{iarc}\tan \frac{1}{x}} \right) ^{n}-\left( \sqrt{1+x^2}e^{-\mathrm{iarc}\tan \frac{1}{x}} \right) ^n \right] 
\\
&=\frac{\left( -1 \right) ^{n-1}\left( n-1 \right) !}{2\mathrm{i}\left( x^2+1 \right) ^{\frac{n}{2}}}\left( e^{\mathrm{i}n\arctan  \frac{1}{x}}-e^{-\mathrm{i}n\arctan  \frac{1}{x}} \right) =\frac{\left( -1 \right) ^{n-1}\left( n-1 \right) !}{2\mathrm{i}\left( x^2+1 \right) ^{\frac{n}{2}}}\cdot 2\mathrm{i}\cdot \sin \left( n\arctan  \frac{1}{x} \right) 
\\
&=\frac{\left( -1 \right) ^{n-1}\left( n-1 \right) !}{\left( x^2+1 \right) ^{\frac{n}{2}}}\sin \left( n\arctan  \frac{1}{x} \right) .
\end{align*}
\end{enumerate}
...
注意到
\begin{align*}
\left[ \ln\left(x + \sqrt{1 + x^2}\right) \right]' &= \left( \mathrm{arcsinh} x \right)' = \frac{1}{\sqrt{1 + x^2}} = \left(1 + x^2\right)^{-\frac{1}{2}} \\
...
\nonumber
\\
&\Longleftrightarrow f^{\left( n+2 \right)}\left( 0 \right) =n^2f^{\left( n \right)}\left( 0 \right) .\label{100.2412424234----2-4-:::::--24..1191}
\end{align}
又 \( f''(0) = 2 \)，因此对 \( \forall n \in \mathbb{N}_1 \)，由 \eqref{100.2412424234----2-4-:::::--24..1191}式可得
\[
\frac{f^{(2n + 2)}(0)}{f^{(2n)}(0)} = 4n^2 \Rightarrow \frac{f^{(2n + 2)}(0)}{f^{(2)}(0)} = \prod_{i = 1}^n 4i^2 \Rightarrow f^{(2n + 2)}(0) = 2^{2n + 1} (n!)^2.
\]
显然上式对 \( n = 0 \) 也成立。故
...
f^{(2n + 2)}(0) = 2^{2n + 1} (n!)^2, \forall n \in \mathbb{N}_0.
\]
又 \( f'''(0) = 0 \)，故由\eqref{100.2412424234----2-4-:::::--24..1191}式可得
\[
f^{(2n - 1)}(0) = (2n - 1)^2 f^{(2n - 3)}(0) = \cdots = \left[ (2n - 1)!! \right]^2 f^{(3)}(0) = 0, \forall n \in \mathbb{N}_1.
\]
...
\begin{align*}
\lim_{x\to 0}h(x)=\lim_{x\to 0}\frac{f(x)}{x} = f'(0)=0,
\end{align*}
故$h$可连续延拓定义$h(0)=0$.由$f\in C^{\infty}(\mathbb{R})$可知$h\in C^{\infty}(\mathbb{R}\backslash \{0\})$.并且
\begin{align}
\frac{f(x)}{x^{n+1}}=\frac{\frac{f(x)}{x}}{x^n}=\frac{h(x)}{x^n},\quad x\neq 0. \label{100.2412424234----2-4-:::::--24..119}
\end{align}
...
\end{align*}
于是由\hyperref[theorem:导数极限定理]{导数极限定理}可知$h^{(k)}(0)=\frac{f^{(k+1)}(0)}{k+1},\forall k\in \mathbb{N}$,故$h$在$x=0$处无穷次可微.又$h\in C^{\infty}(\mathbb{R}\backslash \{0\})$,故$h\in C^{\infty}(\mathbb{R})$.于是
\begin{gather}
