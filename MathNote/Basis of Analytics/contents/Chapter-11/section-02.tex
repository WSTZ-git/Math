...
\]
其中 \( a_n = \frac{f^{(n)}(0)}{n!}, n \in \mathbb{N}_0 \)。再将上式代入\eqref{100.118} 式可得
\begin{align*}
...
\nonumber
\\
&\Longleftrightarrow f^{\left( n+2 \right)}\left( 0 \right) =n^2f^{\left( n \right)}\left( 0 \right) .\label{100.119}
\end{align}
又 \( f''(0) = 2 \)，因此对 \( \forall n \in \mathbb{N}_1 \)，由 \eqref{100.119}式可得
\[
\frac{f^{(2n + 2)}(0)}{f^{(2n)}(0)} = 4n^2 \Rightarrow \frac{f^{(2n + 2)}(0)}{f^{(2)}(0)} = \prod_{i = 1}^n 4i^2 \Rightarrow f^{(2n + 2)}(0) = 2^{2n + 1} (n!)^2.
\]
显然上式对 \( n = 0 \) 也成立。故
