...
\begin{definition}
对\(1\leqslant slant i\leqslant slant n,V_n^{(i)}(x_1,x_2,\cdots,x_n)\)表示删除\(V_n(x_1,x_2,\cdots,x_n)\)的第\(i\)行\((x_1^{i - 1},x_2^{i - 1},\cdots,x_n^{i - 1})\)之后新添第\(n\)行\((x_1^{n},x_2^{n},\cdots,x_n^{n})\)所得\(n\)阶行列式.
\end{definition}
...
\sigma_j=\sum_{1\leqslant slant k_1<k_2<\cdots<k_j\leqslant slant n}x_{k_1}x_{k_2}\cdots x_{k_j},j = 1,2,\cdots,n,
\end{align}
我们有
\begin{align}
...
\end{example}
\begin{proof}
(加边法)不妨设\(x_i,1\leqslant slant i\leqslant slant n\)互不相同. 设
\begin{align*}
