...
从而
\begin{align*}
\delta _2&=\sum_{1\leqslant i<j\leqslant n+1}{x_ix_j}=\sum_{1\leqslant i<j\leqslant n}{x_ix_j}+x_{n+1}\sum_{i=1}^n{x_i}
\\
&=\sum_{1\leqslant i<j\leqslant n}{x_ix_j}-\left( x_1+x_2+\cdots +x_n \right) \sum_{i=1}^n{x_i}
\\
&=\sum_{1\leqslant i<j\leqslant n}{x_ix_j}-\left( \sum_{i=1}^n{x_i} \right) ^2=-\sum_{i=1}^n{x_{i}^{2}}-\sum_{1\leqslant i<j\leqslant n}{x_ix_j}.
\end{align*}
...
\end{matrix} \right|=\prod_{1\leqslant i<j\leqslant n}{\left( x_j-x_i \right) }.
\end{align*}
...
\end{matrix} \right|=V_n\left( x_1,x_2,\cdots ,x_n \right) =\prod_{1\leqslant i<j\leqslant n}{\left( x_j-x_i \right) .}
\end{align*}
...
\end{matrix} \right|
\\
&=\prod_{1\leqslant i<j\leqslant n}{(x_j}-x_i)\cdot \left| \begin{matrix}
1&		0&		0&		\cdots&		0&		0&		0\\
1&		1&		0&		\cdots&		0&		0&		0\\
0&		1&		1&		\cdots&		0&		0&		0\\
\vdots&		\vdots&		\vdots&		\ddots&		\vdots&		\vdots&		\vdots\\
...
\begin{align*}
|\boldsymbol{A}|=\underset{\left( a_{k_1},\cdots ,a_{k_m} \right) \rightarrow \left( 0,\cdots ,0 \right)}{\lim}\prod_{1\leqslant i<j\leqslant n}{(a_ib_j-a_jb_i)}.
\end{align*}
得到的结果与下述证明相同.
\end{remark}
\begin{solution}
...
\end{matrix} \right|
\\
&=\prod_{i=1}^n{a_{i}^{n-1}}\prod_{1\leqslant i<j\leqslant n}{\left( \frac{b_j}{a_j}-\frac{b_i}{a_i} \right)}=\prod_{i=1}^n{a_{i}^{n-1}}\prod_{1\leqslant i<j\leqslant n}{\frac{a_ib_j-a_jb_i}{a_ja_i}}\hyperlink{连乘号计算小结论(1)}{=}\prod_{1\leqslant i<j\leqslant n}{(a_ib_j-a_jb_i)}.
\end{align*}
...
\vdots&		\vdots&		&		\vdots\\
a_{n}^{n-1}&		a_{n}^{n-2}b_n&		\cdots&		a_nb_{n}^{n-2}\\
\end{matrix} \right|=\left( -1 \right) ^{n+i}b_{i}^{n-1}\prod_{\substack{1\leqslant k\leqslant n\\
...
\end{matrix} \right|
\\
&=\left( -1 \right) ^{n+i}b_{i}^{n-1}\prod_{\substack{1\leqslant k\leqslant n\\
k\ne i\\}}{a_{k}^{n-1}}\prod_{\substack{1\leqslant k<l\leqslant n\\
...
\\
&=\prod_{1\leqslant k<i}{a_kb_i}\prod_{i<l\leqslant n}{\left( -a_lb_i \right)}\cdot \prod_{\substack{
1\leqslant k<l\leqslant n\\
k,l\ne i\\
}}{\left( a_kb_l-a_lb_k \right)}
\\
&=\prod_{1\leqslant k<l\leqslant n}{\left( a_kb_l-a_lb_k \right)}\left(\text{其中} a_i=0 \right).
\end{align*}
若至少有两个$a_i=a_j=0$,则第$i$行与第$j$行成比例,因此行列式的值等于0.经过计算发现,后面两种情形的答案都可以统一到第一种情形的答案.

综上所述,$|\boldsymbol{A}|=\prod_{1\leqslant i<j\leqslant n}{(a_ib_j-a_jb_i)}.$
...
\hypertarget{连乘号计算小结论(1)}{(1)}$\prod_{1\leqslant i<j\leqslant n}{a_ia_j}=\prod_{i=1}^n{a_{i}^{n-1}}.$
\begin{align*}
\text{证明:}&\prod_{1\leqslant i<j\leqslant n}{a_ia_j}=\underset{n-1\text{组}}{\underbrace{a_2a_1\cdot a_3a_2a_3a_1\cdot a_4a_3a_4a_2a_4a_1\cdots \cdots \overset{k-1\text{对}}{\overbrace{a_ka_{k-1}a_ka_{k-2}\cdots a_ka_1}}\cdots \cdots \overset{n-1\text{对}}{\overbrace{a_na_{n-1}a_na_{n-2}\cdots a_na_1}}}}
\\
&\xlongequal{\text{从左往右按组计数}}a_{1}^{n-1}a_{2}^{1+n-2}a_{3}^{2+n-3}a_{4}^{3+n-4}\cdots a_{k}^{k-1+n-k}\cdots a_{n}^{n-1}=\prod_{i=1}^n{a_{i}^{n-1}}.
\end{align*}
\hypertarget{连乘号计算小结论(2)}{(2)}$\prod_{\substack{1\leqslant i<j\leqslant n\\i,j\ne k}}{a_ia_j}=\prod_{\substack{
...
\end{matrix} \right|}
\\
&=\left( -1 \right) ^{\frac{n\left( n+1 \right)}{2}}\prod_{0\leqslant j<i\leqslant n}{\left( a_i-a_j \right)}\prod_{i=1}^{n-1}{\mathrm{C}_{n}^{i}\prod_{0\leqslant j<i\leqslant n}{\left( b_i-b_j \right)}}
= \prod_{i=1}^{n-1}{\mathrm{C}_{n}^{i}\prod_{0\leqslant j<i\leqslant n}{\left( a_j-a_i \right) \left( b_i-b_j \right)}}.
\end{align*}
...
\end{matrix} \right|
\\
&=2^{\frac{1}{2}(n-1)(n-2)}\prod_{1\leqslant i<j\leqslant n}{\left( \cos \theta _j-\cos \theta _i \right)}.
\end{align*}
...
\end{matrix} \right|=2^{\frac{1}{2}\left( n-2 \right) \left( n-1 \right) +n-1}\prod_{i=1}^n{\sin \theta _i}\prod_{1\leqslant i<j\leqslant n}{\left( \cos \theta _j-\cos \theta _i \right)}
\\
&=2^{\frac{1}{2}n\left( n-1 \right)}\prod_{i=1}^n{\sin \theta _i}\prod_{1\leqslant i<j\leqslant n}{\left( \cos \theta _j-\cos \theta _i \right)}.
\end{align*}
