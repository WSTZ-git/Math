...
\end{example}
\begin{note}
注意到这个行列式每行元素除了主对角元素外,其余位置元素都相同.因此这个行列式是\hyperref[proposition:爪型行列式的推广]{推广的proposition:爪型行列式}.
\end{note}
\begin{solution}
\begin{align*}
...
\end{matrix} \right|
\\
&\xlongequal{\hyperref[proposition:爪型行列式]{\text{proposition:爪型行列式}}}(x-a_1)^2\prod_{i=2}^n{\left( x^2-2a_ix \right)}-\sum_{i=2}^n{a_{i}^{2}\left( 2a_1x-x^2 \right) \left( x^2-2a_2x \right) \cdots \widehat{\left( x^2-2a_ix \right) }\cdots}\left( x^2-2a_nx \right) 
\\
...
\\
&\xlongequal{\text{区间再现}}(n - 1)!\prod_{1\leqslant i < j\leqslant n}(j - i)\int_{0}^{n}\prod_{k = 0}^{n}(n - k - x)\mathrm{d}x
\\
...
\end{aligned}
\]
当 \(n \geqslant 3\) 时,由\hyperref[theorem:Cauchy-Binet公式]{Cauchy-Binet公式}可知$D_n=0.$
