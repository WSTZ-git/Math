...
循环行列式关于单位根的计算公式见\hyperref[proposition:循环矩阵的特征值和对角化]{命题\ref{proposition:循环矩阵的特征值和对角化}}.
...
\end{example}
\begin{proof}
记 $f(x) \triangleq \sum_{i=1}^n u_i x^i$, $w_j \triangleq e^{\frac{2\pi j \mathrm{i} }{n}}$, $j=0,1,2,\cdots,n-1$, 则由\hyperref[proposition:循环矩阵的特征值和对角化]{命题\ref{proposition:循环矩阵的特征值和对角化}}可知
\begin{align*}
...
\end{example}
\begin{proof}
记$f(x) = \sum_{j=0}^{n-1} (a+jd)w_k^j$,其中$w_k=e^{\frac{2\pi k\mathrm{i}}{n}},k=0,1,2,\cdots ,n-1.$由\hyperref[proposition:循环矩阵的特征值和对角化]{命题\ref{proposition:循环矩阵的特征值和对角化}}可知
\begin{align*}
