...
\begin{note}
记忆三对角行列式的计算方法和结果:
$\boldsymbol{D}_{\boldsymbol{n}}=\boldsymbol{a}_{\boldsymbol{n}}\boldsymbol{D}_{\boldsymbol{n}-\boldsymbol{1}}-\boldsymbol{b}_{\boldsymbol{n}-\boldsymbol{1}}\boldsymbol{c}_{\boldsymbol{n}-\boldsymbol{1}}\boldsymbol{D}_{\boldsymbol{n}-\boldsymbol{2}}\boldsymbol{(n}\geqslant \boldsymbol{2)}$,
...
\end{matrix} \right|
\\
&\xlongequal[]{\text{第二项按最后}\mathbb{一行}\text{展开}}a_n\left| \begin{matrix}
a_1&		b_1&		&		&		&		\\
c_1&		a_2&		b_2&		&		&		\\
&		c_2&		a_3&		\ddots&		&		\\
&		&		\ddots&		\ddots&		\ddots&		\\
...
\begin{corollary}[三对角矩阵的行列式]\label{corollary:对角线元素相同的三对角行列式}
计算$n$阶行列式$(bc\ne0)$:
\begin{equation}
\begin{split}
D_n=\left| \begin{matrix}
...
\left( n+1 \right) \left( \frac{a}{2} \right) ^n,a^2=4bc.\\
\end{cases},
\end{split}
\nonumber
\end{equation}
...
\begin{proposition}[三对角矩阵的特征值]\label{proposition:三对角矩阵必可对角化}
...
{\color{blue}证法一:}
我们先来计算$A$的特征多项式$|\lambda I_n - A|$. 设$x_1,x_2$是二次方程$x^2 - (\lambda - a)x + bc = 0$的两个根,则当$(\lambda-a)^2 = 4bc$时,由\hyperref[corollary:对角线元素相同的三对角行列式]{推论\ref{corollary:对角线元素相同的三对角行列式}}可得
\begin{align*}
|\lambda I_n - A|=(n+1)\left( \frac{\lambda - a}{2}\right)^n=(n+1)\left( 2bc\right)^n \neq 0.
\end{align*}
此时$A$的特征多项式无解.
...
从而$|\boldsymbol{A}|=D_n\xlongequal[\text{命题}\ref{三对角行列式}]{\text{按最后一列展开}}2\cos xD_{n-1}-D_{n-2}\left( n\geqslant 2 \right)$.
...
\end{example}
\begin{note}
第(2)问{\color{blue}解法一}中不仅使用了\hyperlink{行列式计算:升阶法}{升阶法}还使用了\hyperref[proposition:分块proposition:爪型行列式]{分块proposition:爪型行列式的计算方法}.观察到各行各列有不同的公共项,因此可以利用升阶法将各行各列的公共项消去.
\end{note}
\begin{remark}
...
\end{matrix} \right|=\frac{\prod\limits_{1\leqslant i<j\leqslant n}{(a_j}-a_i)(b_j-b_i)}{\prod\limits_{1\leqslant i<j\leqslant m}{\left( a_i+b_j \right)}}.
\nonumber
\end{gather}
\end{proposition}
\begin{note}
...
\\
&=\frac{\prod\limits_{1\leqslant i<j\leqslant n}{(a_j}-a_i)(b_j-b_i)}{\prod\limits_{1\leqslant i\leqslant j\leqslant n}{(a_i}+b_j)\prod\limits_{1\leqslant j<i\leqslant n}{(a_i}+b_j)}=\frac{\prod\limits_{1\leqslant i<j\leqslant n}{(a_j}-a_i)(b_j-b_i)}{\prod\limits_{1\leqslant i<j\leqslant m}{\left( a_i+b_j \right)}}.
\nonumber
\end{align*}
...
对$\forall n\in \mathbb{N}$,设\(f_{ij}(t)\,(i,j=1,2,\cdots,n)\)都是可微函数,
\begin{align*}
F(t) = 
\left| \begin{matrix}
...
\nonumber
\end{align*}
求证:$\frac{\mathrm{d}}{\mathrm{d}t}F\left( t \right) =\sum_{j=1}^n{F_j\left( t \right)}$,其中
\begin{align*}
F_{j}(t) = 
\left| \begin{matrix}
f_{11}(t)&		f_{12}(t)&		\cdots&		\frac{\mathrm{d}}{\mathrm{d}t}f_{1j}(t)&		\cdots&		f_{1n}(t)\\
f_{21}(t)&		f_{22}(t)&		\cdots&		\frac{\mathrm{d}}{\mathrm{d}t}f_{2j}(t)&		\cdots&		f_{2n}(t)\\
\vdots&		\vdots&		&		\vdots&		&		\vdots\\
f_{n1}(t)&		f_{n2}(t)&		\cdots&		\frac{\mathrm{d}}{\mathrm{d}t}f_{nj}(t)&		\cdots&		f_{nn}(t)\\
\end{matrix} \right| 
\nonumber
\end{align*}
\end{proposition}
...
证法三实际上就是把$F$视作复合函数
\begin{align*}
F&=F\left( a_{1j},\cdots ,a_{nj},A_{1j},\cdots ,A_{nj} \right) =F\left( a_{1j},\cdots ,a_{nj},A_{1j}\left( \left( a_{kl} \right) _{k=1,\cdots ,n;l\ne j} \right) ,\cdots ,A_{nj}\left( \left( a_{kl} \right) _{k=1,\cdots ,n;l\ne j} \right) \right) 
\\
&=F\left( a_{1j}\left( t \right) ,\cdots ,a_{nj}\left( t \right) ,A_{1j}\left( \left( a_{kl}\left( t \right) \right) _{k=1,\cdots ,n;l\ne j} \right) ,\cdots ,A_{nj}\left( \left( a_{kl}\left( t \right) \right) _{k=1,\cdots ,n;l\ne j} \right) \right) .
\end{align*}
然后利用链式法则求导.
\end{note}
\begin{proof}
...
从而由归纳假设可得
\begin{gather*}
A_{i1}^{\prime}\left( t \right) =\frac{\mathrm{d}}{\mathrm{d}t}A_{i1}\left( t \right)=\sum_{k=2}^{n}{A_{i1}^{k}(t),i=1,2,\cdots ,n}. 
\\
\text{其中}A_{i1}^{k}(t)=\left| \begin{matrix}
f_{12}\left( t \right)&		\cdots&		\frac{\mathrm{d}}{\mathrm{d}t}f_{1k}\left( t \right)&		\cdots&		f_{1n}\left( t \right)\\
\vdots&		&		\vdots&		&		\vdots\\
f_{i-1,2}(t)&		\cdots&		\frac{\mathrm{d}}{\mathrm{d}t}f_{i-1,k}\left( t \right)&		\cdots&		f_{i-1,n}\left( t \right)\\
f_{i+1,2}\left( t \right)&		\cdots&		\frac{\mathrm{d}}{\mathrm{d}t}f_{i+1,k}(t)&		\cdots&		f_{i+1,n}\left( t \right)\\
\vdots&		&		\vdots&		&		\vdots\\
f_{n2}\left( t \right)&		\cdots&		\frac{\mathrm{d}}{\mathrm{d}t}f_{nk}\left( t \right)&		\cdots&		f_{nn}\left( t \right)\\
\end{matrix} \right|,k=2,3,\cdots ,n.
\nonumber
\end{gather*}
于是,我们就有
\begin{align*}
\frac{\mathrm{d}}{\mathrm{d}t}F\left( t \right) &=\frac{\mathrm{d}}{\mathrm{d}t}\left[ f_{11}\left( t \right) A_{11}\left( t \right) +f_{21}\left( t \right) A_{21}\left( t \right) +\cdots +f_{n1}\left( t \right) A_{n1}\left( t \right) \right] 
\\
...
\begin{align*}
F(t)=\sum_{1\leqslant k_1,k_2,\cdots ,k_n\leqslant n}{(}-1)^{\tau (k_1k_2\cdots k_n)}f_{k_11}(t)f_{k_22}(t)\cdots f_{k_nn}(t).
\end{align*}
因此
\begin{align*}
\frac{\mathrm{d}}{\mathrm{d}t}F(t)&=\sum_{1\leqslant k_1,k_2,\cdots ,k_n\leqslant n}{(}-1)^{\tau (k_1k_2\cdots k_n)}f'_{k_{11}}(t)f_{k_{22}}(t)\cdots f_{k_{nn}}(t)
\\
&\quad+\sum_{1\leqslant k_1,k_2,\cdots ,k_n\leqslant n}{(}-1)^{\tau (k_1k_2\cdots k_n)}f_{k_{11}}(t)f'_{k_{22}}(t)\cdots f_{k_{nn}}(t)
\\
&\quad+\cdots +\sum_{1\leqslant k_1,k_2,\cdots ,k_n\leqslant n}{(}-1)^{\tau (k_1k_2\cdots k_n)}f_{k_{11}}(t)f_{k_{22}}(t)\cdots f'_{k_{nn}}(t)
\\
&=F_1(t)+F_2(t)+\cdots +F_n(t).
\end{align*}

{\color{blue}证法三:}记 $A_{ij}(t)$ 为元素 $f_{ij}(t)$ 的代数余子式。固定第 $j$ 列，按第$j$列展开行列式得
\begin{align}
F(t) = \sum_{i=1}^n f_{ij}(t) A_{ij}(t). \label{eq:F_expand}
\end{align}
将 $F$ 视为关于 $f_{1j},\cdots,f_{nj}$ 以及 $A_{1j},\cdots,A_{nj}$ 的复合函数，则\eqref{eq:F_expand}式两边分别关于$A_{ij},f_{ij}$得
\begin{align}\label{eq::fujeiieiei93933FF}
\frac{\partial F}{\partial A_{ij}}=f_{ij},\quad \frac{\partial F}{\partial f_{ij}}=A_{ij},\quad i=1,2,\cdots,n. 
\end{align}
再由链式法则得
\begin{align}
\frac{\mathrm{d}F}{\mathrm{d}t}
= \sum_{i=1}^n \frac{\partial F}{\partial f_{ij}}\frac{\mathrm{d}f_{ij}}{\mathrm{d}t}
+ \sum_{i=1}^n \frac{\partial F}{\partial A_{ij}}\frac{\mathrm{d}A_{ij}}{\mathrm{d}t} = \sum_{i=1}^n A_{ij}\frac{\mathrm{d}f_{ij}}{\mathrm{d}t}
+ \sum_{i=1}^n f_{ij}\frac{\mathrm{d}A_{ij}}{\mathrm{d}t}. \label{eq:F_deriv}
\end{align}
由于 $A_{ij}$ 仅依赖于除第 $j$ 列以外的元素 $f_{kl}\,(l\ne j)$，应用链式法则得
\begin{align}
\frac{\mathrm{d}A_{ij}}{\mathrm{d}t}
= \sum_{l\ne j}\sum_{k=1}^n \frac{\partial A_{ij}}{\partial f_{kl}}\cdot \frac{\mathrm{d}f_{kl}}{\mathrm{d}t},\quad i=1,2,\cdots,n. \label{eq:dAij}
\end{align}
将 \eqref{eq:dAij} 代入 \eqref{eq:F_deriv} 的第二项，得
\begin{align}
\sum_{i=1}^n f_{ij}\frac{\mathrm{d}A_{ij}}{\mathrm{d}t}
&= \sum_{l\ne j}\sum_{k=1}^n
\left( \sum_{i=1}^n f_{ij}\frac{\partial A_{ij}}{\partial f_{kl}} \right)
\cdot \frac{\mathrm{d}f_{kl}}{\mathrm{d}t}. \label{eq:second}
\end{align}
当 $l\ne j$ 时，考虑 $F$ 关于 $f_{kl}$ 的偏导数。由 \eqref{eq:F_expand}\eqref{eq::fujeiieiei93933FF}式且 $f_{ij}\,(i=1,\cdots,n)$ 中不含 $f_{kl}$，有
\begin{align}
A_{kl}
&= \frac{\partial F}{\partial f_{kl}}
= \sum_{i=1}^n \frac{\partial}{\partial f_{kl}}\left( f_{ij}A_{ij} \right)
= \sum_{i=1}^n f_{ij}\frac{\partial A_{ij}}{\partial f_{kl}},\quad 1\leqslant k\leqslant n,\,l\in \{1,2,\cdots,n\}\setminus \{j\}. \label{eq:Akl}
\end{align}
将 \eqref{eq:Akl}式代入 \eqref{eq:second}式，得到
\begin{align}
\sum_{i=1}^n f_{ij}\frac{\mathrm{d}A_{ij}}{\mathrm{d}t}
= \sum_{l\ne j}\sum_{k=1}^n A_{kl}\frac{\mathrm{d}f_{kl}}{\mathrm{d}t}. \label{eq:second_final}
\end{align}
将 \eqref{eq:second_final}式代回 \eqref{eq:F_deriv}式得
\begin{align*}
\frac{\mathrm{d}F}{\mathrm{d}t}
= \sum_{i=1}^n A_{ij}\frac{\mathrm{d}f_{ij}}{\mathrm{d}t}
+ \sum_{l\ne j}\sum_{k=1}^n A_{kl}\frac{\mathrm{d}f_{kl}}{\mathrm{d}t} = \sum_{k=1}^n \sum_{l=1}^n A_{kl}\frac{\mathrm{d}f_{kl}}{\mathrm{d}t}.
\end{align*}
注意 $\sum_{k=1}^n A_{kl}\frac{\mathrm{d}f_{kl}}{\mathrm{d}t}$ 正是将行列式 $F$ 的第 $l$ 列元素替换为其导数后再按第 $l$ 列展开所得的行列式，因此
\[
\frac{\mathrm{d}F}{\mathrm{d}t} = \sum_{l=1}^n F_l(t),
\]
\end{proof}
