...
\begin{proposition}[\hypertarget{行列式计算常识}{行列式计算常识}]\label{proposition:行列式计算常识}
\begin{enumerate}
...
\item 设$n$阶行列式$D=\det(a_{ij})$,把$D$上下翻转(\textbf{行倒排})、或左右翻转(\textbf{列倒排})分别得到$D_1$、$D_2$;把$D$\textbf{逆时针旋转$90^{\circ}$}、或\textbf{顺时针旋转$90^{\circ}$}分别得到$D_3$、$D_4$;把$D$\textbf{依副对角线翻转}、或\textbf{依主对角线翻转}分别得到$D_5$、$D_6$.易知
\begin{align*}
...
\item\label{proposition:行列式计算常识-3}  设\(A=(a_{i,j})\)为\(n\)阶复矩阵,则一定有\(\left| \overline{A} \right|=\overline{\left| A \right|}=\overline{\det A}\).

\item 若\(\vert A\vert\)是\(n\)阶行列式,\(\vert B\vert\)是\(m\)阶行列式,它们的值都不为零,则
\begin{align*}
...
\begin{align*}
\left| \overline{A} \right|&=\sum_{1\leqslant k_1,k_2,\cdots ,k_n\leqslant n}{\left( -1 \right) ^{\tau (k_1k_2\cdots k_n)}\overline{a_{k_{11}}}\cdot \overline{a_{k_{22}}}\cdots \overline{a_{k_{nn}}}}
\\
&=\overline{\sum_{1\leqslant k_1,k_2,\cdots ,k_n\leqslant n}{\left( -1 \right) ^{\tau (k_1k_2\cdots k_n)}a_{k_{11}}a_{k_{22}}\cdots a_{k_{nn}}}}
\\
