...
|z - a_{ii}| \leq R_{i} = \sum_{j = 1,j\neq i}^{n}|a_{ij}|,\ 1 \leq i \leq n.
\end{align*}
由条件可知\(a_{ii} > R_{i}(1 \leq i \leq n)\),从而这些戈氏圆盘全部位于虚轴的右侧, 因此\(A\)的特征值\(\lambda_{i}\)或者是正实数, 或者是实部为正的共轭虚数, 从而\(|A|=\lambda_{1}\lambda_{2}\cdots\lambda_{n}>0\). 
\end{proof}
