...
\end{matrix} \right) \leqslant \mathrm{r}(\boldsymbol{A})+\mathrm{r}(\boldsymbol{B})\),\(\mathrm{r}\begin{pmatrix}\boldsymbol{A}\\\boldsymbol{B}\end{pmatrix}\leqslant \mathrm{r}(\boldsymbol{A})+\mathrm{r}(\boldsymbol{B})\)；

\item \label{矩阵秩的基本公式6}\(\mathrm{r}\left( \boldsymbol{A}\pm \boldsymbol{B} \right) \leqslant slant \mathrm{r}\left( \begin{array}{c}
\boldsymbol{A}\\
\boldsymbol{B}\\
\end{array} \right) =\mathrm{r}\left( \begin{matrix}
\boldsymbol{A}&		\boldsymbol{B}\\
\end{matrix} \right) \leqslant slant \mathrm{r}\left( \boldsymbol{A} \right) +\mathrm{r}\left( \boldsymbol{B} \right) \)；

\item \label{矩阵秩的基本公式7}\(\mathrm{r}(\boldsymbol{A}-\boldsymbol{B})\geqslant |\mathrm{r}(\boldsymbol{A})-\mathrm{r}(\boldsymbol{B})|\).
\end{enumerate}
\end{proposition}
\begin{proof}
\begin{enumerate}[(1)]
...
\boldsymbol{A}&		\boldsymbol{O}\\
\boldsymbol{O}&		\boldsymbol{B}\\
\end{matrix} \right) \right) \leqslant slant \mathrm{r}\left( \begin{matrix}
\boldsymbol{A}&		\boldsymbol{O}\\
\boldsymbol{O}&		\boldsymbol{B}\\
...
\boldsymbol{I}\\
\boldsymbol{I}\\
\end{array} \right) \right) \leqslant slant \mathrm{r}\left( \begin{matrix}
\boldsymbol{A}&		\boldsymbol{O}\\
\boldsymbol{O}&		\boldsymbol{B}\\
\end{matrix} \right) =\mathrm{r}\left( \boldsymbol{A} \right) +\mathrm{r}\left( \boldsymbol{B} \right) .
\end{align*}
...
\\
&\leqslant slant \mathrm{dim}\left( \mathrm{Ker}\theta \cap \mathrm{Im}\psi \right) =\mathrm{dim}\left( \mathrm{Im}\psi \right) -\mathrm{dim}\left( \mathrm{Im}\theta \psi \right) .
\end{align*}
于是
\begin{gather*}
\mathrm{dim}\left( \mathrm{Im}\psi \varphi \right) -\mathrm{dim}\left( \mathrm{Im}\theta \psi \varphi \right) \leqslant slant \mathrm{dim}\left( \mathrm{Im}\psi \right) -\mathrm{dim}\left( \mathrm{Im}\theta \psi \right) 
\\
...
\sum_{i=1}^n{a_{i}^{2}}-1&		\sum_{i=1}^n{a_i}\\
\sum_{i=1}^n{a_i}&		n-1\\
\end{matrix} \right) \geqslant slant 1$,
于是\(\mathrm{r}\left( \boldsymbol{M} \right) =n-2+\mathrm{r}\left( \begin{matrix}
\sum_{i=1}^n{a_{i}^{2}}-1&		\sum_{i=1}^n{a_i}\\
\sum_{i=1}^n{a_i}&		n-1\\
\end{matrix} \right) \geqslant slant n-1.\),等号成立当且仅当\(M\)不满秩,即\(|M| = 0\).
\end{proof}
