...
\[
\mathrm{r}\left( \boldsymbol{A}+\boldsymbol{B} \right) \leqslant \mathrm{r}\left( \boldsymbol{A}\,\mid \,\boldsymbol{B} \right) \leqslant \mathrm{r}\left( \boldsymbol{A} \right) +\mathrm{r}\left( \boldsymbol{B} \right) -\mathrm{r}\left( \boldsymbol{AB} \right) .
\]
\end{proposition}
\begin{proof}
由\rrefpro{proposition:矩阵秩的基本公式}{矩阵秩的基本公式5}知左起第一个不等号恒成立.将$\boldsymbol{A},\boldsymbol{B}$分别看作在$\mathbb{R} ^n$上左乘诱导的线性变换,则由\refcor{corollary:推论rA,,B=dimImA+ImB}可知
\begin{align}
...
\vdots\\
0\\
\end{array} \right) \Longleftrightarrow a_{il}=0,\,i=1,2,\cdots ,n\Longleftarrow a_{ll}=0.
\end{align*}
