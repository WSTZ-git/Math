...
\item 由于\(\mathrm{r}(\boldsymbol{A}-\boldsymbol{B})=\mathrm{r}(\boldsymbol{B}-\boldsymbol{A})\),故不妨设\(\mathrm{r}(\boldsymbol{A})\geqslant \mathrm{r}(\boldsymbol{B})\),则由(5)可得\(\mathrm{r}(\boldsymbol{A}-\boldsymbol{B})+\mathrm{r}(\boldsymbol{B})\geqslant \mathrm{r}(\boldsymbol{A}-\boldsymbol{B}+\boldsymbol{B})=\mathrm{r}(\boldsymbol{A})\),即\(\mathrm{r}(\boldsymbol{A}-\boldsymbol{B})\geqslant \mathrm{r}(\boldsymbol{A})-\mathrm{r}(\boldsymbol{B})\).
...
\]
等号成立当且仅当$\boldsymbol{X}\boldsymbol{A}+\boldsymbol{B}\boldsymbol{Y}=\boldsymbol{O}$有解.
\end{proposition}
\begin{proof}
{\color{blue}证法一:}
考虑下列矩阵的分块初等变换:
...
取等条件由\hyperref[theorem:Rot消去定理]{Rot消去定理}立得.
\end{proof}
...
$$\mathbb{r}\left( \boldsymbol{A} \right) +\mathbb{r}\left( \boldsymbol{B} \right) \geqslant \mathbb{r}\left( \boldsymbol{A}+\boldsymbol{B} \right) +\mathbb{r}\left( \boldsymbol{AB} \right) .$$
\end{example}
\begin{proof}
...
\begin{align*}
&\mathbb{r}\left( \boldsymbol{A} \right) +\mathbb{r}\left( \boldsymbol{B} \right) =\mathbb{r}\left( \begin{matrix}
\boldsymbol{A}&		\boldsymbol{O}\\
\boldsymbol{O}&		\boldsymbol{B}\\
\end{matrix} \right) \geqslant \mathbb{r}\left( \begin{matrix}
\boldsymbol{A}+\boldsymbol{B}&		\boldsymbol{O}\\
\boldsymbol{B}&		-2025\boldsymbol{B}\\
\end{matrix} \right) 
\\
&\geqslant \mathbb{r}\left( \boldsymbol{A}+\boldsymbol{B} \right) +\mathrm{r}\left( -2025\boldsymbol{B} \right) =\mathbb{r}\left( \boldsymbol{A}+\boldsymbol{B} \right) +\mathrm{r}\left( \boldsymbol{B} \right) 
\\
&\geqslant \mathbb{r}\left( \boldsymbol{A}+\boldsymbol{B} \right) +\mathrm{r}\left( \boldsymbol{AB} \right) .
\end{align*}
...
\end{align}
若$x_i$中只有一个元素非零，不妨设$x_1\ne 0$，则当$i,j\ne 1$时，有
\begin{align*}
0=a_{ii}a_{jj}x_ix_j=\left( \sum\limits_{k\ne i} a_{ik}x_k \right)\left( \sum\limits_{k\ne j} a_{jk}x_k \right) >0,
\end{align*}
显然矛盾！因此$x_i$中至少有两个元素非零，取$|x_{i_0}|=\max\limits_{i=1,2,\cdots,n}|x_i|>0$，$|x_{j_0}|=\max\limits_{i\in\{1,2,\cdots,n\}\backslash\{i_0\}}|x_i|>0$，则由\eqref{eq::::---823hfheofgu3894f34gxzz1}式得
\begin{align*}
