...
\begin{lemma}[源泉定理]\label{lemma:源泉定理}
...
\begin{theorem}[替换定理]\label{theorem:替换定理}
设 \(\alpha_1, \alpha_2, \cdots, \alpha_r\) 线性无关, 且可以被 \(\beta_1, \beta_2, \cdots, \beta_n\) 线性表出, 则可以从 \(\beta_1, \beta_2, \cdots, \beta_n\) 中选出 \(r\) 个向量替换成 \(\alpha_1, \alpha_2, \cdots, \alpha_r\) 后, 得到的新向量组与 \(\beta_1, \beta_2, \cdots, \beta_n\) 等价.
\end{theorem}
\begin{solution}
对 \(r\) 用数学归纳法.
当 \(r=1\) 时, 由于 \(\alpha_1\) 线性无关, 所以 \(\alpha_1 \neq 0\). 已知 \(\alpha_1\) 可以被 \(\beta_1, \beta_2, \cdots, \beta_n\) 线性表出, 设为
\begin{align*}
\alpha_1 = k_1\beta_1 + k_2\beta_2 + \cdots + k_n\beta_n.
\end{align*}
由于 \(\alpha_1 \neq 0\), 所以至少存在一个 \(i\) (\(i=1, 2, \cdots, n\)) 使得 \(k_i \neq 0\), 不妨设 \(k_1 \neq 0\), 那么
\begin{align*}
\beta_1 = \frac{1}{k_1}(\alpha_1 - k_2\beta_2 - \cdots - k_n\beta_n).
\end{align*}
这说明 \(\beta_1\) 可以由 \(\alpha_1, \beta_2, \cdots, \beta_n\) 线性表出, 即 \(\beta_1, \beta_2, \cdots, \beta_n\) 与 \(\alpha_1, \beta_2, \cdots, \beta_n\) 是等价的, 故 \(r=1\) 时命题成立.
假设当 \(r-1\) 时命题成立, 下面证明当 \(r\) 时命题也成立.
由归纳假设可知, 可以从 \(\beta_1, \beta_2, \cdots, \beta_n\) 中选出 \(r-1\) 个向量替换成 \(\alpha_1, \alpha_2, \cdots, \alpha_{r-1}\). 由于向量组的顺序对等价性无关, 不妨设这 \(r-1\) 个被替换的向量为 \(\beta_1, \beta_2, \cdots, \beta_{r-1}\), 于是新向量组 \(\{\alpha_1, \alpha_2, \cdots, \alpha_{r-1}, \beta_r, \cdots, \beta_n\}\) 与 \(\{\beta_1, \beta_2, \cdots, \beta_n\}\) 等价.
因为 \(\alpha_1, \alpha_2, \cdots, \alpha_r\) 线性无关, 所以 \(\alpha_r\) 不能由 \(\alpha_1, \alpha_2, \cdots, \alpha_{r-1}\) 线性表出.
由已知, \(\alpha_r\) 可以被 \(\beta_1, \beta_2, \cdots, \beta_n\) 线性表出, 由于上述两个向量组等价, 所以 \(\alpha_r\) 也可以被 \(\alpha_1, \alpha_2, \cdots, \alpha_{r-1}, \beta_r, \cdots, \beta_n\) 线性表出, 设为
\begin{align*}
\alpha_r = l_1\alpha_1 + l_2\alpha_2 + \cdots + l_{r-1}\alpha_{r-1} + l_r\beta_r + l_{r+1}\beta_{r+1} + \cdots + l_n\beta_n.
\end{align*}
因为 \(\alpha_r\) 不能由 \(\alpha_1, \alpha_2, \cdots, \alpha_{r-1}\) 线性表出, 所以 \(l_r, l_{r+1}, \cdots, l_n\) 中至少有一个不为 0, 不妨设 \(l_r \neq 0\), 则
\begin{align*}
\beta_r = \frac{1}{l_r}(\alpha_r - l_1\alpha_1 - l_2\alpha_2 - \cdots - l_{r-1}\alpha_{r-1} - l_{r+1}\beta_{r+1} - \cdots - l_n\beta_n).
\end{align*}
上式说明 \(\beta_r\) 可由 \(\alpha_1, \alpha_2, \cdots, \alpha_r, \beta_{r+1}, \cdots, \beta_n\) 线性表出, 从而向量组 \(\{\alpha_1, \alpha_2, \cdots, \alpha_r, \beta_{r+1}, \cdots, \beta_n\}\) 与 \(\{\alpha_1, \alpha_2, \cdots, \alpha_{r-1}, \beta_r, \cdots, \beta_n\}\) 等价. 由等价的传递性, 得到 \(\{\alpha_1, \alpha_2, \cdots, \alpha_r, \beta_{r+1}, \cdots, \beta_n\}\) 与 \(\{\beta_1, \beta_2, \cdots, \beta_n\}\) 等价. 故 \(r\) 时命题成立.
综上所述, 对任意的正整数 \(r\) 命题都成立.
\end{solution}
...
\end{remark}
\begin{proof}
不失一般性,可设\(\boldsymbol{\alpha}_1,\boldsymbol{\alpha}_2,\cdots,\boldsymbol{\alpha}_r\)是向量组\(\boldsymbol{\alpha}_1,\boldsymbol{\alpha}_2,\cdots,\boldsymbol{\alpha}_m\)的极大无关组,\(\boldsymbol{\beta}_1,\boldsymbol{\beta}_2,\cdots,\boldsymbol{\beta}_s\)是向量组\(\boldsymbol{\beta}_1,\boldsymbol{\beta}_2,\cdots,\boldsymbol{\beta}_k\)的极大无关组. 因为\(\boldsymbol{\alpha}_1,\boldsymbol{\alpha}_2,\cdots,\boldsymbol{\alpha}_m\)可用\(\boldsymbol{\alpha}_1,\boldsymbol{\alpha}_2,\cdots,\boldsymbol{\alpha}_r\)线性表出,所以\(\boldsymbol{\beta}_1,\boldsymbol{\beta}_2,\cdots,\boldsymbol{\beta}_s\)也可用\(\boldsymbol{\alpha}_1,\boldsymbol{\alpha}_2,\cdots,\boldsymbol{\alpha}_r\)线性表出,从而由\hyperref[lemma:源泉定理]{定理\ref{lemma:源泉定理}(1)}可知\(s\leqslant  r\),结论成立.
\end{proof}
...
\end{proposition}
\begin{proof}
(1) 设\(\boldsymbol{\alpha}_{i_1},\boldsymbol{\alpha}_{i_2},\cdots,\boldsymbol{\alpha}_{i_r}\)线性无关,又设\(\boldsymbol{\alpha}_{j_1},\boldsymbol{\alpha}_{j_2},\cdots,\boldsymbol{\alpha}_{j_r}\)是向量组的极大无关组. 对任意的\(1\leqslant  i\leqslant  m,\)$\boldsymbol{\alpha}_{i_1},$ $\boldsymbol{\alpha}_{i_2},$ $\cdots,$$\boldsymbol{\alpha}_{i_r},$$\boldsymbol{\alpha}_i$均可由\(\boldsymbol{\alpha}_{j_1},\boldsymbol{\alpha}_{j_2},\cdots,\boldsymbol{\alpha}_{j_r}\)线性表示,由\hyperref[lemma:源泉定理]{定理\ref{lemma:源泉定理}(2)}可知\(\boldsymbol{\alpha}_{i_1},\boldsymbol{\alpha}_{i_2},\cdots,\boldsymbol{\alpha}_{i_r},\boldsymbol{\alpha}_i\)必线性相关. 再由\hyperref[proposition:线性无关向量组的命题1]{命题\ref{proposition:线性无关向量组的命题1}}可知\(\boldsymbol{\alpha}_i\)可由\(\boldsymbol{\alpha}_{i_1},\boldsymbol{\alpha}_{i_2},\cdots,\boldsymbol{\alpha}_{i_r}\)线性表示,从而\(\boldsymbol{\alpha}_{i_1},\boldsymbol{\alpha}_{i_2},\cdots,\boldsymbol{\alpha}_{i_r}\)也是向量组的极大无关组.
...
\end{note}
\begin{proof}
必要性由向量组等价的定义和\hyperref[proposition:表出向量组的秩不超过原向量组的秩]{命题\ref{proposition:表出向量组的秩不超过原向量组的秩}}即得,下证充分性. 假设向量组\(A\)可用向量组\(B\)线性表示,且它们的秩都等于\(r\). 不失一般性,设\(\boldsymbol{\alpha}_1,\boldsymbol{\alpha}_2,\cdots,\boldsymbol{\alpha}_r\)是向量组\(A\)的极大无关组,\(\boldsymbol{\beta}_1,\boldsymbol{\beta}_2,\cdots,\boldsymbol{\beta}_r\)是向量组\(B\)的极大无关组. 考虑向量组\(C = \{\boldsymbol{\alpha}_1,\boldsymbol{\alpha}_2,\cdots,\boldsymbol{\alpha}_r,\boldsymbol{\beta}_1,\boldsymbol{\beta}_2,\cdots,\boldsymbol{\beta}_r\}\). 因为\(\boldsymbol{\alpha}_1,\boldsymbol{\alpha}_2,\cdots,\boldsymbol{\alpha}_r\)可用\(\boldsymbol{\beta}_1,\boldsymbol{\beta}_2,\cdots,\boldsymbol{\beta}_r\)线性表示,故\(\boldsymbol{\beta}_1,\boldsymbol{\beta}_2,\cdots,\boldsymbol{\beta}_r\)是向量组\(C\)的极大无关组,从而向量组\(C\)的秩等于\(r\). 又因为\(\boldsymbol{\alpha}_1,\boldsymbol{\alpha}_2,\cdots,\boldsymbol{\alpha}_r\)线性无关,故由\hyperref[proposition:表出向量组的秩不超过原向量组的秩]{命题\ref{proposition:表出向量组的秩不超过原向量组的秩}(1)}可知,\(\boldsymbol{\alpha}_1,\boldsymbol{\alpha}_2,\cdots,\boldsymbol{\alpha}_r\)也是向量组\(C\)的极大无关组,从而\(\boldsymbol{\beta}_1,\boldsymbol{\beta}_2,\cdots,\boldsymbol{\beta}_r\)可用\(\boldsymbol{\alpha}_1,\boldsymbol{\alpha}_2,\cdots,\boldsymbol{\alpha}_r\)线性表示,于是向量组\(B\)也可用向量组\(A\)线性表示. 因此,向量组\(A\)与向量组\(B\)等价. \end{proof}
...
\end{align*}
故$\mathrm{r}( C )=0$，因此$C=O$，即$\{\boldsymbol{\alpha }_i\cdot \boldsymbol{\beta }_{j}^{\prime}\mid 1\leqslant i\leqslant p,1\leqslant j\leqslant q\}$线性无关。\end{proof}

\begin{example}
设 $A, B, C$ 是二阶复方阵，且 $A, B, C$ 在 $M_2(\mathbb{C})$ 中线性无关。证明：存在复数 $x_1, x_2, x_3$ 使得 $x_1A + x_2B + x_3C$ 是可逆矩阵。
\end{example}
\begin{proof}
设
\begin{align*}
A = \begin{pmatrix} a_1 & a_2 \\ a_3 & a_4 \end{pmatrix}, \quad 
B = \begin{pmatrix} b_1 & b_2 \\ b_3 & b_4 \end{pmatrix}, \quad 
C = \begin{pmatrix} c_1 & c_2 \\ c_3 & c_4 \end{pmatrix}.
\end{align*}
由于 $A, B, C$ 在 $M_2(\mathbb{C})$ 中线性无关，所以将 $A, B, C$ 的元素排为一列，可知矩阵
\begin{align*}
\begin{pmatrix} a_1 & b_1 & c_1 \\ a_2 & b_2 & c_2 \\ a_3 & b_3 & c_3 \\ a_4 & b_4 & c_4 \end{pmatrix}
\end{align*}
的秩为3.这里不妨设前三个行向量线性无关，即有
\begin{align}
\begin{pmatrix} a_1 & b_1 & c_1 \\ a_2 & b_2 & c_2 \\ a_3 & b_3 & c_3 \end{pmatrix} \text{ 为可逆矩阵.} \label{eq:abc123}
\end{align}
...
\begin{align*}
\begin{cases} 
a_1x_1 + b_1x_2 + c_1x_3 = 0, \\ 
a_2x_1 + b_2x_2 + c_2x_3 = 1, \\ 
a_3x_1 + b_3x_2 + c_3x_3 = 1.
\end{cases}.
\end{align*}
其系数矩阵为 \eqref{eq:abc123}，这是一个可逆矩阵，所以上述方程组存在唯一解，不妨记为 $(\overline{x}_1, \overline{x}_2, \overline{x}_3)$，此时就有
\begin{align*}
|\overline{x}_1A + \overline{x}_2B + \overline{x}_3C| = \begin{vmatrix} 0 & 1 \\ 1 & * \end{vmatrix} = -1.
\end{align*}
从而 $\overline{x}_1A + \overline{x}_2B + \overline{x}_3C$ 为可逆矩阵。\end{proof}

\begin{example}
设 \(V\) 是实线性空间 \(M_n(\mathbb{R})\) 的线性子空间, 假设 \(V\) 中任意非零矩阵都是可逆矩阵, 证明: \(V\) 的维数不超过 \(n\).
\end{example}
\begin{solution}
对于 \(V\) 中的任意 \(n+1\) 个矩阵 \(A_1, A_2, \cdots, A_{n+1}\), 设它们的第 1 列分别为 \(\alpha_1, \alpha_2, \cdots, \alpha_{n+1}\), 由于这是 \(n+1\) 个 \(n\) 维实列向量, 所以它们线性相关, 即存在不全为零的实数 \(k_1, k_2, \cdots, k_{n+1}\) 使得
\begin{align*}
k_1 \alpha_1 + k_2 \alpha_2 + \cdots + k_{n+1} \alpha_{n+1} = 0,
\end{align*}
即 \(k_1 A_1 + k_2 A_2 + \cdots + k_{n+1} A_{n+1}\) 的第 1 列就是零向量, 于是 \(k_1 A_1 + k_2 A_2 + \cdots + k_{n+1} A_{n+1}\) 不可逆, 而 \(V\) 中不可逆的矩阵仅有零矩阵, 所以
\begin{align*}
k_1 A_1 + k_2 A_2 + \cdots + k_{n+1} A_{n+1} = O,
\end{align*}即 \(A_1, A_2, \cdots, A_{n+1}\) 线性相关. 这说明 \(V\) 的维数不超过 \(n\).
\end{solution}

\begin{example}
如果 \(n\) 阶方阵 \(A_1, A_2, \cdots, A_m\) 满足 \(A_i^2 \neq O \ (i = 1, 2, \cdots, m)\), 且当 \(i \neq j\) 时, \(A_i A_j = O\), 证明 \(m \leqslant n\).
\end{example}
\begin{solution}
对任意的 \(i = 1, 2, \cdots, m\), 由于 \(A_i^2 \neq O\), 所以存在 \(n\) 维列向量 \(\alpha_i\) 使得 \(A_i^2 \alpha_i \neq 0\), 而若
\begin{align*}
k_1 A_1 \alpha_1 + k_2 A_2 \alpha_2 + \cdots + k_m A_m \alpha_m = 0.
\end{align*}上式两边同时左乘 \(A_i \ (i = 1, 2, \cdots, m)\) 可得 \(k_i A_i^2 \alpha_i = 0\), 从而 \(k_i = 0\), 这说明 \(A_1 \alpha_1, A_2 \alpha_2, \cdots, A_m \alpha_m\) 这 \(m\) 个 \(n\) 维列向量线性无关, 进而 \(m \leqslant n\). 
\end{solution}
