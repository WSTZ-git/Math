...
设$A$为$n$阶复矩阵,其全体特征值为$\lambda_1,\lambda_2,\cdots,\lambda_r$,并且$\lambda_i\left( 1\leqslant i\leqslant r \right)$的代数重数为$n_i$,则$\sum_{i=1}^{r}n_i=n$. 若$A$可对角化,则$A$一定相似于$\mathrm{diag}\left\{ A_1,A_2,\cdots ,A_r \right\}$,其中$A_i=\mathrm{diag}\left\{ \lambda _i,\lambda _i,\cdots ,\lambda _i \right\} \left( 1\leqslant i\leqslant r \right)$并且阶数为$n_i$.
\end{theorem}
\begin{proof}
由于$A$可对角化,因此其特征值的代数重数等于几何重数.记$V_i$为$\lambda_i$的特征子空间,则任取$V_i$中一组基$\{e_i1,e_i2,\cdots,e_{i,n_i}\}$.由\hyperref[theorem:可对角化的判定条件]{可对角化的判定条件(3)}及直和的等价条件可知,$\{e_i1,e_i2,\cdots,e_{i,n_i}\}\left( 1\leqslant i\leqslant r \right)$可以拼成$\mathbb{C}^n$的一组基.于是记$P=(e_{11},\cdots,e_{1,n_1},\cdots,e_{r1},\cdots,e_{r,n_r})$,则$P$可逆,并且
\begin{align*}
...
\item 证明见\refthe{theorem:线性变换可对角化的充要条件2}.

\item 证明见\hyperref[theorem:线性变换可对角化的充要条件3]{定理\ref{theorem:线性变换可对角化的充要条件3}}.

\item 由\refpro{proposition:极小多项式对应Jordan标准型的最大块}知,$\boldsymbol{A}$的极小多项式无重根等价于$\boldsymbol{A}$的Jordan块都是一阶的.因此(5)$\iff$(6).

\item 由\refthe{theorem:Jordan标准型}立得.

\item 由\refthe{theorem:实对称和Hermite矩阵的正交对角化}和\refthe{theorem:复正规矩阵等价于酉相似于对角阵的矩阵}立得.
\end{enumerate}
...
\end{proposition}
\begin{note}
这个命题实际上就是\hyperref[proposition:循环矩阵的特征值和对角化]{命题\ref{proposition:循环矩阵的特征值和对角化}}.
\end{note}
\begin{proof}
...
\begin{example}\label{example:例题6.3548561684-(2)}
设 $V$ 为 $n$ 阶矩阵全体构成的线性空间，$V$ 上的线性变换 $\varphi$ 定义为 $\varphi(X)=AXA$，其中 $A\in V$。证明：$\varphi$ 可对角化的充要条件是 $A$ 可对角化。
\end{example}
\begin{proof}

{\heiti 充分性:}{\color{blue}证法一:}
...
\end{align*}
由\hyperref[theorem:矩阵的Kronecker积的基本性质]{矩阵的Kronecker积的基本性质}可知$\varLambda\otimes\varLambda$为对角矩阵，即 $A\otimes A'$ 可对角化，从而 $\varphi$ 可对角化. 

{\heiti 必要性:}用反证法，设 $A$ 不可对角化，则存在可逆矩阵 $P,Q$，使得
\begin{align*}
P^{-1}AP = Q^{-1}A'Q = J = \mathrm{diag}\{J_{r_1}(\lambda_1),\cdots,J_{r_k}(\lambda_k)\}
\end{align*}
为 Jordan 标准型，其中 $r_1 > 1$。设 $P = (\alpha_1,\alpha_2,\cdots,\alpha_n),Q = (\beta_1,\beta_2,\cdots,\beta_n)$ 分别为两个矩阵的列分块，则
\begin{align*}
&A\left( \alpha _1,\cdots ,\alpha _{r_1},\cdots ,\alpha _{n-r_k+1},\cdots ,\alpha _n \right) =\left( \alpha _1,\cdots ,\alpha _{r_1},\cdots ,\alpha _{n-r_k+1},\cdots ,\alpha _n \right) J
\\
&=\left( \lambda _1\alpha _1,\cdots ,\alpha _{r_1-1}+\lambda \alpha _{r_1},\cdots ,\lambda _k\alpha _{n-r_k+1},\cdots ,\alpha _{n-1}+\lambda _k\alpha _n \right) ;
\end{align*}
\begin{align*}
&A'\left( \beta _1,\cdots ,\beta _{r_1},\cdots ,\beta _{n-r_k+1},\cdots ,\beta _n \right) =\left( \beta _1,\cdots ,\beta _{r_1},\cdots ,\beta _{n-r_k+1},\cdots ,\beta _n \right) J
\\
&=\left( \lambda _1\beta _1,\cdots ,\beta _{r_1-1}+\lambda \beta _{r_1},\cdots ,\lambda _k\beta _{n-r_k+1},\cdots ,\beta _{n-1}+\lambda _k\beta _n \right) .
\end{align*}
令 $U = L(\alpha_i\beta_j',1\leqslant  i,j\leqslant  r_1)$，则由\refpro{proposition:线性无关的向量组与另一个转置的乘积积生成的矩阵也线性无关}可知 $\{\alpha_i\beta_j',1\leqslant  i,j\leqslant  r_1\}$ 是 $U$ 的一组基。经简单计算可得
\begin{gather}\label{equation-----::-:-:7.8}
\begin{aligned}
&\varphi (\alpha _1\beta _1')=\left( A\alpha _1 \right) \left( A'\beta _1 \right) '=\lambda _{1}^{2}\alpha _1\beta _1';
\\
&\varphi (\alpha _1\beta _j')=\left( A\alpha _1 \right) \left( A'\beta _j \right) '=\lambda _1\alpha _1\beta _{j-1}'+\lambda _{1}^{2}\alpha _1\beta _j', 2\leqslant j\leqslant r_1;
\\
&\varphi (\alpha _i\beta _1')=\left( A\alpha _i \right) \left( A'\beta _1 \right) '=\lambda _1\alpha _{i-1}\beta _1'+\lambda _{1}^{2}\alpha _i\beta _1', 2\leqslant i\leqslant r_1;
\\
&\varphi (\alpha _i\beta _j')=\left( A\alpha _i \right) \left( A'\beta _j \right) '=\alpha _{i-1}\beta _{j-1}'+\lambda _1\alpha _{i-1}\beta _j'+\lambda _1\alpha _i\beta _{j-1}'+\lambda _{1}^{2}\alpha _i\beta _j', 2\leqslant i,j\leqslant r_1,
\end{aligned}
\end{gather}
于是 $U$ 是 $\varphi$-不变子空间。由于 $\varphi$ 可对角化，故由\refpro{proposition:可对角化的线性变换其限制变换和诱导变换也可对角化}可知 $\varphi|_U$ 也可对角化，但\eqref{equation-----::-:-:7.8}式告诉我们 $\varphi|_U$ 在基 $$\{\alpha_1\beta_1',\cdots,\alpha_1\beta_{r_1}',\cdots,\alpha_{r_1}\beta_1',\cdots,\alpha_{r_1}\beta_{r_1}'\}$$ 下的表示矩阵是一个上三角矩阵，主对角元全为 $\lambda_1^2$，且不是纯量阵，由\refpro{proposition-0.10}可知这个矩阵不可对角化，矛盾！
\end{proof}

\begin{example}
设 $V$ 为 $n$ 阶矩阵全体构成的线性空间，$V$ 上的线性变换 $\varphi$ 定义为 $\varphi(X)=AX - XA$，其中 $A\in V$。证明：$\varphi$ 可对角化的充要条件是 $A$ 可对角化。
\end{example}
\begin{proof}
...
{\heiti 必要性:}用反证法，设 $A$ 不可对角化，则存在可逆矩阵 $P$，使得 $P^{-1}AP = J = \mathrm{diag}\{J_{r_1}(\lambda_1),\cdots,J_{r_k}(\lambda_k)\}$ 为 Jordan 标准型，其中 $r_1 > 1$。设 $P = (\alpha_1,\alpha_2,\cdots,\alpha_n)$ 为列分块，任取 $A'$ 的特征值 $\lambda_0$ 及其特征向量 $\beta$，即 $A'\beta = \lambda_0\beta$。令 $U = L(\alpha_i\beta',1\leqslant  i\leqslant  r_1)$，则由\refpro{proposition:线性无关的向量组与另一个转置的乘积积生成的矩阵也线性无关}可知 $\{\alpha_i\beta',1\leqslant  i\leqslant  r_1\}$ 是 $U$ 的一组基。经简单计算可得
\begin{align}\label{equation::----:7.9}
&\varphi (\alpha _1\beta ' )=A\alpha _1\beta ' -\alpha _1\beta ' A=\left( A\alpha _1 \right) \beta ' -\alpha _1\left( A' \beta \right) ' =(\lambda _1-\lambda _0)\alpha _1\beta ' ,
\nonumber \\
&\varphi (\alpha _2\beta ' )=A\alpha _2\beta ' -\alpha _2\beta ' A=\left( A\alpha _2 \right) \beta ' -\alpha _2\left( A' \beta \right) ' =\alpha _1\beta ' +(\lambda _1-\lambda _0)\alpha _2\beta ' ,
\nonumber \\
&\cdots \cdots \cdots \cdots \cdots \cdots \cdots \cdots 
\nonumber \\
&\varphi (\alpha _{r_1}\beta ' )=A\alpha _{r_1}\beta ' -\alpha _{r_1}\beta ' A=\left( A\alpha _{r_1} \right) \beta ' -\alpha _{r_1}\left( A' \beta \right) ' =\alpha _{r_1-1}\beta ' +(\lambda _1-\lambda _0)\alpha _{r_1}\beta ' ,
\end{align}
于是 $U$ 是 $\varphi$-不变子空间。由于 $\varphi$ 可对角化，故由\refpro{proposition:可对角化的线性变换其限制变换和诱导变换也可对角化}可知 $\varphi|_U$ 也可对角化，但由\eqref{equation::----:7.9}式可知$\varphi|_U$ 在基 $\{\alpha_i\beta',1\leqslant  i\leqslant  r_1\}$ 下的表示矩阵为 $J_{r_1}(\lambda_1 - \lambda_0)$，由\refpro{proposition-0.10}可知这个矩阵不可对角化，矛盾！
\end{proof}
...
{\color{blue}证法二:}
由\refthe{theorem:F(A)=C(A)的充要条件和不变子空间刻画}和\refpro{proposition:矩阵有不同特征值则特征多项式和极小多项式一定相等}可知,$\mathbb{C}^n$是关于$\mathbf{A}$的循环空间,再由\hyperref[theorem:F(A)=C(A)的充要条件和不变子空间刻画]{定理\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画}\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画-8}\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画-9}}即得结论.
\end{proof}
...
先证充分性. 由于$A$有$n$个不同的特征值, 故$A$可对角化,从而由\hyperref[proposition:矩阵A可对角化则f(A)也可对角化]{命题\ref{proposition:矩阵A可对角化则f(A)也可对角化}}知$f(A)$也可对角化,又$B$相似于$f(A)$,于是$B$也可对角化. 
...
设$J=\begin{pmatrix}O&I_{n - 1}\\1&O\end{pmatrix}$,由\hyperref[proposition:循环矩阵的特征值和对角化]{循环矩阵的特征值和对角化}知,$J$有$n$个不同的特征值. 由\hyperref[proposition:有n个不同特征值的矩阵与另一个矩阵可对角化的关系]{命题\ref{proposition:有n个不同特征值的矩阵与另一个矩阵可对角化的关系}}知$B$可对角化的充要条件是存在次数不超过$n-1$的多项式$f(x)$,使得$B$相似于$f(J)$.

{\heiti 必要性:}设$B$相似于$f(J)$,其中$f$是次数不超过$n-1$的多项式.则由\hyperref[proposition:循环矩阵的性质]{循环矩阵的性质\ref{proposition:循环矩阵的性质-2}}可知$f(J)$是循环矩阵,故必要性得证.

{\heiti 充分性:}设$B$相似于某个循环矩阵$C$,由\hyperref[proposition:循环矩阵的性质]{循环矩阵的性质\ref{proposition:循环矩阵的性质-2}}可知,存在次数不超过$n - 1$的多项式$f(x)$,使得$C = f(J)$.再由一开始的结论知$B$可对角化.故充分性得证.
\end{proof}
...
\end{align*}
故由\nrefthe{theorem:直和的等价条件}{(2)}可知\eqref{proposition0.7-6.7}式是直和.
...
\end{pmatrix},
\end{align*}
由\hyperref[proposition:矩阵秩的基本公式-4]{proposition:矩阵秩的基本公式-(4)}可得
\begin{align*}
...
{\color{blue}证法一:}
由$|\boldsymbol{B}\boldsymbol{A}|\ne 0$知$n\leqslant m,$否则由\hyperref[theorem:Cauchy-Binet公式]{Cauchy-Binet公式}知$|\boldsymbol{B}\boldsymbol{A}|=0$矛盾!记其代数重数为\(m_{\boldsymbol{A}}(\lambda_0)\), 几何重数为\(t_{\boldsymbol{A}}(\lambda_0)\)(其他记号同理). 
由\hyperref[theorem:特征值的降价公式]{特征值的降价公式}可得\(|\lambda\boldsymbol{I}_m - \boldsymbol{A}\boldsymbol{B}| = \lambda^{m - n}|\lambda\boldsymbol{I}_n - \boldsymbol{B}\boldsymbol{A}|\), 因此\(\boldsymbol{A}\boldsymbol{B}\)与\(\boldsymbol{B}\boldsymbol{A}\)的非零特征值全相同(代数重数也相同). 由于\(\boldsymbol{B}\boldsymbol{A}\)非异, 故其特征值全部非零, 从而\(0\)作为\(\boldsymbol{A}\boldsymbol{B}\)的特征值, 其代数重数为\(m - n\). 另一方面, 我们有
\begin{align*}
...
\begin{proposition}\label{proposition-0.10}
求证:
\begin{enumerate}[(1)]
