...
\end{align*}
为 Jordan 标准型，其中 $r_1 > 1$。设 $P = (\alpha_1,\alpha_2,\cdots,\alpha_n),Q = (\beta_1,\beta_2,\cdots,\beta_n)$ 分别为两个矩阵的列分块，令 $U = L(\alpha_i\beta_j',1\leq i,j\leq r_1)$，则由\refproposition{proposition:线性无关的向量组与另一个转置的乘积积生成的矩阵也线性无关}可知 $\{\alpha_i\beta_j',1\leq i,j\leq r_1\}$ 是 $U$ 的一组基。经简单计算可得
...
\end{align}
于是 $U$ 是 $\varphi$-不变子空间。由于 $\varphi$ 可对角化，故由\refproposition{proposition:可对角化的线性变换其限制变换和诱导变换也可对角化}可知 $\varphi|_U$ 也可对角化，但\eqref{equation::----:7.9}式告诉我们 $\varphi|_U$ 在基 $\{\alpha_i\beta',1\leq i\leq r_1\}$ 下的表示矩阵为 $J_{r_1}(\lambda_1 - \lambda_0)$，这个矩阵不可对角化，矛盾！
\end{enumerate}
\end{proof}
...
{\color{blue}证法二:}
由\reftheorem{theorem:循环子空间的刻画}和\refproposition{proposition:矩阵有不同特征值则特征多项式和极小多项式一定相等}可知,$\mathbb{C}^n$是关于$\mathbb{A}$的循环空间,再由\reftheorem{theorem:极小多项式等于特征多项式的充要条件}即得结论.
\end{proof}
...
\end{proposition}
\begin{remark}
这个命题的几何版本(见\refproposition{proposition:可对角化的线性变换其限制变换和诱导变换也可对角化})是: 设\(\boldsymbol{\varphi}\)是复线性空间\(V\)上的线性变换, \(U\)是\(\boldsymbol{\varphi}\) -不变子空间, 若\(\boldsymbol{\varphi}\)可对角化, 则\(\boldsymbol{\varphi}\)在不变子空间\(U\)上的限制变换\(\boldsymbol{\varphi}|_U\)以及\(\boldsymbol{\varphi}\)在商空间\(V/U\)上的诱导变换\(\overline{\boldsymbol{\varphi}}\)均可对角化. 
\end{remark}
\begin{proof} 
