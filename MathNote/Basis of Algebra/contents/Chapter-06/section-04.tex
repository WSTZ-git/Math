...
$\mathbf{Step}\,\,\mathbf{2}$  取某个非数量矩阵 \( A_1 \), 于是有 \( V = \bigoplus_{i=1}^s V_i \), 这里 \( s \geq 2 \) 且 \( V_i \) 是属于 \( A_1 \) 不同特征值的特征子空间. 显然由交换性, 对每一个 \( i = 1,2,\cdots,s \), \( V_i \) 是所有 \( A_\lambda \) 不变子空间, 且 \( A_\lambda|_{V_i}, \lambda \in \Lambda \) 是一族两两可交换的矩阵, 由\reflem{lemma:可对角化矩阵限制后仍可对角化}知它们也是可对角化的. 注意到 \( 1 \leq \dim V_i < n, i = 1,2,\cdots,s \), 所以我们的维度降下去了, 因此可使用归纳法.

当 \( n = 1 \), 命题显然成立, 假设命题对 \( \leq n - 1 \) 都成立, 当 \( n \) 时, 由归纳假设, 对每一个 \( i = 1,2,\cdots,s \), 存在 \( V_i \) 的一个基使得 \( A_\lambda|_{V_i}, \lambda \in \Lambda \) 在这个基下表示矩阵是对角矩阵. 于是把这些基合起来构成一个新的基, 我们就得到在这个新的基下 \( A_\lambda, \lambda \in \Lambda \) 都是对角矩阵.
\end{proof}
