...
\begin{proposition}
\begin{enumerate}[(1)]
\item 设 \(A, B\) 是 \(n\) 阶复方阵, 复数 \(x\) 满足 \(x=1\) 或者 \(x^k \ne 1, \forall 1 \leqslant k \leqslant n.\) 若 \(AB=xBA,\) 证明: \(A, B\) 可以同时上三角化, 并且若 \(x\) 是单位根, 则结论可能不成立.
\item 对复数 \(a, b, c, d\) 以及 \(n\) 阶复方阵 \(A, B,\) 这里 \(a, b\) 不全为零, \(c, d\) 不全为零, 且有矩阵方程
\begin{align*}
aA^2+bB^2+cAB+dBA=0.
\end{align*}
...
注意对矩阵 \(A, B\) 做一些线性的替换, 之前与之后的相同上三角性质是等价的 (要证替换之前可以同时上三角, 就等价证明对于替换之后可以同时上三角), 所以先来考虑各种各样情况下, 最终都等价于什么问题.
