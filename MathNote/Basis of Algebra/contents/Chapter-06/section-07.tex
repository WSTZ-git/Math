...
\end{example}
\begin{note}
观察到$M$矩阵与循环矩阵由类似结构,回忆\refpro{proposition:循环矩阵的特征值和对角化}的证明过程,利用与\refpro{proposition:循环矩阵的特征值和对角化}相同的方法构造相似矩阵与对应的过渡矩阵.
\end{note}
\begin{proof}
注意到
\begin{align*}
...
\end{pmatrix}^{-1}.
\end{align*}
进而对$\forall i\in \{1,2,\cdots,k\}$，都有$c_iv_i\in L(\alpha_1,\alpha_2,\cdots,\alpha_k)$，又$c_i\ne 0$，故$v_i\in L(\alpha_1,\alpha_2,\cdots,\alpha_k)$。又因为$\alpha_i\in W\ (1\leqslant i\leqslant k)$，所以$v_i\in W$.
\end{proof}
