...
\item\label{proposition:极小多项式的性质-6} 设$A$是一个分块对角阵
\begin{align*}
A = \begin{pmatrix}
A_1 & & \\
& A_2 & \\
...
\item 设\(n\)阶幂零Jordan块为$A$,则由\hyperref[proposition:基础幂零Jordan块]{命题\ref{proposition:基础幂零Jordan块}}可知$A^k\ne O(k=1,2,\cdots,n-1)$,但$A^n=O$.故\(n\)阶幂零Jordan块$A$的极小多项式为$x^n$.
...
\begin{proposition}[矩阵的逆可由其多项式表示]\label{proposition:矩阵的逆可以用其多项式表示}
...
{\color{blue}证法二:}
若 $\mathrm{r}(A)=n$，则由 Cayley - Hamilton 定理易证结论成立(同证法一)；若 $\mathrm{r}(A)\leqslant  n - 2$，则 $A^* = O$，结论显然成立；若 $\mathrm{r}(A)=n - 1$，则 $A^*\neq O$ 且 $AA^* = A^*A = O$，由\refpro{proposition:与秩为n-1的矩阵乘法可交换的矩阵也可被其多项式表示}可知结论也成立。
\end{proof}
...
由 \hyperref[theorem:Cayley-Hamilton定理]{Cayley - Hamilton 定理}可得 $f(\varphi)=f_1(\varphi)f_2(\varphi)=0$，故由\hyperref[proposition:互素多项式诱导直和分解1]{命题\ref{proposition:互素多项式诱导直和分解1}}可知 $V = V_1\oplus V_2$.

\item 由 $f_1(\varphi)f_2(\varphi)=0$ 可得 $\mathrm{Im}\,f_2(\varphi)\subseteq\mathrm{Ker}\,f_1(\varphi)=V_1$，$\mathrm{Im}\,f_1(\varphi)\subseteq\mathrm{Ker}\,f_2(\varphi)=V_2$. 因为 $V = V_1\oplus V_2$，故由维数公式可得
\begin{align*}
...
\end{align}
注意到 $f_i(\varphi|_{V_i})=f_i(\varphi)|_{V_i}=0$，即 $\varphi|_{V_i}$ 适合多项式 $f_i(\lambda)$，因此 $\varphi|_{V_i}$ 的特征值也适合 $f_i(\lambda)$，即 $g_i(\lambda)$ 的根都是 $f_i(\lambda)$ 的根. 因为 $(f_1(\lambda),f_2(\lambda)) = 1$，故 $f_1(\lambda)$ 与 $f_2(\lambda)$ 没有公共根,从而$f_1$与$g_2$没有公共根,即$(f_1(\lambda),g_2(\lambda))=1.$由\eqref{example03003-6.8}式知$f_1(\lambda)\mid g_1(\lambda)g_2(\lambda)$,故$f_1(\lambda)\mid g_1(\lambda).$同理$f_2(\lambda)\mid g_2(\lambda).$

由$g_i(\lambda)$ 的根都是 $f_i(\lambda)$ 的根和$(f_1,f_2)=1$知$(g_1,g_2)=1$,从而同理可得$g_i\mid f_i$.
再由 $f_i(\lambda)$和$g_i(\lambda)$的首一性即得$f_1(\lambda)=g_1(\lambda)$，$f_2(\lambda)=g_2(\lambda)$. 
\end{enumerate}
\end{proof}

\begin{corollary}\label{corollary:矩阵适合多项式诱导的结论}
...
\end{enumerate}
特别地，如果$f(\lambda)=m(\lambda)$,其中$m(\lambda)$为$\varphi$的极小多项式,并且考虑极小多项式的首一互素因式分解
...
特别地,当$f(\lambda)=m(\lambda)$时,显然$\ker m_1\left( \varphi \right)$和$\ker m_2\left( \varphi \right)$都是$\varphi-$不变子空间.分别取$V_1=\ker m_1\left( \varphi \right)$和$V_2=\ker m_2\left( \varphi \right)$的一组基,拼成$V$的一组基,则由\refthe{theorem:在直和的基下的矩阵}知,$\varphi$在这组基下的表示阵可设为
\begin{align*}
A=\begin{pmatrix}
A_1&		\\
&		A_2
\end{pmatrix},
\end{align*}
其中$A_i$为$\varphi |_{V_i}$在$V_i$的一组基下的表示阵.设$\varphi |_{V_i}$的极小多项式为$n_i\left( \lambda \right)$,则$A_i$的极小多项式为$n_i\left( \lambda \right)$.由\hyperref[proposition:极小多项式的性质]{极小多项式的性质}知,$\varphi$的极小多项式等于$A$的极小多项式等于$\left[ n_1\left( \lambda \right) ,n_2\left( \lambda \right) \right] =m\left( \lambda \right)$.注意到$m_i\left( \varphi |_{V_i} \right) =m_i\left( \varphi \right) |_{V_i}=0$,即$\varphi |_{V_i}$适合多项式$m_i\left( \lambda \right)$.故$n_i\left( \lambda \right) |m_i\left( \lambda \right)$.又$\left( m_1\left( \lambda \right) ,m_2\left( \lambda \right) \right) =1$,故$\left( n_1\left( \lambda \right) ,n_2\left( \lambda \right) \right) =1$.因此
\begin{align*}
m\left( \lambda \right) =\left[ n_1\left( \lambda \right) ,n_2\left( \lambda \right) \right] =n_1\left( \lambda \right) n_2\left( \lambda \right) =m_1\left( \lambda \right) m_2\left( \lambda \right).
\end{align*}
从而$m_1\left( \lambda \right) |n_1\left( \lambda \right) n_2\left( \lambda \right)$.又因为$n_2\left( \lambda \right) |m_2\left( \lambda \right)$且$\left( m_1\left( \lambda \right) ,m_2\left( \lambda \right) \right) =1$所以$\left( m_1\left( \lambda \right) ,n_2\left( \lambda \right) \right) =1$.故$m_1\left( \lambda \right) |n_1\left( \lambda \right)$,同理$m_2\left( \lambda \right) |n_2\left( \lambda \right)$.再结合$n_i\left( \lambda \right) |m_i\left( \lambda \right)$和$m_i\left( \lambda \right),n_i\left( \lambda \right)$的首一性知$m_i\left( \lambda \right) =n_i\left( \lambda \right)$.
\end{proof}
...
\end{align*}
下面用归纳法证明: $p_k = (-1)^k\sigma_k (1\leqslant  k\leqslant  n)$. $p_1 = -\mathrm{tr}(\boldsymbol{A}) = -\sigma_1$，结论成立. 假设小于等于 $k$ 时结论成立，则由条件可知
\begin{align*}
\boldsymbol{A}_{k+1}&=\boldsymbol{A}\left( \boldsymbol{A}_k+p_k\boldsymbol{I}_n \right) =\boldsymbol{A}^2\left( \boldsymbol{A}_{k-1}+p_{k-1}\boldsymbol{I}_n \right) +p_k\boldsymbol{A}
\\
&=\cdots =\boldsymbol{A}^{k+1}+p_1\boldsymbol{A}^k+\cdots +p_k\boldsymbol{A}
\\
&=\boldsymbol{A}^{k+1}-\sigma _1\boldsymbol{A}^k+\cdots +(-1)^k\sigma _k\boldsymbol{A}.
\end{align*}
由Newton公式可得
\begin{align*}
