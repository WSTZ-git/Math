...
若$C=O$,则$A,B$乘法可交换,由\refpro{proposition:特征值全在同一数域的矩阵可上三角化}知结论成立.

若$C\ne 0$,当$n=1$时,结论显然成立.假设结论对阶数小于等于$n-1$时都成立.

记$C$的特征子空间为$V_0$,则$V_0\ne V$且$V_0\subset V$.由$AC=CA,BC=CB$及\refpro{proposition:特征子空间互为不变子空间}知,$V_0$是$A,C$的不变子空间.于是$A\mid_{V_0}^{},B\mid_{V_0}^{}$都是$V_0$上的线性变换,从而
\begin{align*}
