...
\begin{example}\label{example-6.12}
...
求证: $n$阶矩阵$A$是幂零矩阵的充要条件是$\operatorname{tr}(A^k) = 0 \ (1 \leqslant  k \leqslant  n)$.
\end{proposition}
\begin{proof}
设$A$的特征值为$\lambda_1, \lambda_2, \cdots, \lambda_n$. 

{\heiti 必要性:}若$A$是幂零矩阵, 则$A$的特征值全为零, 从而$\operatorname{tr}(A^k) = s_k = 0 \ (k \geqslant  1)$. 

{\heiti 充分性:}
{\color{blue}证法一:}
若$s_k = \operatorname{tr}(A^k) = 0 \ (1 \leqslant  k \leqslant  n)$, 则由Newton公式(白皮书这一部分还没看)或直接利用例 5.64(白皮书这一部分还没看)可计算出$\sigma_r = 0 \ (1 \leqslant  r \leqslant  n)$, 于是$A$的特征多项式为$\lambda^n$, 从而$A$的特征值全为零, 再由\hyperref[proposition:幂零矩阵关于特征值的充要条件]{幂零矩阵关于特征值的充要条件}可知$A$为零矩阵.

{\color{blue}证法二:}若$\operatorname{tr}(A^k) = 0 \ (1 \leqslant  k \leqslant  n)$,则
...
\end{align*}
由迹的线性和交换性可得 \(\operatorname{tr}(C^k) = 0 \ (k \geqslant  1)\), 再由\hyperref[proposition:幂零矩阵关于迹的充要条件]{幂零矩阵关于迹的充要条件}可知 \(C\) 为幂零矩阵, 从而 \(C\) 的特征值全为零。
...
\end{pmatrix}G^{-1}.
\end{align*}
显然$A(t)$的特征值分别为$\lambda_1 + c_1t, \cdots, \lambda_n + c_st$。再设$A$有$s(\leqslant slant n)$个互不相同的特征值，分别记为$\lambda_1', \cdots, \lambda_s'$，则$\lambda_i \in \{\lambda_1', \cdots, \lambda_s'\}, i = 1, 2, \cdots, n$。
...
\end{align*}
又因为 $\deg |\lambda_i f(I) - f(A)| \leqslant slant n$，所以存在 $c_A \ne 0$，使得
\begin{align}
...
\end{example}
\begin{proof}
设$A$的特征值分别为$\lambda_1,\cdots,\lambda_n$，$D_i:|z-a_{ii}|\leqslant slant \sum_{j\ne i}a_{ij}(1\leqslant slant i\leqslant slant n)$是$A$的$n$个戈氏圆盘。由\hyperref[theorem:第一圆盘定理]{第一圆盘定理}可知$\lambda_1,\cdots,\lambda_n$都落在$\bigcup_{i=1}^n D_i$中。对$\forall \lambda \in \bigcup_{i=1}^n D_i$，都存在$k\in \{1,2,\cdots,n\}$，使得$\lambda \in D_k$，从而
\begin{align*}
|\lambda|\leqslant slant |\lambda -a_{kk}|+a_{kk}\leqslant slant \sum_{j\ne k}a_{kj}+a_{kk}\leqslant slant 1.
\end{align*}
因此$|\lambda_1|,\cdots,|\lambda_n|\leqslant slant 1$，故$\det A\leqslant slant |\lambda_1\cdots \lambda_n|\leqslant slant 1$，当且仅当$A$的所有特征值模长为$1$等号成立。
\end{proof}
