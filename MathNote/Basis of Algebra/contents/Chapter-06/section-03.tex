...
\\
&\Rightarrow \frac{\lambda +b}{\lambda +a}=w_i\left( 1\leqslant i\leqslant n \right) \Rightarrow \lambda =\frac{a\omega _i-b}{1-\omega _i}(1\leqslant i\leqslant n).
\end{align*}
...
\begin{align*}
\sum_{1\leqslant i_1<i_2<\cdots <i_r\leqslant n}{\lambda _{i_1}\lambda _{i_2}}\cdots \lambda _{i_r}&=\left( -1 \right) ^ra_r=\left( -1 \right) ^r\left( -1 \right) ^r\sum_{1\leqslant i_1<i_2<\cdots <i_r\leqslant n}{A\left( \begin{matrix}
i_1&		i_2&		\cdots&		i_r\\
i_1&		i_2&		\cdots&		i_r\\
\end{matrix} \right)}
\\
&=\sum_{1\leqslant i_1<i_2<\cdots <i_r\leqslant n}{A\left( \begin{matrix}
i_1&		i_2&		\cdots&		i_r\\
i_1&		i_2&		\cdots&		i_r\\
\end{matrix} \right)}\quad ,1\leqslant r\leqslant n.
\end{align*}
因此第二种结论也成立.
...
\end{example}
\begin{proof}
设 $A$ 的特征值为 $\lambda_1, \lambda_2, \cdots, \lambda_n$, 由条件和\hyperref[proposition:特征值与特征多项式系数的关系]{特征值与特征多项式系数的关系}可知
\begin{align*}
...
\end{example}
\begin{proof}
设 $A$ 的特征值为 $\lambda_1, \lambda_2, \cdots, \lambda_n$, 由 $A$ 非异可知它们都是非零实数. 再由条件和例 6.24 可知
\begin{align}
...
DB - BD = AB - BA = C.
\end{align*}
由\refpro{proposition:矩阵换位子的幂恒等式}知对$\forall p\in \mathbb{N}$, 有
\begin{align*}
...
\end{corollary}
\begin{proof}
对阶数进行归纳. 由\hyperref[proposition:经典矩阵乘法可交换诱导的性质例题]{命题\ref{proposition:经典矩阵乘法可交换诱导的性质例题}证法二}可知, \(C\)的特征值全为\(0\).

若$C=O$,则$A,B$乘法可交换,由\cref{proposition:特征值全在同一数域的矩阵可上三角化}知结论成立.

若$C\ne 0$,当$n=1$时,结论显然成立.假设结论对阶数小于等于$n-1$时都成立.

记$C$的特征子空间为$V_0$,则$V_0\ne V$且$V_0\subset V$.由$AC=CA,BC=CB$及\cref{proposition:特征子空间互为不变子空间}知,$V_0$是$A,C$的不变子空间.于是$A\mid_{V_0}^{},B\mid_{V_0}^{}$都是$V_0$上的线性变换,从而
\begin{align*}
...
\begin{align*}
f(x) = |xI - A| = x^n - \mathrm{tr}(A)x^{n-1} + \cdots + (-1)^n|A| = x^n + p_1(a_{ij}, 1 \leqslant i, j \leqslant n)x^{n-1} + \cdots + p_n(a_{ij}, 1 \leqslant i, j \leqslant n)。
\end{align*}
其中$p_k$都是$n^2$元多项式。设$x_1, \cdots, x_n$是$f(x) = 0$的根，则根据Vieta定理可知，$f(x)$的判别式$\Delta = \prod_{1 \leqslant i < j \leqslant n} (x_i - x_j)^2$是一个关于$p_1(a_{ij}, 1 \leqslant i, j \leqslant n), \cdots, p_n(a_{ij}, 1 \leqslant i, j \leqslant n)$的多项式，记为$F(a_{ij}, 1 \leqslant i, j \leqslant n)$。从而此时$F(a_{ij}, 1 \leqslant i, j \leqslant n) = 0$。故$F(a_{ij}, 1 \leqslant i, j \leqslant n)$就为所求多项式.
...
取$\delta = \frac{1}{2}\min_{1 \leqslant i, j \leqslant s}\left\{\left|\frac{\lambda_j' - \lambda_i'}{c_i - c_j}\right|\right\}$，则对$\forall t \in (0, \delta)$，都有
\begin{align*}
