...
由条件可知
$$A^2+B^2=2AB+BA.$$
待定 $a,b\in \mathbb{R}$ ,使得 $ab=1$ ,这样就有
\begin{align*}
\left( A+aB \right) \left( A+bB \right) &=A^2+abB^2+bAB+aBA=A^2+B^2+bAB+aBA \\
&=2AB+BA+bAB+aBA=\left( 2+b \right) AB+\left( 1+a \right) BA.
\end{align*}
再要求 $\left( 2+b \right) AB+\left( 1+a \right) BA=k\left( AB-BA \right) ,k\in \mathbb{R}$ ,即
\begin{align*}
\left\{ \begin{array}{c}
2+b=k\\
1+a=-k\\
\end{array}\Longrightarrow \right. a+b=-3.
\end{align*}
由 $ab=1,a+b=-3$ 可知 $a,b$ 是方程 $x^2+3x+1=0$ 的根,故可取
$$a=\frac{-3-\sqrt{5}}{2},\quad b=\frac{-3+\sqrt{5}}{2}.$$
此时 $k=2+b=\frac{1+\sqrt{5}}{2}$ ,于是就有
$$\left( A-\frac{3+\sqrt{5}}{2}B \right) \left( A+\frac{\sqrt{5}-3}{2}B \right) =\frac{1+\sqrt{5}}{2}\left( AB-BA \right) .$$
从而
\begin{align}
...
\end{align*}
因为 $B$ 正定, 所以 $C^T B C$ 正定, 因此设 $C^T B C$ 的全部特征值为 $\lambda_1,\lambda_2,\dots,\lambda_n > 0$, 从而只需证明
\begin{align*}
...
上式等号成立条件为 $\lambda_1 = \lambda_2 = \dots = \lambda_n$, 即 $C^T B C$ 是数量矩阵. 注意到
\begin{align*}
