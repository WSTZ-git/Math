...
\]
现在，我们定义$k$个新的向量$\mathbb{c}_1, \mathbb{c}_2, \ldots, \mathbb{c}_k \in \mathbb{R}^{n - l}$，其中每个向量$\mathbb{c}_i$由对应行向量$\mathbf{a}_i$的后$n - l$个分量构成：
\[
...
考虑这些向量的和向量$\mathbf{v} = \sum_{i=1}^k \mathbb{c}_i$。我们来计算其模的平方$\|\mathbf{v}\|^2$。
\begin{align*}
\|\mathbf{v}\|^2 &= \mathbf{v} \cdot \mathbf{v} = \left( \sum_{i=1}^k \mathbb{c}_i \right) \cdot \left( \sum_{j=1}^k \mathbb{c}_j \right) \\
