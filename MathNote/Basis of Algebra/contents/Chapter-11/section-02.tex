...
设 $u_1,\cdots,u_m$ 和 $v_1,\cdots,v_n$ 是两组给定的非零复数，若对任意正整数 $k$ 都有 $u_1^k+\cdots+u_m^k=v_1^k+\cdots+v_n^k$，则$u_1,\cdots,u_m$ 是 $v_1,\cdots,v_n$ 的一个排列,进而$m=n$.
\end{lemma}
\begin{proof}
...
\begin{align*}
C^TU_{3}^{T}U_3C=O\Rightarrow U_{3}^{T}U_3=O \xLongrightarrow{\hyperref[proposition:AA转置秩与A秩相等]{\mathrm{r}\left( U_3 \right) =\mathrm{r}\left( U_{3}^{T}U_3 \right) =0}} U_3=O.
\end{align*}
同理可得 $U_2=O$.因此
\begin{align*}
A=\begin{pmatrix}
...
\begin{example}
设 $A, B \in M_n(\mathbb{R})$ 为可逆矩阵，满足 $ABA = B$ 且 $BAB = A$，证明：$A^4 = B^4 = I$.
\end{example}
\begin{proof}
由题设可知
\begin{align*}
...
从而
\begin{align*}
A^2 = B^2 = (ABA)(ABA) = ABA^2 BA \implies BA^2 B = I \implies A^2 = B^{-2} = B^2.
\end{align*}
于是 $A^4 = B^4 = I$.
\end{proof}

\begin{example}
设 $M$ 是 $n$ 阶方阵，$(i,j)$ 元素是 $i$ 和 $j$ 的最大公因数，其中 $1 \leqslant i,j \leqslant n$. 证明：矩阵 $M$ 是正定矩阵.
\end{example}
\begin{proof}
构造 $n$ 阶实矩阵 $L=(l_{ij})$，其中
\begin{align*}
l_{ij}=
\begin{cases}
\sqrt{\varphi(j)}, & j\mid i,\\
0, & j\nmid i.
\end{cases}
\end{align*}
因为当 $i<j$ 时，$j\nmid i$，所以 $l_{ij}=0$，即 $L$ 是下三角矩阵；且对角元
\begin{align*}
l_{ii}=\sqrt{\varphi(i)}>0,
\end{align*}
所以 $\det L=\prod\limits_{i=1}^n \sqrt{\varphi(i)}>0$，故 $L$ 可逆.

设 $\varphi$ 为欧拉函数,则由\hyperref[Number Theory-theorem:欧拉函数的基本性质-4]{Gauss恒等式}知
\begin{align*}
(LL^T)_{ij}
=\sum_{k=1}^n l_{ik}l_{jk} 
=\sum_{k\mid i,k\mid j}\varphi(k) 
=\sum_{k\mid \gcd(i,j)}\varphi(k) =\gcd(i,j).
\end{align*}
所以
\begin{align*}
M=LL^T.
...
因此对任意非零实向量 $x$，
\begin{align*}
x^TMx=x^TLL^Tx=(L^Tx)^T(L^Tx)=\|L^Tx\|^2>0,
\end{align*}
因为 $L$ 可逆，所以 $L^Tx\neq 0$.
故矩阵 $M$ 是正定矩阵.
\end{proof}
...
\begin{example}
设 $F$ 是一个数域，$A \in \mathbb{F}^{n \times n}$ 是可逆矩阵满足
\begin{align*}
A = \begin{pmatrix} A_{11} & A_{12} \\ A_{21} & A_{22} \end{pmatrix}, \quad
A^{-1} = \begin{pmatrix} B_{11} & B_{12} \\ B_{21} & B_{22} \end{pmatrix}, \quad
A_{11} \in \mathbb{F}^{\ell \times k}, \quad B_{11} \in \mathbb{F}^{k \times \ell}.
\end{align*}
证明：$\text{Ker}\, A_{12} \cong \text{Ker}\, B_{12}$.
\end{example}
\begin{proof}
由
\begin{align*}
...
\begin{pmatrix} B_{11} & B_{12} \\ B_{21} & B_{22} \end{pmatrix}
=
\begin{pmatrix} E_\ell & 0 \\ 0 & E_{n-\ell} \end{pmatrix},
\end{align*}
得
\begin{align*}
B_{11}A_{12} + B_{12}A_{22} = 0, \quad
A_{21}B_{12} + A_{22}B_{22} = E_{n-\ell}.
\end{align*}
设 $x \in \text{Ker}\, A_{12}$，我们有
\begin{align*}
B_{12}(A_{22}x) = B_{11}A_{12}x + B_{12}A_{22}x = 0 \implies A_{22}x \in \text{Ker}\, B_{12},
\end{align*}
于是可定义线性映射
\begin{align*}
f : \text{Ker}\, A_{12} \to \text{Ker}\, B_{12}, \quad x \mapsto A_{22}x.
\end{align*}
注意到
\begin{align*}
x \in \text{Ker}\, f \implies A_{22}x = 0, \quad A_{12}x = 0 \implies A\begin{pmatrix} 0 \\ x \end{pmatrix} = 0,
\end{align*}
又因为$A$可逆,所以$\text{Ker}\,A=\{0\}$,故$x = 0$。即 $f$ 是单射。

现在对 $y \in \text{Ker}\, B_{12}$，取 $x = B_{22}y \in \mathbb{F}^{n-k}$，我们有
\begin{align*}
A_{12}x = -A_{11}B_{12}y = 0 \implies x \in \text{Ker}\, A_{12}
\end{align*}
且
\begin{align*}
f(x) = A_{22}x = A_{22}B_{22}y = y - A_{21}B_{12}y = y,
\end{align*}
即 $f$ 是满射。

现在我们证明了 $\text{Ker}\, A_{12} \cong \text{Ker}\, B_{12}$.
\end{proof}

\begin{example}
设 $A$ 为三阶矩阵，$E$ 为三阶单位矩阵。分别在下列条件下求 $|A + 3E|$：
\begin{enumerate}[(1)]
\item $|A - E| = |A - 2E| = |A + 2E| = 0$;
\item $|A - E| = |A - 2E| = |A + E| = 2$.
\end{enumerate}
\end{example}
\begin{solution}
设 $f(x)=|xE-A|$ 是 $A$ 的特征多项式，因为 $A$ 为三阶矩阵，所以 $\deg f=3$.
\begin{enumerate}[(1)]
\item 由条件知
\begin{align*}
f(1)=f(-2)=f(2)=0 \Longrightarrow f(x)=(x-1)(x-2)(x+2).
\end{align*}
故
\begin{align*}
|A+3E|=-|-3E-A|=-f(-3)=20.
...
\item 令 $g(x)=f(x)+2$，则由条件知
\begin{align*}
f(1)=f(-2)=f(2)=-2 \Longrightarrow g(1)=g(-2)=g(2)=0 \Longrightarrow g(x)=(x-1)(x-2)(x+2).
\end{align*}
故
\begin{align*}
|A+3E|=-|-3E-A|=-f(-3)=2-g(-3)=22.
\end{align*}
\end{enumerate}
\end{solution}

\begin{example}
如果 $n$ 阶方阵 $A_1, A_2, \cdots, A_m$ 满足 $A_i^2 \neq 0$ ($i = 1, 2, \cdots, m$)，且当 $i \neq j$ 时，$A_i A_j = 0$，证明 $m \leqslant n$.
\end{example}
\begin{proof}

{\color{blue}证法一:}
由 $A_i^2 \neq 0$ 知，存在$A_i$的一个列向量 $v_i$ 使得 $A_i v_i \neq 0$. 下证 $v_1, v_2, \cdots, v_m$ 线性无关.

设 $c_1 v_1 + c_2 v_2 + \cdots + c_m v_m = 0$. 用 $A_i$ 左乘等式两端，得
\begin{align*}
c_1 A_i v_1 + c_2 A_i v_2 + \cdots + c_m A_i v_m = 0.
\end{align*}
由于 $i \neq j$ 时 $A_i A_j = 0$，故 $A_i v_j = 0$ ($j \neq i$)，从而上式化为 $c_i A_i v_i = 0$. 又 $A_i v_i \neq 0$，所以 $c_i = 0$. 因此 $v_1, \cdots, v_m$ 线性无关，故 $m \leqslant n$.

{\color{blue}证法二:}
考虑分块矩阵乘积
\begin{align*}
(A_1 \ A_2 \ \cdots \ A_m) \begin{pmatrix}
A_1 \\ A_2 \\ \vdots \\ A_m
\end{pmatrix} = \sum_{i=1}^m A_i^2.
\end{align*}
但由题意 $A_i A_j = 0$ ($i \neq j$)，故
\begin{align*}
\begin{pmatrix}
A_1 \\ A_2 \\ \vdots \\ A_m
\end{pmatrix} (A_1 \ A_2 \ \cdots \ A_m) =
\begin{pmatrix}
A_1^2 & 0 & \cdots & 0 \\
0 & A_2^2 & \cdots & 0 \\
...
\end{pmatrix}.
\end{align*}
左边矩阵的秩不超过 $n$，而右边矩阵的秩等于 $\sum_{i=1}^m \operatorname{rank}(A_i^2)$. 因 $A_i^2 \neq 0$，故 $\operatorname{rank}(A_i^2) \geqslant 1$，从而
\begin{align*}
n \geqslant \operatorname{rank}\left( \sum_{i=1}^m A_i^2 \right) \geqslant \sum_{i=1}^m \operatorname{rank}(A_i^2) \geqslant m.
\end{align*}
所以 $m \leqslant n$.
\end{proof}
