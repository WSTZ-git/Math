...
\begin{enumerate}
\item 设 \(\boldsymbol{A},\boldsymbol{D}\) 是方阵, \(\boldsymbol{M}=\begin{pmatrix}\boldsymbol{A}&\boldsymbol{B}\\\boldsymbol{B}'&\boldsymbol{D}\end{pmatrix}\) 是正定实对称矩阵, 则:
\begin{align*}
|\boldsymbol{M}|\leqslant |\boldsymbol{A}||\boldsymbol{D}|,
\end{align*}
且等号成立当且仅当 \(\boldsymbol{B}=\boldsymbol{O}\).

\item 设$A \in \mathbb{R}^{p \times p}, C \in \mathbb{R}^{q \times q}$，考虑半正定矩阵$M = \begin{pmatrix} A & B \\ B^T & C \end{pmatrix}$，则
\begin{align*}
|M| \leqslant |A||C|.
\end{align*}
且等号成立当且仅当$B=O.$
...
\item 因为$M$半正定，所以所有主子式非负，因此$A,C$半正定。考虑$M + tI_{p+q}, A + tI_p, B + tI_q, t > 0$并让$t \to 0^+$可以不妨设$A,C$都是正定的。
利用正定矩阵存在正定平方根，注意到
\begin{align*}
...
\end{proof}
\begin{remark}
在\refpro{proposition:Fischer不等式}的证明中, 若考虑 \(\boldsymbol{M}\) 的如下对称分块初等变换:
\begin{align*}
