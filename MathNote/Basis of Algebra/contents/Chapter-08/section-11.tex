...
\begin{theorem}[正定阵的判定准则]\label{theorem:正定阵的判定准则}
...
\end{proposition}
\begin{proof}
{\color{blue}证法一:}设 \(\boldsymbol{C}\) 为非异实矩阵, 使得 \(\boldsymbol{C}'\boldsymbol{A}\boldsymbol{C}=\boldsymbol{I}_n\). 注意到问题的条件和结论在同时合同变换 \(\boldsymbol{A}\mapsto\boldsymbol{C}'\boldsymbol{A}\boldsymbol{C}\), \(\boldsymbol{S}\mapsto\boldsymbol{C}'\boldsymbol{S}\boldsymbol{C}\) 下不改变(不等式两边同乘$|C|^2$), 故不妨从一开始就假设 \(\boldsymbol{A}=\boldsymbol{I}_n\) 为合同标准型, 从而由\refpro{proposition:反称矩阵A的I_n+A的行列式相关结论}即得结论. 
...
{\color{blue}证法二:} 设 \(\boldsymbol{A}=\boldsymbol{I}_n-\frac{\boldsymbol{\beta}\boldsymbol{\beta}'}{\boldsymbol{\beta}'\boldsymbol{\beta}}+\frac{\boldsymbol{\alpha}\boldsymbol{\alpha}'}{\boldsymbol{\alpha}'\boldsymbol{\beta}}\), 则由\refpro{proposition:A正定则H正定}可知 \(\boldsymbol{A}\) 为正定阵,则
\begin{align*}
\boldsymbol{A\beta }=\boldsymbol{\beta }-\frac{\boldsymbol{\beta }\left( \boldsymbol{\beta }' \boldsymbol{\beta } \right)}{\boldsymbol{\beta }' \boldsymbol{\beta }}+\frac{\boldsymbol{\alpha }\left( \boldsymbol{\alpha }' \boldsymbol{\beta } \right)}{\boldsymbol{\alpha }'\boldsymbol{\beta }}=\boldsymbol{\beta }-\boldsymbol{\beta }+\boldsymbol{\alpha }=\boldsymbol{\alpha }.
\end{align*}
故结论得证.
...
利用正定矩阵存在正定平方根，注意到
\begin{align*}
W = \begin{pmatrix} A^{-\frac{1}{2}} & 0 \\ 0 & D^{-\frac{1}{2}} \end{pmatrix} \begin{pmatrix} A & B \\ B^T & D \end{pmatrix} \begin{pmatrix} A^{-\frac{1}{2}} & 0 \\ 0 & D^{-\frac{1}{2}} \end{pmatrix} = \begin{pmatrix} I_p & A^{-\frac{1}{2}}BD^{-\frac{1}{2}} \\ D^{-\frac{1}{2}}B^T A^{-\frac{1}{2}} & I_q \end{pmatrix},
\end{align*}
我们有
\begin{align*}
