...
\end{theorem}
\begin{proof}
由\reftheorem{theorem:正定型的充要条件}及\reftheorem{theorem:实二次型的规范标准型}可立即得到证明.
\end{proof}

\begin{corollary}\label{corollary:正定阵的逆必正定}
若矩阵$A$可逆,则
\begin{enumerate}[(1)]
\item $n$ 阶实对称阵 $A$ 是正定阵当且仅当$A^{-1}$是正定阵;

\item $A$ 是负定阵当且仅当$A^{-1}$是负定阵;

\item $A$ 是半正定阵当且仅当 $A^{-1}$是半正定阵;

\item $A$ 是半负定阵当且仅当 $A^{-1}$是半负定阵.
\end{enumerate}
\end{corollary}
\begin{proof}
只证明(1),(2)(3)(4)同理可证.

先证必要性,由\reftheorem{theorem:正定矩阵的充要条件}可知存在非异阵$P$,使得$P'AP=I_n$.两边同时取逆可得
\begin{align*}
(P^{-1})'A^{-1}P^{-1}=I_n.
\end{align*}
故$A^{-1}$也合同于$I_n$,因此由\reftheorem{theorem:正定矩阵的充要条件}可知$A^{-1}$也正定.充分性由必要性同理可得.
\end{proof}
...
\item {\color{blue}证法一:}由(1)的结论,$\boldsymbol{A}$的所有主子式都是正定阵.又$\boldsymbol{A}$的每个主子式都是其自身的顺序主子式,故由\reftheorem{theorem:实正定阵的充要条件}可知$\boldsymbol{A}$的所有主子式都大于零.因此(2)成立.
