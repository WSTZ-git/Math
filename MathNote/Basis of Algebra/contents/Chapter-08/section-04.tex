...
若矩阵$A$可逆,且$A$是半正定阵,则$A$一定是正定阵.
\end{corollary}
\begin{proof}
由\reftheorem{theorem:正定矩阵的充要条件}可知$A$合同于$\left( \begin{matrix}
I_r&		O\\
O&		O\\
\end{matrix} \right) $,其中$r$是$A$的秩.又因为$A$可逆,所以$r=n$,故$A$与$I_n$合同.会因此再由\reftheorem{theorem:正定矩阵的充要条件}可知$A$是正定阵.
\end{proof}
