...
\end{theorem}
\begin{proof}
由\reftheorem{theorem:正定型的充要条件}可立即得到证明.
\end{proof}
...
\item 由(1)的结论,$\boldsymbol{A}$的所有主子式都是正定阵.又$\boldsymbol{A}$的每个主子式都是其自身的顺序主子式,故由\reftheorem{theorem:实正定阵的充要条件}可知$\boldsymbol{A}$的所有主子式都大于零.因此(2)成立.
