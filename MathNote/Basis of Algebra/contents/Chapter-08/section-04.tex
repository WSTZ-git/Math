...
\begin{enumerate}[(1)]
\item\label{proposition:正定阵的性质-1} 若 $\boldsymbol{A}$ 是正定阵，则$\boldsymbol{A}$ 的任一 $k$ 阶主子阵，即由 $\boldsymbol{A}$ 的第 $i_1,i_2,\cdots,i_k$ 行及 $\boldsymbol{A}$ 的第 $i_1,i_2,\cdots,i_k$ 列交点上元素组成的矩阵，必是正定阵;
