...
\end{pmatrix},
\]
其中$d_n = a_{nn}-\boldsymbol{\alpha}'\boldsymbol{A}_{n - 1}^{-1}\boldsymbol{\alpha}$.由此可得$b_{nn}=d_n^{-1}$, 再由$\boldsymbol{A}_{n - 1}$的正定性及\refproposition{proposition:A+S的行列式的相关结论}可知$A_{n-1}^{-1}$也正定,于是
\[
...
\end{theorem}
\begin{remark}
本例题给出了\refproposition{proposition:反对称阵的秩必为偶数}的另一证明. 注意到在本题的证明中, 我们采用的是跨度为 2 的数学归纳法(第二数学归纳法), 故在起始步骤时需要验证$n = 1, 2$这两种情形, 但我们不难发现$n = 2$情形的证明完全包含在归纳过程的证明中, 因此可以用$n = 0, 1$的情形作为起始步骤. 需要注意的是, $n = 0$并不意味着存在零阶矩阵, 而只是说明归纳过程已经完全结束. 后面遇到跨度为 2 的数学归纳法, 我们通常都采用上述约定. 
\end{remark}
\begin{proof}
...
\end{proposition}
\begin{proof}
由\reftheorem{theorem:反对称矩阵的合同标准型}可知, 存在非异实矩阵$\boldsymbol{C}$, 使得
\[
...
\end{proposition}
\begin{proof}
(1) 由\refproposition{proposition:直接计算两个矩阵和的行列式}可知
\[
...
i_1 & i_2 & \cdots & i_k \\
i_1 & i_2 & \cdots & i_k
\end{pmatrix}$是$k$阶实反对称行列式, 故由\refproposition{proposition:实反称矩阵的行列式必非负}可知其值大于等于零, 于是$|\boldsymbol{I}_n+\boldsymbol{A}|\geq 1 + |\boldsymbol{A}|$成立. 当$n\leq 2$时, 容易验证不等式的等号成立. 当$n\geq 3$时, 若不等式的等号成立, 则必有
\[
\boldsymbol{A}\begin{pmatrix}
i & j \\
i & j
