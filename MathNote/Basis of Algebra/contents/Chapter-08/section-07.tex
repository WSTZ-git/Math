...
\end{pmatrix}
\end{align*}
由$\boldsymbol{A}$的正定性知,上式右边仍是正定阵(合同于$A$).再由\hyperref[proposition:正定阵的性质]{命题\ref{proposition:正定阵的性质}(2)}可得$a_{nn}-\boldsymbol{\alpha}'\boldsymbol{A}_{n - 1}^{-1}\boldsymbol{\alpha}>0$. 再由归纳假设, 存在主对角元全为 1 的$n - 1$阶上三角矩阵$\boldsymbol{T}_{n - 1}$, 使得$\boldsymbol{T}_{n - 1}'\boldsymbol{A}_{n - 1}\boldsymbol{T}_{n - 1}=\boldsymbol{D}_{n - 1}$是$n - 1$阶正定对角矩阵. 令
\[
...
\end{pmatrix},
\]
其中$d_n = a_{nn}-\boldsymbol{\alpha}'\boldsymbol{A}_{n - 1}^{-1}\boldsymbol{\alpha}$.由此可得$b_{nn}=d_n^{-1}$, 再由$\boldsymbol{A}_{n - 1}$的正定性及\refpro{proposition:正定阵的性质123}可知$A_{n-1}^{-1}$也正定,于是
\[
...
0 & 1 \\
-1 & 0
\end{pmatrix}$或$\boldsymbol{S}=\begin{pmatrix}
0 & -1 \\
1 & 0
...
只证$\boldsymbol{S}=\begin{pmatrix}
0 & 1 \\
-1 & 0
\end{pmatrix}$的情形,另一种同理可证.
...
\end{pmatrix}.
\]
注意到$\boldsymbol{A}_{n - 2}+\boldsymbol{B}'\boldsymbol{S}^{-1}\boldsymbol{B}$是$n - 2$阶反对称矩阵, 故由归纳假设它合同于\eqref{eq:8.2}式形状的矩阵, 因此分块对角矩阵$\boldsymbol{N}$也合同于\eqref{eq:8.2}式形状的矩阵.故由数学归纳法知则$\boldsymbol{A}$必合同于下列形状的分块矩阵:
\begin{align*}
\text{diag}\{\boldsymbol{S}, \cdots, \boldsymbol{S}, 0, \cdots, 0\}.
\end{align*}
因为$r(S)=2$,又矩阵合同不改变秩,所以$r(A)$必是偶数,并且$\frac{r(A)}{2}$就是上式分块矩阵中$S$的个数.
\end{proof}
...
\end{proposition}
\begin{proof}
(1) {\color{blue}证法一:}由\refpro{proposition:矩阵和的行列式计算公式}可知
\[
...
{\color{blue}证法二:}由\refpro{proposition:实对称(反称)阵的特征值}知，$A$的特征值为$0$或纯虚数，不妨设为
\begin{align*}
