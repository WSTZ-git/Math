...
\end{pmatrix}
\end{align*}
注意到 \(\boldsymbol{A}\) 的第 \(k\) 个顺序主子式 \(|\boldsymbol{A}_k|\) 的每行元素之和都为 \((n - k)/n\), 故用求和法可求出 \(|\boldsymbol{A}_k| = (n - k)/n(1\leq k\leq n)\), 因此 \(\boldsymbol{A}\) 的秩等于 \(n - 1\). 由\refproposition{proposition:和的平方和的半正定型的秩等于其系数矩阵的秩}可知 \(\mathrm{r}(f)=\mathrm{r}(\boldsymbol{A}) = n - 1\), 于是半正定型 \(f\) 的正惯性指数等于 \(n - 1\), 其规范标准型为 \(z_1^2 + z_2^2 + \cdots + z_{n - 1}^2\).
\end{proof}
...
\end{pmatrix}
\end{align*}
经计算可知 \(|\boldsymbol{A}| = 1 - a_1a_2\cdots a_n\) 并且 \(\boldsymbol{A}\) 的左上角的 \(n - 1\) 阶子式等于 1, 于是由\refproposition{proposition:和的平方和的半正定型的秩等于其系数矩阵的秩}可知, 当 \(a_1a_2\cdots a_n = 1\) 时, \(\mathrm{r}(f)=\mathrm{r}(\boldsymbol{A}) = n - 1\), \(f\) 的规范标准型为 \(z_1^2 + z_2^2 + \cdots + z_{n - 1}^2\); 当 \(a_1a_2\cdots a_n\neq1\) 时, \(\mathrm{r}(f)=\mathrm{r}(\boldsymbol{A}) = n\), \(f\) 的规范标准型为 \(z_1^2 + z_2^2 + \cdots + z_n^2\).
\end{proof}
