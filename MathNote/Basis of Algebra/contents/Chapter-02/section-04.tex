...
\begin{enumerate}[(1)]
\item 设\(A=(a_{ij})\)为\(m\times n\)实矩阵,则\(\mathrm{tr}(AA')\geqslant 0\),等号成立当且仅当\(A = O\).

\item 设\(A=(a_{ij})\)为\(m\times n\)复矩阵,则\(\mathrm{tr}(A\overline{A}')\geqslant 0\),等号成立当且仅当\(A = O\).
\end{enumerate}
\end{proposition}
\begin{proof}
\begin{enumerate}[(1)]
\item 设\(A=(a_{ij})\)为\(m\times n\)实矩阵,则通过计算可得
\[
...
\item 设\(A=(a_{ij})\)为\(m\times n\)复矩阵,则通过计算可得
\[
...
\begin{example}
设 $V = \mathbb{R}^{n \times n}$ 为全体 $n$ 阶实矩阵构成的线性空间，设 $W$ 为 $V$ 的一个子空间，且满足：若 $A, B \in W$，$\operatorname{tr}(AB) = 0$，求满足上述 $W$ 的维数 $\dim W$ 的最大值。
\end{example}
\begin{proof}
先考虑 $V$ 中所有对角线元素均为 $0$ 的上三角形矩阵构成的线性空间 $W_0$，任取 $A, B \in W_0$，由于 $AB$ 依旧是对角线元素均为 $0$ 的上三角形矩阵，所以 $\operatorname{tr}(AB) = 0$. 即 $W_0$ 是满足条件的一个线性空间，同时
\begin{align*}
\dim W_0 = 1 + 2 + \cdots + (n-1) = \frac{1}{2}n(n-1).
\end{align*}
另外，记 $U$ 是 $V$ 中所有实对称矩阵构成的线性空间，显然
\begin{align*}
\dim U = 1 + 2 + \cdots + n = \frac{1}{2}n(n+1).
\end{align*}
若线性空间 $W$ 满足题设条件，且 $\dim W > \frac{1}{2}n(n-1)$，那么根据维数公式可知
\begin{align*}
\dim(W \cap U) = \dim W + \dim U - \dim(W + U) > \frac{1}{2}n(n-1) + \frac{1}{2}n(n+1) - n^2 = 0.
\end{align*}
于是 $W \cap U \neq \{O\}$，那么存在非零矩阵 $A \in W \cap U$. 那么 $\operatorname{tr}(A^T A) = \operatorname{tr}(A^2) = 0$，由\cref{proposition:矩阵与其转置乘积的迹}知$A = O$，矛盾! 综上，满足条件的线性空间 $W$ 的维数 $\dim W$ 的最大值为 $\frac{1}{2}n(n-1)$.
\end{proof}

\begin{example}
已知\( A,B \)均为\( n \)阶实矩阵，且\( AB - BA = A^{T} + B^{T} \)，求\( A + B \)。
\end{example}
\begin{proof}
由条件可得
\begin{align*}
...
设$A,B$为实对称方阵，证明：
\begin{align*}
\mathrm{tr}\left[ \left( AB \right) ^2 \right] =\mathrm{tr}\left( ABAB \right) \leqslant \mathrm{tr}\left( AABB \right) =\mathrm{tr}\left( A^2B^2 \right) .
\end{align*}
\end{example}
\begin{proof}
