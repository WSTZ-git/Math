...
\]
上式表明\(g(x)\mid f(x)(u(x)-u_1(x))\)，又由于\(( f(x),g(x) ) = 1\)，因此\(g(x)\mid (u(x)-u_1(x))\)。而\(\mathrm{deg}\,(u(x)-u_1(x))<\mathrm{deg}\,g(x)\)，故\(u(x)-u_1(x)=0\)，即\(u(x)=u_1(x)\)。同理可得\(v(x)=v_1(x)\)。\end{proof}

\begin{proposition}[互素多项式和最大公因式的基本性质]\label{proposition:互素多项式和最大公因式的基本性质}
设$f(x),g(x),f_1(x),f_2(x)\in \mathbb{K}[x]$,则
\begin{enumerate}[(1)]
\item\label{proposition:互素多项式和最大公因式的基本性质-1} 若\(f_1(x)\mid g(x), f_2(x)\mid g(x)\),且\((f_1(x), f_2(x)) = 1\),则\(f_1(x)f_2(x)\mid g(x)\).
...
\item 由\hyperref[theorem:多项式互素的充要条件]{多项式互素的充要条件}可知,存在\(u(x),v(x)\in\mathbb{K}[x]\)，使
\[
f(x)u(x)+g(x)v(x)=1,
\]
则
...
\item 由\hyperref[theorem:多项式互素的充要条件]{多项式互素的充要条件}可知,存在\(u_1(x),v_1(x),u_2(x),v_2(x)\in\mathbb{K}[x]\)，使
\begin{align*}
...
\begin{example}
设 \(f(x),g(x),h(x)\in F[x]\)，且 \(g(x)\) 与 \(h(x)\) 互素。试证明：存在 \(r(x),s(x)\in F[x]\) 使得 \(f(x) = g(x)r(x) + h(x)s(x)\)，且 \(\deg r(x)< \deg h(x)\)。
\end{example}
\begin{proof}
由带余除法，存在 \(q_{0}(x),r_{0}(x)\in F[x]\)，使得
\begin{align}
f(x) = q_{0}(x)h(x) + r_{0}(x), \label{eq:p112dfgg12342233-1}
\end{align}
其中 \(\deg r_{0}(x)< \deg h(x)\)。
因为 \(g(x)\) 与 \(h(x)\) 互素，根据\hyperref[theorem:多项式互素的充要条件]{多项式互素的充要条件}，存在 \(a(x),b(x)\in F[x]\)，使得
\begin{align*}
a(x)g(x) + b(x)h(x) = 1.
\end{align*}
两边同乘 \(r_{0}(x)\)，可得
\begin{align*}
\bigl(r_{0}(x)a(x)\bigr)g(x) + \bigl(r_{0}(x)b(x)\bigr)h(x) = r_{0}(x).
\end{align*}
记\(u(x)\triangleq r_0(x)a(x),v(x)\triangleq r_0(x)b(x)\in F[x]\)，使得
\begin{align}
u(x)g(x) + v(x)h(x) = r_{0}(x). \label{eq:p112dfgg12342233-2}
\end{align}
再对 \(u(x)\) 用 \(h(x)\) 作带余除法，存在 \(q(x),r(x)\in F[x]\)，使得
\begin{align}
u(x) = q(x)h(x) + r(x), \label{eq:p112dfgg12342233-3}
\end{align}
并且$\deg r(x)< \deg h(x).$
将 \eqref{eq:p112dfgg12342233-3}式 代入 \eqref{eq:p112dfgg12342233-2}式，得到
\begin{align}
r_{0}(x)  = \bigl(q(x)h(x) + r(x)\bigr)g(x) + v(x)h(x) \nonumber  = g(x)r(x) + h(x)\bigl(q(x)g(x) + v(x)\bigr). \label{eq:p112dfgg12342233-5}
\end{align}
再把 \eqref{eq:p112dfgg12342233-5}式 代入 \eqref{eq:p112dfgg12342233-1}式，便有
\begin{align*}
f(x) & = q_{0}(x)h(x) + r_{0}(x) \\
& = q_{0}(x)h(x) + g(x)r(x) + h(x)\bigl(q(x)g(x) + v(x)\bigr) \\
& = g(x)r(x) + h(x)\bigl(q_{0}(x) + q(x)g(x) + v(x)\bigr).
\end{align*}
令
\begin{align*}
s(x) = q_{0}(x) + q(x)g(x) + v(x),
\end{align*}
则 \(s(x)\in F[x]\)满足
\begin{align*}
f(x) = g(x)r(x) + h(x)s(x).
\end{align*}
并且$\deg r(x)< \deg h(x).$\end{proof}
...
\begin{gather}
p|\prod_{i\ne 1}{f_i}\Longrightarrow p\left( x \right) \text{必可整除}f_2,f_3\cdots ,f_n\text{中的某一个}\Longrightarrow \text{存在}k_1\in \left\{ 1,2,\cdots ,n \right\}\backslash \{1\} ,\text{使得}p|f_{k_1},\nonumber 
\\
