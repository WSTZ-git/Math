...
\end{align*}
其中$f_1(x)$为任一特解,$k(x)\in \mathbb{C}[x]$.因为$\deg\left[(x + 1)(x^2 + x + 1)(2x^3 + 1)\right] = 6,$所以当$\deg f_1(x)\geq 6$时,可以取合适的$k(x)$,使得$f_1\left( x \right) +k\left( x \right) \left( x+1 \right) \left( x^2+x+1 \right) \left( 2x^3+1 \right)$中高于6次项全部消去.故原方程组的最小次解必定小于等于5次.
\end{remark}
\begin{proof}
因为
\[
