...
取正交矩阵\( T \),使得
\[
T^T A T = \text{diag}\{\lambda_1, \lambda_2, \cdots, \lambda_n\}, \lambda_i \geq 0, i = 1, 2, \cdots, n.
\]
...
1. 设列满秩矩阵\( A \in \mathbb{R}^{m \times n}, m \geq n \),则存在唯一的列向量为正交单位向量组的\( Q \in \mathbb{R}^{m \times n} \),主对角元都为正数的上三角矩阵\( R \in \mathbb{R}^{n \times n} \),使得\( A = QR \).

2. 此外,在标准内积下,对任何\( \beta \in \mathbb{R}^n, R^{-1} Q^T \beta \)是矩阵方程\( A^T A x = A^T \beta \)的唯一解.

3. 设矩阵\( A \in \mathbb{R}^{m \times n}, m \geq n \),则存在列向量为正交单位向量组的\( Q \in \mathbb{R}^{m \times n} \),主对角元都为非负数的上三角矩阵\( R \in \mathbb{R}^{n \times n} \),使得\( A = QR \).
\end{theorem}
\begin{note}
...
则
\[
b_{ij} a_j = a_i b_{ij}', \forall 1 \leq i, j \leq n.
\]
...
设\( Q_1 \in \mathbb{R}^{m_1 \times n_1}, Q_2 \in \mathbb{R}^{m_2 \times n_1}, Q_1^T Q_1 + Q_2^T Q_2 = I_{n_1} \),这里\( \min\{m_1, m_2\} \geq n_1 \).则存在正交矩阵\( V_1 \in \mathbb{R}^{n_1 \times n_1}, U_1 \in \mathbb{R}^{m_1 \times m_1}, U_2 \in \mathbb{R}^{m_2 \times m_2} \)和\( 0 \leq \theta_1 \leq \theta_2 \leq \cdots \leq \theta_{n_1} \leq \frac{\pi}{2} \)使得
\begin{align}
\begin{pmatrix}
U_1 & 0 \\
0 & U_2
...
\[
1 = c_1 = c_2 = \cdots = c_t > c_{t + 1} \geq \cdots \geq c_{n_1} \geq 0.
\]
