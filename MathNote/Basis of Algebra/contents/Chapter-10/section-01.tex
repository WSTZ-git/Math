...
\begin{theorem}[奇异值分解(SVD分解)]\label{theorem:奇异值分解}
...
\end{pmatrix},
\]
这里\( \Lambda \)是\( r(A) \)阶对角矩阵且对角元都为正数,若记$\Lambda=\mathrm{diag}\left\{ \sigma _1,\sigma _2,\cdots ,\sigma _{r\left( A \right)} \right\},$则称$\Lambda $的对角元$\sigma _1,\sigma _2,\cdots ,\sigma _{r\left( A \right)}$为\textbf{奇异值},且$\sigma _1\geqslant \sigma _2\geqslant \cdots \geqslant \sigma _{r\left( A \right)}.$
\end{theorem}
\begin{remark}
如果这个定理已经被证明,那么
\[
...
\[
0 = V_2^T A^T A V_2 = (A V_2)^T (A V_2) \stackrel{\text{\refpro{proposition:AA转置秩与A秩相等}}}{\Longrightarrow } A V_2 = 0, V_1^T A^T A V_1 = \Lambda^2.
\]
待定
\[
...
\item 注意到
\[
r(A) = r(A^T) \stackrel{\text{\nrefpro{proposition:矩阵秩的基本公式}{(2)}}}{\geqslant} r\left(A^T\begin{pmatrix} A & \beta \end{pmatrix}\right)=r\left( \begin{pmatrix} A^T A & A^T \beta \end{pmatrix} \right) \geqslant r(A^T A) \xlongequal{\text{\refpro{proposition:AA转置秩与A秩相等}}} r(A),
\]
...
\begin{align}
-e^{\mathrm{i}\theta _j}, \theta_j \in [0, 2\pi), j = 1, 2, \cdots, s.\label{eq::iof342rf2}
\end{align}
取\( \theta \in [0, 2\pi) \)使得\( \theta \neq \frac{\theta_j}{2} + k\pi, j = 0, 1, 2, \cdots, s, k \in \mathbb{Z} \).

现在考虑\( B = e^{\mathrm{i}\theta } I + e^{-\mathrm{i}\theta } A \),假设$B$不可逆,则存在\( A \)的特征值\( \lambda \),使得\( e^{\mathrm{i}\theta } + e^{-\mathrm{i}\theta } \lambda = 0 \),则\( e^{i2\theta} = -\lambda \),两边取模知$|\lambda|=1$,故$\lambda$为$A$在单位圆周上的特征值.由\eqref{eq::iof342rf2}式知\( \lambda = -e^{\mathrm{i}\theta _j}, j \in \{1, 2, \cdots, s\} \),所以
\begin{align}
e^{i2\theta}=e^{\mathrm{i}\theta _j}\Longrightarrow \exists k\in \mathbb{Z},\mathrm{s}.\mathrm{t}.  2\theta=\theta + 2k\pi.
\end{align}
因此存在\( k \in \mathbb{Z} \),使得\( \theta = \frac{\theta_j}{2} + k\pi \),这和\( \theta \)定义矛盾!因此\( B \)可逆,此时
\[
A \overline{B} = A \left( e^{-\mathrm{i}\theta } I + e^{\mathrm{i}\theta } \overline{A} \right) = e^{\mathrm{i}\theta } I + e^{-\mathrm{i}\theta } A = B,
\]
...
\end{proposition}
\begin{proof}
注意到\( \overline{A} \)满足\refthe{theorem:定理1354846135},所以存在可逆复矩阵\( Q_1 \in \mathbb{C}^{n \times n} \),使得\( A = Q_1 (\overline{Q_1})^{-1} \),记$Q=Q_1^{-1}$,从而\( A = \overline{Q} Q^{-1} \).
\end{proof}
...
\begin{proposition}
设 $A \in M_n(\mathbb{C})$. 证明：存在可逆对称矩阵 $S \in M_n(\mathbb{C})$ 与对称矩阵 $T \in M_n(\mathbb{C})$，使得
\begin{align*}
A = ST\text{或}A=TS.
\end{align*}
\end{proposition}
\begin{note}
证明的想法即利用\hyperref[lemma:反对角矩阵的性质]{反对角矩阵的性质}.
\end{note}
\begin{proof}
...
\]
则利用\hyperref[lemma:反对角矩阵的性质]{反对角矩阵的性质},我们有
\[
\begin{aligned}
...
\begin{proposition}\label{proposition:迹0阵必相似于主对角全0阵}
设 $A$ 是 $n$ 阶实方阵且满足 $\operatorname{tr}(A)=0$, 则存在可逆实方阵 $P$, 使得 $P^{-1}AP$ 的对角元素都是 $0$.
\end{proposition}
\begin{proof}
对 $n$ 进行归纳. 当 $n=1$ 时, 结论显然成立. 下设 $n\geqslant 2$, 假设结论对阶数小于等于 $n-1$ 时都成立, 现在考虑阶数为 $n$ 的情况. 考虑两种情形.

\textbf{情形一}: $\mathbb{R}^n$ 中的所有非零向量都是 $A$ 的特征向量.

{\color{blue}证法一:} 设 $A$ 的所有特征子空间为 $V_1,V_2,\cdots,V_s$, 则
\begin{align*}
\mathbb{R}^n = \bigcup_{i=1}^s V_i,
\end{align*}
由\hyperref[theorem:覆盖定理]{覆盖定理}知, 存在 $V_i=\mathbb{R}^n$, 故 $A$ 此时是纯量矩阵. 再由 $\operatorname{tr}(A)=0$ 知 $A=O$.

{\color{blue}证法二:} 此时所有基本向量 $e_i,i=1,2,\cdots,n$ 都是特征向量, 从而存在特征值 $\lambda_i,i=1,2,\cdots,n$ 使得
\begin{align*}
Ae_i = \lambda_i e_i,\quad i=1,2,\cdots,n.
\end{align*}
因此 $A=\operatorname{diag}\{\lambda_1,\lambda_2,\cdots,\lambda_n\}$. 再由所有 $e_i+e_j$ ($i\ne j$) 都是特征向量知, 存在 $\mu_{ij}$ 使得
\begin{align*}
A(e_i+e_j) = \lambda_i e_i + \lambda_j e_j = \mu_{ij}(e_i+e_j)
\Longrightarrow (\lambda_i-\mu_{ij})e_i + (\lambda_j-\mu_{ij})e_j = 0.
\end{align*}
于是由 $e_i,e_j$ 线性无关知 $\mu_{ij}=\lambda_i=\lambda_j$ ($i\ne j$). 因此 $A$ 是纯量矩阵. 再由 $\operatorname{tr}(A)=0$ 知 $A=O$.

\textbf{情形二}: 存在 $\mathbb{R}^n$ 中的非零向量 $\alpha$ 不是 $A$ 的特征向量, 则 $\alpha,A\alpha$ 线性无关, 将其扩充成 $\mathbb{R}^n$ 的一组基 $\{\alpha,A\alpha,\beta_1,\cdots,\beta_{n-2}\}$. 记 $Q=(\alpha,A\alpha,\beta_1,\cdots,\beta_{n-2})$, 则 $Q$ 为可逆实方阵, 且
\begin{align*}
AQ = Q\begin{pmatrix}
0 & * & \cdots & * \\
1 & * & \cdots & * \\
...
* & B
\end{pmatrix}
\Longleftrightarrow Q^{-1}AQ = \begin{pmatrix}
0 & * \\
* & B
\end{pmatrix},
\end{align*}
其中 $B$ 为 $n-1$ 阶实方阵. 由 $\operatorname{tr}(A)=0$ 得 $\operatorname{tr}(B)=0$. 由归纳假设知, 存在可逆实方阵 $R$, 使得 $R^{-1}BR$ 为主对角元全为 $0$ 的实方阵. 故
\begin{align*}
\begin{pmatrix}
I & O \\
O & R^{-1}
...
\end{pmatrix}
\end{align*}
也是主对角元全为 $0$ 的实方阵, 即 $A$ 相似于主对角元全为 $0$ 的实方阵.
\end{proof}

\begin{example}
证明: 任意 $n$ 阶实方阵 $A$ 可以分解成 $A=A_0+A_1+A_2$, 其中 $A_0=aI_n$, $a$ 是实数, $A_1$ 与 $A_2$ 都是幂零方阵.
\end{example}
\begin{remark}
对$A=A_0+A_1+A_2$两边同时取迹可得$a=\frac{\mathrm{tr}(A)}{n}$.
\end{remark}
\begin{proof}
记 $a=\frac{\operatorname{tr}(A)}{n}$, 则 $\operatorname{tr}(A-aI)=0$. 由\refpro{proposition:迹0阵必相似于主对角全0阵}知, 存在可逆阵 $P$, 使得 $P^{-1}(A-aI)P$ 是主对角全为 $0$ 的实方阵.
记 $P^{-1}(A-aI)P=(a_{ij})_{n\times n}$, 其中 $a_{ii}=0,i=1,2,\cdots,n$. 再记
\begin{align*}
L=\begin{pmatrix}
0 & a_{12} & \cdots & a_{1n} \\
0 & 0 & \cdots & a_{2n} \\
...
\end{pmatrix},
\end{align*}
则 $P^{-1}(A-aI)P=L+U$, 且 $L,U$ 分别是严格上、下三角矩阵. 注意到 $L^n=U^n=O$, 故 $L,U$ 都是幂零矩阵. 因此
\begin{align*}
A=aI+PLP^{-1}+PUP^{-1},
\end{align*}
其中 $PLP^{-1},PUP^{-1}$ 都是幂零矩阵.
\end{proof}
