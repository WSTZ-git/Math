...
\end{proposition}
\begin{remark}
也可以利用\cref{corollary:恒等变换的幂等分解}得到直和分解$\mathbb{F} ^m=\mathrm{Im}\left( \varphi \left( E_{11} \right) \right) \oplus \cdots \oplus \mathrm{Im}\left( \varphi \left( E_{nn} \right) \right) $,再证明$\mathrm{dim}\,\mathrm{Im}\left( \varphi \left( E_{ii} \right) \right) $都相等得到(2)的证明.
\end{remark}
\begin{proof}
\begin{enumerate}[(1)]
\item 设 $E_{ij}\in M_n(\mathbb F)(i,j=1,2,\cdots,n)$基础矩阵，令 $A_{ij}=\varphi(E_{ij})$.又由$\varphi$是线性映射知
\begin{align*}
...
\begin{gather}
A_{ij}A_{jk}=\varphi \left( E_{ij} \right) \varphi \left( E_{jk} \right) =\varphi \left( E_{ij}E_{jk} \right) =\varphi \left( E_{ik} \right) =A_{ik},\label{eq::iojgejjrw39392249322}
\\
A_{ij}A_{lk}=\varphi \left( E_{ij} \right) \varphi \left( E_{lk} \right) =\varphi \left( E_{ij}E_{lk} \right) =\varphi \left( O \right) =O.\nonumber
\end{gather}
故由\cref{theorem:非零相似于基础矩阵}知存在可逆矩阵 $P$，使得对任意的 $1\leqslant i,j\leqslant n$，有
\begin{align*}
P^{-1}A_{ij}P = E_{ij}.
\end{align*}
对$\forall A\in \mathrm{Ker}\varphi$，设$A=\sum\limits_{i,j=1}^n a_{ij}E_{ij}$，则
\begin{align*}
O=\varphi(A)=\sum\limits_{i,j=1}^n a_{ij}\varphi(E_{ij})=\sum\limits_{i,j=1}^n a_{ij}A_{ij}=\sum\limits_{i,j=1}^n a_{ij}\left(PE_{ij}P^{-1}\right)=P\left(\sum\limits_{i,j=1}^n a_{ij}E_{ij}\right)P^{-1}=PAP^{-1}\Longrightarrow A=O.
\end{align*}
因此$\mathrm{Ker}\varphi=\{O\}$，即$\varphi$是单射。

\item 对$\forall A,B\in M_n(\mathbb{F})$，由题设可知
\begin{align*}
f\left(A^k\right)=f^k\left(A\right),\quad \forall k\in \mathbb{N}.
\end{align*}
若$A$可逆，则
\begin{align*}
E_m=f\left(AA^{-1}\right)=f\left(A\right)f\left(A^{-1}\right)\Longrightarrow f^{-1}\left(A\right)=f\left(A^{-1}\right).
\end{align*}
进而若$A$与$B$相似，即存在可逆阵$T\in M_n(\mathbb{F})$，使得
\begin{align*}
A=T^{-1}BT\Longrightarrow T^{-1}f\left(B\right)T=f\left(T^{-1}BT\right)=f\left(A\right),
\end{align*}
即$f\left(A\right)$与$f\left(B\right)$也相似。注意到$E_{ii}\left(i=1,2,\cdots,n\right)$两两相似，故$f\left(E_{ii}\right)\left(i=1,2,\cdots,n\right)$两两相似，因此
\begin{align}
r\left(f\left(E_{11}\right)\right)=r\left(f\left(E_{ii}\right)\right),\quad i=1,2,\cdots,n.
\label{eq:rank_f_Eii_311}
\end{align}
又由\eqref{eq::iojgejjrw39392249322}式知$f\left(E_{ii}\right)=A_{ii}\left(i=1,2,\cdots,n\right)$都是幂等矩阵，且
\begin{align}
\sum\limits_{i=1}^n f\left(E_{ii}\right)=f\left(E_n\right)=E_m.
\label{eq:sum_f_Eii_312}
\end{align}
故由\cref{proposition:幂等矩阵的性质1}和\eqref{eq:rank_f_Eii_311}\eqref{eq:sum_f_Eii_312}式可得
\begin{align*}
m=\mathrm{tr}\left(E_m\right)=\mathrm{tr}\left(\sum\limits_{i=1}^n f\left(E_{ii}\right)\right)=\sum\limits_{i=1}^n \mathrm{tr}\left(f\left(E_{ii}\right)\right)=\sum\limits_{i=1}^n r\left(f\left(E_{ii}\right)\right)=nr\left(f\left(E_{11}\right)\right)\Longrightarrow n\mid m.
\end{align*}
\end{enumerate}
\end{proof}
