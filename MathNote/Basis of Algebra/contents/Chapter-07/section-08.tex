...
\begin{enumerate}[(1)]
\item $J_n(\lambda_0)$的初等因子组为$(\lambda-\lambda_0)^n$.
...
\end{pmatrix},
\end{align*}
则$J_i^k(\lambda_i)$与$J_i(\lambda_i)$相似($i=1,2,\cdots,r$),进而,$J^k$与$J$也相似.
...
\end{align*}
其中$\lambda_i\ne 0,i=1,2,\cdots,r$,
则$J_i^{-1}(\lambda_i)$与$J_i(\lambda_i)$相似($i=1,2,\cdots,r$),进而,$J^{-1}$与$J$也相似.
\end{enumerate}
\end{proposition}
\begin{proof}
\begin{enumerate}[(1)]
\item 由\reflemma{lemma:Jordan块的初等因子组}即得.
...
\item 不妨设 \(J = J_n(1)\) 只有一个 Jordan 块，则 \(J = I_n+J_0\)，其中 \(J_0 = J_n(0)\) 是特征值为 0 的 \(n\) 阶 Jordan 块。注意到
\begin{align*}
J^k=(I_n + J_0)^k
...
\item 不妨设 \(J = J_n(\lambda_0)\) 只有一个 Jordan 块，则 \(J = \lambda_0I_n+J_0\)，其中 \(\lambda_0 \ne 0\)，\(J_0 = J_n(0)\) 是特征值为 0 的 \(n\) 阶 Jordan 块。注意到
\begin{align*}
...
\begin{align*}
(J^{-1}-\lambda_0I)^{n - 1}=(-\lambda_0^2J_0+\cdots+(-1)^{n - 1}\lambda_0^nJ_0^{n - 1})^{n - 1}=\lambda_0J_0^{n - 1}\neq O.
\end{align*}
