...
\end{pmatrix}.
\end{align*}
由Cayley - Hamilton定理可得$P(F)=O$,从而$g^{(i)}(F)=O (0\leqslant slant i\leqslant slant e - 2)$,但$g^{(e - 1)}(F)=(e - 1)!P'(F)^{e - 1}$. 由于$P(\lambda)$是不可约多项式,故$(P(\lambda),P'(\lambda)) = 1$,进一步有$(P(\lambda),P'(\lambda)^{e - 1}) = 1$,从而由\refpro{proposition:g(A)可逆与A的特征多项式与极小多项式的关系}可知,$P'(F)^{e - 1}$是可逆矩阵,于是$\frac{1}{(e - 1)!}g^{(e - 1)}(F)=P'(F)^{e - 1}\neq O$,即有$g(J_e(P(\lambda)))\neq O$. 因此$J_e(P(\lambda))$的极小多项式为$P(\lambda)^e$,其不变因子组为$1,\cdots,1,P(\lambda)^e$.
