...
{\heiti 再证充分性：}用反证法，设 $\varphi$ 不可对角化，则存在 $V$ 的一组基 $\{e_1,e_2\cdots,e_n\}$，使得 $\varphi$ 在这组基下的表示矩阵为 Jordan 标准型 $J = \mathrm{diag}\{J_{r_1}(\lambda_1),\cdots,J_{r_k}(\lambda_k)\}$，其中 $r_1 > 1$。由表示矩阵的定义可得 $\varphi(e_1)=\lambda_1e_1$，$\varphi(e_2)=e_1 + \lambda_1e_2$，于是 $(\varphi - \lambda_1I_V)(e_1)=0$，$(\varphi - \lambda_1I_V)(e_2)=e_1$，从而 $\mathbb{0}\neq e_1\in\mathrm{Ker}(\varphi - \lambda_1I_V)\cap\mathrm{Im}(\varphi - \lambda_1I_V)$，这与假设矛盾。
