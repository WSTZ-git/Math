...
\begin{align*}
x_j=\sum_{i=1}^s{\left[ k_j\left( \varphi \left( x_i \right) -\lambda _ix_i \right) +\delta _{ij}I\left( x_i \right) \right]}=\sum_{i=1}^s{p_j(\varphi )x_i}=p_j(\varphi )x\in W,j=1,2,\cdots ,s.
\end{align*}
...
若$V \neq V_\lambda$,则存在$\varphi$的不变补空间$U_\lambda$使得$V = V_\lambda \oplus U_\lambda$. 现在对任何$\varphi$的不变子空间$W \subset U_\lambda$,取$\varphi$的不变补空间$U_W$使得$V = W \oplus U_W$.
