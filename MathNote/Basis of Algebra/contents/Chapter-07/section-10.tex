...
\end{align*}
因此$\lim_{k \to \infty} A^k$存在当且仅当$\lim_{k \to \infty} J_{r_i}(\lambda_i)^k (1\leqslant slant i \leqslant slant s)$都存在. 不妨取$k > n$, 由\nrefpro{proposition:Jordan块的性质}{(5)}计算可得Jordan块$J_{r_i}(\lambda_i)$的$k$次幂为
\begin{align*}
J_{r_i}(\lambda_i)^k = 
\begin{pmatrix}
...
&\overset{\text{Cauchy-Schwarz不等式}}{\geqslant slant}\sum_{i,j=1}^n{\left| a_{ij} \right|^2}+\sum_{i,j=1}^n{\left| b_{ij} \right|^2}+2\sqrt{\left( \sum_{i,j=1}^n{\left| a_{ij}b_{ij} \right|} \right) ^2}\\
&=\sum_{i,j=1}^n{\left| a_{ij} \right|^2}+\sum_{i,j=1}^n{\left| b_{ij} \right|^2}+2\sum_{i,j=1}^n{\left| a_{ij}b_{ij} \right|}\\
&=\left\| A+B \right\| ^2.
\end{align*}
故结论得证.
...
\begin{align*}
\left| \sum_{k=1}^n{a_{ik}b_{kj}} \right|^2\leqslant slant \left( \sum_{k=1}^n{a_{ik}^{2}} \right) \cdot \left( \sum_{k=1}^n{b_{kj}^{2}} \right) .
\end{align*}
从而先对$i$求和可得
\begin{align*}
\sum_{i=1}^n{\left| \sum_{k=1}^n{a_{ik}b_{kj}} \right|^2}\leqslant slant \sum_{i=1}^n{\left( \sum_{k=1}^n{a_{ik}^{2}} \right) \cdot \left( \sum_{k=1}^n{b_{kj}^{2}} \right)}=\left( \sum_{k=1}^n{b_{kj}^{2}} \right) \left( \sum_{i=1}^n{\left( \sum_{k=1}^n{a_{ik}^{2}} \right)} \right) .
\end{align*}
再对$j$求和可得
\begin{align*}
\sum_{i,j=1}^n{\left| \sum_{k=1}^n{a_{ik}b_{kj}} \right|^2}&\leqslant slant \sum_{j=1}^n{\left[ \left( \sum_{k=1}^n{b_{kj}^{2}} \right) \left( \sum_{i=1}^n{\left( \sum_{k=1}^n{a_{ik}^{2}} \right)} \right) \right]}\\
