...
\end{align*}
因此$\lim_{k \to \infty} A^k$存在当且仅当$\lim_{k \to \infty} J_{r_i}(\lambda_i)^k (1\leqslant i \leqslant s)$都存在. 不妨取$k > n$, 由\reflem{lemma:利用Jordan标准型计算复矩阵的多项式}计算可得Jordan块$J_{r_i}(\lambda_i)$的$k$次幂为
\begin{align*}
J_{r_i}(\lambda_i)^k = 
\begin{pmatrix}
...
\item  由\reflem{lemma:利用Jordan标准型计算复矩阵的多项式}可知
\begin{align*}
