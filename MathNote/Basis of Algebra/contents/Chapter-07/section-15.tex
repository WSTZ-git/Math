...
设$B\in\mathbb{F}^{n\times n}, A\in\mathbb{F}^{m\times m}$，则矩阵方程$AX = XB$在$X\in\mathbb{F}^{m\times n}$中有秩$r$的解的充分必要条件是$A,B$至少有$r$个相同特征值(计重数).
\end{corollary}
\begin{proof}
...
\end{align*}
因为$X_{ii}$的秩小于等于其行数和列数,所以每个$X_{ii}$至多可以提供$T_i, J_i$中较小的阶数的秩，这个数字也恰好是属于$T_i, J_i$的特征值的公共个数(计重数).因此$r(X)$必小于等于$A,B$的公共特征值的个数.这就证明了$AX = XB$在$X\in\mathbb{C}^{n\times m}$中有秩$r$的解的充分必要条件是$A,B$至少有$r$个相同特征值(计重数). 
