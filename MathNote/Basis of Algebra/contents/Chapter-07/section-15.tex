...
\end{align*}
这里$J_i, T_j, 1\leqslant slant i\leqslant slant s, 1\leqslant slant j\leqslant slant t$都是相同特征值对应所有Jordan块构成的分块对角矩阵.
注意到
\begin{align*}
...
\end{align*}
这里$J_i, T_j, 1\leqslant slant i\leqslant slant s, 1\leqslant slant j\leqslant slant t$都是相同特征值对应所有Jordan块构成的分块对角矩阵. 不妨设$0\leqslant slant s_0\leqslant slant\min\{s, t\}$使得$J_1, J_2, \cdots, J_{s_0}$分别和$T_1, T_2, \cdots, T_{s_0}$特征值对应相同，且$A, B$剩下的每一个块对应特征值都不相同.

为了解$AX = XB$，把$X$对应分块为$(X_{ij}), 1\leqslant slant i\leqslant slant s, 1\leqslant slant j\leqslant slant t$，于是$AX = XB$等价于
\begin{align}
J_iX_{ij} = X_{ij}T_j, 1\leqslant slant i\leqslant slant s, 1\leqslant slant j\leqslant slant t. \label{23.13}
\end{align}
注意到由\refpro{proposition:AX-XB线性映射相关等价条件--(1)}，我们知道\eqref{23.13}中只有$1\leqslant slant i = j\leqslant slant s_0$时$X_{ij}$才能非$0$. 于是记$AX = XB$解空间为$W$，我们有
\begin{align}
\dim W = \sum_{i = 1}^{s_0}\dim\{Y : J_iY = YT_i\}. \label{23.14}
\end{align}
对固定的$1\leqslant slant i\leqslant slant s_0$，为了计算$\dim\{Y : J_iY = YT_i\}$，我们设
\begin{align*}
...
做对应分块
\begin{align*}
Y = (Y_{i'j'}), 1\leqslant slant i'\leqslant slant s', 1\leqslant slant j'\leqslant slant t'.
\end{align*}
同样的$J_iY = YT_i$等价于
\begin{align}
J_{n_{i'}}(\lambda)Y_{i'j'} = Y_{i'j'}J_{n_{j'}'}(\lambda), \forall 1\leqslant slant i'\leqslant slant s', 1\leqslant slant j'\leqslant slant t'. \label{23.15}
\end{align}
