...
\end{proposition}
\begin{proof}
{\heiti 必要性：}设 $\varphi|_U$ 的极小多项式为 $n(\lambda)$，则 $m(\varphi|_U)=m(\varphi)|_U = \mathbb{0}$，从而 $n(\lambda)\mid m(\lambda)$。因为 $m(\lambda)$ 不可约，所以 $n(\lambda)=m(\lambda)$。又由于 $\varphi|_U$ 的所有不变因子都要整除 $m(\lambda)$ 且 $m(\lambda)$ 不可约，故所有的非常数不变因子都等于 $m(\lambda)$。最后，由\hyperref[theorem:有理标准型的几何意义]{有理标准型的几何意义}即得
\begin{align*}
U = C(\varphi,\alpha_1)\oplus C(\varphi,\alpha_2)\oplus\cdots\oplus C(\varphi,\alpha_k).
\end{align*}
...
{\heiti 充分性：}用反证法，设 $m(\lambda)=g(\lambda)h(\lambda)$，其中 $g(\lambda),h(\lambda)$ 是 $\mathbb{K}$ 上次数小于 $m(\lambda)$ 次数的首一多项式，则 $\mathbb{0}=m(\varphi)=g(\varphi)h(\varphi)$，故由\hyperref[proposition:无限维线性空间的可逆线性变换充要条件1]{命题\ref{proposition:无限维线性空间的可逆线性变换充要条件1}(1)}的逆否命题可知 $g(\varphi),h(\varphi)$ 中至少有一个是奇异线性变换，不妨设为 $g(\varphi)$，于是 $\mathrm{Ker}g(\varphi)\neq \{0\}$。任取 $\mathrm{Ker}g(\varphi)$ 中的非零向量 $\alpha$，得到循环子空间 $U = C(\varphi,\alpha)$，
由 $g(\varphi)(\alpha)=\mathbb{0}$可知
\begin{align*}
g(\varphi )(\varphi ^k\left( \alpha \right) )=\varphi ^k\left( g\left( \varphi \right) \left( \alpha \right) \right) =0,\quad k=0,1,2,\cdots .
\end{align*}
因此$g(\varphi|_U)=g(\varphi)|_U = \mathbb{0}$，于是 $\varphi|_U$ 的极小多项式$m(\lambda)$整除 $g(\lambda)$，从而其次数 $\leqslant \deg g(\lambda)<\deg m(\lambda)$，这与条件矛盾！
...
\begin{proposition}[迹0矩阵的性质]\label{proposition:迹零阵必相似于主对角全为零的矩阵}
\begin{enumerate}[(1)]
