...
\end{corollary}
\begin{proof}
首先,由\reftheorem{theorem:相抵的lamda矩阵有相同的行列式因子和不变因子}可知，$A(\lambda)$ 与 $\varLambda$ 有相同的不变因子。再由\refproposition{proposition:相抵标准型的行列式因子和不变因子}可知，$\varLambda$ 的不变因子为 $d_1(\lambda),$ $d_2(\lambda),$ $\cdots,$ $d_r(\lambda)$，从而它们也是 $A(\lambda)$ 的不变因子。故$A(\lambda)$的法式可以唯一确定其不变因子.

接着,设$A(\lambda)$ 的不变因子为 $d_1(\lambda),d_2(\lambda),\cdots,d_r(\lambda)$,由\reftheorem{theorem:lambda矩阵相抵于特殊的对角阵},可设$A(\lambda)$相抵于对角阵
\begin{align}
B(\lambda)=\mathrm{diag}\{d_1'(\lambda),d_2'(\lambda),\cdots,d_r'(\lambda);0,\cdots,0\},
\end{align}
其中$d_i'(\lambda)$是非零首一多项式且$d_i'(\lambda)\mid d_{i + 1}'(\lambda)$ ($i = 1,2,\cdots,r - 1$)。再由\refproposition{proposition:相抵标准型的行列式因子和不变因子}可知，$B(\lambda)$的不变因子为 $d_1'(\lambda),$ $d_2'(\lambda),$ $\cdots,$ $d_r'(\lambda)$.由\reftheorem{theorem:相抵的lamda矩阵有相同的行列式因子和不变因子}可知，$A(\lambda)$和$B(\lambda)$的不变因子相同,故根据$d_i(\lambda),d'_i(\lambda)$的整除关系,我们就有
\begin{align*}
...
\end{corollary}
\begin{proof}
若 $A(\lambda)$ 与 $B(\lambda)$ 有相同的法式，显然它们相抵。若 $A(\lambda)$ 与 $B(\lambda)$ 相抵，由\reftheorem{theorem:相抵的lamda矩阵有相同的行列式因子和不变因子} 知 $A(\lambda)$ 与 $B(\lambda)$ 有相同的不变因子，从而由\refcorollary{corollary:不变因子和法式相互唯一确定}可知, $A(\lambda)$ 与 $B(\lambda)$ 有相同的法式。
\end{proof}
...
\end{corollary}
\begin{proof}
设 $\varLambda_1, \varLambda_2$ 是 $A(\lambda)$ 通过不同的初等变换得到的两个法式，则 $\varLambda_1$ 与 $\varLambda_2$ 相抵，由\refcorollary{corollary:相抵当且仅当它们有相同的法式}可得 $\varLambda_1 = \varLambda_2$。
\end{proof}
...
\end{theorem}
\begin{proof}
显然不变因子与行列式因子之间相互唯一确定。再由\reftheorem{theorem:矩阵相似的充要条件是对应的lamda-矩阵相抵}、\refcorollary{corollary:相抵当且仅当它们有相同的法式} 及\refcorollary{corollary:不变因子和法式相互唯一确定}即得结论。
\end{proof}
...
\end{align*}
即 $\lambda I - A$ 与 $\lambda I - B$ 在 $\mathbb{F}$ 上相抵，由\reftheorem{theorem:矩阵相似的充要条件是对应的lamda-矩阵相抵} 可得 $A$ 与 $B$ 在 $\mathbb{F}$ 上相似。
\end{proof}
