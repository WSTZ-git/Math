...
\end{corollary}
\begin{proof}
首先,由\refthe{theorem:相抵的lamda矩阵有相同的行列式因子和不变因子}可知，$A(\lambda)$ 与 $\varLambda$ 有相同的不变因子。再由\refpro{proposition:相抵标准型的行列式因子和不变因子}可知，$\varLambda$ 的不变因子为 $d_1(\lambda),$ $d_2(\lambda),$ $\cdots,$ $d_r(\lambda)0,\cdots,0$，从而它们也是 $A(\lambda)$ 的不变因子。故$A(\lambda)$的法式可以唯一确定其不变因子.
