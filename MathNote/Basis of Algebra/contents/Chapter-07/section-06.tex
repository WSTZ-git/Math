...
\end{theorem}
\begin{proof}
由\reftheorem{theorem:矩阵相似关于不变因子和行列式因子的充要条件}可知,矩阵$A$和$B$相似等价于$A$和$B$有相同的不变因子.又由\refproposition{proposition:初等因子组与不变因子组相互唯一确定}可知,$A$和$B$有相同的不变因子等价于它们有相同的初等因子组.故矩阵 $A$ 与 $B$ 相似的充分必要条件是它们有相同的初等因子组.
\end{proof}
