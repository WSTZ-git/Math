...
\end{example}
\begin{proof}
由\refthe{theorem:循环子空间的刻画}可知，$\mathbb{K}^n$ 是关于 $A$ 的循环空间.或由\refpro{proposition:特征多项式在数域K上的不可约分解则在复数域上可对角化}知$A$的特征多项式等于极小多项式.再由\refthe{theorem:极小多项式等于特征多项式的充要条件}即得结论。
...
\end{example}
\begin{proof}
题目中的 $A$ 是类下三角矩阵，上次对角元全部非零，比如 Frobenius 块、Jordan 块和三对角矩阵都满足这样的特点。考虑特征矩阵 $\lambda\boldsymbol{I}_n - A$ 的前 $n - 1$ 行、后 $n - 1$ 列构成的下三角行列式，其值为 $(-1)^{n - 1}b_1\cdots b_{n - 1}\neq 0$，于是$A$的第$n-1$个行列式因子为$d_{n-1}=1$,故 $A$ 的行列式因子组为 $1,\cdots,1,f(\lambda)$，于是$A$的极小多项式等于特征多项式.从而 $\mathbb{K}^n$ 是关于 $A$ 的循环空间，再由\refthe{theorem:极小多项式等于特征多项式的充要条件}即得结论.
