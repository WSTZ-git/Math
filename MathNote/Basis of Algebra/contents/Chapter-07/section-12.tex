...
\begin{theorem}\label{theorem:循环子空间的另一种刻画}
...
\end{theorem}
\begin{note}
\refthe{theorem:F(A)=C(A)的充要条件和不变子空间刻画}证明了：线性变换 $\varphi$ 的极小多项式等于其特征多项式当且仅当 $V$ 是关于 $\varphi$ 的循环空间。因此，作为\hyperref[theorem:F(A)=C(A)的充要条件和不变子空间刻画]{定理\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画}\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画-8}\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画-9}}的推论，我们给出了循环空间的另一刻画.
\end{note}
\begin{proof}
由\hyperref[theorem:F(A)=C(A)的充要条件和不变子空间刻画]{定理\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画}\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画-8}\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画-9}}及$f(\varphi)$全都与$\varphi$可交换以及\refthe{theorem:F(A)=C(A)的充要条件和不变子空间刻画}立得.
\end{proof}
...
\end{example}
\begin{proof}
由\refthe{theorem:F(A)=C(A)的充要条件和不变子空间刻画}可知，$\mathbb{K}^n$ 是关于 $A$ 的循环空间.或由\refpro{proposition:特征多项式在数域K上的不可约分解则在复数域上可对角化}知$A$的特征多项式等于极小多项式.再由\hyperref[theorem:F(A)=C(A)的充要条件和不变子空间刻画]{定理\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画}\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画-8}\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画-9}}即得结论。
\end{proof}
...
\end{example}
\begin{proof}
若 $A$ 的极小多项式等于特征多项式，则由\hyperref[theorem:F(A)=C(A)的充要条件和不变子空间刻画]{定理\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画}\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画-8}\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画-9}}可知 $C(A)=\mathbb{K}[A]$。若极小多项式不等于特征多项式，则极小多项式必为一次多项式 $x - c$，从而 $A = c\boldsymbol{I}_2$，于是 $C(A)=M_2(\mathbb{K})$。
\end{proof}
...
\end{example}
\begin{proof}
题目中的 $A$ 是类下三角矩阵，上次对角元全部非零，比如 Frobenius 块、Jordan 块和三对角矩阵都满足这样的特点。考虑特征矩阵 $\lambda\boldsymbol{I}_n - A$ 的前 $n - 1$ 行、后 $n - 1$ 列构成的下三角行列式，其值为 $(-1)^{n - 1}b_1\cdots b_{n - 1}\neq 0$，于是$A$的第$n-1$个行列式因子为$d_{n-1}=1$,故 $A$ 的行列式因子组为 $1,\cdots,1,f(\lambda)$，于是$A$的极小多项式等于特征多项式.从而 $\mathbb{K}^n$ 是关于 $A$ 的循环空间，再由\hyperref[theorem:F(A)=C(A)的充要条件和不变子空间刻画]{定理\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画}\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画-8}\ref{theorem:F(A)=C(A)的充要条件和不变子空间刻画-9}}即得结论.
\end{proof}
...
\end{pmatrix}
\end{align*}
其中 $\boldsymbol{A}_i$ 和 $\boldsymbol{B}_i$ 是同阶方阵。设 $\boldsymbol{A}_i$ 适合非零多项式 $g_i(x)$，且 $g_i(x) (1\leqslant  i\leqslant  k)$ 两两互素。求证：若对每个 $i$，存在多项式 $f_i(x)$，使得 $\boldsymbol{B}_i = f_i(\boldsymbol{A}_i)$，则必存在次数不超过 $n - 1$ 的多项式 $f(x)$，使得 $B = f(A)$。
\end{proposition}
\begin{note}
