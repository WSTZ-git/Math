...
线性变换 $\varphi$ 的特征值 $\lambda_1$ 的度数等于 $\varphi$ 的 Jordan 标准型中属于特征值 $\lambda_1$ 的 Jordan 块的个数，$\lambda_1$ 的重数等于所有属于特征值 $\lambda_1$ 的 Jordan 块的阶数之和。并且属于特征值$\lambda_1$的每一个Jordan块都有且仅有一个线性无关的特征向量.
\end{theorem}
\begin{proof}
