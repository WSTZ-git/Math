...
\begin{theorem}[AB和BA的非0特征值的Jordan块完全一致]\label{theorem:AB和BA的非0特征值的Jordan块完全一致}
...
\]
从\(\eqref{eq:23.11}\)可以看到\( AB, BA \)非0特征值都集中在\( B_1 \)上,所以非0特征值完全一致.设\( \lambda \neq 0 \)是\( AB, BA \)特征值.则回忆\refpro{proposition:Jordan块的个数与秩的关系},决定\( C \) Jordan块分布,我们知道只需决定\( (\lambda E - C)^k, k \in \mathbb{N}_0 \)的秩即可.于是对每个\( k \in \mathbb{N}_0 \),我们有
\[
