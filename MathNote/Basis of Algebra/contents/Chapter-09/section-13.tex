...
\end{proposition}
\begin{remark}
这个\refpro{proposition:例9.76}也可通过与\refpro{proposition:反称矩阵A的I_n+A的行列式相关结论}和\refpro{proposition:A+S的行列式的相关结论}完全类似的讨论来得到，具体的细节留给读者完成. 另外，利用摄动法可将这个\refpro{proposition:例9.76}推广到两个矩阵都是半正定阵的情形. 设\(A\)，\(B\)都是\(n\)阶半正定实对称矩阵，则对任意的正实数\(t\)，\(A + tI_n\)是正定阵，因此由这个\refpro{proposition:例9.76}可得\(\vert A + tI_n + B\vert\geq\vert A + tI_n\vert+\vert B\vert\)，令\(t\to0^+\)即得\(\vert A + B\vert\geq\vert A\vert+\vert B\vert\). 当然，也可以分情况讨论来证明. 若\(\vert A\vert=\vert B\vert = 0\)，则结论显然成立；若\(\vert A\vert>0\)或\(\vert B\vert>0\)，则\(A\)或\(B\)正定，直接利用这个\refpro{proposition:例9.76}即得结论.
\end{remark}
\begin{proof}
...
\end{align*}
由此即得\(AA' - M\)是半正定阵. 再由\refpro{proposition:例9.76}的半正定版本或\refpro{example:例9.80}即得\(\vert M\vert\leq\vert AA'\vert\).
\end{proof}
