...
$\mathbf{Step}3$ 若 $A$ 有特征值 $a + bi,a \in \mathbb{R},b \in \mathbb{R} \setminus \{0\}$. 则存在不同时为 0 的 $\beta,\eta \in \mathbb{R}^n$ 使得
\begin{align}
...
$\mathbf{Step}4$  由\reflem{lemma:正规矩阵与其转置有相同特征向量},我们知道 $A^T(\beta + i\eta) = (a - bi)(\beta + i\eta)$,于是类似\eqref{eq:2325y5h345634t.22234820uu0358023r2312r}可得
\begin{align*}
\begin{cases} 
A\beta = a\beta - b\eta \\
A\eta = a\eta + b\beta 
...
\begin{example}
设$A, B$为 2026 阶实反对称矩阵，证明：
$AB$ 的特征多项式至少有一个二重根.
\end{example}
\begin{proof}
记 \(n=2026\). 反证，假设 \(AB\) 的特征多项式只有一重根，记 \(AB\) 的全体特征值为 \(\lambda_1,\cdots,\lambda_n\)，则
\begin{align}
r(\lambda_i I - AB) = n-1 = 2025,\quad i=1,2,\cdots,n. \label{eq:p1_r12342233}
\end{align}
\begin{enumerate}[(1)]
\item 当 \(AB\) 不可逆时，由 \eqref{eq:p1_r12342233} 式知 \(r(AB)=2025\). 由\hyperref[proposition:矩阵秩的基本公式-1]{矩阵秩的基本公式\ref{proposition:矩阵秩的基本公式-1}}和\hyperref[proposition:Sylvester(西尔维斯特)不等式]{Sylvester不等式}知
\begin{align}
r(A)+r(B)-n \leqslant r(AB) = 2025 \leqslant \min\{r(A), r(B)\}. \label{eq:p1_r12342233ank1}
\end{align}
由\refpro{proposition:反对称阵的秩必为偶数}可知 \(r(A), r(B)\) 均为偶数，且 \(r(A), r(B) \leqslant 2026\)，所以
\begin{enumerate}[(i)]
\item 若 \(r(A)=r(B)=2026\)，则由 \eqref{eq:p1_r12342233ank1} 式知 \(2026 = r(A)+r(B)-n \leqslant 2025\)，矛盾！
\item 若 \(r(A)<2026\) 或 \(r(B)<2026\)，则 \(r(A) \leqslant 2024\) 或 \(r(B) \leqslant 2024\). 再结合 \eqref{eq:p1_r12342233ank1} 式知 \(2025 \leqslant \min\{r(A), r(B)\} \leqslant 2024\)，矛盾！
\end{enumerate}

\item 当 \(AB\) 可逆时，由\hyperref[proposition:矩阵秩的基本公式-1]{矩阵秩的基本公式\ref{proposition:矩阵秩的基本公式-1}}和\hyperref[proposition:Sylvester(西尔维斯特)不等式]{Sylvester不等式}知
\begin{align}
r(AB) = 2026 \leqslant \min\{r(A), r(B)\} \Longrightarrow r(A), r(B) = 2026. \label{eq:p1_r12342233ank2}
\end{align}
故此时 \(A, B\) 都可逆. 对任意固定的 \(i \in \{1, 2, \cdots, n\}\)，有
\begin{align*}
\lambda_i I - AB = A(\lambda_i A^{-1} - B).
\end{align*}
记 \(C_i \triangleq \lambda_i A^{-1} - B\)，则 \(C_i\) 也是反对称矩阵且 \(\lambda_i I - AB = AC_i\). 从而由 \eqref{eq:p1_r12342233} 式和\hyperref[proposition:矩阵秩的基本公式-1]{矩阵秩的基本公式\ref{proposition:矩阵秩的基本公式-1}}和\hyperref[proposition:Sylvester(西尔维斯特)不等式]{Sylvester不等式}知
\begin{align}
r(A) + r(C_i) - n \leqslant r(AC_i) = 2025 \leqslant \min\{r(A), r(C_i)\}. \label{eq:p1_c}
\end{align}
由\refpro{proposition:反对称阵的秩必为偶数}或\hyperref[theorem:反对称矩阵的合同标准型]{反对称矩阵的合同标准型}可知 \(r(C_i)\) 为偶数，结合 \eqref{eq:p1_c} 式可得 \(r(C_i) \geqslant 2026\)，即 \(r(C_i)=2026\). 代入 \eqref{eq:p1_c} 式得 \(2026+2026-2026 \leqslant 2025\)，即 \(2026 \leqslant 2025\)，矛盾！
\end{enumerate}
\end{proof}
...
\begin{align*}
C'AC &= I_n,\quad C'SC = \mathrm{diag}\left\{\begin{pmatrix}0 & b_1\\ -b_1 & 0\end{pmatrix},\cdots,\begin{pmatrix}0 & b_r\\ -b_r & 0\end{pmatrix},0,\cdots,0\right\},
\end{align*}
其中 $b_1,\cdots,b_r$ 是非零实数.
\end{proposition}
\begin{proof}
因为 $A$ 是正定阵, 故存在可逆矩阵 $P$, 使得 $P'AP = I_n$. 又矩阵 $P'SP$ 还是实反对称矩阵, 故由\hyperref[theorem:实正规矩阵的正交相似标准型]{实正规矩阵的正交相似标准型}知,存在正交矩阵 $Q$, 使得
\begin{align*}
Q'(P'SP)Q &= \mathrm{diag}\left\{\begin{pmatrix}0 & b_1\\ -b_1 & 0\end{pmatrix},\cdots,\begin{pmatrix}0 & b_r\\ -b_r & 0\end{pmatrix},0,\cdots,0\right\},
\end{align*}
