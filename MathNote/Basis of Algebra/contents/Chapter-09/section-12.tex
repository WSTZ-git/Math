...
\begin{enumerate}[(1)]
\item 设 $A\in\mathbb{R}^{n\times n}$ 是实对称矩阵，
\begin{align*}
\lambda_1(A)\geqslant \lambda_2(A)\geqslant \cdots \geqslant \lambda_n(A)
\end{align*}
是 $A$ 的特征值，则对 $k=1,2,\cdots,n$ 成立
\begin{align}
\lambda_k(A)=\max_{\stackrel{S\subseteq \mathbb{R}^n}{\dim S=k}}\min_{\stackrel{x\in S}{\|x\|=1}} x^T Ax = \min_{\stackrel{S\subseteq \mathbb{R}^n}{\dim S=n-k+1}}\max_{\stackrel{x\in S}{\|x\|=1}} x^T Ax.
\label{eq:realCF}
\end{align}

\item 设 $A\in\mathbb{C}^{n\times n}$ 是 Hermite 矩阵，
\begin{align*}
\lambda_1(A)\geqslant \lambda_2(A)\geqslant \cdots \geqslant \lambda_n(A)
\end{align*}
是 $A$ 的特征值，则对 $k=1,2,\cdots,n$ 成立
...
\end{theorem}
\begin{proof}
设 $\lambda_j \triangleq \lambda_j(A), j=1,2,\cdots,n$，考虑 $A$ 我们只需证明 \eqref{eq:CF} 中第一个等号。设标准正交向量组 $u_1,u_2,\cdots,u_n$ 是从属于 $A$ 特征值 $\lambda_1,\lambda_2,\cdots,\lambda_n$ 的特征向量。

对于子空间 $S\subseteq \mathbb{C}^n, \dim S=k$，我们有
\begin{align*}
\dim S + \dim \operatorname{span}\{u_k,u_{k+1},\cdots,u_n\} = n+1 \Rightarrow S\cap \operatorname{span}\{u_k,u_{k+1},\cdots,u_n\} \neq \{0\},
\end{align*}
于是可取 $x=\sum_{j=k}^n c_j u_j \in S$ 使得
\begin{align*}
\sum_{j=k}^n |c_j|^2 = 1.
\end{align*}
现在
\begin{align*}
x^*Ax = \left(\sum_{j=k}^n c_j \lambda_j u_j, \sum_{j=k}^n c_j u_j\right) = \sum_{j=k}^n |c_j|^2 \lambda_j \leqslant \sum_{j=k}^n |c_j|^2 \lambda_k = \lambda_k,
\end{align*}
即
\begin{align}
S\subseteq \mathbb{C}^n, \dim S=k \Rightarrow x^*Ax \leqslant \lambda_k,
\label{eq:21_17}
\end{align}
取 $S_0=\operatorname{span}\{u_1,u_2,\cdots,u_k\}$ 和 $x=\sum_{j=1}^k c_j u_j \in S_0$，我们有
\begin{align*}
x^*Ax = \sum_{j=1}^k |c_j|^2 \lambda_j.
\end{align*}
容易看到
\begin{align*}
\min_{x\in S_0} \sum_{j=1}^k |c_j|^2 \lambda_j = \lambda_k.
\end{align*}
于是我们结合 \eqref{eq:21_17}式即得 \eqref{eq:CF}式.
\end{proof}

\begin{theorem}[Cauchy交错定理]\label{theorem:Cauchy交错定理}
设 Hermite 矩阵 $A\in\mathbb{C}^{n\times n}$ 的特征值为 $\lambda_1\geqslant \lambda_2\geqslant \cdots \geqslant \lambda_n$。$B$ 是 $A$ 的任意 $m$ 阶主子式，其特征值为
\begin{align*}
...
\end{align*}
由极大极小原理 \eqref{eq:CF}，对每一个 $1\leqslant j\leqslant m$，存在子空间 $S\subseteq \mathbb{C}^m$ 使得
\begin{align*}
...
对 $-A, -B$ 用此结论，我们有
\begin{align*}
-\lambda_{j+n-m} \geqslant -\mu_j \Rightarrow \mu_j \geqslant \lambda_{j+n-m}, \quad j=1,2,\cdots,m.
\end{align*}
现在我们证明了 \eqref{eq:Cauchy}.
\end{proof}
