...
于是对 \( x \neq 0 \), 就有
\[
\frac{x^T A x}{x^T x} = \frac{x^T T^T A T x}{x^T T^T T x} = \frac{y^T A  y}{y^T y} = \frac{\sum\limits_{i=1}^n \lambda_i y_i^2}{\sum\limits_{i=1}^n y_i^2}
\begin{cases} 
...
\end{cases},
\]
并且上式等号成立当且仅当$y=Tx$是$\lambda_{\text{max}}$或$\lambda_{\text{min}}$的特征向量.这就证明了\eqref{eq::::::----89347950-eq:::dfijiowimotio300-=23=2=34243}式.
...
f(X) = X^T A X.
\end{align*}
若存在实 $n$ 维向量 $X_1, X_2$, 使
\begin{align*}
f(X_1) > 0, \quad f(X_2) < 0,
\end{align*}
证明: 存在实 $n$ 维非零向量 $X_0$, 使
\begin{align*}
f(X_0) = 0.
\end{align*}
\end{example}
\begin{proof}
由\cref{proposition:Rayleigh-quotient瑞丽商的基本性质}知$A$的最大,最小特征值分别满足
\begin{align*}
\lambda_{\max}=\max\limits_{X\ne 0}\frac{X^TAX}{X^TX}\geqslant \frac{X_1^TAX_1}{X_1^TX_1}=\frac{f(X_1)}{X_1^TX_1}>0, \\
\lambda_{\min}=\min\limits_{X\ne 0}\frac{X^TAX}{X^TX}\geqslant \frac{X_2^TAX_2}{X_2^TX_2}=\frac{f(X_2)}{X_2^TX_2}>0.
\end{align*}
记 $x_1, x_2$ 分别为 $\lambda_{\max}, \lambda_{\min}$ 对应的单位特征向量, 则
\begin{align*}
x_1^TAx_1=\lambda_{\max}x_1^Tx_1=\lambda_{\max},\quad x_2^TAx_2=\lambda_{\min}x_2^Tx_2=\lambda_{\min}.
\end{align*}
并且由\cref{corollary:Hermite矩阵和实对称矩阵关于特征值的相关性质}知 $x_1, x_2$ 正交. 取 $X_0=\sqrt{-\lambda_{\min}}x_1+\sqrt{\lambda_{\max}}x_2$, 则
\begin{align*}
f(X_0) &= X_0^TAX_0=-\lambda_{\min}x_1^TAx_1+\lambda_{\max}x_2^TAx_2+\sqrt{-\lambda_{\min}\lambda_{\max}}x_1^TAx_2+\sqrt{-\lambda_{\min}\lambda_{\max}}x_2^TAx_1 \\
&= -\lambda_{\min}\lambda_{\max}+\lambda_{\max}\lambda_{\min}=0.
\end{align*}\end{proof}
...
\end{align*}
综上即得\eqref{eq:Cauchy}式。\end{proof}
