...
\begin{theorem}\label{theorem:有限维空间伴随必存在且唯一}
...
\end{theorem}
\begin{remark}
上述结论对$V_1$到$V_2$的线性映射$\varphi $也成立.
\end{remark}
\begin{proof}
设 $\alpha=a_1e_1+\cdots+a_ne_n$, $\beta=b_1e_1+\cdots+b_ne_n$, 记 $x=(a_1,\cdots,a_n)^T$, $y=(b_1,\cdots,b_n)^T$, 则
\begin{align*}
(A(\alpha),\beta)=(Ax)^T\overline{y}=x^TA^T\overline{y}=x^T\overline{(\overline{A}^Ty)}=(\alpha,\overline{A}^Ty).
\end{align*}
故由 $\overline{A}^T$ 左乘诱导的 $V$ 上的线性变换也是 $\varphi$ 的伴随算子. 再由\refthe{theorem:有限维空间伴随必存在且唯一}知, $\varphi^*$ 的表示矩阵就是 $\overline{A}^T$.
