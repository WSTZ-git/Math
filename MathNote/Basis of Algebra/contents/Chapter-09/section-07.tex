...
首先验证 对$\forall n\in \mathbb{N}$,\( p_0, p_1, \cdots, p_n \) 构成空间 \( \mathbb{R}[x]_{\leqslant n} \) 的一组正交基. 由题设,当 \( i \neq j \) 时,
\begin{align*}
(p_i, p_j) = 0,
\end{align*}
...
\end{align*}
从而 \( a_j = 0 \). 故 \( p_0, p_1, \cdots, p_n \) 线性无关. 又因 \( \deg p_i = i \),每个 \( p_i \in \mathbb{R}[x]_{\leqslant n} \),且该空间维数为 \( n+1 \),所以它们构成一组基;再加上两两正交,即为正交基.
