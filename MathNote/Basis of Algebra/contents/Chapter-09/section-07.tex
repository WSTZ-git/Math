...
\end{proposition}
\begin{proof}
容易验证 $(f(x),g(x)) = \int_{0}^{1}f(x)g(x)dx$，故由\hyperref[example:常见内积和内积空间-例9.1]{例题\ref{example:常见内积和内积空间-例9.1}(5)}即得结论。因为 $1,x,\cdots,x^{n - 1}$ 是 $V$ 中一组线性无关的向量，所以由\refpro{theorem:度量矩阵的性质} 知其 Gram 矩阵 $A = \left(\frac{1}{i + j - 1}\right)_{n\times n}$ 是一个正定阵，这也给出了\nrefexa{example:例8.48}{(2)}的几何证明.
\end{proof}
