...
\end{proposition}
\begin{proof}
用反证法证明。假设存在 $n + 2$ 个两两夹角大于直角的向量 $\alpha_1,\alpha_2,\cdots,\alpha_{n + 1},\alpha_{n + 2}\in V$，则由 $\dim V = n$ 可知，$\alpha_1,\alpha_2,\cdots,\alpha_{n + 1}$ 必线性相关，即存在不全为零的实数 $c_1,c_2,\cdots,c_{n + 1}$，使得 $c_1\alpha_1 + c_2\alpha_2 + \cdots + c_{n + 1}\alpha_{n + 1} = 0$。将此式按照系数正负整理为如下形式：
\begin{align}
...
\begin{enumerate}[(1)]
\item $\alpha_1,\alpha_2,\cdots,\alpha_{n + 1}$ 中任意 $n$ 个向量必线性无关；

\item $\alpha_1,\alpha_2,\cdots,\alpha_{n + 1}$ 中任一向量必为其余向量的负系数线性组合。
\end{enumerate}
\end{corollary}
\begin{proof}
\begin{enumerate}[(1)]
\item 反证，设$\alpha_1,\alpha_2,\cdots,\alpha_n$线性相关，则存在不全为零的实数$c_1,c_2,\cdots,c_n$，使得
\begin{align}
c_1\alpha_1+c_2\alpha_2+\cdots+c_n\alpha_n=0.\label{eq::::::--3928htrh21.0}
\end{align}
将上式按照系数正负整理得
\begin{align}
\sum\limits_{c_i>0}c_i\alpha_i=\sum\limits_{c_j<0}(-c_j)\alpha_j.\label{eq::::::--3928htrh21.1}
\end{align}
若\eqref{eq::::::--3928htrh21.1}式两边都等于零，则
\begin{align*}
0=\left(\sum\limits_{c_i>0}c_i\alpha_i,\alpha_{n+1}\right)=\sum\limits_{c_i>0}c_i(\alpha_i,\alpha_n)<0,
\end{align*}
矛盾！故\eqref{eq::::::--3928htrh21.1}式两边都不为0，因此\eqref{eq::::::--3928htrh21.0}式不可能只有正系数或负系数，
于是存在某个$c_i>0$,$c_j<0$，进而
\begin{align*}
0<\left(\sum\limits_{c_i>0}c_i\alpha_i,\sum\limits_{c_i>0}c_i\alpha_i\right)=\left(\sum\limits_{c_i>0}c_i\alpha_i,\sum\limits_{c_j<0}(-c_j)\alpha_j\right)=\sum\limits_{c_i>0}\sum\limits_{c_j<0}c_i(-c_j)(\alpha_i,\alpha_j)<0,
\end{align*}
矛盾！故结论得证。

\item 反证，设$\alpha_{n+1}=c_1\alpha_1+\cdots+c_n\alpha_n$，其中$c_1>0$。记$c_{n+1}=-1<0$，则
\begin{align*}
c_1\alpha_1+\cdots+c_n\alpha_n+c_{n+1}\alpha_{n+1}=0.
\end{align*}
将上式按照系数正负整理得
\begin{align}
\sum\limits_{c_i>0}c_i\alpha_i=\sum\limits_{c_j<0}(-c_j)\alpha_j.\label{eq:------sefgfhrtj:::::--3928htrh21.0}
\end{align}
若\eqref{eq:------sefgfhrtj:::::--3928htrh21.0}式两边都为0，由$\alpha_{n+1}$的系数$c_{n+1}<0$及条件可得
\begin{align*}
0=\left(\sum\limits_{c_i>0}c_i\alpha_i,\alpha_{n+1}\right)=\sum\limits_{c_i>0}c_i\left(\alpha_i,\alpha_{n+1}\right)<0,
\end{align*}
矛盾！故\eqref{eq:------sefgfhrtj:::::--3928htrh21.0}式两边都非零。于是
\begin{align*}
0<\left(\sum\limits_{c_i>0}c_i\alpha_i,\sum\limits_{c_i>0}c_i\alpha_i\right)=\left(\sum\limits_{c_i>0}c_i\alpha_i,\sum\limits_{c_j<0}(-c_j)\alpha_j\right)=\sum\limits_{c_i>0}\sum\limits_{c_j<0}c_i(-c_j)\left(\alpha_i,\alpha_j\right)<0,
\end{align*}
矛盾！故结论得证。
\end{enumerate}
\end{proof}
...
\end{cases}
\]
注意到上述方程组的系数矩阵是基 $\{e_1,e_2,\cdots,e_n\}$ 的 Gram 矩阵，即系数矩阵是度量矩阵,从而系数矩阵是正定矩阵,故其行列式非零，从而上述方程组有唯一解，于是满足条件的 $\alpha$ 存在且唯一。\end{proof}

\begin{proposition}[Parseval恒等式]\label{proposition:Parseval恒等式}
设 $V$ 是 $n$ 维欧氏空间，$\alpha_1, \alpha_2, \cdots, \alpha_n \in V$. 证明：$\alpha_1, \alpha_2, \cdots, \alpha_n$ 是 $V$ 的标准正交基的充要条件是对任意的 $\alpha \in V$，有
\begin{align}
(\alpha, \alpha) = \sum_{i=1}^n (\alpha, \alpha_i)^2. \label{ex8p14eq}
\end{align}
\end{proposition}
\begin{proof}
{\heiti 必要性:} 若 $\alpha_1, \cdots, \alpha_n$ 是标准正交基，则对任意 $\alpha \in V$，有展开式
\begin{align*}
\alpha = \sum_{i=1}^n (\alpha, \alpha_i)\alpha_i.
\end{align*}
于是
\begin{align*}
(\alpha, \alpha) = \left(\sum_{i=1}^n (\alpha, \alpha_i)\alpha_i, \sum_{j=1}^n (\alpha, \alpha_j)\alpha_j\right)
= \sum_{i=1}^n (\alpha, \alpha_i)^2,
\end{align*}
故恒等式 \eqref{ex8p14eq} 成立.

{\heiti 充分性:} 记$W=\text{span}\{\alpha_1,\cdots,\alpha_n\}$,对$\forall \beta \in W^\bot$ ,则 $(\beta, \alpha_i)=0\ (i=1,\cdots,n)$. 代入 \eqref{ex8p14eq} 得
\begin{align*}
(\beta, \beta) = \sum_{i=1}^n (\beta, \alpha_i)^2 = 0.
\end{align*}
由内积正定性得 $\beta=0$. 因此$W^\bot=\{0\}$，故$W=V$. 又因 $\dim V=n$，所以它们构成$V$的一组基. 令其 Gram 矩阵为
\begin{align*}
G = \bigl((\alpha_i, \alpha_j)\bigr)_{n\times n},
\end{align*}
则 $G$ 可逆.
对任意 $x=(x_1,\cdots,x_n)^T\in \mathbb R^n$，令 $\alpha=\sum_{j=1}^n x_j\alpha_j$. 代入 \eqref{ex8p14eq}，左边为
\begin{align*}
(\alpha, \alpha) = \sum_{i=1}^n\sum_{j=1}^n x_i x_j (\alpha_i, \alpha_j) = x^T G x.
\end{align*}
右边为
\begin{align*}
\sum_{i=1}^n{(\alpha ,\alpha _i)^2}=\sum_{i=1}^n{\left( \sum_{j=1}^n{x_j(\alpha _j,\alpha _i)} \right) ^2}=\sum_{i=1}^n{(Gx)_{i}^{2}}=\left( Gx \right) ^T\left( Gx \right) =x^TG^2x.
\end{align*}
因此对任意 $x\in\mathbb R^n$，有
\begin{align}
x^T (G-G^2)x = 0. \label{Gdiff}
\end{align}
由于 $G-G^2$ 是实对称矩阵，再由\cref{proposition:对称阵是零矩阵的充要条件}可得 $G-G^2=0$，即 $G=G^2$. 又 $G$ 可逆，故 $G=I$. 因此$\alpha_1, \cdots, \alpha_n$ 是标准正交基.
\end{proof}

\begin{example}
设 \( V = \mathbb{R}[x] \) 为欧氏空间,其中内积为
\begin{align*}
(f(x), g(x)) = \int_0^1 f(x)g(x) \, \mathrm{d}x, \quad f(x), g(x) \in V.
\end{align*}
设 \( p_i(x) \) (\( i = 0, 1, 2, \cdots \)) 为 \( i \) 次首一实系数多项式,且满足 \((p_i(x), p_j(x)) = 0\) (\( i \neq j \)). 证明:对任意的正整数 \( m \), \( x p_m(x) \) 可由 \( p_{m-1}(x), p_m(x), p_{m+1}(x) \) 线性表出.
\end{example}
\begin{proof}

首先验证 对$\forall n\in \mathbb{N}$,有\( p_0, p_1, \cdots, p_n \) 构成线性空间 \( \mathbb{R}[x]_{\leqslant n} \) 的一组正交基. 由题设,当 \( i \neq j \) 时,
\begin{align*}
(p_i, p_j) = 0,
\end{align*}
且 \( p_i \) 为非零多项式（首一）,故 \((p_i, p_i) > 0\). 若
\begin{align*}
\sum_{i=0}^n a_i p_i = 0,
\end{align*}
则对任意 \( j \in \{0,1,\cdots,n\} \),取内积与 \( p_j \),得
\begin{align*}
0 = \left( \sum_{i=0}^n a_i p_i, p_j \right) = \sum_{i=0}^n a_i (p_i, p_j) = a_j (p_j, p_j),
\end{align*}
从而 \( a_j = 0 \). 故 \( p_0, p_1, \cdots, p_n \) 线性无关.
注意到$1,x,\cdots,x^n$是$\mathbb{R}[x]_{\leqslant n}$的一组基,故$\mathbb{R}[x]_{\leqslant n}$的维数为 \( n+1 \).
又因 \( \deg p_i = i \),每个 \( p_i \in \mathbb{R}[x]_{\leqslant n} \),再加上两两正交,所以\( p_0, p_1, \cdots, p_n \)为$\mathbb{R}[x]_{\leqslant n}$的正交基.

固定正整数 \( m \),令
$q(x) = x p_m(x).$
因为 \( \deg q = m+1 \),所以 \( q \) 可唯一地表示为
\begin{align}
x p_m(x) = \sum\limits_{k=0}^{m+1} c_k p_k(x). \label{xpmrep}
\end{align}
由于 \( p_k \) 两两正交,对上式两边同时与$p_k$做内积可得系数为
\begin{align}
c_k = \frac{(x p_m, p_k)}{(p_k, p_k)}. \label{ckdef}
\end{align}
现在证明当 \( 0 \leqslant k \leqslant m-2 \) 时, \( c_k = 0 \). 由内积定义可知
\begin{align*}
(x p_m, p_k) = (p_m, x p_k).
\end{align*}
而
\begin{align*}
\deg(x p_k) = k+1 \leqslant m-1,
\end{align*}
所以 \( x p_k \) 是次数不超过 \( m-1 \) 的多项式,因而它可以由
\begin{align*}
p_0, p_1, \cdots, p_{m-1}
\end{align*}
线性表示. 又因为 \( p_m \) 与 \( p_0, p_1, \cdots, p_{m-1} \) 都正交,所以
\begin{align*}
(p_m, x p_k) = 0.
\end{align*}
因此
\begin{align*}
c_k = 0 \qquad (0 \leqslant k \leqslant m-2).
\end{align*}
于是展开式 \eqref{xpmrep}变成
\begin{align*}
x p_m(x) = c_{m-1} p_{m-1}(x) + c_m p_m(x) + c_{m+1} p_{m+1}(x).
\end{align*}
所以 \( x p_m(x) \) 可由$p_{m-1}(x),\, p_m(x),\, p_{m+1}(x)$
线性表出.
\end{proof}
