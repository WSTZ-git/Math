...
(5) 设 $V = C[a, b]$ 为闭区间 $[a, b]$ 上的连续函数全体构成的实线性空间，对任意的 $f(t), g(t) \in V$，定义 $(f(t), g(t)) = \int_{a}^{b} f(t)g(t) dt$；
...
\begin{align*}
\left(\int_{a}^{b}f(t)g(t)dt\right)^2\leq\int_{a}^{b}f(t)^2dt\int_{a}^{b}g(t)^2dt.
\end{align*} 
\end{enumerate}
\end{corollary}
