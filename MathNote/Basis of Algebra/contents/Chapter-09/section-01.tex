...
\begin{remark}
实内积空间的定义与复内积空间的定义是相容的。事实上，对一个实数 \(a\)，\(\overline{a}=a\)，故\refdefinition{definition:Euclid空间} 中的 (1) 与\refdefinition{definition:酉空间}中的 (1) 是一致的。因此，我们经常将这两种空间统称为内积空间，在某些定理的叙述及证明中也不区分它们，而统一作为复内积空间来处理。但是，需要注意的是对复内积空间，\refdefinition{definition:酉空间}中的 (1), (3) 意味着:
\begin{align*}
...
\item (1)\refdefinition{definition:Euclid空间}中的 (2), (3) 显然成立。对 (1)，注意到 \(\alpha'G\beta\) 是实数，其转置仍是它自己，而 \(G\) 是对称阵，故
\begin{align*}
...
\end{definition}
\begin{remark}
注意由\refdefinition{definition:Euclid空间}和\refdefinition{definition:酉空间}中的规则 (4) 可知，\((\alpha,\alpha)\) 总是非负实数。从长度的定义知，\(\|\alpha\| = 0\) 当且仅当 \(\alpha = 0\)。当 \(V=\mathbb{R}^n\) 且内积为标准内积时，若 \(\alpha=(x_1,x_2,\cdots,x_n)\)，则
\begin{align*}
