...
\begin{enumerate}[(1)]
\item 设 $V = \mathbb{R}^n$ 为 $n$ 维实列向量空间，$G$ 为 $n$ 阶正定实对称矩阵，对任意的 $\alpha, \beta \in V$，定义 $(\alpha, \beta) = \alpha'G\beta$；

\item 设 $V = \mathbb{R}_n$ 为 $n$ 维实行向量空间，$G$ 为 $n$ 阶正定实对称矩阵，对任意的 $\alpha, \beta \in V$，定义 $(\alpha, \beta) = \alpha G\beta'$；

\item 设 $V = \mathbb{C}^n$ 为 $n$ 维复列向量空间，$H$ 为 $n$ 阶正定 Hermite 矩阵，对任意的 $\alpha, \beta \in V$，定义 $(\alpha, \beta) = \alpha' H\overline{\beta}$；

\item 设 $V = \mathbb{C}^n$ 为 $n$ 维复行向量空间，$H$ 为 $n$ 阶正定 Hermite 矩阵，对任意的 $\alpha, \beta \in V$，定义 $(\alpha, \beta) = \alpha H\overline{\beta}'$；

\item 设 $V = C[a, b]$ 为闭区间 $[a, b]$ 上的连续函数全体构成的实线性空间，对任意的 $f(t), g(t) \in V$，定义 $(f(t), g(t)) = \int_{a}^{b} f(t)g(t) \mathrm{d}t$；

\item 设 $V = \mathbb{R}[x]$ 为实系数多项式全体构成的实线性空间，对任意的 $f(x) = a_0 + a_1x + \cdots + a_nx^n$, $g(x) = b_0 + b_1x + \cdots + b_mx^m$，定义 $(f(x), g(x)) = a_0b_0 + a_1b_1 + \cdots + a_kb_k$，其中 $k = \min\{n, m\}$；

\item\label{example:常见内积和内积空间-例9.1-7} 设 $V = M_n(\mathbb{R})$ 为 $n$ 阶实矩阵全体构成的实线性空间，对任意的 $A = (a_{ij})$, $B = (b_{ij}) \in V$，定义 $(A, B) = \mathrm{tr}(AB') = \sum_{i, j = 1}^{n} a_{ij}b_{ij}$；

\item 设 $V = M_n(\mathbb{C})$ 为 $n$ 阶复矩阵全体构成的复线性空间，对任意的 $A = (a_{ij})$, $B = (b_{ij}) \in V$，定义 $(A, B) = \mathrm{tr}(A\overline{B}') = \sum_{i, j = 1}^{n} a_{ij}\overline{b_{ij}}$。
\end{enumerate}
\end{example}
\begin{proof}
\begin{enumerate}[(1)]
...
\item 类似于 (1) 的证明可得。当 $G = I_n$ 时，上述内积称为 $\mathbb{R}_n$ 上的标准内积。

\item 首先注意到 $\overline{H}' = H$，故 $\overline{(\alpha, \beta)} = \overline{\alpha' H\overline{\beta}} = (\overline{\alpha' H\overline{\beta}})' = \beta'\overline{H}'\overline{\alpha} = \beta' H\overline{\alpha} = (\beta, \alpha)$，即得共轭对称性；其次由矩阵乘法的性质可得第一变量的线性；最后由 $H$ 的正定性可知，$(\alpha, \alpha) = \alpha' H\overline{\alpha} \geqslant  0$，且等号成立当且仅当 $\alpha = 0$，即得正定性。因此上述二元运算是 $\mathbb{C}^n$ 上的内积，称为由正定 Hermite 矩阵 $H$ 定义的内积。当 $H = I_n$ 时，上述内积称为 $\mathbb{C}^n$ 上的标准内积。

\item 类似于 (3) 的证明可得。当 $H = I_n$ 时，上述内积称为 $\mathbb{C}_n$ 上的标准内积。

\item 对称性显然成立；由积分运算的线性可得第一变量的线性；由连续函数的性质可得正定性，因此上述二元运算是 $C[a, b]$ 上的内积。

\item 容易验证对称性、第一变量的线性和正定性都成立。

\item 由求迹运算的对称性、线性和正定性即得上述二元运算的对称性、线性和正定性，因此它是 $M_n(\mathbb{R})$ 上的内积。

\item 证明是类似的。这两种由矩阵的迹定义的内积称为矩阵空间上的 \hypertarget{Frobenius 内积}{\textbf{Frobenius 内积}}。
\end{enumerate}
\end{proof}
...
\begin{example}
设$e_1,e_2,\cdots,e_n$为欧氏空间$V=\mathbb{R}^n$的一个标准正交基, $v_1,v_2,\cdots,v_n\in V$, 且满足对任意的$1\leqslant i\leqslant n$, 有
\begin{align*}
|e_i - v_i| < \frac{1}{\sqrt{n}}.
\end{align*}
证明: $v_1,v_2,\cdots,v_n$是$V$的一个基.
\end{example}
\begin{proof}
设$v_1,\cdots,v_n$线性相关, 则存在$(a_1,\cdots,a_n)\in\mathbb{R}^n\setminus\{0\}$, 使得
\begin{align*}
\sum\limits_{i=1}^n a_i v_i = 0.
\end{align*}
从而
\begin{align}
\sum\limits_{i=1}^n a_i e_i = \sum\limits_{i=1}^n a_i (e_i - v_i). \label{eq:euclid_base_identity}
\end{align}
由$\{e_1,\cdots,e_n\}$是$V$的标准正交基知
\begin{align*}
\left\| \sum\limits_{i=1}^n a_i e_i \right\| = \sqrt{\sum\limits_{i=1}^n a_i^2}.
\end{align*}
又由\hyperref[theorem:范数的基本性质]{三角不等式}和Cauchy不等式知
\begin{align*}
\left\| \sum\limits_{i=1}^n a_i (e_i - v_i) \right\| &\leqslant \sum\limits_{i=1}^n |a_i| \| e_i - v_i \| \\
&\leqslant \sqrt{\left( \sum\limits_{i=1}^n a_i^2 \right) \left( \sum\limits_{i=1}^n \| e_i - v_i \|^2 \right)} \\
&< \sqrt{\left( \sum\limits_{i=1}^n \frac{1}{n} \right) \left( \sum\limits_{i=1}^n a_i^2 \right)} \\
&= \sqrt{\sum\limits_{i=1}^n a_i^2}.
\end{align*}
再结合\eqref{eq:euclid_base_identity}式知
\begin{align*}
\sqrt{\sum\limits_{i=1}^n a_i^2} = \left\| \sum\limits_{i=1}^n a_i e_i \right\| = \left\| \sum\limits_{i=1}^n a_i (e_i - v_i) \right\| < \sqrt{\sum\limits_{i=1}^n a_i^2},
\end{align*}
矛盾!
\end{proof}
