...
设$V,U$为两个有限维线性空间,若\(\dim V = \dim U\),则线性映射\(\varphi:V\rightarrow U\)是线性同构当且仅当\(\varphi\)是单映射,也当且仅当\(\varphi\)是满映射.
\end{proposition}
\begin{proof}
由\hyperref[theorem:线性映射的维数公式]{线性映射的维数公式}容易证明.
\end{proof}
...
设\(a_0,a_1,\cdots,a_n\)是数域\(\mathbb{F}\)中\(n + 1\)个不同的数,\(b_0,b_1,\cdots,b_n\)是\(\mathbb{F}\)中任意\(n + 1\)个数,求证:必存在\(\mathbb{F}\)上次数不超过\(n\)的多项式\(f(x)\),使得\(f(a_i)=b_i(0\leqslant  i\leqslant  n)\),并将\(f(x)\)构造出来.
\end{theorem}
\begin{proof}
由\hyperref[lemma:证明Lagrange插值公式]{引理\ref{lemma:证明Lagrange插值公式}}可知映射\(\varphi\)是映上的(满射),因此存在性已经证明. 现来构造\(f(x)\). 设\(\boldsymbol{e}_i=(0,\cdots,1,\cdots,0)(1\leqslant  i\leqslant  n + 1)\)是\(\mathbb{F}\)上的\(n + 1\)维标准单位行向量. 对任意的\(0\leqslant  i\leqslant  n\),令
\[
f_i(x)=\frac{(x - a_0)\cdots(x - a_{i - 1})(x - a_{i + 1})\cdots(x - a_n)}{(a_i - a_0)\cdots(a_i - a_{i - 1})(a_i - a_{i + 1})\cdots(a_i - a_n)},
\]
则\(f_i(a_i)=1,f_i(a_j)=0(j\neq i)\),于是\(\varphi(f_i)=\boldsymbol{e}_{i + 1}(0\leqslant  i\leqslant  n)\). 再令
\[
f(x)=b_0f_0(x)+b_1f_1(x)+\cdots + b_nf_n(x),
\]
则容易验证\(\varphi(f)=(b_0,b_1,\cdots,b_n)\),即\(f(a_i)=b_i(0\leqslant  i\leqslant  n)\)成立. \end{proof}

\begin{example}
设$A$是数域$P$上的一个$n$阶方阵，$x_1,x_2,\cdots,x_n\in P$两两互异，且它们都不是$A$的特征值，证明：存在$a_1,a_2,\cdots,a_n\in P$，满足
\begin{align*}
a_1\left( A-x_1E \right) ^{-1}+a_2\left( A-x_2E \right) ^{-1}+\cdots +a_n\left( A-x_nE \right) ^{-1}=E.
\end{align*}
\end{example}
\begin{proof}令$g\left( x \right) =\prod\limits_{i=1}^n{\left( x-x_i \right)}$，则$g\left( A \right)$可逆且
\begin{align*}
g\left( A \right) ^{-1}=\prod\limits_{i=1}^n{\left( A-x_iE \right) ^{-1}}.
\end{align*}
进而
\begin{align*}
\left( A-x_iE \right) ^{-1}=g\left( A \right) ^{-1}\prod\limits_{j\ne i}{\left( A-x_jE \right)},\quad i=1,2,\cdots ,n.
\end{align*}
于是只需证明存在$a_1,a_2,\cdots ,a_n\in P$，满足
\begin{align*}
\sum\limits_{i=1}^n{a_i\prod\limits_{j\ne i}{\left( A-x_jE \right)}}=g\left( A \right).
\end{align*}

设$m\left( x \right)$为$A$的极小多项式，则由带余除法知，存在$q\left( x \right) ,r\left( x \right) \in P\left[ x \right]$，使得
\begin{align*}
g\left( x \right) =q\left( x \right) m\left( x \right) +r\left( x \right),
\end{align*}
其中$\deg r\left( x \right) <\deg m\left( x \right) \leqslant n$，进而$\deg r\left( x \right) \leqslant n-1$。由极小多项式定义知$g\left( A \right) =r\left( A \right)$，故只需存在$a_1,a_2,\cdots ,a_n\in P$，满足
\begin{align*}
\sum\limits_{i=1}^n{a_i\prod\limits_{j\ne i}{\left( A-x_jE \right)}}=r\left( A \right).
\end{align*}

现在取
\begin{align*}
a_i\triangleq \frac{r\left( x_i \right)}{\prod\limits_{j\ne i}{\left( x_i-x_j \right)}}\left( i=1,2,\cdots ,n \right),\quad h\left( x \right) \triangleq \sum\limits_{i=1}^n{a_i\prod\limits_{j\ne i}{\left( x-x_j \right)}},
\end{align*}
则$\deg h\left( x \right) =n-1$，且
\begin{align*}
h\left( x_i \right) =r\left( x_i \right),\quad i=1,2,\cdots ,n.
\end{align*}
故$h\left( x \right) \equiv r\left( x \right)$，因此
\begin{align*}
h\left( A \right) =r\left( A \right) \Longleftrightarrow \sum\limits_{i=1}^n{a_i\prod\limits_{j\ne i}{\left( A-x_jE \right)}}=r\left( A \right).
\end{align*}\end{proof}
...
反之,{\color{blue}证法一:}
若\(\boldsymbol{e}_1,\boldsymbol{e}_2,\cdots,\boldsymbol{e}_n\)和\(\boldsymbol{f}_1,\boldsymbol{f}_2,\cdots,\boldsymbol{f}_n\)是\(V\)的两组基,使得\(\varphi(\boldsymbol{e}_i)=\boldsymbol{f}_i(1\leqslant  i\leqslant  n)\),则对任意\(\boldsymbol{\alpha}\in V\),\(\boldsymbol{\alpha}=\lambda_1\boldsymbol{f}_1+\lambda_2\boldsymbol{f}_2+\cdots+\lambda_n\boldsymbol{f}_n\),有\(\varphi(\lambda_1\boldsymbol{e}_1+\lambda_2\boldsymbol{e}_2+\cdots+\lambda_n\boldsymbol{e}_n)=\boldsymbol{\alpha}\),即\(\varphi\)是满映射,从而是自同构.(我们也可以证明\(\varphi\)是单映射,从而是自同构.) 

{\color{blue}证法二:}若\(\boldsymbol{e}_1,\boldsymbol{e}_2,\cdots,\boldsymbol{e}_n\)和\(\boldsymbol{f}_1,\boldsymbol{f}_2,\cdots,\boldsymbol{f}_n\)是\(V\)的两组基,使得\(\varphi(\boldsymbol{e}_i)=\boldsymbol{f}_i(1\leqslant  i\leqslant  n)\).设从基\(\boldsymbol{e}_1,\boldsymbol{e}_2,\cdots,\boldsymbol{e}_n\)到基\(\boldsymbol{f}_1,\boldsymbol{f}_2,\cdots,\boldsymbol{f}_n\)的过渡矩阵为\(\boldsymbol{P}\),则\(\varphi\)在基\(\boldsymbol{e}_1,\boldsymbol{e}_2,\cdots,\boldsymbol{e}_n\)下的表示矩阵就是\(\boldsymbol{P}\),这是一个可逆矩阵,从而\(\varphi\)是可逆变换. 
\end{proof}
...
\end{proposition}
\begin{proof}
取\(U_1\)的一组基\(\boldsymbol{e}_1,\cdots,\boldsymbol{e}_m\),并扩张为\(V\)的一组基\(\boldsymbol{e}_1,\cdots,\boldsymbol{e}_m,\boldsymbol{e}_{m + 1},\cdots,\boldsymbol{e}_n\)；取\(U_2\)的一组基\(\boldsymbol{f}_1,\cdots,\boldsymbol{f}_m\),并扩张为\(V\)的一组基\(\boldsymbol{f}_1,\cdots,\boldsymbol{f}_m,\boldsymbol{f}_{m + 1},\cdots,\boldsymbol{f}_n\). 定义\(\varphi\)为\(V\)上的线性变换,它在基上的作用为:\(\varphi(\boldsymbol{e}_i)=\boldsymbol{f}_i(1\leqslant  i\leqslant  n)\),则由\hyperref[proposition:线性变换是可逆变换的充要条件1]{命题\ref{proposition:线性变换是可逆变换的充要条件1}}可知,\(\varphi\)是可逆线性变换,再由定义容易验证\(\varphi(U_1)=U_2\)成立. 
\end{proof}
...
{\color{blue}证法二:} 只要证明\(I_V - \varphi\)是单映射即可. 任取\(\boldsymbol{\alpha}\in\text{Ker}(I_V - \varphi)\),即\((I_V - \varphi)(\boldsymbol{\alpha}) = 0\),则\(\varphi(\boldsymbol{\alpha})=\boldsymbol{\alpha}\). 设\(m\)为正整数,使得\(\varphi^{m}(\boldsymbol{\alpha}) = 0\),则\(0=\varphi ^m(\boldsymbol{\alpha })=\boldsymbol{\alpha }\varphi ^{m-1}(\boldsymbol{\alpha })=\cdots =\boldsymbol{\alpha }^{m-1}\varphi (\boldsymbol{\alpha })=\boldsymbol{\alpha }^m\),故\(\text{Ker}(I_V - \varphi)=0\),即\(I_V - \varphi\)是单映射. 
\end{proof}
...
{\color{blue}证法二:} 若\(\boldsymbol{A}\)是不可逆矩阵,则对任意的\(n\)阶矩阵\(\boldsymbol{X}\),\(\varphi(\boldsymbol{X})=\boldsymbol{A}\boldsymbol{X}\boldsymbol{B}\)总是不可逆矩阵(行列式一定为零). 因此\(\varphi\)不可能是映
上的(可逆矩阵不在值域里但是在像空间中). 同理,若\(\boldsymbol{B}\)是不可逆矩阵,\(\varphi\)也不是映上的.

{\color{blue}证法三:}注意到$\boldsymbol{A}\otimes \boldsymbol{B}^T$为$\varphi$的表示矩阵,并且$\left| \boldsymbol{A}\otimes \boldsymbol{B}^T \right|=\left| \boldsymbol{A} \right|^n\left| \boldsymbol{B} \right|^n$立得.
\end{proof}
...
\begin{proof}
\begin{enumerate}[(1)]
\item 若\(\varphi\)和\(\psi\)都是可逆变换,则\((\psi^{-1}\varphi^{-1})(\varphi\psi)=(\varphi\psi)(\psi^{-1}\varphi^{-1}) = I_V\),\((\varphi^{-1}\psi^{-1})(\psi\varphi)=(\psi\varphi)(\varphi^{-1}\psi^{-1}) = I_V\),因此\(\varphi\psi\)和\(\psi\varphi\)都是可逆变换. 反之,若\(\varphi\psi\)和\(\psi\varphi\)都是可逆变换,则存在\(V\)上的线性变换\(\xi,\eta\),使得\(\varphi\psi\xi=\xi\varphi\psi = I_V\),\(\psi\varphi\eta=\eta\psi\varphi = I_V\). 由\(\varphi\psi\xi = I_V\)及\hyperref[proposition:线性映射是单射或满射的充要条件1]{命题\ref{proposition:线性映射是单射或满射的充要条件1}(2)}可得\(\varphi\)是满映射,由\(\eta\psi\varphi = I_V\)及\hyperref[proposition:线性映射是单射或满射的充要条件1]{命题\ref{proposition:线性映射是单射或满射的充要条件1}(1)}可得\(\varphi\)是单映射,从而\(\varphi\)是可逆变换. 同理可证\(\psi\)也是可逆变换.
