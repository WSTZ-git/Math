...
\]
其中\(v_k, u_k\in V_k\).上式经整理可得\(v_j - u_i = \sum_{k\neq i, j}(u_k - v_k)\in \bigoplus_{k\neq i, j}V_k\).又\(v_j - u_i\in V_i\oplus V_j\).故\(v_j - u_i\in (V_i\oplus V_j)\cap \bigoplus_{k\neq i, j}V_k\).而由于\(V = \bigoplus_{k = 1}^m V_k\),因此\((V_i\oplus V_j)\cap \bigoplus_{k\neq i, j}V_k = \mathbb{0}\).故\(v_j - u_i = \mathbb{0}\),从而\(v_j = u_i\in V_i\cap V_j\).又因为\(V_i\oplus V_j\),所以\(V_i\cap V_j = \mathbb{0}\).故\(v_j = u_i = \mathbb{0}\).再由\(i, j\)的任意性可知,\(v_i = \mathbb{0}, i = 1, 2, \cdots, m\).因此\(\alpha = \sum_{k\neq i}v_k = \mathbb{0}\).故\(\mathrm{Ker}\varphi_1\cap \mathrm{Ker}\varphi_2\cap \cdots \cap \mathrm{Ker}\varphi_m = \mathbb{0}\).
\end{enumerate}
