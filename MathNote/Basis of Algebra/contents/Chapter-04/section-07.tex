...
\]
其中\(v_k, u_k\in V_k\).上式经整理可得\(v_j - u_i = \sum_{k\neq i, j}(u_k - v_k)\in \bigoplus_{k\neq i, j}V_k\).又\(v_j - u_i\in V_i\oplus V_j\).故\(v_j - u_i\in (V_i\oplus V_j)\cap \bigoplus_{k\neq i, j}V_k\).而由于\(V = \bigoplus_{k = 1}^m V_k\),因此\((V_i\oplus V_j)\cap \bigoplus_{k\neq i, j}V_k = 0\).故\(v_j - u_i = 0\),从而\(v_j = u_i\in V_i\cap V_j\).又因为\(V_i\oplus V_j\),所以\(V_i\cap V_j = 0\).故\(v_j = u_i = 0\).再由\(i, j\)的任意性可知,\(v_i = 0, i = 1, 2, \cdots, m\).因此\(\alpha = \sum_{k\neq i}v_k = 0\).故\(\mathrm{Ker}\varphi_1\cap \mathrm{Ker}\varphi_2\cap \cdots \cap \mathrm{Ker}\varphi_m = 0\).
\end{enumerate}
...
\end{proposition}
\begin{note}
注意到$\varphi$适合$x^2-x=x(x-1)$,从而由互素多项式诱导直和分解也可得到证明.
\end{note}
\begin{note}
...
\end{proposition}
\begin{proof}
注意到$\boldsymbol{A}$的极小多项式整除$x^2-x$,因此$\boldsymbol{A}$的极小多项式无重根,故$\boldsymbol{A}$可对角化且特征值只有1和0.因而(1)成立.
