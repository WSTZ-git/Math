...
\end{proposition}
\begin{proof}
因为\(\varphi_i\neq 0\),所以\(\text{Ker}\varphi_i\)是\(V\)的真子空间. 由\hyperref[theorem:覆盖定理]{覆盖定理}可知,有限个真子空间\(\text{Ker}\varphi_i\)不能覆盖全空间\(V\),故必存在\(\boldsymbol{\alpha}\in V\),使得\(\boldsymbol{\alpha}\)不属于任意一个\(\text{Ker}\varphi_i\),从而结论得证.
\end{proof}
...
\end{proof}
\begin{remark}
也可不证明\(\text{Im}\varphi^m\cap\text{Ker}\varphi^m = 0\),改由\hyperref[theorem:线性映射的维数公式]{线性映射维数公式}\(\dim\text{Im}\varphi^m+\dim\text{Ker}\varphi^m=n\)直接得到\(V=\text{Im}\varphi^m\oplus\text{Ker}\varphi^m\).
\end{remark}

\begin{proposition}[经典结论]\label{proposition:像空间和核空间的直和分解}
...
(8) \(\text{Ker}\varphi\)存在\(\varphi\)-不变补空间,即存在\(\varphi\)-不变子空间\(U\),使得\(V=\text{Ker}\varphi\oplus U\)且$U = \text{Im}\varphi$);

(9) \(\text{Im}\varphi\)存在\(\varphi\)-不变补空间,即存在\(\varphi\)-不变子空间\(W\),使得\(V=\text{Im}\varphi\oplus W\)且$W = \text{Ker}\varphi$.
\end{proposition}
\begin{note}
上述结论还等价于$\varphi$的0特征值的几何重数都是1.
\end{note}
\begin{proof}
...
\]
而由\hyperref[theorem:线性映射的维数公式]{线性映射维数公式},可知$\mathrm{dim}U=\mathrm{dim}\,\mathrm{Ker}\varphi |_U+\mathrm{dim}\varphi (U)$.进而\(\dim\varphi(U)\leqslant \dim U\).
